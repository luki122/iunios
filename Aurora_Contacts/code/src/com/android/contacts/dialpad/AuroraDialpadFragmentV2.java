...
import com.android.contacts.widget.AuroraDialpadAdditionalButtonsV2;
import com.android.contacts.widget.PathInterpolator;
...
import com.gionee.android.contacts.GnAutoScaleTextSizeWatcher;
...
import com.android.contacts.activities.DialButtonFAB;
/**
...
OnScrollListener, OnDialerSearchResult, OnDialerSearchListener, OnTouchListener {


	private static final String TAG = "liyang-AuroraDialpadFragmentV2";
	...
	public AuroraEditText mDigits;

	private View mDelete;
	//    private ToneGenerator mToneGenerator;
	//    private Object mToneGeneratorLock = new Object();
	private View mDigitKeyboard;
	private View digits_area;
	private View digits2;
	private View mYellowPages;

	private View mSingleDialLine, mDoubleDialLine,dial_button_layout;
	...
	private TextView aurora_dialButton_1_text;
	...
	private final int[] textViewsId=new int[]{
			R.id.num_area1,
			R.id.num_area2,
			R.id.num_area3,
			R.id.num_area4,
	...
	//	private LinearLayout digits_ll;

	public LinearLayout dialsearchfooter;
	...
	// gionee xuhz 20121012 modify for CR00711094 start
	//	private boolean mIsGnGuestMode = false;
	// gionee xuhz 20121012 modify for CR00711094 end
	...
	private int mNumAreaIsVisibleSaved=0;//0-不可见 ；1－可见
	public View footer_add_contact;
	private View footer_send_sms;
	//	public static View listview_footer;

	private void performBackgroundTask(int what, Object msgObj){
		switch(what){
		case DIALPAD_GET_AREA:
			final String number = (String) msgObj;
			final String area = NumberAreaUtil.getInstance(getActivity()).getNumAreaFromLocal(getActivity(), number, false);
			Log.d(TAG,"area:"+area);
				...
				getActivity().runOnUiThread(new Runnable() {
					...
					public void run() {
						if (mDigits.getText().toString().equals(number)) {
							...
							boolean currentVisible=mNumArea.getVisibility()==View.VISIBLE;						
							boolean areaShow=!TextUtils.isEmpty(area);

							if(!currentVisible && areaShow){//mNumArea由不可见变为可见，mDigit向下移动
								//								mNumArea.setVisibility(View.VISIBLE);
								AnimUtils.fadeIn(mNumArea, 300,AnimUtils.CURVE_SHOW);
								//								Log.d(TAG,"move down:"+distance);
								//								digits_ll.scrollBy(0, -distance);
							}else if(currentVisible && !areaShow){//mNumArea由可见变为不可见，mDigit向上移动
								//								mNumArea.setVisibility(View.INVISIBLE);
								AnimUtils.fadeOut(mNumArea, 300, AnimUtils.CURVE_HIDE,View.INVISIBLE);
								//								Log.d(TAG,"move up:"+distance);
								//								digits_ll.scrollBy(0, distance);
							}

							//							int mNumAreaIsVisibleCur=TextUtils.isEmpty(area) ?0:1;
							//							if(mNumAreaIsVisibleSaved==0&&mNumAreaIsVisibleCur==1){//mNumArea由不可见变为可见，mDigit向下移动
							//								mNumArea.setVisibility(TextUtils.isEmpty(area) ? View.GONE : View.VISIBLE);
							//								Log.d(TAG,"move down:"+distance);
							//								digits_ll.scrollBy(0, -distance);
							//							}else if(mNumAreaIsVisibleSaved==1&&mNumAreaIsVisibleCur==0){//mNumArea由可见变为不可见，mDigit向上移动
							//								mNumArea.setVisibility(TextUtils.isEmpty(area) ? View.GONE : View.VISIBLE);
							//								Log.d(TAG,"move up:"+distance);
							//								digits_ll.scrollBy(0, distance);
							//							}    			            
							//							mNumAreaIsVisibleSaved=mNumAreaIsVisibleCur;
	...
	public void afterTextChanged(Editable input) {
		//		if(true) return;
		Log.d(TAG, "afterTextChanged0,mDigitsPreviousText:"+mDigitsPreviousText+" input.toString():"+input.toString());
		...
		Log.d(TAG, "afterTextChanged01");
		mDigitsPreviousText = input.toString();
		Log.d(TAG, "mDigitsPreviousText:"+mDigitsPreviousText);
		...
		if (null != mNumArea && digitsLen > 1) {
			...
			Log.d(TAG, "afterTextChanged3");
			//			String area = NumberAreaUtil.getInstance(getActivity()).getNumAreaFromAora(getActivity(), mDigits.getText().toString(), false);

			String area=null;
			if(area != null){
				Log.d(TAG,"if area != null,"+area);
				if (!mNumArea.getText().toString().equals(area)) {
					//					mNumArea.setVisibility(TextUtils.isEmpty(area) ? View.INVISIBLE : View.VISIBLE);            
					mNumArea.setText(area);
			...
			AnimUtils.fadeOut(mNumArea, 300, AnimUtils.CURVE_HIDE,View.INVISIBLE);
			if(mNumAreaIsVisibleSaved==1){
				mNumAreaIsVisibleSaved=0;
			}
		}
		...
		//			dialsearchfooter.setVisibility(View.GONE);
		//		}else{
		//			dialsearchfooter.setVisibility(View.VISIBLE);
		//		}
	}

	private InitSoundTask initSoundTask;
	@Override
	public void onCreate(Bundle state) {
		Log.d(TAG,"onCreate:"+this.getId());
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context arg0, Intent intent) {
			...
			if(ContactsFeatureConstants.ACTION_VOICE_CALL_DEFAULT_SIM_CHANGED.equals(action)) {
				if(FeatureOption.MTK_GEMINI_SUPPORT || isMultiSimEnabled) {
	...
	void setSimIndicatorVisibility(boolean visible) {
		if(mStatusBarMgr == null)
			mStatusBarMgr = (StatusBarManager) getActivity().getSystemService(Context.STATUS_BAR_SERVICE);
		if (visible)
			GnStatusBarManager.showSIMIndicator(mStatusBarMgr, getActivity().getComponentName(), ContactsFeatureConstants.VOICE_CALL_SIM_SETTING);
		else
			GnStatusBarManager.hideSIMIndicator(mStatusBarMgr, getActivity().getComponentName());
	}
	...
	public void onDestroyView() {
		...
		if(isMultiSimEnabled) {
			...
			getActivity().getContentResolver().unregisterContentObserver(mSimInfoObserver);
	...
	public void onDestroy() {
		Log.d(TAG, "onDestroy:"+this.getId());
		if(mBackgroundHandler != null){
	...
	int subscription=1;
	int slotId=1;
	private ImageView aurora_dialButton_0_bgimage,aurora_dialButton_1_bgimage;
	//	public View main_keyboard;
	public View aurora_dialButton_image;
	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedState) {
		Log.v(TAG, "AuroraDialpadFragmentV2 onCreateView1 time="+System.currentTimeMillis());
		
		fragmentView = inflater.inflate(R.layout.aurora_dialpad_fragment_v2, container, false);		
		dialsearchfooter = (LinearLayout) inflater.inflate(R.layout.dialsearchfooter, null);
		return fragmentView;
	}

	boolean isInitYellowPages = false;
	...
	private void create(){
		mContext=ContactsApplication.getInstance().getApplicationContext();
		...
		initSoundTask=new InitSoundTask();
		if(mIsUseSoundPool) {
			if(initSoundTask!=null) initSoundTask.execute(-1);
		}

		Log.v(TAG, "AuroraDialpadFragmentV2 onCreate1 time="+System.currentTimeMillis());  
		...
		if(getActivity() instanceof AuroraActivity){
			...
			auroraActivity.setAuroraSystemMenuCallBack(auroraMenuCallBack);
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		Log.v(TAG, "AuroraDialpadFragmentV2 onCreateView11 time="+System.currentTimeMillis());  
		mLaunch = true;


		// Load up the resources for the text field.
		Resources r = getResources();
		isMultiSimEnabled=ContactsApplication.isMultiSimEnabled;
		mDigits = (AuroraEditText) fragmentView.findViewById(R.id.digits);

		//		main_keyboard= fragmentView.findViewById(R.id.main_keyboard);
		...
		//		digits_ll=(LinearLayout)fragmentView.findViewById(R.id.digits_ll);
		...
		if(ContactsApplication.sDialerSearchSupport) {
			...
			if(mListView != null) {
				...
				//				if(resultLayout==null) resultLayout=fragmentView.findViewById(R.id.result_layout);
				mDialerSearchController = new AuroraDialerSearchController(getActivity(), mListView, mCallOptionHandler,
						resultLayout,this);
				...
				mListView.addFooterView(dialsearchfooter,null,false);
				mListView.setFooterDividersEnabled(false);
				dialsearchfooter.setVisibility(View.GONE);
				footer_add_contact=dialsearchfooter.findViewById(R.id.footer_add_contact);
				footer_send_sms=dialsearchfooter.findViewById(R.id.footer_send_sms);
				footer_add_contact.setOnClickListener(this);
				//				listview_footer=dialsearchfooter.findViewById(R.id.listview_footer);
				footer_send_sms.setOnClickListener(this);
		...
		//		StateListAnimator animation = AnimatorInflater.loadStateListAnimator(mContext, com.aurora.R.anim.float_button_state_list_anim);
		//		mDialButton.setStateListAnimator(animation);
		//		mDialButton0.setStateListAnimator(animation);
		//		mDialButton1.setStateListAnimator(animation);

		aurora_dialButton_image=fragmentView.findViewById(R.id.aurora_dialButton_image);   
		aurora_dialButton_0_bgimage=(ImageView)fragmentView.findViewById(R.id.aurora_dialButton_0_bgimage);
		aurora_dialButton_1_bgimage=(ImageView)fragmentView.findViewById(R.id.aurora_dialButton_1_bgimage);
		...
		digits_area=fragmentView.findViewById(R.id.digits_area);
		digits_area.setOnTouchListener(this);

		digits2=fragmentView.findViewById(R.id.digits2);
		mDigitKeyboard.setOnTouchListener(this);
		...
		//		});
		//Gionee <huangzy> <2013-04-13> add for CR00786343 end
		Log.v(TAG, "AuroraDialpadFragmentV2 onCreateView2 time="+System.currentTimeMillis());   
		...
		mSingleDialLine =fragmentView.findViewById(R.id.single_dial_line);
		mSingleDialLine.setOnTouchListener(this);
		mDoubleDialLine = fragmentView.findViewById(R.id.msim_dial_line);
		mDoubleDialLine.setOnTouchListener(this);

		aurora_dialButton_1_text=(TextView)fragmentView.findViewById(R.id.aurora_dialButton_1_text);

		//		dial_button_layout= fragmentView.findViewById(R.id.dial_button_layout);
		//		dial_button_layout.setOnTouchListener(this);
		//		mDialpadBg =fragmentView.findViewById(R.id.aurora_dialpad_bg);
		if(isMultiSimEnabled) {
			...
			getActivity().getContentResolver().registerContentObserver(GnTelephony.SimInfo.CONTENT_URI, true, mSimInfoObserver);	
		...
		//          	RelativeLayout mNumLayout = (RelativeLayout)fragmentView.findViewById(R.id.digits_area);     	
		...
		try {
			...
			for(int i:textViewsId){
				((TextView)fragmentView.findViewById(i)).setTypeface(typeFace);
			}
		} catch (Exception e) {
		...
		if (Intent.ACTION_DIAL.equals(action) || Intent.ACTION_VIEW.equals(action)) {
			...
			if (uri != null) {
				...
				if (null != mDialerSearchController) {
					...
					Log.i(TAG, "mDialerSearchController.onResume()");
				}
	...
	public void configureScreenFromIntent(Intent intent) {
		Log.d(TAG,"intent:"+intent.getAction()+" intent:"+intent.getData());
		if (!isLayoutReady()) {
	...
	public void onResume() {
		...
		Log.v(TAG, "AuroraDialpadFragmentV2 onResume1 time="+System.currentTimeMillis());
		/*if(getActivity() instanceof AuroraActivity){
			if(Build.VERSION.SDK_INT == 19
					|| Build.VERSION.SDK_INT >= 21) {
		...
		Log.i(TAG, "GnDialpadV2 onResume");
		...
		// gionee xuhz 20121012 modify for CR00711094 start
		//		mIsGnGuestMode = Settings.Secure.getInt(
		...
		/*if(mIAuroraDialpadFragment != null){
			...
			if (isDialIntent) {
				...
				Log.d(TAG,"parent.getIntent():"+parent.getIntent());
		...
		if(ContactsApplication.sIsGnDialerSearchSupport) {
			//Gionee <huangzy><2013-06-03> modify for CR00821783 begin
			/*if(mDialerSearchController != null && !mLaunch) {
                mDialerSearchController.onResume();
                Log.i(TAG, "mDialerSearchController.onResume()");
            }
            // qc begin
            else if (mDialerSearchController != null && GNContactsUtils.isOnlyQcContactsSupport()) {
                mDialerSearchController.onResume();
                Log.i(TAG, "mDialerSearchController.onResume()");
            }
            // qc end*/            
			if (null != mHandler) {
				mHandler.sendEmptyMessageDelayed(
						MSG_DIALER_SEARCH_CONTROLLER_ON_RESUME, 20);
			}
			//Gionee <huangzy><2013-06-03> modify for CR00821783 end
		}

		Log.d(TAG,"mfirstShow:"+mfirstShow);
		if(mfirstShow) { 
			...
			new Thread(phoneIsCdmaRunnable).start();
		}else{
			if(isMultiSimEnabled) {
				if(showDouble){
					AnimUtils.scaleIn(mDialButton0, 0, 0);
					AnimUtils.scaleIn(mDialButton1, 0, 0);
				}else{
					AnimUtils.scaleIn(mDialButton, 0/*FAB_SCALE_IN_DURATION*/, 0);
				}
			}
		...
		Log.v(TAG, "AuroraDialpadFragmentV2 onResume2 time="+System.currentTimeMillis()); 
		if (mListView != null) {
...
//		savedSelection=0;
		...
		new Thread(isShowDoubleButtonRunnable).start();
		new Thread(telephonyManagerListenRunnable).start();

	}

	private boolean oldIsShowDouble=false;
	...
	public void onStop() {
		...
		if (!ContactsUtils.getTopActivityName(getActivity()).contains("DialtactsActivity")) {
			//        	clearDigits();	//aurora changes zhouxiaobing 20130914
		}
...
//		if(savedSelection!=0){
//
//			Log.d(TAG,"savedselec011:"+savedSelection);
//
//			if(savedSelection!=0){
//				CharSequence text = mDigits.getText();
//				if(text.length()>=savedSelection){
//				((DigitsEditText)mDigits).setSelection(savedSelection);
//				}else{
//					
//				}
//
...
//				//				Log.d(TAG,"Spannable text:"+spanText);
//				//				Selection.setSelection(spanText, text.length());
//				//				savedSelection=((DigitsEditText) mDigits).getSelectionStart();
//				//				Log.d(TAG,"savedselec1:"+savedSelection);
//				//			}
//				savedSelection=0;
//			}
//		}
		...
		if(clickFlag){
			Log.d(TAG, "clickFlag=true");
			((DigitsEditText)mDigits).setSelection(mDigits.getText().length());
			clickFlag=false;
		}
		...
		//		int savedSelection=((DigitsEditText) mDigits).getSelectionStart();
		//		mDigits.setVisibility(View.VISIBLE);
		mDigits.onKeyDown(keyCode, event);

		Log.d(TAG,"text:"+mDigits.getText().toString());
	...
	private boolean isSoundThreadRunning=false;
	//	class SoundThread extends Thread{
	//		int viewId,currentVol;
	//		public SoundThread(int viewId){
	//			Log.d(TAG, "SoundThread construct");
	//			this.viewId=viewId;
	//			if (!mDTMFToneEnabled) {
	...
	//		@Override
	//		public void run() {
	//			isSoundThreadRunning=true;
	//			Log.d(TAG, "SoundThread run begin:"+System.currentTimeMillis());
	//			if (!mDTMFToneEnabled) {
	...
	//						currentVol, 1, 0, 1f);
	//			}
	//			Log.d(TAG, "SoundThread run end:"+System.currentTimeMillis());
	...
	public void onClick(View view) {
		...
		case R.id.aurora_dialButton: {
			Log.d(TAG,"click aurora_dialButton");

			vibrate();  // Vibrate here too, just like we do for the regular keys
			Profiler.trace(Profiler.DialpadFragmentEnterClick);
			if(isMultiSimEnabled /*&& ContactsUtils.isShowDoubleButton()*/) {//aurora change zhouxiaobing 20140514
				if(showDouble){	
					...
					if(slotId==1){
						...
						GNdialButtonPressed(view.getId() == R.id.aurora_dialButton_1 ? 0 : 1);
					}else if(slotId==-1){
						GNdialButtonPressed(view.getId() == R.id.aurora_dialButton_1 ? 1 : 0);
					}
				}
				else
				{
					...
					//                	 {
					//                		 GNdialButtonPressed(1);
					//                	 }
					//                	 else
					//                	 {
					//                		 GNdialButtonPressed(0);
					//                	 }
					int sub = 0;
					try {
		...
		//			vibrate();
		//			GNdialButtonPressed(ContactsFeatureConstants.GEMINI_SIM_2);
		//			return;
		//		}				
		//GIONEE:liuying 2012-7-5 modify for CR00637517 end								  
		//		case R.id.digits: {
		...
		case R.id.footer_send_sms:{
			Log.d(TAG,"click footer_send_sms");
			Intent intent = IntentFactory.newCreateSmsIntent(mDigits.getText().toString());
			try {
	...
	public void callVoicemail() {
		...
		if (ContactsApplication.sIsGnDualSimSelectSupport) {
			Uri callUri =  Uri.fromParts("voicemail", EMPTY_NUMBER, null);
			Intent intent = new Intent("com.android.contacts.action.GNSELECTSIM");
			intent.putExtra("callUri", callUri.toString());
			startActivity(intent);
			return;
		} else {
			// Gionee:wangth 20121121 add for CR00734624 begin
			if (ContactsUtils.mIsGnContactsSupport && ContactsUtils.mIsSuperSDCardVersionSupport) {
				startActivity(newVoicemailIntent());
				return;
			}
			// Gionee:wangth 20121121 add for CR00734624 end
		}
	...
	private void returnToInCallScreen(boolean showDialpad) {
		...
		if (GNContactsUtils.isOnlyQcContactsSupport() && isMultiSimEnabled) {
			final boolean isVTActive = GnTelephonyManager.getDefault().isVTCallActive(); 
			if(isVTActive) {
				startActivity(GNContactsUtils.startQcVideoCallIntent(getDigitsText()));
			}

			GnTelephonyManager.getDefault().showCallScreenWithDialpad(showDialpad);
...
Log.d(TAG, "updateDialString");
	...
	private void updateDialAndDeleteButtonEnabledState() {
		...
		if (mDialButton != null) {
			...
			if (ContactsApplication.phoneIsCdma && ContactsUtils.phoneIsOffhook()) {
		...
		//        mDelete.setAlpha(digitsNotEmpty?255:125);
		//		mDeleteMsim.setEnabled(digitsNotEmpty);
		//        mDeleteMsim.setAlpha(digitsNotEmpty?255:125);
	}
	...
	public void queryLastOutgoingCall() {
		Log.d(TAG, "queryLastOutgoingCall");
				...
				new CallLogAsync.GetLastOutgoingCallArgs(
						...
						new CallLogAsync.OnLastOutgoingCallComplete() {
							public void lastOutgoingCall(String number) {
								...
								Log.d(TAG, "mLastNumberDialed:"+mLastNumberDialed);
	...
	protected void dialButtonPressedInner(String number, int type) {
		...
		if (TextUtils.isEmpty(number)) { // No number entered.
			if (ContactsApplication.phoneIsCdma && ContactsUtils.phoneIsOffhook()) {
				...
				// the redial feature.
				if(showDouble){
					scaleOut(mDialButton0,250,View.INVISIBLE);
					scaleOut(mDialButton1,250,View.INVISIBLE);
				}else{
					scaleOut(mDialButton,250,View.INVISIBLE);
				}
			...
			if (number != null
				...
				// gionee xuhz 20121215 modify for Dual Sim Select start
				Intent intent;
				if (ContactsApplication.sIsGnDualSimSelectSupport && type != 2) {
					int[] location = new int[2] ;
					mDialButton.getLocationOnScreen(location);
					...
					// Gionee:wangth 20121119 add for CR00732636 begin
					if (ContactsUtils.mIsGnContactsSupport && ContactsUtils.mIsSuperSDCardVersionSupport && intent != null) {
						if (type == 0) { // voice call
							startActivity(intent);
						} else if (type == 2) { // video call
					...
					// Gionee:wangth 20121119 add for CR00732636 end
				}
				// gionee xuhz 20121215 modify for Dual Sim Select end
...
//			}
			...
			if (number != null
				...
				final Intent intent = AuroraTelephoneManager.getCallNumberIntent(number,simId,type);//IntentFactory.newDialNumberIntent(number, type);//aurora change zhouxiaobing 20140513
//				if (getActivity() instanceof AuroraDialtactsActivityV2) {
//					intent.putExtra(AuroraDialtactsActivityV2.EXTRA_CALL_ORIGIN,
//							AuroraDialtactsActivityV2.CALL_ORIGIN_DIALTACTS);
//				}
			    ActivityManager am = (ActivityManager) ContactsApplication.getInstance().getSystemService(Context.ACTIVITY_SERVICE); 	
	...
	public void setVisible(boolean visible) {
		if(!showDouble){
			mSingleDialLine.setVisibility(visible ? View.VISIBLE : View.GONE);
		}else{
			mDoubleDialLine.setVisibility(visible ? View.VISIBLE : View.GONE);
		}
	}
	...
	public void scaleIn(int delayMs) {
		setVisible(true);
		if(!showDouble){
			AnimUtils.scaleIn(mDialButton, FAB_SCALE_IN_DURATION, delayMs);
			AnimUtils.fadeIn(aurora_dialButton_image, FAB_SCALE_IN_DURATION,
					delayMs + FAB_SCALE_IN_FADE_IN_DELAY, null,null);
		}else{
			AnimUtils.scaleIn(mDialButton0, FAB_SCALE_IN_DURATION, delayMs);
			AnimUtils.fadeIn(aurora_dialButton_0_bgimage, FAB_SCALE_IN_DURATION,
					delayMs + FAB_SCALE_IN_FADE_IN_DELAY, null,null);

			AnimUtils.scaleIn(mDialButton1, FAB_SCALE_IN_DURATION, delayMs);
			AnimUtils.fadeIn(aurora_dialButton_1_bgimage, FAB_SCALE_IN_DURATION,
					delayMs + FAB_SCALE_IN_FADE_IN_DELAY, null,null);
		}
	}
	...
	public void scaleIn(View view,int delayMs) {
	...
	public void scaleOut() {
		Log.d(TAG,"scaleOut");
		if(!showDouble){
			AnimUtils.scaleOut(mDialButton, 250);
			...
			AnimUtils.fadeOut(aurora_dialButton_image, FAB_ICON_FADE_OUT_DURATION, null,View.GONE);
		}else{
			AnimUtils.scaleOut(mDialButton0, 250);
			// Fade out the icon faster than the scale out animation, so that the icon scaling is less
			// obvious. We don't want it to scale, but the resizing the container is not as performant.
			AnimUtils.fadeOut(aurora_dialButton_0_bgimage, FAB_ICON_FADE_OUT_DURATION, null,View.GONE);

			AnimUtils.scaleOut(mDialButton1, 250);
			// Fade out the icon faster than the scale out animation, so that the icon scaling is less
			// obvious. We don't want it to scale, but the resizing the container is not as performant.
			AnimUtils.fadeOut(aurora_dialButton_1_bgimage, FAB_ICON_FADE_OUT_DURATION, null,View.GONE);
		}
	}

	public void scaleOut(View view, int durationMs,int visible) {
		Log.d(TAG,"scaleOut,view:"+view+" durationMs:"+durationMs);
		AnimUtils.scaleOut(view,durationMs,visible);
		// Fade out the icon faster than the scale out animation, so that the icon scaling is less
		// obvious. We don't want it to scale, but the resizing the container is not as performant.
		//		AnimUtils.fadeOut(AuroraDialActivity.fab1, FAB_ICON_FADE_OUT_DURATION, null);
	}
	...
	public void hiddenChange(boolean hidden){
		Log.d(TAG,"hiddenChange:"+hidden);
//		savedSelection=0;
		mDigits.setCursorVisible(false);
		if (activity == null) return;

		if(digits2!=null) digits2.setVisibility(View.VISIBLE);
		//		if(dial_button_layout!=null) dial_button_layout.setVisibility(View.VISIBLE);

		if(dialpadView==null){
			dialpadView = (DialpadView) fragmentView.findViewById(R.id.aurora_dial_digit_keyboard);
		}
		...
		if (!hidden && !isDialpadChooserVisible()) {
			...
			if (null != mSingleDialLine) {
				scaleIn(mAnimate ? 430 : 0);
			}
...
//		int savedSelection;
	...
	public void showDialpad(boolean show, boolean withAnim) {
		...
		if (!show) {
			...
			if (digits2.getVisibility() == View.GONE) {
				return;
			}

//						savedSelection=((DigitsEditText) mDigits).getSelectionStart();
			//			savedCursorVisibility=((DigitsEditText) mDigits).hasSelection();
			//			Log.d(TAG,"savedCursorVisibility0:"+savedCursorVisibility);
			//
			//			//			mDigits.clearFocus();
			//
//						Log.i(TAG, "savedSelection:"+savedSelection);


			Log.i(TAG, "showDialpad begin2");
			if (null == mKeySectionHideAnim) {
				animMoveY=TextUtils.isEmpty(mDigits.getText().toString())?
						animView.getHeight():
							(animView.getHeight()-digits_area.getHeight());

						Log.i(TAG, "showDialpad begin2.2："+animMoveY);
						...
						mKeySectionHideAnim.setAnimationListener(new AnimationListener() {
							...
							public void onAnimationEnd(Animation animation) {
								...
								digits2.setVisibility(visibility);
								mIsDialpadAniming = false;
								Log.i(TAG, "mIsDialpadAniming1:"+mIsDialpadAniming);
								//								dial_button_layout.setVisibility(visibility);
			...
			clickFlag=true;
			//			Log.i(TAG, "showDialpad begin3,savedSelection:"+savedSelection);

			//			mDigits.requestFocus();



			//			Log.d(TAG,"savedCursorVisibility:"+savedCursorVisibility);
			((DigitsEditText) mDigits).setCursorVisible(false);
			...
			if (digits2.getVisibility() == View.VISIBLE&&animView.getVisibility()==View.VISIBLE) {
				return;
			}
			Log.i(TAG, "showDialpad begin4:"+mKeySectionShowAnim);
			if (null == mKeySectionShowAnim) {
				Log.i(TAG, "showDialpad begin4.1");
				//				int animMoveY=0;
				//				if(digits_area.getVisibility()==View.VISIBLE){
				//					Log.d(TAG,"111,animView.getHeight():"+animView.getHeight()+" digits_area.getHeight():"+digits_area.getHeight());
				//					animMoveY=animView.getHeight()-digits_area.getHeight();
				//				}else if(digits_area.getVisibility()==View.GONE){
				//					Log.d(TAG,"222");
				//					animMoveY=animView.getHeight();
				//				}

				Log.i(TAG, "showDialpad begin5:"+animMoveY);
				mKeySectionShowAnim = new TranslateAnimation(0.0f, 0.0f, 800, 0.0f);   
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		Log.d(TAG,"KeyEvent = " + event.getKeyCode() + " keyCode " + keyCode);
		switch (keyCode) {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 3:
				updateCallButtonVisbilityAndWidth((msg.arg1==1)?true:false);
				break;
			}
		}
	};
	...
	public void setImageView()
	{
		...
		//	ivadd.setBackgroundResource(R.drawable.svg_dial_plus);
		...
		digits2.setVisibility(View.VISIBLE);
	...
	//	protected View mAddContactView, mAddContactViewMsim;
	...
	public void clearDigits() {
		Log.d(TAG,"clearDigits");
//		((AuroraDialerSearchController)mDialerSearchController).isShowFooter=false;
		...
		if(mNumAreaIsVisibleSaved==1){
			//			mNumArea.setVisibility(View.INVISIBLE);
			AnimUtils.fadeOut(mNumArea, 300, AnimUtils.CURVE_HIDE,View.INVISIBLE);
			//			Log.d(TAG,"clearDigits move up:"+distance);
			//			digits_ll.scrollBy(0, distance);
			mNumAreaIsVisibleSaved=0;
		}
	...
	private void setKeyboardClickListener() {
		if (null != mDigitKeyboard) {
			...
			if (ContactsApplication.sIsGnSpeedDialSupport) {
				Log.d(TAG,"sIsGnSpeedDialSupport");
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case R.id.digits:{
			Log.d(TAG,"ontouch digits_area");
			if(event.getAction() == MotionEvent.ACTION_DOWN){
				if(digits2.getVisibility()==View.GONE){
					Log.d(TAG,"touch digits_1");
					showDialpad(true, true);					
//					return true;
				}
				if(digits2.getVisibility()==View.VISIBLE/* && TextUtils.isEmpty(mDigits.getText().toString())*/){
					Log.d(TAG,"touch digits_2");
					mDigits.requestFocus();
					mDigits.setCursorVisible(true);
//					return true;
				}				
		...
		case R.id.digits_area:
			return true;	
		case android.R.id.list:{
			if(event.getAction() == MotionEvent.ACTION_UP){
				if(TextUtils.isEmpty(mDigits.getText().toString())){
					Log.d(TAG,"onTouch list_view1,getActivity():"+getActivity()+" getActivity().isFinishing():"+getActivity().isFinishing());
					if(getActivity()!=null&&!getActivity().isFinishing()){
						if(mCallbacks!=null) mCallbacks.onFragmentCallback(FragmentCallbacks.FINISH_DIALPAD,null);
						return true; 
					}
				}
			}
		...
		case R.id.msim_dial_line:
		case R.id.single_dial_line:{
			Log.d(TAG,"touch msim_dial_line or single_dial_line");
			return true;
		}
		...
		case R.id.aurora_deleteButton:{
			if(event.getAction() == MotionEvent.ACTION_DOWN){
				//				Log.d(TAG,"onTouch aurora_deleteButton");
				//				keyPressed(KeyEvent.KEYCODE_DEL);
				return false;
			}
		}
	...
	private AuroraDialActivity activity;
	@Override
	public void onAttach(Activity activity) {
		...
		if(activity instanceof AuroraDialActivity){
			this.activity=(AuroraDialActivity)activity;
		}
	...
	private void playMusic(final int viewId) {		
		...
		if (mIsUseSoundPool) {
			//			if(initSoundTask!=null) initSoundTask.execute(viewId);

			//			if(soundThread==null){
			//				soundThread=new SoundThread(viewId);
			//			}
			//
			//			if(!isSoundThreadRunning) {
			//				soundThread.start();
			//			}
...
//						soundPool.release();
	...
	private boolean showDouble,isMultiSimEnabled;
	private void updateCallButtonVisbilityAndWidth(boolean showDouble) {
		...
		Log.v(TAG, "updateCallButtonVisbilityAndWidth =showDouble:" + showDouble);   
		...
		if(showDouble){/*aurora_dialButton_0_bgimage*/
			//			int subscription=1;
			//
			//			try {
			//				subscription = Settings.Global.getInt(mContext.getContentResolver(),
			...
			try {
				subscription = Settings.Global.getInt(mContext.getContentResolver(),
						"multi_sim_voice_call");
				slotId=SubscriptionManager.getPhoneId(subscription);				
			...
			if(slotId!=1&&slotId!=0) slotId=0;
			String titleString=SIMInfoWrapper.getDefault().getSimDisplayNameBySlotId(slotId);
			aurora_dialButton_1_text.setText(titleString);
			Log.d(TAG,"subScription:"+subscription+" slotId:"+slotId+" titleString:"+titleString);



			if(slotId==0){
				aurora_dialButton_1_bgimage.setImageDrawable(mContext.getResources().getDrawable(R.drawable.svg_dial_dial1));
				aurora_dialButton_0_bgimage.setImageDrawable(mContext.getResources().getDrawable(R.drawable.svg_dial_dial2));

			}else{
				aurora_dialButton_1_bgimage.setImageDrawable(mContext.getResources().getDrawable(R.drawable.svg_dial_dial2));
				aurora_dialButton_0_bgimage.setImageDrawable(mContext.getResources().getDrawable(R.drawable.svg_dial_dial1));
			}
	...
	private Runnable isShowDoubleButtonRunnable = new Runnable() {  
		@Override  
		public void run() {     
			showDouble = ContactsUtils.isShowDoubleButton();
			ContactsApplication.isShowDouble=showDouble;
			Log.d(TAG, "ContactsApplication.isShowDouble:"+showDouble);
			Message message=mHandler.obtainMessage(3);
			message.arg1=showDouble?1:0;
			mHandler.sendMessage(message);
		}  
	}; 

	private Runnable phoneIsCdmaRunnable = new Runnable() {  
		@Override  
		public void run() {     
			ContactsApplication.phoneIsCdma = ContactsUtils.isShowDoubleButton();
			Log.d(TAG, "phoneIsCdma:"+ContactsApplication.phoneIsCdma);
		}  
	}; 
	
	private Runnable telephonyManagerListenRunnable = new Runnable() {  
		@Override  
		public void run() {     
...
//			mDialpadBg.startAnimation(out);
	...
	boolean mIsInitSound = false; 
	private class InitSoundTask extends AsyncTask<Integer, Void, Boolean> {
		int viewId,currentVol;
	...
	private ContentObserver mSimInfoObserver = new ContentObserver(new Handler()) {
		public void onChange(boolean selfChange){
			Log.i(TAG, "mSimInfoObserver onChange ::");
			super.onChange(selfChange);
			new Thread(isShowDoubleButtonRunnable).start();
		}
	};
