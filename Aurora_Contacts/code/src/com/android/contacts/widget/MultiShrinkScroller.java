...
public class MultiShrinkScroller extends FrameLayout {
     ...
     */
    private static final float INTERMEDIATE_HEADER_HEIGHT_RATIO = 0.56f;
    ...
    public interface MultiShrinkScrollerListener {
        ...
        void onStartEnterDimAnimator(int duration);
        
        void onStartExitDimAnimator(int duration);
    }
...
//        mIsTwoPanel = getResources().getBoolean(R.bool.quickcontact_two_panel);
        ...
        mActionBarSize = attributeArray.getDimensionPixelSize(0, 0);
       	mAuroraStatusBarHeight = getResources().getDimensionPixelOffset(com.aurora.R.dimen.status_bar_height);
//        mMinimumHeaderHeight = mActionBarSize;
        mMinimumHeaderHeight = mAuroraStatusBarHeight + mActionBarSize;
...
//        mEdgeGlowBottom.setColor((color & 0xffffff) | Color.argb(edgeEffectAlpha, 0, 0, 0));
    ...
    private boolean snapToTop(int flingDelta) {
        if (mHasEverTouchedTheTop) {
          ...
          int mDismissDistanceOnRelease = getResources().getDimensionPixelOffset( R.dimen.aurora_quick_scroll_top_margin) ;
...
//            }
    ...
    private final PathInterpolator mTransparentPathInterpolator= new PathInterpolator(0.71f, 0.12f, 0.75f, 0.56f);
    private float getTransparentHeightRatio(int transparentHeight) {
        ...
        float result = 1.0f - Math.max(Math.min(1.0f, heightRatio), 0f);
        result = (float)(mTransparentPathInterpolator.getInterpolation(result) * 0.6f);
        return result;
    }
    
    private float getTransparentHeightRatioOrigin(int transparentHeight) {
        final float heightRatio = (float) transparentHeight / getHeight();
        // Clamp between [0, 1] in case this is called before height is initialized.
...
//	    Animation mSlideOut = AnimationUtils.loadAnimation(getContext(), com.aurora.R.anim.aurora_activity_close_exit);
...
//        this.startAnimation(mSlideOut);
    }

    public void scrollOffBottomAsActivity() {
      ...
      translateAnimation.start();      
      if (mListener != null) {
    	  mListener.onStartExitDimAnimator(EXIT_FLING_ANIMATION_DURATION_MS);
          mListener.onStartScrollOffBottom();
      }
  }
    /**
...
//                android.R.interpolator.linear_out_slow_in);
        ...
        animator.setDuration(ENTER_ANIMATION_DURATION_MS);
        
        final ObjectAnimator alphaAnimator = ObjectAnimator.ofFloat(this, "alpha", 0,
                1);
        alphaAnimator.setInterpolator(interpolator);
        alphaAnimator.setDuration(ENTER_ANIMATION_DURATION_MS);        
		...
		bouncer.playTogether(animator, alphaAnimator);
//		bouncer.start();
        animator.start();
        // Aurora xuyong 2016-02-27 modified for bug #19741 start
        if (mListener != null) {
            mListener.onStartEnterDimAnimator(ENTER_ANIMATION_DURATION_MS);
        }
        // Aurora xuyong 2016-02-27 modified for bug #19741 end
//        Animation mSlideIn = AnimationUtils.loadAnimation(getContext(), com.aurora.R.anim.aurora_activity_open_enter);
...
//       	this.startAnimation(mSlideIn);
    ...
    private void scrollUp(int delta) {
        ...
        final FrameLayout.LayoutParams realToolbarLayoutParams = (FrameLayout.LayoutParams)mRealToolbar.getLayoutParams();
   
        realToolbarLayoutParams.topMargin = getToolbarAnimMarginTop();
        mRealToolbar.setLayoutParams(realToolbarLayoutParams);
    ...
    private void scrollDown(int delta) {
        ...
        final LinearLayout.LayoutParams toolbarLayoutParams = (LinearLayout.LayoutParams)mToolbar.getLayoutParams();
...
//        }
        ...
        final float intermediateRatio = calculateHeightRatio((int)
                (mMaximumPortraitHeaderHeight * INTERMEDIATE_HEADER_HEIGHT_RATIO * 2));
        if (!mIsTwoPanel) {
...
//        mBlurPhotoView.setAlpha(colorAlpha);
//        mSmallPhotoView.setAlpha(1 - colorAlpha);
...
//                    + "pointless and harmful");
    ...
    private int mAuroraStatusBarHeight; 
    private View mRealToolbar;
    private int getToolbarAnimMarginTop() {
        int animMargin = 0;
//        float animMarginRatio = getTransparentHeightRatioOrigin(getTransparentViewHeight());
        float animMarginRatio = (float) getTransparentViewHeight() / getHeight();
        if(animMarginRatio < 0.3f) {
        	double ratio = 1- animMarginRatio/0.3f ;
        	if(ratio > 1) ratio = 1;
        	if(ratio < 0) ratio = 0;
//        	int totalMargin = mAuroraStatusBarHeight + getResources().getDimensionPixelOffset( R.dimen.aurora_quick_toolbar_anim_margin);
        	int totalMargin= getResources().getDimensionPixelOffset( R.dimen.aurora_quick_toolbar_margin_top);
        	animMargin =(int)( (mAuroraStatusBarHeight - totalMargin) * ratio);
        }
//        return getTransparentViewHeight() < mAuroraStatusBarHeight ? (mAuroraStatusBarHeight - getTransparentViewHeight() + animMargin) : animMargin;
        return animMargin;
    } 
}
