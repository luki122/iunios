...
public class ImportVCardActivity extends ContactsActivity {
	private static final String LOG_TAG = "liyang-VCardImport";
	...
	private AuroraProgressDialog mProgressDialogForScanVCard;
	private AuroraProgressDialog mProgressDialogForCachingVCard;
	...
	//aurora add zhouxiaobing 20140107 start
	private boolean isSdCard2=false;
	private Button dialog_positive_button;
	private AuroraAlertDialog adialog;
	//aurora add zhouxiaobing 20140107  end   
	...
	private class DialogDisplayer implements Runnable {
		...
		public void run() {
			if (!isFinishing()) {
				showDialog(mResId);
				//aurora add zhouxiaobing 20140107 start
				if(mResId==R.id.dialog_select_multiple_vcard)
				{
					dialog_positive_button=adialog.getButton(DialogInterface.BUTTON_POSITIVE);
					dialog_positive_button.setEnabled(false);
				}
				// dialog_positive_button.setClickable(false);
				//aurora add zhouxiaobing 20140107 end
			}
		}
	}
	...
	implements DialogInterface.OnCancelListener {
		...
		public void run() {
			...
			try {
				...
				//  mProgressDialogForCachingVCard.dismiss();//aurora change zhouxiaobing 20131205
	...
	DialogInterface.OnClickListener {
		...
		public void onClick(DialogInterface dialog, int which) {
			if (which == DialogInterface.BUTTON_POSITIVE) {
				 ...
				 *   CR ID: ALPS00110214
				 */
				 //setResult(AuroraContactImportExportActivity.RESULT_CODE); 
		...
		public void onClick(DialogInterface dialog, int which) {
			if (which == DialogInterface.BUTTON_POSITIVE) {
				if (mSelectedIndexSet != null) {
					if(mSelectedIndexSet.size()>0){	//aurora add zhouxiaobing 20131213
				...
				if (mSelectedIndexSet != null) {
					...
					{
						dialog_positive_button.setEnabled(true);
					}
					//aurora add zhouxiaobing 20140107 end        
				}
	...
	private void startVCardSelectAndImport() {
		...
		if (getResources().getBoolean(R.bool.config_import_all_vcard_from_sdcard_automatically) ||
			 ...
			 *   CR ID: ALPS00120656
			 */                       
			// aurora ukiliu 2013-10-21 modify for BUG #24 begin
			if (size == 1) {            
				//setResult(AuroraContactImportExportActivity.RESULT_CODE);
				setResult(Activity.RESULT_OK);//aurora change zhouxiaobing 20140408
				Log.i(LOG_TAG, "Starting one vCard import using Uri ");
				runOnUiThread(new DialogDisplayer(R.id.dialog_import_confirm));
			...
			// aurora ukiliu 2013-10-21 modify for BUG #24 end
			// aurora ukiliu 2013-11-12 modify for BUG #650 begin
			...
			// aurora ukiliu 2013-11-12 modify for BUG #650 end
		}
	}
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (!ImportVCardActivity.this.isFinishing()) {
					//    showDialog(R.id.dialog_cache_vcard);//aurora change zhouxiaobing 20131205
					startVCardService();
				}
			}
		});
	...
	private Dialog getSelectImportTypeDialog() {
		...
		final AuroraAlertDialog.Builder builder = new AuroraAlertDialog.Builder(this/*, AlertDialog.THEME_AMIGO_FULLSCREEN*/)
	...
	private Dialog getVCardFileSelectDialog(boolean multipleSelect) {
		...
		final AuroraAlertDialog.Builder builder =
				new AuroraAlertDialog.Builder(this/*, AlertDialog.THEME_AMIGO_FULLSCREEN*/)
		.setTitle(R.string.select_vcard_title)
		.setPositiveButton(R.string.aurora_import_title, listener)
		.setOnCancelListener(mCancelListener)
		.setNegativeButton(android.R.string.cancel, mCancelListener);
		...
		for (int i = 0; i < size; i++) {
			...
			//                    new RelativeSizeSpan(1.0f), 0, indexToBeSpanned,
	...
	protected void onCreate(Bundle bundle) {
		// gionee xuhz 20121212 add for GIUI2.0 start
		if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
			if (ContactsApplication.sIsGnDarkStyle) {
				setTheme(R.style.GnImportVCardActivityDark);
			} else {
				setTheme(R.style.GnImportVCardActivityLight);
			}
		}
		// gionee xuhz 20121212 add for GIUI2.0 end
		...
		isSdCard2=intent.getBooleanExtra("isSdcard2", false);//aurora add zhouxiaobing 20131211
		if (!TextUtils.isEmpty(accountName) && !TextUtils.isEmpty(accountType)) {
			...
			if (accountList.size() == 0) {
				...
				Log.v("ImportVCardActivity", "size="+accountList.size());
				for(int i=0;i<accountList.size();i++)
				{
					Log.v("ImportVCardActivity", "type="+accountList.get(i).type);
					if(accountList.get(i).type.equals(AccountType.ACCOUNT_TYPE_LOCAL_PHONE))
					{
						mAccount=accountList.get(i);
						break;
					}
				}
				Log.v("ImportVCardActivity", "mAccount="+mAccount);
				if(mAccount==null)
				{
					Toast.makeText(this, R.string.aurora_cannot_importcontacts, Toast.LENGTH_SHORT);
					finish();
					return;
				}
				// mAccount = accountList.get(0);
				//aurora add zhouxiaobing 20140120 start
				if(intent!=null)//aurora add zhouxiaobing for jianrong disanfang yingyong
				{
					Log.v("ImportVCardActivity", "intent="+intent.toString());   
					Uri uri = intent.getData();
					if(uri!=null)
					{
						new AuroraAlertDialog.Builder(this)
						.setMessage(this.getResources().getString(R.string.aurora_daoru_confirm_gaddsa,uri.getLastPathSegment()))
						.setPositiveButton(R.string.aurora_import_title, new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog, int which) {
								startImport();
							}
						})
						...
						Log.v("ImportVCardActivity", "size1="+accountList.size());
						return;
					}

				}

				//aurora add zhouxiaobing 20140120 end     
			} /*else {
            	Log.v("ImportVCardActivity", "size2="+accountList.size());
                startActivityForResult(new Intent(this, SelectAccountActivity.class),
                        SELECT_ACCOUNT);
                return;
            }*///aurora modify zhouxiaobing 20140227
		}
	...
	protected Dialog onCreateDialog(int resId, Bundle bundle) {
		...
		// aurora ukiliu 2013-10-21 add for BUG #24 begin
		case R.id.dialog_import_confirm: {
			DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {

				@Override
				public void onClick(DialogInterface arg0, int arg1) {
					Log.i(LOG_TAG, "Starting a vCard import using Uri ");
					importVCardFromSDCard(mAllVCardFileList);
				}

			};

			String messageString = getString(R.string.aurora_import_confirm_message, mAllVCardFileList.get(0).mName);
		...
		// aurora ukiliu 2013-10-21 add for BUG #24 end
		case R.id.dialog_searching_vcard: {
			if (mProgressDialogForScanVCard == null) {
				String title = getString(R.string.searching_vcard_title);
				String message = isSdCard2?getString(R.string.searching_vcard2_message):getString(R.string.searching_vcard1_message);
				mProgressDialogForScanVCard =
						/*ProgressDialog.show(this, title, message, true, false);*/
						new AuroraProgressDialog(this, AuroraProgressDialog.THEME_AMIGO_FULLSCREEN);
				// mProgressDialogForScanVCard.setMessage(message);
				mProgressDialogForScanVCard.setTitle(message);
				mProgressDialogForScanVCard.setIndeterminate(true);
				mProgressDialogForScanVCard.setCancelable(false);
				mProgressDialogForScanVCard.setProgressStyle(ProgressDialog.STYLE_SPINNER);//aurora add zhouxiaobing 20131205
		...
		case R.id.dialog_vcard_not_found: {
			final String message = isSdCard2?getString(R.string.import_failure_no_vcard2_file):getString(R.string.import_failure_no_vcard_file);
			AuroraAlertDialog.Builder builder = new AuroraAlertDialog.Builder(this, AuroraAlertDialog.THEME_AMIGO_FULLSCREEN)
			.setTitle(isSdCard2?R.string.aurora_import_sd2_title:R.string.aurora_import_sd_title)
			.setMessage(message)
			.setOnCancelListener(mCancelListener)
			.setPositiveButton(android.R.string.ok, mCancelListener);
			return builder.create();
		...
		case R.id.dialog_cache_vcard: {
			if (mProgressDialogForCachingVCard == null) {
				...
				mProgressDialogForCachingVCard = new AuroraProgressDialog(this, 
	...
	private void doScanExternalStorageAndImportVCard() {
		...
		//                path = ContactImportExportActivity.mSdCard;
		//            } else {
		//                path = ContactImportExportActivity.mSdCard2;
		//            }
		//        } else {
		path = GnStorageManager.getDefaultPath();
		if(isSdCard2)
			path=GnStorageManager.getInstance(ContactsApplication.getInstance()).getExternalStoragePath();//aurora add zhouxiaobing 20131211
		//        }
		//Gionee:wangth 20120420 modify for CR00576392 end
	...
	public static final String BLUETOOTH_PBAP_IMPORT = 
			ContactsApplication.getInstance().getApplicationContext()
			.getString(R.string.bluetooth_pbap_importing_message);
	public static void importFromTempVcf(Activity activity, Uri uri, AccountWithDataSet account) {
		final Context context = ContactsApplication.getInstance().getApplicationContext();
		final ImportRequest request;
		ArrayList<ImportRequest> requests = new ArrayList<ImportRequest>();
		GnImportRequestConnection conn =  new GnImportRequestConnection(activity);

		Log.i(LOG_TAG, "Bind to VCardService.");
		// We don't want the service finishes itself just after this connection.
		Intent intent = new Intent(context, VCardService.class);
		context.startService(intent);
		context.bindService(new Intent(context, VCardService.class),
				conn, Context.BIND_AUTO_CREATE);

		request = gnConstructImportRequest(account, null, uri, BLUETOOTH_PBAP_IMPORT);
		requests.add(request);

		if (!requests.isEmpty()) {
			conn.sendImportRequest(requests);
		} else {
			Log.w(LOG_TAG, "Empty import requests. Ignore it.");
		}
	}

	private static class GnImportRequestConnection implements ServiceConnection {
		private static VCardService mService;
		private static Activity mActivity;

		public GnImportRequestConnection(Activity activity) {
		...
		public void sendImportRequest(final List<ImportRequest> requests) {
			...
			mActivity.runOnUiThread(new Runnable() {
				...
				public void run() {
					// TODO Auto-generated method stub
					mService.handleImportRequest(requests, new NotificationImportExportListener(mActivity));
				}
	...
	private static ImportRequest gnConstructImportRequest(AccountWithDataSet account, final byte[] data,
			final Uri localDataUri, final String displayName) {
		final Context context = ContactsApplication.getInstance().getApplicationContext();
		final ContentResolver resolver = context.getContentResolver();
		VCardEntryCounter counter = null;
		...
		return new ImportRequest(account,
				data, localDataUri, displayName,
				detector.getEstimatedType(),
				detector.getEstimatedCharset(),
				vcardVersion, counter.getCount());
	...
	// aurora ukiliu add for BUG #214 begin
	@Override
	...
	// aurora ukiliu add for BUG #214 end
}
