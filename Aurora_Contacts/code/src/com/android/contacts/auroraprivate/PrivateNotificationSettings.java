...
public class PrivateNotificationSettings extends AuroraPreferenceActivity implements AuroraPreference.OnPreferenceChangeListener {
    ...
    private int getChoice() {
        ...
        Cursor c = getContentResolver().query(GnContactsContract.RawContacts.CONTENT_URI, new String[] {"call_notification_type"}, "_id=" + mRawContactId + " and is_privacy > -1", null, null);
    ...
    public boolean onPreferenceChange(AuroraPreference preference, Object objValue) {
        ...
        for(int i = 0; i < 2; i++) {
            if(PrivateRadioPreference.getSelectedKey().equalsIgnoreCase(mRadio[i].getKey())) {
                ...
                getContentResolver().update(GnContactsContract.RawContacts.CONTENT_URI, values, "_id=" + mRawContactId + " and is_privacy > -1", null); 
                if(i == 0) {
	                for(int j = 0; j < 3; j++) {
