 ...
 */
public class QuickContactActivity extends AuroraActivity {
    ...
    private View mGnNoDataBottomView;
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (ContactsApplication.sIsGnDarkStyle) {
            setTheme(R.style.GnQuickContactBlack);
        } else if (ContactsApplication.sIsGnLightTheme) {
            setTheme(R.style.GnQuickContactLight);
        }
        // gionee xuhz add for support gn theme start
        ...
        if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
            setContentView(R.layout.gn_quickcontact_activity);
        } else {
        ...
        // gionee xuhz 20121128 add for GIUI2.0 start
        if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
            mGnNoDataBottomView = findViewById(R.id.gn_quickcontact_no_data_bottom);
            mViewPager = findViewById(R.id.gn_quickcontact_view_pager);
        }
        // gionee xuhz 20121128 add for GIUI2.0 end
        ...
        final OnClickListener openDetailsClickHandler = new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = IntentFactory.newViewContactIntent(mLookupUri);
                //Gionee:huangzy 20130401 modify for CR00792013 end
                
                //Gionee <huangzy> <2013-05-03> modify for CR00801194 begin
                /*intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);*/
                intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                //Gionee <huangzy> <2013-05-03> modify for CR00801194 end
    ...
    private void show() {
        ...
        // gionee xuhz 20120912 modify for CR00689825 start
        try {
        	Preconditions.checkNotNull(targetScreen, "missing targetScreen");
        } catch (java.lang.NullPointerException e) {
        	Log.d(TAG, "catched exception. targetScreen is null.");
        }
        if (targetScreen != null) {
            mFloatingLayout.setChildTargetScreen(targetScreen);
        }
        // gionee xuhz 20120912 modify for CR00689825 end
    ...
    private void bindData(Cursor cursor) {
        ...
        if (photoView != null) {
            ...
            if (photoBitmap != null) {
        		...
        		// gionee xuhz 20121123 modify for GIUI2.0 start
        		if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
        			Configuration cf = getResources().getConfiguration();
        			if (cf.orientation == Configuration.ORIENTATION_LANDSCAPE) {
        				photoView.setImageResource(R.drawable.gn_quickcontact_default_photo_land);
        			} else if (cf.orientation == Configuration.ORIENTATION_PORTRAIT) {
        				photoView.setImageResource(R.drawable.gn_quickcontact_default_photo);
        			}
        		} else {
                    photoView.setImageResource(ContactPhotoManager.getDefaultAvatarResId(true, false));
        		}
        		// gionee xuhz 20121123 modify for GIUI2.0 end
            }
        }
        ...
        // gionee xuhz 20121128 add for GIUI2.0 start
        if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
            mGnNoDataBottomView.setVisibility(hasData ? View.GONE : View.VISIBLE);
            mViewPager.setVisibility(hasData ? View.VISIBLE : View.GONE);
        }
        // gionee xuhz 20121128 add for GIUI2.0 end
        mListPager.getAdapter().notifyDataSetChanged();
    }
    ...
    private View inflateAction(String mimeType, ResolveCache resolveCache, ViewGroup root) {
    	if (ContactsApplication.sIsGnContactsSupport) {
    		return gnInflateAction(mimeType, resolveCache, root);
    	}
    ...
    private View getActionViewAt(int position) {
...
//            final CheckableImageView actionView = (CheckableImageView)view;
            final String mimeType = (String) view.getTag();
    ...
    private class PageChangeListener extends SimpleOnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            final View actionView = getActionViewAt(position);
    ...
    private View gnInflateAction(String mimeType, ResolveCache resolveCache, ViewGroup root) {
        final CheckableTextView typeView = (CheckableTextView) getLayoutInflater().inflate(
                R.layout.gn_quickcontact_track_button, root, false);
        ...
        // Set icon and listen for clicks
        CharSequence descrip = gnMimeType2TypeTitle(mimeType, this);
        if (null == descrip) {
        	descrip = resolveCache.getDescription(firstInfo); 
        }
        
        typeView.setText(descrip);
        typeView.setOnClickListener(mTypeViewClickListener);

        return typeView;
    }
    
    private String gnMimeType2TypeTitle(String mimeType, Context context) {
    	if (null == mimeType) {
    		return null;
    	}
    	
    	if (mimeType.equals(GnContactsContract.CommonDataKinds.Phone.CONTENT_ITEM_TYPE)) {
    		return context.getString(R.string.phoneLabelsGroup);
    	} else if (mimeType.equals(GnContactsContract.CommonDataKinds.Email.CONTENT_ITEM_TYPE)) {
    		return context.getString(R.string.emailLabelsGroup);
    	} else if (mimeType.equals(GnContactsContract.CommonDataKinds.Website.CONTENT_ITEM_TYPE)) {
