        ...
        if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
            if (PhoneCapabilityTester.isPhone(mContext)) {
                ...
                if (!TextUtils.isEmpty(number)) {
                    ...
                    if (hasPhone) {
                    	if (ContactsApplication.sIsGnContactsSupport) {
                    		...
                    		if (ContactsApplication.sIsGnDualSimSelectSupport) {
                    			...
                    			phoneIntent = IntentFactory.newDialNumberIntent(number);
                    		}
                            // gionee xuhz 20121215 modify for Dual Sim Select end
                    	} else {
                    		phoneIntent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
                    ...
                    final Intent smsIntent = hasSms ?
                    		IntentFactory.newCreateSmsIntent(number) : null;
            ...
            if (PhoneCapabilityTester.isSipPhone(mContext)) {
                ...
                if (!TextUtils.isEmpty(address)) {
                    ...
                    if (ContactsApplication.sIsGnContactsSupport){
                    	mIntent = IntentFactory.newDialNumberIntent(callUri);
                    } else {
