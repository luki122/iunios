...
public class ImageViewDrawableSetter {
    ...
    public boolean mIsUseCover;

    public ImageViewDrawableSetter() {
    }
    
    public ImageViewDrawableSetter(Context mContext) {
    ...
    protected Bitmap setCompressedImage(byte[] compressed) {
    	if(compressed==null){
    		Log.d(TAG,"setCompressedImage,compressed:"+compressed);
    		int contactId=Integer.parseInt(String.valueOf(mContact.getId()));
			...
			if(index<ResConstant.largeRandomContactPhotoId.length&&contactId!=0){
				mTarget.setImageDrawable(mContext.getResources().getDrawable(ResConstant.largeRandomContactPhotoId[index]));
			}else{
				mTarget.setImageDrawable(mContext.getResources().getDrawable(R.drawable.large_svg_dial_default_photo1));
			}		
			mIsUseCover = true;
			return null;
    	}
    ...
    private Drawable defaultDrawable() {
    	Log.d(TAG,"mContact.getPhotoId():"+mContact.getPhotoId());
        if(mContact.getPhotoId()==0){
        	int index=(int) (mContact.getId()%(ResConstant.randomContactPhotoId.length));
			Log.d(TAG,"contactId:"+mContact.getId()+" index:"+index);

			if(index<ResConstant.largeRandomContactPhotoId.length){
				return mContext.getResources().getDrawable(ResConstant.largeRandomContactPhotoId[index]);
			}else{
				return mContext.getResources().getDrawable(R.drawable.large_svg_dial_default_photo1);
			}
        }
