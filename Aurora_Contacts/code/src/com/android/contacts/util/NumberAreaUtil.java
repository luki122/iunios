...
public class NumberAreaUtil {
	...
	private static final String TAG = "NumberAreaUtil";
    ...
    public String getNumAreaFromLocal(Context context, String number, boolean isRemember){

    	if (!ContactsApplication.sIsGnAreoNumAreaSupport) {
    		return null;
    	}
    	String result = YuloreUtils.getInstance(context).getAreaSmart(number);
    	Log.d(TAG, "result0:"+result);
    	result = processAuroraCase(number, result);
    ...
    private static String processAuroraCase(String number, String result) {
    	Log.d(TAG,"processAuroraCase");
