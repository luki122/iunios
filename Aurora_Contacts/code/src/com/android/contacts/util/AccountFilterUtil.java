    ...
    private static boolean updateAccountFilterTitle(View filterContainer,
        ...
        if (isLoading) {
            ...
            if (null == displayName) {
                ...
                if (ContactsUtils.mIsGnContactsSupport && AccountType.ACCOUNT_TYPE_LOCAL_PHONE.equals(filter.accountType)) {
            ...
            if (ads instanceof AccountWithDataSetEx) {
                ...
                */
                if (ContactsUtils.mIsGnContactsSupport && FeatureOption.MTK_GEMINI_SUPPORT
                    ...
                    if (accountType.equals(ads.type) && accountName.equals(ads.name)) {
                        accountDisplayName = ((AccountWithDataSetEx) ads).getDisplayName();
                        int slotId = ((AccountWithDataSetEx) ads).getSlotId();
                        if (slotId == 0) {
                            accountDisplayName = contactsApp.getString(R.string.slot_a)
                                    + accountDisplayName;
                        } else if (slotId == 1) {
                            accountDisplayName = contactsApp.getString(R.string.slot_b)
                                    + accountDisplayName;
                        }
                    } 
                } else {
