...
import com.android.contacts.util.AsyncTask;
import gionee.provider.GnContactsContract;
import android.provider.MediaStore;
//import android.provider.DrmStore;//aurora change zhouxiaobing 20140512 for 4.4
...
public class ContactLoader extends Loader<ContactLoader.Result> {
    ...
    public static final class Result {
        ...
        private final String mCustomRingtoneTitle;
        ...
        */
        ...
        private Result(Uri requestedUri, Uri uri, Uri lookupUri, long directoryId, String lookupKey,
                ...
                String customRingtone, String customRingtoneTitle, boolean isUserProfile) {
            ...
            mCustomRingtoneTitle = customRingtoneTitle;
        ...
        private Result(Result from) {
            ...
            mCustomRingtoneTitle = from.mCustomRingtoneTitle;
        ...
        public ArrayList<ContentValues> getContentValues() {
            if (mEntities.size() != 1) {
            	Log.e(TAG, "<getContentValues()> mEntities.size() != 1");
...
//                        "Cannot extract content values from an aggregated contact");
    ...
    private final class LoadContactTask extends AsyncTask<Void, Void, Result> {
        ...
        private Result loadContactEntity(ContentResolver resolver, Uri contactUri) {
            ...
            if (ContactsApplication.sIsAuroraPrivacySupport && mIsAuroraPrivacyContact) {
            	selection = "is_privacy>=0";
            }
            
            Cursor cursor = resolver.query(entityUri, ContactQuery.COLUMNS, selection, null,
                    Contacts.Entity.RAW_CONTACT_ID);
            if (cursor == null) {
            ...
            try {
                ...
                for (; !cursor.isAfterLast(); cursor.moveToNext()) {
                    ...
                    if (rawContactId != currentRawContactId) {
                        ...
                        // Gionee:wangth 20130305 add for CR00778745 begin
//                        if (GNContactsUtils.isOnlyQcContactsSupport() && GNContactsUtils.isMultiSimEnabled()) {
//                            if (result.getIndicate() > 0) {
//                                result.setSlot(result.getIndicate() - 1);
//                            }
//                        }
                        // Gionee:wangth 20130305 add for CR00778745 end
                        /*
                         * New Feature  by Mediatek End.
                        */
                    ...
                    if (!cursor.isNull(ContactQuery.DATA_ID)) {
                        ...
                        entity.addSubValue(GnContactsContract.Data.CONTENT_URI, data);
        ...
        private Result loadContactHeaderData(final Cursor cursor, Uri contactUri) {
            final String directoryParameter =
                    contactUri.getQueryParameter(GnContactsContract.DIRECTORY_PARAM_KEY);
            ...
            Log.d("liyang","customRingtone:"+customRingtone);
            //Gionee:huangzy 20130308 modify for CR00772601 start
            /*final String customRingtoneTitle = null == customRingtone ? null :
            	RingtoneManager.getRingtone(getContext(), Uri.parse(customRingtone)).getTitle(getContext());*/
            final String customRingtoneTitle = gnGetRingtoneTile(getContext(), customRingtone);
            Log.d("liyang","customRingtoneTitle:"+customRingtoneTitle);
            ...
            return new Result(mRequestedUri, contactUri, lookupUri, directoryId, lookupKey,
                    ...
                    customRingtone, customRingtoneTitle, isUserProfile);
        }
        ...
        private ContentValues loadRawContact(Cursor cursor) {
            ...
            cv.put(RawContacts._ID, rawContactId);
            ...
            //add by liyang for bug 17249.  start
            Log.i("liyang","r:"+cursor.getString(ContactQuery.CUSTOM_RINGTONE));
            cv.put(Contacts.CUSTOM_RINGTONE, cursor.getString(ContactQuery.CUSTOM_RINGTONE));
            //add by liyang for bug 17249.  end
            
            //Ringtone
            if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
            	Cursor ringtoneCursor = null;
            	try {
            		ringtoneCursor = getContext().getContentResolver().query(
                			GnContactsContract.RawContacts.CONTENT_URI, 
                			new String[]{Contacts.CUSTOM_RINGTONE}, RawContacts._ID + "=" + rawContactId, null, null);
            		if (null != ringtoneCursor && ringtoneCursor.moveToFirst()) {
            			String ringtoneUri = ringtoneCursor.getString(0);
            			cv.put(Contacts.CUSTOM_RINGTONE, ringtoneUri);
            			
            			if (null != ringtoneUri)
            				Log.i("James", ringtoneUri.toString());
            			
            			/*03-06 18:23:33.255: I/James(11638): content://media/external/audio/media/70
*/
            		}
            			
            	} finally {
            		if (null != ringtoneCursor) {
    ...
    public ContactLoader(Context context, Uri lookupUri, boolean isPrivacy) {
        this(context, lookupUri, false, false, false, isPrivacy);
    }

    public ContactLoader(Context context, Uri lookupUri, boolean loadGroupMetaData,
            boolean loadStreamItems, boolean loadInvitableAccountTypes, boolean isPrivacy) {
        super(context);
        mLookupUri = lookupUri;
        ...
        mIsAuroraPrivacyContact = isPrivacy;
    }
    ...
    protected void onForceLoad() {
        ...
        // Gionee:wangth 20130326 modify for CR00789799 begin
        /*
        ...
        */
        ...
        // Gionee:wangth 20130326 modify for CR00789799 end
    }
    ...
    //Gionee:huangzy 20130308 add for CR00772601 start
    public static String gnGetRingtoneTile(Context context, String uriStr) {
    	if (null == uriStr) {
    		return context.getResources().getString(R.string.gnDefaultLabel);
    	}
    	
    	return gnGetRingtoneTile(context, Uri.parse(uriStr));
    }
    ...
    public static String gnGetRingtoneTile(Context context, Uri uri) {
        ...
        if (uri != null) {
            ...
            if ("drm".equals(authority)) {//aurora change zhouxiaobing 20140512 for 4.4 old is DrmStore.AUTHORITY
                String[] DRM_COLUMNS = new String[] {
                    "_id",//DrmStore.Audio._ID,
                    "_data",//DrmStore.Audio.DATA,
                    "title",//DrmStore.Audio.TITLE,
                    "display_name"//DrmStore.Audio.DISPLAY_NAME
                };
                cursor = res.query(uri, DRM_COLUMNS, null, null, 
                		/*DrmStore.Audio._ID*/"_id" + " Limit 1");
        ...
        if (null == title) {
        	title = context.getResources().getString(R.string.gnDefaultLabel);
        }
        //Gionee <huangzy><2013-06-05> modify for CR00818865 end
        
        return title;
    }
    //Gionee:huangzy 20130308 add for CR00772601 end
}
