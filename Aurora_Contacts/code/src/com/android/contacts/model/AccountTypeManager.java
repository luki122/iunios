...
class AccountTypeManagerImpl extends AccountTypeManager
     ...
     */
    private static final Uri SAMPLE_CONTACT_URI = GnContactsContract.Contacts.getLookupUri(
            1, "xxx");
    ...
    private static final Comparator<Account> ACCOUNT_COMPARATOR = new Comparator<Account>() {
        ...
        public int compare(Account a, Account b) {
            ...
            // Gionee lihuafang 2012-07-04 add for CR00633437 begin
            ...
            // Gionee lihuafang 2012-07-04 add for CR00633437 end
    ...
    public AccountTypeManagerImpl(Context context) {
        ...
        // Gionee:wangth 20130311 add for CR00782907 begin
        if (GNContactsUtils.isOnlyQcContactsSupport()) {
            IntentFilter gnfilter = new IntentFilter();
            gnfilter.addAction(Intent.ACTION_BOOT_COMPLETED);
            gnfilter.addAction(TelephonyIntents.ACTION_SIM_STATE_CHANGED);
            mContext.registerReceiver(mBroadcastReceiver, gnfilter);
        }
        // Gionee:wangth 20130311 add end
    ...
    protected void loadAccountsInBackground() {
        // gionee xuhz 20120522 add for CR00588969 start
        if (ContactsApplication.sIsGnContactsSupport) {
            gnLoadAccountsInBackground();
            return;
        }
        // gionee xuhz 20120522 add for CR00588969 end
        ...
        for (Account account : accounts) {
            ...
            try {
                syncable = cs.getIsSyncable(account, GnContactsContract.AUTHORITY) > 0;
            } catch (RemoteException e) {
    ...
    public String getSimAccountNameBySlot(int slotId) {
        ...
        // Gionee liuyanbo 2012-07-31 add for CR00657509 start
            ...
            if (ContactsApplication.isMultiSimEnabled && GNContactsUtils.cardIsUsim(slotId)) {
                retSimName = AccountType.ACCOUNT_NAME_USIM;
                if (SimCardUtils.SimSlot.SLOT_ID2 == slotId) {
                    retSimName = AccountType.ACCOUNT_NAME_USIM2;
                }
    ...
    private void registerReceiverOnSimStateAndInfoChanged() {
        ...
        simFilter.addAction("android.intent.action.SIM_INFO_UPDATE");
    ...
    // gionee xuhz 20120522 add for CR00588969 start
    protected void gnLoadAccountsInBackground() {
        if (Log.isLoggable(Constants.PERFORMANCE_TAG, Log.DEBUG)) {
            Log.d(Constants.PERFORMANCE_TAG, "AccountTypeManager.loadAccountsInBackground start");
        }
        Log.i(TAG, "loadAccountsInBackground()+");
        ...
        //Gionee <Huangzy> <2013-04-20> add for CR00799818 begin
        catch (NullPointerException e) {
        	Log.e(TAG, "Problem obtain ContentResolver.getContentService()");
        }
        //Gionee <Huangzy> <2013-04-20> add for CR00799818 end
        ...
        if (SimCardUtils.isSimInserted(slotid)) {
            ...
            Log.i(TAG, "loadAccountsInBackground slotid:" + slotid + " AccountType:"
                    + simAccountType + " simName:" + simName);
            // Gionee:wangth 20130325 modify for CR00788495 begin
            /*
            allAccounts.add(new AccountWithDataSetEx(simName, simAccountType, slotid));
            contactWritableAccounts.add(new AccountWithDataSetEx(simName, simAccountType, slotid));
            if (SimCardUtils.isSimUsimType(slotid)) {
            ...
            */
            ...
            // Gionee:wangth 20130325 modify for CR00788495 end
        }
