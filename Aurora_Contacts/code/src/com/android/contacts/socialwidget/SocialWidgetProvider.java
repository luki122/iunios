...
public class SocialWidgetProvider extends AppWidgetProvider {
    ...
    private static void bindRemoteViews(final Context context, final int widgetId,
        ...
        if (!contactData.isLoaded()) {
            ...
            // Gionee <xuhz> <2013-05-07> modify for CR00807929 begin
            if (ContactsApplication.sIsGnContactsSupport) {
                setPhoto(views, photo != null
                        ? revisionImageSize(photo)
            ...
            // Gionee <xuhz> <2013-05-07> modify for CR00807929 end
            ...
            final PendingIntent pendingIntent = PendingIntent.getActivity(context,
                    0, intent, 0);
            // Gionee:wangth 20130323 remove for CR00787013 begin
            /*
            ...
            */
            ...
            // Gionee:wangth 20130323 remove for CR00787013 end
        ...
        // Configure UI
        //Gionee <wangth><2013-04-24> modify for CR00800216 begin
        /*
        ...
        */
        try {
            ...
            byte[] image = contactData.getPhotoBinaryData();
            Bitmap bitmap = (image != null
                    ? revisionImageSize(image)
                            : ContactBadgeUtil.loadDefaultAvatarPhoto(context, false, false));
            
            if (bitmap != null && !bitmap.isRecycled()) {
                setPhoto(views, bitmap);
                widgetManager.updateAppWidget(widgetId, views);
                
                bitmap.recycle();
            }
        }
        //Gionee <wangth><2013-04-24> modify for CR00800216 end
    }
    ...
    //Gionee <wangth><2013-04-24> add for CR00800216 begin
    private static Bitmap revisionImageSize(byte[] photo) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        
        // Gionee <xuhz> <2013-06-07> modify for CR00822621 begin
        int imageWidth = ContactsApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.gn_social_widget_image_width);
        int imageHeight = ContactsApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.gn_social_widget_image_height);      

        options.inJustDecodeBounds = true;
		BitmapFactory.decodeByteArray(photo, 0, photo.length, options);

		int issW = (0 == imageWidth ? 1 : (int) (options.outWidth / imageWidth));
		int issH = (0 == imageHeight ? 1: (int) (options.outHeight / imageHeight));
		int iss = Math.max(issW, issH);
		iss = Math.max(1, iss);
		options.inSampleSize = iss;
    	
		options.inJustDecodeBounds = false;
		// Gionee <xuhz> <2013-06-07> modify for CR00822621 end
		Bitmap bitmap = BitmapFactory.decodeByteArray(photo, 0, photo.length, options);
        return bitmap;
    }
    //Gionee <wangth><2013-04-24> add for CR00800216 end
}
