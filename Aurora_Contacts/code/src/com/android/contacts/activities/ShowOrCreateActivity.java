...
public final class ShowOrCreateActivity extends ContactsActivity
    ...
    protected Dialog onCreateDialog(int id) {
	    ...
	    case CREATE_CONTACT_DIALOG:
	    	...
	    	if (ContactsApplication.sIsGnContactsSupport) {
                ...
                return new AuroraAlertDialog.Builder(this, AuroraAlertDialog.THEME_HOLO_LIGHT)
    ...
    private void showQuickNumberHandler() {
    	//Gionee:huangzy 20120803 modify for CR00640810 start
        /*//Gionee:huangzy 20120702 modify for CR00632883 start
        AlertDialog dialog = new AuroraAlertDialog.Builder(this, AlertDialog.THEME_HOLO_LIGHT).create();
    	GnStrangeNumberHandler numberHandler = GnStrangeNumberHandler.create(ShowOrCreateActivity.this, mCreateDescrip);
    	//Gionee:huangzy 20120702 modify for CR00632883 end
    	dialog.setView(numberHandler);
    	...
    	if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
    		AuroraAlertDialog dialog = (AuroraAlertDialog) createGnDialog(mCreateDescrip);
        	dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
    			@Override
    			public void onDismiss(DialogInterface dialog) {
    				finish();
        	...
        	//Gionee <huangzy> <2013-07-22> modify for CR00836863 begin
    		/*dialog.show();*/
        	if (!isFinishing()) {
    		    dialog.show();
        	}
    		//Gionee <huangzy> <2013-07-22> modify for CR00836863 end
    	} else {
        	
    	}
    	// gionee xuhz 20121123 modify for GIUI2.0 end
    	//Gionee:huangzy 20120803 modify for CR00640810 end
    }

    // gionee xuhz 20121123 modify for GIUI2.0 start
	private Dialog createGnDialog(final String titleText) {
		ArrayList<String> itemList = new ArrayList<String>();
		
		itemList.add(getResources().getString(R.string.gn_dialpad_new_contact_item));
		itemList.add(getResources().getString(R.string.gn_dialpad_add_exist_contact_item));
    	
        CharSequence[] items = itemList.toArray(new CharSequence[itemList.size()]);
        
		return new AuroraAlertDialog.Builder(this)
        .setTitle(titleText).setTitleDividerVisible(true)
        .setItems(items, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
		        switch (which) {
		            case 0:
		            	startActivity(IntentFactory.newCreateContactIntent(titleText));
		                break;
		            case 1:
		            	startActivity(IntentFactory.newInsert2ExistContactIntent(titleText));
		                break;
