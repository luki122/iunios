...
implements OnSearchViewQuitListener {
	private static final String TAG = "liyang-AuroraSimContactListActivity";
		...
		public void handleMessage(Message msg) {
			...
			case START: {
				if (!isFinishing()) {
					...
					mSaveProgressDialog.setCancelable(false);// 设置是否可以通过点击Back键取消  
					mSaveProgressDialog.setCanceledOnTouchOutside(false);// 设置在点击Dialog外是否取消Dialog进度条
					try {
	...
	public static ArrayList<String> dataIdForAutoRecord = null;
	private void addContacts() {
		...
		dataIdForAutoRecord = new ArrayList<String>();
		long dataId = -1;
		String number;
		String nameBlack;
		String numberBlack;
		...
		for (int position = 0; position < mFragment.mItemCount; position++) {
			...
			if (dataIds.contains(dataId)) {
				...
				if (mIsCallRecord || mIsBlackName) {
					if(!dataIdForAutoRecord.contains(String.valueOf(dataId))){
	...
	public void updateSelectedItemsView(int allCount) {
		...
		count = allCount - mFragment.mAdapter.getStarredCount();
		...
		try {
			...
			mActionBar.getMiddleTextView().setText(mContext.getResources().getString(R.string.selected_total_num, checkedItemCount));
