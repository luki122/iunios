...
public class AuroraContactsSetting extends AuroraPreferenceActivity 
        implements AuroraPreference.OnPreferenceClickListener, AuroraPreference.OnPreferenceChangeListener{
    ...
    private AuroraSwitchPreference mOverTurn;    
    private AuroraSwitchPreference mRinger;
    private AuroraPreference mCallForward;
    private AuroraPreference mCallWaiting;
...
//        		((AuroraPreferenceGroup)(getPreferenceScreen().getPreference(0))).removePreference(mSimContacts02);
        ...
        mOverTurn = (AuroraSwitchPreference) findPreference("reverse_mute");  
        mOverTurn.setOnPreferenceChangeListener(this);
        mRinger = (AuroraSwitchPreference) findPreference("intelligent_alarm");
        mRinger.setOnPreferenceChangeListener(this);
        mCallForward =  findPreference("incoming_call_switching");
        mCallWaiting =  findPreference("call_waiting");
  
        updatePhoneSettingUi();
	    IntentFilter gnfilter = new IntentFilter();
        gnfilter.addAction(TelephonyIntents.ACTION_SIM_STATE_CHANGED);
        registerReceiver(mBroadcastReceiver, gnfilter);
    }
	...
	public boolean onPreferenceChange(AuroraPreference preference,
			Object objValue) {
		Log.i("auroraContactsSetting", "onPreferenceChange preference =" + preference);
		if (preference == mOverTurn) {
			Log.i("auroraContactsSetting", "onPreferenceChange mOverTurn =" + (Boolean) objValue);
			Intent intent = new Intent("com.android.phone.overturn");
			intent.putExtra("value",  (Boolean) objValue);
			sendBroadcast(intent);
		} else if(preference == mRinger) {
			Intent intent = new Intent("com.android.phone.smartringer");
			intent.putExtra("value",  (Boolean) objValue);
			sendBroadcast(intent);
		}
		return true;
    ...
    private void updatePhoneSettingUi() {
		if (FeatureOption.MTK_GEMINI_SUPPORT) {
			if (!SimCardUtils.isSimInserted(0)
					&& !SimCardUtils.isSimInserted(1)) {
				mCallForward.setEnabled(false);
				mCallWaiting.setEnabled(false);
			} else {
				mCallForward.setEnabled(true);
				mCallWaiting.setEnabled(true);
			}
		}
    }
	...
	protected void onDestroy() {
		super.onDestroy();
		Log.d("liyang","AuroraDialactsActivityV2 onDestroy()");
