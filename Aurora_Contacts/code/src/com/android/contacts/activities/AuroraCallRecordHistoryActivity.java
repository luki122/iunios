...
//                            .setMessage(R.string.call_record_delete_message)
	...
	private static final int CLEAR_ALL_ITEM_MENU = 1; 
	...
	private void initActionBar(boolean flag) {
		...
		if(mAdapter.getCount() < 1) { 
			mActionBar.removeItemByItemId(CLEAR_ALL_ITEM_MENU);
		}
	}
	...
	private void buildData() {
		...
		if(mAdapter.getCount() > 0) { 
			mActionBar.addItem(R.layout.aurora_right_actionbar_item, CLEAR_ALL_ITEM_MENU);
	...
	private void updateSelectedItemsView() {
        ...
        if(middleTextView!=null){
			middleTextView.setText(mContext.getString(R.string.selected_total_num, checkedCount));
		}
	}
            ...
            .setPositiveButton(R.string.clear_all,
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog,
                                int whichButton) {
                        	mHandler.sendEmptyMessage(START);   
                        	HashMap<String, AuroraCallRecord> filelist = new HashMap<String, AuroraCallRecord>();
                    		for (int position = 0;
                                    position < mAdapter.getCount(); ++position) {    
                                	filelist.put(String.valueOf(position), mRecords.get(position));                     
                    		}
