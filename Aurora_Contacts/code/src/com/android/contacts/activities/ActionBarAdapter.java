...
public class ActionBarAdapter implements OnQueryTextListener, OnCloseListener {
    ...
    public ActionBarAdapter(Context context, Listener listener, ActionBar actionBar,
        ...
        if (ContactsApplication.sIsGnContactsSupport) {
            customSearchView = LayoutInflater.from(mActionBar.getThemedContext()).inflate(
                    R.layout.gn_custom_action_bar, null);
        }
        // gionee xuhz 20120529 add for remove home icon from ActionBar end
        ...
        mSearchView = (AuroraSearchView) customSearchView.findViewById(R.id.search_view);
        ...
        // gionee xuhz 20120529 add for remove home icon from ActionBar start
        if (ContactsApplication.sIsGnContactsSupport) {
            mGnAciontUpView = (ImageView)customSearchView.findViewById(R.id.gn_action_up);
    	...
    	if (null != mSearchView) {
	        new SimpleAsynTask() {
				...
				protected Integer doInBackground(Integer... params) {
					return ContactsUtils.getContactsCount(mContext.getContentResolver());  
				}
				
				protected void onPostExecute(Integer result) {
					mSearchView.setQueryHint(String.format(mContext.getString(R.string.gn_hint_findContacts), result));
				};
	        }.execute();
        }
	}    
