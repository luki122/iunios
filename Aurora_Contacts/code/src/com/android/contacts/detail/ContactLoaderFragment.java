...
//            optionsRingtone.setVisible(mOptionsMenuOptions);
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_block_voice_incoming_call: {
                ...
                if (ContactsApplication.sIsGnContactsSupport) {
                    ...
                    intent = ContactSaveService.createSetSendToVoicemail(mContext, mLookupUri, mSendToVoicemailState);
                }
            ...
            case R.id.menu_block_video_incoming_call: {             
                ...
                if (ContactsApplication.sIsGnContactsSupport) {
                    ...
                    intent = ContactSaveService.createSetBlockVideoCall(mContext, mLookupUri, mBlockVideoCallState);
                }
                //Gionee:huangzy 20120704 modify for CR00633431 end
    ...
    private void handleRingtonePicked(Uri pickedUri) {
        ...
        if (ContactsApplication.sIsGnContactsSupport) {
            ...
            intent = ContactSaveService.createSetRingtone(mContext, mLookupUri, mCustomRingtone);
        }
        //Gionee:huangzy 20120704 modify for CR00633431 end
    ...
    public boolean ifSupport3G324M() {         
        if (true == FeatureOption.MTK_VT3G324M_SUPPORT) {
            String mOptr = SystemProperties.get("ro.operator.optr");
            if (null != mOptr && mOptr.equals("OP01")) {
                // Gionee:wangth 20130108 remove for CR00760802 begin
                /*
                ITelephony iTel = ITelephony.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE));
                try {
                    mRejectAllVoiceCall = GnITelephony.isRejectAllVoiceCall(iTel);
                    mRejectAllVideoCall = GnITelephony.isRejectAllVideoCall(iTel);
                } catch (Exception e) {
                    Log.i(TAG, "[ifSupport3G324M]: Exception=" + e.toString());
                }
                */
                // Gionee:wangth 20130108 remove for CR00760802 end
                return true;
            } 
        }
        return false;
