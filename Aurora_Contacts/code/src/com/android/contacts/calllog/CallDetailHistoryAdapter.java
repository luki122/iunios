...
public class CallDetailHistoryAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
    	if (ContactsApplication.sIsGnContactsSupport) {
    		return gnGetView(position, convertView, parent);
    	}
        if (position == 0) {
            /**
    ...
    public View gnGetView(int position, View convertView, ViewGroup parent) {
        if (position == 0) {

        	// gionee xuhz 20121126 modify for GIUI2.0 start
        	View header;
        	if (ContactsApplication.sIsGnGGKJ_V2_0Support) {
        		header = convertView == null
                ...
                addToContacts.setVisibility(mPhoneCallDetails[position].contactUri == null ? View.VISIBLE : View.GONE);
        	} else {
        		header = convertView == null
                ? mLayoutInflater.inflate(R.layout.gn_call_detail_history_without_voicemail_header, parent, false)
                : convertView;
        	}
        ...
        // Make sure we have a valid convertView to start with
        final View result  = convertView == null
                ? mLayoutInflater.inflate(R.layout.gn_call_detail_history_item, parent, false)
                : convertView;
...
//                DateUtils.FORMAT_SHOW_WEEKDAY | DateUtils.FORMAT_SHOW_YEAR);
        ...
        */
        ...
        View playRecordIcon = result.findViewById(R.id.gn_phone_record_play_icon);
        if (null != playRecordIcon) {
        	if (null != details.getPhoneRecords()) {
    ...
    private void playRecord(final List<PhoneCallRecord> records) {
    	...
    	for (int i = 0; i < items.length; i++) {
    		items[i] = new File(records.get(i).getPath()).getName();
    	}
    	new AuroraAlertDialog.Builder(mContext)
    	.setTitle(R.string.gn_phone_call_record_tile)
