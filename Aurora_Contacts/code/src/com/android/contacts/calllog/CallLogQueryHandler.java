...
/*package*/ public class CallLogQueryHandler extends AsyncQueryHandler {
    ...
    private static final String TAG = "liyang-CallLogQueryHandler";
    ...
    private static final int QUERY_INIT = 59;
...
//    }
    ...
    public CallLogQueryHandler(ContentResolver contentResolver, AppWidgetListener listener,int flag) {
        super(contentResolver);
        appWidgetListener = listener;
    }
...
//        CallLogDateFormatHelper.refreshData();
    ...
    public void markNewCallsAsOld() {
        ...
        where.append(" = 1");
    	where.append(" AND (privacy_id = " + AuroraPrivacyUtils.getCurrentAccountId() + " or privacy_id = 0)");
    ...
    @Override
    protected synchronized void onQueryComplete(int token, Object cookie, final Cursor cursor) {
        ...
        }else if (QUERY_INIT == token) {
          new Thread(){
        	  public void run() {
        		  if(cursor!=null){
        			  while(cursor.moveToNext()){
        				  madapter.getContactInfo(cursor);
        			  }
        			  cursor.close();
        		  }
        	  };
          }.start();
        }else if(FETCH_APPWIDGET_TOKEN==token){
        	Log.d(TAG,"FETCH_APPWIDGET_TOKEN==token1");
        	updateAdapterData1(cursor);
        	return;
        }
    ...
    private void updateAdapterData1(Cursor combinedCursor) {
        
        Log.d(TAG,"updateAdapterData1(),listener:"+appWidgetListener);
        if (appWidgetListener != null) {
        	appWidgetListener.onCallsFetched(combinedCursor);
        }
    }
    ...
    public void fetchCallsJionDataView(int simFilter, int typeFilter) {
        log("fetchCallsJionDataView()1, simFilter = " + simFilter + " typeFilter " + typeFilter);
        cancelFetch();
        invalidate();
        fetchCalls(QUERY_ALL_CALLS_JOIN_DATA_VIEW_TOKEN, simFilter, typeFilter);
    }
    
    public void fetchCallsJionDataView(int simFilter, int typeFilter,AuroraCallLogAdapterV2 adapter, boolean isPrivate) {
    	madapter=adapter;
        log("fetchCallsJionDataView(), simFilter = " + simFilter + " typeFilter " + typeFilter);
        cancelFetch();
        invalidate();
        fetchCalls(QUERY_ALL_CALLS_JOIN_DATA_VIEW_TOKEN, simFilter, typeFilter, isPrivate);
    }
    
    private void fetchCalls(int token, int simFilter, int typeFilter, boolean isPrivate) {
    	if (ContactsApplication.sIsGnContactsSupport) {
    		gnFetchCalls(token, simFilter, typeFilter, isPrivate);
    		return;
    	}
        ...
        if (QUERY_ALL_CALLS_JOIN_DATA_VIEW_TOKEN == token) {
            ...
            Log.d(TAG,"queryProjection13:"+queryProjection[13]);
        }
//        CallLogDateFormatHelper.refreshData();
...
//		CallLogDateFormatHelper.refreshData();
    ...
    private String getSelection(int simFilter, int typeFilter, boolean isPrivate) {
        StringBuilder builder = new StringBuilder();

        // FILTER_ALL_RESOURCES - null: no condition
        if (Constants.FILTER_SIP_CALL == simFilter) {
            builder.append(Calls.SIM_ID + "=" + ContactsUtils.CALL_TYPE_SIP);
        } else if (simFilter < Constants.FILTER_BASE) {
            builder.append(Calls.SIM_ID + "=" + simFilter);
        }
        ...
        if (typeFilter != Constants.FILTER_TYPE_ALL) {
            ...
            // Gionee:wangth 20130328 add for CR00790182 begin
            if (GNContactsUtils.isOnlyQcContactsSupport()) {
                ...
                builder.append(" or " + Calls.TYPE + "=" + qc);
            }
            // Gionee:wangth 20130328 add for CR00790182 end
        }
        ...
        if(ContactsApplication.sIsAuroraPrivacySupport) {
            ...
            if(isPrivate) {
               	builder.append("(privacy_id = " + AuroraPrivacyUtils.getCurrentAccountId() + ")");	
            } else {
            	builder.append("(privacy_id = " + AuroraPrivacyUtils.getCurrentAccountId() + " or privacy_id = 0)");	
            }
        }
    ...
    private void gnFetchCalls(int token, int simFilter, int typeFilter, boolean isPrivate) {
    	Log.d(TAG,"gnFetchCalls");
        log("fetchCalls(), token = " + token + " simFilter = " + simFilter + " typeFilter " + typeFilter);
        String selection = getSelection(simFilter, typeFilter, isPrivate);
        log("startQuery  selection=  =" + selection);
        ...
        if (QUERY_ALL_CALLS_JOIN_DATA_VIEW_TOKEN == token) {
        	...
        	if (ContactsApplication.sIsGnCombineCalllogMatchNumber) {
            	Log.d(TAG,"gnFetchCalls1");

        		queryUri = Uri.parse("content://call_log/gncallsjoindataview_matchnumber");
                queryProjection = CallLogQuery.PROJECTION_CALLS_JOIN_DATAVIEW;        
        	} else {
            	Log.d(TAG,"gnFetchCalls2");

        		queryUri = Uri.parse("content://call_log/gncallsjoindataview");
                queryProjection = CallLogQuery.GN_PROJECTION_CALLS_JOIN_DATAVIEW;
        	}
...
//        CallLogDateFormatHelper.refreshData();
        startQuery(token, null, queryUri, queryProjection, selection, null,
                Calls.DEFAULT_SORT_ORDER);
        /*startQuery(QUERY_INIT, null, queryUri, queryProjection, selection, null,
                Calls.DEFAULT_SORT_ORDER);*/
        
    }
    
    //aurora add liguangyu 20150127 for #11359 start
    public void fetchCallsJionDataView(int simFilter, int typeFilter, AuroraCallLogAdapterV2 adapter) {
    	Log.d(TAG, "fetchCallsJionDataView0");
    	fetchCallsJionDataView(simFilter, typeFilter, adapter, false);
    }
    
    private void fetchCalls(int token, int simFilter, int typeFilter) {
    	fetchCalls(token, simFilter, typeFilter, false);
    }
    
    private void gnFetchCalls(int token, int simFilter, int typeFilter) {
    	gnFetchCalls(token, simFilter, typeFilter, false); 
    }
    
    private String getSelection(int simFilter, int typeFilter) {
    	return getSelection(simFilter, typeFilter, false);
    }
    
    //aurora add liguangyu 20150127 for #11359 end
    
    public int FETCH_APPWIDGET_TOKEN=-111;
    //获取拨号小工具所需联系人（最常用的4个）
    public void FetchAppWidgetContacts() {
    	Log.d(TAG,"FetchAppWidgetContacts");
        String selection="type=2";
        Uri queryUri = Uri.parse("content://call_log/fetchappwidgetcontacts");
        String[] queryProjection = CallLogQuery.PROJECTION_CALLS_JOIN_DATAVIEW;        			

        startQuery(FETCH_APPWIDGET_TOKEN, null, queryUri, queryProjection, selection, null,
                Calls.DEFAULT_SORT_ORDER);
