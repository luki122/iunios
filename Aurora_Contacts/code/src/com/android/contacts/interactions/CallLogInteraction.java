...
public class CallLogInteraction implements ContactInteraction {
    ...
    public CallLogInteraction(ContentValues values,   ArrayList<PhoneCallRecord> records) {
        mValues = values;
        foundAndSetPhoneRecords(records);
    }
...
//        return date == null ? null : ContactInteractionUtil.formatDateStringFromTimestamp(
//                date, context);
    	CallLogResHolder mCallTypeResHolder;
        mCallTypeResHolder = new CallLogResHolder(context);
    	return mCallTypeResHolder.getCallDurationText(
    			 getType(), getDuration()) ;
    }
...
//                        PorterDuff.Mode.MULTIPLY);
              	 callArrow = res.getDrawable(R.drawable.aurora_ic_call_incoming);
                break;
            case Calls.MISSED_TYPE:
//                callArrow = res.getDrawable(CALL_ARROW_ICON_RES);
//                callArrow.setColorFilter(res.getColor(R.color.call_arrow_red),
//                        PorterDuff.Mode.MULTIPLY);
            	 callArrow = res.getDrawable(R.drawable.aurora_ic_call_missed);
                break;
            case Calls.OUTGOING_TYPE:
//                callArrow = BitmapUtil.getRotatedDrawable(res, CALL_ARROW_ICON_RES, 180f);
//                callArrow.setColorFilter(res.getColor(R.color.call_arrow_green),
//                        PorterDuff.Mode.MULTIPLY);
            	 callArrow = res.getDrawable(R.drawable.aurora_ic_call_outgoing);            	
                break;
        }
        return callArrow;
    }
    ...
    public Integer getSimId() {
        return mValues.getAsInteger("simid");
    }
    
    public boolean isMissedCall() {
    	return  getType() == Calls.MISSED_TYPE;
    }
    
    private boolean foundAndSetPhoneRecords(List<PhoneCallRecord> records) {
    	 if(records == null) {
    		 return false;
    	 }
    	int found = 0;
		...
		for (int i = 0; i < records.size() && found < records.size(); ++i) {
			if (null != records.get(i) && betweenCall(records.get(i).getEndTime())) {
				addPhoneRecords(records.get(i));
				++found;
			}
		}		
    ...
    public boolean betweenCall(long time) {
		...
		return gap >= 0 && gap <= getDuration()*1000 + 999;
	}
    
    private List<PhoneCallRecord> mPhoneRecords = null ;
    
    private void addPhoneRecords(PhoneCallRecord phoneRecord) {
    ...
    public Intent getRecordIntent() {
    	if(mPhoneRecords != null) {
    		return new Intent("PLAY_RECORD_ACTION");    		
    	} else {
    		return null;
    	}
    }
    ...
    public Drawable getPhoto() {
    	byte[] mdata = mValues.getAsByteArray("photo");
    	if(mdata != null) {
	    	Bitmap b  = BitmapFactory.decodeByteArray(mdata, 0, mdata.length);
