...
public class DefaultContactListAdapter extends ContactListAdapter {
	private static final String TAG = "liyang-DefaultContactListAdapter";
	...
	public void configureLoader(CursorLoader loader, long directoryId) {
		Log.d(TAG, "configureLoader,loader:"+loader);
	...
	protected void bindView(View itemView, int partition, Cursor cursor, int position) {
		...
		Log.d(TAG,"placement1:"+getItemPlacementInSection(position).sectionHeader+" placement2:"+getItemPlacementInSection(position+1).sectionHeader);
		ImageView iv = (ImageView) itemView.findViewById(com.aurora.R.id.aurora_listview_divider);
		if ( getItemPlacementInSection(position+1).sectionHeader != null ||position==cursor.getCount()-1){			
...
//		AuroraListView.auroraGetAuroraStateListDrawableFromIndex(contentUi, position);
		...
		if (/*position >= getStarredCount() || isSearch*/true) {
...
//			view.setCheckable(false);
		...
		{
			ImageView photoView = isQuickContactEnabled() ? view.getQuickContact() : (ImageView)view.getPhotoView();
		...
		if (isQuickContactEnabled()) {
			...
			Log.d(TAG,"name:"+cursor.getString(ContactQuery.CONTACT_DISPLAY_NAME) +" realPhotoUri:"+
					cursor.getString(ContactQuery.CONTACT_REAL_PHOTO_URI)+" photouri:"+cursor.getString(ContactQuery.CONTACT_PHOTO_URI));
