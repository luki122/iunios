...
public class SubContactsUtils extends ContactsUtils{
    ...
    public static boolean simStateReady (int simId) {    	
    	...
    	{
            boolean simReady = (TelephonyManager.SIM_STATE_READY 
                   == gionee.telephony.GnTelephonyManager.getSimStateGemini(simId));
            return simReady;
    	}
    	else
    	{
...
//        try {
//            if (FeatureOption.MTK_GEMINI_SUPPORT) {
//                isPhoneBookReady = GnITelephony.isPhbReadyGemini(iPhb, slotID);
//    
//            } else {
//                isPhoneBookReady = GnITelephony.isPhbReady(iPhb);
//            }
//               
//        } catch(Exception e) {
...
//        return isPhoneBookReady;
    ...
    public static Uri getUri(int slotId) {
        ...
        try {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                //qc--mtk
            	if(!AuroraTelephoneManager.isMtkGemini())//aurora add zhouxiaobing 20140421
            	{
            		if(slotId==0)
...
//          builder.withValue(Phone.TYPE, phoneType);   
        ...
        try {
            ContentProviderResult[] result= resolver.applyBatch(GnContactsContract.AUTHORITY,
