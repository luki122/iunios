...
// The previous lines are provided and maintained by Mediatek inc.                              
            ...
            if(Build.VERSION.SDK_INT < 21) {
            	dstSimUri = SimCardUtils.SimUri.getSimUri(dstSlotId);
            } else {
                // Aurora xuyong 2015-07-14 modified for bug #14165 start
                int indicate = SIMInfoWrapper.getDefault().getSimIdBySlotId(dstSlotId);
                dstSimUri = /*SimCardUtils.SimUri.getSimUri(dstSlotId)*/SimCardUtils.getSimContactsUri((int)indicate,
                		SimCardUtils.isSimUsimType(dstSlotId));
...
//
// Previous Code:                        
//     for (int gId : ugrpIdArray) {
//         boolean res = USIMGroup.addUSIMGroupMember(dstSlotId, (int)indexInSim, gId);
//         if (!res)
//             grpIdSet.remove(gId);
//     }
// The previous lines are provided and maintained by Mediatek inc.
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                if (DEBUG) {
                    Log.i(LOG_TAG, "iTel.hasIccCardGemini(simId) is "
                            + GnITelephony.hasIccCardGemini(iTel, slot));
                    Log.i(LOG_TAG, "iTel.isRadioOnGemini(simId) is " + GnITelephony.isRadioOnGemini(iTel, slot));
                    Log.i(LOG_TAG, "iTel.isFDNEnabledGemini(simId) is "
                            + GnITelephony.isFDNEnabledGemini(iTel, slot));
                    Log.i(LOG_TAG, "getSimStateGemini(simId) "
                            + (TelephonyManager.SIM_STATE_READY == GnTelephonyManager.getSimStateGemini(slot)));
                }
                return GnITelephony.hasIccCardGemini(iTel, slot)
                        && GnITelephony.isRadioOnGemini(iTel, slot)
                        && !GnITelephony.isFDNEnabledGemini(iTel, slot)
                        && TelephonyManager.SIM_STATE_READY == GnTelephonyManager.getSimStateGemini(slot);
            } else { // Single SIM
