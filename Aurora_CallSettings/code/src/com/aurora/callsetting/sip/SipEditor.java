...
package com.aurora.callsetting.sip;

import com.android.internal.telephony.CallManager;
import com.android.internal.telephony.Phone;
import com.android.internal.telephony.PhoneConstants;
import com.aurora.callsetting.R;
import com.aurora.callsetting.SipUtil;
 ...
 */
public class SipEditor extends AuroraPreferenceActivity
    ...
    enum PreferenceKey {
        ...
        String getValue() {
            if (preference instanceof AuroraEditTextPreference) {
                return ((AuroraEditTextPreference) preference).getText();
            } else if (preference instanceof AuroraListPreference) {
                return ((AuroraListPreference) preference).getValue();
            }
        ...
        void setValue(String value) {
            if (preference instanceof AuroraEditTextPreference) {
                ...
                ((AuroraEditTextPreference) preference).setText(value);
                if (this != Password) {
                    Log.v(TAG, this + ": setValue() " + value + ": " + oldValue
                            + " --> " + getValue());
                }
            } else if (preference instanceof AuroraListPreference) {
                ((AuroraListPreference) preference).setValue(value);
            }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        AuroraPreferenceGroup screen = (AuroraPreferenceGroup) getPreferenceScreen();
    ...
    private void showAlert(final String message) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                new AuroraAlertDialog.Builder(SipEditor.this)
    ...
    private boolean isEditTextEmpty(PreferenceKey key) {
    	AuroraEditTextPreference pref = (AuroraEditTextPreference) key.preference;
    ...
    private void validateAndSetResult() {
        ...
        for (PreferenceKey key : PreferenceKey.values()) {
        	AuroraPreference p = key.preference;
            if (p instanceof AuroraEditTextPreference) {
            	AuroraEditTextPreference pref = (AuroraEditTextPreference) p;
    ...
    public boolean onPreferenceChange(AuroraPreference pref, Object newValue) {
        ...
        if (pref == PreferenceKey.DisplayName.preference) {
            ((AuroraEditTextPreference) pref).setText(value);
    ...
    private PreferenceKey getPreferenceKey(AuroraPreference pref) {
        for (PreferenceKey key : PreferenceKey.values()) {
    ...
    private boolean isAlwaysSendKeepAlive() {
    	AuroraListPreference pref = (AuroraListPreference)
    ...
    private void setCheckBox(PreferenceKey key, boolean checked) {
    	AuroraCheckBoxPreference pref = (AuroraCheckBoxPreference) key.preference;
        pref.setChecked(checked);
    }

    private void setupPreference(AuroraPreference pref) {
    ...
    private class PrimaryAccountSelector {
        private AuroraCheckBoxPreference mCheckbox;
        ...
        PrimaryAccountSelector(SipProfile profile) {
            mCheckbox = (AuroraCheckBoxPreference) getPreferenceScreen()
    ...
    private class AdvancedSettings
        ...
        private AuroraPreference mAdvancedSettingsTrigger;
        private AuroraPreference[] mPreferences;
        ...
        private void loadAdvancedPreferences() {
            ...
            AuroraPreferenceGroup group = (AuroraPreferenceGroup) screen.findPreference(
                    getString(R.string.advanced_settings_container));
            screen.removePreference(group);

            mPreferences = new AuroraPreference[group.getPreferenceCount()];
            int order = screen.getPreferenceCount();
            for (int i = 0, n = mPreferences.length; i < n; i++) {
            	AuroraPreference pref = group.getPreference(i);
                pref.setOrder(order++);
        ...
        void show() {
            ...
            AuroraPreferenceGroup screen = (AuroraPreferenceGroup) getPreferenceScreen();
            for (AuroraPreference pref : mPreferences) {
