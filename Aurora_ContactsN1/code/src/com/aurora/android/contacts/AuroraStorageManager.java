...
public class AuroraStorageManager {
	...
	// / M: @{
	private static AuroraStorageManager sMe;
	...
	public static AuroraStorageManager getInstance(Context context) {
		if (sMe == null) {
			sMe = new AuroraStorageManager(ContactsApplication.getInstance());

		}
		return sMe;
	}

	public AuroraStorageManager(Context context) {
		mstorage = (StorageManager) context
	...
	public String getGnAvailableExternalStoragePath_ex(long requireSize) {
		...
		for (int i = 0; i < path_canuse.length; i++) {
			if (Environment.MEDIA_MOUNTED
					.equals(getVolumeState(path_canuse[i]))) {
		...
		if (path.equals(Environment.getExternalStorageDirectory().getPath())) {
			if (android.os.Build.VERSION.SDK_INT < 19) {
				try {
					...
					Method method = pc
							.getMethod("getExternalStorageAppCacheDirectory",
									String.class);
					mMTKExternalCacheDir = (File) method.invoke(null,
							packageName);
				} catch (Exception e) {
				}
			} else {
				try {
					...
					Class<?> pc = Class.forName("android.os.Environment");
					Method method = pc.getMethod(
							"buildExternalStorageAppCacheDirs", String.class);
					mMTKExternalCacheDir = ((File[]) method.invoke(null,
							packageName))[0];
				} catch (Exception e) {
				}
				...
				try {
					...
					if (android.os.Build.VERSION.SDK_INT < 19) {
						try {
							...
							Class<?> pc = Class
									.forName("android.os.Environment");
							Method method = pc
									.getMethod("getExternalStorageAndroidDataDir");
						...
						try {
							...
							Class<?> pc = Class
									.forName("android.os.Environment");
							Method method = pc
									.getMethod("buildExternalStorageAndroidDataDirs");
			...
			mMTKExternalCacheDir = new File(new File(new File(new File(
					new File(path), "Android"), "data"), packageName), "cache");
			if (!mMTKExternalCacheDir.exists()) {
				try {
					(new File(new File(new File(new File(path), "Android"),
							"data"), ".nomedia")).createNewFile();
				} catch (IOException e) {
					// do nothing
				}
				if (!mMTKExternalCacheDir.mkdirs()) {
