...
//       boolean cardAvaiable = AuroraStorageManager.getVolumeState("/mnt/sdcard").equals(Environment.MEDIA_MOUNTED) || 
//                               AuroraStorageManager.getVolumeState("/mnt/sdcard2").equals(Environment.MEDIA_MOUNTED);
	   String aString = AuroraStorageManager.getInstance(context).getSdCardPath(0);
	   String bString = AuroraStorageManager.getInstance(context).getSdCardPath(1);
       boolean cardAvaiable = AuroraStorageManager.getVolumeState(aString).equals(Environment.MEDIA_MOUNTED) || 
                                 AuroraStorageManager.getVolumeState(bString).equals(Environment.MEDIA_MOUNTED);
       if (cardAvaiable) {
    	   String pathString = AuroraStorageManager.getInstance(context).getGnAvailableExternalStoragePath_ex(bytes + DEFAULT_REDUNDANCY_VALUE);
    	   return pathString;
