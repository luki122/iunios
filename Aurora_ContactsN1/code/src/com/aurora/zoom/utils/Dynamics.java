...
public abstract class Dynamics {
    ...
    private float mMinPosition = -Float.MAX_VALUE;
    ...
    public boolean isAtRest(final float velocityTolerance, final float positionTolerance) {
        ...
        final boolean withinLimits = mPosition - positionTolerance < mMaxPosition
                && mPosition + positionTolerance > mMinPosition && (0 < mPosition && mPosition < 1);
    ...
    public void gnUpdate(final long now, boolean flag) {
        int dt = (int)(now - mLastTime);
        if (dt > MAX_TIMESTEP) {
            dt = MAX_TIMESTEP;
        }
