...
public class GNContactsUtils {
    ...
    private static final int SIM_STATE_READY = 1;
    
    
    
    
    private static final Uri SINGLE_SIM_URI = Uri.parse("content://icc/adn");
    private static final Uri MULTI_SIM_SUB_1_URI = Uri.parse("content://iccmsim/adn");
    private static final Uri MULTI_SIM_SUB_2_URI = Uri.parse("content://iccmsim/adn_sub2");
    ...
    private static boolean cardIsUsimInternal(int sub) {
    ...
    public static boolean hasIccCard(int sub) {
        return AuroraITelephony.hasIccCardGemini(iTel, sub);
    }
    ...
    public static int querySlotIdByLookupUri(Context context, Uri lookupUri) {
        ...
        if (isMultiSimEnabled()) {
            return ContactsUtils.getSlotBySubId(res);
        }
    ...
    public static boolean isQCSimReady(int sub) {
        ...
        try {
            isRadioOn = AuroraITelephony.isRadioOnGemini(iTel, sub);
        } catch (Exception e) {
            e.printStackTrace();
        }
       
        if (hasIccCard(sub) && AuroraTelephonyManager.isValidSimState(sub)
                && isRadioOn) {
            result = true;
        }
        return result;
...
//        return AuroraTelephonyManager.getIccSubSize(sub);
        if (cardIsUsim(sub)) {
            return 500;
        } else {
            return 250;
    ...
    public static int getContactsInIcc(Context context, int sub) {
        ...
        if (isMultiSimEnabled()) {
            indicatePhoneSim = ContactsUtils.getSubIdbySlot(sub);
        }
		...
		if (isOnlyQcContactsSupport()) {
			if (GNContactsUtils.isMultiSimEnabled()) {
				if (SimCardUtils.isSimStateReady(0)
						&& SimCardUtils.isSimStateReady(1)) {
