...
public class AuroraCallLogAdapterV2 extends CursorAdapter implements
	...
	private boolean isCheckAnim = false;
        ...
        if(!contactInfo.isInitDisplay){
    		...
    		if (isEmergencyNumber || isVoiceMailNumber) {
    			...
    			if (OperatorUtils.getActualOptrProperties().equals("OP02")) {
    				...
    				contactInfo.photoId = SimCardUtils.isSimUsimType(contactInfo.contactSimId) ? -2
    						: -1;
    			}
    		}
    		if (TextUtils.isEmpty(contactInfo.name)) {
    		...
    		contactInfo.callTypeRes = mCallTypeResHolder.getCallTypeDrawable(
    				contactInfo.type,
    				ContactsUtils.getSlotBySubId(contactInfo.simId));
	...
	public float getCharacterWidth(TextView tv, String text){
	 ...
	 String mCountText="";
	 private static final int DEFAULT_LISTVIE_COUNT = 500;
	 private static final int DEFAULT_LISTVIE_COUNT_MORE = 200;
	  
	 int mCount = DEFAULT_LISTVIE_COUNT;
	 private static final boolean mIsSlideDownToShowMore = false;
