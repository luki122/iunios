...
public class KindSectionView extends LinearLayout implements EditorListener {
    ...
    public void onDeleteRequested(Editor editor) {
        ...
        if (getEditorCount() == 1) {
        	...
        	// aurora ukiliu 2013-10-16 add for aurora ui begin
        	if (editor instanceof TextFieldsEditorView) {
        		int mIndex = ((TextFieldsEditorView)editor).getIndex();
	        	if (getEditorCount() == 2) {
	        		if (mIndex == POSITION_TOP) {
                		((TextFieldsEditorView)(mEditors.getChildAt(1))).setIndex(POSITION_SINGLE);
                	} else if (mIndex == POSITION_BOTTOM) {
                		((TextFieldsEditorView)(mEditors.getChildAt(getEditorCount() - 2))).setIndex(POSITION_SINGLE);
                	}

	        	} else if (getEditorCount() > 2) {
	        		
                	if (mIndex == POSITION_TOP) {
                		((TextFieldsEditorView)(mEditors.getChildAt(1))).setIndex(POSITION_TOP);
                	} else if (mIndex == POSITION_BOTTOM) {
                		((TextFieldsEditorView)(mEditors.getChildAt(getEditorCount() - 2))).setIndex(POSITION_TOP);
                		(mEditors.getChildAt(getEditorCount() - 2)).setBackgroundResource(R.drawable.translucent_background);
                	}
	        	}
        	}
        	// aurora ukiliu 2013-10-16 add for aurora ui end
        ...
        if (ContactsApplication.auroraContactsSupport) {
        	...
        	if (count > 0) {
        		...
        		if (view instanceof TextFieldsEditorView) {
        			((TextFieldsEditorView)(mEditors.getChildAt(0))).setIndex(POSITION_TOP);
        			((TextFieldsEditorView) view).setIndex(POSITION_BOTTOM);
        			if (count > 1) {
        				((TextFieldsEditorView)(mEditors.getChildAt(count-1))).setIndex(POSITION_MIDDLE);
        			}
        		}
        	}
        }
    ...
    private void updateEmptyEditors() {
        ...
        if (emptyEditors.size() > 1) {
            for (View emptyEditorView : emptyEditors) {
            	int mIndex = POSITION_SINGLE;
            	if (emptyEditorView instanceof TextFieldsEditorView) {
            		mIndex = ((TextFieldsEditorView)emptyEditorView).getIndex();
            	}
                ...
                if (emptyEditorView.findFocus() == null) {
                    ...
                    count = getEditorCount();
                    if (count == 1) {
                    	if (mIndex == POSITION_TOP || mIndex == POSITION_BOTTOM) {
                    		if (null != mEditors && null != mEditors.getChildAt(0)) {
                    			((TextFieldsEditorView)(mEditors.getChildAt(0))).setIndex(POSITION_SINGLE);
                    		}
                    	}
                    } else if (count > 1) {
                    	...
                    	if (mIndex == POSITION_TOP) {
                    		...
                    		if (null != mEditors && null != mEditors.getChildAt(count - 1)) {
                    		    ((TextFieldsEditorView)(mEditors.getChildAt(count - 1))).setIndex(POSITION_BOTTOM);
                    		}
                    	}	
                    }
                    
                    if (null != mEditors && null != mEditors.getChildAt(count - 1)) {
                    	(mEditors.getChildAt(count - 1)).setBackgroundResource(R.drawable.translucent_background);
                    }
                }
            }
        }
