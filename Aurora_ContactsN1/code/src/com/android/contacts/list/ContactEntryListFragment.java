...
public abstract class ContactEntryListFragment<T extends ContactEntryListAdapter>
    ...
    private boolean mUserProfileExists;
...
//		   try {
//	            mContext.getContentResolver().query(Uri.parse("content://com.android.contacts/dialer_search_init"),
//	                    null, null, null, null);
//	        } catch (Exception e) {
//	            e.printStackTrace();
//	        }
...
//                    mSelection = " mimetype_id = 5 ";
//                } else {
//                    mSelection = "(" + mSelection + ") and mimetype_id = '5' ";
//                }
//            }
//            
//            if (!mMutliPhoneSearch) {
//            	if (TextUtils.isEmpty(mSelection)) {
//                    mSelection = " mimetype_id = 7 ";
//                } else {
//                    mSelection = "(" + mSelection + ") and mimetype_id = '7' ";
//                }
//            }
            
            
            try {
            	mCursor = mContext.getContentResolver().query(Uri.parse("content://com.android.contacts/aurora_dialer_search/" +
    ...
    private void initSearchViewSelection() {
            ...
            if (extra.startsWith("withGroupId/")) {
                ...
                if (extra.isEmpty()) {
                    ...
                    if (filter != null) {
                        ...
                        if (isCustomMode()) {
                            selection.append(" AND in_visible_group=1 ");
                        } else {
                        selection.append(" AND ( account_name='" + filter.accountName + 
                                "' AND account_type='" + filter.accountType + "')");
                        }
                    }
                }
            } else if (extra.startsWith("simContacts/")) {
                selection.append("(index_in_sim >= 0)");
                noCustomOrLocal = true;
            } else if (extra.startsWith("auto_record/")) {
                selection.append("(auto_record = 1 AND vds_indicate_phone_sim = -1)");
                noCustomOrLocal = true;
                privacyDialerSearch = true;
                
                long privacyId = AuroraPrivacyUtils.mCurrentAccountId;
            	if (privacyId > 0) {
            		selection.append(" and(is_privacy > -1)");
            	} else {
            		selection.append(" and(is_privacy = 0)");
            	}
            } else if (extra.startsWith("add_to_auto_record/")) {
                selection.append("(auto_record = 0 AND vds_indicate_phone_sim = -1)");
                noCustomOrLocal = true;
                privacyDialerSearch = true;
            ...
            if (null != selection) {
                if (!noCustomOrLocal) {
                    if (isCustomMode()) {
                        ...
                        selection.append(" AND ( account_name='" + AccountType.ACCOUNT_NAME_LOCAL_PHONE + 
                                "' AND account_type='" + AccountType.ACCOUNT_TYPE_LOCAL_PHONE + "')");
                    }
                }
        ...
        if (isCustomMode()) {
            ...
            selection.append("( account_name='" + AccountType.ACCOUNT_NAME_LOCAL_PHONE + 
                    "' AND account_type='" + AccountType.ACCOUNT_TYPE_LOCAL_PHONE + "')");
        }
    ...
    private StringBuilder initPrivacySelect(StringBuilder selection) {
    	if (ContactsApplication.sIsAuroraPrivacySupport) {
    		if (!TextUtils.isEmpty(selection) && !selection.toString().contains("is_privacy")) {
    			selection.append(" AND is_privacy=0");
    		} else {
    			selection.append(" is_privacy=0");
    		}
