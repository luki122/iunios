    	...
    	if (hasAlternateAction) {               
        	...
        	if (simId > -1) {            	
                ...
                if (txtAssociationSimName != null) { 
                    ...
                    SubscriptionInfo subInfo = SubscriptionManager.from(ContactsApplication.getInstance()).getSubscriptionInfo(simId);
                	if (subInfo != null) {    
                    	txtAssociationSimName.setText(ContactsUtils.getSimDisplayNameById(simId));          
                        int slotId = ContactsUtils.getSlotBySubId(simId);
                        Log.d(TAG,"slotId = "+slotId);
                        if(slotId>=0){
                            Log.d(TAG,"slotId >= 0 ");
                            txtAssociationSimName.setBackgroundResource(0);
                        } else {
                            Log.d(TAG,"slotId < 0 ");
