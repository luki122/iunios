...
public class EditSimContactActivity extends AuroraActivity {
    ...
    public void onCreate(Bundle savedState) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
        	if (mSlotId > -1) {
        		indicate = ContactsUtils.getSubIdbySlot(mSlotId);
        ...
        if (mSlotId == -1) {  
            mSlotId = ContactsUtils.getSlotBySubId((int)indicate);
        }
            ...
            if (mode == MODE_INSERT) {
                ...
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    try {
                        ...
                        if (iTel != null && !AuroraITelephony.isRadioOnGemini(iTel, mSlotId)) {
                            mAirPlaneModeOn = true;
                        }
                        if (iTel != null && AuroraITelephony.isFDNEnabledGemini(iTel, mSlotId)) {
                            mFDNEnabled = true;
                        }
                        /*
                        ...
                        */
                        if (!(TelephonyManager.SIM_STATE_READY == AuroraTelephonyManager.getSimStateGemini(mSlotId))) {
                            mSIMInvalid = true;
                        }
                    ...
                    try {
                        ...
                        if (iTel != null && AuroraITelephony.isFDNEnabled(iTel)) {
                            mFDNEnabled = true;
                        }
                        if (!(TelephonyManager.SIM_STATE_READY == TelephonyManager.getDefault()
                                .getSimState())) {
...
//                    }
                ...
                checkUri = resolver.insert(ContactsUtils.getSimContactsUri((int)indicate,
                		SimCardUtils.isSimUsimType((int)indicate)), values);
                ...
                */
                ...
                if (mSimType == "SIM") {
                    if (TextUtils.isEmpty(updateName) && TextUtils.isEmpty(updatephone)) {// if
                        ...
                        String where;
                        // Aurora xuyong 2015-07-13 modified for bug #14161 start
                        Uri iccUri = ContactsUtils.getSimContactsUri((int)indicate,
                        		SimCardUtils.isSimUsimType((int)indicate));//SubContactsUtils.getUri(mSlotId);//aurora change zhouxiaobing 20140606
                        // Aurora xuyong 2015-07-13 modified for bug #14161 end
...
//                    }
                ...
                // Aurora xuyong 2015-07-13 modified for bug #14161 start
                cursor = resolver.query(/*SubContactsUtils.getUri(mSlotId)*/ContactsUtils.getSimContactsUri((int)indicate,
                		SimCardUtils.isSimUsimType((int)indicate)),
                        ADDRESS_BOOK_COLUMN_NAMES, null, null, null);
                // Aurora xuyong 2015-07-13 modified for bug #14161 end
                if (cursor != null) {
                	// Aurora xuyong 2015-07-13 modified for bug #14161 start
                    result = resolver.update(/*SubContactsUtils.getUri(mSlotId)*/ContactsUtils.getSimContactsUri((int)indicate,
                    		SimCardUtils.isSimUsimType((int)indicate)), updatevalues, null,
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                ...
                try {
                    if (null != iTel
                            ...
                            */
                            && !AuroraITelephony.isRadioOnGemini(iTel, ContactsFeatureConstants.GEMINI_SIM_1)) {
                        sim1RadioOn = false;
                    }
                    if (null != iTel
                            && !AuroraITelephony
                    ...
                    Log
                            .i(
                                    ...
                                    "TelephonyManager.getDefault().getSimStateGemini(com.android.internal.telephony.Phone.GEMINI_SIM_1) is "
                                            + AuroraTelephonyManager.getSimStateGemini(ContactsFeatureConstants.GEMINI_SIM_1));
                    sim1RadioOn = true;
    ...
    PhoneStateListener mPhoneStateListener2 = new PhoneStateListener() {
        ...
        public void onServiceStateChanged(ServiceState serviceState) {
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                ...
                try {
                    if (null != iTel
                            && !AuroraITelephony.isRadioOnGemini(iTel, ContactsFeatureConstants.GEMINI_SIM_2)) {
                        sim2RadioOn = false;
                    }
                    if (null != iTel
                            && !AuroraITelephony.hasIccCardGemini(iTel, ContactsFeatureConstants.GEMINI_SIM_2)) {
                        hasSim2Card = false;
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                    ...
                    Log
                            .i(
                                    ...
                                    "TelephonyManager.getDefault().getSimStateGemini(com.android.internal.telephony.Phone.GEMINI_SIM_2) is "
                                            + AuroraTelephonyManager.getSimStateGemini(ContactsFeatureConstants.GEMINI_SIM_2));
                    sim2RadioOn = true;
    ...
    private void listenPhoneStates() {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
            AuroraTelephonyManager.listenGemini(mPhoneStateListener2,
                    PhoneStateListener.LISTEN_SERVICE_STATE,
                    ContactsFeatureConstants.GEMINI_SIM_2);
            AuroraTelephonyManager.listenGemini(mPhoneStateListener,
                    PhoneStateListener.LISTEN_SERVICE_STATE,
    ...
    private void stopListenPhoneStates() {
        ...
        TelephonyManager telephonyManager = (TelephonyManager) getSystemService(TELEPHONY_SERVICE);
        if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
            AuroraTelephonyManager.listenGemini(mPhoneStateListener, PhoneStateListener.LISTEN_NONE,
                    ContactsFeatureConstants.GEMINI_SIM_1);
            AuroraTelephonyManager.listenGemini(mPhoneStateListener2, PhoneStateListener.LISTEN_NONE,
                    ContactsFeatureConstants.GEMINI_SIM_2);
        } else {
