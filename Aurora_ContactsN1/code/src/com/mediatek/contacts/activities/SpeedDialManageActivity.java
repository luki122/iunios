    ...
    private boolean isSimReady(final int simId) {
        ...
        try {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {
                int slotId = ContactsUtils.getSlotBySubId(simId);
                Log.d(TAG, "isSimReady(), slotId=  " + slotId);
                if (-1 == slotId)
                    return true;
                boolean meLock = TelephonyManager.SIM_STATE_READY == AuroraTelephonyManager.getSimStateGemini(slotId);//qc--mtk

                Log
                        .d(TAG, "isSimReady(), mITel.isSimInsert(slotId)=  "
                                //qc--mtk
                                //+ mITel.isSimInsert(slotId));
                                + AuroraITelephony.isSimInsert(mITel, slotId));
                Log.d(TAG, "isSimReady(), mITel.isRadioOnGemini(slotId)=  "
                        //qc--mtk
                        //+ mITel.isRadioOnGemini(slotId));
                        + AuroraITelephony.isRadioOnGemini(mITel, slotId));
                Log.d(TAG, "isSimReady(), mITel.isFDNEnabledGemini(slotId)=  "
                        //qc--mtk
                        //+ mITel.isFDNEnabledGemini(slotId));
                        + AuroraITelephony.isFDNEnabledGemini(mITel, slotId));
                ...
                */
                if (AuroraITelephony.isRadioOnGemini(mITel, slotId)) {
                    return !AuroraITelephony.hasIccCardGemini(mITel, slotId) || (AuroraITelephony.isRadioOnGemini(mITel, slotId)
                    && !AuroraITelephony.isFDNEnabledGemini(mITel, slotId) 
                    && meLock && !ContactsUtils.isServiceRunning[slotId]);

                } else {
                    return !AuroraITelephony.isSimInsert(mITel, slotId) || (AuroraITelephony.isRadioOnGemini(mITel, slotId)
                     && !AuroraITelephony.isFDNEnabledGemini(mITel, slotId) 
                    && meLock && !ContactsUtils.isServiceRunning[slotId]);
                }
            } else {
...
//                                    && TelephonyManager.SIM_STATE_READY == TelephonyManager
//                                            .getDefault().getSimState() && !ContactsUtils.isServiceRunning[0]);
                    return !AuroraITelephony.isSimInsert(mITel, 0)
                            || (mITel.isRadioOn()
                                    /* && !mITel.isFDNEnabled() */
                                    && TelephonyManager.SIM_STATE_READY == TelephonyManager
                                            .getDefault().getSimState() && !ContactsUtils.isServiceRunning[0]);
    ...
    public long getSimType(int indicate) {
        long photoId = 0;
        mSlot = ContactsUtils.getSlotBySubId(indicate);
        if (null == mITelephony) {
