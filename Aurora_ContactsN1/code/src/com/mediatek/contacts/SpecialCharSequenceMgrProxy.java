...
public class SpecialCharSequenceMgrProxy {
    ...
    private static final boolean gnSalestrackflag = SystemProperties.get("ro.gn.oversea.salestrack").equals("yes");
    //GIONEE:chenxiong 2012-05-31 add end for CR00608860
		
    //Gionee liming 2012-06-10 add for CR00623425 begin
     
    private static final boolean gnSpyflag = SystemProperties.get("ro.gn.oversea.custom").equals("BANGLADESH_SYMPHONY");
   ...
   //Gionee liming 2012-06-10 add for CR00623293 begin //tangzepeng 2012-8-30 merge begin 
    ...
    static void showIMEIPanel(Context context, boolean useSystemWindow) {
        if (TelephonyManager.from(context).isMultiSimEnabled()) {
            ...
            imeiStrs[0] = AuroraTelephonyManager.getDeviceIdGemini(ContactsFeatureConstants.GEMINI_SIM_1);
            
            /*imeiStrs[1] = telephonyManager.getDeviceIdGemini(Phone.GEMINI_SIM_2);*/
			if (!mSingleIMEI) {
				imeiStrs[1] = AuroraTelephonyManager.getDeviceIdGemini(ContactsFeatureConstants.GEMINI_SIM_2);
			}
        ...
        if(TelephonyManager.from(context).isMultiSimEnabled()) {
            ...
            if ((len > 1) && (len < 5) && (input.endsWith("#"))) {
                try {
                    ...
                    final int defaultSim = Settings.Global.getInt(context.getContentResolver(), ContactsFeatureConstants.VOICE_CALL_SIM_SETTING, -3);
                    ...
                    int simCount = ContactsUtils.getActiveSubscriptionInfoCount();

                    int slot = ContactsUtils.getSlotBySubId((int)defaultSim);
                    log("slot = " + slot );
                    ...
                    Uri uri = buildSIMContactQueryUri(slot);
    ...
    static Uri buildSIMContactQueryUri(int slot) {
    	int subId = ContactsUtils.getSubIdbySlot(slot);
    	return ContactsUtils.getSimContactsUri(subId, SimCardUtils.isSimUsimType(subId));
    }
	...
	static boolean fdnRequest(int slot) {
		...
		try {
			if (true == FeatureOption.MTK_GEMINI_SUPPORT) {
				bRet = AuroraITelephony.isFDNEnabledGemini(iTel, slot);
			} else {
        ...
        switch(id) {
            case INTER_VER_DIALOG:{
				...
				TelephonyManager teleMgr = (TelephonyManager) context
						.getSystemService(Context.TELEPHONY_SERVICE);				
				String sn =AuroraTelephonyManager.getSN();//aurora change zhouxiaobing 20140211
