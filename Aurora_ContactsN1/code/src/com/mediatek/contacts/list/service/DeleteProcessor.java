    ...
    private void runInternal() {
        ...
        for (MultiChoiceRequest request : mRequests) {
            ...
            if (request.mIndicator > RawContacts.INDICATE_PHONE) {
                int slot = ContactsUtils.getSlotBySubId((int)request.mIndicator);
                ...
                Uri delSimUri = /*SubContactsUtils.getUri(slot)*/ContactsUtils.getSimContactsUri((int)request.mIndicator, 
                		SimCardUtils.isSimUsimType((int)request.mIndicator));
    ...
    private boolean isReadyForDelete(int slotId) {
        ...
        try {
            if (FeatureOption.MTK_GEMINI_SUPPORT) {  // Gemini
                return AuroraITelephony.hasIccCardGemini(iTel, slotId)
                && AuroraITelephony.isRadioOnGemini(iTel, slotId)
                && !AuroraITelephony.isFDNEnabledGemini(iTel, slotId)
                && SubContactsUtils.checkPhbReady(slotId)
                && TelephonyManager.SIM_STATE_READY == AuroraTelephonyManager.getSimStateGemini(slotId);
            } else {    // Single SIM
