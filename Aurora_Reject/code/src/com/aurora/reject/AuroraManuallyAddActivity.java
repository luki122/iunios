...
//	 			}
				...
				Uri uris = cr.insert(uri, cv);
	 			...
	 			new TotalCount(AuroraManuallyAddActivity.this, "180", "addbl", 1).CountData();
			}else{
				ContentResolver cr = getContentResolver();
     			ContentValues cv = new ContentValues();
     			cv.put("isblack", 1);
...
//     			}
 				...
 				int update = cr.update(uri, cv, "_ID=?", new String[] { targetId });
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setAuroraContentView(R.layout.manually_add,
		...
		phone.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!sms.isChecked()&&!phone.isChecked()){
					phone.setChecked(true);
					Toast.makeText(AuroraManuallyAddActivity.this, AuroraManuallyAddActivity.this.getResources().getString(R.string.at_least_one), Toast.LENGTH_LONG).show();
				}else{
		...
		PhoneTextWatcher=new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				// TODO Auto-generated method stub
				add_number=number.getText().toString().replace("-", "").replace(" ", "");
				if(add_number!=null||!"".equals(add_number)){
					if(contactName==null){
						bef=name.getText().toString();
					}
					contactName=getContactNameByPhoneNumber(AuroraManuallyAddActivity.this,add_number);
					if (contactName != null) {
		...
		mActionBar.getCancelButton().setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if ((number.getText().toString()==null||"".equals(number.getText().toString()))
						&& (name.getText().toString()==null||"".equals(name.getText().toString()))
						&& phone.isChecked()
						&& sms.isChecked()
						&& !mIsAddFromOther) {
					 AuroraManuallyAddActivity.this.finish();
					...
					AuroraAlertDialog dialogs = new AuroraAlertDialog.Builder(
							AuroraManuallyAddActivity.this).setTitle(
			...
			public void onClick(View v) {
				...
				if (add_number!=null&&!"".equals(add_number)) {
					 boolean isDigit=false;
     				 ...
     				 if(!isDigit){
     					Toast.makeText(AuroraManuallyAddActivity.this, AuroraManuallyAddActivity.this.getResources().getString(R.string.format), Toast.LENGTH_LONG).show();
     					touch=false;
						return; 
     				 }
         			if (bundle==null || mIsAddFromOther) {
         				List<String> blaclist = RejectApplication.getInstance().getBlackList();
    					if(blaclist == null || !blaclist.contains(add_number)){
    						lable=YuloreUtil.getUserMark(AuroraManuallyAddActivity.this, add_number);
    						...
    						if(lable==null){
    							...
    							new Thread(){
    								public void run() {
    									try {
        	    							lable=YuloreUtil.getMarkContent(add_number,AuroraManuallyAddActivity.this);
        	    							userMark = YuloreUtil.getMarkNumber(AuroraManuallyAddActivity.this, add_number);
    						...
    						Toast.makeText(AuroraManuallyAddActivity.this, AuroraManuallyAddActivity.this.getResources().getString(R.string.black_number_exist), Toast.LENGTH_LONG).show();
    						touch=false;
    						return;
    					}
         				
         			}else{
         				List<String> lists=RejectApplication.getInstance().getBlackList();
         				String str=null;
         				...
         				if ((mNumberOrig != null && !mNumberOrig.equals(add_number) 
         						&& lists.contains(add_number))) {
         					Toast.makeText(AuroraManuallyAddActivity.this, AuroraManuallyAddActivity.this.getResources().getString(R.string.black_number_exist), Toast.LENGTH_LONG).show();
         					touch=false;
    						return;
         				}
         				
         				if(mNumberOrig != null || !RejectApplication.getInstance().getBlackList().contains(add_number)){
         					lable=YuloreUtil.getUserMark(AuroraManuallyAddActivity.this, add_number);
					...
					Toast.makeText(AuroraManuallyAddActivity.this, AuroraManuallyAddActivity.this.getResources().getString(R.string.no_number), Toast.LENGTH_LONG).show();
					touch=false;
					return;
				}
			}
...
//	}
	...
	private String getContactNameByPhoneNumber(Context context,
			String address) {
				...
				if(!cursor.moveToFirst()){
					...
					do{
						...
						String numberE164 = PhoneNumberUtils.formatNumberToE164(
				                number, AuroraBlackNameActivity.getCurrentCountryIso(context));
						...
						try { // modify in the future
				        	if (numberE164 != null && numberE164.equals(number)
				        			&& AuroraBlackNameActivity.getCurrentCountryIso(context).equals("CN") && number.startsWith("+86")) {
