...
//			removeDialog(DIALOG_REFRESH);
	...
	private Handler smsHandler=new Handler(){
		public void handleMessage(android.os.Message msg) {
			String[] tableCursor = new String[] {"_id","address", "date","body","count","thread_id","ismms","name"};
			cursors = new MatrixCursor(tableCursor);
			for(int i=0;i<smsList.size();i++){
				cursors.addRow(new Object[] { i,smsList.get(i).getDBPhomeNumber(), smsList.get(i).getLastDate(),smsList.get(i).getBody(), smsList.get(i).getCount(), smsList.get(i).getThread_id(), smsList.get(i).getIsMms(),smsList.get(i).getName() });
			}
...
//	        .build());
		...
		// addAuroraActionBarItem(AuroraActionBarItem.Type.Edit,
		// ACTION_BTN_SET);
		contentResolver=getContentResolver();
		mActionBar.addItem(R.drawable.aurora_set_selector, ACTION_BTN_SET, "ccc");
		...
		// 打开滑动删除功能
		phone_list.auroraSetNeedSlideDelete(true);
		phone_list
				.auroraSetAuroraBackOnClickListener(new AuroraListView.AuroraBackOnClickListener() {
					...
					public void auroraOnClick(final int position) {// 点击了垃圾桶的响应事件
		                ...
		                .setTitle(AuroraRejectActivity.this.getResources().getString(R.string.del_phone_record))
		                ...
		                .setPositiveButton(android.R.string.ok,
		                    new DialogInterface.OnClickListener() {
		                        ...
		                        public void onClick(DialogInterface dialog, int whichButton) {
		                        	 ...
		                        	 isShowing=true;
									 showDialog();
		                        	 phone_list.auroraDeleteSelectedItemAnim();
		                        	 phone_list.auroraSetDeleteItemListener(new AuroraDeleteItemListener() {
											   ...
											   new Thread(){
												  public void run() {
													  Cursor pcursor = (Cursor) phone_list
							     						...
							     						String p=pcursor.getString(pcursor.getColumnIndex("number"));
							     						String numberE164 = PhoneNumberUtils.formatNumberToE164(
							     								p, getCurrentCountryIso(AuroraRejectActivity.this));
							     						if(numberE164!=null){
							     							contentResolver.delete(uri, "number=? and type in (1,3) and reject=1",new String[] {numberE164});
							     						}
//							     						mQueryHandler.startDelete(1, null, uri, "number=? and type in (1,3) and reject=1",new String[] { p });
							     						contentResolver.delete(uri, "number=? and type in (1,3) and reject=1",new String[] { p });
					...
					public void auroraPrepareDraged(int position) {
						View view=phone_list.getChildAt(position-phone_list.getFirstVisiblePosition());
						if(view!=null){
							RelativeLayout front = (RelativeLayout)view.findViewById(com.aurora.R.id.aurora_listview_front);
							ImageView imageView=(ImageView)((LinearLayout)((RelativeLayout)(front.getChildAt(0))).getChildAt(2)).getChildAt(1);
		...
		// 打开滑动删除功能
		sms_list.auroraSetNeedSlideDelete(true);
		sms_list
				.auroraSetAuroraBackOnClickListener(new AuroraListView.AuroraBackOnClickListener() {
					...
					public void auroraOnClick(final int position) {// 点击了垃圾桶的响应事件
						AuroraAlertDialog dialogs = new AuroraAlertDialog.Builder(AuroraRejectActivity.this)
		                .setTitle(AuroraRejectActivity.this.getResources().getString(R.string.del_sms_record))
		                ...
		                .setPositiveButton(android.R.string.ok,
		                    new DialogInterface.OnClickListener() {
		                        ...
		                        public void onClick(DialogInterface dialog, int whichButton) {
		                        	 ...
		                        	 isShowing=true;
									 showDialog();
		                        	 sms_list.auroraDeleteSelectedItemAnim();
		                        	 sms_list.auroraSetDeleteItemListener(new AuroraDeleteItemListener() {
...
//			mActionBar.setBackgroundColor(getResources().getColor(R.color.actionbar_disable));
			...
			if(currIndex==0){
				...
				calls.setSelected(false);
				setAuroraMenuCallBack(auroraMenuCallBack); // 必须写在布局前
	...
	private OnAuroraMenuItemClickListener auroraMenuCallBack = new OnAuroraMenuItemClickListener() {
		...
		public void auroraMenuItemClick(int arg0) {
			...
			case R.id.no_reject:
				...
				phone.setChecked(true);
				  AuroraAlertDialog dia = new AuroraAlertDialog.Builder(AuroraRejectActivity.this)
	                .setTitle(AuroraRejectActivity.this.getResources().getString(R.string.no_reject))
	                .setView(view)
	                .setPositiveButton(android.R.string.ok,
	                    new DialogInterface.OnClickListener() {
	                        @Override
			...
			case R.id.showSms:
                ...
                .setMessage(AuroraRejectActivity.this.getResources().getString(R.string.confirm_recover_sms))
                .setPositiveButton(android.R.string.ok,
                    new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int whichButton) {
		...
		auroraBatch = new AuroraBatch<Integer>(mActionBar, this) {
			...
			public void bindToAdapter(SelectionManager<Integer> selectionManager) {
				if(currIndex==0){
					...
					mSmsListAdapter.setSelectionManager(selectionManager);
				}
