...
public class AnalogClock extends ImageView {
    ...
    */
    ...
    public AnalogClock(Context context, AttributeSet attrs,
        ...
        mDial = r.getDrawable(R.drawable.gn_appwidget_clock_dial);
        mHourHand = r.getDrawable(R.drawable.gn_appwidget_clock_hour);
        mMinuteHand = r.getDrawable(R.drawable.gn_appwidget_clock_minute);
        mSecondHand = r.getDrawable(R.drawable.gn_appwidget_clock_second);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!mNoSeconds) {
            ...
            if (changed) {
                ...
                secondHand.setBounds(x - (w / 2), y - (h / 2) - UsefulUtils.dip2px(mContext, 36), x + (w / 2), y + (h / 2) - UsefulUtils.dip2px(mContext, 36));
            }
        ...
        if (changed) {
            ...
            minuteHand.setBounds(x - (w / 2), y - (h / 2) - UsefulUtils.dip2px(mContext, 32), x + (w / 2), y + (h / 2) - UsefulUtils.dip2px(mContext, 32));
        }
        ...
        if (changed) {
            ...
            hourHand.setBounds(x - (w / 2), y - (h / 2) - UsefulUtils.dip2px(mContext, 23), x + (w / 2), y + (h / 2) - UsefulUtils.dip2px(mContext, 23));
        }
  ...
  //      long millis = System.currentTimeMillis() % 1000;
    ...
    private static final int MSG_TIMEZONE_CHANGE = 0X1234 , MSG_NORMAL = 0X4575;
    ...
    private Handler handler = new Handler(){
    	...
    	public void handleMessage(android.os.Message msg) {
    		...
    		switch (msg.what) {
			case MSG_TIMEZONE_CHANGE:
				String tz = (String) msg.obj;
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Intent.ACTION_TIMEZONE_CHANGED)) {
                ...
                msg.obj = tz;
                msg.what = MSG_TIMEZONE_CHANGE;
                handler.sendMessage(msg);
            }else{
            	handler.sendEmptyMessage(MSG_NORMAL);
            }
        }
    };
    ...
    private final Runnable mClockTick = new Runnable () {
        ...
        public void run() {
            onTimeChanged();
            invalidate();
            //Log.e("--mClockTick--tick----");
            //AnalogClock.this.postDelayed(mClockTick, 500);
            long now = SystemClock.uptimeMillis();  
            long next = now + (1000 - now % 1000);
            mHandler.postAtTime(mClockTick, next);
        }
    };
