...
public final class GeometryMathUtils {
    ...
    public static final float SHOW_SCALE = 1f;//Aurora <paul> <2014-06-13> .9f;
...
//                frame.height(), BitmapCache.UTIL_GEOMETRY);
        //SQF MODIFIED TO:
        Matrix m = getCropSelectionToScreenMatrix(null, holder, width, height, (int)crop.width(), (int)crop.height());
        BitmapCache bitmapCache = MasterImage.getImage().getBitmapCache();
		int w = (int)crop.width();
		int h = (int)crop.height();
		if(w <= 0 || h <= 0){
			Log.w(TAG, "w <= 0 || h <= 0 return");
			return image;//paul add
		}
		...
		int tmpH = h;
        while (noBitmap) {
			temp = bitmapCache.getBitmap(tmpW, tmpH, BitmapCache.UTIL_GEOMETRY);
			if(null == temp){
				Log.w(TAG, "num_tries : " + num_tries);
				if(++num_tries >= 5) return image;
				System.gc();
				tmpW /= 2;
				tmpH /= 2;
				if(tmpW <= 0 || tmpH <= 0) return image;
			}else{
				noBitmap = false;
			}
        }
		if(null == temp) return image;
		if(num_tries > 0){
			float scale = (float)tmpW / w;
	...
	//paul add
    public static Matrix getFullGeometryToScreenMatrix(GeometryHolder holder, int bitmapWidth,
            int bitmapHeight, int viewWidth, int viewHeight, int margin) {
        int bh = bitmapHeight;
        int bw = bitmapWidth;
		viewWidth -= 2 * margin;
		viewHeight -= 2 * margin;
        ...
        Matrix m = getFullGeometryMatrix(holder, bitmapWidth, bitmapHeight);
        m.postScale(scale, scale);
        m.postTranslate(viewWidth / 2f + margin, viewHeight / 2f + margin);
        return m;
    }
