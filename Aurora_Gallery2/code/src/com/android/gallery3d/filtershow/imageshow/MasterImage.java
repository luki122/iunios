...
public class MasterImage implements RenderingRequestCaller {
    ...
    private int mShadowMargin = 30; // not scaled, fixed in the asset
	...
	//paul add start
    private float mScaleFactorSave = 1.0f;
    private Point mTranslationSave = new Point();
    private Point mOriginalTranslationSave = new Point();
	private FilterRepresentation mCurrentFilterRepresentationSave;
	private boolean mNeedLoad = false;
	private boolean mRotated = false;
	private boolean mUpdating = false;
	private boolean mShowing = false;
	
	//Aurora <SQF> <2014-10-09>  for NEW_UI begin
	private boolean mIsAuroraEffectUpdating = false;
	
    public boolean isAuroraEffectUpdating() {
		return mIsAuroraEffectUpdating;
	}

	public void setIsAuroraEffectUpdating(boolean isAuroraEffectUpdating) {
		this.mIsAuroraEffectUpdating = isAuroraEffectUpdating;
	}
	//Aurora <SQF> <2014-10-09>  for NEW_UI end
    ...
    public void resetValue() {
		...
		mTranslation.x = 0;
		mTranslation.y = 0;
		mOriginalTranslation.x = 0;
		mOriginalTranslation.y = 0;
		mOriginalTranslation.x = 0;
		mOriginalTranslation.y = 0;
	...
	public void save(){
		mNeedLoad = true;
		mScaleFactorSave = mScaleFactor;
		mTranslationSave.set(mTranslation.x, mTranslation.y);
		mOriginalTranslationSave.set(mOriginalTranslation.x, mOriginalTranslation.y);
	}
	public boolean load(){
		if(mNeedLoad){
			mNeedLoad = false;
			if(MasterImage.getImage().getAndResetRotated()){
				return false;
			}
			mScaleFactor = mScaleFactorSave;
			mTranslation.set(mTranslationSave.x, mTranslationSave.y);
			mOriginalTranslation.set(mOriginalTranslationSave.x, mOriginalTranslationSave.y);
			//invalidatePreview();
			return true;
		}
		mRotated = false;
	...
	public boolean getAndResetRotated(){
		if(mRotated){
			mRotated = false;
			return true;
		}
    ...
    //Aurora <SQF> <2014-07-26>  for NEW_UI begin
    private boolean isPreviewUpdateFinish = false;
    
	public boolean isPreviewUpdateFinish() {
		return isPreviewUpdateFinish;
	}

	public void setPreviewUpdateFinish(boolean isPreviewUpdateFinish) {
		this.isPreviewUpdateFinish = isPreviewUpdateFinish;
	}
	//Aurora <SQF> <2014-07-26>  for NEW_UI end
    ...
    public void setOriginalBounds(Rect r) {
		//paul add
        //Aurora <SQF> <2014-07-08>  for NEW_UI begin
        //ORIGINALLY:
    	/*
    	if(null != r && (ImageLoader.ORI_ROTATE_90 == mZoomOrientation
    			|| ImageLoader.ORI_ROTATE_270 == mZoomOrientation)){
    			
    			Log.i("SQF_LOG", "MasterImage::setOriginalBounds mZoomOrientation: ImageLoader.ORI_ROTATE_90 || ImageLoader.ORI_ROTATE_270" );
    			int right = r.bottom;
    			r.bottom = r.right;
    			r.right = right;
    	}
    	*/
        //SQF MODIFIED TO:
    	if(null != r && (ImageLoader.ORI_ROTATE_90 == mOrientation || ImageLoader.ORI_ROTATE_270 == mOrientation)){
    			//Log.i("SQF_LOG", "MasterImage::setOriginalBounds mOrientation: ImageLoader.ORI_ROTATE_90 || ImageLoader.ORI_ROTATE_270" );
    			int right = r.bottom;
    			r.bottom = r.right;
    			r.right = right;
    	}
        //Aurora <SQF> <2014-07-08>  for NEW_UI end
        mOriginalBounds = r;
    }
    ...
    public void setActivity(FilterShowActivity activity) {
		...
		//paul add
		if(null != mActivity){
			mShadowMargin = mActivity.getResources().getDimensionPixelSize(R.dimen.crop_edge_margin);
		}
    }
	...
	*/
    ...
    public synchronized void setPreset(ImagePreset preset,
        ...
        //mActivity.updateCategories();//SQF ANNOTATED ON 2014-08-30
    }
    ...
    */
    ...
    public Matrix computeImageToScreen(Bitmap bitmapToDraw,
        ...
        if (applyGeometry) {
			...
			//Aurora <paul> <2014-06-13>
			scale = GeometryMathUtils.scale(size.width(), size.height(), mImageShowSize.x, mImageShowSize.y);
			/*
        ...
        m.postScale(getScaleFactor(), getScaleFactor(),
                mImageShowSize.x / 2.0f + mShadowMargin,
                mImageShowSize.y / 2.0f + mShadowMargin);
    ...
    public void needsUpdateHighResPreview() {
        ...
        //Aurora <SQF> <2014-07-03>  for vague super-high image begin
        //mActivity.getProcessingService().postHighresRenderingRequest(mPreset, getScaleFactor(), this);
        //Aurora <SQF> <2014-07-03>  for vague super-high image end
    ...
    public void available(RenderingRequest request) {
        ...
        if (request.getType() == RenderingRequest.FILTERS_RENDERING) {
            ...
            notifyObservers();
            needsCheckModification = true;
            //Aurora <SQF> <2014-10-10>  for NEW_UI begin
            if(mActivity.isLoadingVisible() && MasterImage.getImage().isAuroraEffectUpdating() ) {
            	mActivity.stopLoadingIndicator();//SQF ADDED ON 2014.10.09
            	MasterImage.getImage().setIsAuroraEffectUpdating(false);
            }
            //Aurora <SQF> <2014-10-10>  for NEW_UI end
        }
        if (request.getType() == RenderingRequest.PARTIAL_RENDERING
            ...
            //JUST FOR TEST
            //GalleryUtils.writeBitmapToFile(mPartialBitmap, "/mnt/sdcard/0101.jpg");
