...
public class ImageShow extends View implements OnGestureListener,
	...
	//protected int mShadowMargin = 15;
    protected int mShadowMargin = 30; // not scaled, fixed in the asset 
    protected boolean mShadowDrawn = false;
	...
	//paul add start
	protected RectF mCurrentInnerRect;
    ...
    protected FilterShowActivity mActivity = null;
    ...
    private void setupImageShow(Context context) {
		...
		mShadowMargin = MasterImage.getImage().getEdgeMargin();//paul add
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mActivity.isLoadingVisible() && getFilteredImage() != null) {
            if ((img.getLoadedPreset() == null)
            	...
            	if(! MasterImage.getImage().isAuroraEffectUpdating()) {//SQF ADDED ON 2014.10.09
        ...
        if (mPreview == null || isDoingNewLookAnimation) {
            drawImageAndAnimate(canvas, getFilteredImage());
        } else {
        	if(mStopDrawing && null != mPreview && null != mMatrix){
			    canvas.save();
				canvas.drawBitmap(mPreview, mMatrix, mPaint);
				canvas.restore();
			}else{
        ...
        if(!mStopDrawing && !isDoingNewLookAnimation){//wenyongzhe 2015.10.23 modify " if(!mStopDrawing ){"    for BUG 16400
    ...
    protected void drawHighresImage(Canvas canvas, Bitmap fullHighres) {
	...
	//paul add
    protected void drawHighresImageEx(Canvas canvas, Bitmap fullHighres) {
        Matrix originalToScreen = MasterImage.getImage().originalImageToScreen();
        if (fullHighres != null && originalToScreen != null) {
            Rect rBounds = new Rect();
            rBounds.set(MasterImage.getImage().getPartialBounds());
	...
	*/
    ...
    public boolean onTouchEvent(MotionEvent event) {
		...
		if(null == mCurrentInnerRect) return true;
		...
		if(scaleFactor > 1){
        	Point translation = MasterImage.getImage().getTranslation();
        	constrainTranslationEx(translation, scaleFactor);//constrainTranslation(translation, scaleFactor);
        	MasterImage.getImage().setTranslation(translation);
		}
        invalidate();
        return true;
	...
	//paul add start
	private void constrainTranslationImageShow(Point translation, float scale) {
    ...
    private void constrainTranslationEx(Point translation, float scale) {	
		if(mCurrentInnerRect == null){
			constrainTranslationImageShow(translation, scale);
			return;
		}
		...
		float width = mCurrentInnerRect.width();
		float height = mCurrentInnerRect.height();
        Matrix originalToScreen = MasterImage.getImage().originalImageToScreen();
        Rect originalBounds = MasterImage.getImage().getOriginalBounds();
        RectF screenPos = new RectF(originalBounds);
        originalToScreen.mapRect(screenPos);

        boolean rightConstraint = screenPos.right < mCurrentInnerRect.right;
        boolean leftConstraint = screenPos.left > mCurrentInnerRect.left;
        boolean topConstraint = screenPos.top > mCurrentInnerRect.top;
        boolean bottomConstraint = screenPos.bottom < mCurrentInnerRect.bottom;
   
        if (rightConstraint && !leftConstraint) {
            float tx = screenPos.right - translation.x * scale;
            translation.x = (int) ((mCurrentInnerRect.right - tx) / scale);
            currentEdgeEffect = EDGE_RIGHT;
        } else if (leftConstraint && !rightConstraint) {
            float tx = screenPos.left - translation.x * scale;
            translation.x = (int) ((mCurrentInnerRect.left - tx) / scale);
            currentEdgeEffect = EDGE_LEFT;
        }
        ...
        if (bottomConstraint && !topConstraint) {
            float ty = screenPos.bottom - translation.y * scale;
            translation.y = (int) ((mCurrentInnerRect.bottom - ty) / scale);
            currentEdgeEffect = EDGE_BOTTOM;
        } else if (topConstraint && !bottomConstraint) {
            float ty = screenPos.top - translation.y * scale;
            translation.y = (int) ((mCurrentInnerRect.top - ty) / scale);
            currentEdgeEffect = EDGE_TOP;
        }
        ...
        if (mCurrentEdgeEffect != currentEdgeEffect) {
            ...
            mEdgeEffect.setSize((int)width, mEdgeSize);
        }
        if (currentEdgeEffect != 0) {
            mEdgeEffect.onPull(mEdgeSize);
        }
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        boolean Constraint = screenPos.right < mCurrentInnerRect.right;
		Constraint |= screenPos.left > mCurrentInnerRect.left;
        Constraint |= screenPos.top > mCurrentInnerRect.top;
        Constraint |= screenPos.bottom < mCurrentInnerRect.bottom;
		...
		//paul modify
		float limit = null != mCurrentInnerRect ? 1f : 0.9f;
