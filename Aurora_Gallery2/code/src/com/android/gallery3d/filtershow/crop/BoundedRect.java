...
public class BoundedRect {
    ...
    //paul add start
    public boolean setOuterTo(RectF outerRect) {
        outer.set(outerRect);
        if (!isConstrained()){
            reconstrain();
			return true;
    ...
    public void setInnerTo(RectF innerRect) {
        inner.set(innerRect);
        innerRotated = CropMath.getCornersFromRect(inner);
        rotateInner();
        //if (!isConstrained())
     ...
     */
	public void resizeInnerEx(RectF newInner) {
		 Matrix m0 = getInverseRotMatrix();
		 float[] retCorners = CropMath.getCornersFromRect(newInner);
