...
public class CropObject {
    ...
    private float mTouchTolerance = 90;//45;
	...
	//paul add start
    public boolean setOuterTo(RectF outer) {
        return mBoundedRect.setOuterTo(outer);
    }
	
    public void setInnerTo(RectF inner) {
        mBoundedRect.setInnerTo(inner);
    }
	//paul add end
    ...
    public boolean selectCorner(int edge) {

        if (!checkCorner(edge)) {
            return false;
        }
        if ((mFixAspectRatio && !checkCorner(edge))){
            return false;
        }
        mMovingEdges = edge;
        return true;
    ...
    private int calculateSelectedCorner(float x, float y, RectF cropped) {
		...
		if(edgeSelected == MOVE_NONE && cropped.contains(x, y)){
			edgeSelected = MOVE_BLOCK;
		}
		
        return edgeSelected;
    }

    public int selectCorner(float x, float y,RectF rect) {
        int edgeSelected = calculateSelectedCorner(x, y, rect);
    ...
    public boolean moveCurrentSelection(float dX, float dY) {
        ...
        if (movingEdges == MOVE_BLOCK) {
            ...
            float dx = 0;
            float dy = 0;
			RectF outer = mBoundedRect.getOuter();
	...
	//paul add
    public boolean moveCurrentSelectionEx(float dX, float dY) {
        if (mMovingEdges == MOVE_NONE) {
            return false;
        }
        RectF crop = mBoundedRect.getInner();
        ...
        if (movingEdges == MOVE_BLOCK) {
            ...
            if ((movingEdges & MOVE_LEFT) != 0) {
                dx = Math.min(crop.left + dX, crop.right - minWidthHeight) - crop.left;
				if(crop.left + dx < outer.left) dx = outer.left - crop.left;
            }
            if ((movingEdges & MOVE_TOP) != 0) {
                dy = Math.min(crop.top + dY, crop.bottom - minWidthHeight) - crop.top;
				if(crop.top + dy < outer.top) dy = outer.top - crop.top;
            }
            if ((movingEdges & MOVE_RIGHT) != 0) {
                dx = Math.max(crop.right + dX, crop.left + minWidthHeight)
                        - crop.right;
				if(crop.right + dx > outer.right) dx = outer.right - crop.right;
            }
            if ((movingEdges & MOVE_BOTTOM) != 0) {
                dy = Math.max(crop.bottom + dY, crop.top + minWidthHeight)
                        - crop.bottom;
				if(crop.bottom + dy > outer.bottom) dy = outer.bottom - crop.bottom;
            }
            ...
            if (mFixAspectRatio) {
                ...
                mBoundedRect.resizeInnerEx(crop);
            }
        }
        return true;
    }
