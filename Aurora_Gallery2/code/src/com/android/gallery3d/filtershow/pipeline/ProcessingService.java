...
public class ProcessingService extends Service {
	...
	private boolean mServiceRunCreated = false;//paul add for BUG #15389 <2015-09-01> 
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
		...
		//paul add for BUG #15389 <2015-09-01> start
		if(mServiceRunCreated){
			reSetup();
		}
		mServiceRunCreated = true;
		//paul add for BUG #15389 <2015-09-01> end
        ...
        if (intent != null && intent.getBooleanExtra(SAVING, false)) {
            ...
            //Log.i("SQF_LOG", "ProcessingService::onStartCommand destinationFile: " + destinationFile);
	...
	private void reSetup(){
    	//Aurora <SQF> <2014-07-04>  for NEW_UI begin
    	if(mProcessingTaskController != null) {
    		mProcessingTaskController.clearProcessingTasks();
    		mProcessingTaskController.quit();
    	}
    	this.tearDownPipeline();
    	MasterImage.getImage().getBitmapCache().clearAll();
    	MasterImage.reset();
        mProcessingTaskController = new ProcessingTaskController(this);
        mImageSavingTask = new ImageSavingTask(this);
        mUpdatePreviewTask = new UpdatePreviewTask();
    ...
    public void handleSaveRequest(Uri sourceUri, Uri selectedUri,
    	...
    	//MyLog.i2("SQF_LOG", "ProcessingService::handleSaveRequest");
    ...
    public void completeSaveImage(Uri result, boolean exit) {
        ...
        //wenyongzhe 2015.11.5 NUllpoint BUG17056 start
        if(mFiltershowActivity ==null){
        	return;
        }
        //wenyongzhe 2015.11.5 NUllpoint BUG17056 end
    ...
    private void setupPipeline() {
        ...
        */
        ...
        FiltersManager filtersManager = FiltersManager.getManager();
        filtersManager.addEffects();
        filtersManager.addAuroraRepresentations(this);

        FiltersManager highresFiltersManager = FiltersManager.getHighresManager();
        highresFiltersManager.addEffects();
        highresFiltersManager.addAuroraRepresentations(this);
