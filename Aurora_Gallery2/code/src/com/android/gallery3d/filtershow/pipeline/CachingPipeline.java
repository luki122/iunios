...
public class CachingPipeline implements PipelineInterface {
        ...
        synchronized (CachingPipeline.class) {
            ...
            */
            ...
            if (request.getType() == RenderingRequest.PARTIAL_RENDERING) {
                ...
                if (bitmap == null) {
					//paul modify
					bitmap = ImageLoader.loadDownsampledBitmap(master.getActivity(),
						master.getUri(),
						1, 
						mEnvironment.getBimapCache(),
						request.getBounds());
			...
			*/
            if (request.getType() == RenderingRequest.FULL_RENDERING
                ...
                //GalleryUtils.writeBitmapToFile(bitmap, "/mnt/sdcard/feng444.jpg");
                //Bitmap bmp = preset.apply(bitmap, mEnvironment);
                //GalleryUtils.writeBitmapToFile(bmp, "/mnt/sdcard/feng555.jpg");

                //SQF MODIFIED TO:
                Bitmap bmp = null;
                if(request.getType() == RenderingRequest.PARTIAL_RENDERING) {
	                int rotation = 0;
	                boolean hasAuroraEffects = false;
	                Vector<FilterRepresentation> filters = request.getImagePreset().getFilters();
	                for(int i=0; i<filters.size(); i++) {
	                	FilterRepresentation rep = filters.get(i);
	                	if(rep instanceof FilterRotateRepresentation) {
	                		FilterRotateRepresentation rotateRep = (FilterRotateRepresentation)rep;
	                		rotation = rotateRep.getRotation().value();
	                		continue;
	                	}
	                	if(rep.getFilterType() == FilterRepresentation.TYPE_AURORA_EFFECTS) {
	                		hasAuroraEffects = true;
	                		continue;
	                	}
	                }
	                
	                
	                if(rotation != 0 && hasAuroraEffects) {
	                	Matrix m = new Matrix();
	                	m.postRotate(rotation);
						Bitmap tmpBmp1 = null;
						//paul modify for BUG #15958
						try{
							tmpBmp1 = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), m, true);
						}catch(OutOfMemoryError err){
							Log.e(LOGTAG, "OutOfMemoryError err1");
							return;
						}

						Bitmap tmpBmp2 = preset.apply(tmpBmp1, mEnvironment);
						m.reset();
						m.postRotate(-1 * rotation);
						try{
							bmp = Bitmap.createBitmap(tmpBmp2, 0, 0, tmpBmp2.getWidth(), tmpBmp2.getHeight(), m, true);
						}catch(OutOfMemoryError err){
							Log.e(LOGTAG, "OutOfMemoryError err2");
							return;
						}finally{
							if(null != tmpBmp2) {
								tmpBmp2.recycle();
	                	...
	                	bmp = preset.apply(bitmap, mEnvironment);
	                }
    ...
    */
    ...
    public void compute(SharedBuffer buffer, ImagePreset preset, int type) {
        ...
        */
        setupEnvironment(preset, false);
        if(preset == null)return;//wenyongzhe 2015.12.30 BUG #17947 NullPointerException
    ...
    public synchronized boolean isInitialized() {
        return mOriginalBitmap != null;//getRenderScriptContext() != null && mOriginalBitmap != null;
    }
