...
public class BitmapCache {
    ...
    public synchronized Bitmap getBitmap(int w, int h, int type) {
		//paul add
		if(w <= 0 || h <= 0){
			Log.e(LOGTAG,"getBitmap::width and height must be > 0");
			return null;
		}
        ...
        if (bitmap == null
            ...
            try{
            	...
            	Log.e(LOGTAG, "Bitmap.createBitmap OutOfMemoryError err");
				return null;
			}
            showBitmapCounts();
        }
	...
	//paul add for BUG #15389 <2015-09-01> start
	public synchronized Bitmap getBitmapTryTimes(int w, int h, int type) {
		int num_tries = 0;
		boolean noBitmap = true;
		Bitmap temp = null;
        while (noBitmap) {
			temp = getBitmap(w, h, type);
			if(null == temp){
				Log.w(LOGTAG, "num_tries : " + num_tries);
				if(++num_tries >= 5) return null;
				System.gc();
				w /= 2;
				h /= 2;
	...
	//paul add for BUG #15389 <2015-09-01> end
	
    public synchronized Bitmap getBitmapCopy(Bitmap source, int type) {
    	//paul modify for BUG #15389 <2015-09-01> start
        Bitmap bitmap = getBitmapTryTimes(source.getWidth(), source.getHeight(), type);
    ...
    public void clearAll() {
    	if(mBitmapCache == null) return;
    	Collection<ArrayList<WeakReference<Bitmap>>> values =  mBitmapCache.values();
    	if(values == null) return;
    	Iterator<ArrayList<WeakReference<Bitmap>>> it = values.iterator();
    	while(it != null && it.hasNext()) {
    		ArrayList<WeakReference<Bitmap>> arrayList = it.next();
    		for(int i=0; i<arrayList.size(); i++) {
    			WeakReference<Bitmap> ref = arrayList.get(i);
