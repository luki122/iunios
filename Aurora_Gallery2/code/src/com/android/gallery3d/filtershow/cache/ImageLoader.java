...
public final class ImageLoader {
    ...
    public static String getLocalPathFromUri(Context context, Uri uri) {
        ...
        String mLocalPath = cursor.getString(index);
        cursor.close();
        return mLocalPath;
    ...
    //Aurora <SQF> <2014-07-17>  for Photos have rotation degree begin
	public static Bitmap loadRegionBitmapEx(Context context, BitmapCache cache,
			Uri uri, BitmapFactory.Options options, Rect bounds, int orientation) {
		InputStream is = null;
		int w = 0;
		int h = 0;
		if (options.inSampleSize != 0) {
		...
		try {
			...
			BitmapRegionDecoder decoder = BitmapRegionDecoder.newInstance(is, false);
			Rect r = null;
			if(orientation == ORI_ROTATE_90 || orientation == ORI_ROTATE_270) {
				r = new Rect(0, 0, decoder.getHeight(), decoder.getWidth());
			} else {
				r = new Rect(0, 0, decoder.getWidth(), decoder.getHeight());
			}
			w = decoder.getWidth();
			h = decoder.getHeight();
			Rect imageBounds = new Rect(bounds);
			...
			if(orientation == ORI_ROTATE_90 ) {
				int left = imageBounds.top;
				int top = r.width() - imageBounds.width() - imageBounds.left;
				int right = left + imageBounds.height();
				int bottom = top + imageBounds.width();
				imageBounds.set(left, top, right, bottom);
			} else if(orientation == ORI_ROTATE_180) {
				int left = r.width() - imageBounds.width() - imageBounds.left;
				int top = r.height() - imageBounds.height() - imageBounds.top;
				int right = left + imageBounds.width();
				int bottom = top + imageBounds.height();
				imageBounds.set(left, top, right, bottom);
			} else if(orientation == ORI_ROTATE_270) {
				int left = r.width() - imageBounds.top - imageBounds.height();
				int top = imageBounds.left;
				int right = left + imageBounds.height();
				int bottom = top + imageBounds.width();
				imageBounds.set(left, top, right, bottom);
			}
			...
			Bitmap bitmap = null;
			try{
				bitmap = decoder.decodeRegion(imageBounds, options);
            }catch(OutOfMemoryError err){//paul add for BUG #15389<2015-08-05>
            	Log.e(LOGTAG, "loadRegionBitmapEx OutOfMemoryError err");
				return null;
			}
			...
			Log.e(LOGTAG, "exc, image decoded " + w + " x " + h + " bounds: "
					...
					+ "x" + bounds.height() + " exc: " + e);
		} finally {
			Utils.closeSilently(is);
		}
		return null;
	}
	//Aurora <SQF> <2014-07-17>  for Photos have rotation degree end
    ...
    //paul add
    public static Bitmap loadDownsampledBitmap(Context context, Uri uri, int sampleSize, BitmapCache cache,Rect bounds) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inMutable = true;
        options.inSampleSize = sampleSize;

        Bitmap bmp = loadBitmap(context, uri, options);
		...
		Bitmap target = cache.getBitmap(imageBounds.width(), imageBounds.height(), BitmapCache.REGION);
			//Bitmap.createBitmap(imageBounds.width(), imageBounds.height(), Bitmap.Config.ARGB_8888);
		Canvas canvas = new Canvas(target);
		Paint paint = new Paint(Paint.FILTER_BITMAP_FLAG | Paint.DITHER_FLAG);
		canvas.drawBitmap(bmp, -imageBounds.left, -imageBounds.top, paint);
		bmp.recycle();
		return target;
    }
    ...
    public static Bitmap loadBitmap(Context context, Uri uri, BitmapFactory.Options o) {
        ...
        try {
        	...
        	Log.e(LOGTAG, "loadBitmap OutOfMemoryError err");
        } finally {
            Utils.closeSilently(is);
        }
        return null;
    ...
    public static Bitmap getScaleOneImageForPreset(Context context,
        ...
        */
        ...
        //Aurora <SQF> <2014-07-07>  for NEW_UI begin
        //ORIGINALLY:
        //Bitmap bmap = loadRegionBitmap(context, cache, uri, options, bounds);
        //SQF MODIFIED TO:
        int orientation = getMetadataOrientation(context, uri);
        Bitmap bmap = loadRegionBitmapEx(context, cache, uri, options, bounds, orientation);
        //Aurora <SQF> <2014-07-07>  for NEW_UI end
		//int orientation = getMetadataOrientation(context, uri);//SQF ANNOTATED ON 2014-07-07
		if (bmap != null && ORI_NORMAL != orientation) {
			bmap = orientBitmap(bmap, orientation);
			if (bmap.getConfig()!= Bitmap.Config.ARGB_8888){
				bmap = bmap.copy( Bitmap.Config.ARGB_8888,true);
			}
