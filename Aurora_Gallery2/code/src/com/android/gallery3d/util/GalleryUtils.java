...
public class GalleryUtils {
    ...
    private static void initializeThumbnailSizes(DisplayMetrics metrics, Resources r) {
        ...
        if (false) {//lory modify 2014.4.4 MySelfBuildConfig.USEGALLERY3D_FLAG
...
// Aurora <zhanggp> <2013-12-06> added for gallery begin
	public static void shareContent(AbstractGalleryActivity activity, Intent intent) {
		...
		try {
			(activity).startActivity(Intent.createChooser(intent,
					activity.getAndroidContext().getResources().getString(R.string.share)));
	...
	public static String getMediaSetPath(int typeBits) {
		if ((typeBits & DataManager.INCLUDE_VIDEO) != 0) {
			return (typeBits & DataManager.INCLUDE_IMAGE) == 0
					? "/local/mergevideo/"
					: "/local/allsets/";
		}
		return "/local/mergeimage/";
	}

	//Iuni <lory><2013-12-30> add begin
	private static Rect m_IndexRect;
	public static void setIndexRect(Rect tRect) {
		m_IndexRect = tRect;
		//Log.i(TAG, "zll ---- rrrr left:"+m_IndexRect.left+",top:"+m_IndexRect.top+",right:"+m_IndexRect.right+",bottom:"+m_IndexRect.bottom);
		return;
	}
	
	public static Rect getIndexRect() {
		if (m_IndexRect != null) {
			return m_IndexRect;
		} else {
			return null;
...
// Aurora <zhanggp> <2013-12-06> added for gallery end
	...
	public static boolean isU3() {
		//Log.i("SQF_LOG", "Build.MODEL: --> " + Build.MODEL);
		if(Build.MODEL.equals("IUNI U3") || Build.MODEL.equals("U3")) {
    		return true;
    	}
		return false;
	}
	
	public static boolean needNavigationBarControl() {
		return false;
		//return isU3() && Build.VERSION.SDK_INT >= ApiHelper.VERSION_CODES.KITKAT;
	}
    //Aurora <SQF> <2014-07-31>  for NEW_UI end
	
	
	// JUST FOR TEST
	// filePath : like "/mnt/sdcard/feng777.jpg"
	public static void writeBitmapToFile(Bitmap bmp, String filePath) {
