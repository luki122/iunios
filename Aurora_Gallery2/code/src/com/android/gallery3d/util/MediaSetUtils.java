...
public class MediaSetUtils {
	...
	private static String DISPLAY_FAV_ALL = null;
	private static String DISPLAY_FAV_LOCAL_IMAGE = null;
	private static String DISPLAY_FAV_LOCAL_VIDEO = null;
    ...
    public static void getCameraBucketID(Context c) {
		...
		if(EX_STORAGE != null) {
			CAMERA_BUCKET_ID = GalleryUtils.getBucketId(EX_STORAGE + "/DCIM");
			//DCIM_BUCKET_ID = GalleryUtils.getBucketId(EX_STORAGE + REAL_CAMERA_BUCKET_NAME);
		} else {
		...
		DISPLAY_FAV_ALL = "favorite=1) GROUP BY SUBSTR(_data,0,length(_data)";
		DISPLAY_FAV_LOCAL_IMAGE = imagePathsStr.toString() + ") AND favorite=1 AND " + ImageColumns.SIZE + " > 0) GROUP BY SUBSTR(_data,0,length(_data)";
		DISPLAY_FAV_LOCAL_VIDEO = videoPathsStr.toString() + ") AND favorite=1 AND " + VideoColumns.SIZE + " > 0) GROUP BY SUBSTR(_data,0,length(_data)";
		imagePathsStr.append(") AND " + ImageColumns.SIZE + " > 0) GROUP BY SUBSTR(_data,0,length(_data)");
		videoPathsStr.append(") AND " + VideoColumns.SIZE + " > 0) GROUP BY SUBSTR(_data,0,length(_data)");
	...
	public static String getFavQueryStr(){
		return DISPLAY_FAV_ALL;
	}
	
	public static String getLocalFavQueryStr(boolean isImage){
		if(isImage){
			return DISPLAY_FAV_LOCAL_IMAGE;
		} 
		return DISPLAY_FAV_LOCAL_VIDEO;
	}
