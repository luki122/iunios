...
public class TiledTexture implements Texture {
	...
	//paul add start
    private Tile mFreeTileHead = null;
    private final Object mFreeTileLock = new Object();
	private final Uploader mTiledUploader;
    //paul add end

    private int mUploadIndex = 0;
	private static boolean mIsPhotoPage = false;//paul add
    ...
    public static class Uploader implements OnGLIdleListener {
		...
		//paul add start
        public void usePrivateUploader() {
	        mUploadBitmap = Bitmap.createBitmap(TILE_SIZE, TILE_SIZE, Config.ARGB_8888);
    ...
    private class Tile extends UploadedTexture {
        ...
        protected Bitmap onGetBitmap() {
				...
				if(null != mUploader){
					if(uploaded){
						return mUploader.mUploadBitmap;
					}
					if(null != mUploader.mCanvas && null != bitmap && !bitmap.isRecycled()) {
						
						int x = BORDER_SIZE - offsetX;
						int y = BORDER_SIZE - offsetY;
						int r = bitmap.getWidth() + x;
						int b = bitmap.getHeight() + y;
						mUploader.mCanvas.drawBitmap(bitmap, x, y, mUploader.mBitmapPaint);
						uploaded = true;
					} else {
						uploaded = false;
					}
				...
				//paul add end
		        if(null != sCanvas && null != bitmap && !bitmap.isRecycled()) {
    ...
    private void freeTile(Tile tile) {
        tile.invalidateContent();
        tile.bitmap = null;
		if(null == mTiledUploader){
	        synchronized (sFreeTileLock) {
	            tile.nextFreeTile = sFreeTileHead;
	            sFreeTileHead = tile;
	        }
		} else {
	        synchronized (mFreeTileLock) {
				tile.uploaded = false;
	            tile.nextFreeTile = mFreeTileHead;
	            mFreeTileHead = tile;
	        }
		}
    }

    private Tile obtainTile() {
		if(null == mTiledUploader){
	        synchronized (sFreeTileLock) {
	            Tile result = sFreeTileHead;
	            if (result == null) return new Tile(mTiledUploader);//paul modify
	        ...
	        synchronized (mFreeTileLock) {
	            Tile result = mFreeTileHead;
	            if (result == null) return new Tile(mTiledUploader);//paul modify
				mFreeTileHead = result.nextFreeTile;
				result.nextFreeTile = null;
	            return result;
	        }
    ...
    private boolean uploadNextTile(GLCanvas canvas) {
        ...
        synchronized (mTiles) {
            ...
            if (next.bitmap != null) {
				// Aurora <paul> <2014-01-14> modified for gallery begin
				...
				// Aurora <paul> <2014-01-14> modified for gallery end
            }
        }
	...
	//paul add start
    public TiledTexture(Bitmap bitmap, final Uploader l) {
		mTiledUploader = l;
        mWidth = bitmap.getWidth();
        mHeight = bitmap.getHeight();
        ArrayList<Tile> list = new ArrayList<Tile>();
