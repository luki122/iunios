...
abstract class UploadedTexture extends BasicTexture {
	...
	public boolean mAlwaysUpdate = false;//Aurora <paul> <2014-03-17> for_gif
    ...
    private Bitmap getBitmap() {
		...
		if(mAlwaysUpdate){
	        ...
	        if (mBitmap == null) {
	            mBitmap = onGetBitmap();
				if(null == mBitmap) return null;//Aurora <paul> <2014-05-04> add for MTTF
	            int w = mBitmap.getWidth() + mBorder * 2;
	            int h = mBitmap.getHeight() + mBorder * 2;
	            if (mWidth == UNSPECIFIED) {
	                setSize(w, h);
    ...
    private void freeBitmap() {
    	...
    	try {
			...
			Log.e("liugj", "Throw AssertionError ***");
		}
	...
	protected void refreshBitmap(Bitmap bitmap){}//Aurora <paul> <2014-10-11> for_gif
    ...
    public void updateContent(GLCanvas canvas) {
        if (!isLoaded()) {
			...
			if(null == bitmap) return;//Aurora <paul> <2014-05-15> for MTTF
