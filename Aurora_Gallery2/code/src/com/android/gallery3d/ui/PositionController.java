...
class PositionController {
    ...
    private static final float SCALE_LIMIT = 83000000;//4 for NEW_UI  
    ...
    public PositionController(Context context, Listener listener) {
        ...
        mFilmScroller = new Scroller(context, null, false);
		 ...
		 initAnimationRes(context);//paul add
    ...
    private void snapDirect() {
        mPlatform.snapbackDirect();
        for (int i = -BOX_MAX; i <= BOX_MAX; i++) {
            mBoxes.get(i).snapbackDirect();
        }
        for (int i = -BOX_MAX; i < BOX_MAX; i++) {
            mGaps.get(i).snapbackDirect();
        }
        mFilmRatio.snapbackDirect();
		redraw();
    }

	private LoadingAnimation mLoadingAnimation;
	private ResourceTexture mLoadingIcon;
	
	private void initAnimationRes(Context context){
		if(null != mLoadingIcon) return;
		mLoadingIcon = new ResourceTexture(context, R.drawable.aurora_loading_icon);
		mLoadingAnimation = new LoadingAnimation(mLoadingIcon);
	}
	public void playLoadingAnimation(GLCanvas canvas, int width, int height){
		if(!mLoadingAnimation.isActive()){
			mLoadingAnimation.start(width, height);
		}
		
		mLoadingAnimation.calculate(AnimationTime.get());
		mLoadingAnimation.apply(canvas);
	}
	...
	public class LoadingAnimation extends Animation {
		...
		public LoadingAnimation(ResourceTexture icon) {
			mIcon = icon;
		...
		public void apply(GLCanvas canvas){
			...
			canvas.rotate(360 * mProgress, 0, 0, 1);
			mIcon.draw(canvas,-mWidth>>1, -mHeight>>1, mWidth, mHeight);
			canvas.restore();
		}
	}

	//Aurora <paul> <2014-03-25> end
    ...
    private boolean mAimationMode = false;
    public int auroraScaleBy(float s, float focusX, float focusY) {
        focusX -= mViewW / 2;
        focusY -= mViewH / 2;
        Box b = mBoxes.get(0);
        Platform p = mPlatform;
        ...
        int x = mAimationMode ? p.mCurrentX : (int) (focusX - s * mFocusX + 0.5f);
        int y = mAimationMode ? b.mCurrentY : (int) (focusY - s * mFocusY + 0.5f);
        auroraStartAnimation(x, y, s, ANIM_KIND_SCALE);
        if (s < b.mScaleMin) return -1;
        if (s > b.mScaleMax) return 1;
        return 0;
    }
    
    private boolean auroraStartAnimation(int targetX, int targetY, float targetScale,
            int kind) {
        ...
        Log.i(TAG, "zll ----- auroraStartAnimation 1 mPlatform.mDefaultY:"+mPlatform.mDefaultY+",targetX:"+targetX+",changed:"+changed+",targetScale:"+targetScale);
        if (changed) redraw();
        return changed;
    }
...
//    	Log.i("SQF_LOG", "PositionController::convertBoxToRect--> i:" + i);
	...
	public void moveBoxEx(int fromIndex[], boolean hasPrev, boolean hasNext,
			 boolean constrained, Size[] sizes) {
		 //debugMoveBox(fromIndex);
		 mHasPrev = hasPrev;
		 mHasNext = hasNext;
    ...
    private float getMaximalScale(Box b) {
		...
		float min = getMinimalScale(b);
        if (mFilmMode) return min;
        if (mConstrained && !mConstrainedFrame.isEmpty()) return min;
        return min > 4f ? min : 4f;
		/*
    ...
    private abstract static class Animatable {
		...
		public abstract boolean snapbackDirect();//Aurora <paul> <2014-03-25> added
    ...
    private class Platform extends Animatable {
		...
		//Aurora <paul> <2014-03-25> added start
        @Override
        public boolean snapbackDirect() {
		...
		//Aurora <paul> <2014-03-25> added end
