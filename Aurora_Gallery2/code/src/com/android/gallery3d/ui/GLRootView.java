...
public class GLRootView extends GLSurfaceView
    ...
    private void onDrawFrameLocked(GL10 gl) {
        ...
        if (mContentView != null) {
			//if(mAnimPlayer.enable()) return;
			if(mContentView.renderAnimation(mCanvas)){
    ...
    public void setLightsOutMode(boolean enabled) {
        ...
        if (enabled) {
            ...
            if(ApiHelper.HAS_VIEW_SYSTEM_UI_FLAG_LOW_PROFILE) {
        		int currentFlags = getSystemUiVisibility();
        		currentFlags &= ~SYSTEM_UI_FLAG_LOW_PROFILE;
        		flags = currentFlags;
        		//Log.i("SQF_LOG", "setLightsOutMode ----> true");
        	}
    ...
    */
    ...
    public void setLightsOutMode(boolean enabled) {
    	/*
        if (!ApiHelper.HAS_SET_SYSTEM_UI_VISIBILITY) return;
        if(! ApiHelper.HAS_VIEW_SYSTEM_UI_FLAG_LOW_PROFILE) return;
        int flags = getSystemUiVisibility();
        if (enabled) {
        	//flags &= ~SYSTEM_UI_FLAG_LOW_PROFILE;//SQF ANNOTATED ON 2014.08.08
        } 
        else {
...
//            flags |= SYSTEM_UI_FLAG_LOW_PROFILE;
    ...
    @Override
    protected void updateWindow(boolean s, boolean d){
    	if(canSetFrame){
    		super.updateWindow( s,d);
    	}
    }

  //wenyongzhe 2016.1.14 start
    public boolean canSetFrame = true;
    
	public boolean isCanSetFrame() {
		return canSetFrame;
	}

	public void setCanSetFrame(boolean canSetFrame) {
		this.canSetFrame = canSetFrame;
	}
	 //wenyongzhe 2016.1.14 end
