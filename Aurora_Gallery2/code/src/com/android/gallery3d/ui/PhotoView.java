...
//Aurora <SQF> <2014-04-18>  for NEW_UI begin
import android.util.DisplayMetrics;
//Aurora <SQF> <2014-04-18>  for NEW_UI end
...
public class PhotoView extends GLView {
    ...
    public void setIsShowingAlertDialog(boolean isShowing) {
    	mShowingAlertDialog = isShowing;
    	if(! mShowingAlertDialog ) {
    		invalidate();
    	}
    }
    
    public boolean isShowingAlertDialog() {
    	return mShowingAlertDialog;
    }
    ...
    public interface Model extends TileImageView.Model {
		...
		public void reloadCurrentIndex(); //Aurora <paul> <2015-05-13>
		public void skipTo(int index); //Aurora <paul> <2014-02-27> for NEW_UI
		...
		public boolean isFullPicLoaded(int offset);//Aurora <paul> <2015-5-15> add
   ...
   //wenyongzhe 2015.11.17
    private static final int MSG_RELOAD_IMAGE = 10;

	private static final int MSG_REFRESH_GIF = 9;//Aurora <paul> <2014-03-20> for_gif
	...
	// Aurora <paul> <2014-01-02> modified for gallery begin
    private static final int ICON_RATIO = 6;
	// Aurora <paul> <2014-01-02> modified for gallery end
	...
	private int mSnapImgNum = 0;
	private int mViewH;
	private int mViewW;
	private AbstractGalleryActivity mActivity;
	private static final int SNAP_PAGE_TO_CHANGE_INDEX = 10;// x/2
	private static final int SNAP_PAGE_CLEAR_TIME = 1000;
	private static final int SNAP_PAGE_TO_CHANGE_FLING_TIME = 1000;
	// Aurora <zhanggp> <2013-12-06> added for gallery end
	public PhotoView(AbstractGalleryActivity activity) {

		mThreadPool = activity.getThreadPool();//Aurora <paul> <2014-03-17> for_gif
        ...
        mNoThumbnailText = StringTexture.newInstance(
                mContext.getString(R.string.aurora_load_fail),//paul modify for  BUG #15773 R.string.no_thumbnail
	...
	public boolean CanBackToAlbum(){
		if(mPositionController.isAtMinimalScale()){
			return true;
		}
    ...
    class MyHandler extends SynchronizedHandler {
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
				...
				//Aurora <paul> <2014-03-20> for_gif start
				case MSG_REFRESH_GIF: {
					mPictures.get(0).onGifLoaded();
					invalidate();
					break;
				}
				//Aurora <paul> <2014-03-20> for_gif end
	...
	//Aurora <paul> <2014-02-27> for NEW_UI begin
	public void notifyDataChangeEx(int[] fromIndex, int prevBound, int nextBound) {
		 mPrevBound = prevBound;
		 mNextBound = nextBound;
		 ...
		 // Move the boxes
			 ...
			 mPositionController.moveBoxEx(fromIndex, mPrevBound < 0, mNextBound > 0,
					 mModel.isCamera(0), mSizes);
    ...
    protected void onLayout(
        ...
        //Aurora <SQF> <2014-04-22>  for monkey test NullPointerException begin
        //ORIGINALLY:
        //int displayRotation = root.getDisplayRotation();
        //int compensation = root.getCompensation();
        //SQF MODIFIED TO:
        int displayRotation = (null != root) ? root.getDisplayRotation() : 0;
        int compensation = (null != root) ? root.getCompensation() : 0;
        //Aurora <SQF> <2014-04-22>  for monkey test NullPointerException end
		...
		//Aurora <paul> <2014-04-16> start
		mViewH = getHeight();
		mViewW = getWidth();
		//Aurora <paul> <2014-04-16> end
        if (changeSize) {
			mPictures.get(0).viewSizeChanged();//Aurora <paul> <2014-03-20> for_gif
            mPositionController.setViewSize(mViewW, mViewH);//Aurora <paul> <2014-04-16>
    ...
    private interface Picture {
        void reload();
        boolean draw(GLCanvas canvas, Rect r);//Aurora <paul> <2014-03-17> for_gif
    ...
    //Aurora <SQF> <2014-04-10>  for NEW_UI begin
    private boolean isCurrentIndexEntering() {
    	PhotoDataAdapter pda = null;
    	if((null != mModel) && (mModel instanceof PhotoDataAdapter)) {
    		pda = (PhotoDataAdapter)mModel;
    	}
    	if(pda == null) return false;
    	/*float filmRatio = mPositionController.getFilmRatio();*/
    	return /*(filmRatio == 0.0f) && */pda.getCurrentIndex() == pda.getEnteringIndex();
    }
    
    private void clearEnteringBimtapInfo() {
    ...
    private int getPlaceHolderRotation() {
    	...
    	if(mModel != null && (mModel instanceof PhotoDataAdapter)) {
    		PhotoDataAdapter pda = (PhotoDataAdapter)mModel;
    		rotation = pda.getEnteringBitmapRotation();
    	}
    	return rotation;
    }
    ...
    private Size getPlaceHolderDrawSize(int placeHolderSnWidth, int placeHolderSnHeight) {
    	Size size = new Size();
    	if(null == mContext) return size;
    	...
    	if(isPortrait()) {
    		...
    		newW = sw;
    		newH = (int) ( (float)placeHolderSnHeight / ratio ); 
    		if(newH > sh) {
    			newH = sh;
    			newW = (int)((float)placeHolderSnWidth / (float)placeHolderSnHeight * newH);
    		}
    	} else {
    		ratio = ((float)placeHolderSnHeight / (float)sh);
    		newH = sh;
    		newW = (int) ((float)placeHolderSnWidth / ratio);
    		if(newW > sw) {
    			newW = sw;
    			newH = (int)((float)newW / ((float)placeHolderSnWidth / (float)placeHolderSnHeight));
    		}
    	}
    	size.width = newW;   
    	size.height = newH;
    ...
    private void drawPlaceHolderScreenNail(GLCanvas canvas, Rect r) {
    	PhotoDataAdapter pda = null;
    	if(mModel instanceof PhotoDataAdapter) {
    		pda = (PhotoDataAdapter)mModel;
    	}
    	if(pda != null && isCurrentIndexEntering()) {
    		ScreenNail tmpSn = pda.getPlaceHolderScreenNail();
    		if(tmpSn != null) {
        		canvas.save(GLCanvas.SAVE_FLAG_MATRIX | GLCanvas.SAVE_FLAG_ALPHA);
        		int rotation = getPlaceHolderRotation();
        		//canvas.fillRect(0, 0, 1080, 960, Color.argb(255, 255, 0, 0)); //SQF. JUST FOR TEST.
        		canvas.translate(r.left + r.width() / 2, r.top + r.height() / 2);
				canvas.rotate(rotation, 0, 0, 1);
				int inWidth = getRotated(rotation, tmpSn.getWidth(), tmpSn.getHeight());
				int inHeight = getRotated(rotation, tmpSn.getHeight(), tmpSn.getWidth());
				Size size = getPlaceHolderDrawSize(inWidth, inHeight);
				int drawRectWidth = getRotated(rotation, size.width, size.height);
	            int drawRectHeight = getRotated(rotation, size.height, size.width);
				tmpSn.draw(canvas, - drawRectWidth / 2,  - drawRectHeight / 2, drawRectWidth, drawRectHeight);
                canvas.restore();
                return;
    		}
    	}
    }
    //Aurora <SQF> <2014-04-10>  for NEW_UI end
    ...
    class FullPicture implements Picture {
		...
		private boolean mHasFullPic;//Aurora <paul> <2015-5-15> add
        private int mLoadingState = Model.LOADING_INIT;
        private Size mSize = new Size();
		
		//Aurora <paul> <2014-03-17> for_gif start
		private AuroraGifTexture mGif;
		...
		private MovieData resetMovieData(){
			if(null == mMovieData){
				mMovieData = new MovieData();
			}
			mMovieData.mPath = null;
			mMovieData.mMovie = null;
			mMovieData.mScale = 0.0f;
			
			mMovieData.mDrawLeft = 0;
			mMovieData.mDrawTop = 0;
			mMovieData.mIndex = -1;
			return mMovieData;
		}
		@Override
		public void viewSizeChanged(){
			...
			if(null != item){
				...
				if(AuroraGifTexture.isGif(path)){
					MovieData movieData = resetMovieData();
		...
		public void onGifLoaded(){
			if(null == mMovieData 
				|| null == mMovieData.mMovie
				|| mMovieData.mIndex != mModel.getCurrentIndex()){
				return;
			}
			...
			mMovieData.mScale = Math.min(mViewW / (float)w ,mViewH / (float)h);
			int GifW = (int)(mMovieData.mScale * w);
			int GifH = (int)(mMovieData.mScale * h);
			if(0 >= GifW || 0 >= GifH){
				return;
			}
			mMovieData.mDrawLeft= (mViewW + 1) / 2 - (GifW + 1) / 2;
			mMovieData.mDrawTop= (mViewH + 1) / 2 - (GifH + 1) / 2;
			mGif = new AuroraGifTexture(GifW,GifH,mMovieData);
		}
		
		//Aurora <paul> <2014-03-17> for_gif end
        @Override
        public void reload() {
			...
			mHasFullPic = mModel.isFullPicLoaded(0);//Aurora <paul> <2015-5-15> add
        ...
        @Override
        public boolean draw(GLCanvas canvas, Rect r) {//Aurora <paul> <2014-03-17> for_gif boolean draw
			//Aurora <paul> <2014-03-17> for_gif start
        	if(!mFilmMode && null != mGif && mGif.isValid() 
				&& mPositionController.isAtMinimalScale() 
				&& (mHolding & HOLD_TOUCH_DOWN) == 0
				&& !mPositionController.isScrolling()){
				if(mGif.isContentValid()){
					mGif.resetCanvas();
					mGif.updateContent(canvas);
				}
				mGif.draw(canvas,mGif.getDrawLeft(),mGif.getDrawTop());
        	...
        	//SQF MODIFIED TO:
        	//Log.i("SQF_LOG", " mTileView.hasValidSceenNailTexture :" +  mTileView.hasValidSceenNailTexture()   );
        	//Log.i("SQF_LOG", " isCurrentIndexEntering() : " + isCurrentIndexEntering());
        	//Log.i("SQF_LOG", " hasValidEnteringBitmap() : " + hasValidEnteringBitmap());
        	
        	if(mTileView != null &&  ! mTileView.hasValidSceenNailTexture() && 
        			isCurrentIndexEntering() && hasValidEnteringBitmap()) {
        		drawPlaceHolderScreenNail(canvas, r);
        	} else {
        		drawTileView(canvas, r);
        	}
            //Aurora <SQF> <2014-04-10>  for NEW_UI end
            ...
            if ((mHolding & ~HOLD_TOUCH_DOWN) != 0) return false;//Aurora <paul> <2014-03-17> for_gif return;
			...
			if(mActivity.inCloudView() && !mHasFullPic){
				mPositionController.playLoadingAnimation(canvas, mViewW, mViewH);
				return true;
			}
			//Aurora <paul> <2015-5-15> add end
			return false;//Aurora <paul> <2014-03-17> for_gif
        }
        ...
        private void drawTileView(GLCanvas canvas, Rect r) {
            ...
            } else if (mLoadingState == Model.LOADING_FAIL) {
                drawLoadingFailMessage(canvas);
            //wenyongzhe 2015.11.25 reloadImage start
                mHandler.sendEmptyMessageDelayed(MSG_RELOAD_IMAGE, 10000);
            }else if(mLoadingState == Model.LOADING_COMPLETE){
            	mHandler.removeMessages(MSG_RELOAD_IMAGE);
            }
            //wenyongzhe 2015.11.25 reloadImage end
            
			//Aurora <paul> <2014-03-05> end
        ...
        private void setTileViewPosition(float cx, float cy,
            ...
            //Aurora <SQF> <2014-6-30>  for NEW_UI begin
            //ORIGINALLY:
            //int centerX = (int) (imageW / 2f + (viewW / 2f - cx) / scale + 0.5f);
            //int centerY = (int) (imageH / 2f + (viewH / 2f - cy) / scale + 0.5f);
            //SQF MODIFIED TO:
            int centerX = (int) (imageW / 2f + ((int)(viewW / 2f) - (int)cx) / scale + 0.5f);
            int centerY = (int) (imageH / 2f + ((int)(viewH / 2f) - (int)cy) / scale + 0.5f);
            //Aurora <SQF> <2014-6-30>  for NEW_UI end
...
//            Log.i("SQF_LOG", "PhotoView::setTileViewPosition x:" + x + " y:" + y + " scale:" + scale + 
//            				" imageW:" + imageW + " imageH:" + imageH + " viewW:" + viewW + " viewH:" + viewH +
//            				" centerX:" + centerX + " centerY:" + centerY + " inverseX:" + inverseX + " inverseY:" + inverseY +
//            				" cx:" + cx + " cy:" + cy);
            //Log.i("SQF_LOG", "(viewW / 2f - cx) / scale:  " + ((viewW / 2f - cx) / scale));
            //Log.i("SQF_LOG", "((int)(viewW / 2f) - (int)cx) / scale:  " + ((int)(viewW / 2f) - (int)cx) / scale);
    ...
    private class ScreenNailPicture implements Picture {
        ...
        @Override
        public boolean draw(GLCanvas canvas, Rect r) {//Aurora <paul> <2014-03-17> for_gif void draw
            if (mScreenNail == null) {
                ...
                return false;//Aurora <paul> <2014-03-17> for_gif return;
            }
            int w = getWidth();
            int h = getHeight();
			...
			if(null != mScreenNail) {//paul add for MTTF
    ...
    private void drawPlaceHolder(GLCanvas canvas, Rect r) {
    	//Log.i("SQF_LOG", "PhotoView::drawPlaceHolder");
    ...
    private class MyGestureListener implements GestureRecognizer.Listener {
		...
		private boolean  mIgnoreLargerScalingGesture;//Aurora <paul> <2014-03-20> for_gif
        ...
        public boolean onDoubleTap(float x, float y) {
			...
			if (mPictures.get(0).isGif()) return false;//Aurora <paul> <2014-03-20> for_gif
        ...
        public boolean onScaleBegin(float focusX, float focusY) {
			...
			mIgnoreLargerScalingGesture = mPictures.get(0).isGif();//Aurora <paul> <2014-03-20> for_gif
        ...
        public boolean onScale(float focusX, float focusY, float scale) {
			...
			if(mIgnoreLargerScalingGesture && scale > 1.0f) scale = 1.0f;//Aurora <paul> <2014-03-20> for_gif
        ...
        public void onScaleEnd() {
			mPositionController.resetScaleState();//paul add <2014-06-10>
        ...
        public void onDown(float x, float y) {
            ...
            if (mFilmMode) {
                ...
                if (mTouchBoxIndex < mPrevBound || mTouchBoxIndex > mNextBound) {
					...
					// Aurora <zhanggp> <2013-12-10> modified for gallery begin
                    mTouchBoxDeletable = false;
					/*
                    mTouchBoxDeletable =
                            mPictures.get(mTouchBoxIndex).isDeletable();
					*/
					// Aurora <zhanggp> <2013-12-10> modified for gallery end
                }
            } else {
                mTouchBoxIndex = Integer.MAX_VALUE;
            }
    ...
    public void setFilmMode(boolean enabled) {
		if(mActivity.inCloudView()) return;//paul add
    ...
    public void resume() {
    	//Log.i("SQF_LOG", "PhotoView::resume");
    ...
    protected void render(GLCanvas canvas) {
		...
		boolean more = false;//Aurora <paul> <2014-03-17> for_gif
        // Draw photos from back to front
        for (int i = neighbors; i >= -neighbors; i--) {
            Rect r = mPositionController.getPosition(i);
            more |= mPictures.get(i).draw(canvas, r);//Aurora <paul> <2014-03-17> for_gif mPictures.get(i).draw(canvas, r);
        }
    ...
    private boolean slideToNextPicture() {
		...
		// Aurora <zhanggp> <2013-12-06> added for gallery begin
		if(mSnapImgNum == 0 || mSnapImgNum % 2 != 0){
			//start timer
			sendChangeRequest();
			mSnapImgNum = 2;
		}else if(SNAP_PAGE_TO_CHANGE_INDEX >= mSnapImgNum){
			mSnapImgNum += 2;
			if(SNAP_PAGE_TO_CHANGE_INDEX - mSnapImgNum < 2){
				//TO CHANGE	
				setFilmMode(true);
				mSnapImgNum = 0;
			}
...
// Aurora <zhanggp> <2013-12-06> added for gallery begin
    ...
    public void setFilmModeEx(boolean enabled) {
        if (mFilmMode == enabled) return;
        mFilmMode = enabled;
        mPositionController.setFilmModeEx(mFilmMode);
        mModel.setNeedFullImage(!enabled);
        mModel.setFocusHintDirection(
                mFilmMode ? Model.FOCUS_HINT_PREVIOUS : Model.FOCUS_HINT_NEXT);
        updateActionBar();
	...
	public PhotoFallbackEffect buildFallbackEffectCenter(GLView root, GLCanvas canvas) {
		 Rect location = new Rect();
		 Utils.assertTrue(root.getBoundsOf(this, location));
	
		 Rect fullRect = bounds();
		 PhotoFallbackEffect effect = new PhotoFallbackEffect(getWidth(),getHeight());
		 for (int i = 0; i <= 0; ++i) {
			 MediaItem item = mModel.getMediaItem(i);
			 if (item == null) continue;
			 ScreenNail sc = mModel.getScreenNail(i);
	...
	//Aurora <paul> <2014-02-27> for NEW_UI end
	private Timer mChangeModeTimer = null;
	private void sendChangeRequest()
	{
		stopChangeTimer();
		mChangeModeTimer = new Timer(true);
		mChangeModeTimer.schedule(new TimerTask(){
			public void run(){
				mSnapImgNum = 0;
			}
		}
		,SNAP_PAGE_TO_CHANGE_FLING_TIME);

	}
	private void stopChangeTimer(){
		if(null != mChangeModeTimer){
			mChangeModeTimer.cancel();
			mChangeModeTimer = null;
		}

	}
	private Timer mCalTimer = null;
	private int mIndexFlag = 0;
	private int mPreIndexFlag = 0;
	private void calSnapTime(int index)
	{
		if(null != mCalTimer){
			mCalTimer.cancel();
			mCalTimer = null;
		...
		mPreIndexFlag = index;
		mCalTimer = new Timer(true);
		mCalTimer.schedule(new TimerTask(){
			public void run(){
				if(mPreIndexFlag == mIndexFlag){
					stopChangeTimer();
					mSnapImgNum = 0;
				}
			}
		}
		,SNAP_PAGE_CLEAR_TIME);
	...
	private void loadGif(final MovieData data){
		if(null != mLoadGifTask) mLoadGifTask.cancel();
		mLoadGifTask = mThreadPool.submit(new DecodeGifTask(data),
				new FutureListener<Movie>() {
			@Override
			public void onFutureDone(Future<Movie> future) {
				mLoadGifTask = null;	
				if(null == data) return;
				data.mMovie = future.get();
				if(null == data.mMovie) return;
