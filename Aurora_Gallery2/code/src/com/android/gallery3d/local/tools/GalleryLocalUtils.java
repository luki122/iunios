...
public class GalleryLocalUtils {
	private static final String TAG = "GalleryLocalUtils";

	private static String filepath = "paths.xml";
	...
	public static void setItemBottomBarVisibility(AuroraActionBar auroraActionBar, int position, boolean visibility) {
		if (auroraActionBar != null) {
			// LogUtil.d(TAG, "---position:" + position + " visibility:" +
			// visibility);
	...
	public static void viewImg(long id, Context mContext, int pos, boolean isImage, boolean isFavorite,boolean videoPath) {
		Uri baseUri = isImage ? MediaStore.Images.Media.EXTERNAL_CONTENT_URI : MediaStore.Video.Media.EXTERNAL_CONTENT_URI;
		baseUri = baseUri.buildUpon().appendPath(String.valueOf(id)).build();
		LogUtil.d(TAG, "--baseUri:" + baseUri);
		Intent intent = new Intent();
		intent.putExtra("AuroraFavorite", isFavorite);
		intent.setAction("aurora.aciton.photopage.view");
		intent.putExtra("AuroraOnlyVideo", videoPath);
		intent.setDataAndType(baseUri, isImage ? "image/*" : "video/*");
		try {
			mContext.startActivity(intent);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static CharSequence getEllipsizeEnd(String fileName, TextView textView, int size) {
		...
		return TextUtils.ellipsize(fileName.substring(fileName.lastIndexOf("/") + 1, fileName.length()), textView.getPaint(), size, TextUtils.TruncateAt.END);
	}
}
