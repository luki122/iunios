...
public class LocalImage extends LocalMediaItem {
	...
	private static final int INDEX_FAVORITE = 14;
    ...
    private void loadFromCursor(Cursor cursor) {
		...
		favorite = cursor.getInt(INDEX_FAVORITE);//paul add for UI_20
    }
    ...
    protected boolean updateFromCursor(Cursor cursor) {
		...
		favorite = uh.update(height, cursor.getInt(INDEX_FAVORITE));//paul add for UI_20
        return uh.isUpdated();
    }
    ...
    public static class ClearThumbnailImageRequest extends ImageCacheRequest {
        private String mLocalFilePath;

        ClearThumbnailImageRequest(GalleryApp application, Path path, int type, String localFilePath) {
        ...
        public Bitmap onDecodeOriginal(JobContext jc, final int type) {
        	//Get crop center image...
        	BitmapFactory.Options options = new BitmapFactory.Options();
            options.inPreferredConfig = Bitmap.Config.ARGB_8888;
            int targetSize = MediaItem.getTargetSize(type);
            Bitmap bitmap = DecodeUtils.decodeClearThumbnail(jc, mLocalFilePath, options, targetSize, type);
            //if(bitmap == null) {
            //	Log.e("SQF_LOG", "ERROR here:" + mLocalFilePath);
            //}
            return bitmap;
        }
    }
    
    public Job<Bitmap> requestClearThumbnail(int type) {
    	return new ClearThumbnailImageRequest(mApplication, mPath, type, filePath);
    }
    ...
    public static class LocalImageRequest extends ImageCacheRequest {
        ...
        public Bitmap onDecodeOriginal(JobContext jc, final int type) {
            ...
            analyzePNG(mLocalFilePath); //wenyongzhe2016.3.30
            return DecodeUtils.decodeThumbnail(jc, mLocalFilePath, options, targetSize, type);
        }
        
        //wenyongzhe2016.3.30
        private void analyzePNG(String path){
        	if(path.substring(path.length()-3).contains("png")){
        		isPNG = true;
        	}else
        		isPNG = false;
        }
    ...
    public void delete() {
		...
		mApplication.getContentResolver().delete(baseUri, "_id=?",
                ...
                */
	    //SQF MODIFIED TO:
        int rows = mApplication.getContentResolver().delete(baseUri, "_id=?", new String[]{String.valueOf(id)});
        // Remove file
		if (rows != 0) {
			try {
				...
				Log.e("SQF_LOG", "file: '" + filePath + "' couldn't be deleted", e);
				return;
			}
		}
