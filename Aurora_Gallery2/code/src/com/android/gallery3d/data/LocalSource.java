...
class LocalSource extends MediaSource {
    ...
    // Aurora <zhanggp> <2013-12-21> added for gallery begin	
	private static final int LOCAL_ALLSETS_ALBUM = 8;
	private static final int LOCAL_MERGEIMAGE_ALBUM = 9;
	private static final int LOCAL_MERGEVIDEO_ALBUM = 10;
	
	private static final int LOCAL_ALLFAVORITE_ALBUM = 11;
	private static final int LOCAL_MERGEIMAGE_FAV_ALBUM = 12;
	private static final int LOCAL_MERGEVIDEO_FAV_ALBUM = 13;
	// Aurora <zhanggp> <2013-12-21> added for gallery end
    ...
    public LocalSource(GalleryApp context) {
		...
		// Aurora <zhanggp> <2013-12-21> added for gallery begin
		mMatcher.add("/local/allsets/*", LOCAL_ALLSETS_ALBUM);
		mMatcher.add("/local/mergeimage/*", LOCAL_MERGEIMAGE_ALBUM);
		mMatcher.add("/local/mergevideo/*", LOCAL_MERGEVIDEO_ALBUM);
		
		mMatcher.add("/local/allfavorite/*", LOCAL_ALLFAVORITE_ALBUM);
		mMatcher.add("/local/favmergeimage/*", LOCAL_MERGEIMAGE_FAV_ALBUM);
		mMatcher.add("/local/favmergevideo/*", LOCAL_MERGEVIDEO_FAV_ALBUM);
    ...
    public MediaObject createMediaObject(Path path) {
        ...
        switch (mMatcher.match(path)) {
			...
			// Aurora <zhanggp> <2013-12-21> added for gallery begin
			case LOCAL_MERGEIMAGE_ALBUM:
				return new AuroraLocalAlbum(path, app, mMatcher.getIntVar(0), true, false);
			case LOCAL_MERGEVIDEO_ALBUM:
				return new AuroraLocalAlbum(path, app, mMatcher.getIntVar(0), false, false);
			case LOCAL_ALLSETS_ALBUM:{
				int bucketId = mMatcher.getIntVar(0);
				MediaSet mediaSets[] = new MediaSet[2];
				DataManager dataManager = app.getDataManager();
				mediaSets[0] = (MediaSet)dataManager.getMediaObject(
                        LocalAlbumSet.PATH_MERGE_IMAGE.getChild(bucketId));
				mediaSets[1] = (MediaSet)dataManager.getMediaObject(
                        LocalAlbumSet.PATH_MERGE_VIDEO.getChild(bucketId));
				Comparator<MediaItem> comp = DataManager.sDateTakenComparator;
                return new LocalMergeAlbum( path, comp, mediaSets, bucketId);
				}
			
			case LOCAL_MERGEIMAGE_FAV_ALBUM:
				return new AuroraLocalAlbum(path, app, mMatcher.getIntVar(0), true, true);
			case LOCAL_MERGEVIDEO_FAV_ALBUM:
				return new AuroraLocalAlbum(path, app, mMatcher.getIntVar(0), false, true);
			case LOCAL_ALLFAVORITE_ALBUM:{
				int bucketId = mMatcher.getIntVar(0);
				MediaSet mediaSets[] = new MediaSet[2];
				DataManager dataManager = app.getDataManager();
				mediaSets[0] = (MediaSet)dataManager.getMediaObject(
                        LocalAlbumSet.PATH_MERGE_IMAGE_FAV.getChild(bucketId));
				mediaSets[1] = (MediaSet)dataManager.getMediaObject(
                        LocalAlbumSet.PATH_MERGE_VIDEO_FAV.getChild(bucketId));
				Comparator<MediaItem> comp = DataManager.sDateTakenComparator;
                return new LocalMergeAlbum( path, comp, mediaSets, bucketId);
				}
			// Aurora <zhanggp> <2013-12-21> added for gallery end
