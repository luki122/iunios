...
public class LocationClustering extends Clustering {
    ...
    public HashSet<String> run(Container container) {
		int total = container.items.size();
        ...
        for (int i = 0; i < total; i++) {
			lat = container.items.get(i).latitude;
			lng = container.items.get(i).latLong;
    ...
    public void run(MediaSet baseSet) {
        ...
        for (ArrayList<SmallItem> cluster : clusters) {
            String name = generateName(cluster, geocoder, false);//paul for UI_20 start
            if (name != null) {
                mNames.add(name);
    ...
    private static String generateName(ArrayList<SmallItem> items,
            ReverseGeocoder geocoder, boolean simple) {//paul for UI_20 start
        ...
        return geocoder.computeAddress(set, simple);
    }
