...
public class XCloudTaskListenerManager {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            switch (msg.what) {
            	...
            	case MSG_UPDATE_LIST:
            		...
            		updateList(b, true);//wenyongzhe 2016.2.23
            		notifyUi(false);
            	break;
    ...
    public boolean foundInFakeTaskManager(FileTaskStatusBean bean) {
    	String key = bean.getSource() + bean.getTarget();
    	if(bean.getType() == FileTaskStatusBean.TYPE_TASK_UPLOAD) {
...
//					bean.setStatusTaskCode(FileTaskStatusBean.STATE_TASK_CREATE);//wenyongzhe 2016.2.23 disable
					
					removeBeanFromFakeTaskManager(bean);//wenyongzhe 2016.2.23
				}else{
...
//				return;
			...
			if (bean.getType() == FileTaskStatusBean.TYPE_TASK_UPLOAD) {
				...
				} else if( bean.getStatusTaskCode()==FileTaskStatusBean.STATE_TASK_DONE){//wenyongzhe 2016.2.23
...
//	}
	...
	private void removeBeanFromFakeTaskManager(FileTaskStatusBean bean) {
		String key = bean.getSource() + bean.getTarget();
		if(bean.getType() == FileTaskStatusBean.TYPE_TASK_UPLOAD) {
			if(mFakeTaskManager.getFakeUploadBeans().containsKey(key)) {
				mFakeTaskManager.getFakeUploadBeans().remove(key);
...
//			LogUtil.d(TAG, "------onGetTaskStatus bean :" + bean.toString());
		...
		public void onGetTaskListFinished(
			...
			*/
			...
			notifyUi(false);//wenyongzhe 2016.3.1
	...
	//wenyongzhe
	public void removeAllFakeByType(int type) {
		if(type == FileTaskStatusBean.TYPE_TASK_UPLOAD) {
			mUploadBeans.clear();
