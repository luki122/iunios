...
public class BaiduAlbumUtils {
	...
	public interface INotifyListener {
		void baiduDownloadNotify(FileTaskStatusBean bean);

		void baiduUploadNotify(FileTaskStatusBean bean);
	...
	public void setINotifyListener(INotifyListener mNotifyListener) {
	...
	/**
	 * 查询登陆
	 * //wenyongzhe 2016.2.18 
	 * @param forceLogin
	 */
	public void getLoginBaiduInfo(String token, boolean forceLogin) {
		mThirdAccessToken = token;
		if (AlbumConfig.IUNI_TEST) {
			mThirdAccessToken = AlbumConfig.IUNI_TEST_TOKEN;
		}
		AccountProxy proxy = AccountProxy.getInstance();
		proxy.setThirdToken(mThirdAccessToken);
		proxy.getAccountInfoNotLogin(loginListener, forceLogin,
				AuthInfo.AUTHTYPE_NEWEXPLICITBIND);
	}
			...
			if (baiduTaskListener != null && bean != null) {
				if (bean.getType() == FileTaskStatusBean.TYPE_TASK_UPLOAD) {
					...
					if(mNotifyListener!=null){
						mNotifyListener.baiduUploadNotify(bean);
					}
				} else {
					baiduTaskListener.baiduTaskStatus(bean);
					if(mNotifyListener!=null){
						mNotifyListener.baiduDownloadNotify(bean);
					}
		...
		public void onComplete(AccountInfo account) {
			...
			if (baiduinterface != null) {
				...
				if (TextUtils.isEmpty(cursor)) {
					//wenyongzhe 2016.2.16 disable album
