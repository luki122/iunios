...
public class AlbumDataLoader {
    ...
    public static final int MAX_LOAD_COUNT = 180;//96;//64;//Iuni <lory><2014-01-21> add begin, 
...
//    	public void onAuroraContentChanged(int index, int type);
    ...
    private boolean mToUpdateSlot = true;//Iuni <lory><2014-02-22> add begin
    ...
    public AlbumDataLoader(AbstractGalleryActivity context, MediaSet mediaSet) {
		...
		mLocationGetter = new LocationClustering(mContext);//paul add for UI_20
    }
    ...
    public void setFlagToNotUpdateViews(boolean toUpdateSlot) {
    	if (mToUpdateSlot != toUpdateSlot) {
    		mToUpdateSlot = toUpdateSlot;
		}
		return;
	}
    ...
    public void resume() {
        ...
        //mToUpdateSlot = true;
        if (modeHandler != null) {
        	mToUpdateSlot = modeHandler.getMenuDialogActive()?false:true;
		} else {
			mToUpdateSlot = true;
		}
	...
	LocationClustering mLocationGetter = null;
	private static final String sep = "\u3001";
	private String genNames(Container c){
		HashSet<String> list = mLocationGetter.run(c);
    ...
    private class MySourceListener implements ContentListener {
        ...
        public void onContentDirty() {
        	if (MySelfBuildConfig.USEGALLERY3D_FLAG) {
        		if (mReloadTask != null && mToUpdateSlot){
					//Aurora <paul> <2014-05-19> start
    ...
    public void auroraUpdateDatabase() {
    	if (mReloadTask != null && mToUpdateSlot) mReloadTask.notifyDirty();
	}
    ...
    */
	...
	//paul add start
	private AdressThreadTask mAddressTask;
	private AdressThreadTask mAddressMonthTask;
	//paul add end
    ...
    static final String[] AURORA_PROJECTION_IMG =  {
        ...
        ImageColumns.LONGITUDE//paul add for UI_20
	};
    ...
    static final String[] AURORA_PROJECTION_VIDEO = {
        ...
        VideoColumns.LONGITUDE//paul add for UI_20
	};
    
    public class Container {
        public String title;
		public String address;//paul add for UI_20
        public ArrayList<AuroraItemInfos> items;
    ...
    public class AuroraItemInfos {
    	int 	id;
    	public long	time = 0;
    	public double latLong;
		public double latitude;//paul add for UI_20
    	
    	public AuroraItemInfos(int id, long time, double latLong[]) {
			this.id = id;
			this.time = time;
			this.latitude = latLong[0];//LATITUDE
			this.latLong = latLong[1];//paul add for UI_20
		}
    ...
    private void initDataBaseList() {
    	...
    	synchronized (this) {
        	...
        	if (getCloums() == SlotView.MONTH_CLOUMNS && GalleryUtils.isPortrait(mContext)) {
    ...
    public void setSortMode(int num) {
    	synchronized (this) {
    		if (num == SlotView.MONTH_CLOUMNS && GalleryUtils.isPortrait(mContext) && mDateContainers != null) {
        		mContainers = mDateContainers;
    		} else {
	...
	private void cancelLoadAddress(){
        ...
        if (mAddressMonthTask != null) {
            mAddressMonthTask.cancel(true);
        }
	}
	private void startLoadAddress(boolean isMonth){
		if(isMonth){
	        if (mAddressMonthTask != null) {
	            mAddressMonthTask.cancel(true);
	        }
			mAddressMonthTask = new AdressThreadTask();
			mAddressMonthTask.execute(new Boolean(true));
		} else {
	        if (mAddressTask != null) {
	            mAddressTask.cancel(true);
	        }
			mAddressTask = new AdressThreadTask();
			mAddressTask.execute(new Boolean(false));
		}
	}
	
    private class AdressThreadTask extends AsyncTask<Boolean, Void, Void> {
        protected Void doInBackground(Boolean... params) {
			boolean isMonth = params[0];
			ArrayList<Container> containerSet = mDayContainers;
			if(isMonth){
				containerSet = mDateContainers;
			}
			if(containerSet == null){
				return null;
			}
			for (Container c : containerSet) {
				c.address = genNames(c);
			}
			return null;
        }

	    protected void onPostExecute(Void result) {
			mDataListener.toInvalidate();
	    }
			...
			if (imgCursor != null) {
				...
				if (imgCursor.moveToFirst()) {
					do {
						...
						latLong[0] = imgCursor.getDouble(2);//LATITUDE
						latLong[1] = imgCursor.getDouble(3);//LONGITUDE
						AuroraItemInfos infos = new AuroraItemInfos(idIndex, time, latLong);
			...
			if (videoCursor != null) {
				...
				if (videoCursor.moveToFirst()) {
					do {
						...
						double latLong[] = new double[2];
						latLong[0] = videoCursor.getDouble(2);//LATITUDE
						latLong[1] = videoCursor.getDouble(3);//LONGITUDE
						AuroraItemInfos infos = new AuroraItemInfos(idIndex, time, latLong);
						mAuroraList.add(infos);
    ...
    private void initAuroraMonthContainers() {
    	...
    	synchronized (dateContainerLock) {
			...
			for (AuroraItemInfos item : mAuroraList) {
				if(mDateContainers.size() == 0){
					time = sdf2.format(new Date(item.time));
					container = new Container(time);
					mDateContainers.add(container);
				}
				
				time = sdf2.format(new Date(item.time));
				if (container.title.equals(time)) {
				} else {
					container = new Container(time);
					mDateContainers.add(container);
				}
			...
			startLoadAddress(true);//paul add
    	}
    }
    ...
    private void IniAuroraContainers(ArrayList<AuroraItemInfos> items) {
        ...
        for (AuroraItemInfos item : items) {
			if(mDayContainers.size() == 0){
				time = sdf.format(new Date(item.time));
				container = new Container(time);
				mDayContainers.add(container);
			}
		...
		startLoadAddress(false);//paul add
    }

	
    public int getSixSlotCount(int num) {
		int total = 0;
		if (num == SlotView.MONTH_CLOUMNS && GalleryUtils.isPortrait(mContext)) {
...
//				}
    ...
    //paul add start
    public Container getDateContainer(int position) {
		int header = 0;
		int total = 0;
		...
		for (header = 0; header < size; ++header) {
			Container c = mContainers.get(header);
			total += (c.items.size() + 1);//header(+1) and item count
			if (position <= (total - 1)) {//(total - 1) is index
				return c;
			}
		}
		return null;
    ...
    public int getNoHeaderIndex(int position) {
		int header = 0;
		int total = 0;
	...
	public int mapMonthIndex(int dayIndex){
		int header = 0;
		int totalIndex = -1;
		int index = getNoHeaderIndex(dayIndex);
		if(-1 == index) return -1;
		final int size = mDateContainers.size();
		for (header = 0; header < size; ++header) {
			Container c = mDateContainers.get(header);
			totalIndex += c.items.size();//real item index
			if(totalIndex >= index){
				index += header + 1;
				return index;
			}
	...
	public int mapDayIndex(int dayIndex){
		int header = 0;
		int totalIndex = -1;
		int index = getNoHeaderIndex(dayIndex);

		if(-1 == index) return -1;
		final int size = mDayContainers.size();
		for (header = 0; header < size; ++header) {
			Container c = mDayContainers.get(header);
			totalIndex += c.items.size();//real item index
			if(totalIndex >= index){
				index += header + 1;
				return index;
	...
	public int[] getNoheaderIndexByPos(int x, int y, int itemH, int headerH, int colNum){
		int header = 0;
		int totalIndex = 0;
		int[] ret = new int[2];
		ret[0] = ret[1] = -1;
		if (mContainers == null || x < 0 || y < 0) {
			return ret;
		}
		final int size = mContainers.size();
		int currentY = 0;
		...
		for (header = 0; header < size; ++header) {
			Container c = mContainers.get(header);
			pervousY = currentY;
			currentY += (c.items.size() + colNum - 1) / colNum * itemH + headerH;//header(+1) and item count
			if (y <= currentY) {
				int row = (y - pervousY - headerH);
				if(row < 0) return ret;//err or header
				row /= itemH;
				int col = x / itemH;//index, -1
				if( row * colNum + col >= c.items.size()) return ret;//none index
				ret[0] = (colNum * row + col) + totalIndex;//no header
				ret[1] = ret[0] + header + 1;//with header
				return ret;
			}
			totalIndex += c.items.size();
		}
		return ret;
    ...
    public ItemPositionInfo getItemInfo(int position, int num, boolean month) {
    	int rows = 0;
    	...
    	int t_num = 0;
    	int totalRowSize = 0;
		ArrayList<Container> con = mDayContainers;
		if(month){
			con = mDateContainers;
		}
    	synchronized (this) {
    		try {
            	...
            	for (Container c : con) {
            		...
            		curTotalNum += con.get(header).items.size();
    ...
    public ItemPositionInfo getRowsAndCloumns(int position, int num) {
    	...
    	synchronized (this) {
    		try {
				...
				Log.e(TAG, "getRowsAndCloumns fail -----");
			}
		}
