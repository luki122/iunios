...
public class StateManager {
	...
	public PhotoPage createPhotoPage(Bundle data){
        mPhotoPage = null;
        try {
            mPhotoPage = PhotoPage.class.newInstance();
        } catch (Exception e) {
            throw new AssertionError(e);
        }
        mPhotoPage.initialize(mActivity, data);
        mPhotoPage.mResult = new ActivityState.ResultEntry();
        mPhotoPage.mResult.requestCode = AlbumPage.REQUEST_PHOTO;
		return mPhotoPage;
	}
    ...
    public void setSelectIndex(int index) {
    	if (!mStack.isEmpty()) {
            getTopState().onSetSelectIndex(index);
        }
		return;
	}
    public boolean onMyKeyDownEvent(int keyCode, KeyEvent event) {
    	if (!mStack.isEmpty()) {
           return getTopState().onMyKeyDownEvent(keyCode, event);
        }
		return false;
	}
    ...
    public ActivityState getTopState() {
        ...
        //Aurora <SQF> <2014-6-26>  for NEW_UI begin
        //ORIGINALLY:
    	 //Utils.assertTrue(!mStack.isEmpty());
        //SQF MODIFIED TO:
    	if(mStack == null || mStack.isEmpty()) return null;
        //Aurora <SQF> <2014-6-26>  for NEW_UI end
