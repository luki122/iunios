...
public class PhotoPage extends ActivityState implements PhotoView.Listener,
	...
	private static final int MSG_ENABLE_BOTTOM_MENU_CLICKABLE = 17;//wenyongzhe2016.3.30 bug21979
	...
	public void onCreate(Bundle data, Bundle restoreState) {
		...
		mSelectionManager.setSelectionListener(this);//wenyongzhe 2016.1.26 new_ui
		...
		mHandler = new SynchronizedHandler(mActivity.getGLRoot()) {
			...
			public void handleMessage(Message message) {
				switch (message.what) {
				case MSG_ENABLE_BOTTOM_MENU_CLICKABLE: {//wenyongzhe2016.3.30 bug21979
					mBottomControls.setEnableBottomMenuClick(true);
					break;
				}
				case MSG_HIDE_BARS: {
					hideBars();
		...
		if (mSetPathString != null) {
			...
			//paul for BUG #21575
			if(null == mediaItem){
				Log.e(TAG, "null == mediaItem finishState!!!");
...
//		initMoreMenu();
	...
	public boolean canDisplayTopControl(int control) {
		...
		case R.id.photopage_top_control_bgView:
			return showEdit;
//		case R.id.rightButton://wenyongzhe
			// Aurora <SQF> <2014-6-18> for NEW_UI begin
			// ORIGINALLY:
...
//		case R.id.leftTextView:
	...
	public boolean canDisplayBottomControl(int control) {
		...
		if (!showEdit)
			return false;
//wenyongzhe new_ui 2016.1.27 start

		if(mCurrentPhoto.isFavorite()){
			switch (control) {
			case R.id.photopage_bottom_images:
			case R.id.photopage_bottom_control_share:
			case R.id.photopage_bottom_control_delete:
			case R.id.photopage_bottom_control_unfavorite: 	
			case R.id.photopage_bottom_control_edit_more: 	
			...
			case R.id.photopage_bottom_control_share:
			case R.id.photopage_bottom_control_delete:
				//wenyongzhe 2016.1.11 new_ui
			case R.id.photopage_bottom_control_favorite: 	
			case R.id.photopage_bottom_control_edit_more: 	
...
//wenyongzhe new_ui 2016.1.27 end
		}
		return false;
		/*
	...
	//paul add start
	private void toggleFavorite(boolean toSet, Path path){
		mSelectionManager.deSelectAll();
		mSelectionManager.toggle(path);
		mMenuExecutor.setFavoriteState(toSet);
		mMenuExecutor.setItemsFavorite();
		mBottomControls.toggleFavIcon();
	}
	//paul add end
	...
	public void onBottomControlClicked(int control) {
		...
		mBottomControls.setEnableBottomMenuClick(false);
		mHandler.sendEmptyMessageDelayed(MSG_ENABLE_BOTTOM_MENU_CLICKABLE, 300);
		...
		refreshHidingMessage();
		MediaItem current = mModel.getMediaItem(0);
		initMoreMenu();//wenyongzhe 2016.2.19
...
//					R.plurals.delete_selection, 1);
			confirmMsg = mActivity.getResources().getString(R.string.number_of_files_selected_del_one);//wenyongzhe
			mSelectionManager.deSelectAll();
			mSelectionManager.toggle(path);
			mMenuExecutor.onMenuClickedEx(R.id.action_delete, confirmMsg,
					mDeleteDialogListener);
			return;
		//wenyongzhe new_ui 2016.1.27
		case R.id.photopage_bottom_control_favorite: 	
			toggleFavorite(true, path);
			return;
		case R.id.photopage_bottom_control_unfavorite: 	
			toggleFavorite(false, path);
			return;
		case R.id.photopage_bottom_control_edit_more: 	
			 mActivity.showAuroraMenu(mActivity.getCurrentView(), 
	 ...
	 //wenyongzhe 2016.1.5 viewpager new ui satrt
//   public void changeFavoriteButton(View v){
//	   aurora.widget.AuroraAnimationImageView mAuroraAnimationImageView = (aurora.widget.AuroraAnimationImageView)v;
//	   Log.e(TAG, "ffffff--"+mAuroraAnimationImageView.getDrawable().getCurrent().getConstantState());
//	   Log.e(TAG, "ffffff--"+mActivity.getDrawable(com.aurora.R.drawable.aurora_bottombar_like_selector).getConstantState());
//	   Log.e(TAG, "ffffff--"+mActivity.getDrawable(com.aurora.R.drawable.aurora_bottombar_unlike_selector).getConstantState());
//	   if( mAuroraAnimationImageView.getDrawable().getCurrent().getConstantState() == mActivity.getDrawable(com.aurora.R.drawable.aurora_bottombar_like_selector).getConstantState()){
//		   mAuroraAnimationImageView.setImageDrawable(mActivity.getDrawable(com.aurora.R.drawable.aurora_bottombar_unlike_selector));
//	   }else{
//		   mAuroraAnimationImageView.setImageDrawable(mActivity.getDrawable(com.aurora.R.drawable.aurora_bottombar_like_selector));
//	   }
//	}
  public void initMoreMenu(){
	  ...
	  MediaItem current = mModel.getMediaItem(0);
		if(current instanceof LocalImage) {
			if(!"image/gif".equalsIgnoreCase(current.getMimeType()) ){
				mActivity.setAuroraMenuItems(R.menu.filtershow_menu_more_image);
			}else{
				mActivity.setAuroraMenuItems(R.menu.filtershow_menu_more_image_gif);
			}
		} else if(current instanceof LocalVideo) {
			mActivity.setAuroraMenuItems(R.menu.filtershow_menu_more_video);
		}
		//wenyongzhe 2016.2.22 end
		 mActivity.getAuroraMenu().setAnimationStyle(com.aurora.R.style.AuroraMenuRightBottomAnimation);
		 
	 mActivity.setAuroraSystemMenuCallBack(new OnAuroraMenuItemClickListener() {
		@Override
		public void auroraMenuItemClick(int itemId) {
			...
			switch (itemId) {
    		case R.id.aurora_album_page_move:
    			ArrayList<MediaItem> pathList = new ArrayList<>();//wenyongzhe 2016.3.7
    			pathList.add(current);
    		
    			PhotoPopupWindowUtil popupWindowUtil = PhotoPopupWindowUtil.getInstance(mActivity);//wenyongzhe 2016.3.7
    			popupWindowUtil.showSelectPopupWindow(mActivity.getResources().getString(R.string.aurora_actionbar_move_to), mActivity.getWindow().getDecorView());
    			popupWindowUtil.setSelectPath(pathList);
    			break;
    		case R.id.aurora_album_page_set_for:
    			...
    			return;
    		case R.id.aurora_album_page_edite:
//    			mSelectionManager.deSelectAll();
//    			mSelectionManager.toggle(path);
...
//    					mConfirmDialogListener);
    			launchPhotoEditor();
    			return;
    		case R.id.aurora_album_page_slide_play:
    			AuroraSlideshowPage();
    			return;
    		case R.id.aurora_album_page_detail:
    			showDetails();
    			return;
			default:
				break;
...
//	 mActivity.getAuroraMenu().setAnimationStyle(com.aurora.R.style.AuroraMenuRightBottomAnimation);
	...
	private void launchPhotoEditor() {
		...
		((Activity) mActivity).startActivityForResult(
				intent, REQUEST_EDIT);//wenyongzhe new_ui
		overrideTransitionToEditor();
	}
	...
	public void playVideo(Activity activity, Uri uri, String title) {
        try {
            ...
            activity.overridePendingTransition(R.anim.activity_movie_open_enter, R.anim.activity_movie_open_exit);//wenyonghze2016.3.14
	...
	public void setCurrentPhotoByIntentEx(Intent intent) {
		...
		if (path != null) {
			...
			/**
			 * //wenyongzhe 2016.3.3 disable bug 20425
