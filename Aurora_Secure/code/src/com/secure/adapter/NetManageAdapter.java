...
import com.secure.activity.NetManageActivity;
import com.secure.adapter.PermissionManageAdapter.OptimizeState;
...
public class NetManageAdapter extends ArrayAdapter<BaseData> implements 
	...
	public NetManageAdapter(Activity activity,
    ...
    private void initData(){
        mBackgroundThread = new HandlerThread("NetManageAdapter:Background");
	...
	public View getView(int position, View convertView, final ViewGroup parent) {	
		...
		int progress;
		long totalNetFlow = ((NetManageActivity)getContext()).getTotalNetFlow();
		if(totalNetFlow>0){
			progress = (int)(item.getApkTotalNetBytes()*100/totalNetFlow);
			//对于百分比不够百分之一的，按照百分之一来显示
			if(progress == 0 && item.getApkTotalNetBytes()>0){
				progress = 1;
			}				
			holder.getProgressBar().setProgress(progress);
			holder.getNetworkTraffic().setText(Utils.dealMemorySize(getContext(), item.getApkTotalNetBytes()));
		}else{
