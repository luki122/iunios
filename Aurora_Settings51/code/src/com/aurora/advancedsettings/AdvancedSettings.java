package com.aurora.advancedsettings;
...
public class AdvancedSettings extends AuroraSettingsPreferenceFragment
	...
	private static final String KEY_WECHAT_SET = "wechat_set";

	private static final boolean MUILTI_SIM_ENABLE = GnTelephonyManager
			.isMultiSimEnabled();
			...
			if (serviceState != null) {/*
				if (serviceState.getState() == ServiceState.STATE_IN_SERVICE) {
					...
					if (mTwoSimSettings == null
							&& getPreferenceManager() != null
							&& getActivity() != null) {
						mTwoSimSettings = getPreferenceManager()
								.createPreferenceScreen(getActivity());
						Log.i("gary", "-----------------sssssss----------");
						mTwoSimSettings.setOrder(2);
						mTwoSimSettings.setKey(KEY_TWO_SIM_SETTINGS);
						mTwoSimSettings
								.setTitle(R.string.aurora_sim_settings_title);
						mTwoSimSettings.setFragment(TwoSimSettings.class
								.getName());
					...
					if (mSimSettings == null && getActivity() != null) {
						...
						mSimSettings
								.setOnPreferenceClickListener(AdvancedSettings.this);
					...
					if (mSim02Settings == null && getActivity() != null) {
						...
						mSim02Settings
								.setTitle(R.string.aurora_sim_02_settings_title);
					...
					Log.i("gary",
							"serviceState.getState() == not STATE_IN_SERVICE");
	...
	private AuroraPreferenceScreen mPowerManagerPreferenceScreen;
	...
	*/}
	...
	public void onCreate(Bundle icicle) {
		...
		mPhone.listen(sPhoneStateListener,
	...
	/*	mTwoSimSettings = (AuroraPreferenceScreen) findPreference(KEY_TWO_SIM_SETTINGS);
		 ...
		 * if(!isCanUseSim(0) || !isCanUseSTK(0)){
		 * getPreferenceScreen().removePreference(mSimSettings); mSimSettings =
		 * null; } if(!isCanUseSim(1) || !isCanUseSTK(1)){
		 * getPreferenceScreen().removePreference(mSim02Settings);
		 * mSim02Settings = null; }
		 * 
		 * } else { if(!isCanUseSim() || !isCanUseSTK()) {
		 * getPreferenceScreen().removePreference(mSimSettings); mSimSettings =
		 * null; } getPreferenceScreen().removePreference(mSim02Settings);
		 * mSim02Settings = null; }
		 */
		...
		AuroraPreferenceScreen developScreen = (AuroraPreferenceScreen) findPreference(KEY_DEVELOPMENT_SETTINGS);
		//add by luolaigang for remove wechat set
		if(!DI_ON){
			removePreference(KEY_WECHAT_SET);
		}
		
		if (developScreen != null) {
			boolean isShowDevelop = getActivity().getSharedPreferences(
					DevelopmentSettings.PREF_FILE, Context.MODE_PRIVATE)
					.getBoolean(DevelopmentSettings.PREF_SHOW, false);
			if (!isShowDevelop) {
				getPreferenceScreen().removePreference(developScreen);
			}
		}
		/*
		 * String buildModel = Build.MODEL; // Log.v("xiaoyong",
		 * "productname is " + buildModel);
		 * 
		 * if (factoryScreen != null && !buildModel.equals("GT-I9500") &&
		 * !buildModel.contains("SM-N900")) {
		 * getPreferenceScreen().removePreference(factoryScreen);
		 * getPreferenceScreen
		 * ().removePreference((AuroraPreference)findPreference
		 * ("pref_category_factory_settings"));
		 * 
		 * }
		 */
		...
		/*if (factoryScreen != null) {
			if (deviceName.contains("I9500") || deviceName.contains("N900")) {
				...
				getPreferenceScreen()
						.removePreference(
								(AuroraPreference) findPreference("pref_category_factory_settings"));
			...
			} else if (deviceName.contains("OnePlusOne") 
														 * ||
														 * deviceName.contains
														 * ("FIND7")
														 ) {
				 ...
				 * factoryScreen.setTitle(
				 * R.string.aurora_oneplus_factory_settings_title); }else{
				 * factoryScreen
				 * .setTitle(R.string.aurora_find7_factory_settings_title); }
				 * 
				 * factoryScreen.setFragment(
				 * "com.android.settings.OnePlusFactorySettings");
		...
		mApplicationPreferenceScreen
				.setOnPreferenceClickListener(new AuroraPreference.OnPreferenceClickListener() {
		...
		mPowerManagerPreferenceScreen = (AuroraPreferenceScreen) findPreference("battery_settings");
		mPowerManagerPreferenceScreen
				.setOnPreferenceClickListener(new AuroraPreference.OnPreferenceClickListener() {
					@Override
					public boolean onPreferenceClick(AuroraPreference preference) {
						startPowerManager();
		...
		/*
		 * Intent applicationIntent = new Intent(); ComponentName
		 * applicationName = new ComponentName("com.aurora.secure",
		...
		ComponentName accountName = new ComponentName(
				"com.aurora.auroraaccountsettings",
	...
	private void startPowerManager() {
		Intent intent = new Intent();
		intent.setAction("android.settings.MANAGE_POWER_SETTINGS");
	...
	public boolean isCanUseSTK() {
		...
		ComponentName componentName = new ComponentName("com.android.stk",
	...
	public boolean isCanUseSim() {
		try {
			...
			// getSystemService(Context.TELEPHONY_SERVICE); // del qy 2014 07 16
	...
	public boolean isCanUseSTK(int slotId) {
		...
		/* if(slotId == 0) { */
		componentName = new ComponentName("com.android.stk",
				"com.android.stk.StkLauncherActivity");
		/*
		 * } else { if(AuroraTelephoneManager.isMtkGemini()){ componentName =
		 * new ComponentName("com.android.stk",
		 * "com.android.stk.StkLauncherActivityII"); }else{ componentName = new
	...
	public boolean isCanUseSim(int slotId) {
		return TelephonyManager.SIM_STATE_READY == GnTelephonyManager
