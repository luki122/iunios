...
public class MemoryExts {
    ...
    private AuroraPreferenceCategory mDefaultWriteDiskContainer;
    ...
    private AuroraListPreference mInstallLocationContainer;
    private AuroraPreference mApkInstallerEntrance;

    private StorageVolume[] mStorageVolumes;

    private AuroraPreferenceScreen mRootContainer;

    public MemoryExts(Activity activity, AuroraPreferenceScreen preferenceScreen,
    ...
    private void initInstallLocation() {
        ...
        if (mInstallLocationEnabled) {
            mInstallLocationContainer = new AuroraListPreference(mActivity);
    ...
    private void initApkInstaller() {
        ...
        if (mIsAppInstallerInstalled) {
            mApkInstallerEntrance = new AuroraPreference(mActivity);
    ...
    private void initDefaultWriteDisk() {
        mDefaultWriteDiskContainer = new AuroraPreferenceCategory(mActivity);
        ...
        for (int i = 0; i < mStorageWritePathList.length; i++) {
            ...
            if (Environment.MEDIA_MOUNTED.equals(volumeState)) {
                if (!mWritePathAdded[i]) {
                    volumeItemPreference.setButtonToCycle(true);
...
//        }
    ...
    public boolean isAddPhysicalCategory(StorageVolume volume) {
        return !(FeatureOption.MTK_SHARED_SDCARD
        		...
        		&& volume.isEmulated())
        		&&(volume.getPath().startsWith(Environment.DIRECTORY_USBOTG));
    }
