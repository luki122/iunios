...
//    private RadioButtonPreference mBicr;
    ...
    private List<AuroraCheckBoxPreference> mRadioButtonPreferenceList = new ArrayList<AuroraCheckBoxPreference>();

    private boolean mNeedUpdate = true;
    private boolean mNeedExit = false;

    public AuroraPreferenceScreen addUsbSettingsItem(UsbSettings usbSettings) {
        AuroraPreferenceScreen root = usbSettings.getPreferenceScreen();
        if (root == null) return null;

        mMtp = (AuroraCheckBoxPreference) root.findPreference(KEY_MTP);
        mMtp.setOnPreferenceClickListener(usbSettings);
        mRadioButtonPreferenceList.add(mMtp);

        mPtp = (AuroraCheckBoxPreference) root.findPreference(KEY_PTP);
        mPtp.setOnPreferenceClickListener(usbSettings);
...
/*
        ...
        if (umsExist) {
            ...
            AuroraPreferenceCategory usbConnectionCategory = (AuroraPreferenceCategory) root
...
/*        // Add charge only Mode
       ...
       /* // Add BUild-in CD Mode
        ...
        if (bicrExist) {
        	AuroraPreferenceCategory bicrCategory = new AuroraPreferenceCategory(context);
    ...
    public void updateEnableStatus(boolean enabled) {
        for (AuroraCheckBoxPreference preference : mRadioButtonPreferenceList) {
            preference.setEnabled(enabled);
        }
    }

    public void updateCheckedStatus(String function) {
    	AuroraCheckBoxPreference currentUsb;
        ...
        for (AuroraCheckBoxPreference usb : mRadioButtonPreferenceList) {
    ...
    public String getFunction(AuroraPreference preference) {
     ...
     */
    public boolean isMakeDefault(AuroraPreference preference) {
        //return !(preference == mMtp);
        return false;
    }
