...
public class DeviceInfoSettingsExts {
    ...
    private AuroraPreferenceScreen mRootContainer;
    private AuroraPreferenceFragment mPreferenceFragment;
    private Resources mRes;

    public DeviceInfoSettingsExts(Activity activity, AuroraPreferenceFragment fragment) {
        mActivity = activity;
    ...
    public void initMTKCustomization(AuroraPreferenceGroup parentPreference) {
    ...
    private void updateTitleToActivityLabel(String key) {
    	//-------------updateTitle from by JM begin-------------
    		Log.d(TAG, " updateTitle  come in  :::::");
    		AuroraPreference preference = findPreference(key);
    		if (preference == null) {
    			return;
    		}
    		//mtk_system_update
    		if(key.equals(KEY_MTK_SYSTEM_UPDATE))
    		{
    			preference.setTitle(R.string.system_update_settings_list_item_title);
    		}
    		//-------------end-------------
    ...
    private void initBasebandVersion() {
        ...
        if (hasExternalModem && !FeatureOption.PURE_AP_USE_EXTERNAL_MODEM) {
            ...
            if (FeatureOption.MTK_C2K_SUPPORT) {
                baseband2 = "cdma.version.baseband"; // Aurora linchunhui modify 20160222
            } else if (modemSlot == 1) {
                baseband2 = "cdma.version.baseband1";
            }
    ...
    private AuroraPreference findPreference(String key) {
    ...
    private int getExternalModemSlot() {
        int modemSlot;
        String md = SystemProperties.get("ril.external.md", getString(R.string.device_info_default));
        if (md.equals(getString(R.string.device_info_default))) {
            modemSlot = PhoneConstants.SIM_ID_1;
        } else {
            modemSlot = Integer.valueOf(md).intValue();
        }
        Log.d(TAG, "modemSlot = " + modemSlot);
        return modemSlot;
    }

    public void onCustomizedPreferenceTreeClick(AuroraPreference preference) {
    ...
    /**
	 * SystemUpdate from by JM-----begin-----
	 * @param preference
	 */
    private void systemUpdateEntrance(AuroraPreference preference) {
        if (FeatureOption.MTK_SYSTEM_UPDATE_SUPPORT) {
            startActivity("gn.com.android.update",
					"gn.com.android.update.ui.UpdateUiActivity");
    ...
    private void softwareUpdateEntrance(AuroraPreference preference) {
