...
public class CdmaSimStatus {
    ...
    public void updateNetworkType(String key, String networktype) {
        if (CdmaUtils.getSIMCardType(mSubInfo.getSimSlotIndex()) != CdmaUtils.NOT_CT_SIM) {
            Log.d(TAG,"updateNetworkType = " + networktype);
            String cdmaNetworkTypeName = renameNetworkTypeName(networktype);
            String lteNetWorkTypeName = null;
            if (mServiceState != null && isRegisterUnderLteNetwork(mServiceState)) {
                lteNetWorkTypeName =
                        mTelephonyManager.getNetworkTypeName(mServiceState.getDataNetworkType());
                Log.d(TAG,"lteNetWorkTypeName = " + lteNetWorkTypeName);
                if (lteNetWorkTypeName != null) {
                    cdmaNetworkTypeName = cdmaNetworkTypeName + " , " + lteNetWorkTypeName;
                }
            }
            setSummaryText(key, cdmaNetworkTypeName);
        }
    }
    ...
    private void updateNetworkType(ServiceState state) {
