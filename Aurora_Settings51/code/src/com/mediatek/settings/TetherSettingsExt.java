...
public class TetherSettingsExt implements OnPreferenceClickListener,
	...
	private static final String AURORA_ACTION_WIFI_HOTSPOT = "com.android.Settings.WIFI_TETHER";
	...
	private AuroraPreferenceScreen mPrfscreen;
	public AuroraSwitchPreference mWifiTether;
	public AuroraListPreference mUsbTetherType;
	private AuroraListPreference mTetherIpv6;
	private WifiApEnabler mWifiApEnablerExt;
	...
	public void onCreate(AuroraPreferenceScreen screen) {
		Log.d(TAG, "onCreate");
        mPrfscreen = screen;
	...
	public void onResume() {
		if (mWifiApEnablerExt != null) {
			mWifiApEnablerExt.resume();
		}
	}

	public void onPause() {
		if (mWifiApEnablerExt != null) {
			mWifiApEnablerExt.pause();
		}
	}
	...
	public void updateWifiTether(AuroraSwitchPreference enableWifiAp,
			AuroraPreference wifiApSettings, boolean wifiAvailable) {
		...
		if (wifiAvailable && !Utils.isMonkeyRunning() && !Utils.isWifiOnly(mContext)) {
			mWifiApEnablerExt = new WifiApEnabler(mContext, mWifiTether);
			mWifiApEnablerExt.setTetherSettings(this);
		} else {
			mPrfscreen.removePreference(mWifiTether);
		}
	}
	private void initPreference(AuroraPreferenceScreen screen) {
		      /* add by penggangding begin*/
		apAuroraPreference = createApPreference(R.string.aurora_wifi_tether_configure_ap,"ap_preference",screen);
		apAuroraPreference.setOrder(BASE_ORDER); /*Aurora <linchunhui> adjust the order*/
                      /*  add by penggangding end*/
		// create wifi hotspot preference
		mWifiTether = (HotspotSwitchPreference) createPreference(
				TYPE_CUSTOMIZED_SWITCH, R.string.wifi_tethering_title,
				KEY_WIFI_TETHER, screen);
		mWifiTether.setOrder(BASE_ORDER+2);
		mWifiTether.setChecked(false);
		
		
		// create USB tethering type preference
		if (FeatureOption.MTK_TETHERING_EEM_SUPPORT) {
			mUsbTetherType = (AuroraListPreference) createPreference(TYPE_LIST,
			...
			AuroraPreference usbTetherSettings = screen
		...
		mTetherIpv6 = (AuroraListPreference) createPreference(TYPE_LIST,
	...
	//add by penggangding begin
	public AuroraPreference apAuroraPreference=null;
    private AuroraPreference createApPreference(int  titieRes,String key,AuroraPreferenceGroup screen )
    {
        AuroraPreference apSetting= new AuroraPreference(mContext);
        apSetting.setKey(key);
        apSetting.setTitle(titieRes);
        screen.addPreference(apSetting);
        apSetting.setOnPreferenceChangeListener(this);
        return apSetting;
    }
  //add by penggangding end

	private AuroraPreference createPreference(int type, int titleRes, String key,
			AuroraPreferenceGroup screen) {
		AuroraPreference preference = null;
		switch (type) {
		case TYPE_CATEGORY:
			preference = new AuroraPreferenceCategory(mContext);
			break;
		case TYPE_PREFERENCE:
			preference = new AuroraPreference(mContext);
			break;
		case TYPE_CHECKBOX:
			preference = new AuroraCheckBoxPreference(mContext);
			preference.setOnPreferenceClickListener(this);
			break;
		case TYPE_LIST:
			preference = new AuroraListPreference(mContext);
			preference.setOnPreferenceClickListener(this);
			break;
		case TYPE_CUSTOMIZED_SWITCH:
			preference = new HotspotSwitchPreference(mContext);
		default:
			break;
		}
	...
	public boolean onPreferenceClick(AuroraPreference preference) {
		Log.d(TAG, "preferenceName= "+preference);
...
/*			try {
		...
		} else if(apAuroraPreference.getKey().equals(preference.getKey())) {
/* Aurora <linchunhui> modify
			try {
				Intent intent = new Intent(AURORA_ACTION_WIFI_HOTSPOT);
				mContext.startActivity(intent);
			} catch (ActivityNotFoundException e) {
				Toast.makeText(mContext, R.string.launch_error,
						Toast.LENGTH_SHORT).show();
...
*/
     ...
     */
    public void updateIpv6Preference(AuroraSwitchPreference usbTether, AuroraSwitchPreference bluetoothTether) {
		if (FeatureOption.MTK_TETHERINGIPV6_SUPPORT) {
			if (mTetherIpv6 != null) {
    ...
    public void updateBTPrfSummary(AuroraPreference pref, String originSummary) {
        if (FeatureOption.MTK_TETHERINGIPV6_SUPPORT) {
    ...
    public void updateUSBPrfSummary(AuroraPreference pref, String originSummary , boolean usbTethered, boolean usbAvailable) {
		if (usbTethered) {
			if (FeatureOption.MTK_TETHERINGIPV6_SUPPORT) {
				pref.setSummary(originSummary
	...
	public void updateBtTetherState(AuroraSwitchPreference btPrf) {
