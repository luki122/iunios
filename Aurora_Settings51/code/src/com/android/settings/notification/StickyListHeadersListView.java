	...
	private void scrollChanged(int reportedFirstVisibleItem) {
		...
		if (childCount != 0) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (viewToWatch == null
						|| (!viewToWatchIsFooter
                                                              && !(viewToWatch instanceof WrapperView && ((WrapperView) viewToWatch).hasHeader()))
						|| ((childIsFooter || (child instanceof WrapperView && ((WrapperView) child).hasHeader()))
			...
			if(getLastVisiblePosition() == adapterCount-1){
				//调控件接口，绘制title的阴影
				auroraSetHeaderViewYOffset(0) ;
			}else{
				//调控件接口，绘制title的阴影
				auroraSetHeaderViewYOffset(mHeaderBottomPosition) ;
			}			
		}
		updateHeaderVisibilities();
		invalidate();
