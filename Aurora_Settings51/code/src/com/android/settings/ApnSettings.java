...
public class ApnSettings extends AuroraSettingsPreferenceFragment implements
        OnPreferenceChangeListener, MsimRadioValueObserver.Listener  {
    ...
    private static final int MENU_ID_ADD_APN = Menu.FIRST + 2;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mActionBar.addItem(AuroraActionBarItem.Type.Add, MENU_ID_ADD_APN); //Aurora linchunhui 20160309 modify
    ...
    public boolean onPreferenceTreeClick(AuroraPreferenceScreen preferenceScreen, AuroraPreference preference) {
       ...
       Uri url = ContentUris.withAppendedId(Telephony.Carriers.CONTENT_URI, pos);
        ...
        if(preference instanceof  ApnPreference){
            	...
            	if(apnPreference.getSelectable()){
            		apnPreference.startApnEditor(pos);
            	}
        } else{
        ...
        if (FeatureOption.MTK_SVLTE_SUPPORT) {
            ...
            if (cursor.getCount() > 0) {
                ...
                if (sourceType == 1 && c2kSlot == currentSlot) {
                    resolver.update(mExt.getRestoreCarrierUri(mSubscriptionInfo.getSubscriptionId()),
                            values, null, null);
                    Log.d(TAG, "setSelectedApnKey sourceType == 1 && c2kSlot == currentSlot");
                    resolver.update(mExt.getRestoreCarrierUri(SubscriptionManager.LTE_DC_SUB_ID),
                            values, null, null);
                } else {
