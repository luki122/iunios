...
public abstract class CustomSQLiteOpenHelper {
     ...
     */
    public CustomSQLiteOpenHelper(Context context, String name, String path, CursorFactory factory, int version) {
        this(context, name, path, factory, version, null);
    }
    ...
    void setFilePermissionsFromMode(String name, int mode,
        ...
        try {
            ...
            if (db != null) {
                ...
                if (!dir.exists()) {
                    dir.mkdirs();
                    if (!dir.exists()) {
                        Log.i(TAG, mPath + "is not exist,callUid = " + Binder.getCallingUid());
                        return null;
                    }
                }
...
//huangbin hide end   
                    ...
                    if (DEBUG_STRICT_READONLY && !writable) {
                        ...
                        int mode = mEnableWriteAheadLogging ?
                                Context.MODE_ENABLE_WRITE_AHEAD_LOGGING : 0;
                        ...
                        db = SQLiteDatabase.openDatabase(path, mFactory, flags, mErrorHandler);
