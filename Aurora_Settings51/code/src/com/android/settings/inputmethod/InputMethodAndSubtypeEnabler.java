...
public class InputMethodAndSubtypeEnabler extends AuroraSettingsPreferenceFragment
        implements AuroraPreference.OnPreferenceChangeListener {
    private boolean mHaveHardKeyboard;
    private final HashMap<String, List<AuroraPreference>> mInputMethodAndSubtypePrefsMap =
            new HashMap<>();
    private final HashMap<String, AuroraCheckBoxPreference> mAutoSelectionPrefsMap = new HashMap<>();
    ...
    @Override
    public boolean onPreferenceChange(final AuroraPreference pref, final Object newValue) {
        ...
        for (final String imiId : mAutoSelectionPrefsMap.keySet()) {
            ...
            if (mAutoSelectionPrefsMap.get(imiId) == pref) {
                final AuroraCheckBoxPreference autoSelectionPref = (AuroraCheckBoxPreference) pref;
    ...
    private void addInputMethodSubtypePreferences(final InputMethodInfo imi,
            final AuroraPreferenceScreen root) {
        ...
        final AuroraCheckBoxPreference autoSelectionPref = new AuroraCheckBoxPreference(context);
        ...
        final ArrayList<AuroraPreference> subtypePreferences = new ArrayList<>();
        for (int index = 0; index < subtypeCount; ++index) {
            ...
            if (subtype.overridesImplicitlyEnabledSubtype()) {
                ...
                final AuroraPreference subtypePref = new InputMethodSubtypePreference(
                        context, subtype, imi);
                subtypePreferences.add(subtypePref);
            }
        }
        Collections.sort(subtypePreferences, new Comparator<AuroraPreference>() {
            @Override
            public int compare(final AuroraPreference lhs, final AuroraPreference rhs) {
        ...
        for (int index = 0; index < prefCount; ++index) {
            final AuroraPreference pref = subtypePreferences.get(index);
    ...
    private boolean isNoSubtypesExplicitlySelected(final String imiId) {
        ...
        for (final AuroraPreference pref : subtypePrefs) {
            if (pref instanceof AuroraCheckBoxPreference && ((AuroraCheckBoxPreference)pref).isChecked()) {
                return false;
            }
        }
        return true;
    ...
    private void setAutoSelectionSubtypesEnabled(final String imiId,
        ...
        final AuroraCheckBoxPreference autoSelectionPref = mAutoSelectionPrefsMap.get(imiId);
        ...
        final List<AuroraPreference> subtypePrefs = mInputMethodAndSubtypePrefsMap.get(imiId);
        for (final AuroraPreference pref : subtypePrefs) {
            if (pref instanceof AuroraCheckBoxPreference) {
                ...
                if (autoSelectionEnabled) {
                    ((AuroraCheckBoxPreference)pref).setChecked(false);
                }
            }
        }
        if (autoSelectionEnabled) {
    ...
    private void updateImplicitlyEnabledSubtypesOf(final InputMethodInfo imi, final boolean check) {
        ...
        for (final AuroraPreference pref : subtypePrefs) {
            ...
            final AuroraCheckBoxPreference subtypePref = (AuroraCheckBoxPreference)pref;
