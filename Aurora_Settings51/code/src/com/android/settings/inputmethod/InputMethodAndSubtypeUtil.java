...
class InputMethodAndSubtypeUtil {
    ...
    static void saveInputMethodSubtypeList(AuroraSettingsPreferenceFragment context,
        ...
        for (final InputMethodInfo imi : inputMethodInfos) {
            ...
            final AuroraPreference pref = context.findPreference(imiId);
            if (pref == null) {
                continue;
            }
            // In the choose input method screen or in the subtype enabler screen,
            // <code>pref</code> is an instance of TwoStatePreference.
            final boolean isImeChecked = (pref instanceof AuroraCheckBoxPreference) ?
                    ((AuroraCheckBoxPreference) pref).isChecked()
            ...
            if ((!hasHardKeyboard && InputMethodSettingValuesWrapper.getInstance(
                ...
                for (int i = 0; i < subtypeCount; ++i) {
                    ...
                    final AuroraCheckBoxPreference subtypePref = (AuroraCheckBoxPreference) context
    ...
    static void loadInputMethodSubtypeList(final AuroraSettingsPreferenceFragment context,
            final ContentResolver resolver, final List<InputMethodInfo> inputMethodInfos,
            final Map<String, List<AuroraPreference>> inputMethodPrefsMap) {
        ...
        for (final InputMethodInfo imi : inputMethodInfos) {
            ...
            if (pref instanceof AuroraCheckBoxPreference) {
                ...
                if (inputMethodPrefsMap != null) {
                    for (final AuroraPreference childPref: inputMethodPrefsMap.get(imiId)) {
    ...
    static void setSubtypesPreferenceEnabled(final AuroraSettingsPreferenceFragment context,
            final List<InputMethodInfo> inputMethodProperties, final String id,
            final boolean enabled) {
        final AuroraPreferenceScreen preferenceScreen = context.getPreferenceScreen();
        for (final InputMethodInfo imi : inputMethodProperties) {
            if (id.equals(imi.getId())) {
                final int subtypeCount = imi.getSubtypeCount();
                for (int i = 0; i < subtypeCount; ++i) {
                    final InputMethodSubtype subtype = imi.getSubtypeAt(i);
                    final AuroraCheckBoxPreference pref = (AuroraCheckBoxPreference) preferenceScreen
    ...
    private static void updateSubtypesPreferenceChecked(final AuroraSettingsPreferenceFragment context,
    ...
    static void removeUnnecessaryNonPersistentPreference(final AuroraPreference pref) {
