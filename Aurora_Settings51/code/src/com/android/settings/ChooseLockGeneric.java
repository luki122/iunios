...
public class ChooseLockGeneric extends SettingsActivity {
    ...
    public static class ChooseLockGenericFragment extends
            AuroraSettingsPreferenceFragment {
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            if (savedInstanceState != null) {
                ...
                mEncryptionRequestQuality = savedInstanceState
                        .getInt(ENCRYPT_REQUESTED_QUALITY);
                mEncryptionRequestDisabled = savedInstanceState
                        .getBoolean(ENCRYPT_REQUESTED_DISABLED);
            }
        ...
        public boolean onPreferenceTreeClick(
                AuroraPreferenceScreen preferenceScreen,
                AuroraPreference preference) {
            final String key = preference.getKey();
            // -------------delete warnning dialog by jiyouguang---------------
            /*
			 * if (!isUnlockMethodSecure(key) && mLockPatternUtils.isSecure()) {
			 * // Show the disabling FRP warning only when the user is switching
			 * from a secure // unlock method to an insecure one
			 * showFactoryResetProtectionWarningDialog(key); return true; } else
			 */
            {

                // Add by gary.gou
                if (!isUnlockMethodSecure(key) && mLockPatternUtils.isSecure()) {
                    setLockScreenNotificationValue();
                }
        ...
        public void setLockScreenNotificationValue() {
            int show = 0;
            int value = 0;
            try {
        ...
        /**
         * If the device has encryption already enabled, then ask the user if
         * they also want to encrypt the phone with this password.
         *
         * @param quality
         * @param disabled
         */
        private void maybeEnableEncryption(int quality, boolean disabled) {
            if (Process.myUserHandle().isOwner()
                ...
                // If accessibility is enabled and the user hasn't seen this
                // dialog before, set the
                // default state to agree with that which is compatible with
                // accessibility
                // (password not required).
                final boolean accEn = AccessibilityManager.getInstance(context)
                        .isEnabled();
                final boolean required = mLockPatternUtils
                        .isCredentialRequiredToDecrypt(!accEn);
                Intent intent = getEncryptionInterstitialIntent(context,
                        quality, required);
                startActivityForResult(intent, ENABLE_ENCRYPTION_REQUEST);
            } else {
                mRequirePassword = false; // device encryption not enabled or
                // not device owner.
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
            ...
            final boolean onlyShowFallback = getActivity().getIntent()
                    .getBooleanExtra(LockPatternUtils.LOCKSCREEN_WEAK_FALLBACK,
                            false);
            if (onlyShowFallback) {
                final String fallBackFor = getActivity().getIntent()
                        .getStringExtra(
                ...
                if (fallBackFor.equals(LockPatternUtils.TYPE_FACE_UNLOCK)) {
                    header = v.inflate(getActivity(),
                        ...
                        .equals(LockPatternUtils.TYPE_VOICE_UNLOCK)) {
                    header = v.inflate(getActivity(),
                            R.layout.weak_voice_fallback_header, null);
                }
                // / @}
                ((ListView) v.findViewById(android.R.id.list)).addHeaderView(
                        header, null, false);
            }
        ...
        public void onActivityResult(int requestCode, int resultCode,
            ...
            if (requestCode == CONFIRM_EXISTING_REQUEST
                ...
                updateUnlockMethodAndFinish(mEncryptionRequestQuality,
        ...
        public void onSaveInstanceState(Bundle outState) {
            ...
            // Saved so we don't force user to re-enter their password if
            ...
            outState.putInt(ENCRYPT_REQUESTED_QUALITY,
                    mEncryptionRequestQuality);
            outState.putBoolean(ENCRYPT_REQUESTED_DISABLED,
                    mEncryptionRequestDisabled);
        }
        ...
        private void updatePreferencesOrFinish() {
            ...
            if (quality == -1) {
                // If caller didn't specify password quality, show UI and allow
                // the user to choose.
                quality = intent.getIntExtra(MINIMUM_QUALITY_KEY, -1);
                MutableBoolean allowBiometric = new MutableBoolean(false);
                quality = upgradeQuality(quality, allowBiometric);
                final AuroraPreferenceScreen prefScreen = getPreferenceScreen();
                if (prefScreen != null) {
        ...
        /**
         * increases the quality if necessary, and returns whether biometric is
         * allowed
         */
        ...
        /***
         * Disables preferences that are less secure than required quality. The
         * actual implementation is in disableUnusablePreferenceImpl.
         *
        ...
        protected void disableUnusablePreferencesImpl(final int quality,
            ...
            final AuroraPreferenceScreen entries = getPreferenceScreen();

            final Intent intent = getActivity().getIntent();
            final boolean onlyShowFallback = intent.getBooleanExtra(
                    LockPatternUtils.LOCKSCREEN_WEAK_FALLBACK, false);// M:
            // Modify
            // for
            // voice
            // unlock

            final boolean weakBiometricAvailable = mChooseLockSettingsHelper
                    .utils().isBiometricWeakInstalled();
            ...
            for (int i = entries.getPreferenceCount() - 1; i >= 0; --i) {
                AuroraPreference pref = entries.getPreference(i);
                if (pref instanceof AuroraPreferenceScreen) {
                    final String key = ((AuroraPreferenceScreen) pref).getKey();
                    boolean enabled = true;
                    boolean visible = true;
                    if (KEY_UNLOCK_SET_OFF.equals(key)) {
                        enabled = quality <= DevicePolicyManager.PASSWORD_QUALITY_UNSPECIFIED;
                        visible = singleUser; // don't show when there's more
                        // than 1 user
                        ...
                        visible = weakBiometricAvailable; // If not available,
                        // then don't show
                        // it.
                    } else if (KEY_UNLOCK_SET_VOICE_WEAK.equals(key)) { // M:
                        // add
                        // for
                        // voice
                        // unlock
                    ...
                    if (!visible
                            || (onlyShowFallback && !allowedForFallback(key))) {
        ...
        /**
         * Check whether the key is allowed for fallback (e.g. bio sensor).
         * Returns true if it's supported as a backup.
         *
         * @param key
        ...
        private Intent getBiometricSensorIntent() {
            ...
            // M: Modify for voice unlock @{
            fallBackIntent.putExtra(LockPatternUtils.LOCKSCREEN_WEAK_FALLBACK,
                    true);
            ...
            intent.setClassName("com.android.facelock",
                    "com.android.facelock.SetupIntro");
        ...
        protected Intent getLockPatternIntent(Context context,
                                              final boolean isFallback, final boolean requirePassword,
                                              final boolean confirmCredentials) {
            return ChooseLockPattern.createIntent(context, isFallback,
                    requirePassword, confirmCredentials);
        }

        protected Intent getEncryptionInterstitialIntent(Context context,
                                                         int quality, boolean required) {
            return EncryptionInterstitial.createStartIntent(context, quality,
                    required);
        }

        /**
         * Invokes an activity to change the user's pattern, password or PIN
         * based on given quality and minimum quality specified by
         * DevicePolicyManager. If quality is
         * {@link DevicePolicyManager#PASSWORD_QUALITY_UNSPECIFIED}, password is
         * cleared.
         *
         * @param quality  the desired quality. Ignored if DevicePolicyManager
         *                 requires more security
         * @param disabled whether or not to show LockScreen at all. Only meaningful
         *                 when quality is
         *                 {@link DevicePolicyManager#PASSWORD_QUALITY_UNSPECIFIED}
         */
        void updateUnlockMethodAndFinish(int quality, boolean disabled) {
            // Sanity check. We should never get here without confirming user's
            // existing password.
            if (!mPasswordConfirmed) {
            ...
            final boolean isFallback = getActivity().getIntent()
                    .getBooleanExtra(LockPatternUtils.LOCKSCREEN_WEAK_FALLBACK,
                            false); // M: Modify for voice unlock

            quality = upgradeQuality(quality, null);
            final Context context = getActivity();
            // modify by jiyouguang for simple password lockscreen
            if (quality == DevicePolicyManager.PASSWORD_QUALITY_NUMERIC) {
                ...
                Intent intent = new Intent().setClass(getActivity(),
                        ChooseLockDigit.class);
                intent.putExtra(LockPatternUtils.PASSWORD_TYPE_KEY, quality);
                intent.putExtra(ChooseLockDigit.PASSWORD_MIN_KEY, minLength);
                intent.putExtra(ChooseLockDigit.PASSWORD_MAX_KEY, maxLength);
                ...
                final int maxLength = mDPM.getPasswordMaximumLength(quality);
                Intent intent = getLockPasswordIntent(context, quality,
                        isFallback, minLength, maxLength, mRequirePassword, /*
																			 * confirm
																			 * credentials
																			 */
                        false);
                if (isFallback) {
                    // M: Add for voice unlock @{
                    String isFallbackFor = getActivity()
                            .getIntent()
                            .getStringExtra(
                                    LockPatternUtils.LOCKSCREEN_WEAK_FALLBACK_FOR);
                    ...
                    intent.putExtra(LockPatternUtils.SETTINGS_COMMAND_KEY,
                            commandKey);
                    intent.putExtra(LockPatternUtils.SETTINGS_COMMAND_VALUE,
                            commandValue);
                    intent.putExtra(
                            LockPatternUtils.LOCKSCREEN_WEAK_FALLBACK_FOR,
                            isFallbackFor);
                ...
                // Aurora <hujianwei> <2015-07-15> modify for pattern start
                boolean showTutorial = !mChooseLockSettingsHelper.utils()
                        .isPatternEverChosen();
                Intent intent = new Intent();
                //Log.d(TAG, "showTutorial:" + showTutorial);
                /*intent.setClass(getActivity(),
                        showTutorial ? ChooseLockPatternTutorial.class
                ...
                // Intent intent = getLockPatternIntent(context, isFallback,
                // mRequirePassword,
                ...
                // startActivity(intent);
                // }
                // Aurora <hujianwei> <2015-07-15> modify for pattern end
            } else if (quality == DevicePolicyManager.PASSWORD_QUALITY_BIOMETRIC_WEAK) {
                Intent intent = getBiometricSensorIntent();
                mFinishPending = true;
                startActivity(intent);
            } else if (quality == DevicePolicyManager.PASSWORD_QUALITY_VOICE_WEAK) { // /M:
                // Add
                // for
                // voice
                // unlock
                ...
                mChooseLockSettingsHelper.utils().setLockScreenDisabled(
                        disabled);
                getActivity().setResult(Activity.RESULT_OK);
                finish();
            } else {
        ...
        private boolean setUnlockMethod(String unlockMethod) {
            ...
            if (KEY_UNLOCK_SET_OFF.equals(unlockMethod)) {
            ...
            } else if (KEY_UNLOCK_SET_VOICE_WEAK.equals(unlockMethod)) { // /M:
                // Add
                // for
                // voice
                // unlock
                ...
                Log.e(TAG, "Encountered unknown unlock method to set: "
                        + unlockMethod);
                return false;
            }
            Log.e("JOY", unlockMethod + "!!! end");
            return true;
        }
        ...
        private void showFactoryResetProtectionWarningDialog(
            ...
            FactoryResetProtectionWarningDialog dialog = FactoryResetProtectionWarningDialog
                    .newInstance(title, unlockMethodToSet);
            dialog.show(getChildFragmentManager(), TAG_FRP_WARNING_DIALOG);
        }

        public static class FactoryResetProtectionWarningDialog extends
                DialogFragment {
            ...
            public static FactoryResetProtectionWarningDialog newInstance(
                    int title, String unlockMethodToSet) {
            ...
            public void show(FragmentManager manager, String tag) {
                if (manager.findFragmentByTag(tag) == null) {
                    // Prevent opening multiple dialogs if tapped on button
                    // quickly
                        ...
                        .setPositiveButton(R.string.okay,
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog,
                                                        int whichButton) {
                                        ((ChooseLockGenericFragment) getParentFragment()).setUnlockMethod(args
                                                .getString(ARG_UNLOCK_METHOD_TO_SET));
                                    }
                                })
        ...
        /**
         * M: add for voice unlock Get voice unlock intent to start.
         */
