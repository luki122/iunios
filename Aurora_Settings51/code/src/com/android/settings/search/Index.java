...
public class Index {
    ...
    private static final String NODE_NAME_PREFERENCE_SCREEN = "aurora.preference.AuroraPreferenceScreen";
    private static final String NODE_NAME_CHECK_BOX_PREFERENCE = "aurora.preference.AuroraCheckBoxPreference";
    private static final String NODE_NAME_LIST_PREFERENCE = "aurora.preference.AuroraListPreference";
    ...
    public void updateSearchableDatas(){
        synchronized (mDataToProcess) {
            final UpdateIndexTask task = new UpdateIndexTask();
            UpdateData copy = mDataToProcess.copy();
            Collection<SearchIndexableResource> values = SearchIndexableResources.values();
            if(values != null && values.size() > 0){
            	for(SearchIndexableResource data:values){
            		copy.dataToDelete.remove(data);
            		data.context = mContext;
            		data.enabled = true;
            		copy.dataToUpdate.add(data);
            	}
            }
    ...
    private void indexOneResource(SQLiteDatabase database, String localeStr,
        ...
        if (sir.xmlResId > SearchIndexableResources.NO_DATA_RES_ID) {
            ...
            Log.d(LOG_TAG, "provider:"+provider.getClass().getName());
            if (provider != null) {
    ...
    private Indexable.SearchIndexProvider getSearchIndexProvider(final Class<?> clazz) {
        try {
        	Log.d(LOG_TAG, "provider:"+clazz.getName());
        ...
        try {
            ...
            if (!NODE_NAME_PREFERENCE_SCREEN.equals(nodeName)) {
                throw new RuntimeException(
                        "XML document must start with <aurora.preference.AuroraPreferenceScreen> tag; found:"
    ...
    private void indexFromProvider(Context context, SQLiteDatabase database, String localeStr,
        ...
        if (rawList != null) {
            ...
            Log.w(LOG_TAG, "rawSize: " + rawSize);
            for (int i = 0; i < rawSize; i++) {
                SearchIndexableRaw raw = rawList.get(i);

                // Should be the same locale as the one we are processing
                if (!raw.locale.toString().equalsIgnoreCase(localeStr)) {
                	 Log.w(LOG_TAG, "equalsIgnoreCase: " + raw.locale);
                    continue;
                }
                ...
                Log.w(LOG_TAG, "raw.screenTitle: " + raw.screenTitle+" raw.title"+raw.title);
                updateOneRowWithFilteredData(database, localeStr,
                        raw.title,
                        raw.summaryOn,
                        raw.summaryOff,
    ...
    private String getDataKey(Context context, AttributeSet attrs) {
        return getData(context, attrs,
                com.aurora.R.styleable.AuroraPreference,
                com.aurora.R.styleable.AuroraPreference_aurorakey);
    }

    private String getDataTitle(Context context, AttributeSet attrs) {
        return getData(context, attrs,
                com.aurora.R.styleable.AuroraPreference,
                com.aurora.R.styleable.AuroraPreference_auroratitle);
    }

    private String getDataSummary(Context context, AttributeSet attrs) {
        return getData(context, attrs,
                com.aurora.R.styleable.AuroraPreference,
                com.aurora.R.styleable.AuroraPreference_aurorasummary);
    }

    private String getDataSummaryOn(Context context, AttributeSet attrs) {
        return getData(context, attrs,
        		com.aurora.R.styleable.AuroraCheckBoxPreference,
        		com.aurora.R.styleable.AuroraCheckBoxPreference_aurorasummaryOn);
    }

    private String getDataSummaryOff(Context context, AttributeSet attrs) {
        return getData(context, attrs,
        		com.aurora.R.styleable.AuroraCheckBoxPreference,
        		com.aurora.R.styleable.AuroraCheckBoxPreference_aurorasummaryOff);
    }

    private String getDataEntries(Context context, AttributeSet attrs) {
        return getDataEntries(context, attrs,
        		com.aurora.R.styleable.AuroraListPreference,
        		com.aurora.R.styleable.AuroraListPreference_auroraentries);
    }
    ...
    private String getData(Context context, AttributeSet set, int[] attrs, int resId) {
        ...
        if (tv != null && tv.type == TypedValue.TYPE_STRING) {
            if (tv.resourceId != 0) {
                ...
                String name= context.getResources().getResourceName(tv.resourceId);
            } else {
                data = tv.string;
            }
        }
