...
import com.aurora.lockscreen.ConfirmLockDigit;
...
public final class ChooseLockSettingsHelper {
    ...
    public boolean launchConfirmationActivity(int request, CharSequence message,
        ...
        return launchConfirmationActivity(request, message, details, false, false);
    }

    public boolean launchConfirmationExitActivity(int request, CharSequence message,
                                              CharSequence details) {
        return launchConfirmationActivity(request, message, details, false, true);
    }
    ...
    boolean launchConfirmationActivity(int request, CharSequence message, CharSequence details,
                                       boolean returnCredentials, boolean needExit) {
        boolean launched = false;
        switch (mLockPatternUtils.getKeyguardStoredPasswordQuality()) {
            case DevicePolicyManager.PASSWORD_QUALITY_SOMETHING:
                launched = confirmPattern(request, message, details, returnCredentials, needExit);
                break;
            case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC:
            case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC_COMPLEX:
                launched = confirmDigit(request, needExit);
                break;
    ...
    private boolean confirmPattern(int request, CharSequence message,
                                   CharSequence details, boolean returnCredentials, boolean needExit) {
        ...
        if (needExit) {
            intent.putExtra(ConfirmLockPattern.NEED_PAUSE_EXIT, true);
            intent.putExtra(ConfirmLockPattern.MSG_TITLE, R.string.finger_manager);
        }
    ...
    //Aurora <jiyouguang> <2015-05-14> modify for digit password begin

    /**
     * Launch screen to confirm the existing lock digit.
     *
     * @return true if we launched an activity to confirm digit
     * @see #onActivityResult(int, int, android.content.Intent)
     */
    private boolean confirmDigit(int request, boolean needExit) {
        if (!mLockPatternUtils.isLockPasswordEnabled()) {
            return false;
        }
        final Intent intent = new Intent();
        if (needExit) {
            intent.putExtra(ConfirmLockDigit.NEED_PAUSE_EXIT, true);
            intent.putExtra(ConfirmLockDigit.MSG_TITLE, R.string.finger_manager);
        }
        intent.setClassName("com.android.settings", "com.aurora.lockscreen.ConfirmLockDigit");
        if (mFragment != null) {
            mFragment.startActivityForResult(intent, request);
        } else {
            mActivity.startActivityForResult(intent, request);
    ...
    //Aurora <jiyouguang> <2015-05-14> modify for digit password end
