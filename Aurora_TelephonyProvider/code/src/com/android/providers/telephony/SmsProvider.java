   ...
   //             Xlog.e(TAG, "getSingleMessageFromIcc messages is null");
    ...
    private Cursor getSingleMessageFromIcc(String messageIndexString, int slotId) {
        try {
            ...
            // Aurora xuyong 2015-08-13 added for android 5.0+ feature start
            int subId = (android.telephony.SubscriptionManager.getSubId(slotId))[0];
            // Aurora xuyong 2015-08-13 added for android 5.0+ feature end
            try {
            	// Aurora xuyong 2015-08-13 added for android 5.0+ feature start
...
//            Xlog.e(TAG, "getAllMessagesFromIcc messages is null");
 ...
 //       Xlog.d(TAG, "getAllMessagesFromIcc slotId =" + slotId);
       // Aurora xuyong 2014-06-11 modified for android4.4 new feature start
        ...
        // Aurora xuyong 2015-08-13 added for android 5.0+ feature end
        try {
        	// Aurora xuyong 2015-08-13 modified for android 5.0+ feature start
            messages = SmsManager.getSmsManagerForSubscriptionId(subId).getAllMessagesFromIcc();
            // Aurora xuyong 2015-08-13 modified for android 5.0+ feature end
        } finally {
            if (hasKitKat()) {
                Binder.restoreCallingIdentity(token);
            }
        ...
        try {
            ...
            if (messageIndexString.equals(ALL_SMS)) {
                ...
                if (mQcMultiSimEnabled) {
                	// Aurora xuyong 2015-08-13 modified for android 5.0+ feature start
                    ArrayList<SmsMessage> messages = SmsManager.getSmsManagerForSubscriptionId(subId).getAllMessagesFromIcc();
                    // Aurora xuyong 2015-08-13 modified for android 5.0+ feature end
                    int total = messages.size();
                    int result = 0;
                    for (SmsMessage message : messages) {
                    	// Aurora xuyong 2015-08-13 modified for android 5.0+ feature start
                        result += SmsManager.getSmsManagerForSubscriptionId(subId).deleteMessageFromIcc(message.getIndexOnIcc()) ? 1 : 0;
                        // Aurora xuyong 2015-08-13 modified for android 5.0+ feature end
                    }
                	...
                	// Aurora xuyong 2015-08-13 modified for android 5.0+ feature start
                    return SmsManager.getSmsManagerForSubscriptionId(subId).deleteMessageFromIcc(-1) ? 1 : 0;
                    // Aurora xuyong 2015-08-13 modified for android 5.0+ feature end
                }
                //Gionee guoyx 20130227 added for CR00772069 end
            } else {
