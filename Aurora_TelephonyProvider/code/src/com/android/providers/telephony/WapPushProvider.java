...
//        Xlog.i(TAG, "getOrCreatePushThreadId uri: " + uri);
 ...
 //           Xlog.w(TAG,"getOrCreatePushThreadId: Failed to create an ThreadId");
        ...
        values.put(Threads.TYPE, GnTelephony.Threads.WAPPUSH_THREAD);
...
//        Xlog.i(TAG, "updatePushThread thread: " + threadId);
        ...
        Cursor pushCursor = db.rawQuery("select * from threads where type <>"
                + GnTelephony.Threads.WAPPUSH_THREAD + " AND _id=" + threadId, null);
        if (pushCursor != null) {
            try {
     ...
     */    
        ...
        Cursor c = null;
         if (mIsMsgBox) {
              c = db.query(TABLE_WAPPUSH, 
                    new String[]{WapPush.DATE,WapPush.TEXT,WapPush.URL,WapPush.SIM_ID}, 
       ...
       //gionee gaoj 2012-3-27 added for CR00555790 start
        if (mIsMsgBox) {
                values.put(GnTelephony.GN_SIM_ID, sim_id);
            }
         //gionee gaoj 2012-3-27 added for CR00555790 end
...
//        Xlog.i(TAG,"updatePushAllThread");
        ...
        String query = "SELECT _id FROM threads WHERE " + Threads.TYPE + " = " + GnTelephony.Threads.WAPPUSH_THREAD + "  AND _id IN " 
        ...
        db.delete("threads", "_id NOT IN (SELECT DISTINCT thread_id FROM " + TABLE_WAPPUSH + ")" 
                            + " AND " + Threads.TYPE + " = " + GnTelephony.Threads.WAPPUSH_THREAD, null);
    }
