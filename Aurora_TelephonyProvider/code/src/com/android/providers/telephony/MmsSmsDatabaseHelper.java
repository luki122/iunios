...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
   ...
   private static final String WAPPUSH_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE =
    ...
    // Aurora xuyong 2015-12-25 modified for aurora 2.0 new feature start
    // Aurora xuyong 2015-12-29 modified for aurora 2.0 new feature start
    static final int DATABASE_VERSION = 700008;
    // Aurora xuyong 2015-12-29 modified for aurora 2.0 new feature end
    // Aurora xuyong 2015-12-25 modified for aurora 2.0 new feature end
...
//        Xlog.d(TAG, "recipient ids = " + ids);
    ...
    private void createNewMmsSmsSyncView(SQLiteDatabase db) {
        db.execSQL(MmsSmsSyncProvider.CREATE_SYNC_VIEW_NEW_SQL);
    }
    ...
    */
    ...
    private void createMmsTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_PDU + " (" +
                   ...
                   // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
                   "," + "notification_index" + " INTEGER DEFAULT 0" +
                   // Aurora xuyong 2015-12-04 added for aurora2.0 new feature end
                   // Aurora xuyong 2015-12-25 modified for aurora 2.0 new feature start
    ...
    private void createCBTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + TABLE_CELLBROADCAST
                ...
                // Aurora xuyong 2014-10-23 modified for privacy feature start
                // Aurora xuyong 2015-12-04 modified for aurora2.0 new feature start
                + "read INTEGER DEFAULT 0," + "seen INTEGER DEFAULT 0," + "date_sent INTEGER DEFAULT 0," + "date INTEGER,"+"star INTEGER DEFAULT 0,"+ "update_threads INTEGER DEFAULT 0, " + " is_privacy INTEGER DEFAULT 0, " + "weather_info TEXT DEFAULT NULL," + " fold INTEGER DEFAULT 0," + " notification_index INTEGER DEFAULT 0," + " reject INTEGER DEFAULT 0);");
                // Aurora xuyong 2015-12-04 modified for aurora2.0 new feature end
                // Aurora xuyong 2014-10-23 modified for privacy feature end
                // Aurora xuyong 2014-07-21 modified for reject feature end
...
//                Xlog.e(TAG, ex.getMessage(), ex);
        ...
        case 700005:
        {
            ...
            db.beginTransaction();
            try {
                upgradeDatabaseToVersion700006(db);
                db.setTransactionSuccessful();
            } catch (Throwable ex) {
                Log.e(TAG, ex.getMessage(), ex);
                break;
        ...
        case 700006:
        {
            ...
            db.beginTransaction();
            try {
                upgradeDatabaseToVersion700007(db);
                db.setTransactionSuccessful();
            } catch (Throwable ex) {
                Log.e(TAG, ex.getMessage(), ex);
                break;
        ...
        // Aurora xuyong 2015-12-25 modified for aurora 2.0 new feature end
            case 700007:
            {
                ...
                db.beginTransaction();
                try {
                    upgradeDatabaseToVersion700008(db);
                    db.setTransactionSuccessful();
                } catch (Throwable ex) {
                    Log.e(TAG, ex.getMessage(), ex);
                    break;
 ...
 // Aurora xuyong 2014-07-29 added for database upgrade start
    ...
    // Aurora xuyong 2014-10-23 added for privacy feature end
    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
    private void checkTablesHasWeatherInfoColumns(SQLiteDatabase db) {
        try {
    ...
    private void upgradeDatabaseToVersion700004(SQLiteDatabase db){
        // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
        checkTablesHasWeatherInfoColumns(db);
        // Aurora xuyong 2015-12-04 added for aurora2.0 new feature end
        try {
            // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
            db.execSQL("ALTER TABLE " + MmsSmsSyncProvider.TABLE_SYNC + " ADD COLUMN " + MmsSmsSyncProvider.Sync.LOCAL_FLAG + " TEXT;");
    ...
    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
    private void checkTablesNotifyColumns(SQLiteDatabase db) {
        try {
            db.execSQL("ALTER TABLE threads ADD COLUMN notification_index INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
        try {
            db.execSQL("ALTER TABLE pdu ADD COLUMN notification_index INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
        try {
            db.execSQL("ALTER TABLE sms ADD COLUMN notification_index INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
        try {
            db.execSQL("ALTER TABLE cellbroadcast ADD COLUMN notification_index INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
        try {
            db.execSQL("ALTER TABLE wappush ADD COLUMN notification_index INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
    }
    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature end
    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
    private void upgradeDatabaseToVersion700006(SQLiteDatabase db){
        checkTablesNotifyColumns(db);
        addNotifyTrigger(db);
    }
    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature end
    // Aurora xuyong 2015-12-25 modified for aurora 2.0 new feature start
    private void upgradeDatabaseToVersion700007(SQLiteDatabase db){
        try {
            db.execSQL("ALTER TABLE sms ADD COLUMN fold INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
        try {
            db.execSQL("ALTER TABLE pdu ADD COLUMN fold INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
    }
    private void upgradeDatabaseToVersion700008(SQLiteDatabase db){
        try {
            db.execSQL("ALTER TABLE cellbroadcast ADD COLUMN fold INTEGER DEFAULT 0;");
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
    }
 ...
 // Aurora xuyong 2014-07-29 added for database upgrade start
    ...
    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
    private void addNotifyTrigger(SQLiteDatabase db) {
        try {
            final String update_threads_notification = "UPDATE threads SET notification_index = 1 WHERE _id = NEW.thread_id; ";
            db.execSQL("DROP TRIGGER IF EXISTS sms_insert_threads_update_on_notification;");
            db.execSQL("CREATE TRIGGER sms_insert_threads_update_on_notification " +
                    "AFTER INSERT ON sms " +
                    "WHEN NEW.notification_index = 1 " +
                    "BEGIN " +
                            update_threads_notification +
                    "END"
                    );
            db.execSQL("DROP TRIGGER IF EXISTS pdu_insert_threads_update_on_notification;");
            db.execSQL("CREATE TRIGGER pdu_insert_threads_update_on_notification " +
                    "AFTER INSERT ON pdu " +
                    "WHEN NEW.notification_index = 1 " +
                    "BEGIN " +
                            update_threads_notification +
            ...
            db.execSQL("DROP TRIGGER IF EXISTS sms_update_threads_update_on_notification;");
            db.execSQL("CREATE TRIGGER sms_update_threads_update_on_notification " +
                            "AFTER UPDATE OF notification_index ON sms " +
                            "WHEN NEW.notification_index = 1 " +
                            "BEGIN " +
                            update_threads_notification +
                            "END"
            );
            db.execSQL("DROP TRIGGER IF EXISTS pdu_update_threads_update_on_notification;");
            db.execSQL("CREATE TRIGGER pdu_update_threads_update_on_notification " +
                            "AFTER UPDATE OF notification_index ON pdu " +
                            "WHEN NEW.notification_index = 1 " +
                            "BEGIN " +
                            update_threads_notification +
                            "END"
