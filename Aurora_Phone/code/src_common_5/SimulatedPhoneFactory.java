...
import com.android.internal.telephony.uicc.UiccController;
import com.android.internal.telephony.*;
import com.mediatek.internal.telephony.*;
...
public class SimulatedPhoneFactory extends PhoneFactory {
    ...
    static protected PhoneProxy[] sProxyPhone = null;
...
//                    networkMode = Phone.NT_MODE_LTE_CMDA_EVDO_GSM_WCDMA;
                ...
                int count = 2;
                sc = new SimulatedCommands[count];
                sc[0] = new SimulatedCommands();
                sc[1] = new SimulatedCommands();         
                
                SubscriptionController.init(context, sc);

                // Instantiate UiccController so that all other classes can just call getInstance()
                mUiccController = UiccController.make(context, sc);
                
                mRadioManager = RadioManager.init(context, count, sc);
                //MTK-END [mtk06800] create RadioManager for proprietary power on flow
                mNetworkManager = NetworkManager.init(context, count, sc);
                

                int phoneType = TelephonyManager.getPhoneType(networkMode);
//                if (phoneType == PhoneConstants.PHONE_TYPE_GSM) {
                sProxyPhone = new SimulatedPhoneProxy[count];
                    sProxyPhone[0] = new SimulatedPhoneProxy(new GSMPhone(context,
                            sc[0], testPhoneNotifier,0));
                    sProxyPhone[1] = new SimulatedPhoneProxy(new GSMPhone(context,
                            sc[1], testPhoneNotifier,1));
                    sProxyPhone[0] .setRadioPower(true);
                    sProxyPhone[1] .setRadioPower(true);
...
//                }
                    ...
                    mProxyController = ProxyController.getInstance(context, sProxyPhone,
                            mUiccController, sc);
    ...
    public static Phone getSimulatedGsmPhone() {
        synchronized(PhoneProxy.lockForRadioTechnologyChange) {
            ...
            Phone phone = new GSMPhone(PhoneGlobals.getInstance(), sc[0], testPhoneNotifier, 0);
//            Phone phone = new GSMPhone(sContext, sCommandsInterface, sPhoneNotifier);
    ...
    public static Phone getDefaultPhone() {
       ...
       return sProxyPhone[0];
    }
    
    public static Phone getPhone(int phoneId) {
    	return sProxyPhone[phoneId];
    }
}
