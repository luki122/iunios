...
//MTK end
    ...
    public static void openTTY() {
        if (!((MtkPhoneGlobals)PhoneGlobals.getInstance()).isEnableTTY()) {
            return;
        }
...
//MTK begin:
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            MTKCallManager cmGemini = ((MtkPhoneGlobals)PhoneGlobals.getInstance()).mCMGemini;;
            ...
            try {
                if (phoneIsCdma) {
                    if (app.cdmaPhoneCallState.getCurrentCallState()
                        ...
                        bluetoothHandsfree = ((MtkPhoneGlobals)app).getBluetoothHandsfree();
...
*/
        ...
        if (DualTalkUtils.isSupportDualTalk()) {
            ...
            InCallScreen screen = ((MtkPhoneGlobals)PhoneGlobals.getInstance()).getInCallScreenInstance();
            if ((dt != null) 
                    && (dt.hasMultipleRingingCall() || (dt.isCdmaAndGsmActive() && PhoneGlobals.getInstance().mCM.hasActiveRingingCall())) 
                    && (screen != null)) {
...
//MTK end
    ...
    public static int placeCall(Context context, Phone phone,
        ...
        if (PhoneGlobals.getInstance().mCM.getState() == PhoneConstants.State.IDLE) {
            ((MtkCallNotifier)PhoneGlobals.getInstance().notifier).resetBeforeCall();
            setAudioMode();
        }
...
// Google code:  connection = app.mCM.dial(phone, numberToDial);
            ...
            if (FeatureOption.MTK_GEMINI_SUPPORT && !isSipPhone) {
                if (simId == Phone.GEMINI_SIM_1 || simId == Phone.GEMINI_SIM_2) {
                    ...
                    connection = ((MtkPhoneGlobals)PhoneGlobals.getInstance()).mCMGemini.dialGemini(PhoneGlobals.getInstance().getPhone(), numberToDial, simId);
                } else {
                    int defaultSimId = SystemProperties.getInt(Phone.GEMINI_DEFAULT_SIM_PROP, -1);
                    log("placeCallExt , default simId " + simId);
                    connection = ((MtkPhoneGlobals)PhoneGlobals.getInstance()).mCMGemini.dialGemini(PhoneGlobals.getInstance().getPhone(), numberToDial, defaultSimId);
                }
            } else {
                connection = PhoneGlobals.getInstance().mCM.dial(phone, numberToDial);
            }
        } catch (CallStateException ex) {
...
//            }
            ...
            final BluetoothHandsfree bluetoothHandsfree = ((MtkPhoneGlobals)app).getBluetoothHandsfree();
    ...
    static void mergeCalls(CallManager cm) {
        ...
        if (phoneType == Phone.PHONE_TYPE_CDMA) {
            ...
            try {
                ...
                {
                    InCallScreen incall = ((MtkPhoneGlobals)PhoneGlobals.getInstance()).getInCallScreenInstance();
...
//MTK begin:
    ...
    static boolean cancelMmiCode(Phone phone) {
    	return cancelMmiCodeExt(phone, 0);
    }
...
//MTk end
     ...
     */
    public static AuroraCallerInfo getCallerInfo(Context context, Connection c) {
    	AuroraCallerInfo info = null;
...
//                            if (((GeminiPhone)app.phone).getStateGemini(Phone.GEMINI_SIM_2) != PhoneConstants.State.IDLE) {
//                                simId = Phone.GEMINI_SIM_2;
//                            }
//                            info = AuroraCallerInfo.getCallerInfoGemini(context, number, simId);
//                        } else {
                            info = AuroraCallerInfo.getCallerInfo(context, number);
//                        }
//MTk end
     ...
     */
    public static CallerInfoToken startGetCallerInfo(Context context, Call call,
            AuroraCallerInfoAsyncQuery.OnQueryCompleteListener listener, Object cookie, boolean needClearUserData) {
        Connection conn = null;
        int phoneType = call.getPhone().getPhoneType();
        if (phoneType == Phone.PHONE_TYPE_CDMA) {
            conn = call.getLatestConnection();
...
//                    cit.asyncQuery = AuroraCallerInfoAsyncQuery.startQueryGemini(QUERY_TOKEN, context,
...
//MTK end
     ...
     */
    public static String getCompactNameFromCallerInfo(AuroraCallerInfo ci, Context context) {
...
//MTK begin:
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            if (((GeminiPhone)phone).getState() == PhoneConstants.State.IDLE) {
                return false;
            }
        } else {
...
//MTK end
        ...
        if (phoneType == Phone.PHONE_TYPE_CDMA) {
            ...
            return !cm.hasActiveRingingCall() && cm.hasActiveFgCall()
                    ...
                    && (cm.canConference(cm.getFirstActiveBgCall())
                            || (DualTalkUtils.isSupportDualTalk() && DualTalkUtils.getInstance().isDualTalkMultipleHoldCase()));
        }
    }
    ...
    */
    private static boolean activateSpeakerIfDocked(Phone phone) {
        ...
        if (MtkPhoneGlobals.mDockState != Intent.EXTRA_DOCK_STATE_UNDOCKED) {
            if (DBG) log("activateSpeakerIfDocked(): In a dock -> may need to turn on speaker.");
            PhoneGlobals app = PhoneGlobals.getInstance();
            BluetoothHandsfree bthf = ((MtkPhoneGlobals)app).getBluetoothHandsfree();
...
//        if (FeatureOption.MTK_GEMINI_SUPPORT) {
//            return ((MtkPhoneGlobals)PhoneGlobals.getInstance()).mCMGemini.getDefaultPhoneGemini();
//        } 
//        else return cm.getDefaultPhone();
        ...
        if(isMultiSimEnabled()) {
			...
			if (card2Ready && !card1Ready && subscription == 0) {
				subscription = 1;
			} else if (card1Ready && !card2Ready && subscription == 1) {
				subscription = 0;
			}
       }
...
//MTK end
    ...
    public static boolean isSupportFeature(String feature) {
        ...
        if (feature.equals("TTY")) {
            ...
            //Gionee:niejn 20120418 modify for CR00572981 begin
            //return true;
            ...
            //Gionee:niejn 20120418 modify for CR00572981 end
    ...
    public static boolean hasPendingMmi() {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            return PhoneGlobals.getInstance().phone.getPendingMmiCodes().size() > 0;
        }
    }
    ...
    public static String getNetworkOperatorName(Call call) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (operatorName == null && PhoneGlobals.getInstance().mCM.getState() == PhoneConstants.State.IDLE) {
    ...
    public static SIMInfo getVTUsedSimInfo() {
        final int slot = FeatureOption.MTK_GEMINI_3G_SWITCH ? PhoneGlobals.getInstance().phoneMgr.get3GCapabilitySIM() : 0;
        if (slot == -1) {
            return null;
        }
        SIMInfo simInfo = SIMInfoWrapper.getDefault().getSimInfoBySlot(slot);
...
//        }
    ...
    public static boolean getShouldSendToVoiceMailFlag(final Connection c) {
        ...
        Object userDataObject = c.getUserData();
        if (userDataObject instanceof AuroraCallerInfo) {
        	AuroraCallerInfo callerInfo = (AuroraCallerInfo) userDataObject;
    ...
    public static boolean isVoicemailNumber(String number, int slot) {
        String voiceMailNumber;
        String voiceMailNumber2;
        boolean isVoicemail = false;

        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            voiceMailNumber = TelephonyManagerEx.getDefault().getVoiceMailNumber(Phone.GEMINI_SIM_1);
            voiceMailNumber2 = TelephonyManagerEx.getDefault().getVoiceMailNumber(Phone.GEMINI_SIM_2);
    ...
    public static int getActiveSubscription() {
    	if(isMultiSimEnabled()) {
	        if (DualTalkUtils.isSupportDualTalk()) {
	            return DualTalkUtils.getInstance().getActiveFgCall().getPhone().getMySimId();
	        } else {
    ...
    public static boolean isAnyOtherSubActive(int subscription) {
    	if(isMultiSimEnabled()) {
            ...
            return (other != -1 
            		&& PhoneGlobals.getInstance().getPhone(other).getState() != PhoneConstants.State.RINGING
	...
	public static int getMtkSub(String dataString) {
		long simId = Settings.System.getLong(PhoneGlobals.getInstance().getContentResolver(), 
			  dataString,
