...
public class BluetoothHandsfree {
	...
	private static final int PHONE_VT_RING_INFO = 13;				
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (intent.getAction().equals(Intent.ACTION_BATTERY_CHANGED)) {
                    ...
                    synchronized (BluetoothHandsfree.this) {
                        ...
                        if (state != BluetoothA2dp.STATE_PLAYING/* oldState == BluetoothA2dp.STATE_PLAYING &&
                           mA2dpState == BluetoothProfile.STATE_CONNECTED */) {
                        ...
                        } else { //A2DP state transit to BluetoothA2dp.STATE_PLAYING
                        	
                            /** M: [ALPS00366302] 
                             ** M: [DT][BlueTooth]The ringtone of BT headset is not normal 
                             ** M: Don't know if the following case would happen, just in case..
                            ...
                            if(isA2dpMultiProfile() && sendUpdate())
                            {
                                ...
                                {
                                    if (DBG) log("suspending A2DP stream for incoming call when A2DP changing state to STATE_PLAYING");
        ...
        private synchronized void updateServiceState(boolean sendUpdate, ServiceState state) {
            ...
            if (service != mService) {
                ...
                if (sendUpdate) {
                    ...
                    /// M: ALPS372652 @{
                    if(1 == mService){
                        log("[API] updateServiceState service change from 0 -> 1");
                        log("[API] updateServiceState mService="+mService+" mSignal="+mSignal);
                        result.addResponse("+CIEV: 5," + mSignal);
                    }
                    /// @}                    
                }
            }
        ...
        private synchronized void handlePreciseCallStateChange(boolean sendUpdate,
            ...
            log(String.format("PhoneState: mPhoneState = %d, newState = %d", mPhoneState.ordinal(), newState.ordinal()));
            /** @} */
            ...
            /** M: BT HFP Phone State Info @{ */
            log(String.format("CallState: foregroundCallState = %d, backgroundCallState = %d, ringingCallState = %d", 
            		foregroundCallState.ordinal(), backgroundCallState.ordinal(), ringingCallState.ordinal()));
            /** @} */
            ...
            if (mCallsetup != callsetup) {
                ...
                if(isA2dpMultiProfile() && isA2dpConnected() && sendUpdate)
                {
                    if ((mA2dpSuspended == false) && (mCall == 0) && (mCallsetup == 1)) //Phone ringing
                    {
                        if (DBG) log("suspending A2DP stream for incoming call after HFP SLC connection");
                        mA2dpSuspended = mA2dp.suspendSink(mA2dpDevice);                                    
                    }
                }
                /** @} */
        ...
        private synchronized AtCommandResult toCindResult() {
            ...
            if(isA2dpMultiProfile() && isA2dpConnected())
            {
                if ((mA2dpSuspended == false) && (mCall == 0) && (mCallsetup == 1)) //Phone ringing
                {
                	if (DBG) log("suspending A2DP stream for incoming call before HFP SLC connection");
                	mA2dpSuspended = mA2dp.suspendSink(mA2dpDevice);
                }
            }
            /** @}*/
...
//                    mMCM.registerForServiceStateChangedGemini(mStateChangeHandler, SERVICE_STATE_CHANGED2, null, Phone.GEMINI_SIM_2);                      
        ...
        private void restrictDualTalkStatus(boolean mIsNewHFPConnection) {
            ...
            if (hasRingingCall) {
                ...
                for (Call rCall : ringingCalls) {	// M: check (I, I)
                    if (rCall != ringingCall && (rCall.getState() != Call.State.IDLE)) {
    ...
    public synchronized void handleSCOEvent(int evt, BluetoothDevice device)
    {
        ...
        case BluetoothAudioGateway.SCO_CLOSED:
            ...
            if(!mSCOConnecting){
                 ...
                 */            
                if (VDBG) log("case SCO_CLOSED, mA2dpState: " + mA2dpState +
    ...
    /* package */ synchronized boolean audioOn() {
        ...
        if (mA2dpSuspended == false) {
            if (isA2dpMultiProfile() && mA2dpState == BluetoothA2dp.STATE_PLAYING) {
                ...
                /// M: BT HFP more info @{
                if (DBG) log("Current mA2dpState ="+mA2dpState+", waiting for A2DP streaming to stop");
                /// @}
                mA2dpSuspended = mA2dp.suspendSink(mA2dpDevice);
            ...
            /// M: [ALPS00248346] force to suspend a2dp @{
            else if (isA2dpMultiProfile() && (mA2dpState == BluetoothA2dp.STATE_NOT_PLAYING || mA2dpState == BluetoothA2dp.STATE_CONNECTED))
            {
                if (DBG) log("suspending A2DP stream for SCO");
                /// M: BT HFP more info @{
                if (DBG) log("Current mA2dpState ="+mA2dpState+", A2DP streaming is already stopped");
                /// @}
                mA2dpSuspended = mA2dp.suspendSink(mA2dpDevice);
            }
            /// @}
    ...
    private boolean isA2dpConnected() {
        ...
        return ((mA2dpState == BluetoothA2dp.STATE_PLAYING) || 
            ...
            public AtCommandResult handleActionCommand() {
                ...
                if (isVirtualCallInProgress()) {
                    ...
                    if (mCM.hasActiveFgCall()) {
                        //modify for [ALPS00369668][IOT][Bluetooth]Cannot answer the second call via Moto H375 begin
                        if ((mRemoteBrsf & BRSF_HF_CW_THREE_WAY_CALLING) != 0x0) {
                        ...
                        //modify for [ALPS00369668][IOT][Bluetooth]Cannot answer the second call via Moto H375 end
                    ...
                    if (args[0].equals(0)) {
                        ...
                        if (phoneType == Phone.PHONE_TYPE_CDMA) {
                            if (ringingCall!= null && ringingCall.isRinging()) {
                                ...
                                if (FeatureOption.MTK_GEMINI_SUPPORT == true) {
                                    PhoneUtils.hangupRingingAndActive(((MtkPhoneGlobals)PhoneGlobals.getInstance()).mCMGemini.getDefaultPhoneGemini());
                                } else {
