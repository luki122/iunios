...
public class InCallControlState {
    ...
    public boolean onHold, cdmaOnHold;
    ...
    public void update() {
        ...
        //aurora add liguangyu 20131110 for BUG #539 start
        canGotoContact = (fgCallState != Call.State.DISCONNECTING)
				&& (fgCallState != Call.State.DISCONNECTED)&&(fgCallState != Call.State.DIALING)&&(fgCallState != Call.State.ALERTING)&& (fgCallState != Call.State.IDLE);
        if(fgCallState == Call.State.IDLE)
        	canAddCall=false;
        ...
        if(fgCallState == Call.State.IDLE)
        	      canEndCall=true;//aurora add zhouxiaobing 
        // Swap / merge calls
        ...
        speakerEnabled = (state == PhoneConstants.State.OFFHOOK)
        ...
        if (c != null) isEmergencyCall =
        		AuroraPhoneNumberUtils.isLocalEmergencyNumber(c.getAddress(),
        ...
        } else if(hasActiveForegroundCall && fgCall.getPhone().getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
        	supportsHold = true;
