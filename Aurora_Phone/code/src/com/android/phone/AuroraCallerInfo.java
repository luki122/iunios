...
public class AuroraCallerInfo {
    ...
    public boolean isUseYoloreImage; 
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                ...
                if (columnIndex != -1) {
                    ...
                    // Lookupuri aurora add
                    columnIndex = cursor.getColumnIndex(Contacts.LOOKUP_KEY);
                    if (columnIndex != -1) {
                        String lookup = cursor.getString(columnIndex);
                        info.lookupUri =  Contacts.getLookupUri(info.person_id, lookup);
                    }
                } else {
...
//        info.name = normalize(info.name);
    ...
    public void auroraUpdate(Context context) {    	
        ...
        if(TextUtils.isEmpty(name)) {
	        name = AuroraAreaUtils.getNumName( phoneNumber);
	        mArea = AuroraAreaUtils.getNumLocation(phoneNumber);
        } else {
        ...
        mMark = marks[0];
        mMarkNumber = marks[1];
        if(TextUtils.isEmpty(name) && !TextUtils.isEmpty(mMark) ) {
        	name = mMark;
        }
        ...
        Log.i(TAG, "name = " + name + " mMark = " + mMark + " mArea = " + mArea);
        if(PhoneGlobals.getInstance().mYuloreUtils.isInit() && !mIsEmergency) {
    		Bitmap photo = YuLoreUtils.getPhoto();
    		if (photo != null) {
//    			 int width = photo.getWidth();
//    			 int height = photo.getHeight();
//    			 Log.v(TAG, "getCallerInfo() photo width = " + width + ", height = " + height);
//    			 if(width <=200 && height <=200) {
//    				  photo = Blur.fastblur(PhoneGlobals.getInstance(), photo, 6);
//    			 }    			
    			 isUseYoloreImage = true;
    ...
    private int[] randomContactPhotoId=new int[]{
			R.drawable.contact_photo_8e8a91,//0
			R.drawable.contact_photo_79bbb5,
			R.drawable.contact_photo_bdd5db,
			R.drawable.contact_photo_bdd9ff,
			R.drawable.contact_photo_c6e2a2,//4
			R.drawable.contact_photo_ccdedf,
			R.drawable.contact_photo_d9edbb,
			R.drawable.contact_photo_dfe5e5,
			R.drawable.contact_photo_e3ffa2,//8
			R.drawable.contact_photo_ffa589,
			R.drawable.contact_photo_ffc0ac,
			R.drawable.contact_photo_ffdfa6,
			R.drawable.contact_photo_ffe6e6,//12
			R.drawable.contact_photo_ffe182,
	};
    
    public static int[] largeRandomContactPhotoId=new int[]{
	...
	private void handleRandomPhoto(Context context) {

		String photo_id = null;
		String[] projection = new String[] { ContactsContract.Contacts.PHOTO_ID };
		String selection = ContactsContract.Contacts._ID + " = " + person_id + " AND (is_privacy = "
		...
		if (cur != null) {
			if (cur.getCount() > 0) {
				cur.moveToFirst();
				photo_id = cur.getString(0);
			}
			cur.close();
		}
		if (TextUtils.isEmpty(photo_id)) {
//			int index = (int) (person_id % 14);
//			if (index < randomContactPhotoId.length) {
//				cachedPhoto = context.getResources().getDrawable(
//						randomContactPhotoId[index]);
//			} else {
//				cachedPhoto = context.getResources().getDrawable(
//						R.drawable.contact_photo_bdd5db);
//			}
			
		 	int index=(int) (person_id%(largeRandomContactPhotoId.length));
			if(index<largeRandomContactPhotoId.length){
				photoResource = largeRandomContactPhotoId[index];
				cachedPhoto = context.getResources().getDrawable(largeRandomContactPhotoId[index]);
			}else{
				photoResource = R.drawable.large_svg_dial_default_photo1;
				cachedPhoto = context.getResources().getDrawable(R.drawable.large_svg_dial_default_photo1);
			}	

//			BitmapDrawable bd = (BitmapDrawable) cachedPhoto;
//			Bitmap bitmap = bd.getBitmap();
			Bitmap bitmap = RoundBitmapUtils.drawable2bitmap(cachedPhoto);
			cachedPhotoIcon = RoundBitmapUtils
					.getAuroraPhotoIconWhenAppropriate(context, bitmap);

//			bitmap = Blur.fastblur(PhoneGlobals.getInstance(), bitmap, 2);
			cachedPhoto = new BitmapDrawable(bitmap);
			isCachedPhotoCurrent = true;
		}
