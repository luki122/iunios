...
//            (PhoneGlobals.DBG_LEVEL >= 1) && (SystemProperties.getInt("ro.debuggable", 0) == 1);
    ...
    protected static final int WILD_PROMPT_CHAR_ENTERED = 105;
    ...
    protected static final int EVENT_DIALING = 125;
    //aurora add liguangyu 20140730 for BUG #7013 start
    protected static final int CDMA_ACCEPT = 131;
    ...
    protected boolean mBluetoothConnectionPending;
    protected long mBluetoothConnectionRequestTime;

    /** Main in-call UI elements. */
    protected CallCard mCallCard;

    // UI controls:
    InCallControlState mInCallControlState;
    ...
    protected ProgressBar mWaitingSpinner;
    ...
    private AuroraAlertDialog mMissingVoicemailDialog;
    protected AuroraAlertDialog mGenericErrorDialog;
    private AuroraAlertDialog mSuppServiceFailureDialog;
    protected AuroraAlertDialog mWaitPromptDialog;
    protected AuroraAlertDialog mWildPromptDialog;
    private AuroraAlertDialog mCallLostDialog;
    protected AuroraAlertDialog mPausePromptDialog;
    private AuroraAlertDialog mExitingECMDialog;
    ...
    protected AuroraDisconnectCause mLastDisconnectCause;
    ...
    protected Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
           ...
           Log.v("InCallScreen", "msg.what="+msg.what);
            switch (msg.what) {
 	        ...
 	        if(PhoneUtils.isMultiSimEnabled()) {
                ...
                case CDMA_ACCEPT:
                	if(mCM.getFgPhone().getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                		PhoneGlobals.getInstance().cdmaPhoneCallState.setSiggleDialingState(false);
                	}
                    mApp.mManageRecord.handleRecordProc();
                	requestUpdateScreen();
                	break;
                //aurora add liguangyu 20140730 for BUG #7013 end
                default:
...
//        }
    ...
    protected void onResume() {
	    ...
	    if(!DeviceUtils.hasKey()) {
			mHandler.postDelayed(new Runnable(){//aurora add zhouxiaobing 20140731 for xunijian
	
				@Override
				public void run() {
					// TODO Auto-generated method stub
					//setNavigationBarColor("#00000000");
					int vis=getWindow().getDecorView().getSystemUiVisibility();
	                getWindow().getDecorView().setSystemUiVisibility(vis|View.SYSTEM_UI_FLAG_LOW_PROFILE);
				}
				
			},3000);
	    }

//        mTouchSoundSettings = Settings.System.getInt(getContentResolver(), Settings.System.SOUND_EFFECTS_ENABLED, 1);
//        Settings.System.putInt(getContentResolver(),Settings.System.SOUND_EFFECTS_ENABLED, 0);
        ...
        //aurora add liguangyu 20131102 for ring-home-Notification start
        InCallScreen.mIsOnPuaseWhenRinging = false;
        //aurora add liguangyu 20131102 for ring-home-Notification end
        ...
        if(PhoneUtils.isMultiSimEnabled()) {
			try {
				Class<?> sPolicy = null;
				sPolicy = Class.forName("com.android.internal.telephony.CallManager");
				Method method = sPolicy.getMethod("getPhoneInCall");
				phone = (Phone) method.invoke(mCM);
			} catch (Exception e) {
				e.printStackTrace();
			}
    	} 
        ...
        if (now < PreventTouchManager.mLastProximityActionTime + PRO_DELAY_TIME) {
	        mApp.notifier.removeMessages(27); //UPDATE_IN_CALL_NOTIFICATION
	        PreventTouchManager.mLastProximityActionTime = 0;
        ...
        if (inCallUiState.hasPendingCallStatusCode()) {
            ...
            Log.v("InCallScreen", "hasPendingCallStatusCode");
       ...
       Log.v("InCallScreen", "status="+status+" "+"handledStartupError="+handledStartupError);
        if (status != SyncWithPhoneStateStatus.SUCCESS) {
            ...
            if (handledStartupError) {
                ...
                //aurora modify liguangyu 20140321 for 3410  start
                forceFinish();
//                endInCallScreenSession(false /* force a real finish() call */);
                //aurora modify liguangyu 20140321 for 3410  end
                return;
            }
    ...
    //aurora add liguangyu 20131031 for ring-home-Notification start
    static boolean mIsOnPuaseWhenRinging = false;
    //aurora add liguangyu 20131031 for ring-home-Notification end
    ...
    protected void onPause() {
        ...
        //aurora delete liguangyu 20140417 for #4217 start
	    if(Build.VERSION.SDK_INT >= 18) {
	    	mApp.notificationMgr.updateInCallNotification(); 
	    } 
        //aurora delete liguangyu 20140417 for #4217 end
...
//        Settings.System.putInt(getContentResolver(),Settings.System.SOUND_EFFECTS_ENABLED, mTouchSoundSettings);
    ...
    protected void onStop() {
        ...
        //aurora add liguangyu 20140417 for #4217 start
      long now = SystemClock.uptimeMillis();          
	  if(Build.VERSION.SDK_INT < 18) {
	      if (now > PreventTouchManager.mLastProximityActionTime + PRO_DELAY_TIME) {  
	      	mApp.notificationMgr.updateInCallNotification(); 
	      } else {
	          mHandler.postDelayed(mUpdateNotificationRunnable, PRO_DELAY_TIME);    
	      }
      }
        //aurora add liguangyu 20140417 for #4217 end
        ...
        if (state == PhoneConstants.State.IDLE) {
            ...
            if(mCallCard.getAnimation() != null) {
            	mCallCard.getAnimation().setAnimationListener(null);
                mCallCard.clearAnimation();
            }
            if(mInCallTouchUi.getAnimation() != null) {
	            mInCallTouchUi.getAnimation().setAnimationListener(null);
	            mInCallTouchUi.clearAnimation();
            }
            ...
            if ((mApp.cdmaOtaProvisionData != null) && (mApp.cdmaOtaScreenState != null)
                ...
                //aurora delete liguangyu 20140710 for BUG #6477 start
//                moveTaskToBack(true);
                //aurora delete liguangyu 20140710 for BUG #6477 end
                if (mApp.otaUtils != null) {
        ...
        //aurora add liguangyu 20140925 for 8642 start
        bailOutAfterErrorDialogOnStop();
        //aurora add liguangyu 20140925 for 8642 end
    }
    ...
    protected void onDestroy() {
       ...
       //aurora add zhouxiaobing 20140304 start
//        if (mInCallTouchUi != null) {
//        	mInCallTouchUiAnimController.ReleaseSource();
//        }
        //aurora add zhouxiaobing 20140304 end        
    ...
    protected void endInCallScreenSession(boolean forceFinish) {
        ...
        if (forceFinish) {
        	...
        	CallNotifier.mIsSilenceRinger = false;
       		PhoneGlobals.getInstance().ringer.setAuroraRingerEnable(true);
            //aurora add liguangyu 20140423 for BUG #4492 end   
            
            //aurora add liguangyu 20140603 for BUG #4849 start
            mApp.pokeUserActivity();
            //aurora add liguangyu 20140603 for BUG #4849 end
            
            mApp.mManageRecord.setUserStop(false);
 
            //aurora add liguangyu 20131205 end
//            Settings.System.putInt(getContentResolver(),Settings.System.SOUND_EFFECTS_ENABLED, mTouchSoundSettings);
    ...
    protected void registerForPhoneStates() {
        if (!mRegisteredForPhoneStates) {
            ...
            //aurora add liguangyu 20140730 for BUG #7013 start
            if(PhoneGlobals.getInstance().getPhone(0).getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            	PhoneGlobals.getInstance().getPhone(0).registerForCdmaCallAccepted(mHandler, CDMA_ACCEPT, null);
            }
            if(PhoneGlobals.getInstance().getPhone(1).getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            	PhoneGlobals.getInstance().getPhone(1).registerForCdmaCallAccepted(mHandler, CDMA_ACCEPT, null);
            }
            //aurora add liguangyu 20140730 for BUG #7013 end
    ...
    protected void unregisterForPhoneStates() {
        ...
        //aurora add liguangyu 20140730 for BUG #7013 start
        if(PhoneGlobals.getInstance().getPhone(0).getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
        	PhoneGlobals.getInstance().getPhone(0).unregisterForCdmaCallAccepted(mHandler);
        }
        if(PhoneGlobals.getInstance().getPhone(1).getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
        	PhoneGlobals.getInstance().getPhone(1).unregisterForCdmaCallAccepted(mHandler);
        }
        //aurora add liguangyu 20140730 for BUG #7013 end
...
//                                            + intent);
            ...
            return;            
        		...
        		|| action.equals(Intent.ACTION_CALL_PRIVILEGED)
        		|| action.equals(Intent.ACTION_CALL_EMERGENCY)) {
            //aurora modify liguangyu 20140805 for BUG #6999 start
        	mIsDialing = true;
        	Message msg = mHandler.obtainMessage(EVENT_DIALING, true);
        	mHandler.sendMessage(msg);
        	mDialIntent= intent;
            //aurora modify liguangyu 20140805 for BUG #6999 end
        } else {
    ...
    protected void initInCallScreen() {
        ...
        mFrame =  findViewById(R.id.frame_content);
        ...
        mWaitingSpinner = (ProgressBar) findViewById(R.id.aurora_spinner);
        mWaitingSpinner.setAlpha(0);
    }
    ...
    public void onBackPressed() {
        ...
        // current activity.)
        //aurora add zhouxiaobing 20131015 start
        if(mRespondViaSmsManager.isShowingPopup())
        {
//        	if(mCallCardAnimController.isStartRingToSmsAnim())
//        	{
//        		return;
//        	}
//        	else
//        	{	
            //aurora modify liguangyu 20140423 for BUG #4492 start
            if(!CallNotifier.mIsSilenceRinger) {   
	        	//aurora add liguangyu 20140328 for BUG #3615 start
	            final PhoneConstants.State state = PhoneGlobals.getInstance().mCM.getState();
	        	if(state == PhoneConstants.State.RINGING 
	        			&& PhoneGlobals.getInstance().mCM.getFirstActiveRingingCall().getState() != Call.State.WAITING) {
	        		PhoneGlobals.getInstance().ringer.setAuroraRingerEnable(false);
	        		PhoneGlobals.getInstance().ringer.ring();
	        	}
	            //aurora add liguangyu 20140328 for BUG #3615 end
            }
            //aurora modify liguangyu 20140423 for BUG #4492 end
        	mRespondViaSmsManager.dismissPopup();   
        	requestUpdateScreen();
        	return;
//        	}
        }
        //aurora add zhouxiaobing 20131015 end
      ...
      //aurora modify liguangyu 20150109 for #11041 start
//        super.onBackPressed();
        finish();
      //aurora modify liguangyu 20150109 for #11041 end
     ...
     */
    private boolean isFirstShowBg = true;
    @Override
...
/*        if(isFirstShowBg) {
     ...
     //   setStatusBarTransparent(hasFocus);
    }
    ...
    void onSuppServiceFailed(AsyncResult r) {
        ...
        //aurora modify liguangyu 20140805 for BUG #7233 start
        mSuppServiceFailureDialog = new AuroraAlertDialog.Builder(this)
                .setMessage(errorMessageResId)
                .setPositiveButton(R.string.ok,  new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int whichButton) {
                        if (DBG) log("mSuppServiceFailureDialog, onClick...");
                        requestUpdateScreen();
                    }
                })
                .create();
        //aurora modify liguangyu 20140805 for BUG #7233 end
    ...
    protected void onPhoneStateChanged(AsyncResult r) {
        ...
        //aurora add liguangyu 20131205 for #1080 start
        long now = SystemClock.uptimeMillis();
        if ((now < mLastIncomingCallActionTime + 1000) && state == PhoneConstants.State.RINGING) {
            log("updateState: Too soon after last action; not update");
            return;
        }
        //aurora add liguangyu 20131205 for #1080 end
...
//        mApp.mManageRecord.handleRecordProc();
        ...
        handleStatusBar(state == PhoneConstants.State.RINGING);
        
        //aurora add liguangyu 20131102 for ring-home-Notification start
        if (state != PhoneConstants.State.RINGING || mIsForegroundActivity) {
            InCallScreen.mIsOnPuaseWhenRinging = false;
        }
        //aurora add liguangyu 20131102 for ring-home-Notification end      
    ...
    protected void onDisconnect(AsyncResult r) {
        ...
        Log.v("PhoneAnim", "ringingCall.getState()="+ringingCall.getState()+"fgCall.getState()="+fgCall.getState());
       if(ringingCall.getState()==Call.State.DISCONNECTING||ringingCall.getState()==Call.State.DISCONNECTED||
    		   fgCall.getState()==Call.State.DISCONNECTED||fgCall.getState()==Call.State.DISCONNECTING||
    		   ringingCall.getState()==Call.State.IDLE||fgCall.getState()==Call.State.IDLE)//aurora add zhouxiaobing 20140526
       {
//    	   mCallCardAnimController.ResetCallCardAnimation();
//    	   mInCallTouchUiAnimController.resetInCallTouchUiAnimation();
        mDialer.ResetAnimator();
        mApp.inCallUiState.showDialpad=false;
       }
        //aurora add zhouxiaobing 20140102 end
        Log.v("PhoneAnim", "onDisconnect111");
        ...
        // routine.
        if (cause.mCause == AuroraDisconnectCause.CALL_BARRED) {
            showGenericErrorDialog(R.string.callFailed_cb_enabled, false);
            return;
        } else if (cause.mCause == AuroraDisconnectCause.FDN_BLOCKED) {
            showGenericErrorDialog(R.string.callFailed_fdn_only, false);
            return;
        } else if (cause.mCause == AuroraDisconnectCause.CS_RESTRICTED) {
            showGenericErrorDialog(R.string.callFailed_dsac_restricted, false);
            return;
        //aurora delete liguangyu 20140419 for BUG #4394 start
//        } else if (cause.mCause == AuroraDisconnectCause.CS_RESTRICTED_EMERGENCY) {
//            showGenericErrorDialog(R.string.callFailed_dsac_restricted_emergency, false);
//            return;
        //aurora delete liguangyu 20140419 for BUG #4394 end
        } else if (cause.mCause == AuroraDisconnectCause.CS_RESTRICTED_NORMAL) {
...
//                    && (cause.mCause != AuroraDisconnectCause.INCOMING_MISSED)
//                    && (cause.mCause != AuroraDisconnectCause.NORMAL)
...
//        }
        ...
        boolean showingQuickResponseDialog =
                false;//mRespondViaSmsManager != null && mRespondViaSmsManager.isShowingPopup();//aurora changes zhouxiaobing 20131015
        ...
        if (bailOutImmediately && showingQuickResponseDialog) {
            ...
            //aurora change liguangyu 20131116 for start
//            delayedCleanupAfterDisconnect(c.getCall().getPhone());
            mHandler.sendEmptyMessageDelayed(DELAYED_CLEANUP_AFTER_DISCONNECT,
            		mApp.mManageVibrator.isSilentMode() ? 1000 : 0);
            //aurora change liguangyu 20131116 for end
        } else {
            ...
            // in background.
          //aurora delete liguangyu 20140714 for BUG #6517 start
...
//                return;
//            }
            //aurora delete liguangyu 20140714 for BUG #6517 end
            ...
            if(cause.mCause == AuroraDisconnectCause.LOCAL) {
                callEndedDisplayDelay = CALL_ENDED_SHORT_DELAY;
            } else if(cause.mCause == AuroraDisconnectCause.NORMAL) {
//                  callEndedDisplayDelay = CALL_ENDED_LONG_DELAY;
          	  callEndedDisplayDelay = 1000;
            } else if(cause.mCause == AuroraDisconnectCause.INCOMING_REJECTED) {
            	  callEndedDisplayDelay =  currentlyIdle ? 1000 : 0;
            } else {
...
//                if(ringing.getState() == Call.State.WAITING || ringing.getState() == Call.State.INCOMING) {
...
//        Phone phone = (Phone) mmiCode.getPhone();
        ...
        if(PhoneUtils.isMultiSimEnabled()) {
			try {
				Class<?> sPolicy = null;
				sPolicy = Class.forName("com.android.internal.telephony.MmiCode");
				Method method = sPolicy.getMethod("getPhone");
				phone = (Phone)method.invoke(mmiCode);
			} catch (Exception e) {
				e.printStackTrace();
			}
        }
        ...
        if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
            ...
            if (mmiCode.getState() != MmiCode.State.PENDING) {
                ...
                dismissAllDialogs();
                //aurora modify liguangyu 20140526 for BUG #4802 start
                if (mCM.getState() == PhoneConstants.State.IDLE) {
                    endInCallScreenSession();
                }
                //aurora modify liguangyu 20140526 for BUG #4802 end
            }
        }
    }
    ...
    private void showWaitPromptDialog(final Connection c, String postDialStr) {
        ...
        mWaitPromptDialog = new AuroraAlertDialog.Builder(this)
    ...
    private void showPausePromptDialog(final Connection c, String postDialStrAfterPause) {
        ...
        mPausePromptDialog = new AuroraAlertDialog.Builder(this)
                .setMessage(buf.toString())
                .create();
    ...
    private void showWildPromptDialog(final Connection c) {
        ...
        mWildPromptDialog = new AuroraAlertDialog.Builder(this)
                .setView(v)
                .setPositiveButton(
...
//        if ((now < mLastAnswerCallWaitingTime + 1000) && mCM.getState() == PhoneConstants.State.RINGING) {
//            log("updateState: Too soon after last answer call waiting; not update");
//            return;
//        }
        ...
        //aurora add liguangyu 20140328 for BUG #3500  start
        } else {
        	if(!isDialerOpened()) {
        		mCallCard.setVisibility(View.VISIBLE);
        	}
        }
        //aurora add liguangyu 20140328 for BUG #3500 end
        ...
        //aurora add zhouxiaobing 20140111  start
        Call fgCall = mCM.getActiveFgCall();
        if(fgCall.getState()==Call.State.DISCONNECTED/*||mCM.getState() == PhoneConstants.State.RINGING*/)
        {
        //	mDialer.ResetAnimator();
        //	mCallCard.setVisibility(View.VISIBLE);
        //	mInCallTouchUi.setEndButtonShowOrHide(true);
//        	mCallCardAnimController.ResetCallCardAnimation();
//        	mInCallTouchUiAnimController.resetInCallTouchUiAnimation();
        	mDialer.ResetAnimator();
        }
      //aurora add zhouxiaobing 20140111  end         
        updateInCallTouchUi();
        mCallCard.updateState(mCM);
...
//        updateCallCardVisibilityPerDialerState(false);//aurora change zhouxiaobing 20131125
    ...
    private SyncWithPhoneStateStatus syncWithPhoneState() {
        ...
        //aurora add liguangyu 201411029 for #9418 start
        if(PhoneUtils.isMultiSimEnabled()) {
         	clearDisconnecting(PhoneGlobals.getInstance().getPhone(0));
         	clearDisconnecting(PhoneGlobals.getInstance().getPhone(1));
        } else {
        	clearDisconnecting(mPhone);
        }
        //aurora add liguangyu 201411029 for #9418 end
    ...
    private void handleMissingVoiceMailNumber() {
        ...
        //aurora add liguangyu 20140424 for BUG #4500 start
        if (mMissingVoicemailDialog != null) {
        	mMissingVoicemailDialog.dismiss();
        	mMissingVoicemailDialog = null;
        }
        //aurora add liguangyu 20140424 for BUG #4500 end

        mMissingVoicemailDialog = new AuroraAlertDialog.Builder(this)
    ...
    protected void delayedCleanupAfterDisconnect(Phone phone) {
        ...
        if (stayHere) {
            ...
            //aurora add liguangyu 201411029 for #9444 start
            final boolean hasActiveCall = mCM.hasActiveFgCall();
            final boolean hasHoldingCall = mCM.hasActiveBgCall();
    	    ...
    	    //aurora add liguangyu 201411029 for #9444 end
        } else {
...
/*                if ((mLastDisconnectCause.mCause != AuroraDisconnectCause.INCOMING_MISSED)
                        && (mLastDisconnectCause.mCause != AuroraDisconnectCause.INCOMING_REJECTED)
                ...
                }*/// aurora change zhouxiaobing 20131024
    ...
    public void onClick(View view) {
        ...
        switch (id) {
            ...
            case R.id.waitingCallEnd:{
            	  internalAnswerCallWaitingCall();
            	break;
            }
    ...
    private void onHoldClick() {
    	if(mCM.getFgPhone().getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
    		mInCallControlState.toggleHoldState();
    	}
        final boolean hasActiveCall = mCM.hasActiveFgCall();
        final boolean hasHoldingCall = mCM.hasActiveBgCall();
        ...
        mApp.mManageRecord.handleHoldClick();
    }
    ...
    protected void closeDialpadInternal(boolean animate) {
     ...
     */
    private void entryContact()
    {
    	getWindow().addFlags(WindowManager.LayoutParams.FLAG_DISMISS_KEYGUARD |
                WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED);//aurora add zhouxiaobing 20140324 for suoping
    	Intent intent=new Intent();
    	intent.setAction("com.android.contacts.action.LIST_CONTACTS");
    	startActivity(intent);
    	//aurora add liguangyu 20140423 for BUG #4495 start
        overridePendingTransition(R.anim.right_to_left2, R.anim.right_to_left);
    	//aurora add liguangyu 20140423 for BUG #4495 end
    }
    /* package */ void handleOnscreenButtonClick(int id) {
        if (DBG) log("handleOnscreenButtonClick(id " + id + ")...");
        
        //aurora add liguangyu 20141224 for #10682 start
        if(mCM.getState() == PhoneConstants.State.RINGING) {
        	if(id == R.id.holdButton || id == R.id.swapButton || id == R.id.dialpadButton
        			|| id == R.id.muteButton || id == R.id.recordButton || id == R.id.addButton) {
        		return;
        	}
        }
        //aurora add liguangyu 20141224 for #10682 end
...
//                break;
            ...
            case R.id.incomingCallRespondViaSms:
                ...
                break;
           //aurora add zhouxiaobing 20131012 start
//            case R.id.enterContact:
//            	entryContact();
//                break;
          //aurora add zhouxiaobing 20131012 end       
...
//                break;
            ...
            case R.id.addButton:
            	getWindow().addFlags(WindowManager.LayoutParams.FLAG_DISMISS_KEYGUARD |
                        WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED);//aurora add zhouxiaobing 20140324 for suoping
            	//aurora modify liguangyu 20140423 for BUG #4495 start
//            	PhoneUtils.startNewCall(mCM);  // Fires off an ACTION_DIAL intent
            	startNewCall(mCM);
            	//aurora modify liguangyu 20140423 for BUG #4495 end
                break;
...
//                break;
            case R.id.recordButton:
            	mApp.mManageRecord.onRecordClick();
    ...
    protected void showStatusIndication(CallStatusCode status) {
    	Log.v("InCallScreen", "showStatusIndication status="+status);
        switch (status) {
            case SUCCESS:
            ...
            case EMERGENCY_ONLY:
            	...
            	//aurora delete liguangyu 20131112 for BUG #753 start
//                showGenericErrorDialog(R.string.incall_error_emergency_only,
//                                       true /* isStartupError */);
//                break;
                //aurora delete liguangyu 20131112 for BUG #753 end

            case OUT_OF_SERVICE:
                // No network connection.
            	//aurora change liguangyu 20131031 for BUG #248 start
                ...
                //aurora change liguangyu 20131031 for BUG #248 end
                break;
    ...
    protected void showGenericErrorDialog(int resid, boolean isStartupError) {
        ...
        Log.v("InCallScreen", "showGenericErrorDialog1");
        ...
        // "failure" title?)
        mGenericErrorDialog = new AuroraAlertDialog.Builder(this)
                .setTitle(msg)
        ...
        Log.v("InCallScreen", "showGenericErrorDialog2");
    }
    ...
    protected void showCallLostDialog() {
        ...
        mCallLostDialog = new AuroraAlertDialog.Builder(this)
    ...
    private void showExitingECMDialog() {
        ...
        mExitingECMDialog = new AuroraAlertDialog.Builder(this)
    ...
    protected void bailOutAfterErrorDialog() {
        ...
        // InCallScreen can get stuck in an inconsistent state, somehow
        // causing a *subsequent* outgoing call to fail (bug 4172599).
        //aurora modify liguangyu 20140327 for BUG #4349 start
        endInCallScreenSession(false /* force a real finish() call */);
        //aurora modify liguangyu 20140327 for BUG #4349 end
    }
    ...
    private void updateProgressIndication() {
        ...
        switch (inCallUiState.getProgressIndication()) {
            ...
            case RETRYING:
                ...
                break;
            //aurora add liguangyu 20131202 start
            case DIALING:
                showProgressIndication(
                        R.string.aurora_dialing_dialog_title,
                        R.string.aurora_dialing_dialog_message);
                break;
            //aurora add liguangyu 20131202 end
...
//                    PhoneUtils.answerAndEndActive(mCM, ringing);
    ...
    private void internalAnswerCallWaitingCall() {
        if (DBG) log("internalAnswerCallWaitingCall()...");
        mLastIncomingCallActionTime = SystemClock.uptimeMillis();
        mLastAnswerCallWaitingTime = mLastIncomingCallActionTime;
        ...
        if (hasRingingCall) {
            ...
            Call ringing = mCM.getFirstActiveRingingCall();
            int phoneType = phone.getPhoneType();
            if (DBG) log("internalAnswerCallWaitingCall: answering (GSM)...");
...
//            mIsAnswerCall = true;
    ...
    private void internalRespondViaSms() {
        ...
        //aurora modify liguangyu 20140423 for BUG #4492 start
        boolean isRingerRinging = false;
        if(PhoneGlobals.getInstance().ringer.isRinging()) {
        	isRingerRinging = true;
        }
        internalSilenceRinger();
        if(isRingerRinging) {
        	CallNotifier.mIsSilenceRinger = false;
...
//        closeDialpadInternal(true);  // do the "closing" animation
    ...
    protected void setInCallScreenMode(InCallScreenMode newMode) {
        ...
        //aurora add for 20633  
        if(newMode == InCallScreenMode.UNDEFINED && phoneIsInUse()) {
        	return;
        }
    ...
    /* package */ void onDialerOpen(boolean animate) {
        ...
        //aurora add zhouxiaobing 20131030 start            
        //updateCallCardVisibilityPerDialerState(animate);
//		if (animate) {
//			mCallCard.OnShoworHideCallCard(false);
//		} else {
//			mCallCard.setVisibility(View.GONE);
//		}

//		if (animate) {
//			mInCallTouchUiAnimController.OnHideShowInCallControl(false, true);
//			mInCallTouchUiAnimController.OnShowHideKeypadButton(true, true);
//		} else {
//			mInCallTouchUiAnimController.OnHideShowInCallControl(false, false);// aurora add zhouxiaobing 20131008
//			mInCallTouchUiAnimController.OnShowHideKeypadButton(true, false);
//		}
		
		if (animate) {
			mInCallTouchUi.OnHideShowInCallControl(false, true);
		} else {
			mInCallTouchUi.OnHideShowInCallControl(false, false);
		}
		//aurora add zhouxiaobing 20131030 end           
    ...
    /* package */ void onDialerClose(boolean animate) {
		...
		updateInCallTouchUi();
		if (animate) {
//			mInCallTouchUiAnimController.OnHideShowInCallControl(true, true);
//			mInCallTouchUiAnimController.OnShowHideKeypadButton(false, true);
			mInCallTouchUi.OnHideShowInCallControl(true, true);
		} else {
//			mInCallTouchUiAnimController.OnHideShowInCallControl(true, false); // aurora add zhouxiaobing 20131008
//			mInCallTouchUiAnimController.OnShowHideKeypadButton(false, false);
			// Update the in-call touch UI.
			mInCallTouchUi.OnHideShowInCallControl(true, false);
		}
		// Update CallCard UI, which depends on the dialpad.
		// updateCallCardVisibilityPerDialerState(animate);
		// aurora change liguangyu 20131107 for BUG #462 begin
		if (animate) {
			mCallCard.OnShoworHideCallCard(true);
		} else {
			mCallCard.setVisibility(View.VISIBLE);
		}
        //aurora change liguangyu 20131107 for BUG #462 end
        //aurora change zhouxiaobing 20131030 end         
        // This counts as explicit "user activity".
        //aurora add liguangyu 20131228 start
        mApp.inCallUiState.showDialpad = false;
        //aurora add liguangyu 20131228 end
		// aurora add liguangyu 20131116 for BUG #807 start
		mCallCard.updateState(mCM);
		// aurora add liguangyu 20131116 for BUG #807 end
        mApp.pokeUserActivity();
     	com.aurora.utils.SystemUtils.switchStatusBarColorMode(com.aurora.utils.SystemUtils.STATUS_BAR_MODE_WHITE, this);
    }
    ...
    /* package */ boolean okToShowDialpad() {
        ...
        final Call.State fgCallState = mCM.getActiveFgCallState();
        //aurora change liguangyu 20131110 for BUG #539 start
        return (okToDialDTMFTones() /*|| (fgCallState == Call.State.DIALING)*/) 
        		&& !((fgCallState == Call.State.DISCONNECTING)
//        		|| (fgCallState == Call.State.DISCONNECTED)||(fgCallState == Call.State.DIALING)||(fgCallState == Call.State.ALERTING));
        	    || (fgCallState == Call.State.DISCONNECTED));
            //aurora add zhouxiaobing 20140208
        //aurora change liguangyu 20131110 for BUG #539 end
    }
...
//        mInCallTouchUiAnimController = new AuroraInCallTouchUiAnimationController(this);
//        mInCallTouchUiAnimController.setInCallTouchUi(mInCallTouchUi); 
    ...
    /* package */ boolean isBluetoothAvailable() {
        ...
        if (mApp.getBluetoothHeadset() != null) {
            List<BluetoothDevice> deviceList = mApp.getBluetoothHeadset().getConnectedDevices();
            ...
            if (deviceList.size() > 0) {
                ...
                if (VDBG) log("  - headset state = " +
                              mApp.getBluetoothHeadset().getConnectionState(device));
    ...
    /* package */ public boolean isBluetoothAudioConnected() {
        if (mApp.getBluetoothHeadset() == null) {
            if (VDBG) log("isBluetoothAudioConnected: ==> FALSE (null mApp.getBluetoothHeadset())");
            return false;
        }
        List<BluetoothDevice> deviceList = mApp.getBluetoothHeadset().getConnectedDevices();
        ...
        BluetoothDevice device = deviceList.get(0);
        boolean isAudioOn = mApp.getBluetoothHeadset().isAudioConnected(device);
     ...
     */
    protected void requestUpdateBluetoothIndication() {
    ...
    /* package */ void connectBluetoothAudio() {
        ...
        if (mApp.getBluetoothHeadset() != null) {
            ...
            mApp.getBluetoothHeadset().connectAudio();
        }
    ...
    /* package */ void disconnectBluetoothAudio() {
        ...
        if (mApp.getBluetoothHeadset() != null) {
            mApp.getBluetoothHeadset().disconnectAudio();
        }
        mBluetoothConnectionPending = false;
    }
    ...
    public InCallControlState getUpdatedInCallControlState() {
		...
		//aurora add zhouxiaobing 20140324 for null
    ...
    protected void onNewRingingConnection() {
        ...
        //aurora delete liguangyu 20131231 for #1627 begin
//        mRespondViaSmsManager.dismissPopup();  // safe even if already dismissed
        //aurora delete liguangyu 20131231 for #1627 end
    }
...
//                        mCM.getState() != PhoneConstants.State.RINGING);
            	if(!EmergencyDialer.isEmergencyShow) {
...
//        }
    ...
    protected void processCallIntent(Intent intent) {
        ...
        mSubscription = AuroraPhoneUtils.getSlot(intent);
        Log.w(LOG_TAG, "processCallIntent. mSubscription =" + mSubscription);

        final boolean isExactEmergencyNumber =
                (number != null) && AuroraPhoneNumberUtils.isLocalEmergencyNumber(number, this);
        final boolean isPotentialEmergencyNumber =
                (number != null) && AuroraPhoneNumberUtils.isPotentialLocalEmergencyNumber(number, this);
        if (VDBG) {
            Log.v(LOG_TAG, " - Checking restrictions for number '" + number + "':");
            Log.v(LOG_TAG, "     isExactEmergencyNumber     = " + isExactEmergencyNumber);
            Log.v(LOG_TAG, "     isPotentialEmergencyNumber = " + isPotentialEmergencyNumber);
        }

        if (Intent.ACTION_CALL_PRIVILEGED.equals(action)) {
            if (isPotentialEmergencyNumber) {
                Log.i(LOG_TAG, "ACTION_CALL_PRIVILEGED is used while the number is a potential"
        ...
        if (Intent.ACTION_CALL.equals(action)) {
            if (isPotentialEmergencyNumber) {
                Log.w(LOG_TAG, "Cannot call potential emergency number '" + number
                        + "' with CALL Intent " + intent + ".");
                Log.i(LOG_TAG, "Launching default dialer instead...");
                ...
                if (DBG) Log.v(LOG_TAG, "processCallIntent(): calling startActivity for Dialer: "
                               + invokeFrameworkDialer);
                startActivity(invokeFrameworkDialer);
//                endInCallScreenSession(false);
                //aurora add liguangyu 20140805 for BUG #6999 start
            	Message msg = mHandler.obtainMessage(EVENT_DIALING, false);
            	mHandler.sendMessage(msg);
                //aurora add liguangyu 20140805 for BUG #6999 end
                return;
            }
            callNow = false;
        } else if (Intent.ACTION_CALL_EMERGENCY.equals(action)) {
            if (!isPotentialEmergencyNumber) {
                Log.w(LOG_TAG, "Cannot call non-potential-emergency number " + number
...
//                endInCallScreenSession(false);
            ...
            Log.e(LOG_TAG, "Unhandled Intent " + intent + ". Finish the Activity immediately.");
//            endInCallScreenSession(false);
            //aurora add liguangyu 20140805 for BUG #6999 start
        	Message msg = mHandler.obtainMessage(EVENT_DIALING, false);
        	mHandler.sendMessage(msg);
        ...
        if (TextUtils.isEmpty(number)) {
            if (intent.getBooleanExtra(EXTRA_SEND_EMPTY_FLASH, false)) {
                Log.i(LOG_TAG, "processCallIntent: SEND_EMPTY_FLASH...");
                PhoneUtils.sendEmptyFlash(PhoneGlobals.getPhone());
//                endInCallScreenSession(false);
                return;
            } else {
                Log.i(LOG_TAG, "processCallIntent: null or empty number, setting callNow=true...");
                callNow = true;
            }
        }

        if (callNow) {
            Log.i(LOG_TAG, "processCallIntent(): callNow case! Calling placeCall(): " + intent);
            PhoneGlobals.getInstance().callController.placeCall(intent);
        	Message msg = mHandler.obtainMessage(EVENT_DIALING, false);
        	mHandler.sendMessage(msg);
        } else {
//        	mApp.inCallUiState.setProgressIndication(ProgressIndicationType.DIALING);
//            updateProgressIndication();
            //aurora delete liguangyu 20140805 for BUG #6999 start
//        	mIsDialing = true;
//        	Message msg = mHandler.obtainMessage(EVENT_DIALING, true);
//        	mHandler.sendMessage(msg);
            //aurora delete liguangyu 20140805 for BUG #6999 end
        }

        final String callOrigin = intent.getStringExtra(PhoneGlobals.EXTRA_CALL_ORIGIN);
        if (callOrigin != null) {
            if (DBG) Log.v(LOG_TAG, " - Call origin is passed (" + callOrigin + ")");
            PhoneGlobals.getInstance().setLatestActiveCallOrigin(callOrigin);
        } else {
            if (DBG) Log.v(LOG_TAG, " - Call origin is not passed. Reset current one.");
        ...
        if (Constants.SCHEME_SIP.equals(scheme) || PhoneNumberUtils.isUriNumber(number)) {
            Log.i(LOG_TAG, "The requested number was detected as SIP call.");
...
//            endInCallScreenSession(false);
        ...
        if (DBG) Log.v(LOG_TAG, " - Broadcasting intent: " + broadcastIntent + ".");
        //aurora modify liguangyu 20140819 for BUG #7755 start
...
//                null);  // initialExtras
        new OutgoingCallReceiver().doReceive(PhoneGlobals.getInstance(), broadcastIntent);
        //aurora modify liguangyu 20140819 for BUG #7755 end
    }
    
    public class OutgoingCallReceiver extends BroadcastReceiver {
        private static final String TAG = "InCallScreen OutgoingCallReceiver";
        ...
        public void doReceive(Context context, Intent intent) {
            ...
            if (number == null) {
                ...
                return;
            } else if (AuroraPhoneNumberUtils.isPotentialLocalEmergencyNumber(number, context)) {
    ...
    private void startSipCallOptionHandler(Context context, Intent intent,
        ...
        if (VDBG) {
            Log.i(LOG_TAG, "startSipCallOptionHandler...");
        ...
        if (DBG) {
            Log.v(LOG_TAG, "startSipCallOptionHandler(): " +
...
//        		||CallStatusCode.EMERGENCY_ONLY == mApp.inCallUiState.getPendingCallStatusCode()) {
//        	//aurora add zhouxiaobing 20140813 for anim start
//            Call ringingCall = mCM.getFirstActiveRingingCall();
//            Call fgCall = mCM.getActiveFgCall();
//          if(fgCall.getState()==Call.State.DIALING||fgCall.getState()==Call.State.ALERTING
//        		  ||fgCall.getState()==Call.State.IDLE||ringingCall.getState()==Call.State.INCOMING)
//          {
////            mCallCard.setVisibility(View.INVISIBLE);
//            final AlphaAnimation aa=new AlphaAnimation(0f,1f);
//            aa.setDuration(400);
//            aa.setInterpolator(new AccelerateDecelerateInterpolator());
//            final TranslateAnimation ta=new TranslateAnimation(0,0,-40,0);
//            ta.setDuration(400);
//            ta.setInterpolator(new AccelerateDecelerateInterpolator());
//            final AnimationSet ad=new AnimationSet(true);
//            //ad.setDuration(500);
//            ad.addAnimation(aa);
//            ad.addAnimation(ta);
//            ad.setInterpolator(new AccelerateDecelerateInterpolator());
//            ad.setStartOffset(100);  
//            mInCallTouchUi.startAnimation(ad);
////            mHandler.postDelayed(new Runnable(){//aurora add zhouxiaobing 20140731 for xunijian
////
////    			@Override
////    			public void run() {
////    				// TODO Auto-generated method stub
//////                    mCallCard.setVisibility(View.VISIBLE);
////                    Log.e(LOG_TAG, "callcard in animation");
////                    	mCallCard.startAnimation(ad);
////    			}
////    			
////    		},100);
//            final AnimationSet ad2=new AnimationSet(true);
//            ad2.addAnimation(aa);
//            ad2.addAnimation(ta);
//            ad2.setInterpolator(new AccelerateDecelerateInterpolator());
//            ad2.setStartOffset(200);           
//           	mCallCard.startAnimation(ad2);
//          }
//          //aurora add zhouxiaobing 20140813 for anim end 
//        }
...
//       	updateScreen();       
    ...
    //aurora add liguangyu 20131205 for #1080 end
    public static long mLastAnswerCallWaitingTime;
    ...
    private void setStatusBarTransparent(boolean enable) {
        ...
        builder.setSmallIcon(R.mipmap.ic_launcher_phone);
    ...
    //aurora add liguangyu 20140423 for BUG #4495 start
    private boolean startNewCall(final CallManager cm) {
        final PhoneGlobals app = PhoneGlobals.getInstance();

        if (!PhoneUtils.okToAddCall(cm)) {
...
////        intent.setClassName("com.android.contacts", "com.android.contacts.activities.AuroraDialtactsActivityV2");
////        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
...
////        intent.putExtra("add_call_mode", true);
        ...
        intent.setClassName("com.android.contacts", "com.android.contacts.activities.AuroraDialActivity");
//        intent.putExtra("is_from_import", true);
        intent.putExtra("switch_to_contacts_page", false);
...
//            overridePendingTransition(R.anim.right_to_left2, R.anim.right_to_left);
    ...
    //aurora add liguangyu 20140805 for BUG #6999 start
    protected Intent mDialIntent = null;
    ...
    /**
     * color string鍙互鏄�: #RRGGBB #AARRGGBB 鏍囧噯鏍煎紡, 蹇呴』浠�'#'寮�澶�
     * 鎴栬�咃細
     ...
     *
     * 濡傛灉浼犲叆鐨� color string涓嶇鍚堟爣鍑�, 浼氭姏鍑� IllegalArgumentException exception銆傝鑷澶勭悊銆�
     */
    ...
    /**
     * 璁剧疆浜嗛鑹蹭箣鍚庯紝涓嶅啀闇�瑕佹椂涓�瀹氳閲嶈 mode 涓�-1銆傚惁鍒欐棤娉曟仮澶峚ndroid鏍囧噯鐨勭姸鎬�!!!
     */
    private void unsetNavigationBarColor() {
        Intent intent = new Intent("aurora.action.SET_NAVIBAR_COLOR");
        intent.putExtra("mode", MODE_UNSET);
        intent.putExtra("color", "#00000000");
        sendBroadcast(intent);
    }
    

    
    protected void bailOutAfterErrorDialogOnStop() {
        if (mGenericErrorDialog != null) {
            if (DBG) log("bailOutAfterErrorDialog: DISMISSING mGenericErrorDialog.");
            mGenericErrorDialog.dismiss();
            mGenericErrorDialog = null;
    ...
    protected boolean isDoAnimationOnDisconnect() {        
//    	if(mLastDisconnectCause != null) {
//    		return  mLastDisconnectCause.mCause == AuroraDisconnectCause.LOCAL;
//    	} 
    	return false;
    }
    
    private boolean isDisconnectAnimaRunning = false;
    protected void runDisconnecedAnimation() {
        if (VDBG) log("- runDisconnecedAnimation");
    	isDisconnectAnimaRunning = true;
        final AlphaAnimation aa=new AlphaAnimation(1f, 0f);
        aa.setDuration(400);
        aa.setInterpolator(new AccelerateDecelerateInterpolator());
        final TranslateAnimation ta=new TranslateAnimation(0,0,0,-40);
        ta.setDuration(400);
        ta.setInterpolator(new AccelerateDecelerateInterpolator());
        final AnimationSet ad=new AnimationSet(true);
        //ad.setDuration(500);
        ...
        ad.addAnimation(ta);
        ad.setInterpolator(new AccelerateDecelerateInterpolator()); 
        ad.setAnimationListener(new Animation.AnimationListener() {
...
//				if(isDisconnectAnimaRunning) {
        ...
        ad2.addAnimation(ta);
        ad2.setInterpolator(new AccelerateDecelerateInterpolator());
        ad2.setStartOffset(100);  
        ad2.setAnimationListener(new Animation.AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				// TODO Auto-generated method stub
                if (VDBG) log("- onDisconnect callcard onAnimationEnd:");
//				if(isDisconnectAnimaRunning) {
                if(mCM.getState() == PhoneConstants.State.IDLE) {
					mCallCard.setVisibility(View.INVISIBLE);
					endInCallScreenSession();
        ...
        mHandler.postDelayed(new Runnable(){
        	public void run(){
        		isDisconnectAnimaRunning = false;
        	}
        }, 3000);
    }
    
    //aurora add liguangyu 201411029 for #9418 start
    private void clearDisconnecting(Phone phone) {
        Call call = phone.getForegroundCall();
        int phoneType = phone.getPhoneType();
        if (phoneType == PhoneConstants.PHONE_TYPE_GSM ) {
            Connection conn = null;
            conn = call.getEarliestConnection();
            if(conn != null && conn.getState() == Call.State.DISCONNECTING) {
        		try {
    				Class<?> sPolicy = Class.forName("com.android.internal.telephony.gsm.GsmConnection");
    				Method method = sPolicy.getMethod("onRemoteDisconnect", int.class);
    				method.invoke(conn, 16);
    			} catch (Exception e) {
    				e.printStackTrace();
    			}
            }
    ...
    public boolean onKeyLongPress(int keyCode, KeyEvent event) {
        if (VDBG) log("-onKeyLongPress");
        switch (keyCode) {
            case KeyEvent.KEYCODE_BACK:
            	PreventTouchManager.cancel();
                return true;
        }
        return super.onKeyLongPress(keyCode, event);
    }
    
    //aurora liguangyu add for BUG #9849 鎵嬫満閫氳瘽缁撴潫鍚庢潵鐭俊鏃犳彁绀洪煶 20141215
    private void updateNotificationSound() {
        ...
        String expandedText;         
        expandedText = "adfafadfdsa";
        builder.setContentTitle("adsfasddfas");
    ...
    /* package */ void requestUpdateBluetoothIndication(int bluetoothHeadsetAudioState) {
        if (VDBG) log("requestUpdateBluetoothIndication()... bluetoothHeadsetAudioState= "
    ...
    private void handleStatusBar(boolean isRinging){
    	String mode = Settings.System.getString(mApp.getContentResolver(), "aurora_power_mode");
		log("handleStatusBar mode = " + mode );
		if(mode != null && mode.equalsIgnoreCase("super")) {
			return;
    	} 
    	Intent systemUiIntent;
    	if(isRinging && mIsForegroundActivity) {
    		log("handleStatusBar mode 1 " );
	        systemUiIntent = new Intent("com.android.systemui.recent.AURORA_DISABLE_HANDLER");
    	} else {
    		log("handleStatusBar mode 2 " );
            systemUiIntent = new Intent("com.android.systemui.recent.AURORA_ENABLE_HANDLER");
    	}
        sendBroadcast(systemUiIntent);
    }
    
	public final static int PRO_DELAY_TIME = 1000;
    ...
    public TextView getDtmfCallTime() {
    	return mDialer.getDtmfCallTime();
    }
    
    public TextView getDtmfHold() {
    	return mDialer.getDtmfHold();
    }
}
