...
public class IccNetworkDepersonalizationPanel extends IccPanel {
    ...
    View.OnClickListener mUnlockListener = new View.OnClickListener() {
        public void onClick(View v) {
			...
			//aurora add zhouxiaobing 20131206 start for qc4.3 start
				  if(!(Build.VERSION.SDK_INT>=18&&SystemProperties.get("ro.product.board").contains("MSM")))    
				  	...
				  	try{                  
					    ...
					    Method method=sPolicy.getMethod("supplyNetworkDepersonalization", String.class,Message.class);                 
					    method.invoke(mPhone.getIccCard(),pin, Message.obtain(mHandler, EVENT_ICC_NTWRK_DEPERSONALIZATION_RESULT));          }catch(Exception e){}    
					}  
				  else  
				  	{    
						try{                  
							Class<?> sPolicy=null;                  
						    sPolicy=Class.forName("com.android.internal.telephony.IccCard");                  
						    Method method=sPolicy.getMethod("supplyDepersonalization", String.class,int.class,Message.class);                  
						    method.invoke(mPhone.getIccCard(),pin, /*IccCardApplicationStatus.PersoSubState.PERSOSUBSTATE_SIM_NETWORK.ordinal()*/3, Message.obtain(mHandler, EVENT_ICC_DEPERSONALIZATION_RESULT));      }catch(Exception e){}  
					} 			
//            mPhone.getIccCard().supplyNetworkDepersonalization(pin,
//                    Message.obtain(mHandler, EVENT_ICC_NTWRK_DEPERSONALIZATION_RESULT));
			//aurora add zhouxiaobing 20131206 start for qc4.3 end
            indicateBusy();
        }
    };
