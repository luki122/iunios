...
class CallLogger {
    ...
    public void logCall(Connection c, int callLogType) {
        ...
        final AuroraCallerInfo ci = getCallerInfoFromConnection(c);  // May be null.
        ...
        int slotId = PhoneUtils.getSubscription(c.getCall().getPhone());
        long simId = AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), slotId);
    
        // Don't log OTASP calls.
        if (!isOtaspNumber) {
            logCall(ci, logNumber, presentation, callLogType, date, callLogType == Calls.MISSED_TYPE ? setAuroraMissedCallRingtoneTime(date) : duration, simId);
        }
    }
     ...
     */
    public void logCall(AuroraCallerInfo ci, String number, int presentation, int callType, long start,
                        long duration, long simId) {
        final boolean isEmergencyNumber = AuroraPhoneNumberUtils.isLocalEmergencyNumber(number,
                mApplication);
        ...
        if (isOkToLogThisCall) {
            ...
            CallLogAsync.AddCallArgs args = new CallLogAsync.AddCallArgs(mApplication, ci, number,
                    presentation, callType, start, duration, simId);
     ...
     */
    private AuroraCallerInfo getCallerInfoFromConnection(Connection conn) {
        ...
        if ((o == null) || (o instanceof AuroraCallerInfo)) {
            ...
            ci = AuroraCallerInfo.getCallerInfo(mApplication, (Uri) o);
        } else {
     ...
     */
    private int getPresentation(Connection conn, AuroraCallerInfo callerInfo) {
        int presentation;
    ...
    private void log(String msg) {
	    ...
	    //aurora change liguangyu 20131108 for BUG #498 start
    private long setAuroraMissedCallRingtoneTime(final long start) {
        final long end = System.currentTimeMillis();
        return end - start;
    }
    //aurora change liguangyu 20131108 for BUG #498 end
}
