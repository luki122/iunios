...
//            }
//        });
        preference.setOnPreferenceChangeListener(new AuroraPreference.OnPreferenceChangeListener() {
            @Override
        	public boolean onPreferenceChange(AuroraPreference p, Object objValue) {
            	RadioPowerPreference rp =(RadioPowerPreference) p;
                ...
                if (isChanged(subId, isChecked)) {
                	...
                	rp.setRadioOn(isChecked);
                    setRadionOn(isChecked, subId);
                    rp.setRadioEnabled(false);
                    updateRadioMsimDb(isChecked, subId);
                } else {
                	rp.setRadioOn(!isChecked);
                }
                if(mUpdateListener != null) {
                	mUpdateListener.updateUI();
                }
                return true;
            }
        });
            ...
            if((priviousSimMode & othermodeSlot) > 0) {
            	...
            	if(mSubscriptionManager.getActiveSubscriptionInfoCount() == 2) {
	        		...
	        		mSubscriptionManager.setDefaultSmsSubId(mSubscriptionManager
	        				.getSubIdUsingPhoneId(otherSlot));
	        		boolean mIsEnableDataWhenSwitchDone = mTelephonyManager.getDataEnabled();
