	...
	private final SubscriptionManager.OnSubscriptionsChangedListener mOnSubscriptionsChangeListener = new SubscriptionManager.OnSubscriptionsChangedListener() {
		...
		public void onSubscriptionsChanged() {
	        ...
	        mHandler.postDelayed(new Runnable() {
					public void run() {
			        	if(mSubscriptionManager.getDefaultDataSubId() < 0||mSubscriptionManager.getDefaultDataPhoneId() == SubscriptionManager.INVALID_PHONE_INDEX) {
...
//            mStateHandler.addPhoneServiceStateListener(this);
	        ...
	        mManageReject.setNotificationMgr(new I2RejectNotificationMgr(this));
	        mHandler.postDelayed(new Runnable() {
				public void run() {
					handleSetCdmaNettype();
				}
			}, 10 * 1500);
	    }
...
//	    }
	    ...
	    private void handleSetCdmaNettype() {
			int sub = mSubscriptionManager.getDefaultDataPhoneId();
	        Log.d(LOG_TAG, "handleSetCdmaNettype: sub: " + sub);
			if(sub == SubscriptionManager.INVALID_PHONE_INDEX) {
				return;
			}
			if(PhoneGlobals.getInstance().getPhone(sub).getPhoneType()  == PhoneConstants.PHONE_TYPE_CDMA) {
				   int subid = AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), sub);
				  	...
				  	int value = cdmaType == TelephonyManagerEx.SVLTE_RAT_MODE_3G ? SvlteRatController.RAT_MODE_SVLTE_2G
			                 | SvlteRatController.RAT_MODE_SVLTE_3G : SvlteRatController.RAT_MODE_SVLTE_2G
			                 | SvlteRatController.RAT_MODE_SVLTE_3G | SvlteRatController.RAT_MODE_SVLTE_4G;
				     Log.d(LOG_TAG, "handleSetCdmaNettype: cdmaType: " + value);
			        LteDcPhoneProxy lteDcPhoneProxy = (LteDcPhoneProxy) (PhoneGlobals.getInstance().getPhone(sub));
			        lteDcPhoneProxy.getSvlteRatController().setRadioTechnology(value, null);	
			}	
	    }
}