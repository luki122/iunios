...
//            (PhoneGlobals.DBG_LEVEL >= 1) && (SystemProperties.getInt("ro.debuggable", 0) == 1);
    ...
    protected static final boolean VDBG = (PhoneGlobals.DBG_LEVEL >= 2);
	...
	protected static final int EVENT_PERSO_LOCKED = 3;//aurora add zhouxiaobing 20131205 for 4.3 qc
	protected static final int EVENT_WIRED_HEADSET_PLUG = 7;
    ...
    protected static final int EVENT_CALL_STATE_CHANGED = 18;
    public static int mAnimFactor = 1;
    ...
    protected ProgressDialog mPUKEntryProgressDialog;
    ...
    //aurora add liguangyu 20140331 for proximity sensor start
    private Object mProximitySyncLock = new Object();
    //aurora add liguangyu 20140331 for proximity sensor end
...
//                    handleCallStateChanged();
                ...
                case EVENT_ICC_CHANGED:
                    Intent intent = new Intent("AURORA_UICC_CHANGED");
...
//                    MediaButtonBroadcastReceiver.class.getName()));
            AuroraPlatformUtils.registerMediaButton(new ComponentName(this.getPackageName(),
                  MediaButtonBroadcastReceiver.class.getName()));
   ...
   public Phone getPhone(int subscription) {
    ...
    boolean isBluetoothHeadsetAudioOn() {
        Log.d(LOG_TAG, "mBluetoothHeadsetAudioState =" + mBluetoothHeadsetAudioState);
    ...
    static PendingIntent getPrivateSendSmsFromNotificationPendingIntent(
    ...
    protected static String getCallScreenClassName() {
        if(Build.VERSION.SDK_INT >= 21) {
            return "com.android.phone.I2InCallScreen";
        } else if (PhoneUtils.isMultiSimEnabled()) {
            return "com.android.phone.MSimInCallScreen";
        } else {
    ...
    void goToSleep() {
        synchronized (this) {
            if (DBG) Log.d(LOG_TAG, "goToSleep");            
    ...
    /* package */ void updateWakeState() {
        ...
        //aurora modify liguangyu 20140327 for BUG #3629 start
        ...
        //aurora modify liguangyu 20140327 for BUG #3629 end
...
//        boolean auroraSwitch = (state == PhoneConstants.State.OFFHOOK) && proximitySensorModeEnabled();
     ...
     */
    protected void pokeUserActivity() {
    ...
    /* package */ void updateProximitySensorMode(PhoneConstants.State state) {
        ...
        if (proximitySensorModeEnabled()) {
            //aurora modify liguangyu 20140331 for proximity sensor start
//            synchronized (mProximityWakeLock) {
            synchronized (mProximitySyncLock) {  
            //aurora modify liguangyu 20140331 for proximity sensor end
...
//                                               || isBluetoothHeadsetAudioOn());
              boolean screenOnImmediately = PhoneUtils.isSpeakerOn(this);
              //aurora delete liguangyu 201411029 for #9395 start
//                                               || mIsHardKeyboardOpen);
              //aurora delete liguangyu 201411029 for #9395 end
                
                Log.d(LOG_TAG, "updateProximitySensorMode: mIsHardKeyboardOpen = " + mIsHardKeyboardOpen);
                ...
                //aurora delete liguangyu 20140808 for BUG #7543 start
//                screenOnImmediately |= !isShowingCallScreenForProximity() && horizontal;
                //aurora delete liguangyu 20140808 for BUG #7543 end
                ...
                if (((state == PhoneConstants.State.OFFHOOK) || mBeginningCall) && !screenOnImmediately) {
                	...
                	try {
	                    if (!mProximityWakeLock.isHeld()) {
                		  ...
                		  if (VDBG) Log.d(LOG_TAG, "updateProximitySensorMode: Exception");
                	}
                } else {
                	if(mProximityWakeLock == null) {
                        if (VDBG) Log.d(LOG_TAG, "updateProximitySensorMode null");
                		return;
                	}
                    ...
                    if (mProximityWakeLock.isHeld()) {
                        ...
                        int flags =
                            ((screenOnImmediately || state == PhoneConstants.State.IDLE)? 0 : 1);
...
//            }
    ...
    /* package */ boolean proximitySensorModeEnabled() {
        ...
        sP = this.getSharedPreferences("com.android.phone_preferences", Context.MODE_PRIVATE);
        boolean proSensorEnabled = sP != null && sP.getBoolean("aurora_proximty_sensor_switch", true);
        if (VDBG) Log.d(LOG_TAG, "proximitySensorModeEnabled()...mProximityWakeLock =" + mProximityWakeLock + " proSensorEnabled=" + proSensorEnabled);
        //aurora modify liguangyu 20140331 for proximity sensor start
//        return proSensorEnabled && (mProximityWakeLock != null);
        return proSensorEnabled;
        //aurora modify liguangyu 20140331 for proximity sensor end
    }
    ...
    protected void initForNewRadioTechnology() {
        ...
        if (sim != null) {
          ...
          //aurora change zhouxiaobing 20131206 start  
           // sim.registerForNetworkLocked(mHandler, EVENT_SIM_NETWORK_LOCKED, null);
           		 if(!(Build.VERSION.SDK_INT>=18&&SystemProperties.get("ro.product.board").contains("MSM")))    
				  	{           
				  	try{                  
		...
		//aurora change zhouxiaobing 20131206 end  		  
        }
    }
    ...
    boolean isHeadsetPlugged() {
        Log.d(LOG_TAG, "isHeadsetPlugged =" + mIsHeadsetPlugged);
     ...
     */
    protected static boolean shouldShowBluetoothIndication(int bluetoothState,
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ACTION_HANG_UP_ONGOING_CALL)) {
                ...
                callIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                        | Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
            	int slot = AuroraSubUtils.getSlotBySubId(PhoneGlobals.getInstance(), NotificationMgr.mSimId);
            	if(slot > -1) {
            		callIntent.putExtra(SUBSCRIPTION_KEY, slot);
            	}
                ...
                if(isPrivate) {
                	...
                	smsIntent.putExtra("is_privacy", privateId);
                }
//             	smsIntent.putExtra("dissmissKeyGuard", getInstance().mKeyguardManager.isKeyguardLocked());
                smsIntent.putExtra("fromScreen", true);                
                context.startActivity(smsIntent);
                //aurora add liguangyu 20131128 start
				AuroraPlatformUtils.dismissKeyguard();
        ...
        private void clearMissedCallNotification(Context context) {
            ...
            clearPrivateMissedCallNotification(context);
        }
        
        private void clearPrivateMissedCallNotification(Context context) {
          	Intent clearIntent = new Intent(context, ClearPrivateRingingCallService.class);
            clearIntent.setAction(ClearPrivateRingingCallService.ACTION_CLEAR_MISSED_CALLS);
            context.startService(clearIntent);
        }
        ...
        if(DeviceUtils.isIUNI()) {
	        try {
		        if(ss.getState() == ServiceState.STATE_OUT_OF_SERVICE) {
		            Log.d(LOG_TAG, "- handleServiceStateChanged 1");
		            TelephonyManager telephonyManager = (TelephonyManager)getSystemService(
		                    Context.TELEPHONY_SERVICE);
		            if(telephonyManager.getSimState() == TelephonyManager.SIM_STATE_READY) {
		                Log.d(LOG_TAG, "- handleServiceStateChanged 2");
		            	int settingsNetworkMode = android.provider.Settings.Global.getInt(this.getContentResolver(), 
		            			android.provider.Settings.Global.PREFERRED_NETWORK_MODE,
		                        0);
		            	if(settingsNetworkMode == Phone.NT_MODE_WCDMA_ONLY) {
		            	    Log.d(LOG_TAG, "- handleServiceStateChanged 3");
		            	    String mccmnc = telephonyManager.getSubscriberId();
		            	    Log.d(LOG_TAG, "- telephonyManager.getSubscriberId() = " + mccmnc);
		            		if(mccmnc != null 
		            				&& (mccmnc.startsWith("46000") ||mccmnc.startsWith("46002")||mccmnc.startsWith("46007"))) {
		            		    Log.d(LOG_TAG, "- handleServiceStateChanged 4");
		                        android.provider.Settings.Global.putInt(this.getContentResolver(),
		                                    android.provider.Settings.Global.PREFERRED_NETWORK_MODE, 0);
		                        phone.setPreferredNetworkType(0, null);
		                	}
		            	}
		            }
		        } else {
	            ...
	            if(telephonyManager.getSimState() == TelephonyManager.SIM_STATE_READY) {
	                ...
	                Log.d(LOG_TAG, "- handleIfSimCardChanged preIccid = " + preIccid + " iccid = " + iccid);
		       	    if(!TextUtils.isEmpty(iccid) && !preIccid.equalsIgnoreCase(iccid)) {
			    	    ...
			    	    if(mccmnc != null 
	            				&& (mccmnc.startsWith("46000") ||mccmnc.startsWith("46002")||mccmnc.startsWith("46007"))
	            				||mccmnc.startsWith("46001")||mccmnc.startsWith("46010")) {
			    	    	android.provider.Settings.Global.putInt(this.getContentResolver(),
	                            android.provider.Settings.Global.PREFERRED_NETWORK_MODE, 0);
	                    	phone.setPreferredNetworkType(0, null);
			    	    }
...
//        }
    ...
    protected static final String DEFAULT_CALL_ORIGIN_PACKAGE = "com.android.contacts";
    protected static final String ALLOWED_EXTRA_CALL_ORIGIN =
    ...
    public static boolean isLauncherStarted() {
    	  ...
    	  try {
	    	   ...
	    	   for(ActivityManager.RunningAppProcessInfo pro: run){
		    	    if(pro.processName.contains("Launcher")){
		    	     isStarted = true;
		    	     break;
		    	    }
	    	   }
          ...
          Log.v(LOG_TAG, "isLauncherStarted = " + isStarted);
    	  return isStarted;
    }    
    
    protected void initAuroraObjects() {
        // Aurora xuyong 2015-09-01 deleted for aurora's new feature start
        //Settings.Secure.putInt(getContentResolver(),Settings.Secure.DEVICE_PROVISIONED, 1);
        // Aurora xuyong 2015-09-01 deleted for aurora's new feature end
        SharedPreferences sp = getSharedPreferences("com.android.phone_preferences", Context.MODE_PRIVATE);  
        Boolean init = sp.getBoolean("aurora_phone_init", false);	            
   	    if(!init) {
            Settings.Global.putInt(getContentResolver(), Settings.Global.DATA_STALL_ALARM_NON_AGGRESSIVE_DELAY_IN_MS, 1000 * 60 * 30);
       	    SharedPreferences.Editor editor = sp.edit();
            editor.putBoolean("aurora_phone_init", true);
            editor.commit();
            Settings.Global.putString(getContentResolver(), Settings.Global.NTP_SERVER, "1.asia.pool.ntp.org");
            try {
                AuroraPlatformUtils.setMobileDataEnabled(false);
	            phone.setDataRoamingEnabled(false);
            } catch (Exception e) {
            	e.printStackTrace();
            }
            
            if(DeviceUtils.isIndia()) {
          		SystemProperties.set("ril.ecclist","112,911,000,08,118,119,120,122,100,101,102");
          		SystemProperties.set("ril.ecclist1","112,911,000,08,118,119,120,122,100,101,102");
            }    
   	    }
   	                	                   
        //aurora add liguangyu 20140321 for 3410  start
   	    if(isLauncherStarted()) {
   	    	displayCallScreen();
   	    }
        //aurora add liguangyu 20140321 for 3410  end
   	    
   	    if(RejectUtils.isSupportBlack()) {
//	    	if(!SogouUtils.isInit()) {
//	    		SogouUtils.init(PhoneGlobals.getInstance());
//	    	}
		...
		try {
			Class<?> sPolicy = Class.forName("com.android.internal.telephony.uicc.UiccController");
			Method getInstance = sPolicy.getMethod("getInstance");
			Method registerForIccChanged = sPolicy.getMethod("registerForIccChanged" ,Handler.class, int.class, Object.class);
			registerForIccChanged.invoke((UiccController)(getInstance.invoke(sPolicy)), mHandler, EVENT_ICC_CHANGED, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
//        UiccController mUiccController = UiccController.getInstance();
//        mUiccController.registerForIccChanged(mHandler, EVENT_ICC_CHANGED, null);
		mOverTurnManager = new OverTurnManager(mCM, this);
		mPreventTouchManager = new PreventTouchManager(mCM, this);
	    mManageRecord = new ManageRecord(mCM, this);
	    mManageVibrator = new ManageVibrator(mCM, this);
	    mManageDisconnect = new ManageDisconnect(mCM, this);
	    mManagePhbReading = new ManagePhbReading(this);
	    mManageVcardReading = new ManageVcardReading(this);
	    mManageTest = new ManageTest(this);
	    mManageReject = new ManageReject(this);
	    mManageCallSetting = new ManageCallSetting(this);
	    mManagePowerSave = new ManagePowerSave(mCM, this); 
	 }
    
    PreventTouchManager mPreventTouchManager;
    OverTurnManager mOverTurnManager; 
    ...
    ManageTest mManageTest;
    ManageCallSetting mManageCallSetting;
    ManagePowerSave mManagePowerSave;
    
    protected ManageReject mManageReject;   
    YuLoreUtils mYuloreUtils;
    protected BluetoothAdapter mBluetoothAdapter;
    private BluetoothHeadset mBluetoothHeadset;
    
    void getBluetoothHeadsetOnCreate() {
    ...
    protected final Set<SubInfoUpdateListener> mSubInfoUpdateListeners = new HashSet<SubInfoUpdateListener>();
    /***
    ...
    static InCallScreen getInCallScreen() {
    	return getInstance().mInCallScreen;
    }
