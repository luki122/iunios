 ...
 */
public class NotificationMgr implements AuroraCallerInfoAsyncQuery.OnQueryCompleteListener{
    private static final String LOG_TAG = "NotificationMgr";
    static final boolean useCustomView = true; 
...
//    static final int CALL_FORWARD_NOTIFICATION = 6;
    static final int AURORA_NOTIFICATION_ID = 16000;
    static final int PHONE_NOTIFICATION_ID = 100;
    static final int CALL_FORWARD_NOTIFICATION = 1 + AURORA_NOTIFICATION_ID + PHONE_NOTIFICATION_ID;
    ...
    static final int RINGING_AND_HOME_NOTIFICATION = 500;
    //aurora add liguangyu 20131102 for ring-home-Notification end
    static final int RINGING_AND_HOME_NOTIFICATION_BASE = AURORA_NOTIFICATION_ID + PHONE_NOTIFICATION_ID;
...
//aurora add zhouxiaobing 20140512 end for CallerInfo
    ...
    public class StatusBarHelper {
        ...
        private void updateStatusBar() {
            if (DBG) log("updateStatusBar");
        	String mode = Settings.System.getString(mApp.getContentResolver(), "aurora_power_mode");
        	if(mode != null && mode.equalsIgnoreCase("super") && !EmergencyDialer.isEmergencyShow) {
        	     if (DBG) log("updateStatusBar: return");
        		return; 
        	}
          ...
          //      state |= StatusBarManager.DISABLE_BACK;//aurora change zhouxiaobing 20140623 for back key
            }
...
//        note.defaults |= Notification.DEFAULT_LIGHTS;
    ...
    /* package */ void notifyMissedCall(
    	...
    	if(useCustomView) {
    		notifyMissedCallCustom(name, number, type, photo, photoIcon, date);
    		return;
    	} 
        ...
        String expandedText, callName, reminderText;
        ...
        reminderText =  mContext.getString(R.string.notification_missedCallsMsg,
                mNumberMissedCalls);
...
//            	bitmap = RoundBitmapUtils.toRoundBitmap(bitmap);
        ...
        ReminderUtils.insertPhoneReminder(reminderText);
    }
    ...
    void cancelMissedCallNotification() {
        // reset the number of missed calls to 0.
   	    if (DBG) log("cancelMissedCallNotification()");
        mNumberMissedCalls = 0;
        mNotificationManager.cancel(MISSED_CALL_NOTIFICATION);
        if(AuroraPrivacyUtils.isSupportPrivate()) {
        	PhoneGlobals.getInstance().mManagePrivate.notificationMgr.cancelPrivateMissedCallNotification();
        }
    	stopI2MissCallLed();
    ...
    protected void updateInCallNotification(boolean allowFullScreenIntent) {
        ...
        PendingIntent inCallPendingIntent =
                PendingIntent.getActivity(mContext, 0,
                                          PhoneGlobals.getInstance().createInCallIntent(
                                              PhoneUtils.getSubscription(currentCall.getPhone())), 0);
        ...
        if (hasRingingCall || hasActiveCall) {
            ...
            // call".)
            //aurora delete liguangyu 20131112 for BUG #697 start
//            builder.setTicker(expandedViewLine2);
            //aurora delete liguangyu 20131112 for BUG #697 end
            ...
            if (allowFullScreenIntent) {
                ...
                if ((ringingCall.getState() == Call.State.WAITING) && !mApp.isShowingCallScreen()) {
                    ...
                    //aurora add liguangyu 20131102 for ring-home-Notification start
                    mNotificationManager.cancel(RINGING_AND_HOME_NOTIFICATION);
                    //aurora add liguangyu 20131102 for ring-home-Notification end
                }
            }
        ...
        //aurora change liguangyu 20131102 for ring-home-Notification start
        boolean isShowGreenBar = InCallScreen.mIsOnPuaseWhenRinging && !FloatWindowManager.isWindowShowing() && hasRingingCall;
        log("Notifying IN_CALL_NOTIFICATION ID = " + (isShowGreenBar ? RINGING_AND_HOME_NOTIFICATION : IN_CALL_NOTIFICATION));
        if(InCallScreen.mIsOnPuaseWhenRinging) {
        	mHasSendRingInCall = true;
        }
        mNotificationManager.notify(isShowGreenBar ? RINGING_AND_HOME_NOTIFICATION : IN_CALL_NOTIFICATION, notification);
        //aurora change liguangyu 20131102 for ring-home-Notification end
    ...
    protected void cancelInCall() {
        ...
        mNotificationManager.cancel(IN_CALL_NOTIFICATION);
        //aurora add liguangyu 20131102 for ring-home-Notification start
        if(mHasSendRingInCall) {
        	mNotificationManager.cancel(RINGING_AND_HOME_NOTIFICATION);
        	mHasSendRingInCall = false;
        }
        //aurora add liguangyu 20131102 for ring-home-Notification end
        mInCallResId = 0;
    }
    ...
    protected boolean mHasSendRingInCall = false; 
    ...
    private static final int RINGING_LED_NOTIFICATION = 2467001;
    private static final int MISSCALL_RED_LED_NOTIFICATION = 2467004;
    private static final int MISSCALL_BLUE_LED_NOTIFICATION = 2467020;
    private static final int RED_TIME = 260;
    private static final int BLUE_TIME = RED_TIME * 2;
    private static final int RED_COLOR = 0xffff0000;
    private static final int BLUE_COLOR = 0xff0000ff;
    
    public void startI2RingingRedLed() { 
    	startLedNotification(RED_COLOR, RED_TIME, RED_TIME, RINGING_LED_NOTIFICATION);
    }
    
    public void startI2RingingBlueLed() {
    	startLedNotification(BLUE_COLOR, BLUE_TIME, BLUE_TIME, RINGING_LED_NOTIFICATION);
    }
    
    public void startI2MissCallRedLed() { 
    	startLedNotification(RED_COLOR, RED_TIME, RED_TIME, MISSCALL_RED_LED_NOTIFICATION);
    }
    
    public void startI2MissCallBlueLed() {
    	startLedNotification(BLUE_COLOR, BLUE_TIME, BLUE_TIME, MISSCALL_BLUE_LED_NOTIFICATION);
    }
    
    private void startLedNotification(int color, int onMs, int offMs, int id){
    	if(DeviceUtils.isI2()) {
    		log("startLedNotification()... id = " + id);
	      	Notification.Builder builder = new Notification.Builder(mContext);
    ...
    public void stopI2RingingLed() {
    	if(DeviceUtils.isI2()) {
    		log("stopI2RingingLed()...");
    		mNotificationManager.cancel(RINGING_LED_NOTIFICATION);
    	}
    }
    
    public void stopI2MissCallLed() {
    	if(DeviceUtils.isI2()) {
    		log("stopI2MissCallLed()...");
    		mNotificationManager.cancel(MISSCALL_RED_LED_NOTIFICATION);
    	}
    }
    
    /* package */ void notifyMissedCallCustom(
            String name, String number, String type, Drawable photo, Bitmap photoIcon, long date) {
        ...
        if (VDBG) {
            log("notifyMissedCallCustom(). name: " + name + ", number: " + number
                + ", label: " + type + ", photo: " + photo + ", photoIcon: " + photoIcon
                + ", date: " + date);
        }
...
//                .setContentIntent(PendingIntent.getActivity(mContext, 0, callLogIntent, 0))
        ...
        Notification notification = builder.getNotification();
//        Notification     notification = new Notification(R.drawable.aurora_stat_notify_missed_call, mContext.getString(R.string.notification_missedCallTicker, callName), date);
        RemoteViews contentView = new RemoteViews(mContext.getPackageName(), R.layout.custom_misscall_notification); 
        Bitmap notifyIcon = BitmapFactory.decodeResource(mApp.getResources(), R.drawable.aurora_stat_notify_missed_call2); 
...
//        }
        ...
        if(PhoneUtils.isMultiSimEnabled() && mNumberMissedCalls == 1 && AuroraPhoneUtils.isShowDoubleButton()) {
        ...
        if(Build.VERSION.SDK_INT <= 19) {
	        ...
	        contentView.setOnClickPendingIntent(R.id.image_container, PendingIntent.getActivity(mContext, 0, callLogIntent, 0));
        } else {
        	notification.contentIntent = PendingIntent.getActivity(mContext, 0, callLogIntent, 0);
        }

        
        //aurora modify liguangyu 20140613 for BUG #5470 start
//        if (mNumberMissedCalls == 1
//                && !TextUtils.isEmpty(number)
        if(!TextUtils.isEmpty(number)
        //aurora modify liguangyu 20140613 for BUG #5470 end
                && !TextUtils.equals(number, mContext.getString(R.string.private_num))
                && !TextUtils.equals(number, mContext.getString(R.string.unknown))){
            if (DBG) log("Add actions with the number " + number);
            contentView.setOnClickPendingIntent(R.id.dial, PhoneGlobals.getCallBackPendingIntent(mContext, number));
            contentView.setOnClickPendingIntent(R.id.sms, PhoneGlobals.getSendSmsFromNotificationPendingIntent(mContext, number));
        } 
