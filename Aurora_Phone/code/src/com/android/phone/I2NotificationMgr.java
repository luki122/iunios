 ...
 */
public class I2NotificationMgr extends NotificationMgr {
    private static final String LOG_TAG = "I2NotificationMgr";

    static final int VOICEMAIL_NOTIFICATION_SUB2 = 20;
//    static final int CALL_FORWARD_NOTIFICATION_SUB2 = 21;
    final int CALL_FORWARD_NOTIFICATION_SUB2 = AURORA_NOTIFICATION_ID + PHONE_NOTIFICATION_ID + 2;
     ...
     * @see init()
     */
    private I2NotificationMgr(PhoneGlobals app) {
        super(app);
    }
    ...
    /* package */ static NotificationMgr init(PhoneGlobals app) {
        synchronized (I2NotificationMgr.class) {
            if (sInstance == null) {
                sInstance = new I2NotificationMgr(app);
    ...
    void updateMwi(boolean visible, Phone phone) {
        ...
        int subscription = PhoneUtils.getSubscription(phone);
        ...
        if (visible) {
            ...
            if ((vmNumber == null) && !phone.getIccRecordsLoaded()) {
                ...
                if (mVmNumberRetriesRemaining-- > 0) {
                    ...
                    ((I2CallNotifier)mApp.notifier).sendMwiChangedDelayed(
...
//                            + PhoneUtils.getSubscription(phone), false);
//            if (vibrate) {
...
//                        "com.android.phone.MSimCallFeaturesSetting");
              intent.setClassName("com.android.contacts", "com.android.contacts.activities.AuroraContactsSetting");
...
//                        );
            ...
            Bitmap bitmap;   
            if(subscription == 1) {
            	bitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.stat_sys_phone_call_forward_sub2_big);    
            } else {
            	bitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.stat_sys_phone_call_forward_sub1_big);    
            }
            Bitmap icon = Bitmap.createScaledBitmap(bitmap, 176, 176, true);    
            bitmap.recycle();
//        	notification.largeIcon = bitmap;    
    ...
    private void updateMuteNotification(int subscription) {
        ...
        if ((mCM.getState(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription)) == PhoneConstants.State.OFFHOOK) && PhoneUtils.getMute()) {
    ...
    private void updateInCallNotification(int subscription, boolean allowFullScreenIntent) {
        ...
        // notifications.
        if (mCM.getState(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription)) == PhoneConstants.State.IDLE) {
            cancelInCall();
            cancelMute();
            cancelSpeakerphone();
            return;
        ...
        final boolean hasRingingCall = mCM.hasActiveRingingCall(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription));
        final boolean hasActiveCall = mCM.hasActiveFgCall(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription));
        final boolean hasHoldingCall = mCM.hasActiveBgCall(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription));
        if (DBG) {
        ...
        Call currentCall;
        if (hasRingingCall) {
            currentCall = mCM.getFirstActiveRingingCall(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription));
        } else if (hasActiveCall) {
            currentCall = mCM.getActiveFgCall(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription));
        } else {
            currentCall = mCM.getFirstActiveBgCall(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription));
        }
...
//            builder.setTicker(expandedViewLine2);
            ...
            if (allowFullScreenIntent) {
                ...
                Call ringingCall = mCM.getFirstActiveRingingCall(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription));
    ...
    protected void updateSpeakerNotification() {
        ...
        boolean state = false;
        for (int i = 0; i < TelephonyManager.getDefault().getPhoneCount(); i++) {
            state = (state || (mCM.getState(AuroraSubUtils.getSubIdbySlot(mContext, i)) == PhoneConstants.State.OFFHOOK));
        }
