...
//        mPartialWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, TAG);
        mPartialWakeLock = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK
    ...
    private void onDisconnect(Message msg) {
        ...
        AuroraDisconnectCause cause = new AuroraDisconnectCause(conn);
        if (DBG) log("onDisconnect: connection '" + conn
                     + "', addr '" + conn.getAddress() + "', cause = " + cause);

        if (cause.mCause == AuroraDisconnectCause.OUT_OF_SERVICE) {
...
//            mPhone.setRadioPower(true);
            RadioManager.getInstance().forceSetRadioPower(true, PhoneUtils.getSubscription(mPhone), true);
        }
    }
