...
//        Phone phone = mApp.mCM.getDefaultPhone();
        int sub = AuroraPhoneUtils.getSlot(intent);
        if(sub == -1) {
        	sub = PhoneGlobals.getInstance().getVoiceSubscription();
        }
...
//      mApp.displayCallScreen();
    ...
    protected CallStatusCode placeCallInternal(Intent intent) {
        ...
        try {
            ...
            // Aurora xuyong 2015-09-02 added for bug #15979 start
            if (null == phone) {
            	return CallStatusCode.EMERGENCY_ONLY;
            }
            // Aurora xuyong 2015-09-02 added for bug #15979 end
            sub = PhoneUtils.getSubscription(phone);
            
            //aurora add liguangyu 20140928 for #8739 start
            if(DeviceUtils.isDsds() || DeviceUtils.is7505()) {
                int otherSub = PhoneUtils.getOtherActiveSub(sub);
                if(otherSub != -1) {
                	return CallStatusCode.OTHER_ACTIVE;
                }	
            }
            //aurora add liguangyu 20140928 for #8739 end
            
            if(mCM.hasActiveFgCall() || mCM.hasActiveBgCall()) {
            	if(number.equals("112") || number.equals("911")) {
                 	return CallStatusCode.OTHER_ACTIVE;
            	}
            }
            
            if (VDBG) log("- got Phone instance: " + phone + ", class = " + phone.getClass() + " sub="+ sub);

            // update okToCallStatus based on new phone
            okToCallStatus = checkIfOkToInitiateOutgoingCall(
                    phone.getServiceState().getState());
            
            //add by liguangyu for 15772 start
            phone.getPendingMmiCodes().clear();
            //add by liguangyu for 15772 end
        ...
        boolean isEmergencyNumber = AuroraPhoneNumberUtils.isLocalEmergencyNumber(number, mApp);
        boolean isPotentialEmergencyNumber =
        		AuroraPhoneNumberUtils.isPotentialLocalEmergencyNumber(number, mApp);
...
//            }
        ...
        if (okToCallStatus != CallStatusCode.SUCCESS) {
            ...
            if (isEmergencyNumber && (okToCallStatus == CallStatusCode.POWER_OFF)) {
                ...
                mEmergencyCallHelper.setPhone(phone);
                ...
                long simId = 0;
                if(phone != null) {
                    int slotId = AuroraPhoneUtils.getSlot(intent);
                    if(slotId == -1) {
                    	slotId = PhoneGlobals.getInstance().getVoiceSubscription();
                    }
                    simId = AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), slotId);
                }
                mCallLogger.logCall(null /* callerInfo */, number, 0 /* presentation */,
                        Calls.OUTGOING_TYPE, System.currentTimeMillis(), 0 /* duration */, simId);
...
//        }
        ...
        switch (callStatus) {
            case PhoneUtils.CALL_STATUS_DIALED:
                ...
                if (phone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                    ...
                    if (mApp.cdmaPhoneCallState.getCurrentCallState()
                        ...
                        mApp.cdmaPhoneCallState.setSiggleDialingState(true);                        
            ...
            case PhoneUtils.CALL_STATUS_FAILED:
                ...
                // Log failed call.
                long simId = 0;
            	int slotId = PhoneUtils.getSubscription(phone);
                simId = AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), slotId);
                mCallLogger.logCall(null /* callerInfo */, number, 0 /* presentation */,
                        Calls.OUTGOING_TYPE, System.currentTimeMillis(), 0 /* duration */, simId);
    ...
    private CallStatusCode checkIfOkToInitiateOutgoingCall(int state) {
        ...
        boolean air = Settings.System.getInt(PhoneGlobals.getInstance().getContentResolver(),
    ...
    private void handleOutgoingCallError(CallStatusCode status) {
        ...
        switch (status) {
            ...
            case POWER_OFF:
            	...
            	//aurora change liguangyu 20140317 for BUG #3215 start
                boolean air = Settings.System.getInt(PhoneGlobals.getInstance().getContentResolver(),
                        Settings.System.AIRPLANE_MODE_ON, 0) == 1;
            	if(air) {               
            	...
            	//aurora change liguangyu 20140317 for BUG #3215 end
                break;
            ...
            case DIALED_MMI:
                ...
                inCallUiState.setPendingCallStatusCode(CallStatusCode.DIALED_MMI);                
                break;
            ...
            case CALL_FAILED:
                ...
                //aurora add liguangyu 20140928 for #8739 start
            case  OTHER_ACTIVE:
                inCallUiState.setPendingCallStatusCode(CallStatusCode.OTHER_ACTIVE);
                //aurora add liguangyu 20140928 for #8739 end
            default:
