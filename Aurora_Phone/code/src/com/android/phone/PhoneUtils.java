...
public class PhoneUtils {
    ...
    private static final boolean DBG_SETAUDIOMODE_STACK = true;
    ...
    /* package */ static boolean answerCall(Call ringingCall) {
        ...
        if (ringingCall != null && ringingCall.isRinging()) {
            ...
            try {
                ...
                //aurora delete liguangyu 20131210 begin
//                setAudioMode();
                //aurora delete liguangyu 20131210 end
...
//            } else {
          	...
          	if(!ProviderUtils.isNotToUseContactsProvier()){
	            try{
	                AuroraCallerInfo info = null;
	                String content = phone.getContext().getContentResolver().SCHEME_CONTENT;
	                if ((contactRef != null) && (contactRef.getScheme().equals(content))) {
	                    info = AuroraCallerInfo.getCallerInfo(context, contactRef);
	                	if(contactRef.toString().startsWith("content://com.android.contacts/contacts/lookup/")) {
	                		int index = contactRef.toString().lastIndexOf('/');
	                		info.person_id = Long.valueOf(contactRef.toString().substring(index + 1));
	                	}
	                }
	                ...
	                // the lookup failed.
	                if (null == info) {
	                    info = AuroraCallerInfo.getCallerInfo(context, number);
	                }
	                info.phoneNumber = number;
	                final AuroraCallerInfo finfo = info;
	                new Thread(new Runnable() {
	                    @Override
	                    public void run() {
	                    	finfo.auroraUpdate(PhoneGlobals.getInstance());
	                    }
	                }).start();
...
//            }
    ...
    static void displayMMIComplete(final Phone phone, Context context, final MmiCode mmiCode,
            ...
            AuroraAlertDialog previousAlert) {
        final PhoneGlobals app = PhoneGlobals.getInstance();
        CharSequence text;
        ...
        if ((app.getPUKEntryActivity() != null) && (state == MmiCode.State.COMPLETE)) {
            ...
            if (state != MmiCode.State.PENDING) {
                ...
                AuroraAlertDialog newDialog = new AuroraAlertDialog.Builder(context)
                        .setMessage(text)
                ...
                // build the dialog
                final AuroraAlertDialog newDialog = new AuroraAlertDialog.Builder(context)
                        .setMessage(text)
                        .setView(dialogView)
     ...
     */
    static AuroraCallerInfo getCallerInfo(Context context, Connection c) {
        AuroraCallerInfo info = null;
        ...
        if (c != null) {
            ...
            if (userDataObject instanceof Uri) {
                info = AuroraCallerInfo.getCallerInfo(context, (Uri) userDataObject);
                if (info != null) {
                    c.setUserData(info);
                }
            } else {
                if (userDataObject instanceof CallerInfoToken) {
                    ...
                    //final query result
                    info = (AuroraCallerInfo) userDataObject;
                }
                if (info == null) {
    ...
    public static class CallerInfoToken {
        ...
        public AuroraCallerInfo currentInfo;
        public AuroraCallerInfoAsyncQuery asyncQuery;
    }
    ...
    static CallerInfoToken startGetCallerInfo(Context context, Call call,
    		AuroraCallerInfoAsyncQuery.OnQueryCompleteListener listener, Object cookie) {
    ...
    static CallerInfoToken startGetCallerInfo(Context context, Connection c,
        ...
        if (userDataObject instanceof Uri) {
            ...
            cit = new CallerInfoToken();
            cit.currentInfo = new AuroraCallerInfo();
            cit.asyncQuery = AuroraCallerInfoAsyncQuery.startQuery(QUERY_TOKEN, context,
            ...
            if (!TextUtils.isEmpty(number)) {
                ...
                if (cit.currentInfo.numberPresentation != PhoneConstants.PRESENTATION_ALLOWED && !AuroraPhoneUtils.isSimulate()) {
                    cit.isFinal = true;
                } else {
            ...
            cit = new CallerInfoToken();
            cit.currentInfo = (AuroraCallerInfo) userDataObject;
            cit.asyncQuery = null;
            cit.isFinal = true;
    ...
    static AuroraCallerInfoAsyncQuery.OnQueryCompleteListener sCallerInfoQueryListener =
        new AuroraCallerInfoAsyncQuery.OnQueryCompleteListener () {
            /**
            ...
            public void onQueryComplete(int token, Object cookie, AuroraCallerInfo ci) {
                ...
                if (ci.contactExists || ci.isEmergencyNumber() || ci.isVoiceMailNumber()) {
                    ...
                    AuroraCallerInfo newCi = getCallerInfo(null, conn);
                    ...
                    if (newCi != null) {
                        ...
                        newCi.mArea = ci.mArea;
                        newCi.mRealNumber = ci.phoneNumber;
                        newCi.mMark = ci.mMark;
                        newCi.mMarkNumber = ci.mMarkNumber;
                        newCi.cachedPhoto = ci.cachedPhoto;
                        newCi.cachedPhotoIcon = ci.cachedPhotoIcon;
                        newCi.isCachedPhotoCurrent = ci.isCachedPhotoCurrent;
                        newCi.isCompelete = ci.isCompelete;  
                        newCi.name = AuroraAreaUtils.getNumName( ci.phoneNumber);
                        if(!TextUtils.isEmpty(newCi.mMark) && TextUtils.isEmpty(newCi.name)) {
                        	newCi.name = newCi.mMark;
                        }
                        newCi.mArea = AuroraAreaUtils.getNumLocation( ci.phoneNumber);
                        ci = newCi;
                    }
                }
     ...
     */
    static String getCompactNameFromCallerInfo(AuroraCallerInfo ci, Context context) {
    ...
    /* package */ static boolean startNewCall(final CallManager cm) {
        ...
        //aurora delete liguangyu 20131106 start
...
//        }
        //aurora delete liguangyu 20131106 end
    ...
    static boolean isSpeakerOn(Context context) {
        AudioManager audioManager = (AudioManager) context.getSystemService(Context.AUDIO_SERVICE);
        if (DBG) log("isSpeakerOn = " +  audioManager.isSpeakerphoneOn());
    ...
    private static void setMuteInternal(Phone phone, boolean muted) {
        ...
        //aurora modify liguangyu 20140606 for BUG #5423 start
        String prop = SystemProperties.get("ro.build.display.id");
        boolean isMi3t = prop.contains("MI3T");
        if (routeToAudioManager && !isMi3t) {
        //aurora modify liguangyu 20140606 for BUG #5423 end
    ...
    /* package */ static void setAudioMode(CallManager cm) {
        ...
        if(state == PhoneConstants.State.RINGING) {
          ...
          ///M: ALPS01765149, change audio mode to normal @{
            boolean mFlightModeOn = System.getInt(context.getContentResolver(),
            ...
            //ALPSALPS01781841, not set Ecc audio mode when the state is CONNECTING
//            if (call.isEmergencyCall() && call.getState() == CallState.CONNECTING) {
//                return;
//            }
    ...
    private static void abandonAudioFocus() {
        if (hasFocus()) {
            ...
            Log.v(LOG_TAG, "abandoning audio focus");
            Context context = PhoneGlobals.getInstance();
            AudioManager audioManager = (AudioManager)
                    context.getSystemService(Context.AUDIO_SERVICE);
    ...
    private static void setMode(int newMode) {
        ...
        Log.v(LOG_TAG, "Request to change audio mode from oldMode = " + oldMode + " newMode= " + newMode);
        ...
        if (oldMode != mode) {
            ...
            if (oldMode == AudioManager.MODE_IN_CALL && newMode == AudioManager.MODE_RINGTONE) {
                Log.i(LOG_TAG, "Transition from IN_CALL -> RINGTONE. Resetting to NORMAL first.");
                audioManager.setMode(AudioManager.MODE_NORMAL);
            }
            audioManager.setMode(newMode);
//            mMostRecentlyUsedMode = newMode;
        }
        Log.v(LOG_TAG, "Request to change audio mode from oldMode end");
    }
    
    
    private static int audioModeUpdateDualModem(int mode) {
        Log.d(LOG_TAG, "audioModeUpdateDualModem, mode:" + mode + " isDualtalk:" + isDualtalk());
        CallManager cm = I2PhoneGlobals.getInstance().mCM;
        ...
        if (isDualtalk() || isEvdoDualtalk() || isC2kSupported()) {
            ...
            if (hasActiveCall && mode == AudioManager.MODE_IN_CALL) {
            	  phoneId = getSubscription(call.getPhone());
                if (phoneId == SubscriptionManager.INVALID_PHONE_INDEX) {
                    Log.d(LOG_TAG, "Can't get the exactly phone id now.");
        ...
        Log.d(LOG_TAG, "getExternalModemSlot:" + getExternalModemSlot() + ", mode:" + mode);
        ...
        if ((isEvdoDualtalk() || isC2kSupported())
            ...
            && (oldMode == AudioManager.MODE_IN_CALL || oldMode == AudioManager.MODE_IN_CALL_2
                || oldMode == AudioManager.MODE_IN_CALL_EXTERNAL)) {
            if (PhoneGlobals.getInstance().getPhone(phoneId).getPhoneType() != PhoneConstants.PHONE_TYPE_GSM) {
        ...
        Log.d(LOG_TAG, "audioModeUpdateDualModem, oldMode:" + oldMode + ", newMode:" + newMode);
        return newMode;
    }
    
    private static boolean isSvlteSupported() {
        return (SystemProperties.get("ro.mtk_svlte_support").equals("1"));
    }

    private static int getPhone1Type() {
        int subIds[] = SubscriptionManager.getSubId(getExternalModemSlot());
        if (subIds == null) {
            Log.d(LOG_TAG, "getPhone1Type, subIds is null");
            return PhoneConstants.PHONE_TYPE_NONE;
        } else {
            Log.d(LOG_TAG, "getPhone1Type, subIds[0]:" + subIds[0]);
    ...
    private static int mDualModemCall = 0;
    private static int getTelephonyMode() {
        if (sTelephonyMode < 0)
            sTelephonyMode = SystemProperties.getInt("ril.telephony.mode", 0);
        return sTelephonyMode;
    }

    private static int getFirstMD() {
    ...
    private static void audioModeCusUpdate(int mode) {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    fw2.write(state2);
                    fw2.close();
                    Log.v(LOG_TAG, "Call state for kpd is  %s" + state2);
                } catch (Exception e) {
                	e.printStackTrace();
                }
            }
    ...
    private static int mAudioFocusStreamType = STREAM_NONE;
    private static void requestAudioFocusAndSetMode(int stream, int mode) {
        Log.i(LOG_TAG, "requestAudioFocusAndSetMode, stream = " + mAudioFocusStreamType + " mode = " + mode);
        ...
        if (mAudioFocusStreamType != stream) {
            Log.v(LOG_TAG, "requesting audio focus for stream =" + stream);
            Context context = PhoneGlobals.getInstance();
            AudioManager audioManager = (AudioManager)
                    context.getSystemService(Context.AUDIO_SERVICE);
//            audioManager.requestAudioFocusForCall(stream,  AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
            audioManager.requestAudioFocusForCall(stream,  AudioManager.AUDIOFOCUS_GAIN);
        }
     ...
     */
 	//aurora add liguangyu 20140324 for BUG #3483  start
    private static boolean isLongPressBefore = false;
 	//aurora add liguangyu 20140324 for BUG #3483  end
    /* package */ static boolean handleHeadsetHook(Phone phone, KeyEvent event) {
        ...
        if (hasRingingCall &&
            ...
            int phoneType = phone.getPhoneType();
            if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
            	AuroraPhoneUtils.answerCallAndSleepSoon(phone.getRingingCall());
            } else if ((phoneType == PhoneConstants.PHONE_TYPE_GSM)
                    || (phoneType == PhoneConstants.PHONE_TYPE_SIP)
                    || phoneType == PhoneConstants.PHONE_TYPE_IMS) {
            ...
            if (event.isLongPress()) {
             	//aurora modify liguangyu 20140324 for BUG #3483  start
            	if(hasRingingCall){
                  if (DBG) log("handleHeadsetHook: longpress -> hangup");
            		hangup(app.mCM);
            	} else {
	                if (DBG) log("handleHeadsetHook: longpress -> hold");
	                if (hasActiveCall && !hasHoldingCall) {
	                    switchHoldingAndActive(app.mCM.getFirstActiveBgCall()); 
	                } else if (!hasActiveCall && hasHoldingCall) {
	                    switchHoldingAndActive(app.mCM.getFirstActiveBgCall()); 
	                } 	     
            	}
                isLongPressBefore = true;
//                closeDialpadInternal(true);
            }
            else if (event.getAction() == KeyEvent.ACTION_UP &&
                     event.getRepeatCount() == 0 &&
                     !isLongPressBefore) {
            	  if (DBG) log("handleHeadsetHook: press -> hangup");
...
//                }
                hangup(app.mCM);
                //aurora modify liguangyu 20140324 for BUG #3483  end
            }
        }
     ...
     */
    /* package */ static String modifyForSpecialCnapCases(Context context, AuroraCallerInfo ci,
    ...
    /* package */ static void checkAndCopyPhoneProviderExtras(Intent src, Intent dst) {
        ...
        Uri uri = (Uri) src.getParcelableExtra("contactUri");
        dst.putExtra("contactUri", uri);
    ...
    */
    ...
    public static Phone pickPhoneBasedOnNumber(CallManager cm,
        ...
        if(isMultiSimEnabled()) {
         	 try {
                 int simMode = Settings.System.getInt(PhoneGlobals.getInstance().getContentResolver(),
                         Settings.System.MSIM_MODE_SETTING, -1);
         		TelephonyManager mTelephonyManager = TelephonyManager.getDefault();
         		boolean iscard1Enable = (mTelephonyManager.getSimState(0) != TelephonyManager.SIM_STATE_ABSENT) && ((simMode & 1) > 0);
        		boolean iscard2Enable = (mTelephonyManager.getSimState(1) != TelephonyManager.SIM_STATE_ABSENT) && ((simMode & 2) > 0);          
                 Log.d(LOG_TAG, "pickPhoneBasedOnNumber iscard1Enable= " + iscard1Enable + " iscard2Enable = " + iscard2Enable);
                 if(subscription > 1 || subscription < 0) { 
                	 subscription = 0;
                 }
                 if(!iscard1Enable && iscard2Enable && subscription == 0) {
                	 subscription = 1;
                 } else if(iscard1Enable && !iscard2Enable && subscription == 1) {
                	 subscription = 0;
                 }
                 
                 if(!TextUtils.isEmpty(number) && number.equalsIgnoreCase("112")) {
            		 if(PhoneGlobals.getInstance().getPhone(subscription).getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            			 subscription = subscription > 0 ? 0 : 1;
            		 }                	 
                 }
             } catch (Exception e) {
          	    e.printStackTrace();
...
//        context.startService(intent);
    ...
    public static int getSubscription(Phone phone) {
    ...
    public static int getActiveSubscription() {
      	if(isMultiSimEnabled()) {
      		if(PhoneGlobals.getInstance().getPhone(1).getState() != PhoneConstants.State.IDLE){
      			return 1;	
      		}
      	} 
      	return 0;
    ...
    public static boolean isAnyOtherSubActive(int subscription) {
    	if(isMultiSimEnabled()) {
        	int other = getOtherActiveSub(subscription);
            return (other != -1 
            		&& !PhoneGlobals.getInstance().mCM.hasActiveRingingCall(other) 
            		&& PhoneGlobals.getInstance().getPhone(other).getRingingCall().getState() != Call.State.DISCONNECTING) ? true : false;
    	} else {
    		return false;
    	}
    }
    ...
    public static void switchToOtherActiveSub(int subscription) {
    	if(isMultiSimEnabled()) {
    		...
    		int other = getOtherActiveSub(subscription);
            if(other != -1) {
            	int otherSubId = AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), other);
                boolean hasfgCall = cm.hasActiveFgCall(otherSubId);
                boolean hasbgCall = cm.hasActiveBgCall(otherSubId);
                if(hasfgCall && hasfgCall) {
                    Call fgCall = cm.getActiveFgCall(otherSubId);
                	hangup(fgCall);
                }           
            }    		

            int subid = AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), subscription);
            boolean hasbgCall = cm.hasActiveBgCall(subid);
            if(hasbgCall) {
                Call bgCall = cm.getFirstActiveBgCall(subid);
            	switchHoldingAndActive(bgCall);
            }
    ...
    public static void switchToOtherActiveSubRemainInLch(int subscription) {
    	if(isMultiSimEnabled()) {  
	        int count = getPhoneCount();
	        CallManager cm = I2PhoneGlobals.getInstance().mCM;
	
	        Log.d(LOG_TAG, "switchToOtherActiveSubRemainInLch: sub = " + subscription
	                +  " count = "+ count);
	        for (int i = 0; i < count; i++) {
	            if ((i != subscription) && (cm.getState(AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), i)) != PhoneConstants.State.IDLE)) {
...
//	                setMute(true);
    ...
    public static int getOtherActiveSub(int subscription) {
    	if(isMultiSimEnabled()) { 
            ...
            for (int i = 0; i < count; i++) {
                if ((i != subscription) && PhoneGlobals.getInstance().getPhone(i).getState() != PhoneConstants.State.IDLE) {
    ...
    public static int getSimState(int slot) {
    	return TelephonyManager.getDefault().getSimState(slot);
    }
