...
public class CallNotifier extends Handler
        implements AuroraCallerInfoAsyncQuery.OnQueryCompleteListener {
...
//            (PhoneGlobals.DBG_LEVEL >= 1) && (SystemProperties.getInt("ro.debuggable", 0) == 1);
    ...
    // out...)
    protected boolean mCallWaitingTimeOut = false;
    ...
    protected static final int PHONE_STATE_CHANGED = 1;
    protected static final int PHONE_NEW_RINGING_CONNECTION = 2;
    protected static final int PHONE_DISCONNECT = 3;
    protected static final int PHONE_UNKNOWN_CONNECTION_APPEARED = 4;
    protected static final int PHONE_INCOMING_RING = 5;
    protected static final int PHONE_STATE_DISPLAYINFO = 6;
    protected static final int PHONE_STATE_SIGNALINFO = 7;
    protected static final int PHONE_CDMA_CALL_WAITING = 8;
    protected static final int PHONE_ENHANCED_VP_ON = 9;
    protected static final int PHONE_ENHANCED_VP_OFF = 10;
    protected static final int PHONE_RINGBACK_TONE = 11;
    protected static final int PHONE_RESEND_MUTE = 12;
    ...
    protected static final int DISPLAYINFO_NOTIFICATION_DONE = 24;
    protected static final int EVENT_OTA_PROVISION_CHANGE = 25;
    protected static final int CDMA_CALL_WAITING_REJECT = 26;
    protected static final int UPDATE_IN_CALL_NOTIFICATION = 27;
    protected static final int ENABLE_TOUCH_DELAY = 17000;
    protected static final int PHONE_VOICE_RINGING_CONNECTION = 101;
    ...
    protected ToneGenerator mSignalInfoToneGenerator;
    ...
    protected CallNotifier(PhoneGlobals app, Phone phone, Ringer ringer, CallLogger callLogger) {
        ...
        listen();
        //aurora add liguangyu 20140811 start
        createScreenOnLock(mApplication);
        //aurora add liguangyu 20140811 end
    }
    ...
    protected void createSignalInfoToneGenerator() {
...
//                if (now < InCallScreen.mLastAnswerCallWaitingTime + 1000) {
...
//                        //aurora modify liguangyu 20140808 for BUG #7663 start
//                        try {                        	
//	                        if(!RejectUtils.isAuroraNeedHangup(pb.getRingingCall().getLatestConnection().getAddress(), false)) {
//	                            mRinger.ring();
//	                        }
//	                    } catch (Exception e) {}
//                        //aurora modify liguangyu 20140808 for BUG #7663 end
//                    } else {
...
//                }
    ...
    protected void onNewRingingConnection(AsyncResult r) {
        ...
        mApplication.mPreventTouchManager.reset();
		
		if(RejectUtils.isAuroraNeedHangup(c.getAddress(), true)) {
...
//        mApplication.notificationMgr.updateNotificationAndLaunchIncomingCallUi();
    ...
    protected void onPhoneStateChanged(AsyncResult r) {
        ...
        if (state == PhoneConstants.State.OFFHOOK) {
            ...
            long now = SystemClock.uptimeMillis();
            if (now < PreventTouchManager.mLastProximityActionTime + 3000) {
        	...
        	mApplication.notificationMgr.stopI2RingingLed();
        }

        if (fgPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            Connection c = fgPhone.getForegroundCall().getLatestConnection();
            if ((c != null) && (AuroraPhoneNumberUtils.isLocalEmergencyNumber(c.getAddress(),
                                                                        mApplication))) {
...
//			mApplication.notificationMgr.notifyMissedCall(ci.name,
//					ci.phoneNumber, ci.phoneLabel, ci.cachedPhoto,
//					ci.cachedPhotoIcon, ((Long) cookie).longValue());
            AuroraPrivacyUtils.auroranotifyPrivateMissedCall(ci, ci.name, ci.phoneNumber, ((Long) cookie).longValue());
            ...
            if (isQueryExecutionTimeOK) {
            	...
            	if(ci.mRawContactId > 0) {
            		mApplication.notificationMgr.startI2RingingRedLed();
            	} else {
             		mApplication.notificationMgr.startI2RingingBlueLed();
            	}
                ...
                Ringer ringer = ((CallNotifier) cookie).mRinger;
                ringer.setContactUri(ci.lookupUri);
     ...
     */
    protected void onCustomRingtoneQueryTimeout(String number) {
    ...
    protected void onDisconnect(AsyncResult r) {
        ...
        if (c != null) {
            ...
            } else if (cause.mCause == AuroraDisconnectCause.CDMA_REORDER) {
                if (DBG) log("- need to play CDMA_REORDER tone!");
                toneToPlay = InCallTonePlayer.TONE_REORDER;
            } else if (cause.mCause == AuroraDisconnectCause.CDMA_INTERCEPT) {
                if (DBG) log("- need to play CDMA_INTERCEPT tone!");
                toneToPlay = InCallTonePlayer.TONE_INTERCEPT;
            } else if (cause.mCause == AuroraDisconnectCause.CDMA_DROP) {
            ...
            } else if (cause.mCause == AuroraDisconnectCause.UNOBTAINABLE_NUMBER) {
                if (DBG) log("- need to play TONE_UNOBTAINABLE_NUMBER tone!");
                toneToPlay = InCallTonePlayer.TONE_UNOBTAINABLE_NUMBER;
            } else if (cause.mCause == AuroraDisconnectCause.ERROR_UNSPECIFIED) {
        ...
        if ((toneToPlay == InCallTonePlayer.TONE_NONE)
            ...
            && (c != null)) {
           	AuroraDisconnectCause cause = new AuroraDisconnectCause(c);
            if ((cause.mCause == AuroraDisconnectCause.NORMAL)  // remote hangup
                || (cause.mCause == AuroraDisconnectCause.LOCAL)) {  // local hangup
        ...
        if (c != null) {
            ...
            final boolean isEmergencyNumber =
            		AuroraPhoneNumberUtils.isLocalEmergencyNumber(number, mApplication);
            ...
            final AuroraDisconnectCause cause = new AuroraDisconnectCause(c);
...
//                new InCallTonePlayer(toneToPlay).start();
                new InCallTonePlayer(toneToPlay).run();
            ...
            if (c.isIncoming()) {
                callLogType = (cause.mCause == AuroraDisconnectCause.INCOMING_MISSED ?
                               Calls.MISSED_TYPE : Calls.INCOMING_TYPE);
            } else {
                callLogType = Calls.OUTGOING_TYPE;
            }
            if(duration == 0 && callLogType == Calls.INCOMING_TYPE && RejectUtils.isToAddBlack(number)) {
	            AuroraCallerInfo callerInfo = getCallerInfoFromConnection(c);            
	            String name = "";
	            if(callerInfo != null) {
	            	name = callerInfo.name;
	            }
	            mApplication.mManageReject.notificationMgr.notifyAddBlackCall(number , name);
            }
        }
    }
    ...
    static boolean mIsSilenceRinger = false;
...
//                  toneVolume = TONE_RELATIVE_VOLUME_HIPRI;
     ...
     */
    protected void onDisplayInfo(AsyncResult r) {
    ...
    protected void onCdmaCallWaiting(AsyncResult r) {
        ...
        CdmaCallWaitingNotification cw = (CdmaCallWaitingNotification) r.result;
        //aurora modify liguangyu 20140918 for #8306 start				
		
		if(RejectUtils.isAuroraNeedHangup(cw.number, true)) {
        	sendCdmaCallWaitingReject();
            return;
		}
				
		
        //aurora modify liguangyu 20140918 for #8306 end

		

        if (PhoneUtils.isMultiSimEnabled()) {
            PhoneUtils.setActiveSubscription(AuroraMSimConstants.SUB1);
        }
     ...
     */
    protected void onCdmaCallWaitingReject() {
        final Call ringingCall = mCM.getFirstActiveRingingCall();
    ...
    protected void showMissedCallNotification(Connection c, final long date) {
    	int slot = PhoneUtils.getSubscription(c.getCall().getPhone());
        long simId = AuroraSubUtils.getSubIdbySlot(PhoneGlobals.getInstance(), slot);
    	if(simId > 0) {
    		NotificationMgr.mSimId = (int)simId;
    		PrivateNotificationMgr.mSimId = (int)simId;
    	}
        ...
        if (info != null) {
            ...
            if (info.isFinal) {
                ...
                AuroraCallerInfo ci = info.currentInfo;
                ...
                } else if (ci.numberPresentation != PhoneConstants.PRESENTATION_ALLOWED && !AuroraPhoneUtils.isSimulate()) {
...
//	                        ci.phoneLabel, ci.cachedPhoto, ci.cachedPhotoIcon, date);
                AuroraPrivacyUtils.auroranotifyPrivateMissedCall(ci, name, number, date);
            }
        } else {
    ...
    protected void onRingbackTone(AsyncResult r) {
     ...
     */
    private String getLogNumber(Connection conn, AuroraCallerInfo callerInfo) {
        String number = null;
     ...
     */
    AuroraCallerInfo getCallerInfoFromConnection(Connection conn) {
     ...
     */
    int getPresentation(Connection conn, AuroraCallerInfo callerInfo) {
        int presentation;
    ...
    void playRecordTone() {
            log("- starting record tone");
            new InCallTonePlayer(InCallTonePlayer.TONE_INTERCEPT).start();
    }
    
    public boolean isRingingTouchOn = true;

    //aurora add liguangyu 20140811 start
    protected PowerManager.WakeLock mBrightWakeLock;
    
    protected void createScreenOnLock(Context context) {
    	PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);  
   	    mBrightWakeLock = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK|PowerManager.ACQUIRE_CAUSES_WAKEUP, LOG_TAG);
        mBrightWakeLock.setReferenceCounted(false);
   }
   //aurora add liguangyu 20140811 end
