...
import com.android.phone.Recorder;
...
public class PhoneRecorder extends Recorder implements MediaScannerConnection.MediaScannerConnectionClient {
    ...
    public void startRecord() {
        ...
        } else if (!GnPhoneRecordHelper.diskSpaceAvailable(Constants.PHONE_RECORD_LOW_STORAGE_THRESHOLD)) {
            mSampleInterrupted = true;
...
//            sSDcardFullFlag = !GnPhoneRecordHelper.diskSpaceAvailable(Constants.PHONE_RECORD_LOW_STORAGE_THRESHOLD);
//        } else {
            sSDcardFullFlag = !GnPhoneRecordHelper.diskSpaceAvailable(Constants.PHONE_RECORD_LOW_STORAGE_THRESHOLD);
//        }
        // Gionee fangbin 20121114 modified for CR00728364 end
                ...
                if (sSDcardFullFlag) {
                    Toast.makeText(mContext, mContext.getResources().
               	 	...
               	 	Toast.makeText(PhoneGlobals.getInstance(), 
               	 			PhoneGlobals.getInstance().getResources().getString(R.string.gn_record_save), Toast.LENGTH_SHORT).show();
                }
            }
        } else {
            delete();
...
//        cv.put("drm_dataLen", mAddedDate + this.sampleLength());
    ...
    private long getCreateTime() {
        Call call = PhoneGlobals.getInstance().mCM.getActiveFgCall();
        if (PhoneGlobals.getInstance().mCM.hasActiveRingingCall()) {
            call = PhoneGlobals.getInstance().mCM.getFirstActiveRingingCall();
        }
        if (null != call) {
            return call.getEarliestCreateTime();
        }
        return 0;
    }
    // Gionee fangbin 20121114 added for CR00728364 end
