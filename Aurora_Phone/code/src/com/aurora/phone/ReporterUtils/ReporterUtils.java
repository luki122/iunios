...
//    “设置”的点击频次 
    ...
    private static void addInternal(String item){
        ...
        try {
        	int id = PhoneGlobals.getInstance().getContentResolver().update(report_uri, values, null, null);
			Log.v(LOG_TAG, " id =" + id);
        } catch (Exception e) {
        	e.printStackTrace();
        }
    ...
    private static int queryInternal(String item){
    	int count = -2;	
		Cursor cursor = PhoneGlobals.getInstance().getContentResolver().query(report_uri,
				null,
