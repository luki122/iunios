...
//		Cursor cursor = PhoneGlobals.getInstance().getContentResolver().query(black_uri, BLACK_PROJECTION,
//				"(isblack = '1' OR isblack = '3') AND number = '" + number + "'", null, null);
    ...
    public static boolean isToAddBlack(String number) {
		...
		if(AuroraPrivacyUtils.isSupportPrivate()) {
    	 ...
    	 Cursor cursor = PhoneGlobals.getInstance().getContentResolver().query(uri, projection,
...
//        return PhoneGlobals.getInstance().getResources().getBoolan(R.bool.aurora_black_list);
...
//		mCache.put(number, result);
    ...
    public static String formatAuroraTimeStampString(Context context, long when) {
        Time then = new Time();
        then.set(when);
        Time now = new Time();
        now.setToNow();
