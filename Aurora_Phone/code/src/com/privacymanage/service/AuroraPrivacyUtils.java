...
public class AuroraPrivacyUtils {
	...
	public static void bindService(Context context) {
		if (!mIsServiceConnected) {
			try {
				context.bindService(AuroraPhoneUtils.getExplicitIntent(context, intent), serviceConnection, Context.BIND_AUTO_CREATE|Context.BIND_IMPORTANT);
				mIsPrivacyMode = true;
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static void killPrivacyActivity() {
		try {
			if (PhoneApp.mPrivacyActivityList != null) {
				for (Activity ac : PhoneApp.mPrivacyActivityList) {
					ac.finish();
				}
			}
		} catch (Exception e) {
	 ...
	 public static boolean isSupportPrivate() {
	    ...
	    public static int[] getPrivateData(Uri uri) {
			...
			if (url.startsWith("content://com.android.contacts/contacts/lookup/") || url.startsWith("content://com.android.contacts/contacts/")) {
				long RawContactId = ProviderUtils.queryForRawContactId(PhoneGlobals.getInstance()
				...
				Cursor c = PhoneGlobals.getInstance().getContentResolver()
				...
				Cursor cursor = PhoneGlobals.getInstance().getContentResolver()
		...
		public static String getPrivateRingNotificationText(long accountId) {	
			Log.v("getPrivateRingNotificationText", " accountId = " + accountId);
		   	String defaultText = PhoneGlobals.getInstance().getResources().getString(R.string.private_notification_text);
			String[] columns={MSG_NOTIFY_HINT};
			
			Cursor cursor = PhoneGlobals.getInstance().getContentResolver().query(PRIVATE_CONTENT_URI,
					columns,
					getQueryWhere(),
		...
		public static String getPrivateHomePath(long accountId) {	
			...
			Cursor cursor = PhoneGlobals.getInstance().getContentResolver().query(PRIVATE_PATH_URI,
					columns,
					getQueryWhere(),
					getQueryValue(accountId),
					null);	 
...
//	    	String defaultText = PhoneGlobals.getInstance().getResources().getString(R.string.private_notification_text);
//	    	try {
//		    	Context context =  PhoneGlobals.getInstance().createPackageContext("com.android.contacts", Context.CONTEXT_IGNORE_SECURITY);    	
...
//	    }
	    ...
	    public static void auroranotifyPrivateMissedCall(AuroraCallerInfo ci, String name, String number, long date) {
			...
			if(privateId > 0) {
				if (privateId != currentPrivateId) {
			    	...
			    	PhoneGlobals.getInstance().mManagePrivate.notificationMgr.notifyPrivateMissedCall(name, number,
		                    ci.phoneLabel, ci.cachedPhoto, ci.cachedPhotoIcon, date, privateId);
				}
			} else {
		    	logs("auroranotifyPrivateMissedCall = 2");   
		    	PhoneGlobals.getInstance().notificationMgr.notifyMissedCall(name, number,
	            ci.phoneLabel, ci.cachedPhoto, ci.cachedPhotoIcon, date);
			}
		 	if(ci.mRawContactId > 0) {
		 		PhoneGlobals.getInstance().notificationMgr.startI2MissCallRedLed();
        	} else {
        		PhoneGlobals.getInstance().notificationMgr.startI2MissCallBlueLed();
        	}
