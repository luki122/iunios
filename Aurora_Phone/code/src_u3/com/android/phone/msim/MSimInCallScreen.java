...
public class MSimInCallScreen extends InCallScreen {
    ...
    protected void onDisconnect(AsyncResult r) {
        ...
        DisconnectCause cause = c.getDisconnectCause();
...
//              return;
        ...
        if (phoneIsCdma) {
            ...
            if ((callState == Call.State.ACTIVE)
                ...
                showCallLostDialog();
                        ...
                        && (cause != DisconnectCause.LOCAL)
                        && (cause != DisconnectCause.INCOMING_REJECTED)
                        && (cause != DisconnectCause.EMERGENCY_TEMP_FAILURE)
                        && (cause != DisconnectCause.EMERGENCY_PERM_FAILURE)) {
        ...
        mLastDisconnectCause = new AuroraDisconnectCause(c);
        ...
        if(!currentlyIdle) {
        	//aurora add liguangyu 20140925 for #8692 start
        	mInCallTouchUi.setLastIncomingCallActionTime(0);
        	//aurora add liguangyu 20140925 for #8692 end
        	mIsUserHangup = false;
        }
...
//          delayedCleanupAfterDisconnect(c.getCall().getPhone());
          mHandler.sendEmptyMessageDelayed(DELAYED_CLEANUP_AFTER_DISCONNECT,
        		  mApp.mManageVibrator.isSilentMode() ? 1000 : 500);
          //aurora change liguangyu 20131116 for end
            ...
            //aurora modify liguangyu 20140808 for BUG #7372 start
            if(!currentlyIdle || cause != DisconnectCause.NORMAL || !c.isIncoming()) {
            	updateScreen();
            }
            //aurora modify liguangyu 20140808 for BUG #7372 end
            ...
            //aurora delete liguangyu 20140925 for #8611 start
...
//                return;
//            }
            //aurora delete liguangyu 20140925 for #8611 end
          			...
          			public void run() {
          			    boolean isFront = false;
          		   	    try {  		   		  
          		   	        ...
          		   	        if(cn.getClassName().equalsIgnoreCase("com.android.phone.MSimInCallScreen")) {
          		   	        	isFront = true;
          		   	        }
          		   	    } catch (Exception e) {
          		   		   e.printStackTrace();
          		   	    }
          		   	    if(isFront && (mCM.getState() == PhoneConstants.State.IDLE)) {
          		   	        Log.v(LOG_TAG, "hide InCallScreen force");
          		   	        forceFinish(); 
          		   	    }
          			}    		
            	}, 3000);
...
//                if ((mLastDisconnectCause != DisconnectCause.INCOMING_MISSED)
//                        && (mLastDisconnectCause != DisconnectCause.INCOMING_REJECTED)
