...
public class MSimCallNotifier extends CallNotifier {
    ...
    private void onMwiChanged(boolean visible, Phone phone) {
        ...
        ((MSimPhoneGlobals)PhoneGlobals.getInstance()).handleNetMode(phone);
...
//                PhoneGlobals.getInstance().sendBroadcast(intent);
            ...
            public void onDataActivity(int state) {
            	if(state == TelephonyManager.DATA_ACTIVITY_IN) {
            		 ((PhoneBase) (((PhoneProxy) PhoneGlobals.getInstance().getPhone(mSubscription)).getActivePhone())).notifyDataConnection("lgy");            		
            	}
          }
    ...
    protected void onNewRingingConnection(AsyncResult r) {
        ...
        final int subscription = c.getCall().getPhone().getSubscription();
        ...
        if(mCM.hasActiveRingingCall(otherSub)) {
            PhoneUtils.hangupRingingCall(ringing);
            return;
//        } else if(mCM.hasActiveFgCall(otherSub) || mCM.hasActiveBgCall(otherSub)) {
//            this.postDelayed(new Runnable() {
//                public void run() {
//                	if(mCM.hasActiveRingingCall(subscription)) {
//                		PhoneUtils.hangupRingingCall(mCM.getFirstActiveRingingCall(subscription));
//                	}
//                }
//            }, 5000);   
//            stopSignalInfoTone();
...
//        }
    ...
    protected void showIncomingCall(int subscription) {
        ...
        //aurora modify liguangyu 20140807 for BUG #7294 start
...
//        mApplication.startService(intent);  
        //aurora modify liguangyu 20140807 for BUG #7294 end
        ...
        this.postDelayed(new Runnable() {
  			public void run() {
  			    boolean isStarted = true;
  		   	    ...
  		   	    if(!isStarted 
  		   			    && (mCM.getState(0) == PhoneConstants.State.RINGING || mCM.getState(1) == PhoneConstants.State.RINGING)) {
        ...
        if(mCM.hasActiveFgCallAnyPhone()) {
...
//      				mBrightWakeLock.acquire(100);
    ...
    protected void onDisconnect(AsyncResult r) {
        ...
        if (c != null) {
            ...
            if (VDBG) log("- isToAddBlack duration = " + duration + " callLogType = " + callLogType);
