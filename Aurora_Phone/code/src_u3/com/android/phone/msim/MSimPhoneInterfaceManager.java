        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case CMD_SET_DATA_SUBSCRIPTION:
                    ...
                    if (subManager != null) {
                    	//aurora modify liguangyu 20140527 for BUG #5143 start
                    	try {
                            Log.w(LOG_TAG, "CMD_SET_DATA_SUBSCRIPTION subscription =" + subscription);
...
//                            subManager.setDataSubscription(subscription, isTempSwitch, onCompleted);
                    	...
                    	//aurora modify liguangyu 20140527 for BUG #5143 end
                    } else {
    ...
    public boolean supplyPin(String pin, int subscription) {
        ...
        checkSimPin.start();
        return (checkSimPin.unlockSim(null, pin)==PhoneConstants.PIN_RESULT_SUCCESS)?true:false;
    }
    ...
    public boolean supplyPuk(String puk, String pin, int subscription) {
        ...
        checkSimPuk.start();
        return (checkSimPuk.unlockSim(puk, pin)==PhoneConstants.PIN_RESULT_SUCCESS)?true:false;
    }
	...
	public static int mRetryCount;//aurora change zhouxiaobing 20140512 for 4.4 build and the 4.4 phone is not have this
    ...
    public int[] supplyPukReportResult(String puk, String pin, int subscription)
    {
		...
		mRetryCount=-1;
        checkSimPuk.start();
        result[0]=checkSimPuk.unlockSim(puk, pin);
		result[1]=mRetryCount;
		return result;
	...
	public int[] supplyPinReportResult(String pin, int subscription)
	{
		...
		mRetryCount=-1;
        checkSimPin.start();
        result[0]=checkSimPin.unlockSim(null, pin);
		result[1]=mRetryCount;
		return result; 
	}//aurora change zhouxiaobing 20140512 for 4.4 build and the 4.4 phone is not have this
    /**
                        ...
                        switch (msg.what) {
                            case SUPPLY_PIN_COMPLETE:
                                ...
                                synchronized (UnlockSim.this) {
                                   ...
                                   mRetryCount=msg.arg1;//aurora change zhouxiaobing 20140512 for 4.4 build and the 4.4 phone is not have this
    ...
    public int enableApnType(String type) {
		// aurora add zhouxiaobing 20140508 start
		if (type != null && (type.equalsIgnoreCase("SIMORUSIM1") || type.equalsIgnoreCase("SIMORUSIM2"))) {
			int sub = Integer.valueOf(type.substring(type.length() - 1)) - 1;
			Log.v("AuroraContactImportExportActivity", "sub =" + sub);
			try {
				if(!DeviceUtils.isIUNI()) {
					...
					Object o = method.invoke(((PhoneBase) (((PhoneProxy) getPhone(sub)).getActivePhone())));
					String s = o.toString();
					Log.v("AuroraContactImportExportActivity", "s1=" + s);		
					...
					int s =  ((PhoneBase) (((PhoneProxy) getPhone(sub)).getActivePhone())).getCurrentUiccAppType().ordinal();
					Log.v("AuroraContactImportExportActivity", "s1=" + s);	
					return s;
				}		
				...
				e.printStackTrace();
				Log.v("AuroraContactImportExportActivity", "e=" + e.toString());
			}
			return 0;
		} else if (type != null && type.equalsIgnoreCase("SIM2APN")) {
			enforceModifyPermission();
			int result = PhoneConstants.APN_REQUEST_FAILED;
			result = getPhone(1).enableApnType(PhoneConstants.APN_TYPE_DEFAULT);
			return result;
		} else {
			// aurora add zhouxiaobing 20140508 end
			enforceModifyPermission();
			int result = PhoneConstants.APN_REQUEST_FAILED;
			int numPhones = MSimTelephonyManager.getDefault().getPhoneCount();
			int dds = ((MSimPhoneGlobals) mApp).getDataSubscription();
			Log.d(LOG_TAG, "enableApnType dds is " + dds + "type= " + type);
			for (int i = 0; i < numPhones; i++) {
				// aurora add liguangyu 20141013 for #9032 start
				if (!DeviceUtils.isSupportDualData()
						&& i == 1
						&& type.equalsIgnoreCase(PhoneConstants.APN_TYPE_DEFAULT)) {
					continue;
				}
				// aurora add liguangyu 20141013 for #9032 end
    ...
    public int disableApnType(String type) {
    	if(type!=null&&type.equalsIgnoreCase("SIM2APN"))
    	{
    		enforceModifyPermission();
    		int result = PhoneConstants.APN_REQUEST_FAILED;
    		result=getPhone(1).disableApnType(PhoneConstants.APN_TYPE_DEFAULT);
    		return result;
    	}
    	else
    	{
        ...
        int numPhones = MSimTelephonyManager.getDefault().getPhoneCount();
        int dds = ((MSimPhoneGlobals)mApp).getDataSubscription();
        Log.d(LOG_TAG, "disableApnType dds is " + dds +  "type= " + type);
        for (int i = 0; i < numPhones; i++) {
    	...
    	//aurora add liguangyu 20141013 for #9032 start
        if(!DeviceUtils.isSupportDualData() && type.equalsIgnoreCase(PhoneConstants.APN_TYPE_MMS)) {
            Log.d(LOG_TAG, "disableApnType setMobileDataEnabled");
            mMainThreadHandler.postDelayed(new Runnable() {
            	public void run(){
                	boolean isUserDataEnabledSlot = false;
                	try {
                         isUserDataEnabledSlot = android.provider.Settings.Global.getInt(
                        		 mPhone.getContext().getContentResolver(), android.provider.Settings.Global.MOBILE_DATA + 0 , 0) == 1;
                 		 Log.d(LOG_TAG, "disableApnType isUserDataEnabledSlot =" + isUserDataEnabledSlot);
                	} catch (Exception e) {
                    	e.printStackTrace();
                    }
    	 		    ConnectivityManager cm =
    	 		            (ConnectivityManager)PhoneGlobals.getInstance().getSystemService(Context.CONNECTIVITY_SERVICE);
    	 		    cm.setMobileDataEnabled(isUserDataEnabledSlot);
            		
            	}
            }, 3000);
        } else if(type.equalsIgnoreCase(PhoneConstants.APN_TYPE_MMS)) {
            mMainThreadHandler.postDelayed(new Runnable() {
            	public void run(){
                	boolean isUserDataEnabledSlot = false;
                	try {
                         boolean isUserDataEnabledSlot0 = android.provider.Settings.Global.getInt(mPhone.getContext().getContentResolver(), android.provider.Settings.Global.MOBILE_DATA + 0 , 0) == 1;
                         boolean isUserDataEnabledSlot1 = android.provider.Settings.Global.getInt(mPhone.getContext().getContentResolver(), android.provider.Settings.Global.MOBILE_DATA + 1 , 0) == 1;
                         isUserDataEnabledSlot = isUserDataEnabledSlot0 || isUserDataEnabledSlot1;
                 		 Log.d(LOG_TAG, "disableApnType isUserDataEnabledSlot =" + isUserDataEnabledSlot);
                	} catch (Exception e) {
                    	e.printStackTrace();
                    }
