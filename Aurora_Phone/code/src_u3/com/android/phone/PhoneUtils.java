    ...
    /* package */ static boolean answerCall(Call ringingCall) {
        ...
        if (ringingCall != null && ringingCall.isRinging()) {
            ...
            try {
                ...
                handleWaitingCallOnLchSub(getSubscription(phone), true);
...
//                setAudioMode();
    ...
    static boolean hangupRingingCall(Call ringing) {
        ...
        if (state == Call.State.INCOMING) {
            ...
            if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                ...
                handleWaitingCallOnLchSub(getSubscription(ringing.getPhone()), false);
        ...
        try {
            if (c != null) {
                if (c.getCall().getPhone().getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA &&
                    ...
                    handleWaitingCallOnLchSub(getSubscription(c.getCall().getPhone()), false);
                }
                c.hangup();
            }
        } catch (CallStateException ex) {
    ...
    public static int placeCall(Context context, Phone phone,
        ...
        if (null == connection) {
            ...
            setActiveSubscription(getSubscription(phone));
            if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
    ...
    static void setMute(boolean muted) {
        ...
        if (cm.getLocalCallHoldStatus(activeSub) == true) {
            if (muted == false) {
                ...
                //cm.deactivateLchState(activeSub);//aurora change zhouxiaobing 20140512 for 4.4 build and the 4.4 phone is not have this
                cm.setAudioMode();
            }
        ...
        if(isMultiSimEnabled()) {
         	 try {
                 int simstate = android.provider.Settings.Global.getInt(PhoneGlobals.getInstance().getContentResolver(),
           		   "mobile_data"+ 2);            
                 Log.d(LOG_TAG, "updateCardState restore simstate= " + simstate);
                 if(simstate == 2 && subscription == 0) {
                	 subscription = 1;
                 } else if(simstate == 1 && subscription == 1) {
                	 subscription = 0;
                 }
             } catch (Exception e) {
          	    e.printStackTrace();
...
//        context.startService(intent);
    ...
    public static void setActiveSubscription(int subscription) {    	
        ...
        Log.d(LOG_TAG, "setActiveSubscription: sub = " + subscription);
        CallManager cm = PhoneGlobals.getInstance().mCM;
        int activeSub = getActiveSubscription();
    	...
    	if(isMultiSimEnabled()) {    	    
            ...
            for (int i = 0; i < count; i++) {
                if ((i != subscription) && (cm.getState(i) != PhoneConstants.State.IDLE)) {
                    ...
                    //cm.deactivateLchState(i);//aurora change zhouxiaobing 20140512 for 4.4 build and the 4.4 phone is not have this
...
//	                setMute(true);
	...
	public static String getNetworkTitle(OperatorInfo ni) {
		if(DeviceUtils.isIUNI()) {
		    String[] mNetworkNums;
		    String[] mNetworkNames;
			
	        mNetworkNums = PhoneGlobals.getInstance().getResources().getStringArray(R.array.aurora_operator_number);
	        mNetworkNames = PhoneGlobals.getInstance().getResources().getStringArray(R.array.aurora_operator_name);
		    
			String title = "";
			String opratorNum = ni.getOperatorNumeric();
			for (int i = 0; i < mNetworkNums.length; i++) {
				if (opratorNum.equalsIgnoreCase(mNetworkNums[i])) {
					title = mNetworkNames[i];
					break;
				}
			}
			...
			String nettype = ni.getRadioTech();
			if(!TextUtils.isEmpty(nettype)) {
				return title + convertNetTypeToName(Integer.valueOf(nettype));	
			} else {
				return title;
			}
		} else {
    ...
    private static String convertNetTypeToName(int nettype) {
    	switch(nettype) {
    ...
    public static void setPreferredDataSubscription(int subscription) {
        if (isMultiSimEnabled()) {
			...
			if (subManager != null) {
				try {
					Log.w(LOG_TAG, "setPreferredDataSubscription");
					subManager.setDataSubscription(subscription, null);
				} catch (Exception e) {
					Log.w(LOG_TAG, "CMD_SET_DATA_SUBSCRIPTION setDataSubscription false " + e);
				}
			}
        }
    }
    ...
    public static String getOperator(int slot) {
        if (isMultiSimEnabled()) {
        	return MSimTelephonyManager.getDefault().getNetworkOperator(slot);
        } else {
