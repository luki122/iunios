...
//            mMostRecentlyUsedMode = newMode;
    ...
    private static int audioModeUpdateDualModem(int mode) {
        Log.d(LOG_TAG, "audioModeUpdateDualModem, mode:" + mode + ", phonecount:" + getPhoneCount());
        ...
        if (isDualtalk()) {
            ...
            if (hasActiveCall) {
                if (mode == AudioManager.MODE_IN_CALL) {
                    int phoneId = getSubscription(call.getPhone());
                    if (phoneId == SubscriptionManager.INVALID_PHONE_INDEX) {
                        Log.d(LOG_TAG, "At this point, we can't get the exactly phone id, only waiting the next time...");
