...
//        pointTweener.animator.setRepeatCount(ValueAnimator.INFINITE);
    ...
    private void handleBgAnim(MotionEvent event) {
         ...
         if(!mDragging) {
//         	AuroraCallCardAnimationController.getInstance().resetPhotoRotateAnimation();
    		if(mBackgroundView != null && mBackgroundView.is_anim_start2) {
        		mBackgroundView.stopDisplayAnim2();
        	}
        	 return;
         }
...
//                    Log.v(TAG, "per = " + per);
//                	AuroraCallCardAnimationController.getInstance().setPhototRotateAnimation(per);                	
                }
                 break;
...
//        mOuterRing.draw(canvas);
    ...
    public void setInCallScreen(InCallScreen c) {
	       mInCallScreen = c;
	        if (c != null) {	  
	        	mBackgroundView = (BackgroundView) c.findViewById(com.android.phone.R.id.background);
