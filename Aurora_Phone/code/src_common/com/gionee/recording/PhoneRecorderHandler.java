...
//        }
    ...
    private void checkRecordDisk() {
        // Gionee fangbin 20121114 added for CR00728364 start
//        if (PhoneApp.ISGNPHONE) {
            if (!GnPhoneRecordHelper.diskSpaceAvailable(Constants.PHONE_RECORD_LOW_STORAGE_THRESHOLD)) {
                ...
                if (PhoneRecorder.isRecording() || isVTRecording()) {
                    if(GnPhoneRecordHelper.getDiskAvailableSize() != 0){
                        Toast.makeText(PhoneGlobals.getInstance(), PhoneGlobals.getInstance().getResources().getString(R.string.gn_vt_recording_saved_sd_full), Toast.LENGTH_SHORT).show();
                    }
...
//        } else {
//        // Gionee fangbin 20121114 added for CR00728364 end
//            if (!GnPhoneRecordHelper.diskSpaceAvailable(mRecordStoragePath, Constants.PHONE_RECORD_LOW_STORAGE_THRESHOLD)) {
//                Log.e("AN: ", "Checking result, disk is full, stop recording...");
//                if (PhoneRecorder.isRecording() || isVTRecording()) {
//                    if (PhoneRecorder.isRecording()) {
//                        stopVoiceRecord();
