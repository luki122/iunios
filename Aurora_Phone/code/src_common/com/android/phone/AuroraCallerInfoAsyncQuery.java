 ...
 * {@hide}
 */
public class AuroraCallerInfoAsyncQuery {
    private static final boolean DBG = false;
    private static final String LOG_TAG = "AuroraCallerInfoAsyncQuery";
    ...
    private static final int EVENT_AURORA_UPDATE = 6;
    ...
    private class CallerInfoAsyncQueryHandler extends AsyncQueryHandler {
        ...
        private AuroraCallerInfo mCallerInfo;
            ...
            public void handleMessage(Message msg) {
                ...
                if (cw == null) {
                    ...
                    switch (cw.event) {
                    	...
                    	case EVENT_AURORA_UPDATE:
                    		mCallerInfo.auroraUpdate(mQueryContext);
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            if(cw.event == EVENT_AURORA_UPDATE) {
                ...
                if (cw.event == EVENT_EMERGENCY_NUMBER) {
                    ...
                    mCallerInfo = new AuroraCallerInfo().markAsEmergency(mQueryContext);
                } else if (cw.event == EVENT_VOICEMAIL_NUMBER) {
                    mCallerInfo = new AuroraCallerInfo().markAsVoiceMail();
                } else {
                    mCallerInfo = AuroraCallerInfo.getCallerInfo(mQueryContext, mQueryUri, cursor);
                    if (DBG) Log.d(LOG_TAG, "==> Got mCallerInfo: " + mCallerInfo);

                    AuroraCallerInfo newCallerInfo = AuroraCallerInfo.doSecondaryLookupIfNecessary(
...
//                                AuroraCallerInfo.getCurrentCountryIso(mQueryContext));
                      mCallerInfo.phoneNumber = cw.number;
                    }
                    
    				CookieWrapper auroraMarker = new CookieWrapper();
    				auroraMarker.event = EVENT_AURORA_UPDATE;
    				auroraMarker.listener = cw.listener;
    				auroraMarker.cookie = cw.cookie;
    				auroraMarker.number = cw.number;
    				startQuery(token, auroraMarker, null, null, null, null, null);
    				return;
                }
     ...
     */
    private AuroraCallerInfoAsyncQuery() {
    }
     ...
     */
    public static AuroraCallerInfoAsyncQuery startQuery(int token, Context context, Uri contactRef,
            OnQueryCompleteListener listener, Object cookie) {

    	AuroraCallerInfoAsyncQuery c = new AuroraCallerInfoAsyncQuery();
        ...
        c.mHandler.startQuery(token, cw, contactRef, null, null, null, PRIVATE_SQL_ORDER);
     ...
     */
    public static AuroraCallerInfoAsyncQuery startQuery(int token, Context context, String number,
            OnQueryCompleteListener listener, Object cookie) {
        if (DBG) {
            Log.d(LOG_TAG, "##### AuroraCallerInfoAsyncQuery startQuery()... #####");
