...
public class PermissionAppDetailAdapter extends BaseExpandableListAdapter {
    ...
    public PermissionAppDetailAdapter(Context context) {
        mContext = context;
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView,
        ...
        button1.setOnClickListener(new Button.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mStateList[position] != PermissionsInfo.STATUS_PERMIT) {
                    mStateList[position] = PermissionsInfo.STATUS_PERMIT;
                    if (PermissionsInfo.getInstance().updatePermission(mContext, mPermissions.get(position),
                            mPackageName, mStateList[position])) {
        ...
        button2.setOnClickListener(new Button.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mStateList[position] != PermissionsInfo.STATUS_REQUEST) {
                    mStateList[position] = PermissionsInfo.STATUS_REQUEST;
                    if (PermissionsInfo.getInstance().updatePermission(mContext, mPermissions.get(position),
                            mPackageName, mStateList[position])) {
                        notifyDataSetChanged();
                        setchecked();
        ...
        button3.setOnClickListener(new Button.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mStateList[position] != PermissionsInfo.STATUS_PROHIBITION) {
                    mStateList[position] = PermissionsInfo.STATUS_PROHIBITION;
                    if (PermissionsInfo.getInstance().updatePermission(mContext, mPermissions.get(position),
                            mPackageName, mStateList[position])) {
                        notifyDataSetChanged();
                        setchecked();
    ...
    public Object getGroup(int groupPosition) {
        // TODO Auto-generated method stub
        return mPkgPermLable.get(groupPosition);
    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        info = getTitle(mPermissions.get(groupPosition));
        int permissionSort = PermissionsInfo.getInstance().mPerMissionMap
                .get(mPermissions.get(groupPosition));
        /*Drawable icon = mContext.getResources().getDrawable(
                PermissionsInfo.getInstance().DRAWABLE_ID[permissionSort]);*/
        if (info != null) {
            ...
            int state = mStateList[groupPosition];
            int important = mImportantList[groupPosition];
            if (state == 1) {
                viewHolder.mSummary.setText(R.string.text_allow);
            } else if (state == 0) {
                viewHolder.mSummary.setText(R.string.text_ask);

            } else {
                viewHolder.mSummary.setText(R.string.text_forbidden);
            }
    ...
    private void queryDB(String pkgname) {
        ...
        String[] projection = new String[] {"permission", "packagename", "status"};
        Cursor cursor = mContext.getContentResolver().query(Uri.parse(PermissionsInfo.URI), projection,
                "packagename = ?", new String[] {pkgname}, null);
    ...
    public void operatePermissionDB(boolean trust) {
        ...
        PermissionsInfo.getInstance().updatePermission(mContext, cv, mPackageName);
        if (PermissionSettingAdapter.PARTIAL_TRUST.equalsIgnoreCase(SystemProperties
                .get("persist.sys.permission.level"))) {
            if (!trust) {
                cv.put("status", 1);
                PermissionsInfo.getInstance().updatePermissionInPartialTrust(mContext, cv, mPackageName);
            }
        }
        notifyDataSetChanged();
    }

   /* private int getDrawable(String permission) {
        int drawableId = 0;
        ...
        return PermissionsInfo.DRAWABLE_ID[drawableId];
    }*/

    private String getTitle(String permission) {
        String title = null;
        String[] titles = mContext.getResources().getStringArray(R.array.ui_privacy_policy);
        for (int i = 0; i < PermissionsInfo.PERMISSIONS.length; i++) {
            if (permission.equalsIgnoreCase(PermissionsInfo.PERMISSIONS[i])) {
                title = titles[i];
                break;
    ...
    private String getPkgPermLabel(final String permission) {
        ...
        if (permission.contains("WRITE_SMS_MMS")) {
            return mContext.getResources().getString(R.string.permlab_writeMms);
        }
        if (permission.contains("READ_SMS_MMS")) {
            return mContext.getResources().getString(R.string.permlab_readMms);
        }
        if (permission.contains("SEND_SMS_MMS")) {
            return mContext.getResources().getString(R.string.perm_send_mms);
        }
        ...
        if (null != mPm) {
            try {
                ...
                if (tempGrpName.equals("DefaultGrp")) {
                    tempGrpName = mContext.getResources().getString(R.string.perm_default);
                }
            }
            if (null != pgi) {
