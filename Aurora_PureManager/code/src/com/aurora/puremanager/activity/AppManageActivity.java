...
public class AppManageActivity extends AuroraActivity implements Observer,
	...
	private HashSet<String> mFreezedApps;
	...
	private void loadAppList() {
		...
		mFreezedApps = FreezedAppProvider
    ...
    private final Handler updateDefSoftViewHandler = new Handler() {
	    ...
	    public void handleMessage(Message msg) {
	    	...
	    	((MainActivityItemData)itemList.get(3)).setVilibileFlag(View.VISIBLE);
			((MainActivityItemData)itemList.get(3)).setHintStrFront(""+
					DefSoftModel.getInstance(AppManageActivity.this).getNotSetDefSoftNum());
	...
	private void initData() {
		...
		itemData.setComponentName(new ComponentName(this,AutoStartActivity.class));
		...
		itemData.setItemName(getString(R.string.app_uninstall));
		 ...
		 int UserApp = ConfigModel.getInstance(this).getAppInfoModel()
					.getUserAppsNum();
		itemData.setHintStrTail("已安装" + String.valueOf(UserApp) + "款应用");
		itemData.setHintStrFront("0");
		itemList.add(itemData);
		...
		int freezed = mFreezedApps.size();
		itemData.setHintStrTail("已冻结" + String.valueOf(freezed) + "个应用");
		itemData.setHintStrFront("0");
		itemList.add(itemData);
