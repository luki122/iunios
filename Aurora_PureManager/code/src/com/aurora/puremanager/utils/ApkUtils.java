...
//    }
    ...
    public static synchronized Drawable getApkIcon(Context context, String packageName) {
        Drawable icon = null;
        try {
            icon = Utils2Icon.getInstance(context).getIconDrawable(
                    packageName,
                    Utils2Icon.INTER_SHADOW);
        } catch (Exception e) {
            //return UsageSummaryFragment.iconForDetail;
            icon = null;
        }
        if (icon != null) {
            return icon;
...
//    	} 
     ...
     * @param context
     * @param tmpCpName
     * @param targetState (1)PackageManager.COMPONENT_ENABLED_STATE_DEFAULT
