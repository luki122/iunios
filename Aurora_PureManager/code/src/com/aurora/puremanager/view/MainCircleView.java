...
public class MainCircleView extends View {
	...
	public MainCircleView(final Context context, AttributeSet attrs) {
		super(context, attrs);
		mPaint = new Paint();
		StrokeWidth = context.getResources().getDimension(
				R.dimen.main_circle_view_stroke_width);
		diameter = context.getResources().getDimension(
				R.dimen.main_circle_view_diameter);
	}
	...
	public void init(int userAppNum, int sysAppNum) {
		...
		circleRect = new RectF(StrokeWidth / 2, StrokeWidth / 2,
				canvasRect.width() - StrokeWidth / 2, canvasRect.width()
						- StrokeWidth / 2);
		userAppTotalAngles = (360 - spaceAngles) * userAppNum
				/ (userAppNum + sysAppNum);
	...
	public void updateViewWhenAppNumChange(int userAppNum, int sysAppNum) {
		...
		Log.i("zhangwei", "the userAppTotalAngles=" + userAppTotalAngles
				+ " the sysAppTotalAngles=" + sysAppTotalAngles);
		Log.i("zhangwei", "the curSweepAngleOfUseApp=" + curSweepAngleOfUseApp
				+ " the curSweepAngleOfSysApp=" + curSweepAngleOfSysApp);
		postInvalidate();
	}
	...
	private Bitmap createProessBitmap() {
		...
		canvas.drawArc(circleRect, startAngleOfUseApp, curSweepAngleOfUseApp,
				false, paint);

		paint.setColor(Color.parseColor("#59bdef"));
		canvas.drawArc(circleRect, startAngleOfSysApp, curSweepAngleOfSysApp,
				false, paint);
