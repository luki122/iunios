...
public class DownloadInfo {
	...
	// Gionee <wangpf> <2013-07-02> modify for CR00832279 begin
	private static final String GN_ACTION_DOWNLOAD_COMPLETE = "gn.android.intent.action.DOWNLOAD_COMPLETE";

	// Gionee <wangpf> <2013-07-02> modify for CR00832279 end
	...
	public static class Reader {
		...
		public DownloadInfo newDownloadInfo(Context context,
				SystemFacade systemFacade, StorageManager storageManager,
				DownloadNotifier notifier) {
			final DownloadInfo info = new DownloadInfo(context, systemFacade,
					storageManager, notifier);
		...
		public void updateFromDatabase(DownloadInfo info) {
			...
			info.mPriorityLevel = getInt(DownloadConstants.DownloadsColum.COLUMN_PRIORITY_LEVEL);
			info.mLastChangePriorityTime = getInt(DownloadConstants.DownloadsColum.COLUMN_LAST_CHANGE_PRIORITY_TIME);
			info.mLastRunningTime = getInt(DownloadConstants.DownloadsColum.COLUMN_LAST_RUNNING_TIME);
			info.mStorage = getInt(DownloadConstants.DownloadsColum.COLUMN_STORAGE);
			// Gionee <wangpf> <2013-08-01> modify for CR00834830 begin
			info.mAddTaskTime = getLong(DownloadConstants.DownloadsColum.COLUMN_ADD_TASK_TIME);
			// Gionee <wangpf> <2013-08-01> modify for CR00834830 end
			// Gionee <duansw><2013-2-22> modify for CR00789329 end
			
			//add by JXH 2014-7-24 begin
			info.mIsVisibleInDownloadUi=getInt(Downloads.Impl.COLUMN_IS_VISIBLE_IN_DOWNLOADS_UI)!=0;
	...
	//add by JXH 2014-7-24 end

	public int mFuzz;

	// Gionee <duansw><2013-2-22> modify for CR00789329 begin
	public int mPriorityLevel;
	public int mLastChangePriorityTime;
	public int mLastRunningTime;
	// Gionee <wangpf> <2013-08-01> modify for CR00834830 begin
	public long mAddTaskTime;
	// Gionee <wangpf> <2013-08-01> modify for CR00834830 end
	...
	//add by JXH 2014-7-23 begin
	public boolean isActivityNetwork(DownloadInfo mInfo) {
	...
	// Gionee <wangpf> <2013-07-02> modify for CR00832279 begin
	private Intent getCompleteIntent() {
		if (mStatus == Downloads.Impl.STATUS_RUNNING
				|| mStatus == Downloads.Impl.STATUS_CANCELED) {
			return new Intent(DownloadManager.ACTION_DOWNLOAD_COMPLETE);
		}
		return new Intent(GN_ACTION_DOWNLOAD_COMPLETE);
	}
		...
		case Downloads.Impl.STATUS_DEVICE_NOT_FOUND_ERROR:
			...
			return Environment.getExternalStorageState().equals(
					Environment.MEDIA_MOUNTED); // &&
												// GnStorage.getStorage(mStorage).exist();
		...
		synchronized (this) {
			...
			if (isReady && !isActive) {
				if (mStatus != Impl.STATUS_RUNNING) {
					...
					if (executor.getActiveCount() < DownloadService.MAX_CONCURRENT) {
						values.put(Impl.COLUMN_STATUS, Impl.STATUS_RUNNING);
						mContext.getContentResolver().update(
								getAllDownloadsUri(), values, null, null);
					} else {
				...
				mTask = new DownloadThread(mContext, mSystemFacade, this,
						mStorageManager, mNotifier);
	...
	public boolean shouldScanFile() {
		return (mMediaScanned == 0)
				&& (mDestination == Downloads.Impl.DESTINATION_EXTERNAL
						|| mDestination == Downloads.Impl.DESTINATION_FILE_URI || mDestination == Downloads.Impl.DESTINATION_NON_DOWNLOADMANAGER_DOWNLOAD)
