...
public class DownloadReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, final Intent intent) {
		...
		if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
			...
			if (Constants.LOGVV) {
				Log.v(Constants.TAG, "CONNECTIVITY_ACTION");
			}
			...
			if (Constants.LOGVV) {
				Log.v(Constants.TAG, "Received broadcast intent for  " + action);
			}
			// this intent is used to resume paused download, only need to start
			// service
			startService(context);
		...
		}else if (action.equals(Constants.ACTION_COMPLETE)) {
			DownloadNotifier.isCancleCompleteNofi= intent.getBooleanExtra("hid", true);
			startService(context);
		}
	}
	...
	private void openDownload(Context context, long id) {
		...
		try {
			...
			Log.d(Constants.TAG, "no activity for " + intent);
	...
	private void sendNotificationClickedIntent(Context context, long[] ids) {
		...
		if(Constants.LOGV){
			Log.d(TAG, "appIntent=="+appIntent.toString());
		}
