...
public final class OnePaneController extends AbstractActivityController {
    ...
    protected OnClickListener mToggleDrawerListener = new OnClickListener() {
    	@Override
    	public void onClick(View v) {
    		if(v.getId() == R.id.drawer_toggle_button || 
    			v.getId() == R.id.aurora_actionbar_custom_view) {
    			if(mDrawerListener.getDrawerState() == DrawerLayout.STATE_IDLE) {
    				toggleDrawerState();
    			}
    		}
    	}
    ...
    private static boolean isDefaultInbox(FolderUri candidate, Account account) {
    	//Log.i("SQF_LOG", "OnePaneController::isDefaultInbox");
    ...
    private static boolean inInbox(final Account account, final ConversationListContext context) {
    	//Log.i("SQF_LOG", "OnePaneController::inInbox");
    ...
    public void changeAccount(Account account) {
    	//Log.i("SQF_LOG", "OnePaneController::changeAccount");
    ...
    public boolean onCreate(Bundle savedInstanceState) {
    	//Log.i("SQF_LOG", "OnePaneController::onCreate");
        //Aurora <SQF> <2014-10-21>  for NEW_UI begin
        //ORIGINALLY:
    	//mActivity.setContentView(R.layout.one_pane_activity);
        //SQF MODIFIED TO:
//    	mActivity.setAuroraContentView(R.layout.one_pane_activity, AuroraActionBar.Type.Custom, true);
    	mActivity.setAuroraPicContentView(R.layout.one_pane_activity);//modify by shihao
    	
    	mActivity.addSearchviewInwindowLayout();
		...
		boolean launchConversationMode = intent.getBooleanExtra("launchConversationMode", false);
		if (action != null && action.equals(Intent.ACTION_VIEW) && 
				launchConversationMode) {
			mActivity.getContentView().post(new Runnable() {
				
				@Override
				public void run() {
					// TODO Auto-generated method stub
					mActivity.getAuroraActionBarManager().setMode(AuroraActionBarManager.Mode.DETAIL_MODE);
				}
			});
		}
        ...
        /*mDrawerPullout.setBackgroundResource(R.color.aurora_drawlayout_background_grey);*/
        mDrawerPullout.setBackgroundResource(R.drawable.aurora_drawlayout_background);
        mActivity.getAuroraActionBarManager().setOnClickListener(mToggleDrawerListener);
        mActivity.getAuroraActionBarManager().setDetailCallBack(mDetailCallBack);//paul add
    ...
    public void onViewModeChanged(int newMode) {
        ...
        //Log.i("SQF_LOG", "OnePaneController::onViewModeChanged");
    ...
    protected void showConversation(Conversation conversation, boolean inLoaderCallbacks) {
    	//((AuroraActivity)mActivity).hideSearchviewLayout();//paul add  //cjs delete
        super.showConversation(conversation, inLoaderCallbacks);
        //Log.i("SQF_LOG", "OnePaneController::showConversation");
		...
		//paul add start
        mActivity.getAuroraActionBarManager().trytoSetDetailMode();
		//paul add end
    ...
    protected void hideWaitForInitialization() {
    	//Log.i("SQF_LOG", "OnePaneController::hideWaitForInitialization");
        transitionToInbox();
    ...
    public boolean doesActionChangeConversationListVisibility(final int action) {
    	//Log.i("SQF_LOG", "OnePaneController::doesActionChangeConversationListVisibility");
    ...
    private int replaceFragment(Fragment fragment, int transition, String tag, int anchor) {
    	//Log.i("SQF_LOG", "OnePaneController::replaceFragment");
        final FragmentManager fm = mActivity.getFragmentManager();
    ...
    public boolean handleBackPress() {
        ...
        if (mode == ViewMode.SEARCH_RESULTS_LIST) {
        	...
        	if(mViewMode.isConversationMode()) {
        		setIsBackFromConversationMode(true);
        	}
        	//Aurora <SQF> <2014-12-09>  for NEW_UI end
            transitionBackToConversationListMode();
        } else {
    ...
    private void transitionToInbox() {
    	//Log.i("SQF_LOG", "OnePaneController::transitionToInbox");
    ...
    public void onFolderSelected(Folder folder) {
    	//Log.i("SQF_LOG", "OnePaneController::onFolderSelected");
    ...
    public boolean handleUpPress() {
    	//Log.i("SQF_LOG", "OnePaneController::handleUpPress");
        final int mode = mViewMode.getMode();
        if (mode == ViewMode.SEARCH_RESULTS_LIST) {
            mActivity.finish();
    ...
    private void transitionBackToConversationListMode() {
    	//Log.i("SQF_LOG", "OnePaneController::transitionBackToConversationListMode");
        final int mode = mViewMode.getMode();
        enableCabMode();
        mConversationListVisible = true;
        if (mode == ViewMode.SEARCH_RESULTS_CONVERSATION) {
            ...
            //Aurora <SQF> <2014-11-29>  for ANNOTATION begin
            /*
             * when back to ConversationList from Conversation Mode, may call "enterConversationListMode" twice.  
             * one in mViewMode.enterConversationListMode();
             * the other in onFolderChanged(folder, force = true);
             * so we annotate this line below.
            */
        	//mViewMode.enterConversationListMode();
            //Aurora <SQF> <2014-11-29>  for ANNOTATION end
    ...
    public void onUndoAvailable(ToastBarOperation op) {
    	//Log.i("SQF_LOG", "OnePaneController::onUndoAvailable");
    ...
    public void launchFragment(final Fragment fragment, final int selectPosition) {
    	//Log.i("SQF_LOG", "OnePaneController::launchFragment");
	...
	private AuroraMenuBase.OnAuroraMenuItemClickListener mDetailCallBack = new AuroraMenuBase.OnAuroraMenuItemClickListener() {
		@Override
		public void auroraMenuItemClick(int itemId) {
			switch (itemId) {
			case R.id.aurora_detail_menu_item_del:
				deleteCurrentMessage();
				break;
			case R.id.aurora_detail_menu_item_reply:
			case R.id.aurora_detail_menu_item_replyall:
			case R.id.aurora_detail_menu_item_forward:
