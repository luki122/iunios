...
public class ConversationCursorLoader extends AsyncTaskLoader<ConversationCursor> {
	...
	private String mSelection = null;//paul add
	...
	//paul add start
    public ConversationCursorLoader(Activity activity, Account account, Uri uri, String name, String selection) {
        super(activity);
        mUri = uri;
        mName = name;
        mInitialConversationLimit =
    ...
    public ConversationCursor loadInBackground() {
        if (!mInit) {
			mConversationCursor.setSelection(mSelection);//paul add
