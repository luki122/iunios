...
public class FolderItemView extends RelativeLayout {
    ...
    private ImageView mListDividerHead;
    private ImageView mListDividerEnd;
    /**
    ...
    protected void onFinishInflate() {
        ...
        mListDividerHead = (ImageView) findViewById(R.id.folder_list_divider_head);
        mListDividerEnd = (ImageView) findViewById(R.id.folder_list_divider_end);
    }
    ...
    public void bind(final Folder folder, final DropHandler dropHandler,
            final BidiFormatter bidiFormatter, final boolean isLastFolder) {
        mFolder = folder;
        mDropHandler = dropHandler;

        mFolderTextView.setText(bidiFormatter.unicodeWrap(folder.name));
        //Aurora <shihao> <2014-11-16> for DrawLayout User_folder textColor begin
		if(folder.getTypeDescription().equalsIgnoreCase("user_folder")){
			mFolderTextView.setTextColor(getResources().getColor(R.color.aurora_user_folder_textcolor));
			mListDividerHead.setBackgroundResource(R.drawable.aurora_listview_divider_virtual_bitmap);
			if(isLastFolder){
				//mListDividerEnd.setVisibility(View.VISIBLE);  //cjs delete
				mListDividerEnd.setBackgroundResource(R.drawable.aurora_listview_divider_virtual_bitmap);
			}else
				mListDividerEnd.setVisibility(View.GONE);
		}else{			
			mFolderTextView.setTextColor(getResources().getColor(R.color.aurora_local_folder_textcolor));
			mListDividerHead.setBackgroundResource(R.color.aurora_border_gradient_color);
			if(isLastFolder){
				//mListDividerEnd.setVisibility(View.VISIBLE);  //cjs delete
				mListDividerEnd.setBackgroundResource(R.color.aurora_border_gradient_color);
			}else
				mListDividerEnd.setVisibility(View.GONE);
		}
		//Aurora <shihao> <2014-11-16> for DrawLayout User_folder textColor end
...
//        mUnreadCountTextView.setVisibility(count > 0 ? View.VISIBLE : View.GONE);
    	mUnreadCountTextView.setVisibility(View.GONE);//add shihao
        if (count > 0) {
...
//        mUnseenCountTextView.setVisibility(count > 0 ? View.VISIBLE : View.GONE);
    	mUnseenCountTextView.setVisibility(View.GONE);//modify by shihao
        if (count > 0) {
