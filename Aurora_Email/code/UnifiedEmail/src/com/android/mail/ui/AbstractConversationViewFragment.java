...
public abstract class AbstractConversationViewFragment extends Fragment implements
	...
	//paul add start
	public void pageAction(int itemId){
		if(!mCursor.moveToFirst()) return;
		if (R.id.aurora_detail_menu_item_reply == itemId) {
			ComposeActivity.reply(getActivity(), mAccount, mCursor.getMessage());
		} else if (R.id.aurora_detail_menu_item_replyall == itemId) {
			ComposeActivity.replyAll(getActivity(), mAccount, mCursor.getMessage());
		} else if (R.id.aurora_detail_menu_item_forward == itemId) {
			ComposeActivity.forward(getActivity(), mAccount, mCursor.getMessage());
		}
	}
	//paul add end
