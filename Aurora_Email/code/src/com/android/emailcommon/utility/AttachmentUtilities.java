...
//        Log.d("JXH", "saveAttachment.uri:"+uri);
...
//                Log.d("JXH", "saveAttachment.contentUri:"+contentUri);
                cv.put(AttachmentColumns.CONTENT, absolutePath);
            } else {
        ...
        if (contentUri != null && attachment.mContentId != null) {
            ...
            if (body != null && body.mHtmlContent != null) {
                ...
                */
				Pattern contentIdRe =
						  Pattern.compile("\\s+(?i)(src=|background=)\"?cid(?-i):\\Q" + attachment.mContentId + "\\E\"?");
				html = contentIdRe.matcher(html).replaceAll(" $1\""+contentUri + "\"");
				//paul modify end
	...
	//add by chenhl start
	public static boolean copyAttachmentToDownload(Context context,IEmailServiceCallback callback,Attachment attachment){
		Uri uri = ContentUris.withAppendedId(Attachment.CONTENT_URI, attachment.mId);
        ContentValues cv = new ContentValues();
		long attachmentId = attachment.mId;
        long accountId = attachment.mAccountKey;
		...
		try {
			...
			File saveAs = getAttachmentFilename(context, accountId,
	                    attachmentId);

			FileInputStream in = new FileInputStream(saveAs);
			...
			while (-1 != (n = in.read(buffer))) {
				...
				if(attachment.mUiState==UIProvider.AttachmentState.NOT_SAVED){
					callbackSate(attachment,callback,0,EmailServiceStatus.CANCEL);
					return false;
				}
				out.write(buffer, 0, n);
				count += n;
				callbackSate(attachment,callback,count,EmailServiceStatus.IN_PROGRESS);
			}
			in.close();
    		out.flush();
    		out.close();
			...
			cv.put(AttachmentColumns.UI_STATE, UIProvider.AttachmentState.FAILED);
			cv.put(AttachmentColumns.UI_DOWNLOADED_SIZE, 0);
			result =false;
		}

		context.getContentResolver().update(uri, cv, null, null);		
		callbackSate(attachment,callback,0,result?EmailServiceStatus.SUCCESS:EmailServiceStatus.CONNECTION_ERROR);	
		return result;
	}
	private static void callbackSate(Attachment att,IEmailServiceCallback callback,int size,int state){
		if(callback!=null){
			try {
				callback.loadAttachmentStatus(att.mMessageKey,att.mId,state,size);
