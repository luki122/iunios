...
public class ImapService extends Service {
    ...
    private static final String[] domains = {"139.com"};
    /**
    ...
    private final EmailServiceStub mBinder = new EmailServiceStub() {
        ...
        public int searchMessages(long accountId, SearchParams searchParams, long destMailboxId) {
            try {
            	//MyLog.i2("SQF_LOG", "ImapService::EmailServiceStub mBinder searchMessages ---");
    ...
    static void loadUnsyncedMessages(final Context context, final Account account,
        ...
        for (Message message : messages) {
            ...
            Utilities.copyOneMessageToProvider(context, message, account, toMailbox,
                    EmailContent.Message.FLAG_LOADED_COMPLETE, false);//paul modify , for pop3
        }
    }
    ...
    private synchronized static void synchronizeMailboxGeneric(final Context context,
            ...
            throws MessagingException {
    	//MyLog.i2("SQF_LOG", "ImapService::synchronizeMailboxGeneric account:" + account + " mailbox:" + mailbox + 
    	//			" loadMore:" + loadMore + " uiRefresh:" + uiRefresh);
        ...
        //Aurora <shihao> <2014-11-14> for sync DRAFTS begin
//        if (mailbox.mType == Mailbox.TYPE_DRAFTS || mailbox.mType == Mailbox.TYPE_OUTBOX) {
        if (mailbox.mType == Mailbox.TYPE_OUTBOX) {
            return;
        }
         //Aurora <shihao> <2014-11-14> for sync DRAFTS end
        ...
        //Aurora <shihao> <2014-11-14> modify
//        final Folder remoteFolder = remoteStore.getFolder(mailbox.mServerId);
    	...
    	//AUrora <shihao> <2014-11-14> for load Message of folders begin
//        if (mailbox.mType == Mailbox.TYPE_TRASH || mailbox.mType == Mailbox.TYPE_SENT) {   	
    	if (mailbox.mType == Mailbox.TYPE_TRASH || mailbox.mType == Mailbox.TYPE_SENT || mailbox.mType == Mailbox.TYPE_DRAFTS) {
    		boolean isExist = remoteFolder.exists();
            LogUtils.e("shihao","ImapService synchronizeMailboxGeneric  remoteFolder.exists()="+isExist +"  name ="+mailbox.mServerId);
            if (!isExist) {
                ...
                HashMap<Integer, String> imapTypeMap = new HashMap<Integer, String>();
                imapTypeMap = remoteStore.getNamesMap();
                String newMailboxName = new String();
                newMailboxName = imapTypeMap.get(mailbox.mType);
            	LogUtils.w("shihao","ImapService synchronizeMailboxGeneric  newMailboxName="+newMailboxName);
            	if(!TextUtils.isEmpty(newMailboxName)){
            		remoteFolder = remoteStore.getFolder(newMailboxName);
//                	LogUtils.w("shihao","ImapService synchronizeMailboxGeneric remoteFolder.exists()="+remoteFolder.exists());
    	...
    	//AUrora <shihao> <2014-11-14> for load Message of folders end

    	//Aurora <shihao> <2014-11-14> for try to catch Exception when the remoteFolder exist but open failed begin
//        remoteFolder.open(OpenMode.READ_WRITE);
    	try{
            ...
            String newMailboxName = new String();
            newMailboxName = imapTypeMap.get(mailbox.mType);
        	LogUtils.i("shihao","ImapService synchronizeMailboxGeneric  newMailboxName1111="+newMailboxName);
        	if(!TextUtils.isEmpty(newMailboxName)){
        		remoteFolder = remoteStore.getFolder(newMailboxName);
//            	LogUtils.i("shihao","ImapService synchronizeMailboxGeneric  111 remoteFolder.exists()="+remoteFolder.exists());
        		if(!remoteFolder.exists())
        			return;
        		else
        			remoteFolder.open(OpenMode.READ_WRITE);
        ...
        //Aurora <shihao> <2014-11-14> for try to catch Exception when the remoteFolder exist but open failed end
        ...
        //MyLog.i("SQF_LOG", "ImapService::synchronizeMailboxGeneric remoteMessageCount:" + remoteMessageCount);
        ...
        //add by shihao
        final String[] emailParts = account.mDisplayName.split("@");
        final String domain = emailParts[1].trim();
        boolean isNeedChangestartDate = false;
        for(String newDomain : domains){
        	if(domain.equalsIgnoreCase(newDomain)){
        		isNeedChangestartDate = true;
        		break;
        	}
        }
        //end by shihao
		...
		LogUtils.e("shihao","ImapService  additionalMessagesNeeded="+ additionalMessagesNeeded);
        if (additionalMessagesNeeded > 0) {
            ...
            long startDate = 0;
            if(isNeedChangestartDate)
                startDate = endDate - windowIncreaseSize;
            else
            	startDate = endDate - 1;
			//end by shihao
            Message[] additionalMessages = new Message[0];
            Message[] oldMessages = new Message[0]; //add by shihao 
            ...
            while (additionalMessages.length < additionalMessagesNeeded && endDate > 0) {
                ...
                if(additionalMessages.length > 0)
                	oldMessages = additionalMessages.clone();
            }
            LogUtils.e("shihao","ImapService  last endDate="+ endDate);
			 //add by shihao	
            if(additionalMessages.length <= 0)
            	additionalMessages = oldMessages.clone();
...
//            final long queryEndDate = endDate - DateUtils.DAY_IN_MILLIS - DateUtils.HOUR_IN_MILLIS;
         ...
         */
        LogUtils.e("shihao","ImapService saveOrUpdate  unsyncedMessages.size()=="+unsyncedMessages.size() + "  mailbox.mServerId="+mailbox.mServerId  + " remoteMessages.length="+remoteMessages.length);
    ...
    private static void processPendingDeletesSynchronous(Context context, Account account,
    	...
    	//MyLog.i2("SQF_LOG", "processPendingDeletesSynchronous accountIdArgs:" + accountIdArgs);
    ...
    private static boolean processPendingAppend(Context context, Store remoteStore, Mailbox mailbox,
        ...
        if (deleteMessage || updateMessage) {
            ...
            if (deleteMessage) {
                ...
                Log.i("SQF_LOG", "ImapService::processPendingAppend ----> resolver.update SERVER_TIMESTAMP");
            }
        }
    ...
    private static int searchMailboxImpl(final Context context, final long accountId,
    	...
    	//MyLog.i("SQF_LOG", "ImapService::searchMailboxImpl ----");
        ...
        //MyLog.i("SQF_LOG", "ImapService::searchMailboxImpl set Mailbox.UI_SYNC_STATUS  to  UIProvider.SyncStatus.LIVE_QUERY");
