...
/**
    ...
    public void loadAttachment(final IEmailServiceCallback cb, final long attachmentId,
        ...
        try {
			...
			mDownloadsInProgress.put(attachmentId,fp); 
    ...
    public void updateFolderList(long accountId) throws RemoteException {
        ...
        try {
			...
			mContext.getContentResolver().notifyChange(EmailProvider.UIPROVIDER_ALL_ACCOUNTS_NOTIFIER,null);//add by chenhl for bug 10201
            ...
            while (localFolderCursor.moveToNext()) {
                ...
                switch (mailboxType) {
					...
					//Aurora <shihao> <2014-11-17> for add JUNK Mailbox begin
                    case Mailbox.TYPE_JUNK:
					//Aurora <shihao> <2014-11-17> for add JUNK Mailbox end
    ...
    public static void sendMailImpl(Context context, long accountId) {
		...
		if(isSendComplete){
			//Aurora <shihao> <20150209> for data count
			EmailDataCount.updataData(context, EmailDataCount.AURORA_EMAIL_SEND_ID, 1);
			AuroraNotificationUtils.getInstance(context).showSendSuccessNotification(context);
		}else{
			AuroraNotificationUtils.getInstance(context).showSendFailNotification(context);
		}
		
    }
	//modify by chenhl end
	//add by chenhl start
	final ConcurrentHashMap<Long, FetchProfile> mDownloadsInProgress =
        new ConcurrentHashMap<Long, FetchProfile>();

	@Override
	public void cancelAttachment(long attachmentId){
		FetchProfile folder = mDownloadsInProgress.get(attachmentId);
		if(folder!=null){
				folder.setCancle(true);
		}
	}
