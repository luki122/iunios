...
public class EmailServiceUtils {
    ...
    public static void setRemoteServicesLogging(Context context, int debugBits) {
        for (EmailServiceInfo info: getServiceInfoList(context)) {
        	Log.i("SQF_LOG", "EmailServiceUtils::setRemoteServicesLogging info.protocol:" + info.protocol);
            if (info.intentAction != null) {
    ...
    public static AccountManagerFuture<Bundle> setupAccountManagerAccount(final Context context,
        ...
        //---------add by tangjie 2014111/17 start---------//
        AuroraAutoCompleteDBHelper dbHelper = AuroraAutoCompleteDBHelper.getInstance(context);
        dbHelper.setCurrentTableName(account.mEmailAddress.replace("@", "").replace(".", ""));
        //---------add by tangjie 2014111/17 end---------//
