...
public class AttachmentDownloadService extends Service implements Runnable {
    ...
    /*package*/ class DownloadSet extends TreeSet<DownloadRequest> {
        ...
        public synchronized void onChange(Context context, Attachment att) {
            ...
            if (priority == PRIORITY_NONE) {
                ...
                if (req != null) {
					...
					cancelAttachment(req);//add by chenhl
                    remove(req);
                }
            } else {
        ...
        private void startDownload(EmailServiceProxy service, DownloadRequest req)
			...
			mDownloadsService.put(req.attachmentId, service);//add by chenhl
        ...
        private void cancelDownload(DownloadRequest req) {
			...
			mDownloadsService.remove(req.attachmentId);//add by chenhl
        ...
        /*package*/ synchronized void endDownload(long attachmentId, int statusCode) {
			...
			mDownloadsService.remove(attachmentId);//add by chenhl
		...
		//add by chenhl start 
		final ConcurrentHashMap<Long, EmailServiceProxy> mDownloadsService =
		new ConcurrentHashMap<Long, EmailServiceProxy>();
		private void cancelAttachment(DownloadRequest req){

			if(req.inProgress){
				EmailServiceProxy service = mDownloadsService.get(req.attachmentId);
				req.inProgress = false;
		    	mDownloadsInProgress.remove(req.attachmentId);
				mDownloadsService.remove(req.attachmentId);
				if(service!=null){
						service.cancelAttachment(req.attachmentId);
				}
			}
		}
		//add by chenhl end
        ...
        public void loadAttachmentStatus(long messageId, long attachmentId, int statusCode,
            ...
            if (req != null) {
                ...
                Attachment attachment = Attachment.restoreAttachmentWithId(mContext, attachmentId);
                 if (attachment != null  && statusCode == EmailServiceStatus.IN_PROGRESS 
				 	&&attachment.mUiDestination == 1/*add by chenhl*/) {
                    ContentValues values = new ContentValues();
                    /*values.put(AttachmentColumns.UI_DOWNLOADED_SIZE,
                            	attachment.mSize * progress / 100);*/
                    values.put(AttachmentColumns.UI_DOWNLOADED_SIZE,progress);//modify by chenhl
            ...
            switch (statusCode) {
                case EmailServiceStatus.IN_PROGRESS:
                    break;
				//add by chenhl
				case EmailServiceStatus.CANCEL:
						{
							Attachment attachment = Attachment.restoreAttachmentWithId(mContext, attachmentId);
							if(attachment != null){
									ContentValues values = new ContentValues();
									values.put(AttachmentColumns.UI_STATE, AttachmentState.NOT_SAVED);
