...
class ImapFolder extends Folder {
            ...
            do {
                ...
                try {
                    ...
                    if (fp.contains(FetchProfile.Item.ENVELOPE)) {
						...
						*/
						ImapString imap = fetchList.getKeyedStringOrEmpty(
                                ImapConstants.BODY_BRACKET_HEADER, true);
						
                        String header = imap.getString();
						String[] subStr =	header.split("\r\n")[0].split(";");
						byte[] data = null;
						if(2 == subStr.length && subStr[1].equalsIgnoreCase("charset=GB2312")){
							data = imap.getData();
							if(null != data){
								header = new String(data,"GB2312");
							}
						}
                        ...
                        message.setSize(size);
						if(null != data){
							message.parse(new ByteArrayInputStream(data), header);
						}else{
                        	message.parse(Utility.streamFromAsciiString(header));
						}
						//paul modify end
    ...
    private static Body decodeBody(InputStream in, String contentTransferEncoding, int size,
            MessageRetrievalListener listener,FetchProfile fp) throws IOException {
            ...
            while (-1 != (n = in.read(buffer))) {
                ...
                if (listener != null) {
                    ...
                    if (size == 0) {
                        ...
                        listener.loadAttachmentProgress(count);						
                    } else {					
