...
public class EmailProvider extends ContentProvider {
    ...
    private static final String DATABASE_NAME = "Aurora_EmailProvider.db";//paul modify
    private static final String BODY_DATABASE_NAME = "Aurora_EmailProviderBody.db";//paul modify
    private static final String BACKUP_DATABASE_NAME = "Aurora_EmailProviderBackup.db";//paul modify
            ...
            switch (match) {
                ...
                case QUICK_RESPONSE:
                    ...
                    switch(match) {
                        ...
                        case ATTACHMENT:
                            int flags = 0;
							Log.d("chenhl","ATTACHMENT values:"+values);
        ...
        try {
            switch (match) {
                ...
                case UI_ALL_FOLDERS:
                    ...
                    c = uiQuery(match, uri, projection, unseenOnly, selection);//paul modify :  c = uiQuery(match, uri, projection, unseenOnly);
                    return c;
                case UI_FOLDERS:
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        //Aurora <shihao> <20150409> for 5.0(with4.4) because in android5.0 (uri == ACCOUNT_BACKUP_URI)=false throw Exception begin
        ...
        if (uri.toString().equalsIgnoreCase(INTEGRITY_CHECK_URI.toString())) {
            checkDatabases();
            return 0;
        } else if (uri.toString().equalsIgnoreCase(ACCOUNT_BACKUP_URI.toString())) {
            return backupAccounts(getContext(), getDatabase(getContext()));
        }
        //Aurora <shihao> <20150409> for 5.0(with4.4) end
    ...
    private final AttachmentService DEFAULT_ATTACHMENT_SERVICE = new AttachmentService() {
        ...
        public void attachmentChanged(Context context, long id, int flags) {
            ...
            Log.d("chenhl","attachmentChanged......flags:"+flags);
    ...
    private static final String FOLDER_ICON = "CASE " + MailboxColumns.TYPE
            ...
            + " WHEN " + Mailbox.TYPE_JUNK + " THEN " + R.drawable.aurora_junk_mailbox_icon2 //add by shihao
            + " ELSE -1 END";
    ...
    //Aurora <shihao> <2014-11-10> for DrawLayout folder order_by begin
    /**
   ...
   /* private static final String MAILBOX_ORDER_BY = "CASE " + MailboxColumns.TYPE
    ...
    private static final String MAILBOX_ORDER_BY = "CASE " + MailboxColumns.TYPE
        + " WHEN " + Mailbox.TYPE_INBOX   + " THEN 0"
        + " WHEN " + Mailbox.TYPE_STARRED  + " THEN 1"
        + " WHEN " + Mailbox.TYPE_SENT    + " THEN 2"
        + " WHEN " + Mailbox.TYPE_DRAFTS   + " THEN 3"
        + " WHEN " + Mailbox.TYPE_JUNK    + " THEN 4"
        + " WHEN " + Mailbox.TYPE_TRASH   + " THEN 5"
        // Other mailboxes (i.e. of Mailbox.TYPE_MAIL) are shown in alphabetical order.
        + " ELSE 10 END";
//        + " ," + MailboxColumns.DISPLAY_NAME + " COLLATE LOCALIZED ASC";
  //Aurora <shihao> <2014-11-10> for DrawLayout folder order_by end
    ...
    private static ProjectionMap getAttachmentMap() {
        if (sAttachmentMap == null) {
            sAttachmentMap = ProjectionMap.builder()
                ...
                .add(UIProvider.AttachmentColumns.CONTENTID,AttachmentColumns.CONTENT_ID)//add by chenhl
                .build();
        }
...
//                    }
					...
					//paul add
                    if (att.mContentId != null && att.getContentUri() != null) {
                        uiAtt.type = AttachmentType.INLINE_CURRENT_MESSAGE;
                    }
    ...
    private static String genQueryMailboxMessages(String[] uiProjection, final boolean unseenOnly, String selection) {
		...
		if(selection != null) sb.append(" AND " + selection);
    ...
    private static String genQueryAccountMailboxes(String[] uiProjection) {
        ...
        //Aurura <shihao> <2014-11-10> for Drawlayout not diaplay Parent_folder only display subFolder begin
        ...
        sb.append(" FROM " + Mailbox.TABLE_NAME + " WHERE " + MailboxColumns.ACCOUNT_KEY +
                "=? AND " + MailboxColumns.TYPE + " < " + Mailbox.TYPE_NOT_EMAIL +
                " AND " + MailboxColumns.TYPE + " != " + Mailbox.TYPE_SEARCH +
                " AND " + MailboxColumns.TYPE + " != " + Mailbox.TYPE_OUTBOX +
                " AND " + MailboxColumns.SERVER_ID + " NOT IN " + "(select " + Mailbox.PARENT_SERVER_ID + " from " + Mailbox.TABLE_NAME 
                + " where " + MailboxColumns.PARENT_SERVER_ID + " not null" +")" +
        		" ORDER BY ");
      //Aurura <shihao> <2014-11-10> for Drawlayout not diaplay Parent_folder only display subFolder end
    ...
    private static String genQueryAccountAllMailboxes(String[] uiProjection) {
        ...
        sb.append(" FROM " + Mailbox.TABLE_NAME + " WHERE " + MailboxColumns.ACCOUNT_KEY +
                "=? AND " + MailboxColumns.TYPE + " < " + Mailbox.TYPE_NOT_EMAIL +
                " AND " + MailboxColumns.TYPE + " != " + Mailbox.TYPE_SEARCH +
                //Aurora <shihao> <2014-11-18> for filter Parent folder when we choose which folder we can moveTo begin
                " AND " + MailboxColumns.SERVER_ID + " NOT IN " + "(select " + Mailbox.PARENT_SERVER_ID + " from " + Mailbox.TABLE_NAME 
                + " where " + MailboxColumns.PARENT_SERVER_ID + " not null" +")" +
                //Aurora <shihao> <2014-11-18> for filter Parent folder when we choose which folder we can moveTo end
                " ORDER BY h_name");
        return sb.toString();
    }
    ...
    private Object[] getVirtualMailboxRow(long accountId, int mailboxType) {
        ...
        switch (mailboxType) {
            ...
            case Mailbox.TYPE_UNREAD: {
                ...
                //Aurora <shihao> <2014-11-11> for set Mailbox.TYPE_STARRED to Star_folder begin
                values[UIProvider.FOLDER_TYPE_COLUMN] = UIProvider.FolderType.STARRED;
                //Aurora <shihao> <2014-11-11> for set Mailbox.TYPE_STARRED to Star_folder end
                break;
            }
        }
...
//            LogUtils.i("haozi","combinedAccount =="+combinedAccount);
        	//Aurora <shihao> <2014-11-08> for remove "合并视图" begin
/*            if (combinedAccount) {
                addCombinedAccountRow(mc);
            }*/
        	//Aurora <shihao> <2014-11-08> for remove "合并视图" end
        } finally {
    ...
    private Cursor uiFolders(final Uri uri, final String[] uiProjection) {
        ...
        if (id.equals(COMBINED_ACCOUNT_ID_STRING)) {
          ...
          //Aurora <shihao> <2014-11-08> for add Star_folder to the last begin
//            Cursor[] cursors = new Cursor[] {vc, c};
            Cursor[] cursors = new Cursor[] {c, vc};
          //Aurora <shihao> <2014-11-08> for add Star_folder to the last begin
    ...
    private Cursor uiVirtualMailboxes(final String id, final String[] uiProjection) {
        ...
        if (id.equals(COMBINED_ACCOUNT_ID_STRING)) {
        	...
        	//Aurora <shihao> <2014-11-08> for remove  Unread_folder begin
...
//            mc.addRow(getVirtualMailboxRow(acctId, Mailbox.TYPE_UNREAD));
                        ...
                        if (mailbox != null) {
                            ...
                            if (mailbox.mSyncInterval == 0) {
                                ...
                                if (timeSinceLastSync > AUTO_REFRESH_INTERVAL_MS) {
                                	//Log.i("SQF_LOG", "detect last sync and auto sync .....timeSinceLastSync:" + timeSinceLastSync + 
     ...
     */
	 //paul modify : add selection
    private Cursor uiQuery(int match, Uri uri, String[] uiProjection, final boolean unseenOnly, String selection) {
        Context context = getContext();
        ContentResolver resolver = context.getContentResolver();
        SQLiteDatabase db = getDatabase(context);
        ...
        switch(match) {
            ...
            case UI_MESSAGES:
                ...
                if (isVirtualMailbox(mailboxId)) {
                    c = getVirtualMailboxMessagesCursor(db, uiProjection, mailboxId, unseenOnly, selection);
                } else {
                    c = db.rawQuery(
                            genQueryMailboxMessages(uiProjection, unseenOnly, selection), new String[] {id});
                }
...
//                    addCombinedAccountRow(mc);
    ...
    private Uri uiSaveMessage(Message msg, Mailbox mailbox, Bundle extras) {
        ...
        if (!atts.isEmpty()) {
            ...
            msg.mFlagAttachment = 1;//paul modify
    ...
    private int uiUpdateAttachment(Uri uri, ContentValues uiValues) {
		...
		Log.d("chenhl","uiUpdateAttachment stateValue:"+stateValue);
        if (stateValue != null) {
    ...
    private static android.accounts.Account getAccountManagerAccount(final Context context,
        ...
        final EmailServiceInfo info = EmailServiceUtils.getServiceInfo(context, protocol);
		if(null == info) return null;//paul add
    ...
    private static void startSync(final android.accounts.Account account, final long mailboxId,
	    ...
	    //paul add start
        if(null == account){
			LogUtils.e(TAG, "startSync error : null == account");
			return;
		}
		//paul add end
    ...
    private Cursor uiSearch(Uri uri, String[] projection) {
        ...
        return uiQuery(UI_FOLDER, ContentUris.withAppendedId(Mailbox.CONTENT_URI,
                searchMailbox.mId), projection, false, null);//paul modify
    }
