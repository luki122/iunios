...
class Logic {
    ...
    void onDelete() {
        if (getText().equals(mResult) || mIsError) {
			...
			// Aurora liugj 2014-03-08 modified for aurora's new feature start
            //mDisplay.dispatchKeyEvent(new KeyEvent(0, KeyEvent.KEYCODE_DEL)); //event
        	try{
			...
			// Aurora liugj 2014-03-08 modified for aurora's new feature end
        }
    }
  ...
  //add by xiexiujie 11.4 start
        if(input.contains("%")){
            ...
            while (m.find()) {
            	String src=m.group();
            	String src2=mSymbols.eval(src)+"";
            	input=input.replaceAll(src, src2);
            	 m = p.matcher(input);
            }     	
        }
