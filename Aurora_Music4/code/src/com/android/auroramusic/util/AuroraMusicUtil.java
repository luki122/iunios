 ...
 * @author chenhl
 */
public class AuroraMusicUtil {
	private static final String TAG = "AuroraMusicUtil";
	...
	public static final String TIMEOFF_CLOSE_ACTION = "com.android.auroramusic.timeoffclose";
	...
	public static boolean isNoPermission(OnlineSong item){
		try {
			...
			if(!permission.isAvailable()|permission.getNeed_vip().contains(AuroraMusicUtil.getOnlineSongQuality().toString().toLowerCase())){
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @param mBarFlag
	 */
	public static void changeBarColor(Context mContext, boolean mBarFlag) {
		if (Build.VERSION.SDK_INT >= 19 && Globals.SWITCH_FOR_TRANSPARENT_STATUS_BAR) {
			try {
				Method method = AuroraActivity.class.getMethod("changeStatusBar", boolean.class);
				method.invoke(mContext, mBarFlag);
			} catch (Exception e) {
				e.printStackTrace();
			}
	 ...
	 * @param activity
	 */
	public static void statusBarBackgroundTransparent(Activity activity) {
		try {
			if (Build.VERSION.SDK_INT >= 21 && Globals.SWITCH_FOR_TRANSPARENT_STATUS_BAR) {
				Method setBarBackgroundTransparent = Class.forName("com.aurora.utils.SystemUtils").getMethod("setStatusBarBackgroundTransparent", Activity.class);
				setBarBackgroundTransparent.invoke(null, activity);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static List<String> onGetMusicConfig(Context context) {
		...
		initData(context);
		String filepath = gionee.os.storage.GnStorageManager.getInstance(context).getExternalStoragePath();
		...
		List<String> list = doParseXml(context, MUSIC_PATH);
		for (int i = 0; i < list.size(); i++) {
			musicList.add(musicpath + list.get(i));
			// Log.i(TAG,
		}
	...
	public static String[][] getTrc(String trc) {
		...
		try {
			...
			trc = trc.trim();
			String[] trcString = trc.replaceAll(AuroraMusicUtil.lrcRegularExpressions, "#").replaceFirst("#", "").split("#");
	...
	public static int idForplaylist(String name, Context context) {
		Cursor c = MusicUtils.query(context, MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI, new String[] { MediaStore.Audio.Playlists._ID }, MediaStore.Audio.Playlists.NAME + "=?",
				new String[] { Globals.AURORA_PLAYLIST_TIP + name }, MediaStore.Audio.Playlists.NAME);
		int id = -1;
		if (c != null) {
			c.moveToFirst();
			if (!c.isAfterLast()) {
	...
	private static Bitmap CircleSmall(Bitmap bitmap) {
		// Log.i(TAG,"zll ---- CircleSmall 1 bitmap.getWidth():"+bitmap.getWidth()+",getHeight:"+bitmap.getHeight()+",mCicleWidth:"+mCicleWidth);
		if (bitmap == null) {
			return null;
		}
		...
		matrix.postScale(scaleWidth, scaleHeight);
		// Log.i(TAG,
		// "zll ---- CircleSmall 1 bitmap.getWidth():"+bitmap.getWidth()+",getHeight:"+bitmap.getHeight()+",mCicleWidth:"+mCicleWidth);
	...
	public static Bitmap setAlphaImgEx(Bitmap sourceImg, int offset, int hight) {
		...
		sourceImg.getPixels(argb, 0, sourceImg.getWidth(), 0, 0, sourceImg.getWidth(), sourceImg.getHeight()); // get
																												...
																												// photo
		int linearAlphaOffset = hight;
		// int linearAlphaOffset =
		// 100;//getIntegerValueFromResourcesDimens(R.dimen.quick_index_set_linear_alpha_offset);
		// Log.i(TAG, "zll --------- setLrcAlpha = "+linearAlphaOffset);
		int number = 0;
	...
	public static Spanned getTransColorText(Context context, int... value) {
		...
		return Html.fromHtml(context.getString(R.string.aurora_fold_num, "<font color=\"#000000\">" + value[0] + "</font>", "<font color=\"#000000\">" + value[1] + "</font>"));

	}

	/**
	 * 渐入动画 lory add 2014.5.27
	 */
	...
	public static void showDeleteToast(Context context, int size, int startMode) {
		...
		if (startMode == 1) {
			...
			message = context.getResources().getString(R.string.aurora_songs_to_remove, size);
		}
	...
	public static void setMusicProp() {
		try {
			String prop = SystemProperties.get("ro.product.model");// ro.gn.extmodel
																	// ro.gn.gnznvernumber
			LogUtil.d(TAG, "----- setMusicProp 1 prop:" + prop);
			Globals.currentMode = prop;
			if (prop.equalsIgnoreCase("IUNI i1") || prop.contains("GiONEE i1") || prop.equalsIgnoreCase("IUNI N1") || prop.equalsIgnoreCase("IUNI U5")) {// u3m,8905
				Globals.SWITCH_FOR_SOUND_CONTROL = true;
				if(prop.equalsIgnoreCase("IUNI N1") || prop.equalsIgnoreCase("IUNI U5")){
			...
			LogUtil.i(TAG, "zll ----- setMusicProp 2 value:" + value);
			if (!value.equalsIgnoreCase("true") || TextUtils.isEmpty(value)) {// for
																				// india
				Globals.SWITCH_FOR_ONLINE_MUSIC = true;
			} else {
				Globals.SWITCH_FOR_ONLINE_MUSIC = false;
			}
			// add by chenhl for canzhan
			if (Globals.SWITCH_FOR_CANZHAN) {
				Globals.SWITCH_FOR_ONLINE_MUSIC = false;
			}

			String buid = SystemProperties.get("ro.gn.gnznvernumber");
			Log.i(TAG, "----------gnznvernumber:" + buid);
			if (new File(Globals.mTestPath).exists()) {
				Globals.mTestMode = true;
				Globals.STORAGE_PATH_SETTING=true;
			}
		} catch (Exception e) {
	...
	public static void getMountedStorage(Context context) {
			...
			try {
				if (sdIsMounted(temp)) {
					...
					if (k == 0) {
						...
						builder.append(" AND " + MediaStore.Audio.Media.DATA + " NOT LIKE \"" + temp + Globals.AURORA_DIAL_RECODE_DIR);
						builder.append(" AND " + MediaStore.Audio.Media.DATA + " NOT LIKE \"" + temp + Globals.AURORA_NOTE_DIR);
						// add by JXH begin
						builder.append(" AND " + MediaStore.Audio.Media.DATA + " NOT LIKE \"" + temp + Globals.AURORA_ANDROID_DATA);
						builder.append( "AND " + MediaStore.Audio.Media.DATA + " NOT LIKE \"" + temp + Globals.AURORA_CALLRECORDING);
						// add by JXH end
						builder.append(" AND " + MediaStore.Audio.Media.DATA + " NOT LIKE \"" + temp + Globals.AURORA_NOTE_DIR2 + " )");
					} else {
		...
		// Log.i(TAG,
		// "zll ---- getMountedStorage 1 time:"+(System.currentTimeMillis() -
		// time));
		// time = System.currentTimeMillis();
		// refreshQueryFilter(context);
		// Log.i(TAG,
		// "zll ---- getMountedStorage 2 time:"+(System.currentTimeMillis() -
		// time));
	}
	...
	public static void refreshQueryFilter(Context context) {
		synchronized (mSynch) {
			...
			// Log.i(TAG,
			// "zll ---- refreshQueryFilter 4 path:"+Globals.QUERY_SONG_FILTER);
		}
	...
	private static List<String> refreshQueryFilterEx(Context context) {
		{
			StringBuilder builder = new StringBuilder();
			List<String> list = AuroraMusicUtil.doParseXml(context, "paths.xml");
			int len = list.size();
			if (len < 0) {
				return null;
			}
			...
			for (int i = 0; i < len; i++) {
				...
				if (path != null && !TextUtils.isEmpty(path)) {
					...
					// Log.i(TAG,
					// "zll ---- refreshQueryFilter 2 path:"+path+",len:"+len+",bkt:"+bkt);
					// map.put(Integer.valueOf(bkt), Integer.valueOf(bkt));
					if (i == 0) {
			...
			// Log.i(TAG,
			// "zll ---- refreshQueryFilter 2.1 changed:"+changed+",mPathsXml.size():"+mPathsXml.size());
			/*
			...
			try {
				// Log.i(TAG,
				// "zll ---- refreshQueryFilter 3 builder:"+builder.toString());
				...
				if (cursor != null) {
					...
					// Log.i(TAG,
					// "zll ---- refreshQueryFilter 3.0 count:"+count);
					if (count <= 0) {
						return null;
					}
					...
					if (!cursor.isAfterLast()) {
						do {
							...
							// Log.i(TAG,
							// "zll ---- refreshQueryFilter 3.1 path:"+path);
							if (path != null) {
								String dir = path.substring(0, path.lastIndexOf("/"));
								// Log.i(TAG,
								// "zll ---- refreshQueryFilter 3.2 dir:"+dir);
				...
				Log.i(TAG, "zll ---- refreshQueryFilter 3.5 fail ----");
				e.printStackTrace();
			} finally {
				if (cursor != null) {
					cursor.close();
	...
	public static int getNavigationBarPixelHeight(Context context) {
		...
		// Log.d("felix", "zll getNaviHeight() in pixelSize = " + height);
		return height;
	}
	...
	public static boolean isNetWorkActive(Context context) {
		boolean gprsNetDialog = MusicUtils.getIntPref(context, "network_dialog_gprs", 0) != 1;
		if(gprsNetDialog){
			return false;
		}
	...
	public static String formatCurrentTime(Context context, String time) {
		...
		try {
			SimpleDateFormat format = new SimpleDateFormat(context.getString(R.string.aurora_collect_time));
			time = context.getString(R.string.aurora_collect_show_time) + format.format(new Date(Long.parseLong(time)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void addflyWindow(Context context) {
		...
		btn_floatView.setImageResource(R.drawable.aurora_note3);
	...
	public static void startFly(Context context, int flystartPointX, int flyendPointX, int flystartPointY, int flyendPointY, final AnimationEndListener listener, boolean flag) {
		...
		if (flag) {
			flyendPointX += context.getResources().getDimension(R.dimen.aurora_bezier_endx);
			flyendPointY -= context.getResources().getDimension(R.dimen.aurora_bezier_endy);
			flystartPointX += context.getResources().getDimension(R.dimen.aurora_bezier_startx);
			flystartPointY -= context.getResources().getDimension(R.dimen.aurora_bezier_starty);
		} else {
			flyendPointX += context.getResources().getDimension(R.dimen.aurora_bezier_endx);
			flyendPointY -= context.getResources().getDimension(R.dimen.aurora_bezier_endy);
			flystartPointX += context.getResources().getDimension(R.dimen.aurora_bezier_startx);
			flystartPointY -= context.getResources().getDimension(R.dimen.aurora_bezier_starty2);
		}
	...
	public static String getTimeString(Context context, long time) {
		...
		String strTime = context.getString(R.string.aurora_close_on_time) + "（" + format.format(new Date(time)) + "）";
