...
public class DataConvertUtil {
	...
	public static ArrayList<AuroraAlbum> ConvertToAlbum(Cursor cursor) {
		int resultCounts = cursor.getCount();
		if (resultCounts == 0 || !cursor.moveToFirst()) {
			return null;
		}
		ArrayList<AuroraAlbum> rentalCar = new ArrayList<AuroraAlbum>();
		for (int i = 0; i < resultCounts; i++) {
			AuroraAlbum mAlbum = new AuroraAlbum();
...
//					.getColumnIndex(ALBUM_ART)));
	...
	public static ArrayList<AuroraListItem> ConvertToTrack(Cursor cursor,List<String> paths,Context mContext) {
		int resultCounts = cursor.getCount();
		if (resultCounts == 0 || !cursor.moveToFirst()) {
			return null;
		}
		ArrayList<AuroraListItem> rentalCar = new ArrayList<AuroraListItem>();
		for (int i = 0; i < resultCounts; i++) {
			...
			String imgUri=AuroraMusicUtil.getImgPath(mContext, AuroraMusicUtil.MD5(mTitle+mArtistName+mAlbumName));
			String mPinyin = MusicUtils.getSpell(mTitle);
			if (mPinyin.charAt(0) < 65 || mPinyin.charAt(0) > 90) {
				mPinyin = String.valueOf((char) 91);
			}
			AuroraListItem mTrack = new AuroraListItem(mId, mTitle, mUri,
