...
public class AuroraMusicDownloadManager extends BasicActivity implements OnClickListener {
	...
	private void initView() {
		...
		mAuroraMusicDownloadAdapter = new AuroraMusicDownloadAdapter(this, mListView);
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				boolean is = mAuroraMusicDownloadAdapter.onItemclick(info, arg2);
				if (!is) {
	...
	public boolean isEditAnimRunning() {
		return mAuroraActionBar.auroraIsEntryEditModeAnimRunning() || mAuroraActionBar.auroraIsExitEditModeAnimRunning();
	}
	...
	public void changeMenuState() {
		if (mAuroraMusicDownloadAdapter.getCheckedCount() == mAuroraMusicDownloadAdapter.getCount()) {
			if (mAuroraMusicDownloadAdapter.getCount() == 0) {
				selectRightBtn.setText(getResources().getString(R.string.selectAll));
				selectRightBtn.setEnabled(false);
				selectRightBtn.setTextColor(getResources().getColor(R.color.aurora_select_disable));
			} else {
	...
	private AuroraBackOnClickListener mAuroraBackOnClickListener = new AuroraBackOnClickListener() {
		...
		public void auroraOnClick(int arg0) {
			...
			deleteInfo = (DownloadInfo) mAuroraMusicDownloadAdapter.getItem(arg0 - 1);
	...
	private OnAuroraActionBarItemClickListener mOnAuroraActionBarItemClickListener = new OnAuroraActionBarItemClickListener() {
		...
		public void onAuroraActionBarItemClicked(int itemId) {
			...
			case PLAY_BUTTON:
				...
				intent.setClass(AuroraMusicDownloadManager.this, AuroraPlayerActivity.class);
	...
	protected Dialog onCreateDialog(int id, Bundle bundle) {
		...
		case AURORA_ID_REMOVE_DOWNLOADS:
			...
			AuroraAlertDialog.Builder build = new AuroraAlertDialog.Builder(this).setTitle(message1)
			// .setIcon(android.R.drawable.ic_dialog_info).setMessage(message1)
					.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {
							if (mAuroraMusicDownloadAdapter.canDelete(deleteInfo)) {
	...
	public void notifySongsizeChange() {
		mSongSize.setText(getString(R.string.aurora_num_songs_of_single, mAuroraMusicDownloadAdapter.getCount()));
		if (mAuroraMusicDownloadAdapter.getCount() == 0) {
			mListView.setVisibility(View.GONE);
			noDownloadTask.setVisibility(View.VISIBLE);
			// 如果在编辑模式退出编辑模式
...
//		if (mAuroraAlertDialog == null) {
			...
			int count = mAuroraMusicDownloadAdapter.getDatasNum();
			if(count>1){
				msg = getString(R.string.remove_download_num_message,count);
			}
			AuroraAlertDialog.Builder build = new AuroraAlertDialog.Builder(this)// .setTitle(R.string.remove_download)
					.setTitle(msg).setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
						...
						@Override
						public void onClick(DialogInterface arg0, int arg1) {
							mAuroraMusicDownloadAdapter.deleteSelectItem();
							exitEditMode();
							notifySongsizeChange();
						}
					});
