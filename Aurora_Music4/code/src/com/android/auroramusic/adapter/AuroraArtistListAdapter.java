...
public class AuroraArtistListAdapter extends BaseAdapter implements
...
//		TextView tv_trackCount;
	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		if (arg1 == null) {
			View artistView = LayoutInflater.from(mContext).inflate(
					R.layout.aurora_artistlist_item, null);
			...
			vh.front = ((RelativeLayout) arg1
					.findViewById(com.aurora.R.id.aurora_listview_front));
			vh.tv_artistName = (TextView) artistView
					.findViewById(R.id.artist_name);
			vh.tv_albumCount = (TextView) artistView
			...
			rlp.setMargins((int)mContext.getResources().getDimension(R.dimen.aurora_checkbox_margin), 0, 0, 0);
		...
		if (arg0 == getPositionForSection(section)) {
			...
			vh.header = LayoutInflater.from(mContext).inflate(
					R.layout.aurora_artistheader_layout, null);
			((TextView) vh.header.findViewById(R.id.tv_artist_header))
	...
	public int getPositionForSection(int section) {
		for (int i = 0; i < mList.size(); i++) {
			...
			if (c == section || mList.get(i).mPinyin.charAt(0) < 65
					|| mList.get(i).mPinyin.charAt(0) > 90) {
				return i;
			}
		}
		return 0;
	...
	public View getHeaderView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.aurora_artistheader_layout,
					parent, false);
		}
		TextView labelText = (TextView) convertView
				.findViewById(R.id.tv_artist_header);
		char c = mList.get(position).mPinyin.charAt(0);
		if (c > 90 || c < 65) {
			c = '#';
		}
