 ...
 * 
 */
public class AuroraMainMenuAdapter extends BaseAdapter {

	private static final String TAG = "AuroraMainMenuAdapter";
	private Context mContext;
	private List<AuroraMainMenuData> datas = new ArrayList<AuroraMainMenuData>();
	private int layoutId = -1;
	private SparseIntArray defaultSize = new SparseIntArray();
	private List<String> mPathsXml = new ArrayList<String>();
	//private DownloadManager mDownloadManager;
	
	public AuroraMainMenuAdapter(Context context) {
		mContext = context;
	//	mDownloadManager=DownloadManager.getInstance(context.getApplicationContext());		
	}

	public AuroraMainMenuAdapter(Context context, int layoutid) {
		mContext = context;
		this.layoutId = layoutid;
	//	mDownloadManager=DownloadManager.getInstance(context.getApplicationContext());
	}

	public void addData(AuroraMainMenuData info) {
		datas.add(info);
	...
	public void addDatas(List<AuroraMainMenuData> list) {
		if (list == null) {
			return;
		}
		datas.addAll(list);
	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		if (arg1 == null) {
			...
			if (layoutId != -1) {
				...
				arg1 = LayoutInflater.from(mContext).inflate(
						R.layout.aurora_first_main_item, null);
			}
			...
			holdView.point=(ImageView)arg1.findViewById(R.id.aurora_red_point);
			arg1.setTag(holdView);
		} else {
			holdView = (HoldView) arg1.getTag();
		}

		AuroraMainMenuData info = datas.get(arg0);
		...
		new ShowSongSizeTask(holdView.songsize, info.getSongSizeType(),
				info.getPlaylistId(),holdView.point).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
		if (info.isShowArrow()) {
		...
		// lory modify end :for icon & more picture show or hidde in separate
	...
	private class ShowSongSizeTask extends AsyncTask<Void, Integer, Integer> {
		...
		protected Integer doInBackground(Void... params) {
			...
			int size = 0;
			mPathsXml=AuroraMusicUtil.doParseXml(mContext, "paths.xml");
			try {
				switch (sizeType) {
				case 0:
					...
					cursor = mContext.getContentResolver().query(uri1,
				...
				case 3:
					...
					cursor = mContext.getContentResolver().query(
							MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, cons,
					...
					break;
				case 4:
					size = MusicUtils.mSongDb.querySongIdFromFavorites(
							mPathsXml).size();
					break;
				case 5:
					cursor = MusicUtils.query(mContext,
							MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
							new String[] { MediaStore.Audio.Media.DATA },
							Globals.QUERY_SONG_FILTER, null,
				...
				case 6:
					...
					break;
				case 7:
					HashMap<Long, DownloadInfo> listmap = DownloadManager.getInstance(mContext.getApplicationContext())
							.getDownloadingMapData();
					List<DownloadInfo> datas = new ArrayList<DownloadInfo>();
					Iterator<DownloadInfo> iterator = listmap.values()
							.iterator();
		...
		private void setSizeText(int size) {
			...
			case 1:
				textview.setText(mContext.getString(
						R.string.aurora_playlist_total_songs, size));
				break;
			case 2:
				textview.setText(mContext.getString(
						R.string.aurora_playlist_total_singer, size));
				break;
			case 3:
				textview.setText(mContext.getString(
						R.string.aurora_playlist_total_fold, size));
				break;
			case 6:
				textview.setText(mContext.getString(R.string.aurora_collect_size, size));
				break;
			}
		}
	}
