...
public class OnlineMusicMainAdapter extends BaseAdapter {
	...
	public void setDatas(List<OnlineAlbum> albumlist, List<OnlineCollect> playlist,
			List<AuroraRankItem> ranklist,List<RadioCategory> radioList) {
		if (albumlist == null || playlist == null || ranklist == null) {
			return;
		}
		datas.clear();
		OnlineItem item = new OnlineItem(mContext.getResources().getString(R.string.aurora_online_new_album),
				R.drawable.aurora_online_new_album);
		item.setAlbumlist(albumlist);
		datas.add(item);
		item = new OnlineItem(mContext.getResources().getString(R.string.aurora_recommend_playlist),
				R.drawable.aurora_online_recommend_playlist);
...
//		LogUtil.d(TAG, "listview-------------->getview:" + arg0 + " view:" + view);
		...
		if (view == null) {
			...
			arg1 = mInflater.inflate(com.aurora.R.layout.aurora_slid_listview,
					null);
			// 获取添加内容的对象
			RelativeLayout mainUi = (RelativeLayout) arg1
					.findViewById(com.aurora.R.id.aurora_listview_front);
			View myView = mInflater.inflate(R.layout.aurora_online_main_item,
					null);
			holdView.mGridView = (GridView) myView
					.findViewById(R.id.aurora_mian_grid_view);
			holdView.mListView = (AuroraListView) myView
					.findViewById(R.id.aurora_mian_list_view);
			holdView.name = (TextView) myView
					.findViewById(R.id.aurora_icon_name);
			holdView.icon = (ImageView) myView
					.findViewById(R.id.aurora_icon_flag);
			holdView.more = (TextView) myView
					.findViewById(R.id.aurora_more_icon);
			if (arg0 == 0) {
				...
				holdView.mListView
						.setSelector(R.drawable.aurora_playlist_item_clicked);
			}
			// 将要显示的内容添加到mainUi中去
	...
	class OnlineMusicGridAdapter extends BaseAdapter {
		...
		public OnlineMusicGridAdapter(Context context, int is) {
			gridContext = context;
			this.isList = is;
			rankBitmap = BitmapFactory.decodeResource(mContext.getResources(),
					R.drawable.default_music_icon2);
			albumBitmap = BitmapFactory.decodeResource(mContext.getResources(),
					R.drawable.aurora_online_music_defualt_item_bg);
			playlistBitmap = BitmapFactory.decodeResource(
					mContext.getResources(), R.drawable.aurora_online_recommend_default);
		}
...
//			LogUtil.d(TAG, "getView..............................arg0:" + arg0);
			if (arg1 == null) {
				...
				if (isList==1) {
					arg1 = LayoutInflater.from(gridContext).inflate(
							R.layout.aurora_online_ranking_item, null);
				} else if(isList==2) {
					arg1 = LayoutInflater.from(gridContext).inflate(
							R.layout.aurora_online_radio_item, null);
					holdView.artistname = (TextView) arg1
							.findViewById(R.id.aurora_artist_name);
				}
				else {
					arg1 = LayoutInflater.from(gridContext).inflate(
							R.layout.aurora_online_music_grid_item, null);
					holdView.artistname = (TextView) arg1
							.findViewById(R.id.aurora_artist_name);
					holdView.bgmask=arg1.findViewById(R.id.aurora_item_mask);
				}
				holdView.icon = (ImageView) arg1.findViewById(R.id.aurora_icon);
				holdView.albumname = (TextView) arg1
						.findViewById(R.id.aurora_album_name);
