...
public class OnlineMusicMainAdapter extends BaseAdapter {
	...
	private SparseArray<View> storeViews = new SparseArray<View>(); // 缓存View
	...
	public void setDatas(List<OnlineAlbum> albumlist, List<OnlineCollect> playlist, List<AuroraRankItem> ranklist, List<RadioCategory> radioList) {
		if (albumlist == null || playlist == null || ranklist == null) {
			return;
		}
		datas.clear();
		OnlineItem item = new OnlineItem(mContext.getResources().getString(R.string.aurora_online_new_album), R.drawable.aurora_online_new_album);
		item.setAlbumlist(albumlist);
		datas.add(item);
		item = new OnlineItem(mContext.getResources().getString(R.string.aurora_recommend_playlist), R.drawable.aurora_online_recommend_playlist);
		item.setPlaylist(playlist);
		datas.add(item);
		// add radio
		item = new OnlineItem(mContext.getResources().getString(R.string.xiam_radio), R.drawable.aurora_online_radio);
		item.setRadioList(radioList);
		datas.add(item);
		item = new OnlineItem(mContext.getResources().getString(R.string.aurora_ranking), R.drawable.aurora_online_rank);
	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		// LogUtil.d(TAG, "listview-------------->getview:" + arg0 + " view:" + view);
		HoldView holdView;
		if (view == null) {
			...
			View myView = mInflater.inflate(R.layout.aurora_online_main_item, null);
			holdView.mGridView = (GridView) myView.findViewById(R.id.aurora_mian_grid_view);
			holdView.mListView = (AuroraListView) myView.findViewById(R.id.aurora_mian_list_view);
			holdView.name = (TextView) myView.findViewById(R.id.aurora_icon_name);
			holdView.icon = (ImageView) myView.findViewById(R.id.aurora_icon_flag);
			holdView.more = (TextView) myView.findViewById(R.id.aurora_more_icon);
			if (arg0 == 0) {
				holdView.mGridView.setVerticalSpacing(mContext.getResources().getDimensionPixelSize(R.dimen.aurora_gridview_spading_low));
				holdView.mGridView.setHorizontalSpacing(mContext.getResources().getDimensionPixelSize(R.dimen.aurora_gridview_h_spading));
				...
				holdView.mGridView.setVerticalSpacing(mContext.getResources().getDimensionPixelSize(R.dimen.aurora_gridview_spading_large));
				holdView.mGridView.setHorizontalSpacing(mContext.getResources().getDimensionPixelSize(R.dimen.aurora_gridview_h_spading));
				holdView.mGridView.setColumnWidth(mContext.getResources().getDimensionPixelSize(R.dimen.aurora_gridview_width_large));
				holdView.adapter = new OnlineMusicGridAdapter(mContext, 0);
				holdView.mGridView.setAdapter(holdView.adapter);
				holdView.mListView.setVisibility(View.GONE);
				myView.findViewById(R.id.id_music_listview).setVisibility(View.GONE);
				...
				holdView.mGridView.setBackgroundResource(R.drawable.aurora_radio_bg);
				...
				holdView.mListView.setSelector(R.drawable.aurora_playlist_item_clicked);
			}
			// 将要显示的内容添加到mainUi中去
		...
		if (info != null) {
			...
			holdView.more.setOnClickListener(new OnClickListener() {
				@Override
			...
			holdView.mGridView.setOnItemClickListener(new OnItemClickListener() {
				@Override
			...
			holdView.mListView.setOnItemClickListener(new OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
	...
	public class OnlineItem {
		...
		public void setRanklist(List<AuroraRankItem> list) {
			this.ranklist = list;
		}
	...
	class OnlineMusicGridAdapter extends BaseAdapter {
		...
		private List<AuroraRankItem> ranklists = new ArrayList<AuroraRankItem>();
		...
		public OnlineMusicGridAdapter(Context context, int is) {
			...
			albumBitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.aurora_online_music_defualt_item_bg);
			playlistBitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.aurora_online_recommend_default);
		}
		...
		public void setRanklist(List<AuroraRankItem> list, int is) {
			if (list != null) {
				this.ranklists = list;
			}
			this.type = is;
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			...
			if (arg1 == null) {
				...
				if (isList == 1) {
					arg1 = LayoutInflater.from(gridContext).inflate(R.layout.aurora_online_ranking_item, null);
				} else if (isList == 2) {
					arg1 = LayoutInflater.from(gridContext).inflate(R.layout.aurora_online_radio_item, null);
					holdView.artistname = (TextView) arg1.findViewById(R.id.aurora_artist_name);
				} else {
					arg1 = LayoutInflater.from(gridContext).inflate(R.layout.aurora_online_music_grid_item, null);
					holdView.artistname = (TextView) arg1.findViewById(R.id.aurora_artist_name);
					holdView.bgmask = arg1.findViewById(R.id.aurora_item_mask);
				}
				holdView.icon = (ImageView) arg1.findViewById(R.id.aurora_icon);
				holdView.albumname = (TextView) arg1.findViewById(R.id.aurora_album_name);
				arg1.setTag(holdView);
			} else {
			...
			if (info != null) {
				...
				if (type == 0) {
					...
					holdView.icon.setBackgroundResource(R.drawable.aurora_online_music_defualt_item_bg);
					holdView.artistname.setText(item.getArtistName());
					holdView.artistname.setVisibility(View.VISIBLE);
					holdView.bgmask.setBackgroundResource(R.drawable.aurora_new_album_mask);
					mIconWidth = albumBitmap.getWidth();
					mIconHeight = albumBitmap.getHeight();
					url = item.getImageUrl(330);
					defualtIcon = R.drawable.aurora_online_music_defualt_item_bg;
					...
					holdView.icon.setBackgroundResource(R.drawable.aurora_online_recommend_default);
					...
					holdView.bgmask.setBackgroundResource(R.drawable.aurora_recommend_mask);
					defualtIcon = R.drawable.aurora_online_recommend_default;
					FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) holdView.icon.getLayoutParams();
					params.height = mIconHeight;
					holdView.icon.setLayoutParams(params);
				} else if (type == 2) {
					...
					// holdView.artistname.setVisibility(View.GONE);
					if (arg0 == 0) {
						arg1.setBackgroundResource(R.drawable.aurora_corner_left_top);
						holdView.icon.setImageResource(R.drawable.aurora_radio_constellation);
						holdView.artistname.setText(R.string.xing_zuo_info);
					} else if (arg0 == 1) {
						arg1.setBackgroundResource(R.drawable.aurora_corner_top_right);
						holdView.icon.setImageResource(R.drawable.aurora_radio_age);
						holdView.artistname.setText(R.string.nian_dai_info);
					} else if (arg0 == 2) {
						arg1.setBackgroundResource(R.drawable.aurora_corner_bottom_left);
						holdView.icon.setImageResource(R.drawable.aurora_radio_style);
						holdView.artistname.setText(R.string.style_info);
					} else {
						arg1.setBackgroundResource(R.drawable.aurora_corner_bottom_right);
						holdView.icon.setImageResource(R.drawable.aurora_radio_mood);
						holdView.artistname.setText(R.string.mood_info);
					}
				} else {
					AuroraRankItem item = (AuroraRankItem) info;
	...
	private void initImageCacheParams() {
		options = new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.aurora_online_music_defualt_item_bg)
				.showImageForEmptyUri(R.drawable.aurora_online_music_defualt_item_bg).showImageOnFail(R.drawable.aurora_online_music_defualt_item_bg)
