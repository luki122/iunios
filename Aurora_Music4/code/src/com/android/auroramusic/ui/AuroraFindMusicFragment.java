...
public class AuroraFindMusicFragment implements OnClickListener {
	...
	private AuroraSearchHistoryAdapter mAuroraSearchHistoryAdapter;
...
//				changeButton(1);
				...
				AuroraMusicUtil.hideInputMethod(mContext, mainListView);
				return false;
			}
		});
	...
	private void initeHead(View headView, View rootView) {
		...
		rootLayoutView = (FrameLayout) rootView.findViewById(R.id.online_music_main);// (FrameLayout)
																						// mContext.getSearchViewGreyBackground();
		...
		mContext.getAuroraActionBar().getAuroraActionbarSearchView().setOnQueryTextListener(new SongSearchViewQueryTextChangeListener());
		mContext.getAuroraActionBar().getAuroraActionbarSearchViewBackButton().setOnClickListener(new OnClickListener() {
...
//			mContext.overridePendingTransition(R.anim.slide_right_in, R.anim.slide_left_out);
	...
	private class AuroraRunnale implements Runnable {
		long mSearchTime;
		Handler mHandler;
