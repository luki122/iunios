...
public class AuroraSoundControl extends AuroraActivity {
	...
	private static String[] mEffects = { "default", "pop", "dance", "rock", "normal", "jazz", "hip hop", "classical", };
	...
	private void initDtsHeadsetName() {
		...
		if (mHeadsetView != null && mHeadsetView.getVisibility() == View.VISIBLE) {
			mHeadsetPos = MusicUtils.getIntPref(this, AURORA_DATA_DTS_HEADSET_STATUS, -1);
			LogUtil.d(TAG, "----initDtsHeadsetName----mHeadsetPos:" + mHeadsetPos);
			String name = getString(R.string.aurora_null);
			if (mHeadsetPos == 1) {
				if (Globals.currentMode.equalsIgnoreCase("IUNI U0003")) {
					name = getString(R.string.aurora_iuni_new_headset);
				} else {
	...
	private void updateViewsAndSetConfig() {
		synchronized (this) {
			...
			if (Globals.SWITCH_FOR_SOUND_CONTROL) {
				if ((mbHasEar == mtmpbHasEar) && (mInitHifi == mtmpInitHifi) && (mNoise == mtmpNoise) && (mDtsPos == mtmpDtsPos)) {
				...
				if (mbHasEar) {
					if (mInitHifi) {
						...
						if (localAudioManager != null) {
							...
							localAudioManager.setParameters("anc_enabled=false"); // to
																					// disable
						...
						if (localAudioManager != null) {
							...
							if (noise) {
								localAudioManager.setParameters("anc_enabled=true"); // to
																						// disable
							} else {
								localAudioManager.setParameters("anc_enabled=false"); // to
																						// disable
	...
	private void setAuroraEffect(int pos) {
		...
		for (int j = 0; j < tmp.length; j++) {
			// Log.i(TAG,
			// "zll ---- yyyy 1 j:"+j+",tmp[j]:"+tmp[j]+",pos:"+pos+",mEffects[pos]:"+mEffects[pos]+",tmp.length:"+tmp.length);
	...
	private void initviews() {
		...
		if (Globals.SWITCH_FOR_SOUND_CONTROL) {
			...
			if (localAudioManager.isWiredHeadsetOn() && mDtsPos > 0/* dts open */) {
				 ...
				 * 
				 * if (headseTextView != null) {
				 * headseTextView.setEnabled(true); headseTextView.setAlpha(1f);
				 * }
				 */
			} else {
				if (headseTextView != null) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == HeasetSelectActivity.AURORA_HEADSET_CHANGDE) {
			if (resultCode == RESULT_OK) {/*
										 * mHIfi = 0; mInitHifi = false;
										 * 
										 * if (hifiSwitchView != null) {
										 * hifiSwitchView.setEnabled(true);
										 * hifiSwitchView.setChecked(false); }
										 * 
										 * localAudioManager.setParameters(
										 * "HIFI_SWITCH=0"); int num2 = 0;
										 * while(
										 * !localAudioManager.getParameters
										 * ("HIFI_SWITCH").contentEquals("0")){
										 * if (num2 > 10) { Log.i(TAG,
										 * "zll ----- onActivityResult 1 set fail :hifi > 1s"
										 * ); break; }
										 * 
										 * num2++; try { Thread.sleep(10); }
										 * catch (InterruptedException e) {
										 * e.printStackTrace(); } }
										 */
			}
	...
	private void showSoundControl() {
		...
		ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(controlMainView, "translationY", new float[] { height, 0 });
	...
	private void disSoundControl() {
		...
		AnimatorSet animatorSet = new AnimatorSet();
		ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(controlMainView, "translationY", new float[] { 0, height });
	...
	private void upControlMainView() {
		try {
			if (mHasNaviBar || Globals.currentMode.equalsIgnoreCase("U3")) {
				boolean hide = Settings.System.getInt(this.getContentResolver(), NAVI_KEY_HIDE, 0) != 0;
