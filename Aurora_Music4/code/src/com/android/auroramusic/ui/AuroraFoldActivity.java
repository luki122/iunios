...
public class AuroraFoldActivity extends AbstractBaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent != null) {
			...
			if (startMode == 1) {
				playlistInfo = (AuroraMainMenuData) intent
						.getSerializableExtra(AuroraSongSingle.EXTR_PLAYLIST_INFO);
				mAddList = intent
						.getStringArrayListExtra(AuroraNewPlayListActivity.EXTR_ADDLIST_DATA);
				if (mAddList == null) {
					finish();
				}
	...
	protected void initView() {
		...
		totalFold.setText(AuroraMusicUtil.getTransColorText(
				AuroraFoldActivity.this, totalFolds, totalSongs));
		...
		try {
			...
			if (startMode == 1) {//添加歌曲到歌单
				...
				mAuroraActionBar.initActionBottomBarMenu(
						R.menu.menu_add_playlist_song_bottombar, 1);
				bottomMenu = mAuroraActionBar.getAuroraActionBottomBarMenu();
				bottomBarTitle = bottomMenu.getTitleViewByPosition(0);
				bottomBarTitle
						.setText(getString(
								R.string.aurora_add_playlist_song_num,
								mAddList.size()));
				if (mAddList.size() > 0) {
					bottomMenu.setBottomMenuItemEnable(1, true);
				} else {
				...
				mAuroraActionBar.addItem(R.drawable.song_playing, PLAY_BUTTON,
						null);
	...
	private OnClickListener mOnClickListener = new OnClickListener() {
		...
		public void onClick(View arg0) {
			Intent intent = new Intent(AuroraFoldActivity.this,
					AuroraIgnoreFoldActivity.class);
	...
	private AuroraDeleteItemListener mAuroraDeleteItemListener = new AuroraDeleteItemListener() {
		...
		public void auroraDeleteItem(View arg0, int arg1) {
			...
			AuroraMusicUtil.writeToXml(AuroraFoldActivity.this,
					AuroraMusicUtil.writeToString(mIgnorPathList), "paths.xml");
			...
			Toast.makeText(AuroraFoldActivity.this,
					R.string.aurora_not_scan_the_fold, Toast.LENGTH_SHORT)
					.show();
		}
	...
	private AuroraBackOnClickListener mAuroraBackOnClickListener = new AuroraBackOnClickListener() {
		...
		public void auroraOnClick(int position) {
			AuroraAlertDialog.Builder builder = new AuroraAlertDialog.Builder(
					AuroraFoldActivity.this)
					.setTitle(R.string.aurora_ignor_scan)
					.setMessage(R.string.aurora_ignoer_scan_message)
	...
	private OnAuroraActionBarBackItemClickListener mOnActionBarBackItemListener = new OnAuroraActionBarBackItemClickListener() {
		...
		public void onAuroraActionBarBackItemClicked(int arg0) {
			...
			tIntent.putStringArrayListExtra(
					AuroraNewPlayListActivity.EXTR_RESULT_DATA, mAddList);
		...
		public void auroraMenuItemClick(int itemId) {
			...
			case R.id.song_add:
				if (mAddList != null && mAddList.size() != 0) {
					...
					MusicUtils.addToPlaylist(AuroraFoldActivity.this, mAddList,
							playlistInfo.getPlaylistId(),
							playlistInfo.getName());
					Intent intent = new Intent(AuroraFoldActivity.this,
							AuroraSongSingle.class);
					intent.putExtra(AuroraSongSingle.EXTR_PLAYLIST_INFO,
							playlistInfo);
					startActivity(intent);
					// gobackToSongSingle();
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		if (startMode == 1) {
			...
			intent.putStringArrayListExtra(
					AuroraNewPlayListActivity.EXTR_ADDLIST_DATA, mAddList);
			startActivityForResult(intent,
					AuroraNewPlayListActivity.REQUEST_CODE_SONGS);
		} else {
			startActivity(intent);
		}
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case AuroraNewPlayListActivity.REQUEST_CODE_SONGS:
			...
			int mode = data.getIntExtra(
					AuroraNewPlayListActivity.EXTR_RESULT_MODE, 0);
			if (mode == 0) {
				mAddList = data
						.getStringArrayListExtra(AuroraNewPlayListActivity.EXTR_RESULT_DATA);
				if (mAddList.size() > 0) {
					bottomMenu.setBottomMenuItemEnable(1, true);
				} else {
					bottomMenu.setBottomMenuItemEnable(1, false);
