...
public class AuroraSongSingle extends AbstractBaseActivity implements Runnable, OnClickListener {
	...
	protected Dialog onCreateDialog(int id) {
		...
		case AURORA_ID_DELETE_SINGLE:
			...
			AuroraAlertDialog.Builder build = new AuroraAlertDialog.Builder(this).setTitle(R.string.delete)
			// .setIcon(android.R.drawable.ic_dialog_info)
					.setMessage(R.string.aurora_delete_message).setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
						@Override
		...
		case AURORA_ID_DELETE_SONGS:
			...
			if (mStartMode == 1) {
				...
				if (deleteId != -1) {
					message = getString(R.string.deleteMessage, 1);
				} else {
					message = getString(R.string.deleteMessage, mAuroraSongSingleAdapter.getCheckedCount());
				}

			} else {
				if (deleteId != -1) {
					message = getString(R.string.aurora_remove_message, 1);
				} else {
					message = getString(R.string.aurora_remove_message, mAuroraSongSingleAdapter.getCheckedCount());
				}
				title = getString(R.string.aurora_remove);
			}
			AuroraAlertDialog.Builder build1 = new AuroraAlertDialog.Builder(this).setTitle(title)
			// .setIcon(android.R.drawable.ic_dialog_info)
