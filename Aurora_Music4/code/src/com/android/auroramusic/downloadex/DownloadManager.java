...
public class DownloadManager {
	...
	private void initData() {
		...
		if (mHashMap == null) {
			LogUtil.iv(TAG, "zll initData --- 1 fail ----:");
		...
		LogUtil.iv(TAG, "zll initData --- ok mHashMap len:" + mHashMap.size());
		return;
	}
	...
	public static boolean hasNetwork() {
		return AuroraMusicUtil.isNetWorkActive(mContext);
	}
	...
	public void pauseDownloadById(long id) throws Exception {
		...
		if (task != null && !task.isCancelled()) {
			// LogUtil.iv(TAG, "zll pauseDownloadById 1 ----- id:"+id);
			task.pauseTask();
			info.setTask(null);
		} else {
			// LogUtil.iv(TAG, "zll pauseDownloadById 2 ----- id:"+id);
			if (info != null && info.isWaiting()) {
				// LogUtil.iv(TAG, "zll pauseDownloadById 3 ----- id:"+id);
	...
	public void pauseAllDownload() throws Exception {
		...
		while (iter.hasNext()) {
			...
			if (data != null) {
				...
				// LogUtil.iv(TAG, "zll tttt 7.2 --- data:"+data.getState());
				mList.add(data);
	...
	public void resumeDownloadById(long id) {
		...
		LogUtil.iv(TAG, "zll resumeDownloadById ---- 1 id:" + id + ",mList.size():" + mHashMap.size());
		if (mdb == null) {
			mdb = Dao.getInstance(mContext);
		}
		...
		LogUtil.iv(TAG, "zll resumeDownloadById ---- 2.2 ok:" + downloadInfo.toString());
		if (downloadInfo != null) {
			...
			if (task != null && !task.isCancelled()) {
				LogUtil.iv(TAG, "zll resumeDownloadById ---- 2.3 ok:");
		...
		mdb.savaOrUpdate(downloadInfo);
		LogUtil.iv(TAG, "zll resumeDownloadById ---- 2.5 ok:");
	...
	public void resumeAllDownloadAndListener(List<DownloadInfo> list, final HashMap<Long, DownloadStatusListener> callbacks) {
		...
		updateAllDb(list2);
		// LogUtil.iv(TAG,
		// "zll tttt 4 --- time5:"+(System.currentTimeMillis()-time5));
	...
	private void resumeOneDownload(long id, List<DownloadInfo> list) {
		...
		// LogUtil.iv(TAG,
		// "zll resumeOneDownload ---- 1 id:"+id+",mList.size():"+mHashMap.size());
		if (mdb == null) {
			mdb = Dao.getInstance(mContext);
		}
		...
		// LogUtil.iv(TAG,
		// "zll resumeOneDownload ---- 2.2 ok:"+downloadInfo.toString());
		if (downloadInfo != null) {
			DownloadTask task = downloadInfo.getTask();
			if (task != null && !task.isCancelled()) {
				// LogUtil.iv(TAG, "zll resumeOneDownload ---- 2.3 ok:");
				task.cancelTaskRunOnly();
			}
		}
		...
		// LogUtil.iv(TAG, "zll resumeOneDownload ---- 2.5 ok:");
		// LogUtil.iv(TAG,
		// "zll tttt 5 --- time5:"+(System.currentTimeMillis()-time5));
	...
	private void resumeAllDownload() throws Exception {
		...
		LogUtil.iv(TAG, "zll resumeAllDownload ---- 1 mList.size():" + mHashMap.size());
		...
		while (iter.hasNext()) {
			...
			if (downloadInfo != null) {
				...
				if (oldtask != null && !oldtask.isCancelled()) {
					LogUtil.iv(TAG, "zll resumeAllDownload ---- 2:");
	...
	public void removeDownloadById(long id) throws Exception {
		...
		LogUtil.iv(TAG, "zll removeDownloadById ---- 1 id:" + id + ",mList.size():" + mHashMap.size());
	...
	// -----add by tangjie 2014/01/08 start-----//
	...
	private static void notifyToScanDir() {
		...
		LogUtil.iv(TAG, "zll notifyToScanDir -----");

		Intent intent = new Intent();
		intent.setAction(AuroraMediaPlayHome.ACTION_DIR_SCAN);
	...
	private final DownloadStatusListener mListener = new DownloadStatusListener() {
		...
		public void onDownload(String url, long id, int status, long downloadSize, long fileSize) {
			// LogUtil.iv(TAG, "zll gggg onDownload ---- 1 id:"+ id
