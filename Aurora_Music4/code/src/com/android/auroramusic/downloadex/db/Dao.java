package com.android.auroramusic.downloadex.db;
...
import com.android.auroramusic.db.AuroraDbData;
import com.android.auroramusic.downloadex.DownloadInfo;
import com.android.auroramusic.downloadex.DownloadTask.State;
...
public class Dao extends SQLiteOpenHelper {
	...
	public static final String matchUri = "aurora_download";
	public static final String mUri = "content://aurora_download/download";
	...
	private void dropTable(SQLiteDatabase db) {
		try {
			LogUtil.iv(TAG, "zll dropTable start");
			db.execSQL("DROP TABLE IF EXISTS download");
		} catch (SQLException ex) {
			LogUtil.iv(TAG, "zll dropTable fail");
			throw ex;
		}
	}
	...
	public static synchronized Dao getInstance(Context context) {
		if (mInstance == null)
			mInstance = new Dao(context, DATABASE_NAME, AuroraDbData.VERSION);
	...
	private void openDataBase() {
		try {
			LogUtil.iv(TAG, "zll dao openDataBase !!!");
	...
	public void updateStatus(long id, int status) {
		...
		try {
			...
			if ((this.mDatabase == null) || (!this.mDatabase.isOpen())) {
				LogUtil.iv(TAG, "zll updateStatus --- fail 0");
				return;
			}
			...
			LogUtil.iv(TAG, "zll updateStatus --- success");
	...
	public void updateDownloadSize(long id, long size) {
		...
		try {
			...
			if ((this.mDatabase == null) || (!this.mDatabase.isOpen())) {
				LogUtil.iv(TAG, "zll updateDownloadSize --- fail 0");
				return;
			}
			...
			this.mDatabase.execSQL(sql, bindArgs);
			// LogUtil.iv(TAG, "zll updateDownloadSize ccc --- success");
	...
	public void updateFileName(long id, String name) {
		...
		try {
			...
			if ((this.mDatabase == null) || (!this.mDatabase.isOpen())) {
				LogUtil.iv(TAG, "zll updateFileName --- fail 0");
				return;
			}
			...
			this.mDatabase.execSQL(sql, bindArgs);
			LogUtil.iv(TAG, "zll updateFileName --- success");
	...
	public void updateFileInfo(long id, String name, String path, String lrcPath) {
		...
		try {
			if ((this.mDatabase == null) || (!this.mDatabase.isOpen())) {
				LogUtil.d(TAG, "---------------updateFileInfo fail");
				return;
			}
			...
			LogUtil.d(TAG, "---------------updateFileInfo success");
		...
		try {
			...
			if (cursor != null) {
				...
				if (!cursor.isAfterLast()) {
					...
					LogUtil.iv(TAG, "zll getDownloadStatusById --- 1 success ,status:" + status);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
			LogUtil.iv(TAG, "zll getDownloadStatusById --- fail 0");
		} finally {
	...
	public boolean isDownloadFinished(long id) {
		...
		try {
			...
			if (cursor != null) {
				num = cursor.getCount();
				LogUtil.iv(TAG, "zll isDownloadFinished --- 1 num:" + num);
			}

		} catch (Exception e) {
			e.printStackTrace();
			num = 0;
			LogUtil.iv(TAG, "zll isDownloadFinished --- fail 0");
		} finally {
			DbUtils.closeQuietly(cursor);
		}
	...
	public boolean findDownLoadingOrFinishedByUrl(long id) {
		...
		try {
			...
			if (cursor != null) {
				num = cursor.getCount();
				LogUtil.iv(TAG, "zll findDownLoadingOrFinishedByUrl --- 1 num:" + num);
			}

		} catch (Exception e) {
			e.printStackTrace();
			num = 0;
			LogUtil.iv(TAG, "zll findDownLoadingOrFinishedByUrl --- fail 0");
		} finally {
			DbUtils.closeQuietly(cursor);
		}
		...
		try {
			...
			if (cursor != null) {
				...
				while (!cursor.isAfterLast()) {
					...
					cursor.moveToNext();
					// LogUtil.iv(TAG, "zll getAllDownLoadData --- 1 item:" +
					// entity.toString());
					// Log.i(TAG, "zll getAllDownLoadData --- 1 item:" +
					// entity.toString());
				}
			}

			LogUtil.iv(TAG, "zll getAllDownLoadData --- 2 success:");
			// Log.i(TAG, "zll getAllDownLoadData --- 2 success:");
		} catch (Exception e) {
			e.printStackTrace();
			LogUtil.iv(TAG, "zll getAllDownLoadData --- fail 0");
		} finally {
			DbUtils.closeQuietly(cursor);
		}
			...
			try {
				...
				if (cursor != null) {
					...
					while (!cursor.isAfterLast()) {
						...
						LogUtil.iv(TAG, "zll isItemDownloaded --- downurl:" + downurl);
				...
				LogUtil.iv(TAG, "zll isItemDownloaded --- isExist:" + isExist);
			} catch (Exception e) {
				LogUtil.iv(TAG, "zll isItemDownloaded --- fail 0");
			} finally {
				DbUtils.closeQuietly(cursor);
			}
		}
	...
	public boolean isItemExist(long id) throws Exception {
		...
		if (id >= 0) {
			...
			try {
				...
				LogUtil.iv(TAG, "zll isItemExist --- isExist:" + isExist);
			} catch (Exception e) {
				LogUtil.iv(TAG, "zll queryItemEx --- fail 0");
			} finally {
				DbUtils.closeQuietly(cursor);
			}
		}
	...
	private void updateFileSizeEx(long id, long size) {
		if ((this.mDatabase == null) || (!this.mDatabase.isOpen())) {
			LogUtil.iv(TAG, "zll updateFileSizeEx --- fail 0");
			return;
		}
		...
		try {
			...
			Object[] bindArgs = { size, String.valueOf(id) };
			this.mDatabase.execSQL(sql, bindArgs);
			LogUtil.iv(TAG, "zll updateFileSizeEx --- success");
		} catch (Exception e) {
	...
	private DownloadInfo queryItemEx(String urlpath, long songid) {
		...
		try {
			...
			LogUtil.iv(TAG, "zll queryItemEx --- 1 songid:" + songid + ",urlpath:" + urlpath);
			...
			if (cursor != null) {
				...
				while (!cursor.isAfterLast()) {
					...
					info.setImgUrl(cursor.getString(4));// add by chenhl
					cursor.moveToNext();
					LogUtil.iv(TAG, "zll queryItemEx --- success --- info:" + info.toString());
				}
			}
			...
			e.printStackTrace();
			LogUtil.iv(TAG, "zll queryItemEx --- fail 1");
		} finally {
			DbUtils.closeQuietly(cursor);
		}
	...
	private void deleteEx(String urlpath, long songid) {
		if ((this.mDatabase == null) || (!this.mDatabase.isOpen())) {
			LogUtil.iv(TAG, "zll deleteEx fail --- 0");
			return;
		}
		...
		try {
			...
			LogUtil.iv(TAG, "zll deleteEx 1 success ret:" + ret + ",urlpath:" + urlpath + ",songid:" + songid);
		} catch (Exception e) {
			e.printStackTrace();
			LogUtil.iv(TAG, "zll deleteEx failed --- 1");
		}
	}
	...
	private void savaOrUpdateEx(DownloadInfo infos) {
		...
		LogUtil.iv(TAG, "zll savaOrUpdateEx  --- 1");
		long id = infos.getId();
		if (id < 0) {
			return;
		}
		...
		try {
			...
			LogUtil.iv(TAG, "zll savaOrUpdateEx  --- 2 has:" + has);

			// Log.i(TAG, "zll ---- bbb 1 time:"+(System.currentTimeMillis() -
			// time)+",has:"+has);
			// time = System.currentTimeMillis();
		...
		// Log.i(TAG, "zll ---- bbb 2 time:"+(System.currentTimeMillis() -
		// time));
		return;
	}
	...
	private boolean insertInternal(DownloadInfo infos) {
		try {
			...
			LogUtil.iv(TAG, "zll insertInternal 1 infos:" + infos.getDownloadUrl());
			...
			// modify by chenhl set title replace filename add save imgurl
			...
			LogUtil.iv(TAG, "zll insertInternal success");
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			LogUtil.iv(TAG, "zll insertInternal fail ---");
		}
	...
	private void updateEx(DownloadInfo infos) {
		if ((this.mDatabase == null) || (!this.mDatabase.isOpen())) {
			LogUtil.iv(TAG, "zll updateEx fail --- 0");
			return;
		}
		...
		try {
			...
			LogUtil.iv(TAG, "zll updateEx success ---");
		} catch (Exception e) {
			e.printStackTrace();
			LogUtil.iv(TAG, "zll updateEx fail --- 1");
		}
		return;
	}
	...
	// --- add by tangjie 2015/01/08 start---//
	public long deleteItemByFilePath(String filePath) {
		...
		try {
			...
			LogUtil.iv(TAG, "zll deleteItem 1 success ret:" + ret + ",filePath:" + filePath);
		} catch (Exception e) {
			e.printStackTrace();
			LogUtil.iv(TAG, "zll deleteItem failed --- 1");
		}
		return 0;
	}
	// --- add by tangjie 2015/01/08 end---//
