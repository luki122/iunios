                                                              ...
                                                              int size))
{
	...
	int downloadsize=0,cursize;	//add by JXH 
    len = 0;
    while (len < size_min) {
        if (ff_check_interrupt(&h->interrupt_callback))
            return AVERROR_EXIT;
		//add by chenhl start
		downloadsize = ff_get_download_size(&h->downloadsize_callback,size);
		if(downloadsize>0){
			av_usleep(1000);
			continue;
		}
...
//add by chenhl start
int ff_get_download_size(AVIOInterruptCB *cb,int cursize){
	int ret=0;
	if(cb&&cb->callback_1&& (ret = cb->callback_1(cb->opaque,cursize))){
		return ret;
	}	
	return 0;
}
