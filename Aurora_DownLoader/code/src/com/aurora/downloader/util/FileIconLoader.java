...
public class FileIconLoader implements Callback {
	...
	private void processLoadedIcons() {
		...
		while (iterator.hasNext()) {
			...
			if (!fileId.mPath.equals(view.getTag())) {
				AuroraLog.elog(TAG, "fileId.mPath != view.getTag()");
				return;
			}
	 ...
	 */
	private ArrayList<WeakReference<Bitmap>> bitmapWf = new ArrayList<WeakReference<Bitmap>>();
	...
	private class LoaderThread extends HandlerThread implements Callback {
		...
		public boolean handleMessage(Message msg) {
			...
			while (iterator.hasNext()) {
				...
				AuroraLog.log(TAG, "id.mPath==" + id.mPath);
				if (holder != null && holder.state == ImageHolder.NEEDED) {
					...
					case Video:
						...
						if (id.mId == 0) {
							int count = weakHashMap.get(id.mPath)==null?0:weakHashMap.get(id.mPath);
							Log.e(TAG, "Fail to get dababase id for:"
									+ id.mPath+" count=="+count+" weakHashMap=="+weakHashMap.size());
							if(count>300){
								mImageCache.remove(id.mPath,ImageHolder.NEEDED);
								isload=true;
							}
							count++;
						...
						try {
							...
							Log.e(TAG,
									"getVideoThumbnail/getImageThumbnail fail");

						}
						if (bitmap != null||isload) {
							isload=false;
							holder.setImage(bitmap);
							bitmapWf.add(new WeakReference<Bitmap>(bitmap));
							holder.state = BitmapHolder.LOADED;
						} else {
							holder.state = ImageHolder.NEEDED;
						}
		...
		private Bitmap getImageThumbnail(long id) {
			...
			return Images.Thumbnails.getThumbnail(
					mContext.getContentResolver(), id, MICRO_KIND, options);
		}
