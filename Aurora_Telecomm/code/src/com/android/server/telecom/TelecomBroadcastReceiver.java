...
public final class TelecomBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_SEND_SMS_FROM_NOTIFICATION.equals(action)) {
            ...
            callIntent.setComponent(new ComponentName("com.android.mms", 
            		"com.android.mms.ui.ComposeMessageActivity"));
            boolean isPrivate = intent.getBooleanExtra("isprivate", false);
            if(isPrivate) {
            	long privateId = intent.getLongExtra("privateId", 0);
            	callIntent.putExtra("is_privacy", privateId);
            }
            callIntent.putExtra("fromScreen", true);  
        ...
        // Call back recent caller from the missed call notification.
            ...
            int slot = AuroraSubUtils.getSlotBySubId(context, MissedCallNotifier.mSimId);
        	if(slot > -1) {
        		callIntent.putExtra("subscription", slot);
        	}
            context.startActivity(callIntent);
