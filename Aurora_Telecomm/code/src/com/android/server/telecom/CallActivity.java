...
public class CallActivity extends Activity {
    ...
    private void processIntent(Intent intent) {
        ...
        Log.d(this, "slot = " + slot);
        if(slot != -1) {
			TelephonyManager telMgr = (TelephonyManager) getSystemService(TELEPHONY_SERVICE);
			if (telMgr.isMultiSimEnabled()) {
				try {
					int simMode = Settings.System.getInt(
							this.getContentResolver(),
							Settings.System.MSIM_MODE_SETTING, -1);
					boolean iscard1Enable = (telMgr.getSimState(0) != TelephonyManager.SIM_STATE_ABSENT)
							&& ((simMode & 1) > 0);
					boolean iscard2Enable = (telMgr.getSimState(1) != TelephonyManager.SIM_STATE_ABSENT)
							&& ((simMode & 2) > 0);
					Log.d(this, "pickPhoneBasedOnNumber iscard1Enable= "
							+ iscard1Enable + " iscard2Enable = "
							+ iscard2Enable);
					if (slot > 1) {
						slot = 0;
					}
					if (!iscard1Enable && iscard2Enable && slot == 0) {
						slot = 1;
					} else if (iscard1Enable && !iscard2Enable && slot == 1) {
						slot = 0;
					}
				} catch (Exception e) {
					e.printStackTrace();
        	...
        	PhoneAccountHandle accountHandle = getPhoneAccountById(this, String.valueOf(getSubIdbySlot(this, slot)));
	...
	private int getSlot(final Intent intent) {
		int sub = intent.getIntExtra("subscription", -1);
		return sub;
	}
	
	private static PhoneAccountHandle getPhoneAccountById(Context context, String id) {
	...
	private static int getSubIdbySlot(Context ctx, int slot) {
