...
//    };
	private static final long[] VIBRATION_PATTERN = new long[] {0, 1800, 200, 500, 1800, 200, 100};
    ...
    private void startRingingOrCallWaiting() {
        ...
        if (mRingingCalls.contains(foregroundCall)) {
            ...
            String number = foregroundCall.getHandle() == null ? null : foregroundCall.getHandle().getSchemeSpecificPart();
    		if(RejectUtils.isAuroraNeedHangup(number, false)) {
    			return;
    		}
...
//        sP = mContext.getSharedPreferences("com.android.phone.settings", Context.MODE_PRIVATE);
    	try {
			Context phoneContext = mContext.createPackageContext("com.aurora.callsetting", Context.CONTEXT_IGNORE_SECURITY);  
			sP = phoneContext.getSharedPreferences("com.android.phone.settings", Context.MODE_WORLD_READABLE);
		} catch(NameNotFoundException e){
			e.printStackTrace();
		}
        boolean def = mContext.getResources().getBoolean(R.bool.aurora_ringer);
        boolean result = sP != null && sP.getBoolean("ringermode", def); 
    	Log.v(this, "isAuroraRinger result = " + result);
        return result;        
    } 
