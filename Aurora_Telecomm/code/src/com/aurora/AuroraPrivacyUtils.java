...
public class AuroraPrivacyUtils {
	...
	public static void killPrivacyActivity() {
		try {
			if (AuroraGlobals.mPrivacyActivityList != null) {
				for (Activity ac : AuroraGlobals.mPrivacyActivityList) {
					ac.finish();
				}
			}
		} catch (Exception e) {
	 ...
	 public static boolean isSupportPrivate() {
	    ...
	    public static int[] getPrivateData(Uri uri) {
			...
			if (url.startsWith("content://com.android.contacts/contacts/lookup/") || url.startsWith("content://com.android.contacts/contacts/")) {
				long RawContactId = ProviderUtils.queryForRawContactId(AuroraGlobals.getInstance()
				...
				Cursor c = AuroraGlobals.getInstance().getContentResolver()
				...
				Cursor cursor = null;
				try {
					cursor = AuroraGlobals.getInstance().getContentResolver()
	    ...
	    public static int[] getPrivateData(String number) { 
			...
			Cursor cursor = null;
			try {
				cursor = AuroraGlobals.getInstance().getContentResolver().query(Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, number), 
		...
		public static String getPrivateRingNotificationText(long accountId) {	
			Log.v("getPrivateRingNotificationText", " accountId = " + accountId);
		   	String defaultText = AuroraGlobals.getInstance().getResources().getString(R.string.private_notification_text);
			String[] columns={MSG_NOTIFY_HINT};
			
			Cursor cursor = AuroraGlobals.getInstance().getContentResolver().query(PRIVATE_CONTENT_URI,
					columns,
					getQueryWhere(),
		...
		public static String getPrivateHomePath(long accountId) {	
			...
			Cursor cursor = AuroraGlobals.getInstance().getContentResolver().query(PRIVATE_PATH_URI,
					columns,
					getQueryWhere(),
					getQueryValue(accountId),
					null);	 
...
//	    public static String getPrivateRingNotificationText(long privateId) { 
//	    	String defaultText = AuroraGlobals.getInstance().getResources().getString(R.string.private_notification_text);
//	    	try {
//		    	Context context =  AuroraGlobals.getInstance().createPackageContext("com.android.contacts", Context.CONTEXT_IGNORE_SECURITY);    	
//		        SharedPreferences sp = context.getSharedPreferences("PrivateSettings",Context.MODE_WORLD_READABLE);
//		        String result = sp.getString("private_ring_notification_text" + privateId, defaultText);
//				Log.v("getPrivateRingNotificationText", " text = " + result);
//		        return result;
//	    	} catch (Exception e){
//	    		e.printStackTrace();
//	    	}
...
//	    public static boolean isPrivateSendSms(long privateId) { 
//	    	boolean defaultValue = true;
//	    	try {
//		    	Context context =  AuroraGlobals.getInstance().createPackageContext("com.android.contacts", Context.CONTEXT_IGNORE_SECURITY);    	
//		        SharedPreferences sp = context.getSharedPreferences("PrivateSettings",Context.MODE_WORLD_READABLE);
//		        boolean result = sp.getBoolean("private_sms_notification" + privateId, defaultValue);
//				Log.v("isPrivateSendSms", " result = " + result);
//		        return result;
//	    	} catch (Exception e){
//	    		e.printStackTrace();
//	    	}
...
//	    }
	    ...
	    public static boolean auroranotifyPrivateMissedCall(Call call) {
	    	// lgy
	    	if(call.getTargetPhoneAccount() != null) {
	    		int subId = Integer.valueOf(call.getTargetPhoneAccount().getId());
	    		int slot = SubscriptionManager.from(call.getContext()).getSlotId(subId);	        	
	        	SIMInfo simInfo = SIMInfo.getSIMInfoBySlot(AuroraGlobals.getInstance(), slot);
		    	if(simInfo != null) {
		    		MissedCallNotifier.mSimId = (int)simInfo.mSimId;
		    		PrivateNotificationMgr.mSimId = (int)simInfo.mSimId;
		    	}
	    	}
	    		
	    	
	    	AuroraCallerInfo ci = call.getCallerInfo();
	    	
	    	//add by liguangyu for 17080
	    	if(ci == null) {
	    		logs("auroranotifyPrivateMissedCall ci is null");  
	    		return false;
	    	}
	    	//add end
	        int privateData[] = getPrivateData(ci.contactRefUri);
	        long currentPrivateId = getCurrentAccountId();
	        long privateId = privateData != null ? privateData[1] : 0;    
			if(privateId > 0) {
				if (privateId != currentPrivateId) {
		        	logs("auroranotifyPrivateMissedCall = 0");   
		        	AuroraGlobals.getInstance().mManagePrivate.notificationMgr.notifyHangupPrivateRingingCallFake(privateId);
				} else {
			    	logs("auroranotifyPrivateMissedCall = 1");   
			    	AuroraGlobals.getInstance().mManagePrivate.notificationMgr.notifyPrivateMissedCall(ci.name, ci.phoneNumber,
		                    ci.phoneLabel, ci.cachedPhoto, ci.cachedPhotoIcon, call.getCreationTimeMillis(), privateId);
				}
				return true;
			} else {
		    	logs("auroranotifyPrivateMissedCall = 2");
		    	//liguangyu
//		    	AuroraGlobals.getInstance().notificationMgr.notifyMissedCall(name, number,
