...
import com.aurora.note.util.DebugUtil;
...
ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnTouchModeChangeListener {
    ...
    /**
     * Indicates whether the list selector should be drawn by inner padding
     */
    boolean mDrawSelectorByInnerPadding = false;

    /**
     * The selector's inner left padding
     */
    int mSelectorInnerLeftPadding = 0;

    /**
     * The selector's inner top padding
     */
    int mSelectorInnerTopPadding = 0;

    /**
     * The selector's inner right padding
     */
    int mSelectorInnerRightPadding = 0;

    /**
     * The selector's inner bottom padding
     */
    int mSelectorInnerBottomPadding = 0;
    ...
    private void positionSelector(int l, int t, int r, int b) {
        ...
        if (mDrawSelectorByInnerPadding) {
            positionSelectorByInnerPadding();
        }
    }

    private void positionSelectorByInnerPadding() {
        mSelectorRect.set(
                mSelectorRect.left + mSelectorInnerLeftPadding, 
                mSelectorRect.top + mSelectorInnerTopPadding, 
                mSelectorRect.right - mSelectorInnerRightPadding, 
                mSelectorRect.bottom - mSelectorInnerBottomPadding);
    }
    ...
    public void setDrawSelectorByInnerPadding(boolean byInnerPadding) {
        mDrawSelectorByInnerPadding = byInnerPadding;
    }

    public void setSelectorInnerPadding(int leftPadding, int topPadding, int rightPadding, int bottomPadding) {
        mDrawSelectorByInnerPadding = true;
        mSelectorInnerLeftPadding = leftPadding;
        mSelectorInnerTopPadding = topPadding;
        mSelectorInnerRightPadding = rightPadding;
        mSelectorInnerBottomPadding = bottomPadding;
    }
                        ...
                        if (child != null && !child.hasFocusable()) {
                            ...
                            if (mTouchMode == TOUCH_MODE_DOWN || mTouchMode == TOUCH_MODE_TAP) {
                                ...
                                if (!mDataChanged && mAdapter.isEnabled(motionPosition)) {
                                    ...
                                    }, ViewConfiguration.getPressedStateDuration() * 3);
                                } else {
                                    mTouchMode = TOUCH_MODE_REST;
                                }
                                return true;
