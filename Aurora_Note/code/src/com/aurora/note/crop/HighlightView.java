...
class HighlightView {
	...
	private static final int DEFAULT_HIGHLIGHT_COLOR = 0xFFFFFFFF/*0xFF33B5E5*/;
	private static final float HANDLE_RADIUS_DP = 10f;
	private static final float OUTLINE_DP = 1f;
	private static final float HANDLE_SHADOW_OFFSET_DP = 2.5f;
	...
	private final Paint leftHandlePaint = new Paint();
	private final Paint rightHandlePaint = new Paint();
	private final Paint topHandlePaint = new Paint();
	private final Paint bottomHandlePaint = new Paint();
	...
	public void setup(Matrix m, Rect imageRect, RectF cropRect,
			boolean maintainAspectRatio, IgnoreMode ignoreMode) {
		matrix = new Matrix(m);

		this.cropRect = cropRect;
		this.imageRect = new RectF(imageRect);
		this.maintainAspectRatio = false/*maintainAspectRatio*/;
		...
		float handleShadowOffest = dpToPx(HANDLE_SHADOW_OFFSET_DP);

		leftHandlePaint.setColor(highlightColor);
		leftHandlePaint.setStyle(Paint.Style.FILL);
		leftHandlePaint.setAntiAlias(true);
		leftHandlePaint.setShadowLayer(handleShadowOffest, handleShadowOffest, 0, 0x1A000000);

		rightHandlePaint.setColor(highlightColor);
		rightHandlePaint.setStyle(Paint.Style.FILL);
		rightHandlePaint.setAntiAlias(true);
		rightHandlePaint.setShadowLayer(handleShadowOffest, -handleShadowOffest, 0, 0x1A000000);

		topHandlePaint.setColor(highlightColor);
		topHandlePaint.setStyle(Paint.Style.FILL);
		topHandlePaint.setAntiAlias(true);
		topHandlePaint.setShadowLayer(handleShadowOffest, 0, handleShadowOffest, 0x1A000000);

		bottomHandlePaint.setColor(highlightColor);
		bottomHandlePaint.setStyle(Paint.Style.FILL);
		bottomHandlePaint.setAntiAlias(true);
		bottomHandlePaint.setShadowLayer(handleShadowOffest, 0, -handleShadowOffest, 0x1A000000);
	...
	private void drawHandles(Canvas canvas) {
		...
		if (IgnoreMode.None == ignoreMode) {
			canvas.drawCircle(drawRect.left, yMiddle, handleRadius, leftHandlePaint);
			canvas.drawCircle(xMiddle, drawRect.top, handleRadius, topHandlePaint);
			canvas.drawCircle(drawRect.right, yMiddle, handleRadius, rightHandlePaint);
			canvas.drawCircle(xMiddle, drawRect.bottom, handleRadius, bottomHandlePaint);
		} else if (IgnoreMode.X == ignoreMode) {
			canvas.drawCircle(xMiddle, drawRect.top, handleRadius, topHandlePaint);
			canvas.drawCircle(xMiddle, drawRect.bottom, handleRadius, bottomHandlePaint);
		} else if (IgnoreMode.Y == ignoreMode) {
			canvas.drawCircle(drawRect.left, yMiddle, handleRadius, leftHandlePaint);
			canvas.drawCircle(drawRect.right, yMiddle, handleRadius, rightHandlePaint);
		}
	}
	...
	void growBy(float dx, float dy) {
		if (IgnoreMode.X == ignoreMode) {
			dx = 0;
		} else if (IgnoreMode.Y == ignoreMode) {
			dy = 0;
		} else if (IgnoreMode.XY == ignoreMode) {
			dx = 0;
			dy = 0;
		}
