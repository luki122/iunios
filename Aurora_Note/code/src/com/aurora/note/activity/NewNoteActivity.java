...
//    private static final int AURORA_GEN_PICTURE = 2;
    ...
    private static final int[] BULLET_MENU_SRC_RES = {
            R.drawable.new_note_menu_no_bullet_list_normal,
            R.drawable.new_note_menu_bullet_list_normal,
            R.drawable.new_note_menu_bullet_list_with_indentation_normal
    };
...
//    private static int mImageRightExtraSpace = -1;
    ...
    private void initAuroraMenu() {
        ...
        int menuAnimId = com.aurora.R.style.AuroraMenuLeftBottomAnimation;
        if (type == 1) {
            menuResId = R.menu.pic_note;
        } else if (type == 2) {
            menuResId = R.menu.video_note;
            menuAnimId = com.aurora.R.style.AuroraMenuRightBottomAnimation;
        } else if (type == 4) {
        ...
        if (mLastAuroraMenuResId != menuResId) {
            ...
            getAuroraMenu().setAnimationStyle(menuAnimId);
...
//        return mImageRightExtraSpace;
    ...
    boolean handleDeleteAttachment(final String del_type, final int spanEndIndex, final String imageGroupSource,
        ...
        if (del_type.equals(String.valueOf(Globals.ATTACHMENT_TYPE_IMAGE)) ||
            ...
            title = getString(R.string.dialog_delete_image_title);
            //message = getString(R.string.dialog_delete_image_message);
        } else if (del_type.equals(String.valueOf(Globals.ATTACHMENT_TYPE_VIDEO))) {
        	title = getString(R.string.dialog_delete_video_title);
            //message = getString(R.string.dialog_delete_video_message);
        } else if (del_type.equals(String.valueOf(Globals.ATTACHMENT_TYPE_RECORD))) {
        	title = getString(R.string.dialog_delete_audio_title);
            //message = getString(R.string.dialog_delete_audio_message);
        }
    ...
    private void updateMenuStatus() {
        ...
        if (sign == SIGN_NO_BULLET) {
            if (hasMark) {
                mMarkMenu.setImageResource(R.drawable.new_note_menu_mark_clear_normal);
            } else {
                mMarkMenu.setImageResource(R.drawable.new_note_menu_mark_normal);
            }
    ...
    public void onImageClick() {
        String[] imageMenus;
        if (spanCount > 1) {
            imageMenus = new String[2];
        } else {
            imageMenus = new String[3];
            imageMenus[2] = getString(R.string.menu_crop_image);
        }
        imageMenus[0] = getString(R.string.menu_remove_image);
        imageMenus[1] = getString(R.string.menu_view_image);

        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, R.layout.note_image_menu_item, imageMenus);
...
//            }
    ...
    private String doScreenShot(TextView tv, DoScreenShotResult result) {
        ...
        if (hasError) {
            if (result != null) {
                result.errorInfo = getString(errorInfoResId);
            }
            return null;
        }
...
//                testImageFile(bitmap, Globals.PIC_DIR.getAbsolutePath() + "/");
        	...
        	result.errorInfo = getString(R.string.new_note_gen_pic_error_unknow);
...
//            mLabel1Tv.setText("");
    ...
    private void updateBackgroud(String backgroudPath) {
        ...
        if (TextUtils.isEmpty(backgroudPath)) {
            paperResourceId = R.color.note_paper_color;
    ...
    private int getPaperSignResourceId() {
        ...
        if (TextUtils.isEmpty(mBackgroudPath)) {
            paperSignResourceId = R.drawable.note_paper_sign_00;
    ...
    private int getPaperNoSignResourceId() {
        ...
        if (TextUtils.isEmpty(mBackgroudPath)) {
            paperSignResourceId = R.color.note_paper_color;
        } else if (mBackgroudPath.startsWith(Globals.DRAWABLE_PROTOCOL)) {
            for (int i = 0; i < Globals.NOTE_PAPERS.length; i++) {
                if (Globals.NOTE_PAPERS[i].equals(mBackgroudPath)) {
...
//                });
    ...
    private final Runnable mShowAuroraMenuCommon = new Runnable() {
        ...
        public void run() {
            ...
            // showAuroraMenu();
            if (type == 2) {
                showAuroraMenu(getCurrentView(), Gravity.END | Gravity.BOTTOM, 0, 0);
            } else {
                showAuroraMenu(); 	
            }
            content.setIsHandleEvent(false);
