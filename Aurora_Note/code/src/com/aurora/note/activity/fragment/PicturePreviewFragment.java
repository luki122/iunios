...
public class PicturePreviewFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		int picWidth = getResources().getDisplayMetrics().widthPixels - 
				getResources().getDimensionPixelOffset(R.dimen.picbrowser_padding_left) * 2;
		ViewGroup.LayoutParams params = mImageView.getLayoutParams();
		int[] wh = BitmapUtil.computeWH_4(mImageUrl);
		if (wh[0] != 0 && wh[1] != 0) {
			params.width = picWidth;
			params.height = picWidth * wh[1] / wh[0];
		} else {
			wh = BitmapUtil.computeWH_1(mImageUrl);
			if (wh[0] != 0 && wh[1] != 0) {
				params.width = picWidth;
				params.height = picWidth * wh[1] / wh[0];
			}
