...
public class ImageResizer extends ImageWorker {
	...
	private static final Context mContext = NoteApp.ysApp;
...
//		mContext = context;
	...
	protected Bitmap processBitmap(Object data) {
		...
		/*
		 * 时间长 long time1 = System.currentTimeMillis(); Bitmap tmp =
		 * decodeSampledBitmapFromFile(String.valueOf(data), mImageWidth,
		 * mImageHeight, getImageCache()); Log.i(TAG,
		 * "zll --- processBitmap need time is :"+(System.currentTimeMillis() -
		 * time1)+",name:"+String.valueOf(data));
		 */

		// long time2 = System.currentTimeMillis();
		int type = Integer.parseInt(String.valueOf(data).substring(0, 1));
		data = String.valueOf(data).substring(1);
		Bitmap tmp;
		if (type == 1) {
			Log.i(TAG, "zll ---- processBitmap data:" + String.valueOf(data)
					+ ",mImageWidth:" + mImageWidth + ",mImageHeight:"
					+ mImageHeight);
			tmp = decodeSampledBitmapFromFile(String.valueOf(data),
					mImageWidth, mImageHeight, getImageCache());
			// 如果图片被旋转了，则把它转过来
			int degree = BitmapUtil.readPictureDegree(String.valueOf(data));
			if (degree > 0) {
			    tmp = BitmapUtil.rotateBitmap(tmp, degree);
			}
		} else {
			tmp = BitmapUtil.getRoundVideoThumbnail(String.valueOf(data), mImageWidth, mImageHeight, 
			...
			// id, Thumbnails.MINI_KIND, options);
		}

		// return
		// MediaStore.Images.Thumbnails.getThumbnail(mContext.getContentResolver(),
		// id, Thumbnails.MINI_KIND, options);
		// Log.i(TAG, "zll --- processBitmap size:" +
		// (tmp.getRowBytes()*tmp.getHeight())/1024+",outWidth:"+options.outWidth+",outHeight:"+options.outHeight);

		// Log.i(TAG,
		// "zll --- processBitmap need time is :"+(System.currentTimeMillis() -
		// time2));
		if (tmp == null) {
			Log.i(TAG, "zll --- processBitmap fail -------");
		}
		return tmp;
	}
	...
	public static Bitmap decodeFromSearchDB(String data) {
		BitmapFactory.Options options = new BitmapFactory.Options();
		options.inDither = false;
		...
		return MediaStore.Images.Thumbnails.getThumbnail(
				mContext.getContentResolver(), id, Thumbnails.MINI_KIND,
				options);
	}
	...
	public static Bitmap decodeSampledBitmapFromDescriptor(
		...
		// Calculate inSampleSize
		options.inSampleSize = calculateInSampleSize(options, reqWidth,
				reqHeight);// 使图片变成原来的几分之几：譬如：inSampleSize=2,则宽和高均为1/2,大小为1/4

		// Log.i(TAG,
		// "zll --ccc options outWidth:"+options.outWidth+",outHeight:"+options.outHeight);
		// Decode bitmap with inSampleSize set
