...
public abstract class ImageWorker {
	...
	private Drawable mLoadEmptyDrawable;
	...
	public void loadImage(Object data, ImageView imageView) {
		...
		Log.i(TAG,
				"zll ---aaaaaaaaa String.valueOf(data):" + String.valueOf(data));
		if (mImageCache != null) {
			// Log.i(TAG,
			// "zll ---aaaaaaaaa String.valueOf(data):"+String.valueOf(data));
		...
		if (value != null) {
			...
			// Log.i(TAG, "zll --- bbbbbbbbbbbbb 1");
		} else if (cancelPotentialWork(data, imageView)) {
			// Log.i(TAG, "zll --- bbbbbbbbbbbbb 2");
	...
	public void setLoadEmptyImage(Drawable drawable) {
		mLoadEmptyDrawable = drawable;
	}

	public void setLoadEmptyImage(int resId) {
		mLoadEmptyDrawable = mResources.getDrawable(resId);
	}
	 ...
	 * ImageCache.ImageCacheParams cacheParams) { mImageCacheParams =
	 * cacheParams; mImageCache = ImageCache.getInstance(fragmentManager,
	...
	// private static Long m_sizeLong = (long) 0;
	/**
	...
	private class BitmapWorkerTask extends
		...
		protected BitmapDrawable doInBackground(Object... params) {
			if (BuildConfig.DEBUG) {
				// Log.d(TAG, "zll doInBackground - starting work");
			}
			...
			synchronized (mPauseWorkLock) {
				while (mPauseWork && !isCancelled()) {
					try {
						// Log.i(TAG, "zll ---  doInBackground cccccc 0");
						mPauseWorkLock.wait();
			...
			Log.i(TAG, "zll ---  doInBackground cccccc 0.1 dataString:"
					+ dataString);
			...
			if (mImageCache != null && !isCancelled()
				...
				// Log.i(TAG, "zll ---  doInBackground cccccc 1");
			...
			if (bitmap == null && !isCancelled()
					&& getAttachedImageView() != null && !mExitTasksEarly) {
				// Log.i(TAG, "zll ---  doInBackground cccccc 2");
			...
			if (bitmap != null) {
				// Log.i(TAG, "zll ---aaa size:"+(bitmap.getRowBytes() *
				...
				if (mImageCache != null) {
					// Log.d(TAG,
					// "zll doInBackground - dataString:"+dataString);
					mImageCache.addBitmapToCache(dataString, drawable);
				}
				// m_sizeLong += (bitmap.getRowBytes() *
				// bitmap.getHeight())/1024;
				// Log.i(TAG, "zll --- m_sizeLong:"+m_sizeLong);
				// Log.i(TAG, "zll --- bbb size:"+(bitmap.getRowBytes() *
				// bitmap.getHeight()/1024)+",getWidth:"+bitmap.getWidth()+",getHeight:"+bitmap.getHeight());
			}

			if (BuildConfig.DEBUG) {
				// Log.d(TAG, "zll doInBackground - finished work");
			}
		...
		protected void onPostExecute(BitmapDrawable value) {
			...
			// Log.d(TAG, "zll onPostExecute - starting work");
			...
			if (imageView != null && value == null && mLoadEmptyDrawable != null) {
				imageView.setImageDrawable(mLoadEmptyDrawable);
			}
		}
		...
		protected void onCancelled(BitmapDrawable value) {
			...
			// Log.d(TAG, "zll onCancelled - starting work");
			synchronized (mPauseWorkLock) {
	...
	private void setImageDrawable(ImageView imageView, Drawable drawable) {
		...
		if (mFadeInBitmap) {
			...
			// Log.i(TAG, "zll --- setImageDrawable 1");
		} else {
			imageView.setImageDrawable(drawable);
			// Log.i(TAG, "zll --- setImageDrawable 2");
		}
	...
	public void setPauseWork(boolean pauseWork) {
		synchronized (mPauseWorkLock) {
			// Log.i(TAG, "zll --- setPauseWork pauseWork:"+pauseWork);
