...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageView imageView, DisplayImageOptions options,
		...
		displayImage(uri, new ImageViewAware(imageView), options, null, roundBitmapForCache);
	}

	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
			ImageLoadingListener listener, boolean roundBitmapForCache) {
		checkConfiguration();
		if (imageAware == null) {
			throw new IllegalArgumentException(ERROR_WRONG_ARGUMENTS);
		}
		...
		ImageSize targetSize = ImageSizeUtils.defineTargetSizeForView(imageAware, configuration.getMaxImageSize());
		// String memoryCacheKey = MemoryCacheUtil.generateKey(uri, targetSize);
		String memoryCacheKey = MemoryCacheUtil.generateKey(uri);
		engine.prepareDisplayTaskFor(imageAware, memoryCacheKey);
		...
		if (bmp != null && !bmp.isRecycled()) {
			...
			// 		.getHandler());
			LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(engine, imageLoadingInfo, options
					.getHandler(), roundBitmapForCache);
			if (options.isSyncLoading()) {
				displayTask.run();
			} else {
				engine.submit(displayTask);
