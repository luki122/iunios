...
final class LoadAndDisplayImageTask implements Runnable {
	...
	/*
	 * added by likai on 2014-07-10 begin
	 */
	private boolean roundBitmapForCache = false;

	public LoadAndDisplayImageTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler,
			boolean roundBitmapForCache) {
		this(engine, imageLoadingInfo, handler);
		this.roundBitmapForCache = roundBitmapForCache;
	}
	/*
	 * added by likai on 2014-07-10 end
	 */
	...
	private Bitmap tryLoadBitmap() {
		...
		try {
			if (imageFile.exists()) {
				...
				// Jim added on 2014-05-13 begin
				int degree = BitmapUtil.readPictureDegree(imageFile.getAbsolutePath());
				if (degree > 0) {
				    bitmap = BitmapUtil.rotateBitmap(bitmap, degree);
				}
				// Jim added on 2014-05-13 end
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				...
				if (!checkTaskIsNotActual()) {
					...
					// Jim added on 2014-05-13 begin
	                int degree = BitmapUtil.readPictureDegree(imageUriForDecoding);
	                if (degree > 0) {
	                    bitmap = BitmapUtil.rotateBitmap(bitmap, degree);
	                }
	                // Jim added on 2014-05-13 end
                    ...
                    // added by likai on 2014-07-10 begin
                    if (roundBitmapForCache && bitmap != null && imageAware instanceof ImageViewAware) {
                        bitmap = RoundedBitmapDisplayer.roundCorners(bitmap, (ImageViewAware) imageAware, 10);
                    }
                    // added by likai on 2014-07-10 end
