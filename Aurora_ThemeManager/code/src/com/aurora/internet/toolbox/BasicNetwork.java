...
package com.aurora.internet.toolbox;

import android.os.SystemClock;

import com.aurora.internet.*;
...
/**
 * A network performing internet requests over an {@link HttpStack}.
 */
public class BasicNetwork implements Network {
    protected static final boolean DEBUG = InternetLog.DEBUG;
	...
	@Override
	public NetworkResponse performRequest(Request<?> request) throws InternetError {
		...
		while (true) {
			...
			try {
				...
				InternetLog.e("Unexpected response code %d for %s", statusCode, request.getUrl());
    ...
    private void logSlowRequests(long requestLifetime, Request<?> request,
        ...
        if (DEBUG || requestLifetime > SLOW_REQUEST_THRESHOLD_MS) {
            InternetLog.d("HTTP response for request=<%s> [lifetime=%d], [size=%s], " +
    ...
    private void attemptRetryOnException(String logPrefix, Request<?> request,
            InternetError exception) throws InternetError {
    ...
    protected void logError(String what, String url, long start) {
        ...
        InternetLog.v("HTTP ERROR(%s) %d ms to fetch %s", what, (now - start), url);
    }
