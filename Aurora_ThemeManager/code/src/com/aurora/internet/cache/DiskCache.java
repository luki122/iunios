...
package com.aurora.internet.cache;

import android.os.SystemClock;

import com.aurora.internet.InternetLog;
...
public class DiskCache {
    ...
    public synchronized Entry getEntry(String key) {
        ...
        try {
            ...
            InternetLog.d("%s: %s", file.getAbsolutePath(), e.toString());
            removeEntry(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                InternetLog.e("Unable to create cache dir %s", mRootDirectory.getAbsolutePath());
            }
            return;
        }
    ...
    public synchronized void putEntry(String key, Entry entry) {
        ...
        if (!deleted) {
            InternetLog.d("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    public synchronized void removeEntry(String key) {
        ...
        if (!deleted) {
            InternetLog.d("Could not delete cache entry for key=%s, filename=%s",
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (InternetLog.DEBUG) {
            InternetLog.v("Pruning old cache entries.");
        }
        ...
        if (InternetLog.DEBUG) {
            InternetLog.v("pruned %d files, %d bytes, %d ms",
