package com.aurora.internet.request;
 ...
 * we highly recommend you to use it with the {@link com.aurora.internet.toolbox.FileDownloader},
...
//		addHeader("Accept-Encoding", "identity");
		...
		if (!isCanceled()) {
			if (mTemporaryFile.canRead() && mTemporaryFile.length() > 0) {
				if (mTemporaryFile.renameTo(mStoreFile)) {
					...
					return Response.error(new InternetError("Can't rename the download temporary file!"));
				}
			} else {
				return Response.error(new InternetError("Download temporary file was invalid!"));
			}
		}
		return Response.error(new InternetError("Request was Canceled!"));
	}
	...
	public byte[] handleResponse(HttpResponse response, Delivery delivery) throws IOException, ServerError {
		...
		if (fileSize <= 0) {
			InternetLog.d("Response doesn't present Content-Length!");
		}
