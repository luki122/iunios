...
public class ToCircleView extends View {
	...
	private void init() {
        ...
        getContext().getTheme().resolveAttribute(R.attr.progressBtnMainColor,
                outValue, true);
       int color = outValue.resourceId;
	    ...
	    paint.setColor(getResources().getColor(color));    //设置画笔颜色  
	    ...
	    startPaint.setColor(getResources().getColor(color));    //设置画笔颜色  
	    ...
	    endPaint.setColor(getResources().getColor(color));    //设置画笔颜色  
	    endPaint.setStrokeWidth(0);              //线宽  
	    endPaint.setStyle(Style.FILL);                   //实心效果  
	    
	    minRpx = dip2px(getContext(), minRdp);
	}
