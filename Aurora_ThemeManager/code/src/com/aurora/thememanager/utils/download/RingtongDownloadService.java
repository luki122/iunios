...
public class RingtongDownloadService extends Service {
...
public void onCreate() {
	...
	if (isNeedInitData()) {
		...
		mDownloadDaseController = DatabaseController.getController(this, DatabaseController.TYPE_RINGTONG_DOWNLOAD);
...
public static Map<Integer, FileDownloader> getDownloaders(Context context) {
	if (null == sDownloaders) {
		...
		if (context.getApplicationContext() != null) {
			Intent i = new Intent(context.getApplicationContext(), RingtongDownloadService.class);
...
public static void checkInit(Context context, DownloadInitListener serviceListener) {
	...
	if (isNeedInitData()) {
		...
		Intent initIntent = new Intent(context, RingtongDownloadService.class);
...
public static void startDownload(Context context, DownloadData downloadData) {
	Intent startDownload = new Intent(context, RingtongDownloadService.class);
	Bundle startDownloadBundle = new Bundle();
	startDownloadBundle.putParcelable(RingtongDownloadService.DOWNLOAD_DATA, downloadData);
	startDownloadBundle.putInt(RingtongDownloadService.DOWNLOAD_OPERATION, 
			RingtongDownloadService.OPERATION_START_DOWNLOAD);
...
public static void pauseOrContinueDownload(Context context, DownloadData downloadData) {
	...
	startDownloadBundle.putParcelable(RingtongDownloadService.DOWNLOAD_DATA, downloadData);
	startDownloadBundle.putInt(RingtongDownloadService.DOWNLOAD_OPERATION, 
			RingtongDownloadService.OPERATION_PAUSE_CONTINUE_DOWNLOAD);
	startDownload.putExtras(startDownloadBundle);
	context.startService(startDownload);
}
...
public static void cancelDownload(Context context, DownloadData downloadData) {
	...
	Bundle startDownloadBundle = new Bundle();
	startDownloadBundle.putParcelable(RingtongDownloadService.DOWNLOAD_DATA, downloadData);
	startDownloadBundle.putInt(RingtongDownloadService.DOWNLOAD_OPERATION, RingtongDownloadService.OPERATION_CANCLE_DOWNLOAD);
	startDownload.putExtras(startDownloadBundle);
	context.startService(startDownload);
}
