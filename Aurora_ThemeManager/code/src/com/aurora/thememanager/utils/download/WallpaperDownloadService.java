...
public void onCreate() {
	...
	if (isNeedInitData()) {
		...
		mDownloadDaseController = DatabaseController.getController(this, DatabaseController.TYPE_WALLPAPER_DOWNLOAD);
...
public static Map<Integer, FileDownloader> getDownloaders(Context context) {
	if (null == sDownloaders) {
		...
		if (context.getApplicationContext() != null) {
			Intent i = new Intent(context.getApplicationContext(), WallpaperDownloadService.class);
...
public static void checkInit(Context context, DownloadInitListener serviceListener) {
	...
	if (isNeedInitData()) {
		...
		Intent initIntent = new Intent(context, WallpaperDownloadService.class);
...
public static void startDownload(Context context, DownloadData downloadData) {
	Intent startDownload = new Intent(context, WallpaperDownloadService.class);
	Bundle startDownloadBundle = new Bundle();
	startDownloadBundle.putParcelable(WallpaperDownloadService.DOWNLOAD_DATA, downloadData);
	startDownloadBundle.putInt(WallpaperDownloadService.DOWNLOAD_OPERATION, 
			WallpaperDownloadService.OPERATION_START_DOWNLOAD);
...
public static void pauseOrContinueDownload(Context context, DownloadData downloadData) {
	...
	startDownloadBundle.putParcelable(WallpaperDownloadService.DOWNLOAD_DATA, downloadData);
	startDownloadBundle.putInt(WallpaperDownloadService.DOWNLOAD_OPERATION, 
			WallpaperDownloadService.OPERATION_PAUSE_CONTINUE_DOWNLOAD);
	startDownload.putExtras(startDownloadBundle);
	context.startService(startDownload);
}
...
public static void cancelDownload(Context context, DownloadData downloadData) {
	...
	Bundle startDownloadBundle = new Bundle();
	startDownloadBundle.putParcelable(WallpaperDownloadService.DOWNLOAD_DATA, downloadData);
	startDownloadBundle.putInt(WallpaperDownloadService.DOWNLOAD_OPERATION, WallpaperDownloadService.OPERATION_CANCLE_DOWNLOAD);
	startDownload.putExtras(startDownloadBundle);
	context.startService(startDownload);
}
