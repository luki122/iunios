...
public class DownloadManager {
	...
	private int mType = FileDownloader.TYPE_NORMAL;
	...
	public void updateProgress(DownloadData data, OnClickListener onClickListener) {
		...
		FileDownloader downloader = null;
		 ...
		 if(type == Theme.TYPE_RINGTONG){
			 ...
			 tempData = RingtongDownloadService.getDownloadController().getDownloadData(data.downloadId);
		 }else if(type == Theme.TYPE_THEME_PKG){
			 dbController = DownloadService.getDownloadController();
			 downloader =  DownloadService.getDownloaders(mContext).get(data.downloadId);
			 tempData = DownloadService.getDownloadController().getDownloadData(data.downloadId);
			 ...
			 tempData = TimeWallpaperDownloadService.getDownloadController().getDownloadData(data.downloadId);
		 }else{
			 dbController = WallpaperDownloadService.getDownloadController();
			 downloader =  WallpaperDownloadService.getDownloaders(mContext).get(data.downloadId);
			 tempData = WallpaperDownloadService.getDownloadController().getDownloadData(data.downloadId);
		 }
		 
	   dbController.openDatabase();
	   
	    boolean appied  = dbController.getApplied(data.downloadId);
	    Log.d(TAG, "appied:"+appied+"   themeId:"+data.downloadId);
		// 未使用的情况
		if (!appied) {
			
			
			// 如果下载器任务存在, 显示各状态信息
			Log.d(TAG, "hasDownloader:"+(downloader != null));
			if (downloader != null) {
				...
				Log.d(TAG, "has tempData:"+(tempData == null) );
				if (tempData == null) {
					showOperationDownload(data);
				} else {
					...
					.setPositiveButton(android.R.string.ok,
							new DialogInterface.OnClickListener() {
								...
								public void onClick(
									...
									DownloadService.pauseOrContinueDownload(context,
