...
import com.aurora.thememanager.utils.TimeUtils;
...
public class NetWorkReceiver extends BroadcastReceiver {
	...
	/*	boolean isfirst = sp.getBoolean(Globals.SHARED_MOBILE_DISCONNECT_COUNT,
		...
		/*	ed.putBoolean(Globals.SHARED_WIFI_DISCONNECT_ISEXITS, true);
			...
			Intent networkChange = new Intent(context, DownloadService.class);
			networkChange.putExtra(DownloadService.DOWNLOAD_OPERATION,
					DownloadService.OPERATION_NETWORK_MOBILE_PAUSE);
		...
		else if((netstatus ==2) && (isCurrentNetStatus ==1))
		{
			Intent networkChange = new Intent(context, DownloadService.class);
			networkChange.putExtra(DownloadService.DOWNLOAD_OPERATION,
					DownloadService.OPERATION_NETWORK_MOBILE_CONTINUE);
			context.startService(networkChange);
		}
		...
		/*else if ((iswifi) && (!isMobile)) {
			...
			Intent networkChange = new Intent(context, DownloadService.class);
			networkChange.putExtra(DownloadService.DOWNLOAD_OPERATION,
					DownloadService.OPERATION_NETWORK_CHANGE);
			context.startService(networkChange);
		}
			...
			if (!time.equals(TimeUtils.getStringDateShort())) {
				...
				new Thread() {
					...
					public void run() {/*
						...
						ArrayList<Theme> down_theme = up_data.getUpdateData(m_context);

						if ((null != down_theme) && (down_theme.size() > 0)) {
//							if (bl) {
