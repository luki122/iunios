...
public class WallPaperPreviewActivity extends BaseActivity implements DownloadStatusCallback{
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		mThemeManager.setCallBack(new ThemeOperationCallBack() {
			...
			public void onCompleted(final boolean success, int statusCode) {
				...
				if(success){
					ThemeManager.restartApplications(WallPaperPreviewActivity.this, mCurrentTheme.themeId);
        ...
        if (mIntent != null) {
        	mCurrentTheme = mIntent
					.getParcelableExtra(Action.KEY_SHOW_WALL_PAPER_PREVIEW);
            ...
            if (mCurrentTheme != null && mCurrentTheme.previews != null) {
                ...
                mPreviewAdapter = new WallpaperPreviewAdapter(getApplicationContext(), mPreviews);
    ...
    private void setupView() {
        ...
        mPreView = (ImageView) findViewById(R.id.wallpaper_preview_show);
...
//		}
    ...
    private void initAuroraActionBar() {
    	...
    	authorName.setText(mCurrentTheme.author);
	...
	private void setWallpaper() {
		...
		try {
			InputStream is = null;
			String filename = ThemeConfig.THEME_WALL_PAPER_DOWNLLOAD_PATH
					+ mCurrentTheme.downloadPath.substring(mCurrentTheme.downloadPath.lastIndexOf("/")+1, mCurrentTheme.downloadPath.length());
