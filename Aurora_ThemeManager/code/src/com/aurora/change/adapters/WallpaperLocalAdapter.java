...
import com.aurora.thememanager.utils.StringUtils;
...
//        mImageLoader = new ImageLoader(context);
    ...
    public WallpaperLocalAdapter(Context context, List<PictureGroupInfo> groupInfos,
            List<List<PictureInfo>> pictureInfos,boolean hasHeaderView) {
        this.mGroupInfos = groupInfos;
        this.mPictureInfos = pictureInfos;
        this.mContext = context;
        mInflater = ( LayoutInflater ) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
//        mImageLoader = new ImageLoader(context);
    ...
    public void deleteItem(List<PictureGroupInfo>  group){
    	for(PictureGroupInfo g:group){
    		boolean deleted  = mGroupInfos.remove(g);
    	}
    	onResume();
    }
    ...
    public List<PictureGroupInfo> getSelectedItems(){
    	int size = mChoice.size();
    	if(size < 1){
    		return null;
    	}
    	ArrayList<PictureGroupInfo> results = new ArrayList<PictureGroupInfo>();
		...
		while(in.hasNext()){
			...
			if(selected){
				PictureGroupInfo item = (PictureGroupInfo) getItem(position);
				results.add(item);
			}
		}
    	return results;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (itemType == SINGLE_TYPE) {
			...
			if (singleView == null || (holder != null && holder.mViewType != SINGLE_TYPE)) {
                ...
                holder.mEditBox = (AuroraCheckBox)view.findViewById(R.id.lockpaper_item_selected);
                ...
                holder.topDivider = view.findViewById(R.id.top_divider_view);
        ...
        String customName = mContext.getString(R.string.wallpaper_crop_custom_name);
        if(!groupName.startsWith("Custom Series") && !groupName.contains(customName)){
        	groupName = StringUtils.deleteNumberTail(groupName);
        }
...
//        	setImagePre(holder.mLockpaperPreImg1, mPictureInfos.get(position).get(0), position);
        ...
        holder.mEditBox.setVisibility(mEditMode?View.VISIBLE:View.GONE);
        holder.mLockpaperSettings.setVisibility(mEditMode?View.GONE:View.VISIBLE);
        if(mEditMode){
	        Boolean selected = mChoice.get(position);
			if(selected == null){
				selected = false;
			}
			holder.mEditBox.setChecked(selected);
...
//        mGroupInfos = mDbControl.queryAllGroupInfos();
	...
	public void selectedItem(int position,boolean selected) {
		...
		PictureGroupInfo groupInfo = (PictureGroupInfo) getItem(position);
		 boolean editable = (getGroupState().get(String.valueOf(position)))
                   || (groupInfo.getSystem_flag() == 1);
		 if(editable){
			 mChoice.put(position, false);
		 }else{
			 mChoice.put(position, selected);
		 }
	...
	public void selectedItemOrNot(int position) {
		...
		PictureGroupInfo groupInfo = (PictureGroupInfo) getItem(position);
		 ...
		 if(editable){
			 mChoice.put(position, false);
			 ToastUtils.showToast(mContext, R.string.tip_delelte_system_timewallpaper);
		 }else{
	...
	private void selectAllOrNot(boolean select){
		...
		for(int i = 0;i< size;i++){
			PictureGroupInfo groupInfo = (PictureGroupInfo) getItem(i);
			 boolean editable = (getGroupState().get(String.valueOf(i)))
	                    || (groupInfo.getSystem_flag() == 1);
			 if(editable){
				 mChoice.put(i, false);
			 }else{
