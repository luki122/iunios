...
//	        if (DEBUG_VOL) 
	        ...
	        if (mStreamVolumeAlias[streamType] == AudioSystem.STREAM_RING) {
	        	...
	        	try {
					Method m = clz.getDeclaredMethod("notifyRingerVolumeChanged", int.class,int.class,String.class);
					if(m != null){
						ClassHelper.callMethod(obj, "notifyRingerVolumeChanged", oldIndex, index, null);
					}
...
//	                            rcse.mPlaybackStream = value;
	                            break;
	                        case PLAYBACKINFO_PLAYSTATE:
	                        	ClassHelper.setIntField(rcse, "mPlaybackState", value);
//	                            rcse.mPlaybackState = value;
	                            synchronized (mMainRemote) {
	                            	int mrccId = ClassHelper.getIntField(mMainRemote, "mRccId");
	                                if (rccId == mrccId) {
	                                	int result=  (Integer) ClassHelper.callMethod(obj, "isPlaystateActive", value);
	                                	ClassHelper.setIntField(obj, "mMainRemoteIsActive", result);
...
//	        ensureValidStreamType(streamType);
	       ...
	       final int aliasIndex = (Integer) ClassHelper.callMethod(streamState, "getIndex", device,(value != 0));
//	        final int aliasIndex = streamState.getIndex(device,(streamState.muteCount() != 0) /* lastAudible */);
	        ...
	        if ((direction == AudioManager.ADJUST_RAISE) &&
	                !(Boolean)ClassHelper.callMethod(obj, "checkSafeMediaVolume", streamTypeAlias, aliasIndex + step, device)
	        ...
	        if ((streamTypeAlias == AudioSystem.STREAM_MUSIC) &&
	            ...
	            if (((flags & AudioManager.FLAG_ALLOW_RINGER_MODES) != 0) ||
	                    (streamTypeAlias == (Integer)ClassHelper.callMethod(obj, "getMasterStreamType")/*getMasterStreamType()*/)) {
	                ...
	                int mRingerMode = ClassHelper.getIntField(obj, "mRingerMode");
	                if ((streamTypeAlias == getMasterStreamType) &&
	                        (mRingerMode == AudioManager.RINGER_MODE_SILENT)) {
	                	ClassHelper.callMethod(streamState, "setLastAudibleIndex", 0, device);
//	                    streamState.setLastAudibleIndex(0, device);
	                }
	            }

	            // If stream is muted, adjust last audible index only
	            VolumeStreamState state = mStreamStates[streamType];
	            
	            int oldvalue = (Integer) ClassHelper.callMethod(state, "muteCount");
	            oldIndex = (Integer) ClassHelper.callMethod(state, "getIndex", device,(oldvalue != 0));
...
//	                if (DEBUG_VOL) Log.d(TAG, "adjustStreamVolume: MuteCount > 0.");
	                	ClassHelper.callMethod(streamState, "adjustLastAudibleIndex", direction * step, device);
//	                    streamState.adjustLastAudibleIndex(direction * step, device);
	                ...
	                VolumeStreamState state2 = mStreamStates[streamType];
	                index = (Integer) ClassHelper.callMethod(state2, "getIndex", device, true);//mStreamStates[streamType].getIndex(device, true  /* lastAudible */);
	            } else {
...
//	                if (DEBUG_VOL) Log.d(TAG, "adjustStreamVolume: MuteCount = 0.");
	                ...
	                VolumeStreamState state3 = mStreamStates[streamType];
	                index = (Integer) ClassHelper.callMethod(state3, "getIndex", device, false);//mStreamStates[streamType].getIndex(device, false  /* lastAudible */);
	            }
	        }
	        Log.e("volume", "adjustStreamVolume----->"+"type:"+streamType+"oldIndex:"+oldIndex+" index:"+index);
