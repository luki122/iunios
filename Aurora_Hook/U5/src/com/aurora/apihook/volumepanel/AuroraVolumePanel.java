...
//        listenToRingerMode();
    ...
    private void reorderSliders(int activeStreamType) {
        ...
        if (active == null) {
            if(LOGD)Log.e("VolumePanel", "Missing stream type! - " + activeStreamType);
            mActiveStreamType = -1;
        } else {
    ...
    private void updateSlider(StreamControl sc) {
    	...
    	if(LOGD)Log.e(TAG, "index:"+progress);
    ...
    public void postVolumeChanged(int streamType, int flags) {
        ...
        synchronized (mLock) {
            if (mStreamControls == null) {
            	if(LOGD)Log.e("volume", "mStreamControls == null");
                createSliders();
            }
        }
        if(LOGD)Log.e("volume", "mStreamControls != null  streamType:"+streamType);
    ...
    public void postMuteChanged(int streamType, int flags) {
    	if(LOGD)Log.d(TAG, "hasMessages:"+hasMessages(MSG_VOLUME_CHANGED));
        if (hasMessages(MSG_VOLUME_CHANGED)) return;
        synchronized (mLock) {
            if (mStreamControls == null) {
                createSliders();
...
//                setRingerIcon();
        ...
        if (sc != null) {
            ...
            if(LOGD)Log.d(TAG, "isMuted:"+isMuted(streamType));
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case MSG_NOTIFICATION_EFFECTS_SUPPRESSOR_CHANGED: {
                if (mDialog.isShowing()) {
                	if(LOGD)Log.d(TAG, "MSG_RINGER_MODE_CHANGED:");
                    updateStates();
                }
            	}
                break;
...
//                }
    ...
    public void onProgressChanged(SeekBar seekBar, int progress,
        ...
        final StreamControl sc = (StreamControl) tag;
