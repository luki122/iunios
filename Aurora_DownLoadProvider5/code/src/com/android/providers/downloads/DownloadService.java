...
public class DownloadService extends Service {
    ...
    public static final int maxConcurrent = 3;//add by JXH
...
//                com.android.internal.R.integer.config_MaxConcurrentDownloadsAllowed);
    ...
    public void enqueueUpdate() {
        if (mUpdateHandler != null) {
			...
			//paul modify for BUG15036 <2015-08-05> start
//			Message msg = mUpdateHandler.obtainMessage(MSG_UPDATE, mLastStartId, -1);
//			mUpdateHandler.sendMessageDelayed(msg, 1000);
            mUpdateHandler.obtainMessage(MSG_UPDATE, mLastStartId, -1).sendToTarget();
			//paul modify for BUG15036 <2015-08-05> end
        }
    }
    ...
    private Handler.Callback mUpdateCallback = new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            ...
            Log.d(Constants.TAG, "----isActive:"+isActive);
            if (isActive) {
    ...
    private boolean updateLocked() {
        ...
        Log.d(Constants.TAG, "-----------------------staleIds:"+staleIds.size()+" mDownloads:"+mDownloads.size());
        ...
        try {
            ...
            Log.d(Constants.TAG, " -----cursor:"+cursor.getColumnCount());
            while (cursor.moveToNext()) {
                ...
                if (info.mDeleted) {
                	 Log.d(Constants.TAG, "-----------------------info.mFileName:"+info.mFileName);
                    ...
                    int del= resolver.delete(info.getAllDownloadsUri(), null, null);
                    //add by JXH begin
                    if(del>0){
                    	mDownloads.remove(id);
                    }
                    Log.d(Constants.TAG, "-----------------------del:"+del+" info.getAllDownloadsUri():"+info.getAllDownloadsUri());
                  //add by JXH end
                } else {
        ...
        for (Long id : staleIds) {
        	Log.d("JXH", "---------deleteDownloadLocked-----id:"+id);
    ...
    private void updateDownload(DownloadInfo.Reader reader, DownloadInfo info, long now) {
        ...
        if (Constants.LOGVV) {
            Log.v(Constants.TAG, "processing updated download " + info.mId +" mFileName:"+info.mFileName+
                    ", status: " + info.mStatus);
        }
    }
