 ...
 * </div>
 */
public class AuroraDialog extends Dialog implements DialogInterface, Window.Callback,
        KeyEvent.Callback, OnCreateContextMenuListener,AnimationListener {
...
//            applyAnimation();
     ...
     *            uses the window manager and theme in this context to present
     *            its UI.
     */
     ...
     * 
     * @param context The Context in which the Dialog should run. In particular,
     *            it uses the window manager and theme from this context to
     *            present its UI.
    ...
    protected AuroraDialog(Context context, int theme, boolean createContextThemeWrapper) {
        super(context, theme);
        if (createContextThemeWrapper) {
            if (theme == 0) {
                // TypedValue outValue = new TypedValue();
                // context.getTheme().resolveAttribute(com.aurora.lib.R.attr.auroraDialogTheme,
                // outValue, true);
                // theme = outValue.resourceId;
                theme = com.aurora.lib.R.style.AuroraAlertDialogTheme;
            }
    ...
    /**
     * if you want to not only show a dialog but also play it's animation with 3d,
     * you should call this method before show();
     */
...
//            sendShowMessage();
    ...
    private void applyAnimation() {
        View dialogContentView = mDecor.findViewById(com.aurora.lib.R.id.aurora_parentPanel);
//        if (dialogContentView != null) {
//            applyEnterAnimation(mRotationDegree, 0, dialogContentView);
//
//        }
        View datePickerContent = mDecor
                .findViewById(com.aurora.lib.R.id.aurora_date_picker_layout);
        View timePickerContent = mDecor
                .findViewById(com.aurora.lib.R.id.aurora_time_picker_layout);
...
//        }
     ...
     *            the hosting activity was previously shut down, holds the
     *            result from the most recent call to
     *            {@link #onSaveInstanceState}, or null if this is the first
     *            time.
     */
        ...
        while (activity == null && context != null) {
            if (context instanceof Activity) {
                ...
                context = (context instanceof ContextWrapper) ?
                        ((ContextWrapper) context).getBaseContext() : // unwrap
                                                                      // one
                                                                      // level
                        null; // done
            }
