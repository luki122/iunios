...
class StkAppInstaller {
    ...
    private static void setAppState(Context context, boolean install, int sim_id) {
        ...
        if(service != null && sim_id != STK_MENU_LAUNCH_ID){
        	 if(service.StkQueryAvailable(0) != StkAppService.STK_AVAIL_AVAILABLE && service.StkQueryAvailable(1) != StkAppService.STK_AVAIL_AVAILABLE) {
        		 getInstance().unInstall(context, STK_MENU_LAUNCH_ID);
        	 } else {
        		 getInstance().install(context, STK_MENU_LAUNCH_ID);
        	 }
        }
    }
