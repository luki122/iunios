...
public class FileIconLoader implements Callback {
	...
	private static class BitmapHolder extends ImageHolder {
		...
		public boolean setImageView(ImageView v) {
			if (bitmapRef.get() == null)
				return false;
			// LogUtil.log(TAG,
			// "v.getTag()=="+v.getTag()+" bitmapRef.get()=="+bitmapRef.get());
	...
	private static Drawable apkCache;
	...
	public FileIconLoader(Context context) {
		mContext = context;
		if (apkCache == null) {
			apkCache = context.getResources().getDrawable(
	...
	private boolean loadCachedIcon(ImageView view, String path,
		...
		if (holder == null) {
			...
			if (holder.isNull()) {
				// LogUtil.elog(TAG, " holder.isNull()");
				return true;
			}
			...
			if (view != null && view.getTag() != null
				...
				if (holder.setImageView(view)) {
					// LogUtil.elog(TAG, " true path==" + path);
					return true;
				}
			}
		}
...
//			LogUtil.log(TAG, "MESSAGE_ICON_LOADED mPaused==" + mPaused);
			if (!mPaused) {
				processLoadedIcons();
			}
			return true;
	...
	private void processLoadedIcons() {
		...
		while (iterator.hasNext()) {
			...
			if (fileId.mPath.equals(view.getTag())) {
	...
	private WeakHashMap<String, Integer> weakHashMap = new WeakHashMap<String, Integer>();
		...
		public boolean handleMessage(Message msg) {
			...
			while (iterator.hasNext()) {
				...
				if (holder != null && holder.state == ImageHolder.NEEDED) {
					// LogUtil.log(TAG, "id.mPath==" + id.mPath
					...
					case Apk:
						...
						if(Util.isLowVersion()){
							 icon = Util.getAPKIcon(mContext, id.mPath);
						}else {
							icon = Util.getApkIconForLollipop(mContext, id.mPath);
						}
						if (icon == null) {
							holder.setImage(apkCache);
					...
					case Video:
						...
						if (id.mId == 0) {
							int count = weakHashMap.get(id.mPath) == null ? 0
									: weakHashMap.get(id.mPath);
//							LogUtil.w(TAG, "Fail to get dababase id for:"
//									+ id.mPath + " count==" + count
//									+ " weakHashMap==" + weakHashMap.size());
							if (count > 300) {
								mImageCache
										.remove(id.mPath, ImageHolder.NEEDED);
								isload = true;
							}
							count++;
							weakHashMap.put(id.mPath, count);
						}
						Bitmap bitmap = null;
						if (isVideo) {
						...
						// LogUtil.log(TAG, "get dababase id=" + id.mId
						// + " for:" + id.mPath + " bitmap==" +
						// bitmap+" isVideo=="+isVideo);
						if (bitmap != null) {
