...
public class AuroraFileBrowserAdater extends FileBaseAdapter {
	...
	public void setItemViewData(AuroraFilesItemView itemView, final FileInfo fileInfo) {
		...
		if (fileInfo != null) {
			if (isRoot()) {
				fileName.setText(getActivity().getStorageNodeName(fileInfo.filePath));
				modifiedTime.setVisibility(View.GONE);
				fileCount.setVisibility(View.GONE);
				fileSize.setVisibility(View.GONE);

				if (fileInfo.filePath.equals(FileExplorerTabActivity.mSDCardPath)) {
					itemView.getFile_image().setImageResource(R.drawable.internal_storage_light);
				} else {
					itemView.getFile_image().setImageResource(R.drawable.sd_card_light);
				}
			} else {
				if (fileInfo.IsDir) {
					...
					if (fileInfo.Count == -1) {
						if (fileInfo.filePath != null) {
							Integer v = cacheByInteger.getBitmapFromMemCache(fileInfo.filePath + fileInfo.ModifiedDate);
...
//							LogUtil.d(TAG, " ----" + fileInfo.filePath + fileInfo.ModifiedDate + " fileCount:" + v);
				...
				fileName.setText(Util.getEllipsize(fileInfo.fileName, itemView.getFile_name(), getMaxLength()));
				modifiedTime.setText(Util.formatInfoDateString(getActivity(), fileInfo.ModifiedDate));
	 ...
	 * @param normal
	 */
	private void setTextViewColor(TextView fileName, TextView fileSize, TextView fileCount, TextView modifiedTime, boolean normal) {
		setTextViewColor(fileName, fileSize, modifiedTime, normal);
		if (normal) {
			fileCount.setTextAppearance(getActivity(), R.style.aurora_file_name_normal);
		} else {
			fileCount.setTextAppearance(getActivity(), R.style.aurora_file_name_select);
		}
	}
	 ...
	 * @param normal
	 */
	private void setTextViewColor(TextView fileName, TextView fileSize, TextView modifiedTime, boolean normal) {
		if (normal) {
			fileName.setTextAppearance(getActivity(), R.style.aurora_file_name_normal);
			fileSize.setTextAppearance(getActivity(), R.style.aurora_file_other_normal);
			modifiedTime.setTextAppearance(getActivity(), R.style.aurora_file_other_normal);
		} else {
			if (!isRoot()) {
				fileName.setTextAppearance(getActivity(), R.style.aurora_file_name_select);
			} else {
				fileName.setTextAppearance(getActivity(), R.style.aurora_file_name_select_rlayout);
			}
			fileSize.setTextAppearance(getActivity(), R.style.aurora_file_other_select);
			modifiedTime.setTextAppearance(getActivity(), R.style.aurora_file_other_select);
		}
	}
