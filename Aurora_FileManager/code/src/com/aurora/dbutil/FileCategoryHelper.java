...
public class FileCategoryHelper {
	 ...
	 * 存储详情分类
	 */
	public static FileCategory[] sCategories = new FileCategory[] { FileCategory.Music, FileCategory.Video, FileCategory.Picture, FileCategory.Apk, FileCategory.Doc, FileCategory.Other };
	...
	private void refreshFileCategory(final FileCategory fc, final Uri uri, final String sdPath, long delayMillis) {
		...
		backgroundQueryHandler.postDelayed(new Runnable() {
			@Override
			public void run() {
				backgroundQueryHandler.startQuery(FILE_SD_CATEGORY_NUM_TOKEN, fc, sdPath, uri, columns, buildSelectionByCategoryFromSdType(fc, sdPath), null, null);
			}
		}, delayMillis);
	}
	...
	public void query(FileCategory fc, SortMethod sort) {
		...
		if (fc == FileCategory.Video || fc == FileCategory.Music) {
			columns = new String[] { MediaColumns._ID, MediaColumns.DATA, MediaColumns.SIZE, MediaColumns.DATE_MODIFIED, MediaColumns.DISPLAY_NAME, MediaColumns.TITLE };
		} else if (fc == FileCategory.Picture) {
			columns = new String[] { MediaColumns._ID, MediaColumns.DATA, MediaColumns.SIZE, MediaColumns.DATE_MODIFIED, MediaColumns.TITLE, FILECOLUMNS_ORIENTATION };
		} else {
	...
	public void searchQuery(String key) {
		...
		where += " and " + FileColumns.DATA + " not like LOWER('%/android%') and " + FileColumns.DATA + " not like LOWER('%/cache%') and " + FileColumns.DATA + " not like LOWER('%/.%'))";
		List<FileInfo> fileInfos = activity.getStorages();
	...
	private synchronized String buildSelectionByCategory(FileCategory fc, String key) {
		...
		return buildSelectionByCategory(fc) + " and " + FileColumns.DATA + " LIKE '" + key + "%'  and bucket_display_name = '" + Util.getNameFromFilepath(key) + "' ";
	}
	...
	private String buildSelectionByCategoryFromSdType(FileCategory fc, String sdPath) {
		...
		case Doc:
			...
			break;
		case Picture:
			selection += " (" + MediaColumns.SIZE + " > " + AuroraConfig.AURORA_PIC_SIZE + " or " + FileColumns.DATA + " LIKE '" + FileExplorerTabActivity.mSDCardPath + "/QQ_Screenshot%' "
					+ " ) and " + FileColumns.DATA + " NOT LIKE '%/aurora/change/lockscreen%' and " + FileColumns.DATA + " NOT LIKE '" + FileExplorerTabActivity.mSDCardPath
					+ "/iuni/wallpaper/save%' and " + FileColumns.DATA + " NOT LIKE '%.pcx' and " + FileColumns.DATA + " NOT LIKE '%.tif' ";
			// modify by Jxh end
			break;
		case Music:
			selection += MediaColumns.SIZE + " > " + AuroraConfig.AURORA_VIDEO_SIZE + " and " + FileColumns.DATA + " NOT LIKE '" + FileExplorerTabActivity.mSDCardPath + File.separator
					+ context.getResources().getString(R.string.phone_audios) + "%' and " + FileColumns.DATA + " NOT LIKE '" + FileExplorerTabActivity.mSDCardPath + "/VoiceWakeUp%' and "
					+ FileColumns.DATA + " NOT LIKE '" + FileExplorerTabActivity.mSDCardPath + "/note/sound%' ";
			break;
		case Video:
		...
		if (fc != FileCategory.DownLoad) {
			String temp = FileColumns.DATA + " NOT LIKE '" + FileExplorerTabActivity.mSDCardPath + File.separator + "Android/%'" + " and " + FileColumns.DATA + " NOT LIKE ('%/.%')";
	...
	private synchronized String buildSelectionByCategory(FileCategory fc) {
		...
		List<FileInfo> fileInfos = activity.getStorages();
		LogUtil.d(TAG, "-----sd :" + fileInfos.size());
		int sizs = fileInfos.size();
		if (sizs == 0) {
			return result;
		}
	...
	private String buildDocSelection() {
		...
		return selection + "(" + FileColumns.DATA + " LIKE '%.pptx') OR " + "(" + FileColumns.DATA + " LIKE '%.xlsx') OR " + "(" + FileColumns.DATA + " LIKE '%.docx'))";
	}
			...
			case FILE_CATEGORY_NUM_TOKEN:
				...
				if (cursor != null && !cursor.isClosed()) {
					if (((FileCategory) cookie == FileCategory.DownLoad)) {
						...
						// LogUtil.log(TAG, (FileCategory) cookie + " count=="
						// + count);
						...
						// LogUtil.log(TAG, (FileCategory) cookie
						// + " totalCount==" + totalCount + " totalSize=="
						// + totalSize);
