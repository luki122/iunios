...
public class PictureCategoryFragment extends AuroraFragment {
	...
	private void editState() {
		...
		if (!isDestroy) {
			if (getGridView().getCount() == 0) {
				...
				if (getFileExplorerActivity().getIsFromWallpaperManager() != null && getFileExplorerActivity().getIsFromWallpaperManager()) {
		...
		public void handleMessage(Message msg) {
			...
			case picInfos:
				...
				if (getFileExplorerActivity().isPicItemBack()) {// 是从ITEM 返回
					getGridView().setSelection(getFileExplorerActivity().getSelectPicItem());
				}
				editState();
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		// 初始化2格Picture适配器
		auroraPicTwoAdapter = new AuroraPicTwoAdapter(categoryPiclistMap, picFilePath, getFileExplorerActivity(), this);
		getGridView().setAdapter(auroraPicTwoAdapter);
		// modify by JXH 2014-7-28 begin
		SoftReference<ConcurrentHashMap<String, List<FileInfo>>> cate = getFileExplorerActivity().getCategoryPiclistMapCache();
		SoftReference<ConcurrentHashMap<Integer, String>> path = getFileExplorerActivity().getPicFilePathCache();
		SoftReference<List<FileInfo>> cacheFileInfo = getFileExplorerActivity().getCachePic();
		if (path != null && cate != null && cacheFileInfo != null) {
			...
			if (tempCate != null && tempInfos != null && tempPath != null) {
				...
				if (getFileExplorerActivity().isPrivacy()) {
					List<FileInfo> pList = getFileExplorerActivity().getHashMap().get(FileCategory.Picture);
					if (pList != null) {
						String pathTemp = FileExplorerTabActivity.getPrivacyMedioPath(FileCategory.Picture);
					}
				}
		...
		if (!AuroraConfig.isOtherAppDel(getFileExplorerActivity()) || getFileExplorerActivity().isPicItemBack()) {
			refreshCategoryPics();
		}
		...
		// 初始化、设置actionBar回调
		getAuroraActionBar().initActionBottomBarMenu(R.menu.aurora_operation_bar_menu, 1);

		// 多选图
		if (getFileExplorerActivity().getIsFromOtherAPP() && getFileExplorerActivity().getIsGetMorePic()) {
			getAuroraActionBar().changeItemLayout(R.layout.aurora_more_pic, R.id.aurora_action_bar_all_edit);
	...
	public void auroraOnItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		getFileExplorerActivity().setSelectPicItem(getGridView().getFirstVisiblePosition());
		// 选择壁纸----废弃
		if (getFileExplorerActivity().getIsFromWallpaperManager() != null && getFileExplorerActivity().getIsFromWallpaperManager()) {
		...
		getSelectPageInterface().selectPage(AuroraConfig.AURORA_PIC_ITEM_PAGE, null);
	...
	private void changeActionBarTextView() {
		...
		if (picPath != null && picPath.size() > 0) {
			textView.setText(String.format(getFileExplorerActivity().getString(R.string.have_select_pic), picPath.size()));
	...
	public void onRightClick() {
		...
		if (getFileExplorerActivity().isPrivacy() && picFilePath.contains(FileExplorerTabActivity.getPrivacyMedioPath(FileCategory.Picture))) {
			size -= 1;
		}
		if (deleteFileFolder.size() == size) {
			...
			if (getFileExplorerActivity().isPrivacy()) {
				deleteFileFolder.remove(FileExplorerTabActivity.getPrivacyMedioPath(FileCategory.Picture));
			}
		}
	...
	private void updateAuroraItemActionBarStatePic() {
		...
		if (deleteFileFolder.size() == size && getRightView() != null) {
			getRightView().setText(getResources().getString(R.string.invert_select));
		} else if (getRightView() != null) {
			getRightView().setText(getResources().getString(R.string.all_select));
		}
	}
	...
	public void updateAuroraItemBottomBarState() {
		...
		if (deleteFileFolder.size() == 0) {
			getAuroraActionBar().getAuroraActionBottomBarMenu().setBottomMenuItemEnable(0, false);
		} else {
			getAuroraActionBar().getAuroraActionBottomBarMenu().setBottomMenuItemEnable(0, true);
		}
		...
		getAuroraActionBar().getMiddleTextView().setText(String.format(getResources().getString(R.string.select_num, deleteFileFolder.size())));
	...
	public boolean onFragmentBack() {
		...
		getCategoryHelper().cancelQueryEvent(FileCategoryHelper.FILE_CATEGORY_DATAS_TOKEN);// 取消没有开始到查询任务
		...
		// 返回相册 壁纸
		if (getFileExplorerActivity().getIsFromPicManager() != null && getFileExplorerActivity().getIsFromPicManager()
				|| (getFileExplorerActivity().getIsFromWallpaperManager() != null && getFileExplorerActivity().getIsFromWallpaperManager()) || getFileExplorerActivity().getIsFromOtherAPP() != null
				&& getFileExplorerActivity().getIsFromOtherAPP()) {
			getFileExplorerActivity().finish();
			return true;
		}
		// 图片别名activity返回
		if (getFileExplorerActivity().getActivityInfo().name.endsWith(getString(R.string.activity_picture))) {
			getFileExplorerActivity().finish();
			return true;
		}
	...
	private class CursorAsyncTask extends AsyncTask<Cursor, Void, Void> {
		...
		protected Void doInBackground(Cursor... params) {
			...
			// LogUtil.e(TAG, "doInBackground fileInfos.size()==" +
			// fileInfos.size());
			try {
		...
		protected void onPostExecute(Void result) {
			...
			SoftReference<ConcurrentHashMap<String, List<FileInfo>>> cate = new SoftReference<ConcurrentHashMap<String, List<FileInfo>>>(categoryPiclistMap);
			getFileExplorerActivity().setCategoryPiclistMapCache(cate);
			SoftReference<ConcurrentHashMap<Integer, String>> path = new SoftReference<ConcurrentHashMap<Integer, String>>(picFilePath);
			getFileExplorerActivity().setPicFilePathCache(path);
			SoftReference<List<FileInfo>> cachePic = new SoftReference<List<FileInfo>>(fileInfos);
	 ...
	 * 图片数据处理(截取与排序)
	 */
	private synchronized void sortPicInfos(List<FileInfo> fileInfosTemp) throws Exception {
		...
		for (FileInfo fileInfo : fileInfosTemp) {
			if (fileInfo != null) {
				...
				if (path != null && fileInfo.fileName != null) {
					...
					if (filePath != null) {
						if (TextUtils.isEmpty(FileExplorerTabActivity.getPrivacyHomePath()) || !filePath.startsWith(FileExplorerTabActivity.getPrivacyHomePath())) {
							// modify by JXH 2014-7-30 begin
							if (getFileExplorerActivity().getStoragesStrings().contains(filePath)) {
								filePath = getFileExplorerActivity().getStorageName(filePath);
							}
							...
							if (categoryPiclistMapTemp.containsKey(filePath)) {// 防止重复添加fileinfo
								if (!categoryPiclistMapTemp.get(filePath).contains(fileInfo)) {
									categoryPiclistMapTemp.get(filePath).add(fileInfo);
								}
							} else {
								List<FileInfo> tempFileInfos = new ArrayList<FileInfo>();
								tempFileInfos.add(fileInfo);
								categoryPiclistMapTemp.put(filePath, tempFileInfos);
							}
						}
					}
				}
		...
		if (getFileExplorerActivity().isPrivacy() && !getFileExplorerActivity().getIsGetMoreNoPrivacy() && !getFileExplorerActivity().getIsFromOtherAPP()) {
			getFileExplorerActivity().clearCache();
			List<FileInfo> pList = getFileExplorerActivity().getHashMap().get(FileCategory.Picture);
			if (pList == null) {
				synchronized (object) {
				...
				pList = getFileExplorerActivity().getHashMap().get(FileCategory.Picture);
				priFileInfos = pList;
			}
			if (pList != null) {
				isBeforePri = true;
				String path = FileExplorerTabActivity.getPrivacyMedioPath(FileCategory.Picture);
	...
	public void deleteComplete(List<FileInfo> ids) {
		...
		for (int i = 0; i < picFilePathTemp.size(); i++) {
			...
			String path = getFileExplorerActivity().getStoragePath(picFilePathTemp.get(i));
			if (!setTemp.contains(path)) {
				picFilePath.put(k, picFilePathTemp.get(i));
				categoryPiclistMap.put(picFilePathTemp.get(i), categoryPiclistMapTemp.get(picFilePathTemp.get(i)));
				k++;
			}
		}
		unDoOperation();
