...
public class CdmaNetworkSettings {
    ...
    public static final String MTK_C2KWP_SIMSWITCH_SUPPOR = "ro.mtk.c2kwp.simswitch.support";
    // Feature support.
    ...
    public CdmaNetworkSettings(PreferenceActivity activity, ListPreference noUsedpreferredNetworkMode,
            ListPreference buttonPreferredNetworkMode, Phone phone) {
        mActivity = activity;
        mPhone = phone;
        mSubId = phone.getSubId();
        if (buttonPreferredNetworkMode != null) {
            activity.getPreferenceScreen().removePreference(buttonPreferredNetworkMode);
        }
        if (noUsedpreferredNetworkMode != null) {
            activity.getPreferenceScreen().removePreference(noUsedpreferredNetworkMode);
        }
    ...
    private boolean isCapabilityPhone() {
        if (!SystemProperties.get(MTK_C2KWP_SIMSWITCH_SUPPOR).equals("1")) {
            return true;
        }
