...
public class AuroraMarkUtils {
	...
	public static void getNumberInfoInternal(String number) {
		if(RejectUtils.isSupportBlack()) {
	    	String result = getUserMarkInternal(InCallApp.getInstance(), number);
	    	if(TextUtils.isEmpty(result)) {
//	    		result = getInstance().getMarkContentInternal(InCallApp.getInstance(), number);
	    		if(SogouUtils.isInit()) {
	    			result =  SogouUtils.getMarkInternal(number);      
	    		} else if(InCallApp.getInstance().mYuloreUtils.isInit()) {
	    			result =  YuLoreUtils.getMarkInternal(number);
	    		}
	    	}
	    	if(TextUtils.isEmpty(result)) {
	    		getMarkContentFromCalllogInternal(InCallApp.getInstance(), number);
	    	}
		}	
	}
}