...
public class YuLoreUtils extends AuroraMarkUtils{
    ...
    private static Map<String, RecognitionTelephone> mCache;
    ...
    public void init(Context context) {
       	mCache = new HashMap<String, RecognitionTelephone>();
...
//    limit  1=不限制 2=wifi 下获取数据 3=不联网
    public RecognitionTelephone queryNumberInfo(String telNum, int type, boolean immediately, int limit) {
    	...
    	if(mCache.get(telNum) != null) {
    		return mCache.get(telNum);
    	}
    	try {
...
//    }
    ...
    public static String getMarkInternal(String number) {
        if(InCallApp.getInstance().mYuloreUtils.isInit()) {
        	RecognitionTelephone r = InCallApp.getInstance().mYuloreUtils.queryNumberInfo(number, 1, false, 1);
        	if(r == null) {
        		return "";
        	}
        	slogan = r.getSlogan();
