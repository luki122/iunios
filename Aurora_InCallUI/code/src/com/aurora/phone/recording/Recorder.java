...
public class Recorder implements OnErrorListener {
    ...
    public void startRecording(int outputfileformat, String extension) throws IOException {
        ...
        StorageManager storageManager = (StorageManager) InCallApp.getInstance().getSystemService(
...
//        }
        ...
        if(mLocalFile != null && mLocalFile.exists()) {
        	if(mSampleFile != null) {
        		File sampleDir = null;
        		if(mPrivateId > 0) {
        			sampleDir = new File(GnPhoneRecordHelper.getPrivateRecordPath(InCallApp.getInstance(), mPrivateId)); 
        		  	  if(!sampleDir.exists()){
        			      sampleDir.mkdirs();
        		  	  }
        		} else {
        			sampleDir = new File(GnPhoneRecordHelper.getRecordPath(InCallApp.getInstance())); 
        		}
...
//            prefix = GnPhoneRecordHelper.getRecordFileName(context, contacts, number);
            ...
            //aurora add liguangyu 20140419 for BUG #4384 start
            File file = InCallApp.getInstance().getDir ("localrecord", Context.MODE_PRIVATE);
            ...
            setError(SDCARD_ACCESS_ERROR);
            Log.i(this, "----***------- can't access sdcard !!");
            e.printStackTrace();
        }
