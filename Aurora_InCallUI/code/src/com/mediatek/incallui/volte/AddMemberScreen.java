...
public class AddMemberScreen extends AlertActivity implements DialogInterface.OnClickListener {
    ...
    private void processAddConferenceMemberAction() {
        ...
        if (list.size() > maxMembers - currentMembers) {
            log("processAddConferenceMemberAction()...max number / current number / add number: "
                    + maxMembers + " / " + currentMembers + " / " + list.size());
        ...
        // TODO: to prevent adding the same number twice ? (need check, for "10010")
        String msg = getResources().getString(R.string.multi_conference_member_adding);
        if (!TextUtils.isEmpty(msg)) {
            Toast.makeText(this, msg, Toast.LENGTH_SHORT).show();
        }

        TelecomAdapter.getInstance().inviteConferenceParticipants(conferenceCallId, list);
        finish();
    }
