 ...
 * 
 * @author hao jingjing
 * 实际上都是PagedView，只是PagedView被更改后文件夹的显示不正常了
 */
 ...
 */
public abstract class FolderPagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    void setCurrentPage(int currentPage) {
        ...
        Log.i("DEBUG","mCurrentPage="+mCurrentPage);
    ...
    /**
     * Desc:like setCurrentPage,but don't overwriting their code.dangerous!
     * Author:Hazel
     * */
    protected void setCurrentPageAndIndicator(int currentPage){
    	if (!mScroller.isFinished()) {
            mScroller.abortAnimation();
        }
    	 if (getChildCount() == 0) {
         ...
         mCurrentPage = Math.max(0, Math.min(currentPage, getPageCount() - 1));
        updateCurrentPageScroll();
        updateScrollingIndicator(currentPage);
        notifyPageSwitchListener();
    }
    ...
    protected void onPageEndMoving() {
        ...
        if(mOnCalenderPluginListening2!=null){
        	mOnCalenderPluginListening2.CalenderPluginListening2();
        }
    ...
    protected void determineScrollingStart(MotionEvent ev, float touchSlopScale) {
        ...
        //avoid shake when move_up widgets in folder
        boolean xPaged = xDiff > Math.round(mPagingTouchSlop * 2.1f);
    ...
    //add by xiexiujie for calender plugin icon start 10.13
    public interface OnCalenderPluginListening2{
        void CalenderPluginListening2();
    }
    
    OnCalenderPluginListening2 mOnCalenderPluginListening2=null;
    
    public void setOnCalenderPluginListening2(OnCalenderPluginListening2 e){
    	mOnCalenderPluginListening2=e;
    }
    //add by xiexiujie for calender plugin icon end 10.13
    ...
    protected void snapToPage(int whichPage, int delta, int duration) {
        mNextPage = whichPage;
        //Log.e("HJJ", "FolderPagedView mNextPage:" + mNextPage);
        // Aurora <haojj> <2013-10-16> add for ALPHA begin
        getScrollingIndicator();
        if(mScrollIndicator!=null){
        	ViewGroup parent = (ViewGroup) getParent();
        	if(parent instanceof Folder) {
        		((FolderPageIndicator)mScrollIndicator).setCurrentPosition(mNextPage);
        	} 
        }
        // Aurora <haojj> <2013-10-16> end
    ...
    protected View getScrollingIndicator() {
        ...
        if (mHasScrollIndicator && mScrollIndicator == null) {
            ...
            if (parent != null) {
            	if(parent instanceof Folder) {
            		mScrollIndicator = (View) (parent.findViewById(R.id.folder_indicator));
            	} 
                
                mHasScrollIndicator = mScrollIndicator != null;
                // Added by haojj
                // if (mHasScrollIndicator && !(parent instanceof Folder) ) {
...
//AURORA-END::Comment out for page indicator showing all the time::Comment out::Shi guiqiang::20130908
    ...
    /**
     * Desc:Overwriting updateScrollingIndicator  depends on the screen 
     * */
    private void updateScrollingIndicator(int screen){
    	  getScrollingIndicator();
          if(mScrollIndicator!=null){
          	ViewGroup parent = (ViewGroup) getParent();
          	if(parent instanceof Folder) {
          		((FolderPageIndicator)mScrollIndicator).setCurrentPosition(screen);
          	} 
          }
    }
      
    private void updateScrollingIndicator() {
    	//Log.e("HJJ", "FolderPagedView updateScrollingIndicator");
        if(mScrollIndicator instanceof FolderPageIndicator){
        	if (getChildCount() <= 1) return;
	        if (!isScrollingIndicatorEnabled()) return;
	        getScrollingIndicator();
	        if(mScrollIndicator!=null){
	        	ViewGroup parent = (ViewGroup) getParent();
	        	if(parent instanceof Folder) {
	        		((FolderPageIndicator)mScrollIndicator).updatePoints(getContext(), getChildCount());
//	        		((FolderPageIndicator)mScrollIndicator).setCurrentPosition(mCurrentPage);
	        	} 
	        }
