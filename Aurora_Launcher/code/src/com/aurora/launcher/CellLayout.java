...
public class CellLayout extends ViewGroup {
	...
	//Aurora xiejun modify for cell layout scale background begin
	...
	private ShadowTextView mPressedOrFocusedIcon;
	...
	private boolean mFromHotseat = false;
    // Aurora <jialf> <2013-09-10> add for dock data end
	//AURORA-START:
	private int[] mEmpty=new int[2];
	//AURORA-END
	
	private boolean mHandupItems = false;
    ...
    //AURORA-START::灵活添加CellLayout::Add::郝晶晶::2013-09-03
    public CellLayout(Context context, int cellWidth, int cellHeight, int widthGap, int heightGap, int cellCountX, int cellCountY){
    ...
    private void initLayout(Context context){
		...
		mNormalBackground = res.getDrawable(R.drawable.celllayout_scale_bg);
		//Aurora xiejun modify for cell layout scale background begin
		...
		//Aurora xiejun modify for cell layout scale background end
				...
				public void onAnimationUpdate(ValueAnimator animation) {
					...
					if (outline == null) {
						...
						if (debug) {
							...
							ALog.d(TAG, "anim " + thisIndex + " update: " + val
		...
		mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight,
				mWidthGap, mHeightGap, mFromHotseat);
	    // Aurora <jialf> <2013-09-10> modify for dock data end
		addView(mShortcutsAndWidgets);

    }
    //AURORA-END::灵活添加CellLayout::Add::郝晶晶::2013-09-03
	...
	public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        // Aurora <jialf> <2013-09-10> add for dock data begin
        mFromHotseat = a.getBoolean(R.styleable.CellLayout_fromHotseat, false);
        // Aurora <jialf> <2013-09-10> add for dock data end
		...
		// AURORA-START::将原来的代码全部写入到initLayout中，以用于方便灵活添加layout::Update::郝晶晶::2013-09-09
		initLayout(context);
		// AURORA-END::将原来的代码全部写入到initLayout中，以用于方便灵活添加layout::Update::郝晶晶::2013-09-09
	}
	...
	private void invalidateBubbleTextView(ShadowTextView icon) {
		// AURORA-START:Application Icon 设计::xiejun::modify
	...
	void setPressedOrFocusedIcon(ShadowTextView icon) {
		// AURORA-END:Application Icon 设计::xiejun::modify
		// We draw the pressed or focused BubbleTextView's background in
		// CellLayout because it
		// requires an expanded clip rect (due to the glow's blur radius)
		ShadowTextView oldIcon = mPressedOrFocusedIcon;
		mPressedOrFocusedIcon = icon;
		if (oldIcon != null) {
			invalidateBubbleTextView(oldIcon);
		}
//AURORA-START::Fix bug #260::Shi guiqiang::20131025
		if (mLauncher.getEditMode() != EditMode.APPWIDGET_ADD) {
			if (mPressedOrFocusedIcon != null) {
				invalidateBubbleTextView(mPressedOrFocusedIcon);
			}
		}
//AURORA-END::Fix bug #260::Shi guiqiang::20131025
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		int previewPaddingTop = FolderRingAnimator.sPreviewPadding;
		...
		for (int i = 0; i < mFolderOuterRings.size(); i++) {
			...
			/*if(DeviceProperties.isNeedScale()){
				previewPaddingTop = 29;
			}*/
			int centerY = mTempLocation[1] + previewOffset / 2 + previewPaddingTop;
			canvas.save();
	...
	public boolean addWorkSpaceCalenderViewToCellLayout(View child, int index,
			int childId, LayoutParams params, boolean markCells) {
		final LayoutParams lp = params;
		...
		if (lp.cellX >= 0 && lp.cellX <= mCountX - 1 && lp.cellY >= 0
			...
			mShortcutsAndWidgets.addView(child, -1, lp);
	...
	// Aurora <jialf> <2013-10-26> add for fix bug #250 begin
	public float getAuroraDistanceFromCell(float x, float y, int[] cell) {
		cellToCenterPoint(cell[0], cell[1], mTmpPoint);
		float distance = (float) Math.sqrt(Math.pow(x - mTmpPoint[0], 2)
				+ Math.pow(Math.abs(y - mTmpPoint[1]) - Hotseat.TRANSLATION_Y, 2));
		return distance;
	}
	// Aurora <jialf> <2013-10-26> add for fix bug #250 end

	// Aurora <jialf> <2013-10-26> add for fix bug #174 begin
	public float getAuroraOldDistanceFromCell(float x, float y, int[] cell) {
		int[] point = new int[2];
		final int hStartPadding = getPaddingLeft();
		final int vStartPadding = getPaddingTop();
		int count = mCountX - 1;
		int tempCellX = cell[0] - 1;
		int cellWidth ;
		if(count != 0) {
			cellWidth = (getWidth() - hStartPadding - getPaddingRight()) / count;
			if (count > 1 && cell[0] == 0) {
				tempCellX = cell[0];
			}
		} else {
			cellWidth = getWidth() - hStartPadding - getPaddingRight();
		}
		point[0] = hStartPadding + tempCellX * (cellWidth + mWidthGap) + cellWidth / 2;
		point[1] = vStartPadding + cell[1] * (mCellHeight + mHeightGap) + mCellHeight / 2;
		float distance = (float) Math.sqrt(Math.pow(x - point[0], 2)
				+ Math.pow(Math.abs(y - point[1]) - Hotseat.TRANSLATION_Y, 2));
		return distance;
	}
	// Aurora <jialf> <2013-10-26> add for fix bug #174 end
	
	public boolean isReallyOverIcon(float x,float y,int[] cell){
    	cellToCenterPoint(cell[0], cell[1], mTmpPoint);
    	float left = mTmpPoint[0]-mCellWidth/2;
    	float top = mTmpPoint[1]-mCellHeight/2+ Hotseat.TRANSLATION_Y;
    	float right = mTmpPoint[0]+ mCellWidth/2;
    	float bottom = mTmpPoint[1]+ mCellHeight/2+Hotseat.TRANSLATION_Y;
	...
	//vulcan added it in 2014-6-17
	public int getShortcutsAndWidgetsChildCount() {
		if(mShortcutsAndWidgets != null) {
			return mShortcutsAndWidgets.getChildCount();
		}
		else {
			Log.d("vulcan-repeat",String.format("getShortcutsAndWidgetsChildCount: mShortcutsAndWidgets is null"));
			return 0;
		}
	}
	
	//vulcan added it in 2014-6-17
	public View getShortcutsAndWidgetsChildAt(int index) {
	...
	private void computeDirectionVector(float deltaX, float deltaY, int[] result) {
		...
		result[0] = 0;
		result[1] = 0;
		//Aurora-start:xiejun:modify for derection
		//if (Math.abs(Math.cos(angle)) > 0.5f) {
		if (Math.abs(Math.cos(angle)) > 0.01f) {
			result[0] = (int) Math.signum(deltaX);
		}
		//Aurora-end:xiejun:modify for derection
		/*
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (c != null) {
				animateChildToPosition(child, c.x, c.y,
						REORDER_ANIMATION_DURATION, delay, DESTRUCTIVE_REORDER,
						false);
				delay+=delayAmount;
				delayAmount*=0.9;
	...
	public void beginOrAdjustHintAnimations() {
		//if(mLockScreenShakeAnimators!=null)mLockScreenShakeAnimators.clear();
		completeAndClearReorderHintAnimations2();
		int childCount = mShortcutsAndWidgets.getChildCount();
		for (int i = childCount-1; i >= 0; i--) {
			View child = mShortcutsAndWidgets.getChildAt(i);
			ReorderHintAnimation2 rha = new ReorderHintAnimation2(child);
			rha.animate();
		}
		setHandupItems(false);
	}
	...
	int[] createArea(int pixelX, int pixelY, int minSpanX, int minSpanY,
		...
		//Aurora-start:xiejun:20130926:ID138
		ItemConfiguration swapSolution=null;
		ItemConfiguration finalSolution = null;
		if(!mFromHotseat){
//AURORA-START::For widget that is too big::Shi guiqiang::20131114
			if(result[0]>=0&&result[1]>=0){
				boolean foundEmptyPlace=getUnocupiedCell(result[0],result[1],mDirectionVector[0]==1?true:false,mEmpty);
				if(!foundEmptyPlace){
					mDirectionVector[0]*=-1;
					foundEmptyPlace=getUnocupiedCell(result[0],result[1],mDirectionVector[0]==1?true:false,mEmpty);
				}
				boolean isWidgtAtPlace=(getChildAt(result[0],result[1]) instanceof AppWidgetHostView);
				boolean isOcupied=isOccupied(result[0],result[1]);
				boolean canPush=(mDirectionVector[0]!=0&&foundEmptyPlace&&(!(isOcupied&&isWidgtAtPlace)))||(mDirectionVector[0]==0&&!isOcupied);
				if(!canPush){
					result[0] = result[1] = resultSpan[0] = resultSpan[1] = -1;
					return result;
				}
				swapSolution = computeSolution(pixelX, pixelY, minSpanX,
...
//AURORA-START::For widget that is too big::Shi guiqiang::20131114
			...
			swapSolution = simpleSwap(pixelX, pixelY, minSpanX,
					minSpanY, spanX, spanY, mDirectionVector, dragView, true,
					new ItemConfiguration());
	...
	int[] getLastOccupiedCells() {
		int[] loc = new int[] { -1, -1 };
		out: for (int x = mCountY - 1; x >= 0; x--) {
			for (int y = mCountX - 1; y >= 0; y--) {
				if (mOccupied[y][x]) {
					loc[0] = y;
					loc[1] = x;
					break out;
		...
		if(loc[0]!=-1&&loc[1]!=-1){
			if (loc[0] == mCountX-1) {
				loc[0]=0;
				loc[1]++;
				if(loc[1]>mCountY-1)return null;
			} else {
				loc[0]=loc[0] + 1;
			}
		}
		ALog.i(TAG, "cellX = " + loc[0] + ",cellY = " + loc[1]);
		return loc;
	}
	 ...
	 * @param empty
	 * @param spanX
	 * @param spanY      tangjun 9.23
	 */
	public void findEmpytCell(int empty[], int spanX, int spanY) {
		out:for (int j = 0; j < mCountY; j++) { 
		  for (int i = 0; i < mCountX; i++) {
				boolean canDrop = true;
				if (!mOccupied[i][j]) {
					inner: for (int k = 0; k < spanX; k++) {
						for (int l = 0; l < spanY; l++) {
							if ((i + k) >= mCountX || (j + l) >= mCountY || mOccupied[i + k][j + l]) {
								canDrop = false;
								break inner;
							}
						}
	...
	//Aurora-start:xiejun:20130926:ID138
	ItemConfiguration computeSolution(int pixelX, int pixelY, int minSpanX,
			int minSpanY, int spanX, int spanY, int[] direction, View dragView,
			boolean decX, ItemConfiguration solution) {
		copyCurrentStateToSolution(solution, false);
		copyOccupiedArray(mTmpOccupied);
		...
		success = rearrangementExists_Aurora(result[0], result[1], spanX, spanY,
				direction, dragView, solution);
	...
	private boolean rearrangementExists_Aurora(int cellX, int cellY, int spanX,
			int spanY, int[] direction, View ignoreView,
			ItemConfiguration solution){
		if (cellX < 0 || cellY < 0)
			return false;
		mOccupiedRect.set(cellX, cellY, cellX + spanX, cellY + spanY);
		boolean canPush=getUnocupiedCell(cellX,cellY,mDirectionVector[0]==1?true:false,mEmpty);
		...
		for (View child : solution.map.keySet()) {
			...
			CellAndSpan c = solution.map.get(child);
			LayoutParams lp = (LayoutParams) child.getLayoutParams();
			if (isInPosition(cellX,cellY,mEmpty[0],mEmpty[1],c.x,c.y)&&!(child instanceof AppWidgetHostView)) {
				if (!lp.canReorder) {
					return false;
				}
				mIntersectingViews.add(child);
		...
		if(attemptPushInDirection_Arura(mIntersectingViews, mOccupiedRect,
				direction, ignoreView, solution)){
			return true;
		}
		return true;	
	}
	private boolean  attemptPushInDirection_Arura(ArrayList<View> intersectingViews,
			Rect occupied, int[] direction, View ignoreView,
			ItemConfiguration solution){
		if(direction[0]!=0){
			return(pushViewsToTempLocation_Aurora(intersectingViews, occupied, direction,
					ignoreView, solution));
		}else{
			return false;
		}		
	}
	private boolean pushViewsToTempLocation_Aurora(ArrayList<View> views,
			Rect rectOccupiedByPotentialDrop, int[] direction, View dragView,
			ItemConfiguration currentState){
		for (View v : views) {
			CellAndSpan c = currentState.map.get(v);
		...
		currentState.save();
		if(views.size()>1){
			comparator_Aurora.setItemConfiguration(currentState);
			Collections.sort(views, comparator_Aurora);
		}
		shift(views,direction,currentState);
		for (View v : views) {
			CellAndSpan c = currentState.map.get(v);
			if(!(v instanceof AppWidgetHostView)){
				markCellsForView(c.x, c.y, c.spanX, c.spanY, mTmpOccupied, true);
	...
	private PositionComparator_Aurora comparator_Aurora=new PositionComparator_Aurora();
	class PositionComparator_Aurora implements Comparator<View> {
		ItemConfiguration mItemConfiguration;
		public PositionComparator_Aurora(){
			super();
		}
		public void setItemConfiguration(ItemConfiguration currentState){
			mItemConfiguration=currentState;
		}
		public int compare(View left, View right) {
			CellAndSpan l = mItemConfiguration.map.get(left);
			CellAndSpan r = mItemConfiguration.map.get(right);
			if(l.y>r.y||l.y==r.y&&l.x>r.x){
				return 1;
			}else{
				return -1;
			}
	...
	private  void shift(ArrayList<View> views,int[] direction ,ItemConfiguration currentState) {
		int whichEdge=0;
		if(direction[0]<0){
			whichEdge=0;
		}else{
		...
		case 0:
			for(int i=0;i<size;i++){
				...
				CellAndSpan c = currentState.map.get(view);
				if(i==0||headWidget){
					tempX=c.x;
					tempY=c.y;
					c.x=mEmpty[0];
	...
	private boolean isWidgetArea(ArrayList<Rect> rects,Rect judgedRect,Rect result){
		for(Rect r:rects){
			if(judgedRect.intersect(r)){
				result=r;
				return true;
			}
		}
	...
	private boolean isInPosition(int x1 ,int y1,int x2,int y2,int x,int y){
		boolean first=readingOrderGreaterThan(x1,y1,x2,y2);
		if(first){
			if(y2<y&&y<y1){
				return true;
			}else if(y1==y2&&y==y1&&x<=x1&&x>=x2){
				return true;
			}else if(y==y2&&y!=y1&&(x>=x2&&x<mCountX)){
				return true;
			}else if(y==y1&&y!=y2&&x>=0&&x<=x1){
				return true;
			}
			return false;
		}else{
			if(y1<y&&y<y2){
				return true;
			}else if(y1==y2&&y==y2&&x<=x2&&x>=x1){
				return true;
			}else if(y==y1&&y!=y2&&(x>=x1&&x<mCountX)){
				return true;
			}else if(y==y2&&y!=y1&&x>=0&&x<=x2){
				return true;
			}
			return false;
		}
	}
	boolean readingOrderGreaterThan(int x1,int y1,int x2,int y2){
	...
	private boolean getUnocupiedCell(int cellX ,int cellY,boolean first,int[] result){
			...
			if(first){
				tempCellX=cellX;
				tempCellY=cellY;
				for(;tempCellX<mCountX;tempCellX++){
					if(!mOccupied[tempCellX][tempCellY]){
						result[0]=tempCellX;
						result[1]=tempCellY;
						return true;
					}
				...
				tempCellY++;
				for(;tempCellY<mCountY;tempCellY++){
					for(tempCellX=0;tempCellX<mCountX;tempCellX++){
						if(!mOccupied[tempCellX][tempCellY]){
							result[0]=tempCellX;
							result[1]=tempCellY;
							return true;
				...
				for(tempCellY=0;tempCellY<=cellY;tempCellY++){
					int lastCellX=mCountX-1;
					if(tempCellY==cellY){
						lastCellX=cellX;
					}
					for(tempCellX=0;tempCellX<=lastCellX;tempCellX++){
						if(!mOccupied[tempCellX][tempCellY]){
							result[0]=tempCellX;
							result[1]=tempCellY;
						}
				...
				if(result[0]>-1&&result[1]>-1)return true;
			}
			return false;
	}
	//Aurora-END:xiejun:20130926:ID138
	//Aurora-START:xiejun:20130930:ID194
	public void updateCelllayoutScreen() {
		mCellInfo.screen = ((ViewGroup) getParent()).indexOfChild(this);
		int childCount = mShortcutsAndWidgets.getChildCount();
		for (int i = 0; i < childCount; i++) {
			View child = mShortcutsAndWidgets.getChildAt(i);
	...
	//Aurora-END:xiejun:20130930:ID194

	// Aurora <jialf> <2013-10-08> add for Dock data begin
	public void setAuroraCellX(boolean add) {
		if (add) {
			mCountX++;
			if (!mTempRectStack.isEmpty()) {
				mTempRectStack.push(new Rect());
		...
		if (mCountX != 0) {
			mCellWidth = (getWidth() - getPaddingLeft() - getPaddingRight()) / mCountX;
		} else {
			mCellWidth = getWidth() - getPaddingLeft() - getPaddingRight();
		}
		if (mCellWidth == 0) {
			ALog.i(TAG, "the device screen width is : " + mLauncher.getmScreenWidth());
			mCellWidth = mLauncher.getmScreenWidth() - getPaddingLeft() - getPaddingRight();
		}
	}
	
	public boolean hasAuroraEmptyCell() {
		return mCountX < 4;
	}
	
	public void setAuroraGridSize(int emptyCell) {
		ALog.i(TAG, "mCountX = " + mCountX +",mCountY = " + mCountY +",getWidth() = " + getWidth());
		mOccupied = new boolean[mCountX][mCountY];
		mTmpOccupied = new boolean[mCountX][mCountY];
		for (int i = 0; i < mCountY; i++) {
			for (int j = 0; j < mCountX; j++) {
				// Aurora <jialf> <2013-10-31> modify for fix bug #313 begin
				if (emptyCell != -1 && j == emptyCell) {
					mOccupied[j][i] = false;
				} else {
					mOccupied[j][i] = true;
				}
	...
	public void setAuroraGridWidth(int width, int count) {
		mCellWidth = width;
		mCountX = count;
		mOccupied = new boolean[mCountX][mCountY];
		mTmpOccupied = new boolean[mCountX][mCountY];
		mTempRectStack.clear();
		ALog.i(TAG, "mCountX = " + mCountX + ",mCellWidth = " + mCellWidth
				+ ",mCellHeight = " + mCellHeight + ",mHeightGap = "
				+ mHeightGap);
		mShortcutsAndWidgets.setCellDimensions(mCellWidth, mCellHeight,
				mWidthGap, mHeightGap);
		requestLayout();
	...
	private HashMap<View, ReorderHintAnimation2> mLockScreenShakeAnimators = new HashMap<View, ReorderHintAnimation2>();

	class ReorderHintAnimation2 {
		View child;
		float finalDeltaX;
		float finalDeltaY;
		float initDeltaX;
		...
		public ReorderHintAnimation2(View child) {
			this.child = child;
			//child.setVisibility(View.VISIBLE);
			finalDeltaX = 0;
			finalDeltaY = 0;
			initDeltaX =0;
			initDeltaY = (child.getTranslationY())*(child.getScaleY());
			initAlpha = child.getAlpha() ;
			if(initAlpha!=0.0f){
				fianlAlpha = initAlpha;
			}else {
				fianlAlpha = 1.0f;
			}
			ALog.i("screen","initAlpha = "+initAlpha);
		}
		...
		void animate() {
			...
			//Add hardWare Layer to animation
			...
			b = vb;
			//vb.setRepeatMode(ValueAnimator.REVERSE);
			vb.setDuration(DURATION);
			vb.setStartDelay((int) (Math.random() * 100 -50));
			vb.addUpdateListener(new AnimatorUpdateListener() {
				@Override
				public void onAnimationUpdate(ValueAnimator animation) {
					float r = ((Float) animation.getAnimatedValue())
							.floatValue();
					if(initAlpha!=fianlAlpha)child.setAlpha(r);
				}
			});
			mLockScreenShakeAnimators.put(child, this);
			AnimatorSet set = LauncherAnimUtils.createAnimatorSet();
			animatorSet = set;
			set.playTogether(va,vb);
			set.start();
		}
	...
	private void completeAndClearReorderHintAnimations2() {
		mLockScreenShakeAnimators.clear();
	}
	
	public void handupChilds(int px){
		setHandupItems(true);
		ShortcutAndWidgetContainer sw =this.getShortcutsAndWidgets();
		int childCount = sw.getChildCount();
		for(int i=0;i < childCount;i++){
			View view = sw.getChildAt(i);
		...
		if (object!=null && object instanceof ItemInfo) {
				...
				int cellY = ((ItemInfo)object).cellY;
				if(cellY==0){
	...
	public  boolean isHandupItems(){
		return mHandupItems;
	}
	public  void setHandupItems(boolean b){
		mHandupItems = b;
	}
	
	public boolean isEmptyCellLayout() {
		boolean empty = true;
		outer:for(int i=0;i<mCountY;i++) {
			for(int j=0;j<mCountX;j++) {
				if(mOccupied[j][i]) {
					empty = false;
					break outer;
	...
	public ArrayList<FolderIcon> getAllFodlerIcon() {
		if(mShortcutsAndWidgets == null) {	
			return new ArrayList<FolderIcon>();
		}
		return mShortcutsAndWidgets.getAllFodlerIcon();
	}
}
