...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    private void completeDrop(DragObject d) {
		...
		if (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION
			...
			//delete appwidget binded to the ShortcutInfo
			if(item instanceof ShortcutInfo){
				ShortcutInfo shortcut = (ShortcutInfo)item;
				if(shortcut.appWidgetId != -1){
					Log.v("mywidget", "DeleteDropTarget---Remove Shortcut:"+shortcut.title+",appWidgetId="+shortcut.appWidgetId);
					mLauncher.getAppWidgetHost().deleteAppWidgetId(shortcut.appWidgetId);
				}
			}
