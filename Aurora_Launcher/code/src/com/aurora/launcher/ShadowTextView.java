...
public class ShadowTextView extends TextView {
    ...
    private boolean isMovingupWidget;

	public boolean isMovingupWidget() {
		return isMovingupWidget;
	}

	public void setMovingupWidget(boolean isMovingupWidget) {
		this.isMovingupWidget = isMovingupWidget;
	}
	...
	protected void init() {
		...
		/*setShadowLayer(SHADOW_LARGE_RADIUS, 0.0f, SHADOW_Y_OFFSET,
				SHADOW_LARGE_COLOUR);*/
		mScaleOverFour = res.getInteger(R.integer.hotseat5_scale)/100f;
	}
	...
	public void setTag(Object tag,boolean needCheck) {
		super.setTag(tag);
	}
   ...
   private boolean isInHotseat(){
		...
		if( this.getParent()!=null && this.getParent().getParent()!=null){
			ViewParent container = this.getParent();
			ViewParent parent = container.getParent().getParent();
		return parent instanceof Hotseat ;
		}
		return false;
	}
	
	private boolean isInHotseatNeedScale(){
		
		if( this.getParent()!=null && this.getParent().getParent()!=null){
			ViewParent container = this.getParent();
			ViewParent parent = container.getParent().getParent();
		return parent instanceof Hotseat && ((ViewGroup)container).getChildCount()==5;
		}
		return false;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		Log.v("double",this.getText()+"  ShadowTextView---event: "+event);
		boolean result = super.onTouchEvent(event);
		mIsNeedScale = isInHotseatNeedScale();
		...
		case MotionEvent.ACTION_DOWN:
			...
			*/
            if(mIsNeedScale){
               iconScale(mScaleOverFour-0.03f);
            }else{
			   iconScale(0.97f);
            }
            ...
            postDelayed(new Runnable() {
				...
				public void run() {
					  if(mIsNeedScale){
						  Log.v("double",ShadowTextView.this.getText()+"  ShadowTextView--ACTION_DOWN--mIsInHotseat");
						  iconScale(mScaleOverFour);
					  }
					  else{
						  iconScale(1.0f);
					  }
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (isMovingupWidget) {
            	if(mIsNeedScale){
  				  Log.v("double",this.getText()+"  ShadowTextView--ACTION_UP--mIsInHotseat");
  				  iconScale(mScaleOverFour);
  			   }
  			   else{
  				  iconScale(1.0f);
	...
	private void iconScale(float f) {
		if (isInHotseat()) {
				...
				if (mIsNeedScale) {
					if (f < mScaleOverFour) {
						Log.v("double", this.getText()
								+ "  ShadowTextView--getRootView()---");
						this.setAlpha(0.6f);
					} else {
						this.setAlpha(1.0f);
					}
