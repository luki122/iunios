...
import com.aurora.reminder.ReminderWidgetProvider;
...
//		mThemeChanged = config.themeConfig.themeChanged;
		AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(this);
		ComponentName reminderProvider  = new ComponentName(this, ReminderWidgetProvider.class);
		int widgetIdlength = appWidgetManager.getAppWidgetIds(reminderProvider).length;
		Log.v("reminder", "widgetIdlength: "+widgetIdlength);
		if (widgetIdlength > 0) {
			Intent reminderIntent = new Intent(ReminderWidgetProvider.ACTION_WIDGET_SERVICE);
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		Log.i("xiejun","LauncherApplication newConfig = "+ newConfig.toString());
		super.onConfigurationChanged(newConfig);
		int newTheme = newConfig.themeConfig.themeChanged;
		Log.i("xiejun","newTheme != mThemeChanged "+ (newTheme != mThemeChanged) );
		if(newTheme != mThemeChanged){
			mThemeChanged = newTheme;
			resetResources();
		}
