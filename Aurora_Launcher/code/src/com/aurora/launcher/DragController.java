    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (Y_touch > mLauncher.getWorkspace().getHeight()* MOVE_RESPONSE_LINE_Y&& isAreaToMove) {
				X_end = ev.getX();
				if (Math.abs(X_end - X_start) > mLauncher.getWorkspace()	.getWidth() * MOVING_THRESHOLD) {
					if (pointerCount == 1
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (Y_touch > mLauncher.getWorkspace().getHeight()* MOVE_RESPONSE_LINE_Y&& mLauncher.getEditMode() != EditMode.QUICK_INDEX) {
				...
				isAreaToMove = true;
				if(mLauncher.getTransModeEnum() == TransModeEnum.NONE && mLauncher.getEditMode() !=EditMode.CLASSIFICATE_ICONS){
