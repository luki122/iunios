...
public class FolderWorkspace extends FolderPagedView{
	...
	protected boolean createAndAddShortcut(int page, final ShortcutInfo item) {
		...
		final AnimatorSet animBubbleUp = mLauncher.animateBubbleTextViewUp(textView);
		final AnimatorSet animBubbleDown = mLauncher.animateBubbleTextViewDown(textView , false);
		...
		textView.setOnTouchListener(new View.OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_DOWN:
					mLauncher.isStarted = false;
					textView.setMovingupWidget(mLauncher.isStarted);
					mPosY = event.getY();
				...
				case MotionEvent.ACTION_MOVE:
					mCurrentPosY = event.getY();
					if(!mLauncher.animAppWidget.isRunning() && !(mLauncher.isStarted) && mLauncher.getEditMode()==EditMode.NONE && mCurrentPosY - mPosY < mLauncher.moveup_offset){
						animBubbleUp.start();
						mLauncher.isStarted = true;
						textView.setMovingupWidget(mLauncher.isStarted);
					}
					break;
				
				case MotionEvent.ACTION_UP:
					Log.v("mywidget","mCurrentPosY-mPosY="+(mCurrentPosY - mPosY)+", mLauncher.getEditMode()="+mLauncher.getEditMode()
							+", info.enableWidgets="+(item.enableWidgets)
							+", info.hostView="+(item.hostView)
							);
					if (item.hostView != null && mLauncher.getEditMode()==EditMode.NONE && LauncherSettings.isEnableWidgets && item.enableWidgets==1 && mLauncher.isStarted){
						 mLauncher.widgetHost.removeAllViews();
						 mLauncher.widgetHost.addView(item.hostView);
						 mLauncher.appLable.setText(item.title);
						 
						 int flag = mLauncher.addAllAppWidgets(textView, item);
						 if( flag == 1){
						   mLauncher.animateAppWidget(mLauncher.widgetCover , 0f , 1f , null, true);
						   AnimatorSet animBubbleDownWidget = mLauncher.animateBubbleTextViewDown(textView , true);
				    	   animBubbleUp.cancel();
				    	   animBubbleDownWidget.start();
						 }
						 return true;
					}else if(mLauncher.isStarted){
						   animBubbleUp.cancel();
			...
			for (int j = 0; j < DEFAULT_CELL_COUNT_Y; j++) {
				for (int i = 0; i < DEFAULT_CELL_COUNT_X; i++) {
					...
					if (v != null) {
						...
						if (!idList.contains(info.id) && (info != mCurrentDragInfo || includeCurrentDragItem) ) {
