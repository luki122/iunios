...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    private static final float RETURN_TO_ORIGINAL_PAGE_THRESHOLD = 0.1f;
    // The page is moved more than halfway, automatically move to the next page on touch up.

	//iht 2014-12-10 较小的位移即可触发翻页    
	private static final float SIGNIFICANT_MOVE_THRESHOLD = 0.02f;
    //private static final float SIGNIFICANT_MOVE_THRESHOLD = 0.2f;
...
//AURORA-END::customized widget::Shi guiqiang::20131018
    ...
    protected void init() {
        ...
        mTouchSlop = Math.round(mTouchSlop);
        mPagingTouchSlop = Math.round(mTouchSlop);
...
//AURORA-START::Fix bug #361::Shi guiqiang::20131031
        if (mScrollIndicator != null && this instanceof Workspace) {
        	//AURORA_START iht 2014-11-12 fix BUG#9745;
        	//Analysis：mCurrentPage out of bounds
        	View v = ((PageIndicator) mScrollIndicator).getChildAt(mCurrentPage);
        	if(v != null){
        		v.setActivated(true);
        	}
        	//AURORA_END iht 2014-11-12 fix BUG#9745;
        }
//AURORA-END::Fix bug #361::Shi guiqiang::20131031
    ...
    protected void onPageEndMoving() {
        ...
        if(mOnCalenderPluginListening!=null){
        	mOnCalenderPluginListening.CalenderPluginListening();
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //if(this instanceof Workspace)ALog.i("*******************onMeasure********************");
    ...
    public void onChildViewAdded(View parent, View child) {
    	...
    	//if(this instanceof Workspace)ALog.i("*******************sonChildViewAdded********************");
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
            ...
            while (rightScreen < pageCount - 1 &&
                    currPage.getX() - currPage.getPaddingLeft()-currPage.getTranslationX()<getScrollX() + screenWidth) {
                rightScreen++;
                currPage = getPageAt(rightScreen + 1);
            }
            range[0] = leftScreen;
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        //duration = 2 * Math.round(2000 * Math.abs(distance / velocity));
        duration =  2*Math.round(2000 * Math.abs(distance / velocity));
        duration = Math.max(duration, 250);
        /*duration = Math.min(duration, MAX_PAGE_SNAP_DURATION)<200?200:duration;*/
    ...
    //add by xiexiujie for calender plugin icon start 10.13
    public interface OnCalenderPluginListening{
        void CalenderPluginListening();
    }
    
    OnCalenderPluginListening mOnCalenderPluginListening=null;
    
    public void setOnCalenderPluginListening(OnCalenderPluginListening e){
    	mOnCalenderPluginListening=e;
    }
    //add by xiexiujie for calender plugin icon end 10.13
    protected void snapToPage(int whichPage, int delta, int duration) {
        mNextPage = whichPage;
		//AURORA_START:xiejun:2013-9-10
        getScrollingIndicator();
        if(mScrollIndicator!=null){
        	((PageIndicator)mScrollIndicator).moveIndicatorToPage(mNextPage);
        }
		//AURORA_END:xiejun:2013-9-10
        
		//AURORA_START:tangjun:2013-9-26
        ViewGroup parent = (ViewGroup) getParent();
        ThumbnailImage previewContent = (ThumbnailImage)parent.findViewById(R.id.preview_content);
        if ( previewContent != null ) {
        	previewContent.movetoItem(mNextPage);
        }
		//AURORA_END:tangjun:2013-9-26
    ...
    protected View getScrollingIndicator() {
        ...
        // found
//AURURA-START::Add to get customized page indicator::Modify::Shi guiqiang::20130908 	
        if (mHasScrollIndicator && mScrollIndicator == null) {
            ViewGroup parent = (ViewGroup) getParent();
            if (parent != null) {
                mScrollIndicator = (View) (parent.findViewById(R.id.paged_view_indicator));
                if(mScrollIndicator instanceof PageIndicator){
                	((PageIndicator)mScrollIndicator).setPagedView(this);
                	((PageIndicator) mScrollIndicator).setCount(getPageCount());
                }
...
//AURURA-END::Add to get customized page indicator::Modify::Shi guiqiang::20130908
    }
    ...
    protected void showScrollingIndicator(boolean immediately) {
    	Log.i("DEBUG","showScrollingIndicator");
//AURORA-START::Comment out for page indicator showing without animation::Comment out::Shi guiqiang::20130908
    	/*
        ...
        */
//AURORA-END::Comment out for page indicator showing without animation::Comment out::Shi guiqiang::20130908
    }
...
//AURORA-START::Set default pageindicator active::Shi guiqiang::20131108
        	getScrollingIndicator();
            if (mScrollIndicator != null && this instanceof Workspace) {
            	if (((PageIndicator) mScrollIndicator).getChildAt(mCurrentPage) != null) {
            		((PageIndicator) mScrollIndicator).getChildAt(mCurrentPage).setActivated(true);
            	}
            }
//AURORA-START::Set default Pageindicator active::Shi guiqiang::20131108        	
	        /*if (getChildCount() <= 1) return;
...
//AURORA-START::Add page indicator control::Comment out::Shi guiqiang::20130908
    	/*
    	Log.i("DEBUG","updateScrollingIndicatorPosition");
        if (!isScrollingIndicatorEnabled()) return;
        ...
        Log.i("DEBUG","updateScrollingIndicatorPosition indicatorWidth="+indicatorWidth);
        ...
        */
//AURORA-END::Add page indicator control::Comment out::Shi guiqiang::20130908
    }
    ...
    public interface PageCountChangeListener{
    	void onPageCountChange(int screen,boolean delete);
    }
    private ArrayList<PageCountChangeListener> mPageCountChangeListeners
    	=new ArrayList<PagedView.PageCountChangeListener>();
    public void registPageCountListener(PageCountChangeListener l){
    	mPageCountChangeListeners.add(l);
    }
    public void notifyPageCountChange(int screen,boolean delete){
    	//Thread.dumpStack();
    	if(mPageCountChangeListeners!=null){
    		for (PageCountChangeListener l:mPageCountChangeListeners) {
				l.onPageCountChange(screen,delete);
			}
    	}
    }
