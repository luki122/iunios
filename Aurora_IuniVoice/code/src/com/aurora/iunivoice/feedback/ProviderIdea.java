...
public class ProviderIdea extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.feedback_provider_advice);
		initView();
		initData();
	}
	...
	public void setupAuroraActionBar() {
		setTitleRes(R.string.privider_sup);
	}
	...
	private void uploadDate() {
		...
		if (adPicName != null && adPicName.size() > 1) {
			for (int index = 0; index < adPicName.size(); index++) {
				...
				if (picMap.get(adPicName.get(index)) != null) {
					...
					// File file =
					// FeedBackUtil.saveBitmapTofile(adPicName.get(index),adPicName.get(index).toString());
					File file = new File(FeedBackUtil.storePath(picMap
							.get(adPicName.get(index))));
		...
		try {
			...
			respondeCode = HttpConnection.upLoad(this, files,
					adProblem_description_detail.getText().toString(),
					adContact_infor_detail.getText().toString(),
					HttpConnection.SUGGESTS);
		} catch (Exception e) {
			respondeCode = -1;
		}
	...
	private void createNoticeDialog() {
		adNoticeDialog = new AlertDialog.Builder(this)
				.setTitle(R.string.feed_back)
