package com.aurora.iunivoice.http;
...
public class HttpRequstData {
	...
	private static final String session_str_up = "\"returnCode\":\"-1\"";
	...
	public static String getShareURLStr(String http_url, String tid,
			int pid) {

		String url = http_url + tid + "-" + pid + "-1.html";
	...
	public static String getDecodeStr(String http_param) {
		...
		try {
			...
			//jo_str = new String(Base64.encode(http_param.getBytes(), Base64.DEFAULT));
	...
	public static String doPost(Map<String, String> map, String http_url)
		...
		// 判断cookie失效
		if (strResult.contains(session_str) || strResult.contains(session_str_up)) {
	...
	public static String doRequest(String request_url)
		...
		try {
			...
			if (stutas == 200) {
				...
				// 判断cookie失效
				if (result.contains(session_str) || result.contains(session_str_up)) {
					FileLog.e(TAG, "the cookie is timeout");
					if (getCookie()) {
	...
	public static boolean getCookie() {
		...
		if (AccountUtil.getInstance().isIuniOS()) {
			...
			Cursor cursor = IuniVoiceApp.getInstance().getContentResolver()
				...
				try {
					...
					try {
						...
						if (obj.getCode() == BaseResponseObject.CODE_SUCCESS) {
							String cookie = obj.getTgt();
							pref.setUserCookie(cookie);
							AccountHelper.cookie = cookie;
							r = true;
						}
	 ...
	 * @throws IOException
	 */
	public static String uploadImageFile(String actionUrl,
			Map<String, String> params, String fileName, String path)
			throws IOException {
		String BOUNDARY = java.util.UUID.randomUUID().toString();
		String PREFIX = "--", LINEND = "\r\n";
		...
		try {
			...
			// 发送文件数据
			if (null != isStream) {
				Log.i(TAG, "isStream length: " + isStream.available());
	 ...
	 * @throws IllegalStateException
	 */
	public static String doPost(String http_url, String jr) throws IllegalStateException, IOException {
		String strResult = "";
		DefaultHttpClient httpClient = new DefaultHttpClient();
		HttpPost post = new HttpPost(http_url);
		...
		Log.i(TAG, "dopost the url = " + http_url);
		...
		Log.i(TAG, "doPost the result = " + strResult);
		return strResult;
	}
}
