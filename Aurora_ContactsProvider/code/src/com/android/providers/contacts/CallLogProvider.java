...
public class CallLogProvider extends ContentProvider {
	...
	private static final int FETCH_APPWIDGET_CONTACTS=20000;
	private static final int CALLS_JION_DATA_VIEW_FOR_SMS = 20001;
	...
	static {
		...
		sURIMatcher.addURI(GnCallLog.AUTHORITY, "fetchappwidgetcontacts", FETCH_APPWIDGET_CONTACTS);
		sURIMatcher.addURI(GnCallLog.AUTHORITY, "callsjoindataviewforsms", CALLS_JION_DATA_VIEW_FOR_SMS);
	}
	...
	static {
		...
		sCallsProjectionMap.put(CallLog.Calls.NUMBER_PRESENTATION, CallLog.Calls.NUMBER_PRESENTATION);
		sCallsProjectionMap.put(Calls.DATE, Calls.DATE);
		sCallsProjectionMap.put(Calls.DURATION, Calls.DURATION);
		sCallsProjectionMap.put(CallLog.Calls.DATA_USAGE, CallLog.Calls.DATA_USAGE);
		sCallsProjectionMap.put(Calls.TYPE, Calls.TYPE);
		sCallsProjectionMap.put(CallLog.Calls.FEATURES, CallLog.Calls.FEATURES);
		sCallsProjectionMap.put(CallLog.Calls.PHONE_ACCOUNT_COMPONENT_NAME, CallLog.Calls.PHONE_ACCOUNT_COMPONENT_NAME);
		sCallsProjectionMap.put(CallLog.Calls.PHONE_ACCOUNT_ID, CallLog.Calls.PHONE_ACCOUNT_ID);
		...
		sCallsProjectionMap.put("photo", "photo");
	...
	static {
		...
		sCallsJoinDataViewProjectionMap.put("photo", "photo");
		sCallsJoinDataViewProjectionMap.put("name", "name");
	}
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
			String sortOrder) {
		Log.d(TAG, "callLogProvider query,uri:"+uri+" selection:" + selection+" projection:"+projection+" sortOrder:"+sortOrder+" selectionArgs:"+selectionArgs);
		...
		case CALLS_FILTER: {
			...
			if (!TextUtils.isEmpty(phoneNumber)) {
				...
				qb.appendWhere(CallLog.Calls.NUMBER_PRESENTATION + "!="
		...
		case FETCH_APPWIDGET_CONTACTS:{
			Log.d("liyang","FETCH_APPWIDGET_CONTACTS2");
			qb.setTables(mstableCallsJoinData);
			qb.setProjectionMap(sCallsJoinDataViewProjectionMap);
			qb.setStrict(true);
//			StringBuilder sb = new StringBuilder();
//			sb.append(Tables.CALLS + ".name is not null AND len("+Tables.CALLS+".name)<>0");			
//			qb.appendWhere(sb);
			break;
		}
		
		case CALLS_JION_DATA_VIEW_FOR_SMS:
		{
			qb.setTables(mstableCallsJoinData);
			qb.setProjectionMap(sCallsJoinDataViewProjectionMap);
			qb.setStrict(true);
			Cursor c = qb.query(db, projection, selection, selectionArgs, groupBy, null, sortOrder, null);
			return c;
		}
		...
		case AURORA_DIALER_CALLLOG_SEARCH:
		case FETCH_APPWIDGET_CONTACTS:
			c = combineCalllogMatchNumber1(c);
			break;		
...
//			}
	...
	private boolean isMatch(int flag,String curNumber,String savedNumber,int curType,int savedType,long duration,long savedDuration){

		if(flag==0){
			if(!curNumber.equals(savedNumber)) return false;//号码不相等，另起一行
		}else if(flag==1){
			if(!savedNumber.endsWith(curNumber)) return false;
		}
		if((curType==3)&&(curType!=savedType)) {
			Log.d("liyang","isMatch1");
			return false;//前一次不是漏接电话，本次是漏接电话，另起一行
		}
		if((curType==1)&&(duration==0)&&(curType!=savedType)) {
			Log.d("liyang","isMatch2");
			return false;//前一次不是拒接电话，本次是拒接电话，另起一行
		}
		if((curType==1)&&(duration==0)&&(curType==savedType)&&(savedDuration>0)) {
			Log.d("liyang","isMatch3");
			return false;//前一次是已接电话，本次是拒接电话，另起一行
		}

		if((savedType==3)&&(curType!=savedType)){
			Log.d("liyang","isMatch4");
			return false;//前一次是漏接电话，如果本次不是漏接电话，则另起一行
		}

		if((savedType==1)&&(savedDuration==0)&&(curType!=savedType)){
			Log.d("liyang","isMatch5");
			return false;//前一次是拒接电话，如果本次不是拒接电话，则另起一行
		}

		if((savedType==1)&&(savedDuration==0)&&(curType==savedType)&&(duration>0)){
			Log.d("liyang","isMatch6");
			return false;//前一次是拒接电话，如果本次是已接电话，则另起一行
		}
	...
	private Cursor combineCalllogMatchNumber(Cursor src) {
		...
		int typeIndex=src.getColumnIndex(sCallsJoinDataViewProjectionMap.get(Calls.TYPE));
		int durationIndex=src.getColumnIndex(sCallsJoinDataViewProjectionMap.get(Calls.DURATION));
		...
		do {
			...
			int curType = src.getInt(typeIndex);
			long duration=src.getLong(durationIndex);
			...
			for(int i=0;i<raws.size();i++){				
				if(i!=raws.size()-1) continue;
				Object[] r=raws.get(i);
				savedNumber = r[numberIndex].toString();
				savedNumberLen = savedNumber.length();
				savedType=Integer.parseInt(r[typeIndex].toString());
				savedDuration=Long.parseLong(r[durationIndex].toString());

				if (MATCH_LEN > curNumberLen) {
					/*isNumberMatch = ((curNumber.equals(savedNumber)) && (curType==savedType)&&(duration>0))
							||(curNumber.equals(savedNumber)) && (curType==1||curType==3)&&(duration==0L);*/

					/*isNumberMatch=(curNumber.equals(savedNumber) && (curType==savedType)&&(curType==1)&&(duration>0L)&&(savedDuration>0L))
							||(curNumber.equals(savedNumber) && (curType==savedType)&&(curType==1)&&(duration==0L)&&(savedDuration==0L))
							||(curNumber.equals(savedNumber) && (curType==savedType)&&(curType==2||curType==3));*/

					isNumberMatch=isMatch(0,curNumber,savedNumber,curType,savedType,duration,savedDuration);


				} else {
					/*isNumberMatch = ((savedNumber.endsWith(curNumber)) && (curType==savedType)&&(duration>0))
							||(savedNumber.endsWith(curNumber))&& (curType==1||curType==3)&&(duration==0L);*/

					/*isNumberMatch=(savedNumber.endsWith(curNumber) && (curType==savedType)&&(curType==1)&&(duration>0L)&&(savedDuration>0L))
							||(savedNumber.endsWith(curNumber) && (curType==savedType)&&(curType==1)&&(duration==0L)&&(savedDuration==0L))
							||(savedNumber.endsWith(curNumber) && (curType==savedType)&&(curType==2||curType==3));*/

					isNumberMatch=isMatch(1,curNumber,savedNumber,curType,savedType,duration,savedDuration);
				}
	...
	private Cursor combineCalllogMatchNumber1(Cursor src) {
		if (null == src || !src.moveToFirst()) {
			return src;
		}
