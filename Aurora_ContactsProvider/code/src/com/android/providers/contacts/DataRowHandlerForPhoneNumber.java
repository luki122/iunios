...
public class DataRowHandlerForPhoneNumber extends DataRowHandlerForCommonDataKind {
    ...
    public void appendSearchableData(IndexBuilder builder) {
        ...
        // aurora wangth 20140410 add begin
        try {
        	if (numberE164 != null && numberE164.endsWith(number)
        			&& mDbHelper.getCurrentCountryIso().equals("CN") && number.startsWith("+86")) {
            	numberE164 = number.substring(3, number.length());
            }
        } catch (Exception e) {
        	e.printStackTrace();
        }
        // aurora wangth 20140410 add end
