...
//    private static final boolean gnFLYflag = SystemProperties.get("ro.gn.oversea.custom").equals("RUSSIA_FLY"); 
    ...
    public static final byte[][] GN_PINYINS = {
         ...
         * “Zha”
         */
        {79, 117, 0, 0, 0, 0}, {72, 101, 0, 0, 0, 0}, {89, 117, 101,  0, 0, 0},
        {90, 104, 97,  0, 0, 0}, 
    };
    ...
    public static HanziToPinyin getInstance() {
        synchronized (HanziToPinyin.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA) || locale[i].equals(Locale.CHINESE)) { // aurora wangth 20140606 modify
    ...
    // aurona begin:
    private ArrayList<Token> auronaGetToken(char character, boolean flag) {
        ArrayList<Token> tokenResult = new ArrayList<Token>();
        Token token = new Token();
        final String letter = Character.toString(character);
        token.source = letter;
        int offset = -1;
        ...
        String[][] pinyinArrays = ChineseToPinyinHelper.translateMulti(letter, true);
        if (pinyinArrays.length > 1) {
            for (String[] pinyinArray : pinyinArrays) {
                for (String quanpin : pinyinArray) {
                    StringBuilder py = new StringBuilder();
                    py.append(quanpin);
                    Token t = new Token();
                    t.source = letter;
                    t.type = Token.PINYIN;
                    t.target = py.toString();
                    tokenResult.add(t);
                }
            }
        } else {
    ...
    public ArrayList<Token> get(final String input) {
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (character == ' ') {
                ...
                // light begin:
//                ArrayList<Token> tk = auronaGetToken(character, false);
//                for (Token t : tk) {
    ...
    public String getTokensForDialerSearch(final String input, StringBuilder offsets){
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (character == '-' || character == ',' ){
				...
				Character c;
				//aurora liumx 2013-08-31 delete for forgein language
//				if (gnFLYflag) { 
