...
public class GnDialerSearchHelper {
	...
	public interface GnDialerSearchViewColumns {
		...
		//		String PHOTO_URI="vds_photo_uri";
...
//		private String mPhotoUri;
		...
		public void read(Cursor cursor, String searchKey,int curIndex) {
			...
			if(curIndex==0){
				...
				preContactId = cursor.getString(CONTACT_ID_INDEX);
				cursor.moveToNext();
			}

			//			Log.d("liyang","preContactId1:"+preContactId+" mContactId:"+mContactId);


			if(!preContactId.equals(mContactId)){
				//				mPhotoUri=cursor.getString(PHOTO_URI_INDEX);
...
//			objs[GnDialerSearchResultColumns.PHOTO_URI_INDEX] = mPhotoUri;
...
//		public int PHOTO_URI_INDEX            = 12;
	...
	public Cursor query(SQLiteDatabase db, Uri uri, String selection) {
		...
		for (char c : searchKey.toCharArray()) {
			Log.d(TAG,"c:"+c);
			if (('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z')) {
				Log.d(TAG,"isAllDigit:"+isAllDigit);
				isAllDigit = false;
...
Log.d(TAG, "selection:"+selection);
		...
		Log.d(TAG,"queryCursor:"+queryCursor);
		if(queryCursor!=null) Log.d(TAG,"queryCursor count:"+queryCursor.getCount());
		...
		if (queryCursor != null && queryCursor.moveToFirst()) {
			...
			do {
				try {
					...
					sdr.read(queryCursor, searchKey,curIndex++);
					int index = -1;
					do {
						...
						if (-1 == index) {
							...
							for (int i = 0, srcLen = sdr.mQuanT9Len; i < sqlits.length; ++i) {
								...
								if (tmpSearchKey!=null&&tmpSearchKey.length()>0&&tmpSearchKey.charAt(0) != sqlits[i].charAt(0)) {
									if (i > 0) {
		...
		Log.d(TAG,"resultCursor:"+resultCursor);
		if(resultCursor!=null) Log.d(TAG,"resultCursor count:"+resultCursor.getCount());
	...
	public void createGnDialerSearchView(SQLiteDatabase db) {
		...
		/*//add for photoUri
		"," + GDS_DETAIL_TABLE + "." + Contacts.PHOTO_URI + 
		" AS " + GnDialerSearchViewColumns.PHOTO_URI +*/
		...
		/*"," +Views.CONTACTS + "." + Contacts.PHOTO_URI + " AS " + Contacts.PHOTO_URI+*/
