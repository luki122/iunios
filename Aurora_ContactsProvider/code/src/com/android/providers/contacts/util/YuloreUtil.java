...
public class YuloreUtil {
	...
	private YuloreUtil(Context context) {
		mYuloreHelper = new YuloreHelper(context);
	}
	...
	public static YuloreUtil getInstance(Context context) {
		...
		if (mYuloreUtil == null) {
			synchronized (mClock) {
				if (mYuloreUtil == null) {
					if (context != null) {
						mYuloreUtil = new YuloreUtil(context);
					} else {
						Log.e("YuloreUtil",
	...
	private RecognitionTelephone queryNumberInfo(String telNum, int type,
		...
		RecognitionTelephone r = null;
		try {
			r = mYuloreHelper.queryNumberInfo(KEY, SECRET, telNum, type, samrt,
					immediately, limit);
		} catch(Exception e) {
			e.printStackTrace();
		}
