...
/* package-protected */ final class PhotoProcessor {
    ...
    private void process() throws IOException {
        ...
        if (ContactsProvidersApplication.sIsGnZoomClipSupport) {
        	mThumbnailPhoto = gnGetScaledBitmapByGnCrop(mMaxThumbnailPhotoDim);
        } else {
    ...
    //Gionee:huangzy 20120728 add for CR00658678 start
    private Bitmap gnGetScaledBitmapByGnCrop(int maxDim) {
    	Bitmap scaledBitmap = null;
    	
        int oriWidth = mOriginal.getWidth();
        int oriHeight = mOriginal.getHeight();        
        if (oriWidth != oriHeight) {
            ...
            Log.i("James", "gnGetScaledBitmapByGnCrop  x=" + x + "  y=" +y + "    dstWidth=" +dstWidth);
            scaledBitmap = Bitmap.createBitmap(mOriginal, x, y, dstWidth, dstWidth);
        }
        
        if (null == scaledBitmap) {
        	scaledBitmap = mOriginal;
        }
        
        int scaledWidth = scaledBitmap.getWidth();
        int scaledHeight = scaledBitmap.getHeight();
        float scaleFactor = ((float) maxDim) / Math.max(scaledWidth, scaledHeight);
        if (scaleFactor < 1.0) {
            // Need to scale or crop the photo.
            Matrix matrix = new Matrix();
            matrix.setScale(scaleFactor, scaleFactor);
    ...
    //Gionee:huangzy 20120728 add for CR00658678 end
