...
import gionee.provider.GnContactsContract.Contacts.Photo;
...
import gionee.provider.GnContactsContract.PhoneticNameStyle;
...
public class ContactsDatabaseHelper extends SQLiteOpenHelper {
     ...
     */
    // modify for ota
//    static final int DATABASE_VERSION = 4001;
    
 // modify for 白名单（by liyang)
    static final int DATABASE_VERSION = 4003;
    
    static final String GROUP_RINGTONE = "group_ringtone";

    private static final String DATABASE_NAME = "contacts2.db";
    private static final String DATABASE_PRESENCE = "presence_db";
    
    static final String AURORA_SORT_KEY = "aurora_sort_key";
    
    // aurora reject begin
 	public static final class BlackTableData implements BaseColumns {
 		public static final String TABLE_NAME = "black";
 		public static final Uri CONTENT_URI = Uri.parse("content://"
 				+ GnContactsContract.AUTHORITY + "/black");
 	...
 	public static final class MarkTableData implements BaseColumns {
 		public static final String TABLE_NAME = "mark";
 		public static final Uri CONTENT_URI = Uri.parse("content://"
 				+ GnContactsContract.AUTHORITY + "/mark");
...
//                + "JOIN raw_contacts ON (data.raw_contact_id = raw_contacts._id)";
     ...
     // NOTE: If you want to refer to account name/type/data_set, AccountsColumns.CONCRETE_XXX
        ...
        public static final String RAW_CONTACTS_JOIN_SETTINGS_DATA_GROUPS = Tables.RAW_CONTACTS
                 ...
                 */
                 +"(" 
                 + "accounts.account_name IS NULL AND "
                 + "settings.account_name = 'Phone' AND "
                 + "accounts.account_type IS NULL AND "
...
//                + "      AND groups._id = data." + GroupMembership.GROUP_ROW_ID + ") ";
     // NOTE: If you want to refer to account name/type/data_set, AccountsColumns.CONCRETE_XXX
        ...
        public static final String GN_DIALER_SEARCH = "gn_dialer_search";
    ...
    public interface Joins {
        ...
        public static final String GROUP_MEMBER_COUNT =
            " LEFT OUTER JOIN (SELECT "
                ...
                + "FROM view_data "
                + "WHERE "
                    + " is_privacy=0 AND " // add by wangth 20141028
...
//                + Groups.DATA_SET + "=?";
    ...
    public interface ContactsColumns {
        ...
        public static final String CONCRETE_CONTACT_LAST_UPDATED_TIMESTAMP = Tables.CONTACTS + "."
                +  android.provider.ContactsContract.Contacts.CONTACT_LAST_UPDATED_TIMESTAMP;
    ...
    public interface GnSyncColumns {
    	String GN_VERSION = "gn_version";
    }
...
//                Tables.GROUPS + "." + Groups.ACCOUNT_TYPE;
    ...
    public interface NameLookupColumns {
        ...
        public static final String NAME_JIANPINTONUMBER = "name_jianpintonumber";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        // reset aurora privacy contacts and calls
        AuroraPrivacyUtils.resetPrivacyNumOfAllAccount(mContext,
        ...
        db.execSQL("CREATE TABLE " + Tables.CONTACTS + " (" +
                ...
                "is_privacy INTEGER NOT NULL DEFAULT 0," + // add by aurora wangth 20140923
                "is_whitelist INTEGER NOT NULL DEFAULT 0," + // add by liyang 20160112
                ContactsColumns.LAST_STATUS_UPDATE_ID + " INTEGER REFERENCES data(_id)," +
                android.provider.ContactsContract.Contacts.CONTACT_LAST_UPDATED_TIMESTAMP + " INTEGER" +
        ");");
        ...
        db.execSQL("CREATE TABLE " + Tables.RAW_CONTACTS + " (" +
               ...
               /* RawContacts.ACCOUNT_NAME + " STRING DEFAULT NULL, " +
                ...
                GnSyncColumns.GN_VERSION + " UNSIGNED BIG INT NOT NULL DEFAULT 0" + //Gionee:huangzy 20121128 add for CR00736966
                " , " + AURORA_SORT_KEY + " TEXT," + // aurora <wangth> <2013-12-13> add for aurora
                "is_privacy INTEGER NOT NULL DEFAULT 0," +
                "is_whitelist INTEGER NOT NULL DEFAULT 0," + // add by liyang 20160112
...
//        ");");
        ...
        db.execSQL("CREATE TABLE " + Tables.DATA + " (" +
                ...
                ", auto_record  INTEGER NOT NULL DEFAULT 0" +    // add by wangth
                ", rejected  INTEGER NOT NULL DEFAULT 0" +       // add by wangth
                ", is_privacy INTEGER NOT NULL DEFAULT 0" +          // add by wangth
        ");");
...
//        ");");
        ...
        db.execSQL("CREATE TABLE " + Tables.CALLS + " (" +
                ...
                GnSyncColumns.GN_VERSION + " UNSIGNED BIG INT NOT NULL DEFAULT 0," + //Gionee:huangzy 20121128 add for CR00736966 start
                "area" + " TEXT" +
                ",reject" + " INTEGER NOT NULL DEFAULT 0" +  // add by wangth
                ",mark" + " TEXT" +
                ",black_name" + " TEXT" +
...
//        db.execSQL("INSERT INTO " + Tables.ACCOUNTS + " VALUES(NULL, NULL, NULL)");
        ...
        if (ContactsProvidersApplication.sIsGnDialerSearchSupport) {
            GnDialerSearchHelper.getInstance().createGnDialerSearchTable(db);
            GnDialerSearchHelper.getInstance().createGnDialerSearchView(db);
        }
        //Gionee:huangzy 20121011 add for CR00710695 end
        
        createRejectTables(db);
        upgradeToVersion738(db);
        
        createGroupsView(db);
        
        createContactsTriggers(db);  //createContactsTriggersEx(db); // aurora wangth 201400917 modify
        ...
        ContentResolver.requestSync(null /* all accounts */,
                GnContactsContract.AUTHORITY, new Bundle());
    ...
    private void createContactsViews(SQLiteDatabase db) {
        ...
        String contactOptionColumns =
                ...
                + ContactsColumns.CONCRETE_PINNED
                        + " AS " + ContactsContract.RawContacts.PINNED + ", "
        ...
        String contactNameColumns =
                ...
                + ", contacts.is_privacy AS is_privacy"  // add by wangth
                + ", contacts.is_whitelist AS is_whitelist"  // add by liyang
                + ", name_raw_contact.call_notification_type AS call_notification_type"
                ;
...
//                        + Tables.DATA + "." + GroupMembership.GROUP_ROW_ID + ")";
        ...
        /*String rawContactsSelect = "SELECT "
                ...
                + syncColumns  + ", "
                + GnSyncColumns.GN_VERSION //Gionee:huangzy 20121128 add for CR00736966
                + ", " + AURORA_SORT_KEY  // aurora <wangth> <2013-12-13> add for aurora
                + ", is_privacy"
        ...
        String contactsColumns =
                ...
                + ContactsColumns.CONCRETE_STARRED
                        + " AS " + Contacts.STARRED + ", "
                + ContactsColumns.CONCRETE_PINNED
                        + " AS " + ContactsContract.Contacts.PINNED + ", "
                + ContactsColumns.CONCRETE_TIMES_CONTACTED
...
//                + " FROM " + Tables.GROUPS_JOIN_PACKAGES;
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        //Gionee:huangzy 20120828 add for CR00680614,CR00680572 start
        if (oldVersion < 667) {
            oldVersion = 667;
        }
        //Gionee:huangzy 20120828 add for CR00680614,CR00680572 end

        //Gionee:huangzy 20121011 add for CR00710695 start
        if (oldVersion < 669) {
            if (ContactsProvidersApplication.sIsGnDialerSearchSupport) {
                upgradeToVersion669(db);    
            }
            oldVersion = 669;
        }
        //Gionee:huangzy 20121011 add for CR00710695 end
        ...
        //Gionee:huangzy 20121128 add for CR00736966 start
        if (oldVersion < 670) {
            upgradeToVersion670(db);
            oldVersion = 670;
            upgradeViewsAndTriggers = true;
        }
        //Gionee:huangzy 20121128 add for CR00736966 end
        
        // the following lines added by aurora
        if (oldVersion < 671) {
            upgradeToVersion671(db);
            oldVersion = 671;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 672) {
            upgradeToVersion672(db);
            oldVersion = 672;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 673) {
            upgradeToVersion673(db);
            oldVersion = 673;
            upgradeViewsAndTriggers = true;
        }
...
//            createContactsViews(db);
        ...
        // modify for highlight correct(fuhao ok)
        if (oldVersion < 718) {
            upgradeToVersion672(db);
            oldVersion = 718;
            upgradeViewsAndTriggers = true;
        ...
        // default group(family, friend, classmates, colleague) check
        if (oldVersion < 721) {
            upgradeToVersion721(db);
            oldVersion = 721;
            upgradeViewsAndTriggers = true;
        ...
        if (oldVersion < 724) {
            upgradeToVersion724(db);
            oldVersion = 724;
            upgradeViewsAndTriggers = true;
        }
        
        // modify for sort_key(android 4.4)
        if (oldVersion < 728) {
            upgradeToVersion728(db);
            oldVersion = 728;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 729) {
            upgradeToVersion729(db);
            oldVersion = 729;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 737) {
            upgradeToVersion737(db);
            oldVersion = 737;
            upgradeViewsAndTriggers = true;
        }
        ...
        if (oldVersion < 739) {
            upgradeToVersion739(db);
            oldVersion = 739;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 740) {
            upgradeToVersion740(db);
            oldVersion = 740;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 741) {
            upgradeToVersion741(db);
            oldVersion = 741;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 3000) {
            upgradeToVersion3000(db);
            oldVersion = 3000;
            upgradeViewsAndTriggers = true;
        }
        
        // add for aurora privacy
        if (oldVersion < 3001) {
            upgradeToVersion3001(db);
            oldVersion = 3001;
            upgradeViewsAndTriggers = true;
        }
        ...
        if (oldVersion < 3006) {
            oldVersion = 3006;
            upgradeToVersion3006(db);
            upgradeViewsAndTriggers = true;
        }
        
        if(oldVersion < 4000){
            oldVersion = 4000;
            upgradeToVersion4000(db);
            upgradeNameLookup = true;
            upgradeViewsAndTriggers = true;
            upgradeSearchIndex = true;
        }
     ...
     // add for whitelist
        if (oldVersion < 4002) {
            upgradeToVersion4002(db);
            oldVersion = 4002;
            upgradeViewsAndTriggers = true;
        }
        
        if (oldVersion < 4003) {
            upgradeToVersion4003(db);
            oldVersion = 4003;
            upgradeViewsAndTriggers = true;
        }
        ...
        if (upgradeViewsAndTriggers) {
            ...
            createContactsTriggers(db);//createContactsTriggersEx(db); // modify for wangth 20140917
            createContactsIndexes(db);
            updateSqliteStats(db);
            upgradeLegacyApiSupport = true;
            mReopenDatabase = true;
    ...
    //Gionee:huangzy 20121011 add for CR00710695 start
    private void upgradeToVersion669(SQLiteDatabase db) {
    	final GnDialerSearchHelper dsHelper = GnDialerSearchHelper.getInstance();
    	dsHelper.createGnDialerSearchTable(db);
		
		Cursor c = db.query(Tables.RAW_CONTACTS, new String[]{RawContacts._ID, RawContacts.DISPLAY_NAME_PRIMARY}, 
				RawContacts.DELETED + "=0", null, null, null, RawContacts.SORT_KEY_PRIMARY);
		 
		if (null != c) {
			if (c.moveToFirst()) {
				do {
					dsHelper.insertNameForGnDialerSearch(db, c.getInt(0), c.getString(1));
				} while(c.moveToNext());
			}
			c.close();
		}
    ...
    //Gionee:huangzy 20121128 add for CR00736966 start
    private void upgradeToVersion670(SQLiteDatabase db) {
    	if (ContactsProvidersApplication.sIsGnSyncSupport) {
    		//mark
    	}
    	try {
    		db.execSQL("ALTER TABLE " + Tables.RAW_CONTACTS + 
    				" ADD " + GnSyncColumns.GN_VERSION + " UNSIGNED BIG INT NOT NULL DEFAULT 0");
...
//            createContactsViews(db);
    ...
    // aurora <wangth> <2013-10-11> modify for aurora ui begin
    private void upgradeToVersion671(SQLiteDatabase db) {
        try {
            db.execSQL("ALTER TABLE " + Tables.GROUPS + 
                    " ADD " + GROUP_RINGTONE + " TEXT;");
        } catch (SQLException e) {
            return;
        }
    }
    
    private void upgradeToVersion672(SQLiteDatabase db) {
        final GnDialerSearchHelper dsHelper = GnDialerSearchHelper.getInstance();
        dsHelper.createGnDialerSearchTable(db);
    ...
    private void upgradeToVersion673(SQLiteDatabase db) {
        try {
            db.execSQL("ALTER TABLE " + Tables.RAW_CONTACTS + 
                    " ADD " + AURORA_SORT_KEY + " TEXT; ");
        } catch (SQLException e) {
            return;
        }
        
        Cursor c = db.query(Tables.RAW_CONTACTS, new String[]{RawContacts._ID, RawContacts.DISPLAY_NAME_PRIMARY, RawContacts.SORT_KEY_PRIMARY}, 
                RawContacts.DELETED + "=0", null, null, null, RawContacts.SORT_KEY_PRIMARY);
         
        if (null != c) {
            if (c.moveToFirst()) {
                do {
                    auroraUpdateAuroraSortKey(db, c.getString(1), c.getString(2), c.getString(0));
                } while(c.moveToNext());
            }
            c.close();
        }
    ...
    private void upgradeToVersion721(SQLiteDatabase db) {
    	String[] groupsName = new String[]{null, null, null, null,};
        groupsName[0] = mContext.getString(R.string.aurora_group_family);
        groupsName[1] = mContext.getString(R.string.aurora_group_friend);
        groupsName[2] = mContext.getString(R.string.aurora_group_classmate);
        groupsName[3] = mContext.getString(R.string.aurora_group_colleague);
        
        for (int i = 0; i < groupsName.length; i++) {
            if (checkName(db, groupsName[i])) {
            	ContentValues values = new ContentValues();
            	values.put(Groups.TITLE, groupsName[i]);
            	values.put(Groups.ACCOUNT_NAME, "Phone");
            	values.put(Groups.ACCOUNT_TYPE, "Local Phone Account");
            	values.put(Groups.GROUP_VISIBLE, 1);
                db.insert(Tables.GROUPS, null, values);
            }
        }
    }
    
    private void upgradeToVersion722(SQLiteDatabase db) {
        try {
            db.execSQL("ALTER TABLE " + Tables.DATA + 
                    " ADD auto_record INTEGER NOT NULL DEFAULT 0; ");            
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    
    private void upgradeToVersion724(SQLiteDatabase db) {
        updatePhoneLookup(db);
        
        updateCalls(db);
    }
    
    private void upgradeToVersion728(SQLiteDatabase db) {
        Cursor c = db.query(Tables.RAW_CONTACTS, new String[]{RawContacts._ID, RawContacts.DISPLAY_NAME_PRIMARY, RawContacts.SORT_KEY_PRIMARY}, 
                RawContacts.DELETED + "=0", null, null, null, RawContacts.SORT_KEY_PRIMARY);
    ...
    private void createRejectTables(SQLiteDatabase db) {
    	db.execSQL("create table " + BlackTableData.TABLE_NAME + "("
				+ BlackTableData._ID
				+ " INTEGER PRIMARY KEY autoincrement,"
				+ BlackTableData.ISBLACK + " INTEGER,"
				+ BlackTableData.LABLE + " TEXT,"
				+ "user_mark" + " INTEGER NOT NULL DEFAULT 0,"
				+ BlackTableData.BLACK_NAME + " TEXT,"
				+ BlackTableData.NUMBER + " TEXT,"
				+ BlackTableData.REJECT + " INTEGER)" + ";");
    	
		db.execSQL("create table " + MarkTableData.TABLE_NAME + "("
				+ MarkTableData._ID
				+ " INTEGER PRIMARY KEY autoincrement,"
				+ MarkTableData.LABLE + " TEXT,"
				+ "number TEXT)" + ";");
    }
    
    private void upgradeToVersion729(SQLiteDatabase db) {
    	try {
    		// calls
            db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD reject INTEGER NOT NULL DEFAULT 0; ");
            db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD mark TEXT; ");
            db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD black_name TEXT; ");
            ...
            // data
            db.execSQL("ALTER TABLE " + Tables.DATA + 
                    " ADD rejected INTEGER NOT NULL DEFAULT 0; ");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private void upgradeToVersion737(SQLiteDatabase db) {
    	try {
    		// calls
    		db.execSQL("CREATE TABLE temp_calls" + " (" +
                    Calls._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," +
                    Calls.NUMBER + " TEXT," +
                    Calls.DATE + " INTEGER," +
                    Calls.DURATION + " INTEGER," +
    		...
    		Cursor cursor = db.query(Tables.CALLS, null, null, null, null, null, null);
    		if (cursor != null) {
    			if (cursor.moveToFirst()) {
    				do {
    					ContentValues cv = new ContentValues();
    					cv.put(Calls._ID, cursor.getString(0));
    		            cv.put(Calls.NUMBER, cursor.getString(1));
    		            cv.put(Calls.DATE, cursor.getString(2));
    		            cv.put(Calls.DURATION, cursor.getString(3));
    		            cv.put(Calls.TYPE, cursor.getString(4));
    		            cv.put(Calls.NEW, cursor.getString(5));
    		            cv.put(Calls.CACHED_NAME, cursor.getString(6));
    		            cv.put(Calls.CACHED_NUMBER_TYPE, cursor.getString(7));
    		            cv.put(Calls.CACHED_NUMBER_LABEL, cursor.getString(8));
    		            cv.put(Calls.COUNTRY_ISO, cursor.getString(9));
    		            cv.put(Calls.VOICEMAIL_URI, cursor.getString(10));
    		            cv.put(Calls.IS_READ, cursor.getString(11));
    		            cv.put(Calls.GEOCODED_LOCATION, cursor.getString(12));
    		            cv.put(Calls.CACHED_LOOKUP_URI, cursor.getString(13));
    		            cv.put(Calls.CACHED_MATCHED_NUMBER, cursor.getString(14));
    		            cv.put(Calls.CACHED_NORMALIZED_NUMBER, cursor.getString(15));
    		            cv.put(Calls.CACHED_PHOTO_ID, cursor.getString(16));
    		            cv.put(Calls.CACHED_FORMATTED_NUMBER, cursor.getString(17));
    		            cv.put(Voicemails._DATA, cursor.getString(18));
    		            cv.put(Voicemails.HAS_CONTENT, cursor.getString(19));
    		            cv.put(Voicemails.MIME_TYPE, cursor.getString(20));
    		            cv.put(Voicemails.SOURCE_DATA, cursor.getString(21));
    		            cv.put(Voicemails.SOURCE_PACKAGE, cursor.getString(22));
    		            cv.put(Voicemails.STATE, cursor.getString(23));
    		            cv.put(Calls.SIM_ID, cursor.getString(24));
    		            cv.put(Calls.VTCALL, cursor.getString(25));
    		            cv.put(Calls.RAW_CONTACT_ID, cursor.getString(26));
    		            cv.put(Calls.DATA_ID, cursor.getString(27));
    		            cv.put(GnSyncColumns.GN_VERSION, cursor.getString(28));
    		            cv.put("area", cursor.getString(29));
    		            cv.put("reject", cursor.getString(30));
    		            cv.put("mark", cursor.getString(31));
    		            cv.put("black_name", cursor.getString(32));
    		            
    		            db.insert("temp_calls", null, cv);
    		...
    		cursor = db.query("temp_calls", null, null, null, null, null, null);
    		if (cursor != null) {
    			if (cursor.moveToFirst()) {
    				do {
    					ContentValues cv = new ContentValues();
    		...
    		db.execSQL("DROP TABLE IF EXISTS temp_calls");
    ...
    private void upgradeToVersion738(SQLiteDatabase db) {
    	String[] marks = new String[]{null, null, null, null, null};
    	marks[0] = mContext.getString(R.string.aurora_mark_1);
    	marks[1] = mContext.getString(R.string.aurora_mark_2);
    	marks[2] = mContext.getString(R.string.aurora_mark_3);
    	marks[3] = mContext.getString(R.string.aurora_mark_4);
    	marks[4] = mContext.getString(R.string.aurora_mark_5);
        
        for (int i = 0; i < marks.length; i++) {
            if (checkMarkName(db, marks[i])) {
            	ContentValues values = new ContentValues();
            	values.put("lable", marks[i]);
                db.insert(MarkTableData.TABLE_NAME, null, values);
            }
        }
    }
    
    private void upgradeToVersion739(SQLiteDatabase db) {
    	try {
    		db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD user_mark INTEGER NOT NULL DEFAULT 0; ");
        	db.execSQL("ALTER TABLE " + "black" + 
                    " ADD user_mark INTEGER NOT NULL DEFAULT 0; ");
    ...
    private void upgradeToVersion740(SQLiteDatabase db) {
    	try {
    		db.execSQL("ALTER TABLE mark ADD number TEXT; ");
    	} catch (SQLiteException e) {
    		e.printStackTrace();
    	}
    }
    
    private void upgradeToVersion741(SQLiteDatabase db) {
    	try {
    ...
    private void upgradeToVersion3000(SQLiteDatabase db) {
        try {
    ...
    private void upgradeToVersion3001(SQLiteDatabase db) {
        try {
    ...
    //add by liyang:添加白名单字段，0－不是白名单 1－是白名单
    private void upgradeToVersion4002(SQLiteDatabase db) {
        try {
            db.execSQL("ALTER TABLE " + Tables.CONTACTS + 
                    " ADD is_whitelist INTEGER NOT NULL DEFAULT 0; ");
            db.execSQL("ALTER TABLE " + Tables.RAW_CONTACTS + 
                    " ADD is_whitelist INTEGER NOT NULL DEFAULT 0; ");
        } catch (SQLiteException e) {
            e.printStackTrace();
    ...
    private void upgradeToVersion4003(SQLiteDatabase db) {
        try {
            db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD photo BLOB; ");
        } catch (SQLiteException e) {
...
//    	upgradeToVersion800(db);  字段和我们之前的upgradeToVersion741一样，不知道含义是否一样
    ...
    private void createDefaultAccount(SQLiteDatabase db){
    	ContentValues values = new ContentValues();
    	values.put(AccountsColumns.ACCOUNT_NAME, "Phone");
    	values.put(AccountsColumns.ACCOUNT_TYPE, "Local Phone Account");
    	long id = db.insert(Tables.ACCOUNTS, null, values);
    	String[] groupsName = new String[]{null, null, null, null,};
        groupsName[0] = mContext.getString(R.string.aurora_group_family);
        groupsName[1] = mContext.getString(R.string.aurora_group_friend);
        groupsName[2] = mContext.getString(R.string.aurora_group_classmate);
        ...
        for (int i = 0; i < groupsName.length; i++) {
        	values.clear();
        	values.put(Groups.TITLE, groupsName[i]);
        	values.put(GroupsColumns.ACCOUNT_ID, id);
        	values.put(Groups.GROUP_VISIBLE, 1);
            db.insert(Tables.GROUPS, null, values);
        }
        
    	values.clear();
    	values.put(Settings.ACCOUNT_NAME, "Phone");
    	values.put(Settings.ACCOUNT_TYPE, "Local Phone Account");
    	values.put(Settings.UNGROUPED_VISIBLE, 1);
    	 id = db.insert(Tables.SETTINGS, null, values);
    }
    
    public AccountWithDataSet getDefaultAccount(){
    	return AccountWithDataSet.get("Phone", "Local Phone Account", null);
    }
    
    public boolean isDefaultAccount(AccountWithDataSet accountWithDataSet){
        if (accountWithDataSet.getDataSet() == null) {
            if ("Phone".equals(accountWithDataSet.getAccountName()) && "Local Phone Account".equals(accountWithDataSet.getAccountType())) {
                return true;           
            }
         }
         return false;
    ...
    private boolean checkMarkName (SQLiteDatabase db, String name) {
    	String tempName = null;
        boolean nameExists = false;
        
        if (!nameExists) {
            Cursor cursor = db.query(
            		MarkTableData.TABLE_NAME,
                    new String[] { Groups._ID },
                    "lable='" + name + "'",  null, null, null, null);
        ...
        Log.d(TAG, "name = " + name + "   nameExists = " + nameExists);
        if (nameExists) {
            return false;
        } else {
            return true;
    ...
    // update phone_lookup(min_match)
    private void updatePhoneLookup(SQLiteDatabase db) {
        try {
            String[] sel = new String[] {PhoneLookupColumns.RAW_CONTACT_ID, PhoneLookupColumns.NORMALIZED_NUMBER};
            Cursor cursor = db.query(Tables.PHONE_LOOKUP, sel, null, null, null, null, null);
            ...
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    ...
                    do {
                        cv.clear();
                        rawId = cursor.getString(0);
                        number = cursor.getString(1);
                        if (number != null) {
                            minMatch = PhoneNumberUtils.toCallerIDMinMatch(number);
                        }
                        
                        cv.put(PhoneLookupColumns.MIN_MATCH, minMatch);
                        db.update(Tables.PHONE_LOOKUP, cv, 
                                PhoneLookupColumns.RAW_CONTACT_ID + "='" + rawId + "' and " + 
                                PhoneLookupColumns.NORMALIZED_NUMBER + "='" + number + "'", null);
                    } while (cursor.moveToNext());
                }
    ...
    // update calls(raw_contact_id and data_id)
    public static void updateCalls(SQLiteDatabase db) {
        try {
            String[] sel = new String[] {"_id", "number", "name", "raw_contact_id", "data_id"};
            Cursor cursor = db.query(Tables.CALLS, sel, null, null, null, null, null);
            ...
            if (cursor != null) {
                if (cursor.moveToFirst()) {
                    ...
                    do {
                        cv.clear();
                        rawContactId = cursor.getString(3);
                        dataId = cursor.getString(4);
                        if (rawContactId != null && dataId != null) {
                            name = cursor.getString(2);
                            number = cursor.getString(1);
                            boolean match = matchForNumber(db, number);
                            if (!match) {
                                id = cursor.getString(0);
                                Log.e(TAG, "id = " + id);
                                cv.put("raw_contact_id", nullStr);
                                cv.put("data_id", nullStr);
                                cv.put("name", nullStr);
                                db.update(Tables.CALLS, cv, Calls._ID + "=" + id, null);
                            }
                        }
                    } while (cursor.moveToNext());
                }
    ...
    public static boolean matchForNumber(SQLiteDatabase db, String number) {
        boolean result = false;
        String minNumber = PhoneNumberUtils.toCallerIDMinMatch(number);
        try {
            Cursor cursor = db.query(Tables.PHONE_LOOKUP, new String[] {"data_id", "raw_contact_id"}, 
                    "min_match='" + minNumber + "'", null, null, null, null);
    ...
    private boolean checkName(SQLiteDatabase db, String name) {
...
//      }
      ...
      if (!nameExists) {
          Cursor cursor = db.query(
                  ...
                  Groups.TITLE + "=? AND " + Groups.ACCOUNT_NAME + " =? AND "
                          ...
                          + "=0",
                  new String[] {name, "Phone", "Local Phone Account"}, null, null, null);
          if (cursor == null || cursor.getCount() == 0) {
              if (cursor != null) {
                  cursor.close();
              }
    ...
    // aurora <wangth> <2013-10-11> modify for aurora ui end
    ...
    private void upgradeToVersion714(SQLiteDatabase db) {
        try {           
            Cursor c = db.query(Tables.RAW_CONTACTS, 
                    ...
                    RawContacts.DELETED + "=0", null, null, null, null);
                ...
                if (c.moveToFirst()) {
                    do {
                        ...
                        if (accountType == null && accountName == null) {
                            ContentValues cv = new ContentValues();
                            cv.put(RawContacts.ACCOUNT_NAME, "Phone");
                            cv.put(RawContacts.ACCOUNT_TYPE, "Local Phone Account");
                            db.update(Tables.RAW_CONTACTS, cv, RawContacts._ID + "=" + id, null);
                        }
                    } while (c.moveToNext());
                }
     ...
     */
    public void setLocale(ContactsProvider2 provider, Locale locale, boolean updateDialerSearch) {
        ...
        try {
            ...
            loadNicknameLookupTable(db);
            insertNameLookup(db);
            rebuildSortKeys(db, provider, updateDialerSearch);
            createContactsIndexes(db);
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
     ...
     */
    private void rebuildSortKeys(SQLiteDatabase db, ContactsProvider2 provider, boolean updateDialSearch) {
        Cursor cursor = db.query(Tables.RAW_CONTACTS, new String[]{RawContacts._ID},
                null, null, null, null, null);
        try {
            while (cursor.moveToNext()) {
                long rawContactId = cursor.getLong(0);
                updateRawContactDisplayName(db, rawContactId, updateDialSearch);
            }
        } finally {
            cursor.close();
        }
    ...
    private long lookupMimeTypeId(String mimetype, SQLiteDatabase db) {
        // Gionee:wangth 20121211 add for CR00741253 begin
        if (ContactsProvidersApplication.sIsGnContactsSupport && !db.isOpen()) {
            return -1;
        }
        // Gionee:wangth 20121211 add for CR00741253 end
...
//    }
     ...
     */
    public void updateRawContactDisplayName(SQLiteDatabase db, long rawContactId, boolean updateDialSearch) {
        ...
        try {
            while (c.moveToNext()) {
                ...
                if (mimeType == getMimeTypeIdForStructuredName()) {
                    ...
                    bestDisplayName = c.getString(RawContactNameQuery.DATA1);
        ...
        if (bestDisplayNameSource == DisplayNameSources.STRUCTURED_NAME) {
            ...
            // aurora <wangth> <2013-12-17> modify for aurora begin
            /*
            displayNamePrimary = mNameSplitter.join(bestName, true, true);
            displayNameAlternative = mNameSplitter.join(bestName, false, true);
            */
            displayNamePrimary = bestDisplayName;
            displayNameAlternative = bestDisplayName;
            // aurora <wangth> <2013-12-17> modify for aurora end
        ...
        if (ContactsProvidersApplication.sIsGnDialerSearchSupport) {
			...
			GnDialerSearchHelper.getInstance().updateOrInsertNameDialerSearch(db,
					rawContactId, displayNamePrimary);			
		}
        //Gionee:huangzy 20121011 add for CR00710695 end
        
        // aurora <wangth> <2013-12-13> add for aurora begin
        Log.d(TAG, "updateRawContactDisplayName name = ");
        auroraUpdateAuroraSortKey(db, displayNamePrimary, sortKeyPrimary, String.valueOf(rawContactId));
        // aurora <wangth> <2013-12-13> add for aurora end
    }
    ...
    //Gionee:huangzy 20121128 add for CR00736966 start
    public static long toNextGnVersion(Context context) {
    	if (ContactsProvidersApplication.sIsGnSyncSupport) {
        	SharedPreferences preference = PreferenceManager.getDefaultSharedPreferences(context);
        	long nextGnVersion = preference.getLong(GnSyncColumns.GN_VERSION, 0) + 1;
        	long curTime = System.currentTimeMillis();
        	if (nextGnVersion < curTime) {
        		nextGnVersion = curTime;
        	}
        	preference.edit().putLong(GnSyncColumns.GN_VERSION, nextGnVersion).commit();
        	return nextGnVersion;
        }
    	return 0;
    }
    
    public static void writeGnVersion(Context context, ContentValues values) {
    	if (ContactsProvidersApplication.sIsGnSyncSupport) {
        	values.put(GnSyncColumns.GN_VERSION, toNextGnVersion(context));
        }
    }
    //Gionee:huangzy 20121128 add for CR00736966 end
    
    // aurora <wangth> <2013-12-13> add for aurora begin
    public static boolean firstIsFuHao(String tag) {
        if (tag == null) {
            return false;    
        }
    ...
    public static boolean hasHanZi(String tag) {
        if (tag == null) {
            return false;    
        }
    ...
    public static void auroraUpdateAuroraSortKey(SQLiteDatabase db, String displayName, String sortKeyPrimary, String rawContactId) {
        final GnDialerSearchHelper dsHelper = GnDialerSearchHelper.getInstance();
        StringBuilder sortKey = new StringBuilder();
        
        if (displayName != null && sortKeyPrimary != null && sortKeyPrimary.equals(displayName)) {
            boolean hasChin = hasHanZi(displayName);
            String[][] pinyinArrays = null;
            
            if (hasChin) {
                String subStr = null;
                for (int i = 0; i < displayName.length(); i++) {
                    subStr = displayName.substring(i, i + 1);
                    if (hasHanZi(subStr)) {
                        if (i > 0) {
                            sortKey.append(" ");
                        }
                        pinyinArrays = ChineseToPinyinHelper.translateMulti(subStr, true);
                        
                        if (pinyinArrays != null && pinyinArrays != null) {
                            sortKey.append(pinyinArrays[0][0].toUpperCase());
                        }
                        sortKey.append(" ");
                        sortKey.append(subStr);
                    } else {
                        sortKey.append(subStr);
                    }
                }
            } else {
                sortKey.append(displayName);
            }
            
            ContentValues cv = new ContentValues();
            cv.put("sort_key", sortKey.toString());
            db.update(Tables.RAW_CONTACTS, cv, RawContacts._ID + " = " + rawContactId, null);
        }
    }
    // aurora <wangth> <2013-12-13> add for aurora end
}
