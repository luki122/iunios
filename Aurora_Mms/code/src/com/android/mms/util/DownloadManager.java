...
public class DownloadManager {
            ...
            if (TelephonyIntents.ACTION_SERVICE_STATE_CHANGED.equals(intent.getAction())) {
                ...
                synchronized (sInstance) {
                    ...
                    if (MmsApp.mGnMultiSimMessage) {//Gionee guoyx 20130111 CR00754375
                        ...
                        if (null == si) {
                            Log.e(MmsApp.TXN_TAG, "Download manager:SIMInfo is null for slot " + intent.getIntExtra(GnPhone.GEMINI_SIM_ID_KEY, -1));
                            return;
                        }
    ...
    private DownloadManager(Context context) {
        ...
        mPreferences = AuroraPreferenceManager.getDefaultSharedPreferences(context);
    ...
    static boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming) {
        // Aurora xuyong 2014-04-19 modified for bug #4418 start
        // we should set the default value to be FALSE but not TRUE,or the MMS 
        // would be auto downloaded if we haven't initialnize the message setting preference
        boolean autoDownload = prefs.getBoolean(
                MessagingPreferenceActivity.AUTO_RETRIEVAL, false);
        // Aurora xuyong 2014-04-19 modified for bug #4418 end
    ...
    private boolean getAutoDownloadState(SharedPreferences prefs, boolean roaming, int simId) {
        // Aurora xuyong 2014-06-09 modified for bug #5403 start
        boolean autoDownload = prefs.getBoolean(MessagingPreferenceActivity.AUTO_RETRIEVAL, false);
        // Aurora xuyong 2014-06-09 modified for bug #5403 end
        Log.d(MmsApp.TXN_TAG, "sim " + simId + " auto download without roaming -> " + autoDownload);
        if (autoDownload) {
            // Aurora xuyong 2014-06-09 modified for bug #5403 start
            boolean alwaysAuto = prefs.getBoolean(MessagingPreferenceActivity.RETRIEVAL_DURING_ROAMING, false);
            // Aurora xuyong 2014-06-09 modified for bug #5403 end
            Log.d(MmsApp.TXN_TAG, "sim " + simId + " auto download during roaming -> " + alwaysAuto);
            if (!roaming || alwaysAuto) {
                return true;
            }
        }
    ...
    public boolean isAuto(int simId) {
        SharedPreferences preferences = AuroraPreferenceManager.getDefaultSharedPreferences(mContext);
    ...
    private boolean isRoaming(int simId) {
        ...
        return GnTelephonyManager.isNetworkRoamingGemini(slotId);
    }
    ...
    public int getState(Uri uri) {
        Cursor cursor = SqliteWrapper.query(mContext, mContext.getContentResolver(),
                            ...
                            uri, new String[] {Mms.STATUS}, "is_privacy >= 0", null, null);
