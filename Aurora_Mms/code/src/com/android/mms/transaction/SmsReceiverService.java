...
// Aurora xuyong 2015-01-30 added for aurora's new feature end
// Aurora xuyong 2016-01-30 added for xy-smartsms start
import com.android.mms.ui.MessageUtils;
// Aurora xuyong 2016-01-30 added for xy-smartsms end
    ...
    private void handleSmsReceived(Intent intent, int error) {
        ...
        if (messageUri != null) {
            ...
            if (MmsApp.sHasXySmartSmsFeature) {
                ...
                int slotId = MessageUtils.getSlotById(this, intent.getIntExtra(GnPhone.GEMINI_SIM_ID_KEY, -1));
                XySdkUtil.parseMsg(messageUri.getLastPathSegment(), msgs, XySdkUtil.SMARTSMS_PARSE_TYPE_POUPP, slotId);
     ...
     */
     private Uri insertMessage(Context context, Intent intent, int error, String format) {
        ...
        if (!Utils.hasLollipop()) {
            ...
            // Aurora xuyong 2015-06-29 added for android 5.1+ adapt start
        	intent.putExtra(GnPhone.GEMINI_SIM_ID_KEY, slotId);
            // Aurora xuyong 2015-06-29 added for android 5.1+ adapt end
        }
...
//        }
        ...
        // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
        values.put("notification_index", Utils.isNotificationMsg(SmsReceiverService.this, mInstance.getNumber(), mInstance.getBody()) ? 1 : 0);
        // Aurora xuyong 2015-12-04 added for aurora2.0 new feature end
        // Aurora xuyong 2014-07-02 added for reject feature start
        ...
        if (MmsApp.mGnMultiSimMessage) {//guoyx 20130116
            // conver slot id to sim id
            // Aurora xuyong 2015-06-26 modified for android 5.1+ adapt start     
            SIMInfo si = null;
            if (!Utils.hasLollipop()) {
                si = SIMInfo.getSIMInfoBySlot(context, gnSms.getMessageSimId(sms));
		        if (null == si) {
		            Log.e(MmsApp.TXN_TAG, "SmsReceiverService:SIMInfo is null for slot " + gnSms.getMessageSimId(sms));
		            return null;
		        }
            } else {
                si = SIMInfo.getSIMInfoById(context, gnSms.getMessageSimId(sms));
		        if (null == si) {
		            Log.e(MmsApp.TXN_TAG, "SmsReceiverService:SIMInfo is null for subscription " + gnSms.getMessageSimId(sms));
		            return null;
		        }
            }
            // Aurora xuyong 2015-06-26 modified for android 5.1+ adapt end
