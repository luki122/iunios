...
//Aurora xuyong 2015-07-22 added for bug #14413 start
import java.util.Queue;
import java.util.concurrent.ArrayBlockingQueue;
//Aurora xuyong 2015-07-22 added for bug #14413 end
        ...
        for (TxnRequest req : txnRequestsMap ) {
            if (req.serviceId == startId) {
                ...
                if (req.anyRequestFailed ==1) {
                    ...
                    // Aurora xuyong 2015-08-05 deleted for bug #15425 start
                    ...
                    // Aurora xuyong 2015-08-05 deleted for bug #15425 end
                }
            }
        }
    }
...
//                    mToastHandler.sendEmptyMessage(TOAST_SWITCH_DATA_BACK_TIMEOUT);
  ...
  //Gionee guoyx 20130221 add for CR00773050 end
    ...
    private synchronized int startUsingNetworkFeature(ConnectivityManager connMgr) {
        ...
        // Aurora xuyong 2015-07-22 modified for bug #14413 start
        mPreMobileDateStatusQueue.add(mMobleStatusIsEnable);
        // Aurora xuyong 2015-07-22 modified for bug #14413 end
    ...
    protected void endMmsConnectivity() {
        try {
            ...
            if (mConnMgr != null) {
	             // Aurora xuyong 2014-04-19 modified for bug #3367 start
                // Aurora xuyong 2015-07-22 modified for bug #14413 start
            	if (!mPreMobileDateStatusQueue.peek()) {
            		mPreMobileDateStatusQueue.clear();
            ...
            switch (msg.what) {
                ...
                case EVENT_CONTINUE_MMS_CONNECTIVITY:
                    ...
                    try {
                        ...
                        int result = 0;
                        if (MmsApp.mGnMultiSimMessage) {//guoyx 20130226
                            // Aurora xuyong 2015-06-29 modified for android 5.1+ start
                        	SIMInfo si = null;
                        	if (!Utils.hasLollipop()) {
                                si = SIMInfo.getSIMInfoBySlot(getApplicationContext(), msg.arg2);
                        	} else {
                        		si = SIMInfo.getSIMInfoById(getApplicationContext(), msg.arg2);
                        	}
                            // Aurora xuyong 2015-06-29 modified for android 5.1+ end
                            if (null == si) {
                            ...
                            if (!Utils.hasLollipop()) {
                            	...
                            	result = beginMmsConnectivityGemini(msg.arg2);
                            }
                            // Aurora xuyong 2015-06-29 modified for android 5.1+ end
                        } else {
...
//                                      int subId = msg.arg2 - 1;
        ...
        private boolean processTransaction(Transaction transaction) throws IOException {
            ...
            // Aurora xuyong 2015-08-19 deleted for bug #15408 start
            //int requestResult = GnPhone.APN_REQUEST_FAILED;
            // Aurora xuyong 2015-08-19 deleted for bug #15408 end
    ...
    // add for gemini
    // Aurora xuyong 2015-07-22 added for bug #14413 start
    private Queue<Boolean> mPreMobileDateStatusQueue = new ArrayBlockingQueue<Boolean>(100);
    // Aurora xuyong 2015-07-22 added for bug #14413 end
    protected int beginMmsConnectivityGemini(int simId) throws IOException {
        ...
        // Aurora xuyong 2015-008-11 added for bug #15408 start
        boolean dataEnabeld = getMobileDataEnabled(simId);
        // Aurora xuyong 2015-008-11 added for bug #15408 end
        // Aurora xuyong 2015-07-22 added for bug #14413 start
        mPreMobileDateStatusQueue.add(dataEnabeld);
        // Aurora xuyong 2015-07-22 added for bug #14413 end
        while (!dataEnabeld) {
            // Aurora xuyong 2015-06-29 modified for android 5.1+ start
            setMobileDataEnabled(true, simId);
            // Aurora xuyong 2015-06-29 modified for android 5.1+ end
            // Aurora xuyong 2015-008-11 added for bug #15408 start
            dataEnabeld = getMobileDataEnabled(simId);
            // Aurora xuyong 2015-008-11 added for bug #15408 end
        }
        sleep(5000);
        ...
        try {
            ...
            if (mConnMgr != null) {
                ...
                if ("NBL8910A".equals(SystemProperties.get("ro.gn.gnprojectid"))) {
                    ...
                    // Aurora xuyong 2015-07-22 modified for bug #14413 start
                    // Aurora xuyong 2015-08-19 modified for bug #15408 start
                	if (mPreMobileDateStatusQueue.size() > 0 && !mPreMobileDateStatusQueue.peek()) {
                    // Aurora xuyong 2015-08-19 modified for bug #15408 end
                		mPreMobileDateStatusQueue.clear();
	                    // Aurora xuyong 2015-06-29 modified for android 5.1+ start
	                    setMobileDataEnabled(false, simId);
	                    // Aurora xuyong 2015-06-29 modified for android 5.1+ end
    ...
    */
    ...
    private void setMobileDataEnabled(boolean enable) {
        android.telephony.TelephonyManager.getDefault().setDataEnabled(enable);
    }
    
    private void setMobileDataEnabled(boolean enable, int sub) {
        android.telephony.TelephonyManager.getDefault().setDataEnabled(sub, enable);
    }
    // Aurora xuyong 2015-08-11 added for bug #15408 start
    private boolean getMobileDataEnabled(int sub) {
    	return  android.telephony.TelephonyManager.getDefault().getDataEnabled(sub);
    }
    // Aurora xuyong 2015-08-11 added for bug #15408 end
    // Aurora xuyong 2015-06-29 modified for android 5.1+ end
}
