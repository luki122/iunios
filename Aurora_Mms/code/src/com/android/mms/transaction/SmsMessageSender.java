...
public class SmsMessageSender implements MessageSender {
    ...
    private void updateMsgNotificationIndex(Context context, Uri uri) {
        ...
        Cursor result = null;
        try {
            result = context.getContentResolver().query(uri, projection, null, null, null);
            if (result != null && result.moveToFirst()) {
                String address = result.getString(0);
                String body = result.getString(1);
                ContentValues values = new ContentValues();
                values.put("notification_index", Utils.isNotificationMsg(context, result.getString(ADDRESS_INDEX), result.getString(BODY_INDEX)) ? 1 : 0);
        ...
        for (int i = 0; i < mNumberOfDests; i++) {
            try {
                ...
                // Aurora xuyong 2014-07-25 modified for multiSim start
                    ...
                    uri = gionee.provider.GnTelephony.Sms.addMessageToUri(mContext.getContentResolver(),
...
//                    Log.d("ra_cma", mContext.getContentResolver().toString()+Uri.parse("content://sms/queued")+"  "+mDests[i]+"  "+mMessageText+"  "+mTimestamp+"  "+requestDeliveryReport+"  "+mThreadId+"  "+mSimId);
                    ...
                    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature start
                    updateMsgNotificationIndex(mContext, uri);
                    // Aurora xuyong 2015-12-04 added for aurora2.0 new feature end
                } else {
                    ...
                    // Aurora xuyong 2014-11-25 added for bug #10052 end
                    // Aurora xuyong 2015-12-04 modified for aurora2.0 new feature start
                    uri = Sms.addMessageToUri(mContext.getContentResolver(),
                    // Aurora xuyong 2015-12-04 modified for aurora2.0 new feature end
                            // Aurora xuyong 2014-11-25 modified for bug #10052 start
                            uri, mDests[i],
                            // Aurora xuyong 2014-11-25 modified for bug #10052 end
