...
public abstract class Transaction extends Observable {
    ...
    protected boolean mAvoidReject = false;
    // Aurora xuyong 2014-11-08 added for reject new feature end
    // Aurora xuyong 2015-08-19 added for bug #15408 start
    // A cache of MmsNetworkManager for SIMs
    private final SparseArray<AuroraMmsNetworkManager> mNetworkManagerCache = new SparseArray<>();
    // Aurora xuyong 2015-08-19 added for bug #15408 end
    /**
...
*/
    ...
    public boolean isEquivalent(Transaction transaction) {
        Log.d(MmsApp.TXN_TAG, "Transaction: isEquivalent mId=" + mId 
        ...
        Log.d(MmsApp.TXN_TAG, "Transaction: isEquivalent Id1=" + Id1 + "\t Id2=" + Id2);
    ...
    // Aurora xuyong 2014-11-08 added for reject new feature start
    public void setAvoidReject(boolean need) {
        mAvoidReject = need;
    }
    // Aurora xuyong 2014-11-08 added for reject new feature end
    // Aurora xuyong 2015-08-19 added for bug #15408 start
    public AuroraMmsNetworkManager getNetworkManager(Context context, int subId) {
        synchronized (mNetworkManagerCache) {
        	AuroraMmsNetworkManager manager = mNetworkManagerCache.get(subId);
            if (manager == null) {
                manager = new AuroraMmsNetworkManager(context, subId);
