...
import gionee.net.http.GnAndroidHttpClient;
...
public class HttpUtils {
    ...
    protected static byte[] httpConnection(Context context, long token,
        ...
        Log.d(MmsApp.TXN_TAG, "httpConnection: params list");
        Log.d(MmsApp.TXN_TAG, "\ttoken\t\t= " + token);
        Log.d(MmsApp.TXN_TAG, "\turl\t\t= " + url);
        Log.d(MmsApp.TXN_TAG, "\tmethod\t\t= "
                    + ((method == HTTP_POST_METHOD) ? "POST"
                            : ((method == HTTP_GET_METHOD) ? "GET" : "UNKNOWN")));
        Log.d(MmsApp.TXN_TAG, "\tisProxySet\t= " + isProxySet);
        Log.d(MmsApp.TXN_TAG, "\tproxyHost\t= " + proxyHost);
        Log.d(MmsApp.TXN_TAG, "\tproxyPort\t= " + proxyPort);
        ...
        try {
            ...
            try {
                Log.d(MmsApp.TXN_TAG, "===before execute ");
                response = client.execute(target, req);
                Log.d(MmsApp.TXN_TAG, "===after execute, response is null = " 
                        + ((null == response) ? true : false));
            } catch (IOException e){
                e.printStackTrace();
                Log.e(MmsApp.TXN_TAG, "AndroidHttpClient.execute exception: " + e.getMessage());
            }

            //gionee gaoj 2012 -6-15 added for CR00625006 start
            if (MmsApp.mGnMessageSupport) {
                if (response == null) {
                    return null;
                }
            }
            //gionee gaoj 2012 -6-15 added for CR00625006 end
            //HttpResponse response = client.execute(target, req);
            StatusLine status = response.getStatusLine();
            Log.d(MmsApp.TXN_TAG, "httpConnection: execute status="+status);
    ...
    private static void handleHttpConnectionException(Exception exception, String url)
        ...
        Log.e(MmsApp.TXN_TAG, "handle Http Connection Exception, Url: " + url + "\n" + exception.getMessage());
    ...
    private static AndroidHttpClient createHttpClient(Context context) {
        ...
        //Gionee guoyangxu 20120517 modified for mms send too long time CR00568184 begin.
        //HttpConnectionParams.setSoTimeout(params, soTimeout);
        //Add-Begin-MTK 
        //Default connection and socket timeout of 60 seconds & 180 seconds. 
        //The socket buffer size to 2K 
        ...
        //Add-End 
        //Gionee guoyangxu 20150517 modified for mms send too long time CR00568184 end.
        
        //Enable HTTP Retry 
        GnAndroidHttpClient.setHttpRequestRetryHandler(client);
