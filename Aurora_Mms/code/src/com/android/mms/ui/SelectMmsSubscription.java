...
public class SelectMmsSubscription extends Service {
    static private final String TAG = "Mms/SelectMmsSubscription";

    private Context mContext;
    private Intent startUpIntent;
    private int requestedSub =0;
    private int ABORT_SEND = 2;
        ...
        @Override
            protected void onPostExecute(Integer result) {
                ...
                if (result == -1) {
                    ...
                    tempBundle.putInt(Mms.SIM_ID, -1);
                ...
                if (result == -1 || result == 1) {
                    ...
                    if(result == -1) {
                        ...
                        //Gionee guoyx 20130226 remove for CR00773050 begin
//                        showNotificationAbortAndSwitchBack();
                        //Gionee guoyx 20130226 remove for CR00773050 end
                    }
                }
            }
        ...
        public void removeAbortNotification() {
            Log.d(TAG, "removeAbortNotification");
            String ns = Context.NOTIFICATION_SERVICE;
            NotificationManager mNotificationManager = (NotificationManager)
                    mContext.getSystemService(ns);
            //Gionee guoyx 20130226 remove for CR00773050 begin
//            mNotificationManager.cancel("ABORT", ABORT_SEND); //ID=2, abort notification
            //Gionee guoyx 20130226 remove for CR00773050 end
        ...
        private void showNotificationAbortAndSwitchBack() {
            ...
            src.putExtra(Mms.SIM_ID, getOtherSub(requestedSub));

            Intent notificationIntent = new Intent(mContext, SelectMmsSubscription.class);
            ...
            mNotificationManager.notify("ABORT", ABORT_SEND, notification); //ID=2 for the abort.
        ...
        private int switchSubscriptionTo(int sub) {
            ...
            if (MmsApp.mQcMultiSimEnabled) {
                ...
                int result = (GnTelephonyManagerEx.getDefault().setPreferredDataSubscription(sub))? 1:0;
                if (result == 1) { //Success.
                    Log.d(TAG, "Subscription switch done.");
                    sleep(1000);

                    while(GnTelephonyManager.getSimStateGemini(sub) != TelephonyManager.SIM_STATE_UNKNOWN
                            && !isNetworkAvailable()) {
    ...
    private void abortTransactionService() {
        Log.d(TAG, "abortTransactionService");
        Intent svc = new Intent(mContext, com.android.mms.transaction.TransactionService.class);
        svc.putExtra("ABORT_TRANSACTION", true);
...
//Gionee guoyx 20130328 removed for begin           
...
//Gionee guoyx 20130328 removed for end
    ...
    private void removeStatusBarNotification() {
        ...
        //Gionee guoyx 20130226 remove for CR00773050 begin
//        mNotificationManager.cancel(ABORT_SEND);
        //Gionee guoyx 20130226 remove for CR00773050 end
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        requestedSub = startUpIntent.getIntExtra(Mms.SIM_ID, 0);
    ...
    private int getCurrentSubcription() {
        if (MmsApp.mQcMultiSimEnabled) {
            return GnTelephonyManagerEx.getDefault().getPreferredDataSubscription();
