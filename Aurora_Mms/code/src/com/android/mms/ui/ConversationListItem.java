...
public class ConversationListItem extends RelativeLayout implements Contact.UpdateListener, IXYConversationListItemHolder {
...
//    private QuickContactBadge mAvatarView;
    ...
    protected void onFinishInflate() {
        ...
        // Aurora xuyong 2016-02-26 added for aurora 2.0 new feature start
        mPurposeView = (TextView) findViewById(R.id.aurora_purpose);
        // Aurora xuyong 2016-02-26 added for aurora 2.0 new feature end
...
//        updateAvatarView();
    ...
    //gionee gaoj 2012-3-22 added for CR00555790 start
    // Aurora xuyong 2016-03-10 added for bug #18928 start
    private int mTabIndex = -1;
    public final void bind(Context context, final ConversationListItemData ch, final Conversation conversation,
                           GnSelectionManager<Long> manager, int position, int tabIndex) {
        mTabIndex = tabIndex;
        this.bind(context, ch, conversation, manager, position);
    }
    // Aurora xuyong 2016-03-10 added for bug #18928 end
    public final void bind(Context context, final ConversationListItemData ch, final Conversation conversation,
...
//    }
    ...
    //Gionee <gaoj> <2013-05-13> added for CR00811367 begin
     // Aurora liugj 2013-11-18 modified for aurora's new feature start
    // Aurora xuyong 2016-03-10 added for bug #18928 start
    public final void bind(Context context, final Conversation conv, int position, boolean ischecked, int tabIndex) {
        mTabIndex = tabIndex;
        this.bind(context, conv, position, ischecked);
    }
    // Aurora xuyong 2016-03-10 added for bug #18928 end
...
//                mUnreadView.setVisibility(View.INVISIBLE);
    ...
    public CharSequence formatMessage(Conversation conv) {
          // Aurora liugj 2014-01-10 modified for listItem optimize start
...
//        }
    ...
    // Aurora liugj 2013-11-18 modified for aurora's new feature start
    // Aurora liugj 2013-12-20 modified for list scroll optimize start
    private CharSequence rebuildDraftConvSnippet(String snipped) {
    ...
    private CharSequence rebuildErrorConvSnippet(String snipped) {
    ...
    private void addContactDrawable(Context context, Contact contact) {
        ...
        // Aurora xuyong 2016-01-30 modified for xy-smartsms start
        //mAvatarView.addContactDrawable(initContactInfo(contact), contact.getAvatar(context, null));
        // Aurora xuyong 2016-03-10 modified for bug #18928 start
        if (MmsApp.sHasXySmartSmsFeature && mTabIndex == ConvFragment.NOTIFICATION_TAB) {
        // Aurora xuyong 2016-03-10 modified for bug #18928 end
            bindTextImageView(contact);
        } else {
            mAvatarView.addContactDrawable(initContactInfo(contact), contact.getAvatar(context, null));
        }
        // Aurora xuyong 2016-01-30 modified for xy-smartsms end
        // Aurora xuyong 2016-01-08 modified for bug #18250 end
    }

    // Aurora xuyong 2016-01-30 modified for aurora 2.0 new feature start
    private void bindAvatarView(final Context context, final Conversation conv, boolean mIsCheckBoxVisibility) {
    // Aurora xuyong 2016-01-30 modified for aurora 2.0 new feature end
        ...
        mAvatarView.setmHasClicked(mIsCheckBoxVisibility);
        // Aurora xuyong 2016-01-30 added for aurora 2.0 new feature end 
        for (final Contact contact : contactList) {
            // Aurora xuyong 2015-01-08 deleted for bug #18237 start
            //Drawable avatarDrawable = contact.getAvatar(context, null);
            // Aurora xuyong 2015-01-08 deleted for bug #18237 end
            ...
            // Aurora xuyong 2015-01-08 deleted for bug #18237 end
               // Aurora xuyong 2016-03-29 deleted for bug #18928 start
               // mHandler.postDelayed(new Runnable() {
               //     public void run() {
               // Aurora xuyong 2016-03-29 deleted for bug #18928 end
    ...
    private void updateUi(Context context, Conversation conv, boolean isChecked) {
        // Aurora xuyong 2016-01-30 modified for aurora 2.0 new feature start
        bindAvatarView(context, conv, isChecked);
        // Aurora xuyong 2016-01-30 modified for aurora 2.0 new feature end
        mFromView.setVisibility(VISIBLE);
        //mFromView.setText(formatMessage(conv));
        ...
        if (conv.hasUnreadMessages() && count > 0) {
            ...
            if (count > 0) {
                ...
                // start
                mDraftView.setTextColor(mContext.getResources().getColor(R.color.aurora_conv_unread_color));
                mFromView.setTextColor(mContext.getResources().getColor(R.color.aurora_conv_unread_color));
                // Aurora xuyong 2016-02-26 added for aurora 2.0 new feature start
                mPurposeView.setTextColor(mContext.getResources().getColor(R.color.aurora_conv_unread_color));
                // Aurora xuyong 2016-02-26 added for aurora 2.0 new feature end
                // Aurora liugj 2013-10-10 modified for aurora's new feature end
                mDraftView.setVisibility(View.VISIBLE);
            } else {
                mDraftView.setVisibility(View.GONE);
                mFromView.setTextColor(mContext.getResources().getColor(R.color.aurora_conv_list_title_text_color));
                // Aurora xuyong 2016-02-26 added for aurora 2.0 new feature start
                mPurposeView.setTextColor(mContext.getResources().getColor(R.color.aurora_conv_list_snippet_text_color));
                // Aurora xuyong 2016-02-26 added for aurora 2.0 new feature end
            }
            //mErrorIndicator.setVisibility(GONE);
        } else {
      ...
      // Aurora xuyong 2014-10-23 added for privacy feature start
        ...
        if (!conv.hasDraft()) {
            if (!conv.hasError()) {
                ...
                mSubjectView.setText(rebuildErrorConvSnippet(conv.getSnippet()));
            }
        } else {
            mSubjectView.setText(rebuildDraftConvSnippet(conv.getSnippet()));
        }
    ...
    private void bindTextImageView(Contact contact){
        ...
        xyPublicInfoItem.bindTextImageView((IXYConversationListItemHolder)this, mFromView, mPurposeView, mAvatarView, contact, mConversation);
