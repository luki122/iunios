...
public class MessageUtils {
    ...
    // Aurora xuyong 2016-01-06 added for bug #18278 start
    private static String getTime(Context context, long when) {
        int format_flags = DateUtils.FORMAT_NO_NOON_MIDNIGHT |
                DateUtils.FORMAT_ABBREV_ALL |
                DateUtils.FORMAT_CAP_AMPM;
        format_flags |= DateUtils.FORMAT_SHOW_TIME;
    ...
    // Aurora xuyong 2016-01-06 added for bug #18278 end
    ...
    public static String formatAuroraTimeStampString(Context context, long when, boolean isConv) {
        ...
        if (then.year != now.year) {
            if (isConv) {
                ...
                // Aurora xuyong 2016-01-05 modified for bug #18230 start
                // Aurora xuyong 2016-01-06 modified for bug #18278 start
                sRet = DateFormat.format("yyyy-MM-dd", when).toString();
                sRet += " " + getTime(context, when);
                // Aurora xuyong 2016-01-06 modified for bug #18278 end
                // Aurora xuyong 2016-01-05 modified for bug #18230 end
            }
        } else {            
            if (then.yearDay == now.yearDay) {
                // Aurora xuyong 2016-01-06 modified for bug #18278 start
                return getTime(context, when);
                // Aurora xuyong 2016-01-06 modified for bug #18278 end
            } else {
                if (isConv) {
                    // Aurora xuyong 2016-01-05 modified for bug #18230 start
                    if (now.yearDay - then.yearDay == 1) {
                        sRet = context.getString(R.string.str_ipmsg_yesterday);
                    } else {
    ...
    private static void selectMediaByType(
         ...
         if (context instanceof AuroraActivity) {
            ...
            // Aurora xuyong 2016-01-07 deleted for bug #18259 start
            //Intent wrapperIntent = Intent.createChooser(innerIntent, null);
            // Aurora xuyong 2016-01-07 deleted for bug #18259 end
            // Aurora xuyong 2016-01-07 modified for bug #18259 start
            ((AuroraActivity) context).startActivityForResult(innerIntent, requestCode);
            // Aurora xuyong 2016-01-07 modified for bug #18259 end
        }
    }
...
//                .setIconAttribute(android.R.attr.alertDialogIcon)
                ...
                .setTitle(R.string.discard_message_reason)
                .setPositiveButton(R.string.discard_message, listener)
...
//                textView.setTextColor(Color.GRAY);
    ...
    // Aurora xuyong 2016-01-29 added for xy-smartsms start
    public static int getSlotById(Context context, int simId) {
        int result = -1;
        int slot = -1;
        if(simId > 0) {
            SIMInfo simInfo = getSIMInfo(context, simId);
            if (simInfo != null) {
                slot = simInfo.mSlot;
            }
        }
    ...
    // Aurora xuyong 2016-01-29 added for xy-smartsms end
