...
//gionee zhouyj 2012-08-07 add for CR00664063 start 
import java.io.IOException;
//gionee zhouyj 2012-08-07 add for CR00664063 end 
...
public class TempFileProvider extends ContentProvider {
    ...
    public static String getScrapPath(Context context, String fileName) {
        ...
        if (mStorageManager != null) {
            ...
            if(MmsApp.mGnMessageSupport) {
                String sdcardPath = getAvailableSdcard(context, MmsConfig.getMaxMessageSize());
                if(null != sdcardPath) {
                    File file = new File(new File(new File (new File(new File(sdcardPath),
                        "Android"), "data"), context.getPackageName()), "cache");
                    if(!file.exists()) {
                        try {
                            (new File(new File (new File(new File(sdcardPath),
            ...
            File mFile = GnStorageManager.getInstance(context).getMTKExternalCacheDir(context.getPackageName());
    ...
    public static String getAvailableSdcard(Context context, long request) {
        ...
        //return storageManager.getGnAvailableExternalStoragePath(request);
        // Aurora xuyong 2013-11-14 modified for S4 adapt start
        return GnStorageManager.getInstance(context).getGnAvailableExternalStoragePath_ex(request);
