...
public class ContactsSectionIndexer implements SectionIndexer {
     ...
     */
    public ContactsSectionIndexer(Object[] sections, Object[] counts) {
        ...
        for (int i = 0; i < counts.length; i++) {
            if (TextUtils.isEmpty((String)mSections[i])) {
                mSections[i] = BLANK_HEADER_STRING;
            } else if (!mSections[i].equals(BLANK_HEADER_STRING)) {
                mSections[i] = mSections[i].toString().trim();
            }

            mPositions[i] = position;
            position += Integer.valueOf(counts[i].toString());
        }
        mCount = position;
    }
    ...
    public void setProfileHeader(String header) {
        if (mSections != null) {
            ...
            for (int i = 1; i <= mPositions.length; i++) {
                tempSections[i] = (String) mSections[i - 1];
