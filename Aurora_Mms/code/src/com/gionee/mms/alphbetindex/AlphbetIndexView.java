...
public final class AlphbetIndexView extends AbsListIndexer {
    ...
    private int MAX_TOUCHBLE_WIDTH = 50;
    ...
    public AlphbetIndexView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.AlphbetIndexView);
        Resources res = getResources();

        MAX_WAVE_Y_RANGE = a.getDimensionPixelSize(
                R.styleable.AlphbetIndexView_waveSideRange, 90);
        MAX_REDRAW_Y_RANGE = MAX_WAVE_Y_RANGE + 20;

        mLetterTextSizeOri = a
                .getDimensionPixelSize(
                        R.styleable.AlphbetIndexView_sectionFontSize,
                        toRawTextSize(13));
        mLetterTextSize = mLetterTextSizeOri;

        //Gionee <zhouyj> <2013-06-08> modify for begin for super theme
        /*mEnableLetterColor = a.getColor(
                R.styleable.AlphbetIndexView_enableSectionColor, Color.LTGRAY);
        mDisableLetterColor = a.getColor(
                R.styleable.AlphbetIndexView_disableSectionColor, Color.DKGRAY);
        // gionee 20120705 xuhz modify for CR00637559 start
        // old: mShowingLetterColor =
        // a.getColor(R.styleable.AlphbetIndexView_promptSectionColor,
        // Color.GREEN);
        mShowingLetterColor = a.getColor(
                R.styleable.AlphbetIndexView_showingLetterColor, Color.GREEN);
        // gionee 20120705 xuhz modify for CR00637559 end
        // gionee 20120705 xuhz modify for CR00637559 start
        // old: mPromptTextColor = mShowingLetterColor;
        mPromptTextColor = a.getColor(
                R.styleable.AlphbetIndexView_promptSectionColor, Color.GREEN);
        // gionee 20120705 xuhz modify for CR00637559 end*/
        
        if (MmsApp.mDarkStyle || MmsApp.mDarkTheme || MmsApp.mTransparent) {
            ...
            mEnableLetterColor = res.getColor(R.color.alphbetIndexView_enable_section_light);
        ...
        mAnimationDuration = a.getInt(
                R.styleable.AlphbetIndexView_promptAnimationDuration,
                PROMPT_ANIM_DURATION);

        mPromptTextSize = a.getDimensionPixelSize(
                R.styleable.AlphbetIndexView_promptSectionFontSize, 40);

        mPromptLeftOffset = a.getDimensionPixelOffset(
                R.styleable.AlphbetIndexView_promptLeftOffset, 0);

        Log.i("James", "mLetterTextSizeOri  :" + mLetterTextSizeOri
                + "\nmPromptTextSize     :" + mPromptTextSize);

        int promptViewBgRes = a.getResourceId(
                R.styleable.AlphbetIndexView_promptBgRes, 0);
        if (0 != promptViewBgRes) {
            setPromptViewBg(res.getDrawable(promptViewBgRes));
        }

        a.recycle();
        mAlphbet = context.getResources().getStringArray(
                R.array.alphbet_index);
    ...
    protected void init(Context context) {
        ...
        mEnterAnim = new PromptRotateAnimation(-PROMPT_HIDE_ANGLE,
                PROMPT_SHOW_ANGLE, mPromptBgWidth >> 1);
        mEnterAnim.setDuration(mAnimationDuration);
        mEnterAnim.setFillAfter(true);
        mEnterAnim.setInterpolator(new LinearInterpolator());

        mExitAnim = new PromptRotateAnimation(PROMPT_SHOW_ANGLE,
                PROMPT_HIDE_ANGLE, mPromptBgWidth >> 1);
    ...
    public void layout(int l, int t, int r, int b) {
        ...
        if (getHeight() > 0) {
            ...
            WAVE_FACTOR = -(float) MAX_WAVE_Y_HEIGHT
                    / (MAX_WAVE_Y_RANGE * MAX_WAVE_Y_RANGE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mLetterHolders.length; ++i) {
            ...
            } /* else */
            {
                ...
                if (offset > 0) {
                    mPaint.setTextSize(getTextSizeOffset(offset)
                            + mLetterTextSize);
                }
    ...
    public float getTextSizeOffset(int offsetLeft) {
        float size = (TEXT_SIZE_FACTOR_A * offsetLeft * offsetLeft + TEXT_SIZE_FACTOR_B
                * offsetLeft)
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (!mIsTouching && getWidth() - x > MAX_TOUCHBLE_WIDTH) {
            ...
            case MotionEvent.ACTION_DOWN:
                cancelFling();
                mPromptView.setAnimation(mEnterAnim);
    ...
    protected Rect getRedrawRect(int touchingY) {
        mRectRedraw.set(0, touchingY - MAX_REDRAW_Y_RANGE, getWidth(),
                touchingY + MAX_REDRAW_Y_RANGE);
    ...
    protected int getTouchingIndex(int touchingY) {
        /*
         * for (int i = 0; i < mLetterHolders.length; i++) { if (touchingY <=
         * mLetterHolders[i].mOrigRect.bottom) return i; }
         * 
    ...
    public void setList(AuroraListView listView,
    ...
    private void moveListToSection(int sectionIndex) {
        if (null == mList || null == mSectionIndexer
                || INVALID_INDEX == sectionIndex) {
            return;
        }
    ...
    class LetterHolder {
        ...
        public LetterHolder(int left, int top, int right, int bottom,
                String letter) {
    ...
    private void initLetterHolders(int viewHeight) {
        ...
        for (int i = 0; i < alphbetLen; ++i) {
            mLetterHolders[i] = new LetterHolder(ORIG_LEFT, top, right, bottom,
                    mAlphbet[i]);
    ...
    private void adjustLetterTextSize(int holderHeight) {
        ...
        while (letterMaxHeight <= mLetterHeight
                && mLetterHeight > MIN_SUGGETS_LETTER_HEIGHT) {
            --mLetterTextSize;
            mPaint.setTextSize(mLetterTextSize);
            mPaint.getTextBounds(LETTER_DISPLAY_AREA_REPRESENTER, 0, 1, bounds);
    ...
    private boolean pickDisableSection() {
        ...
        /*
         * if (null != mLetterHolders && null != mSectionStrings &&
         * mSectionStrings.length > 0) { for (int i = 0; i <
         * mLetterHolders.length; ++i) { mLetterHolders[i].setEnable(false); for
         * (int j = 0; j < mSectionStrings.length; j++) { if
         * (mLetterHolders[i].mLetter.equalsIgnoreCase(mSectionStrings[j])) {
         * mLetterHolders[i].setEnable(true); break; } } } return true; }
         */
        if (null != mLetterHolders) {
            for (int i = 0; i < mLetterHolders.length; ++i) {
                ...
                if (null != mSectionStrings && mSectionStrings.length > 0) {
                    for (int j = 0; j < mSectionStrings.length; j++) {
                        if (mLetterHolders[i].mLetter
                                .equalsIgnoreCase(mSectionStrings[j])) {
    ...
    private TextView createPromptView() {
        ...
        Rect bounds = new Rect();
        tv.getPaint().getTextBounds(LETTER_DISPLAY_AREA_REPRESENTER, 0, 1,
                bounds);
        tv.setFocusable(false);
        tv.setMinWidth(bounds.width() + 4);
        tv.setGravity(Gravity.CENTER);
        tv.setTextColor(mPromptTextColor);
        tv.setBackgroundDrawable(mPromptViewBg);
        return tv;
    }
    ...
    private void dismissPromptWindow() {
        if (null != mPromptWindow) {
            mPromptView.startAnimation(mExitAnim);
    ...
    private void positionPromptWindow() {
        ...
        int nActualY = mLetterHolders[mTouchingAlphbetIndex].mDrawRect
                .centerY() + visibleRect.top - mPromptView.getHeight() / 2;
        ...
        if (!mPromptWindow.isShowing()) {
            mPromptWindow.showAtLocation(this, Gravity.NO_GRAVITY, x, nActualY);
        } else {
    ...
    public class PromptRotateAnimation extends Animation {
         ...
         * 
         * @param fromDegree
         *            The rotate start degree.
         * @param toDegree
         *            The rotate end degree.
         * @param pivotX
         *            The X axis pivot of the rotate.
         */
        public PromptRotateAnimation(float fromDegree, float toDegree,
                float pivotX) {
         ...
         *            The rotate end degree.
         */
        public void setDegreeInterval(final float fromDegree,
                final float toDegree) {
            mFromDegree = fromDegree;
            mToDegree = toDegree;
        }
        ...
        protected void applyTransformation(final float interpolatedTime,
            ...
            float degree = fromDegree
                    + ((mToDegree - fromDegree) * interpolatedTime);
