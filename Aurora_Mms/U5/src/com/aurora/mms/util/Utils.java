...
// Aurora xuyong 2016-01-23 added for bug #19532 start
import android.view.animation.Animation;
import android.view.animation.AnimationUtils;
// Aurora xuyong 2016-01-23 added for bug #19532 end
...
// Aurora xuyong 2016-01-23 added for bug #19532 end
// Aurora xuyong 2015-03-30 added for yulore feature end
// Aurora xuyong 2016-02-24 added for bug #19668 start
...
// Aurora xuyong 2016-02-24 added for bug #19668 end
...
public class Utils {
  ...
  // Aurora xuyong 2014-07-02 added for reject feature end
   ...
   // Aurora xuyong 2015-09-08 added for bug #15971 end
   // Aurora xuyong 2016-01-19 added for bug #18253 start
   public static void updateWidget(Context context) {
       Intent intent = new Intent("com.android.mms.intent.action.ACTION_NOTIFY_DATASET_CHANGED");
       context.sendBroadcast(intent);
   }
    // Aurora xuyong 2016-01-19 added for bug #18253 end
    // Aurora xuyong 2016-01-23 added for bug #19532 start
    public static void setOperator(Context context, TextView view, int slot) {
        // Aurora xuyong 2016-02-24 modified for bug #19668 start
        ...
        if (subInfoList != null) {
            ...
            if (subInfoLength == 2) {
                final SubscriptionInfo sir = subInfoList.get(slot);
                operatorName = sir.getDisplayName().toString();
                if(TextUtils.isEmpty(operatorName)) {
                    operatorName = GnTelephonyManager.getSimOperatorGemini(slot);
                    if (operatorName.endsWith("00") || operatorName.endsWith("02") || operatorName.endsWith("07")) {
                        operatorName = context.getResources().getString(R.string.aurora_operator_cm);
                    } else if (operatorName.endsWith("01")) {
                        operatorName = context.getResources().getString(R.string.aurora_operator_cu);
                    } else if (operatorName.endsWith("03")) {
                        operatorName = context.getResources().getString(R.string.aurora_operator_ct);
                    }
                }
            }
        // Aurora xuyong 2016-02-24 modified for bug #19668 end
        }
        //GnTelephonyManager.getLine1NumberGemini(slot)
        view.setText(context.getResources().getString(R.string.aurora_slot_indi_str, slot + 1, operatorName));
        Animation anim = AnimationUtils.loadAnimation(context, R.anim.aurora_slot_indi_anim);
    ...
    public static boolean isSuperMode(Context context) {
        return isSuperMode(context, true);
    }

    public static boolean isSuperMode(Context context, boolean needShowToast) {
        String mode = Settings.System.getString(context.getContentResolver(), "aurora_power_mode");
        if (mode != null && mode.equals("super")) {
            if (needShowToast) {
                Toast.makeText(context, R.string.aurora_supermode_tip, Toast.LENGTH_SHORT).show();
            }
            return true;
        }
        return false;
