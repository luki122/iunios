...
public class BasePopupView extends RelativeLayout {
	...
	public void initData(BusinessSmsMessage message) {
		this.mBusinessSmsMessage = message;
		// setBackground(mBusinessSmsMessage);
		initUIAfter();
		setBackgroundResource(R.drawable.duoqu_pop_bg_gray);
	}
	...
	public void initUIPartBefore(Activity mContext,
		...
		// int bgType = SysParamEntityManager.getIntParam(mContext,
		// Constant.POPUP_BG_TYPE);

		mView = this;
		// LogManager.i("TopBodySplit", "initUIPartBefore bgType: " + mView);
		// if (bgType == 0) {
		...
		// businessSmsMessage.getImgNameByKey("popubgDrawableName");
		// if (!StringUtils.isNull(popubgDrawableName)) {
		// mView = (ViewGroup) ViewManger.createContextByLayoutId(mContext,
		// R.layout.duoqu_popup_oneside, null);
		// LayoutParams params = new LayoutParams(LayoutParams.FILL_PARENT,
		// LayoutParams.FILL_PARENT);
		// mView.setLayoutParams(params);
		// this.addView(mView, params);
		// }
		// ViewManger.setViewBg(mContext, mView, popubgDrawableName,
		// R.color.duoqu_dark_transparent, -1);
		// } catch (Exception e) {
		// e.printStackTrace();
		// }
		//
		// } else {
		// this.setBackgroundResource(R.color.duoqu_dark_transparent);
		// }

		mView.setPadding(mView.getPaddingLeft(),
				ViewManger.getIntDimen(mContext, R.dimen.base_margin_top),
