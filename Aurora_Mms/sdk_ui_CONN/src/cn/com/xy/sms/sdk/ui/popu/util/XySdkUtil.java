...
public class XySdkUtil {
     ...
     */
    private static final LruCache<Long,JSONObject> mBubbleDataCache = new LruCache<Long,JSONObject>(DUOQU_BUBBLE_DATA_CACHE_SIZE);
//    private static final  HashMap<Integer, Integer> mWidthMap= new HashMap<Integer, Integer>();
    /**
     * init sdk custom
     * @param context
     * @param channel
...
//            String iccid =getICCID(context);
//
//            ParseManager.initSdk(context, DUOQU_SDK_CHANNEL,iccid , true, true, extend);
//
...
//    }
    ...
    public static void putBubbleDataToCache(Long key,JSONObject value){
        if(key == null || value == null){
            LogManager.w(TAG, "cn.com.xy.sms.sdk.SmartSmsSdkUtil putBubbleDataToCache key is null or value is null.");
            return;
        }
    ...
    public static JSONObject getBubbleDataFromCache(Long key){
        if(key == null){
            LogManager.w(TAG, "cn.com.xy.sms.sdk.SmartSmsSdkUtil getBubbleDataFromCache key is null");
            return null;
        }
   ...
   public static final int SMARTSMS_PARSE_TYPE_POUPP =2;
     ...
     * @param msgId  消息ID
     * @param msgs   短信BODY
     * @param type   type 0:只需要丰富汽泡，1：通知栏浮窗，2：弹窗
     * @param simIndex 卡位
     */
...
//                        if (sms.mWrappedSmsMessage != null) {
                ...
                if(parseType ==0){
                    ...
                    //通知栏，浮窗结果，内含丰富汽泡流程。同步方法调用。后续通知栏依据消息ID，调用 getNotifyDataCacheByMsgId  是否能获取到数据，依据有无数据，是否显示智能短信通知栏。
//                    Map<String,Object> map = ParseSmsToBubbleUtil.parseSmsToBubbleResult(msgId, phoneNum, bodyText, centerNum,  msgs[0].getTimestampMillis(), 4,
//                            false, true, null);
                	HashMap<String,String> extendMap = getExtendMap(simIndex,msgId,msgs[0].getTimestampMillis());
                    
                	Map<String,Object> map =  ParseNotificationManager.parseNotificationMsg(Constant.getContext(), msgId, phoneNum, centerNum, bodyText, msgs[0].getTimestampMillis(), extendMap);
                    ...
                    ParseManager.parseMsgToPopupWindow(Constant.getContext(), phoneNum, centerNum, bodyText, true,extendMap);
                    //加入后台队列，丰富汽泡流程。
//                    BubbleTaskQueue.addDataToQueue(0, msgId, phoneNum, bodyText, centerNum,  msgs[0].getTimestampMillis(), 3, null);
                	...
                	Map<String,Object> map =  ParseNotificationManager.parseNotificationMsgAndPopupWindow(Constant.getContext(), 
                    		msgId, phoneNum, 
                    		centerNum, bodyText, msgs[0].getTimestampMillis(), extendMap);
                	 if(map != null && map.size() > 1){
                         notifyDataCacheMap.put(msgId,map);
                     }
//                    ParseManager.parseMsgToPopupWindow(Constant.getContext(), phoneNum, centerNum, bodyText, extendMap);
    ...
    public static void parseMsg(String msgId,String bodyText,String phoneNum,String centerNum,long time,int parseType,int simIndex){
        ...
        try{           
                ...
                if(parseType ==0){
                    ...
                    //通知栏，浮窗结果，内含丰富汽泡流程。同步方法调用。后续通知栏依据消息ID，调用 getNotifyDataCacheByMsgId  是否能获取到数据，依据有无数据，是否显示智能短信通知栏。
//                    Map<String,Object> map = ParseSmsToBubbleUtil.parseSmsToBubbleResult(msgId, phoneNum, bodyText, centerNum,  time, 4,
//                            false, true, null);
                    Map<String,Object> map =  ParseNotificationManager.parseNotificationMsg(Constant.getContext(), msgId, phoneNum, centerNum, bodyText, time, null);
                    if(map != null && map.size() > 1){
                        notifyDataCacheMap.put(msgId,map);
                    }
                }else if(parseType ==2){
    ...
    private static LruCache<String, Map<String,Object>> notifyDataCacheMap = new LruCache<String, Map<String,Object>>(10);
