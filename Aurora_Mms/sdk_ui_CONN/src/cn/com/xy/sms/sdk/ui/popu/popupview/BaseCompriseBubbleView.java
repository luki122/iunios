...
public class BaseCompriseBubbleView {
    ...
    public List<UIPart> mFootUIPartList = null;// View at the bottom of the collection
    public Map<String, PartViewParam> mPartViewMap = null;// The view parameter rules
    ...
    public BaseCompriseBubbleView(Activity context, XyCallBack callback,
            BusinessSmsMessage message, ViewGroup root) {
    ...
    public void addViews(ViewGroup root, BasePopupView bubbleView)
            throws Exception {
        if (root != null){
            this.mRoot = root;
        }
...
//        Log.e("duoqu_test", "bodyUIPartList size: " + mBodyUIPartList.size());
    ...
    public void initData(Activity context, XyCallBack callback,
            BusinessSmsMessage message, ViewGroup root) {
        ...
        try {
            ...
            if (mPartViewMap == null) {
                LogManager.e("duoqu_xiaoyuan", "BaseCompriseBubbleView.initData mPartViewMap is null.",null);
                return;
            }
    ...
    public List<UIPart> initUIPart(String type, Map<String, PartViewParam> partViewMap)
            throws Exception {
        ...
        for (int i = 0; i < size; i++) {
            tempPart = ViewManger.getUIPartByPartId(mContext,
                    mBusinessSmsMessage, mCallback, mRoot,
    ...
    public void setViewLayoutParam(View view, PartViewParam viewParamRule, UIPart part) throws Exception {
        ...
        Integer mgType = (Integer) part.getParam("MLR");// Obtaining UI_PART about spacing
        if (mgType != null) {
            margin = ViewManger.getInnerLayoutMargin(Constant.getContext(),
                    mgType);
        }
        ...
        Integer mtpo = (Integer) part.getParam("MTPO");// Obtaining UI_PART about spacing
//        LogManager.e("duoqu_test", "MTPOMTPO: "+mtpo+" partid: "+part.mPartId);
        if (mtpo != null) {
        ...
        if (view instanceof IViewAttr) {
            ...
            int height = ViewManger.getDouquAttrDimen(iAttr,
                    R.styleable.duoqu_attr_duoqu_height);
            if (height > 0) {
    ...
    public static RelativeLayout.LayoutParams getRelativeLayoutParam(
            RelativeLayout.LayoutParams lp, int verb, int viewId, int... verbs) {
        if (lp == null) {
    ...
    public static RelativeLayout.LayoutParams getRelativeLayoutParam2(
            RelativeLayout.LayoutParams lp, int viewId, int... verbs) {
        if (lp == null) {
            lp = new RelativeLayout.LayoutParams(
                    RelativeLayout.LayoutParams.WRAP_CONTENT,
                    RelativeLayout.LayoutParams.WRAP_CONTENT);
    ...
    public RelativeLayout.LayoutParams getGloabRelativeLayoutParams(View view,
            int prevId) {
    ...
    private void addUIPartToRoot(String uiType, List<UIPart> uiPartList,
            BasePopupView bubbleView) throws Exception {

        if (uiPartList != null) {
            int size = uiPartList.size();
            if (size <= 0)
                return;
//            Log.i("MainActivity", "$$$$$$$$$$$$$$$$$$$ uiPartList size: " + size);
            UIPart uiPart = null;
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (uiPart.mView != null && viewParamRule != null && i > 0) {// MarginTop between set view and view
                    ViewManger.setLayoutMarginTop(Constant.getContext(),uiPart.mView.getLayoutParams(),viewParamRule.mUiPartMarginTopType);
                }
                if(isBodyUi){
                  ViewManger.setBodyViewPadding(Constant.getContext(), uiPart.mView, uiPart.mView,viewParamRule, mPopupContentPadding);
                }
                mPrevId = newViewId;
                newViewId++;
