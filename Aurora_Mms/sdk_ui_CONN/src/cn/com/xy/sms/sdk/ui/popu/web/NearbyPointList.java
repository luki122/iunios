...
//    private ImageView mMenuView = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mNearbyPointNotFindLinearLayout.setOnClickListener(new RetryOnClickListener());
        mNearbyPointNetworkLoseLinearLayout = (LinearLayout) findViewById(R.id.duoqu_ll_nearby_point_network_lose);// 网络出错
        mNearbyPointNetworkLoseLinearLayout.setOnClickListener(new RetryOnClickListener());
        ...
        mNearbyPointListViewAdapter = new NearbyPointListViewAdapter(NearbyPointList.this, mListItems); // 创建适配器
...
//        mHeadBackView = (ImageView) actionBarLayout.findViewById(R.id.duoqu_header_back);
//
//        mTitleNameView = (TextView) actionBarLayout.findViewById(R.id.duoqu_title_name);
//        mTitleNameView.setText(mAddress);
//
//        mMenuView = (ImageView) actionBarLayout.findViewById(R.id.duoqu_header_menu);

        View loadMoreView = getLayoutInflater().inflate(R.layout.duoqu_nearby_point_list_bottom, null);

        mLoadMoreImageView = (ImageView) loadMoreView.findViewById(R.id.duoqu_iv_load_more);

        mLoadMoreTextView = (TextView) loadMoreView.findViewById(R.id.duoqu_tv_load_more);

        // 添加底部点击加载更多数据
        mloadMoreLinearLayout = (LinearLayout) loadMoreView.findViewById(R.id.duoqu_ll_load_more);
        ...
        mListView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                ...
                // 跳转到地图
                DuoquUtils.getSdkDoAction().openMap(getApplicationContext(),
                        (String) mListItems.get(position).get("name"),
                        (String) mListItems.get(position).get("address"),
                        (Double) mListItems.get(position).get("longitude"),
                        (Double) mListItems.get(position).get("latitude"));
            }
        });
    /*    // 当版本大于4.4时启用沉浸式状态栏
...
//                MenuWindow mLifeHallWindow = new MenuWindow(NearbyPointList.this, new OnClickListener() {
//                    public void onClick(View v) {
//                    }
//                });
//                Rect frame = new Rect();
//                getWindow().getDecorView().getWindowVisibleDisplayFrame(frame);
//                int yOffset = frame.top + NearbyPointList.this.findViewById(R.id.duoqu_header).getHeight();
...
//    }
    ...
    private class LoadMoreOnClickListener implements OnClickListener {
        ...
        public void onClick(View arg0) {
            ...
            // 修改提示为“加载中...”
			ViewUtil.setTextViewValue(mLoadMoreTextView, getApplication().getString(R.string.duoqu_tip_loading));
...
//            Log.d(LTAG, "加载更多数据");
            ...
            mNearbyPoint.sendMapQueryUrl(mAddress, mLocationLatitude, mLocationLongitude, mPageNum);
        }
    }
    ...
    private void getLocation() {
        // 显示附近网点加载中提示
        setViewVisibility(View.VISIBLE, View.GONE, View.GONE, View.GONE, View.GONE);
    ...
    private void doSendMapQueryUrl(double longitude, double latitude) {
        ...
        if (longitude <= 0 || latitude <= 0) {
            // 显示没有找到相关结果提示
            setViewVisibility(View.GONE, View.VISIBLE, View.GONE, View.GONE,View.GONE);
            return;
        } else {// 经纬度不为空
    ...
    private class GetLocationThread extends Thread {
        ...
        public void run() {
            try {
                ...
                mHandler.obtainMessage(NearbyPoint.DO_GET_LOCATION).sendToTarget();
    ...
    private class DoGetLocationThread extends Thread {
        ...
        public void run() {
            try {
                DuoquUtils.getSdkDoAction().getLocation(NearbyPointList.this, mHandler);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private void doGetLocation(double locationLongitude, double locationLatitude) {
        ...
        try {
            // 扩展参数的经纬度为空时执行定位
            DuoquUtils.getSdkDoAction().getLocation(getApplicationContext(), mHandler);
        } catch (Exception ex) {
            LogManager.i(LTAG, "获取经纬度失败");
            // 经纬度格式不正确，显示没有找到相关结果提示
            setViewVisibility(View.GONE, View.VISIBLE, View.GONE, View.GONE,View.GONE);
     ...
     * 显示/隐藏View
     * 
     * @param loadingVisibility 附近网点加载中(true:显示 false:隐藏)
     * @param notFindVisibility 没有找到相关结果(true:显示 false:隐藏)
     * @param networkLoseVisibility 网络出错(true:显示 false:隐藏)
     * @param nearbyPointListVisibility 附近网点(true:显示 false:隐藏)
     */
    private void setViewVisibility(int loadingVisibility, int notFindVisibility, int networkLoseVisibility,
        ...
        mNearbyPointNetworkLoseLinearLayout.setVisibility(networkLoseVisibility);// 网络出错
        ...
        // 没有找到相关结果或网络出错重置分页页码
        if (notFindVisibility == View.VISIBLE || networkLoseVisibility == View.VISIBLE) {
            mPageNum = 0;
        }
    }
    ...
    private ArrayList<HashMap<String, Object>> getListItems(String strQueryResult) {
        try {
            ...
            for (int i = 0; i < jsonResultsArray.length(); i++) {
                ...
                map.put("distance", jsonObject.getJSONObject("detail_info").getInt("distance"));
    ...
    private void analysisResult(String strQueryResult) {
        ...
        // 显示附件网点列表
        setViewVisibility(View.GONE, View.GONE, View.GONE, View.VISIBLE,View.GONE);
    }
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case NearbyPoint.QUERY_RESULT_RECEIVE:
                analysisResult(msg.getData().getString(NearbyPoint.QUERY_RESULT));
                if (mTotal > (mPageNum + 1) * 10) {
                    // 显示图片
                    mLoadMoreImageView.setVisibility(View.VISIBLE);
                    // 修改提示为“点击加载更多数据”
                    ViewUtil.setTextViewValue(mLoadMoreTextView, getApplication().getString(R.string.duoqu_tip_load_more));
                    // 启用LinearLayout
            ...
            case NearbyPoint.DO_GET_LOCATION:
                if (XyUtil.checkNetWork(getApplicationContext(), 2) != 0) {
                    // 显示网络出错提示
                    setViewVisibility(View.GONE, View.GONE, View.VISIBLE, View.GONE,View.GONE);
                } else {
                    ...
                    if (!StringUtils.isNull(locationLongitude) && StringUtils.isNull(locationLatitude)) {
                        mLocationLongitude = Double.parseDouble(locationLongitude);
                        mLocationLatitude = Double.parseDouble(locationLatitude);
                    } else {
            ...
            case QUERY_GPS_CLOSED_ERROR:
                ...
                // 百度地图检索url请求失败，显示没有搜索到相关结果提示
                setViewVisibility(View.GONE, View.GONE, View.GONE, View.GONE,View.VISIBLE);
                break;
            default:
                break;
            }
