 ...
 * the device.
 */
public class AccountPreference extends AuroraPreference {
    ...
    public AccountPreference(Context context, Account account, Drawable icon,
       ...
       /* if (showTypeIcon) {
        ...
        Log.e("JOY", "AccountPreference " + mAccount.name);
...
//            mSyncStatusIcon.setImageResource(getSyncStatusIcon(status));
        ...
        if (updateSummary) {
        	// qy getContext().getResources().getString(getSyncStatusMessage(status))
            setSummary(getSyncStatusMessage(status));
        	auroraSetArrowText("",true);
