...
import com.aurora.auroraaccountsettings.R;
...
public class AccountSyncSettings extends AccountPreferenceBase {
    ...
    private static final int MENU_MORE_ID = 103;
    ...
    private OnAuroraActionBarItemClickListener mAuroActionBarItemClickListener = new OnAuroraActionBarItemClickListener() {
    	@Override public void onAuroraActionBarItemClicked(int itemId) { 
    		switch (itemId) {
    		 case MENU_MORE_ID: 
    			 	
	            try { 
	            	((AuroraPreferenceActivity)getActivity()).showAuroraMenu(); 
	            	}
	            catch (Exception e) 
	            { 
	            	e.printStackTrace(); 
    ...
    private OnAuroraMenuItemClickListener auroraMenuCallBack = new OnAuroraMenuItemClickListener() {
    	@Override public void auroraMenuItemClick(int itemId) { 
    		switch (itemId) {
    ...
    //Gionee <wangguojing> <2013-07-25> add for CR00837650 begin
    /*private static AccountSyncSettings mAccountSyncSettings ;
    public static AccountSyncSettings getAccountSyncSettingsInstance() {
		return mAccountSyncSettings ;
		
    }
    public AccountSyncSettings() {
        mAccountSyncSettings = this;
        Settings.mKey = "AccountSyncSettings" ;
    }
    ...
    public Dialog onCreateDialog(final int id) {
        ...
        if (id == REALLY_REMOVE_DIALOG) {
            dialog = new AuroraAlertDialog.Builder(getActivity())
                        ...
                        AccountManager.get(AccountSyncSettings.this.getActivity())
                            ...
                            public void run(AccountManagerFuture<Boolean> future) {
                                ...
                                if (failed) {
                                	...
                                	// return the account main interface
                                    Intent intent = new Intent("com.aurora.auroraaccountsettings.action.ACCOUNT_ACTIVITY");
                    				
                    				startActivity(intent);
                    				
                    				getActivity().overridePendingTransition(R.anim.aurora_close_enter,R.anim.aurora_close_exit);
                    				finish();
...
//            mProviderId.setText(mAccount.type);
     ...
     //  qy set title
		mAuroraActionBar = ((AuroraPreferenceActivity)getActivity()).getAuroraActionBar();
		mAuroraActionBar.setTitle(mAccount.name); 
		
		((AuroraPreferenceActivity)getActivity()).addAuroraActionBarItem(AuroraActionBarItem.Type.More,
				MENU_MORE_ID);
		mAuroraActionBar.setOnAuroraActionBarListener(mAuroActionBarItemClickListener);
		((AuroraPreferenceActivity)getActivity()).setAuroraMenuCallBack(auroraMenuCallBack);
		
		if( mAccount!=null && mAccount.type.equals("com.tencent.mm.account")){
			((AuroraPreferenceActivity)getActivity()).setAuroraMenuItems(R.menu.weixin);
		}else{
			((AuroraPreferenceActivity)getActivity()).setAuroraMenuItems(R.menu.account_sync);
		}
    }
    ...
    public void onPause() {
        ...
        ManageAccountsSettings.isSynStateUpdate = true;// qy
