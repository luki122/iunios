...
// Aurora <zhanggp> <2013-10-18> added for systemui begin
	private static final int MSG_EXICON = 20 << MSG_SHIFT;
// Aurora <zhanggp> <2013-10-18> added for systemui end	
    private static final int MSG_SET_WINDOW_STATE           = 21 << MSG_SHIFT;
    //update to 5.0 begin
    private static final int MSG_BUZZ_BEEP_BLINKED           = 22 << MSG_SHIFT;
    private static final int MSG_NOTIFICATION_LIGHT_OFF           = 23 << MSG_SHIFT;
    private static final int MSG_NOTIFICATION_LIGHT_PULSE           = 24 << MSG_SHIFT;
    private static final int MSG_SHOW_RECENT_APPS           = 25 << MSG_SHIFT;
    private static final int MSG_HIDE_RECENT_APPS           = 26 << MSG_SHIFT;
    //update to 5.0 begin
...
// Aurora <zhanggp> <2013-10-18> added for systemui begin
	private StatusBarIconList mListEx;
// Aurora <zhanggp> <2013-10-18> added for systemui end
    //update to 5.0 begin
...
// Aurora <zhanggp> <2013-10-18> modified for systemui begin
    public CommandQueue(Callbacks callbacks, StatusBarIconList list,StatusBarIconList externList) {
...
// Aurora <zhanggp> <2013-10-17> modified for systemui begin
    ...
    /// @}
	public void transparentizeStatusBar(boolean toSet){
    }
	
	public void toggleNotificationPanel(){
	}

	public void setExIcon(int index, StatusBarIcon icon) {
		synchronized (mListEx) {
			int what = MSG_EXICON | index;
			mHandler.removeMessages(what);
			mHandler.obtainMessage(what, OP_SET_ICON, 0, icon.clone()).sendToTarget();
		}
	}
	// Aurora <zhanggp> <2013-10-17> modified for systemui end
	// Aurora <tongyh> <2014-09-17> sanxing fc begin
	public void setTransGradationMode(boolean paramBoolean){
		  
	}
	// Aurora <tongyh> <2014-09-17>  sanxing fc end
    // Aurora <xiaoyong> <2014-10-10> sanxing S5 fc begin
	public void setSemiTransparentMode(boolean paramBoolean){
		  
	}

	public void setMultiWindowBg(boolean paramBoolean) {
		
	}
	// Aurora <xiaoyong> <2014-10-10> sanxing S5 fc end
...
//                    mCallbacks.setImeWindowStatus((IBinder)msg.obj, msg.arg1, msg.arg2);
                ...
                case MSG_EXICON: {
                    final int index = msg.what & INDEX_MASK;
                    final int viewIndex = mListEx.getViewIndex(index) + mList.getViewIndex(mList.size() - 1);
                    switch (msg.arg1) {
                        case OP_SET_ICON: {
                            StatusBarIcon icon = (StatusBarIcon)msg.obj;
                            StatusBarIcon old = mListEx.getIcon(index);
                            if (old == null) {
                                mListEx.setIcon(index, icon);
                                mCallbacks.addIcon(mListEx.getSlot(index), index, viewIndex, icon);
                            } else {
                                mListEx.setIcon(index, icon);
                                mCallbacks.updateIcon(mListEx.getSlot(index), index, viewIndex,
                                        old, icon);
                            }
                            break;
                        }
                        case OP_REMOVE_ICON:
                            if (mListEx.getIcon(index) != null) {
                                mListEx.removeIcon(index);
                                mCallbacks.removeIcon(mListEx.getSlot(index), index, viewIndex);
                            }
                            break;
                    }
                    break;
...
//                    mCallbacks.notificationLightPulse((Integer) msg.obj, msg.arg1, msg.arg2);
                ...
                case MSG_SHOW_RECENT_APPS:
                	Log.d("0407", "MSG_SHOW_RECENT_APPS");
//                    mCallbacks.showRecentApps(msg.arg1 != 0);
                    break;
                case MSG_HIDE_RECENT_APPS:
                	Log.d("0407", "MSG_HIDE_RECENT_APPS");
