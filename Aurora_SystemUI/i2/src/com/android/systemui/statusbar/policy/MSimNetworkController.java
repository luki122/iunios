...
//    private MSimTelephonyManager mPhone;
  ...
  //update to 5.0 end
	...
	// Aurora <Steve.Tang> 2014-09-23, init view list. start
	private final boolean IS_CTA_SUPPORT = SystemProperties.getBoolean("ro.config.ctaenable", false);
    ArrayList<TextView> mCTALabelViews = new ArrayList<TextView>();
    ArrayList<ImageView> mCTATypeViews = new ArrayList<ImageView>();
	// Aurora <Steve.Tang> 2014-09-23, init view list. end
...
//        mDefaultSubscription = MSimTelephonyManager.getDefault().getDefaultSubscription();
        mDefaultSubscription = (int)SubscriptionManager.getDefaultSubId();
        //update to 5.0 end
        ...
        mEmergencyCallsOnly = context.getString(R.string.emergency_calls_only);
    }
...
//        	 final int subscription = intent.getIntExtra(MSimConstants.SUBSCRIPTION_KEY, 0);
            final int subscription = intent.getIntExtra(SUBSCRIPTION_KEY, 0);
          //update to 5.0 end
...
//			for (int i = 0; i < MSimTelephonyManager.getDefault().getPhoneCount(); i++) {
	...
	// Aurora <Steve.Tang> 2014-09-23, add method for add view (PhoneStatusBar). start
    public void addCTANetworkName(TextView v) {
        mCTALabelViews.add(v);
    }

	public void addCTANetworkType(ImageView v){
		mCTATypeViews.add(v);
	}
	// Aurora <Steve.Tang> 2014-09-23, add method for add view (PhoneStatusBar). end
    ...
    private void updateCarrierText(int sub) {
        ...
        if (mAirplaneMode) {
            ...
            switch (mMSimState[sub]) {
                ...
                case UNKNOWN:
                case NOT_READY:
                    textResId = R.string.lockscreen_missing_sim_message_short;
                    break;
                case PIN_REQUIRED:
    ...
    private void setCarrierText() {
    	...
    	//update to 5.0 begin
//    	String carrierNameSUB1 = mCarrierTextSub[MSimConstants.SUB1]+"";
//        String carrierNameSUB2 = mCarrierTextSub[MSimConstants.SUB2]+"";
    	String carrierNameSUB1 = mCarrierTextSub[SUB1]+"";
        String carrierNameSUB2 = mCarrierTextSub[SUB2]+"";
      //update to 5.0 end
        //Aurora <rock.tong> 2014-10-08 no sim card shows no service begin
        if(mPhone != null){
        	if(mPhone.getSimState(0) == TelephonyManager.SIM_STATE_ABSENT){
            	carrierNameSUB1 = mContext.getString(R.string.lockscreen_missing_sim_message_short);
    	    }
        	
            if(mPhone.getSimState(1) == TelephonyManager.SIM_STATE_ABSENT){
            	carrierNameSUB2 = mContext.getString(R.string.lockscreen_missing_sim_message_short);
    	    }
        }
        ...
        if(carrierNameSUB1 != null && carrierNameSUB1.equals(mContext.getString(R.string.lockscreen_missing_sim_message_short))){        	
        ...
        if(carrierNameSUB1 != null && carrierNameSUB1.equals(mContext.getString(R.string.lockscreen_carrier_default))){
        	carrierNameSUB1 = mContext.getString(R.string.carrier_default);
			hasService01 = false;
        }
        if(carrierNameSUB2 != null && carrierNameSUB2.equals(mContext.getString(R.string.lockscreen_carrier_default))){
        	carrierNameSUB2 = mContext.getString(R.string.carrier_default);
			hasService02 = false;
        }
        String carrierName = carrierNameSUB1 + "    " + carrierNameSUB2;
		//steve.tang 2014-07-15 do not show 'sim card not insert' as only one sim card insert. start
		boolean simNotClose01 = false;
		boolean simNotClode02 = false;

		if((!hasIccCard01 && hasIccCard02) || (hasIccCard01 && !hasIccCard02)){
				carrierName = hasIccCard01 ? carrierNameSUB1 : carrierNameSUB2;
		// Aurora <Steve.Tang> 2015-03-02, do not show dual sim info as one card has closed. start
		} else if(hasIccCard01 && hasIccCard02){
			simNotClose01 = isSimEnable(0);
			simNotClode02 = isSimEnable(1);
			if(!simNotClose01)
				carrierName = carrierNameSUB2;
			else if(!simNotClode02)
				carrierName = carrierNameSUB1;

		}
		// Aurora <Steve.Tang> 2015-03-02, do not show dual sim info as one card has closed. end
		//steve.tang 2014-07-15 do not show 'sim card not insert' as only one sim card insert. end
		boolean isAirmodeOn = false;
        if((carrierNameSUB1 != null && carrierNameSUB1.equals(mContext.getString(R.string.lockscreen_airplane_mode_on)))
        		&& (carrierNameSUB1 != null && carrierNameSUB2.equals(mContext.getString(R.string.lockscreen_airplane_mode_on)))){
        	carrierName = mContext.getString(R.string.lockscreen_airplane_mode_on);
			isAirmodeOn = true;
        }
		...
		if(IS_CTA_SUPPORT){
			updateCtaNetworkLabel(new SIMInfoForCta(hasIccCard01, hasService01, carrierNameSUB1), new SIMInfoForCta(hasIccCard02, hasService02, carrierNameSUB2), isAirmodeOn, isEmergencyMode);
		}
	...
	class SIMInfoForCta{
		...
		public SIMInfoForCta(boolean hasIccCard, String carrierName){
			this.hasIccCard = hasIccCard;
			this.carrierName = carrierName;
		}
		
		public SIMInfoForCta(boolean hasIccCard, boolean hasService, String carrierName){
			this.hasIccCard = hasIccCard;
	...
	//update views about cta
	private void updateCtaNetworkLabel(SIMInfoForCta ctaInfo01, SIMInfoForCta ctaInfo02, boolean isAirmodeOn, boolean isEmergencyMode){

		String ctainfo = "";
		int[] voicetypeId = {0, 0};

		if(isAirmodeOn){
			ctainfo = mContext.getString(R.string.lockscreen_airplane_mode_on);
			updateCtaView(ctainfo, true);
			updateCtaTypeView(voicetypeId);
			return;
		}

		if(isEmergencyMode){
			ctainfo = mContext.getString(R.string.quick_settings_rssi_emergency_only);
			updateCtaView(ctainfo, true);
			updateCtaTypeView(voicetypeId);
			return;
		}

		boolean showCtaLabel = ctaInfo01.hasIccCard || ctaInfo02.hasIccCard;
		if(!showCtaLabel){
			updateCtaView(ctainfo, false);
			updateCtaTypeView(voicetypeId);
			return;
		}

		StringBuilder sb = new StringBuilder();
		if(ctaInfo01.hasIccCard){
			String label01 = getNetworkName(0, ctaInfo01);
			voicetypeId[0] = getVoiceNetworkTypeResId(0);
			if(label01 != null){
				sb.append(label01.trim());
			}	
		}
		if(ctaInfo02.hasIccCard){
			String label02 = getNetworkName(1, ctaInfo02);
			voicetypeId[1] = getVoiceNetworkTypeResId(1);
			if(label02 != null){
				if(ctaInfo01.hasIccCard){
					sb.append("  ");
				}
				sb.append(label02.trim());
			}
		}
		...
		ctainfo = sb.toString();
		updateCtaView(ctainfo, showCtaLabel);
		updateCtaTypeView(voicetypeId);
	}
	// refresh network name views(CTA)
	private void updateCtaView(String ctaInfo, boolean showCtaLabel){
		for(int i = 0; i < mCTALabelViews.size(); i++) {
            TextView v = mCTALabelViews.get(i);
            v.setText(ctaInfo);
			v.setVisibility(showCtaLabel ? View.VISIBLE : View.GONE);
        }
	}

	//refresh voice network type views(CTA)
	private void updateCtaTypeView(int[] typeId){
		if(mCTATypeViews.size() != typeId.length) return;
		for(int i = 0; i < mCTATypeViews.size(); i++) {
            ImageView v = mCTATypeViews.get(i);
			v.setVisibility(typeId[i] != 0 ? View.VISIBLE : View.GONE);
            v.setImageResource(typeId[i]);
        }
	}

	//get voice network type res id
	private int getVoiceNetworkTypeResId(int sub){
		  //update to 5.0 begin
//		int voiceType = MSimTelephonyManager.getDefault().getVoiceNetworkType(sub);
		int voiceType = TelephonyManager.getDefault().getVoiceNetworkType(sub);
		  //update to 5.0 end
		Log.e("steve", " voiceType: " + voiceType);
		switch (voiceType) {
			case TelephonyManager.NETWORK_TYPE_UNKNOWN:
				return 0;
			case TelephonyManager.NETWORK_TYPE_EDGE:
				return R.drawable.voice_type_e;
			case TelephonyManager.NETWORK_TYPE_GPRS:
            case TelephonyManager.NETWORK_TYPE_GSM:
			case TelephonyManager.NETWORK_TYPE_CDMA:
			case TelephonyManager.NETWORK_TYPE_1xRTT:
			case TelephonyManager.NETWORK_TYPE_IDEN:
				return R.drawable.voice_type_2g;
...
//			case TelephonyManager.NETWORK_TYPE_TD_SCDMA:
        		  //update to 5.0 end
				return R.drawable.voice_type_3g;
			case TelephonyManager.NETWORK_TYPE_LTE:
      		  //update to 5.0 begin	
//            case TelephonyManager.NETWORK_TYPE_IWLAN:
      		  //update to 5.0 end
				return R.drawable.voice_type_4g;
			default:
				return 0;
		}
	}

	// get network name by using network operator code (e.g 46000 46001...)
	private String getNetworkName(int sub, SIMInfoForCta ctaInfo){

		if(!ctaInfo.hasService){
			return mContext.getString(R.string.carrier_default);
		}

		int operatorCode = 0;
		String voiceTypeLabel = "";
		 
		try {
            //update to 5.0 begin
//		    operatorCode = Integer.parseInt(MSimTelephonyManager.getDefault().getNetworkOperator(sub));
			operatorCode = Integer.parseInt(TelephonyManager.getDefault().getNetworkOperator(sub));
            //update to 5.0 end
		...
		Log.e("steve", "operatorCode: " + operatorCode);
		switch (operatorCode) {
			case 46000:
			case 46002:
			case 46007:
				operatorName = mContext.getString(R.string.cta_chinamobile);
				break;
			case 46001:
				operatorName = mContext.getString(R.string.cta_chinaunicom);
				break;
			case 46003:
				operatorName = mContext.getString(R.string.cta_chinatelecom);
				break;
		}
		return operatorName + voiceTypeLabel;
	}
    ...
    private PhoneStateListener getPhoneStateListener(int subscription) {
        ...
        PhoneStateListener mMSimPhoneStateListener = new PhoneStateListener(subscription) {
            ...
            public void onSignalStrengthsChanged(SignalStrength signalStrength) {
				android.util.Log.e("stevetang","\n onSignalStrengthsChanged the level is " + signalStrength.getLevel() + " and Sub id is: " + mSubscription);
                if (DEBUG) {
        ...
        if (ss != null) {
            switch (ss.getState()) {
				...
				case ServiceState.STATE_EMERGENCY_ONLY:
                    return mMSimServiceState[subscription].getDataRegState() == ServiceState.STATE_IN_SERVICE;
...
/*            mMSimPhoneSignalIconId[subscription] = R.drawable.stat_sys_signal_null;
	...
	// Aurora <Steve.Tang> 2015-03-02, add sim card enable judgement.
	private boolean isSimEnable(int slotid){
		try{
			return !gionee.telephony.GnTelephonyManager.isSimClosed(mContext, slotid);
	...
	private int getSimCountInsert(){
		int count = gionee.provider.GnTelephony.SIMInfo.getInsertedSIMCount(mContext); 
		if(count == TWO_CARD_INSERT){
			for(int i = 0; i < TWO_CARD_INSERT; i++){
				if(!isSimEnable(i)) return 1;
			}
		}
		return count;
	}
...
//    		mPhone = (MSimTelephonyManager)context.getSystemService(Context.MSIM_TELEPHONY_SERVICE);
    	...
    	operator = mPhone.getNetworkOperator(subscription);
    	if(operator!=null){ 
    		if(operator.equals("46000") || operator.equals("46002")|| operator.equals("46007")){
    			...
    			return mPhone.getNetworkOperatorName(subscription);
    		}
    	}
    	return null;
    }
