...
public class PhoneStatusBar extends BaseStatusBar {
 ...
 // Aurora <tongyh> <2013-12-06> add voicewakeup unlock begin
    private static final String VOICEWAKEUP_UNLOCK = "com.qualcomm.listen.voicewakeup.unlock";
 // Aurora <tongyh> <2013-12-06> add voicewakeup unlock end
...
//    ImageView mQuickLockScreenSecond;
 // Aurora <tongyh> <2013-12-06> add quick lock screen function end
	...
	// Aurora <Steve.Tang> 2014-09-23, add for cta. start
	LinearLayout mNetworkOper;
	// Network name view
	TextView ctaLabel;
	// Voice Network type view
	ImageView voiceType01;
	ImageView voiceType02;
	// Aurora <Steve.Tang> 2014-09-23, add for cta. end
    ...
    // color holder
    private String mCustomStatusBarColor = null; // future use
    private String mCustomNaviBarColor = null;
 ...
 // Aurora <tongyh> <2014-7-10> Prohibit full-screen pull-down begin
    private boolean isFullScreenPullDown = true;
 // Aurora <tongyh> <2014-7-10> Prohibit full-screen pull-down end
//Aurora <tongyh> <2015-01-13> add account begin
...
//    	return mNotificationData.size() > 0;
        return mNotificationData.getActiveNotifications().size() > 0;
    	//update to 5.0 end
    }
    // Gionee <fengjianyi><2013-05-10> add for CR00800567 end
 ...
 // Aurora <tongyh> <2014-03-08> add phone call state idle  can quick lock screen begin
    ...
    private static final String ACTION_SAMSUNG_COVER_OPEN_CLOSE = "com.samsung.cover.OPEN";
    private static final String BOOLEAN_EXTRA_COVER_OPEN = "coverOpen";
    
    // Aurora <tongyh> <2014-7-17> add hall switch begin
    private static boolean isCanHallOn = true;
    private static boolean isCanHallOff = true;
    // Aurora <tongyh> <2014-7-17> add hall switch end
    
    ColorInverter mColorInverter;
 ...
 // Aurora <tongyh> <2014-03-08> add phone call state idle  can quick lock screen begin
    ...
    private ContentObserver mUserSetupObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            boolean userSetup = 0 != Settings.Secure.getIntForUser(
            ...
            userSetup = true;//iuni no user setup, set 1 default, 2014-02-25 xiaoyong
    ...
    private static final int TRANSPARENCY_CHANGE_DELAY = 100; //ms TODO delay is not safe
    private Runnable mSetTransparentStatusBar = new Runnable() {
        public void run() {
            checkBarMode(BarTransitions.MODE_TRANSPARENT, mStatusBarWindowState, mStatusBarView.getBarTransitions());
        }
    };
    private Runnable mUnSetTransparentStatusBar = new Runnable() {
        public void run() {
    ...
    // Set transparent for invert mode
    void setTransparentSBar() {
        Log.d("felix", "unset status bar");
        mHandler.removeCallbacks(mSetTransparentStatusBar);
        mHandler.postDelayed(mSetTransparentStatusBar, TRANSPARENCY_CHANGE_DELAY);
    }

    // Unset transparent for normal mode
    void unSetTransparentSBar() {
        Log.d("felix", "unset status bar");
        mHandler.removeCallbacks(mUnSetTransparentStatusBar);
        mHandler.postDelayed(mUnSetTransparentStatusBar, TRANSPARENCY_CHANGE_DELAY);
    }
	...
	public void invertColorStatusBar(boolean invert) {
		// Aurora <tongyh> <2015-02-27> bug #11761 begin
		STATUSBAR_IS_INVERT = invert;
		// Aurora <tongyh> <2015-02-27> bug #11761 end
		
        if (invert)
            mColorInverter.invert(true, mStatusBarMode);
        else
            mColorInverter.invert(false, mStatusBarMode);
    }
    // Aurora <Felix.Duan> <2015-2-6> <END> StatusBar color invert

    // Aurora <Felix.Duan> <2014-6-18> <BEGIN> Support NaviBar full mode change from intent
    // Use mAuroraNaviBarMode or android standard mode to update NavigationBar mode
    private Runnable mUpdateNaviBarMode = new Runnable() {
        public void run() {
			if (mNavigationBarView == null) {
				Log.v(TAG, "mUpdateNaviBarMode mNavigationBarView == null");
				return;
			}
			
            if (mAuroraNaviBarMode == -1)
                checkBarMode(mNavigationBarMode, mNavigationBarWindowState, mNavigationBarView.getBarTransitions());
            else
                checkBarMode(mAuroraNaviBarMode, mNavigationBarWindowState, mNavigationBarView.getBarTransitions());
        }
    };
    // Aurora <Felix.Duan> <2014-6-18> <END> Support NaviBar full mode change from intent
...
// Aurora <zhanggp> <2013-10-08> added for systemui begin
		mPolicyCallback = new PhoneStatusBarPolicy.Callback(){
            ...
            // Aurora <Felix.Duan> <2014-6-18> <BEGIN> Support NaviBar full mode change from intent
            // Aurora <Felix.Duan> <2014-7-30> <BEGIN> Feature : custom navi bar color
            // Update interface to accept color value
            @Override
            public void updateNaviBarBgColor(int mode, String hexColor) {
                mAuroraNaviBarMode = mode;
                mCustomNaviBarColor = hexColor;
                mHandler.removeCallbacks(mUpdateNaviBarMode);
                // May be override by later checkBarMode()
                // TODO mechanism to hold aurora style and to return android style on time
                mHandler.post(mUpdateNaviBarMode);
            }
            // Aurora <Felix.Duan> <2014-7-30> <END> Feature : custom navi bar color
            // Aurora <Felix.Duan> <2014-6-18> <END> Support NaviBar full mode change from intent
        ...
        mHandlerBar.setNavBarView(mNavigationBarView);
...
//delete AuroraCreatQcNvItems begin, do this in MMI 2014-12-09 xiaoyong
        //AuroraCreatQcNvItems();
//delete end  

        // Aurora <Felix.Duan> <2015-2-6> <BEGIN> StatusBar color invert      
//        mColorInverter = new ColorInverter(mStatusBarView, this, false);
        // Aurora <Felix.Duan> <2015-2-6> <END> StatusBar color invert
    }
...
//rocktong
        ...
        try {
            ...
            if (showNav) {
                // Aurora <Felix.Duan> <2014-7-20> <BEGIN> Feature: navigation key swap
                if (naviKeySwapOn()){
					...
					if(!isNavgationBarHeight40dp()){
						mNavigationBarView =
							(NavigationBarView) View.inflate(context, R.layout.navigation_bar_swap_honor, null);
					}else{
						mNavigationBarView =
							(NavigationBarView) View.inflate(context, R.layout.navigation_bar_swap, null);
					}
					...
					if(!isNavgationBarHeight40dp()){
						mNavigationBarView =
							(NavigationBarView) View.inflate(context, R.layout.navigation_bar_honor, null);
					}else{
                ...
                // Aurora <Felix.Duan> <2014-7-20> <END> Feature: navigation key swap
        ...
        // Aurora <Felix.Duan> <2014-6-6> <BEGIN> Support kitkat translucent bar
        mPixelFormat = PixelFormat.TRANSLUCENT;//PixelFormat.OPAQUE;
        // Aurora <Felix.Duan> <2014-6-6> <END> Support kitkat translucent bar
		...
		// Aurora <Steve.Tang> 2014-09-23, network name and type view init. start
		mNetworkOper = (LinearLayout)mStatusBarView.findViewById(R.id.network_oper_area);
		ctaLabel = (TextView)mStatusBarView.findViewById(R.id.cta_network_label);
		voiceType01 = (ImageView)mStatusBarView.findViewById(R.id.voice_network_type01);
		voiceType02 = (ImageView)mStatusBarView.findViewById(R.id.voice_network_type02);
		// Aurora <Steve.Tang> 2014-09-23, network name and type view init. end
...
//        mTicker = new MyTicker(context, mStatusBarView);
        mTicker = new MyTicker(context, mStatusBarView, this);
...
//		// Aurora <zhanggp> <2013-10-08> added for systemui end
        ...
        if(GnTelephonyManager.isMultiSimEnabled()){
			...
			// Aurora <Steve.Tang> 2014-09-23, add network name & type view to msimnetworkcontroller. start
			mMSimNetworkController.addCTANetworkName(ctaLabel);
			mMSimNetworkController.addCTANetworkType(voiceType01);
			mMSimNetworkController.addCTANetworkType(voiceType02);
			// Aurora <Steve.Tang> 2014-09-23, add network name & type view to msimnetworkcontroller. end
...
//        }
     ...
     // Aurora <tongyh> <2014-03-08> add phone call state idle  can quick lock screen end
        ...
        filter.addAction(ACTION_SAMSUNG_COVER_OPEN_CLOSE);
     ...
     // Aurora <tongyh> <2014-7-10> Prohibit full-screen pull-down begin
        filter.addAction(HandlerBar.ENABLE_HANDLER);
        filter.addAction(HandlerBar.DISABLE_HANDLER);
     // Aurora <tongyh> <2014-7-10> Prohibit full-screen pull-down end
        
        // Aurora <tongyh> <2014-7-17> add hall switch begin
        filter.addAction("com.aurora.hall.button_on");
		filter.addAction("com.aurora.hall.button_off");
		// Aurora <tongyh> <2014-7-17> add hall switch end
		
		// Aurora <tongyh> <2013-12-06> add voicewakeup unlock begin
		filter.addAction(VOICEWAKEUP_UNLOCK);
	    // Aurora <tongyh> <2013-12-06> add voicewakeup unlock end
        ...
        // Aurora <Felix.Duan> <2015-2-6> <BEGIN> StatusBar color invert
		filter.addAction("felix.duan.invertColor");
        // Aurora <Felix.Duan> <2015-2-6> <END> StatusBar color invert
...
//Aurora <tongyh> <2015-01-13> add account end
//Aurora <tongyh> <2015-01-30> statusbarIcon hook begin
        initSystemDrawableResourceId();
//Aurora <tongyh> <2015-01-30> statusbarIcon hook end
        // Aurora <Felix.Duan> <2015-2-6> <BEGIN> StatusBar color invert      
        mColorInverter = new ColorInverter(mStatusBarView, this, false);
        // Aurora <Felix.Duan> <2015-2-6> <END> StatusBar color invert
        // Aurora <tongyh> <2015-02-27> bug #11761 begin
        STATUSBAR_IS_INVERT = false;
        // Aurora <tongyh> <2015-02-27> bug #11761 begin
    ...
    private boolean isNavgationBarHeight40dp() {
        ...
        Log.e("stevetang", "judgepix = " + judgepix + "  navheightPix = " + navheightPix);
...
//        mWindowManager.updateViewLayout(mNavigationBarView, lp);
    ...
    private void prepareNavigationBarView() {
        ...
        // No longer perform recent app task
...
//        updateSearchPanel();
      //Aurora <tongyh> <2013-12-05> delete the searchPanelView  for htc end
        ...
        mNavigationBarView.setDelegateView(mHandlerBar.getHandlerBarView());
    ...
    public void addIcon(String slot, int index, int viewIndex, StatusBarIcon icon) {
        ...
        StatusBarIconView view = new StatusBarIconView(mContext, slot, null, this);
        view.set(icon);
...
//    public void addNotification(IBinder key, StatusBarNotification notification) {
    public void addNotification(StatusBarNotification notification, RankingMap ranking) {
    	...
    	//update to 5.0 end
    	Log.d("0401", "addNotification--------notification.PackageName = " + notification.getPackageName() + " ; notification.getId() = " + notification.getId());
    	// Aurora <tongyh> <2013-10-19> delete the sansung msic notification begin
        if("com.sec.android.pagebuddynotisvc".equals(notification.getPackageName()) || "com.qualcomm.logkit".equals(notification.getPackageName())){
        	return;
        }
        if(notification.getNotification().icon  == mContext.getResources().getSystem().getIdentifier("stat_notify_rssi_in_range", "drawable", "android")){
        	return;
        }
        
        // Aurora <tongyh> <2014-12-18> Use caution traffic begin
        if(notification.getNotification().icon  == mContext.getResources().getSystem().getIdentifier("stat_notify_error", "drawable", "android") 
        		&& "android".equals(notification.getPackageName()) && notification.getNotification().tickerText.equals(mContext.getString(R.string.data_usage_warning_title))){
        	return;
        }
        ...
        if(Build.MODEL.contains("I9500") || Build.MODEL.contains("SM-N90")){
            ...
            if(notification.getId() == notification_wifi_id){
            	return;
            }
        }
//        int notification_wifi_id = mContext.getResources().getSystem().getIdentifier("stat_notify_wifi_in_range", "drawable", "android");		
//        if (notification.getNotification().icon == notification_wifi_id) {
//        	return;
//        }
		...
		if(handleSpecialNotification(key,notification)){
			Log.d("0401", "handleSpecialNotification");
			return;
		}
       ...
       /* if (FeatureOption.MTK_2SDCARD_SWAP) {
            try {
                ...
                if ((applicationInfo.flags & applicationInfo.FLAG_EXTERNAL_STORAGE) != 0) {
                    ...
                    if (DEBUG) Slog.d(TAG, "applicationInfo pkg = " + notification.getPackageName() + " to remove notification key = " + key);
                }
...
//        if (iconView == null) return;
        ...
        if (shadeEntry == null) {
        	Log.d("0401", "shadeEntry == null------return");
            return;
        }
        addNotificationViews(shadeEntry, ranking);
		Log.d("0401", "addNotificationViews(shadeEntry, ranking);------end");
    	//update to 5.0 end
...
//          if (CLOSE_PANEL_WHEN_EMPTIED && mNotificationData.size() == 0 && !mAnimating) {
            if (CLOSE_PANEL_WHEN_EMPTIED && mNotificationData.getActiveNotifications().size() == 0 && !mAnimating) {
            	//update to 5.0 end
                animateCollapsePanels();
            }
			
			// Aurora <zhanggp> <2013-10-08> added for systemui begin
			if("com.android.phone".equals(old.getPackageName()) && 2 == old.getId()){
			...
			// Aurora <zhanggp> <2013-10-08> added for systemui end
			// Aurora <tongyh> <2014-04-11> Add alarm retreated to the background of green tips begin
			if("com.android.deskclock".equals(old.getPackageName()) && 16220 == old.getId()){
				mAlarmClockView.setVisibility(View.GONE);
				// Aurora <tongyh> <2014-09-16> alarm clock statusbar's view can drag statusbar begin
				isShowAlarmClockView = false;
				// Aurora <tongyh> <2014-09-16> alarm clock statusbar's view can drag statusbar end
    ...
    private void loadNotificationShade() {
    	   Log.d("0401", "loadNotificationShade()");
...
//            Entry ent = mNotificationData.get(N-i-1);
            ...
            if("com.aurora.privacymanage".equals(ent.notification.getPackageName()) && ent.notification.getId() == 10000){
            	continue;
            }
            //Aurora <tongyh> <2014-11-05> add aurora privacymanage icon end
            if("com.android.email".equals(ent.notification.getPackageName()) && ent.notification.getId() == 1000){
            	continue;
            }
            if (!notificationIsForCurrentUser(ent.notification)) continue;
            // Aurora <tongyh> <2014-06-10> the iuni music does not support the darg notice begin
            if("com.android.music".equals(ent.notification.getPackageName())){
            ...
            if("com.aurora.account".equals(ent.notification.getPackageName()) && (ent.notification.getId() == 1000)){
            	continue;
            }
			...
			if(ent.notification.isClearable()){
            	toShow.add(ent.row);
			}else{//tymy_20150518_bug13159
				//toShowUnClickable.add(ent.row);
				if("android".equals(ent.notification.getPackageName())&&mContext.getString(com.android.internal.R.string.adb_active_notification_title).equals(ent.notification.getNotification().tickerText)){
					if(toShowUnClickable.size()==0){
						toShowUnClickable.add(ent.row);
					}else{
					    toShowUnClickable.add(toShowUnClickable.size()-1,ent.row);					
					}
				}else if("android".equals(ent.notification.getPackageName())&&com.android.internal.R.drawable.stat_sys_data_usb==ent.notification.getNotification().icon){
					toShowUnClickable.add(ent.row);
				}else{
					toShowUnClickable.add(0,ent.row);
				}
			}
            //toShow.add(ent.row);
			// Aurora <zhanggp> <2013-10-08> modified for systemui end
    ...
    protected void updateNotificationIcons() {
    	Log.d("0401", "updateNotificationIcons()---mNotificationIcons == null " + (mNotificationIcons == null ));
...
//            String key = ent.notification.getPackageName() + String.valueOf(ent.notification.getNotification().icon);
            String key = null;
            if("com.tencent.mobileqq".equals(ent.notification.getPackageName()) 
					|| "com.tencent.qqlite".equals(ent.notification.getPackageName())
					|| "com.tencent.mm".equals(ent.notification.getPackageName())){
            	key = ent.notification.getPackageName();
			}else{
			    key = ent.notification.getPackageName() + String.valueOf(ent.notification.getNotification().icon);
			}
            ...
            if("com.tencent.mobileqq".equals(ent.notification.getPackageName()) || "com.tencent.qqlite".equals(ent.notification.getPackageName())){
...
//Aurora <tongyh> <2015-01-30> statusbarIcon hook begin
                setAuroraStatusBarIconView(ent, toShow, drawableId);
//Aurora <tongyh> <2015-01-30> statusbarIcon hook end
        		
        	}else if("com.tencent.mm".equals(ent.notification.getPackageName())){
//Aurora <tongyh> <2015-01-30> statusbarIcon hook begin
                setAuroraStatusBarIconView(ent, toShow, R.drawable.aurora_weixin);
//Aurora <tongyh> <2015-01-30> statusbarIcon hook end
        	}else if("com.android.mms".equals(ent.notification.getPackageName())){
//Aurora <tongyh> <2015-01-30> statusbarIcon hook begin
                setAuroraStatusBarIconView(ent, toShow, R.drawable.aurora_message);
//Aurora <tongyh> <2015-01-30> statusbarIcon hook end
        	}else if("com.android.providers.downloads".equals(ent.notification.getPackageName())){//tymy_20150511_bug13111supplement         		
            	setAuroraStatusBarIconView(ent, toShow, com.aurora.R.drawable.stat_sys_download_anim0);
        	}else{
        		//Aurora <tongyh> <2014-11-05> add aurora privacymanage icon begin 
        		if("com.aurora.privacymanage".equals(ent.notification.getPackageName()) && ent.notification.getId() == 10000){
...
//Aurora <tongyh> <2015-01-30> statusbarIcon hook begin
        			int drawableId = getFrameworkResourcesId(ent.notification);
        			if(drawableId != 0){
        				toShow.add(getAuroraStatusBarIconView(ent,drawableId));
        			}else{
        				toShow.add(ent.icon);
        			}
//Aurora <tongyh> <2015-01-30> statusbarIcon hook end
...
//			uniqueIcon.put(key, ent.notification.getNotification().iconLevel);
			if("com.tencent.mobileqq".equals(ent.notification.getPackageName()) 
					|| "com.tencent.qqlite".equals(ent.notification.getPackageName())
					|| "com.tencent.mm".equals(ent.notification.getPackageName())){
				uniqueIcon.put(ent.notification.getPackageName(), ent.notification.getNotification().iconLevel);
			}else{
                uniqueIcon.put(key, ent.notification.getNotification().iconLevel);
			}
			//Aurora <tongyh> <2014-10-09> qq mm can show one icon end
...
//        }
    ...
    protected void setAreThereNotifications() {
    	...
    	//update to 5.0 begin
    	ArrayList<Entry> activeNotifications = mNotificationData.getActiveNotifications();
    	 final boolean any = activeNotifications.size() > 0;
//        final boolean any = mNotificationData.size() > 0;
//        final boolean clearable = any && mNotificationData.hasClearableItems();
        final boolean clearable = any &&  mNotificationData.hasActiveClearableNotifications();
    	//update to 5.0 end
        if (DEBUG) {
/*            Slog.d(TAG, "setAreThereNotifications: N=" + mNotificationData.size()
                    + " any=" + any + " clearable=" + clearable);*/
            Slog.d(TAG, "setAreThereNotifications: N=" + activeNotifications.size()
                    + " any=" + any + " clearable=" + clearable);
        }
...
/*            Slog.d(TAG, "gnSetAreThereNotifications: N=" + mNotificationData.size()
                    + " any=" + any + " clearable=" + clearable);*/
            Slog.d(TAG, "gnSetAreThereNotifications: N=" + activeNotifications.size()
                    + " any=" + any + " clearable=" + clearable);
        }
...
//                setAreThereNotifications();
    ...
    private void checkBarMode(int mode, int windowState, BarTransitions transitions) {
        ...
        //Log.v("Baisha", "mTag = " + transitions.getTag() + " mode = " + mode + " isKeyguard() = " + isKeyguard());
        if (("BarTransitions.PhoneStatusBarView".equals(transitions.getTag())) && mode == MODE_TRANSLUCENT) {
            mode = MODE_TRANSPARENT;
        }

        // Aurora <Felix.Duan> <2014-7-30> <BEGIN> Feature : custom navi bar color
        // update custom navi color
        if (mode == MODE_CUSTOM_COLOR) {
            if (("BarTransitions.PhoneStatusBarView".equals(transitions.getTag()))) {
                transitions.transitionTo(mode, anim, mCustomStatusBarColor);
            }
            if (("BarTransitions.NavigationBarView".equals(transitions.getTag()))) {
                transitions.transitionTo(mode, anim, mCustomNaviBarColor);
            }
        } else{
...
//        a.start(); //2014-05-26, run camera systemui crash, 8910
 ...
 // Aurora <tongyh> <2014-6-6> Support kitkat hiden bar end
	...
	private final int IM_FLAG = View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.STATUS_BAR_TRANSLUCENT | View.STATUS_BAR_TRANSIENT;
    private void notifyUiVisibilityChanged() {
        try {
			...
			if(mSystemUiVisibility == IM_FLAG){
				android.util.Log.e("ahah", "every time come here, mSystemUiVisibility: " + mSystemUiVisibility);
				CountUtil.getInstance(mContext).update(CountUtil.COUNT_ITEM_004, 1);
			}
...
//    protected void tick(IBinder key, StatusBarNotification n, boolean firstTime) {
    protected void tick(StatusBarNotification n, boolean firstTime) {
    	//update to 5.0 begin
    	// Aurora <tongyh> <2015-03-02> ticker icon invert begin
    	mTicker.getmIconSwitcher().setTag(n.getPackageName());
    	// Aurora <tongyh> <2015-03-02> ticker icon invert end
    ...
    private class MyTicker extends Ticker {
    	//Aurora <tongyh> <2015-02-28> ticker icon invert begin
//    	MyTicker(Context context, View sb) {
//            super(context, sb);
//        }
    	
    	MyTicker(Context context, View sb, PhoneStatusBar mPhoneStatusBar) {
            super(context, sb, mPhoneStatusBar);
        }
      //Aurora <tongyh> <2015-02-28> ticker icon invert end
...
//                    NotificationData.Entry e = mNotificationData.get(i);
                	NotificationData.Entry e = activeNotifications.get(i);
                	//update to 5.0 end
...
//        lp.gravity = getStatusBarGravity();
    ...
    private View.OnClickListener mClearButtonListener = new View.OnClickListener() {
        public void onClick(View v) {
            synchronized (mNotificationData) {
				// Aurora <Steve.Tang> 2015-03-02, Count notification clear button click counts. start
				CountUtil.getInstance(mContext).update(CountUtil.COUNT_ITEM_001, 1);
				// Aurora <Steve.Tang> 2015-03-02, Count notification clear button click counts. end
...
//Aurora <rocktong> <2015-01-19> account bug end
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            //gionee fengxb 2013-4-27 add for CR00802140 end
            /// M: [SystemUI] Support "Dual SIM PLMN Change". }@
            else if (action.equals(ACTION_SAMSUNG_COVER_OPEN_CLOSE)) {
                boolean bOpen = intent.getBooleanExtra(BOOLEAN_EXTRA_COVER_OPEN, true);
                if (bOpen) {
                    PowerManager pm = (PowerManager)mContext.getSystemService(Context.POWER_SERVICE);
                    PowerManager.WakeLock wl = pm.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.SCREEN_DIM_WAKE_LOCK, "samsung_cover_open");
                    wl.acquire();
                }
            }
          //Aurora <tongyh> <2014-06-05> add ACTION_SIM_STATE_CHANGED broadcast begin
		  ...
		  // Aurora <Steve.Tang> 2014-08-12 update as air plane mode change, end
            	if(GnTelephonyManager.isMultiSimEnabled()){
					...
					//boolean mSimOneIsAbsent = MSimTelephonyManager.getDefault().getSimState(0) == TelephonyManager.SIM_STATE_ABSENT;
                	//boolean mSimTwoIsAbsent = MSimTelephonyManager.getDefault().getSimState(1) == TelephonyManager.SIM_STATE_ABSENT;
...
android.util.Log.e("xiuyong","the air mode is: " + isAirplaneModeOn());
          ...
          //Aurora <tongyh> <2014-06-05> add ACTION_SIM_STATE_CHANGED broadcast end
         // Aurora <tongyh> <2014-7-10> Prohibit full-screen pull-down begin
            else if(action.equals(HandlerBar.ENABLE_HANDLER)){//mmi d
            	isFullScreenPullDown = true;
            }else if(action.equals(HandlerBar.DISABLE_HANDLER)){//mmi v
            	isFullScreenPullDown = false;
            }
         // Aurora <tongyh> <2014-7-10> Prohibit full-screen pull-down end
            
         // Aurora <tongyh> <2014-7-17> add hall switch begin
            else if(action.equals("com.aurora.hall.button_on")){
            	isCanHallOff = true;
            	if(isCanHallOn){
            		isCanHallOn = false;
            		PowerManager pm = (PowerManager)mContext.getSystemService(Context.POWER_SERVICE);
                    PowerManager.WakeLock wl = pm.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.SCREEN_DIM_WAKE_LOCK, "hall_on");
                    wl.acquire(30);
            	}
            }
            else if(action.equals("com.aurora.hall.button_off")){
            	isCanHallOn = true;
            	if(isCanHallOff){
            		isCanHallOff = false;
         ...
         // Aurora <tongyh> <2013-12-06> add voicewakeup unlock begin
            else if(action.equals(VOICEWAKEUP_UNLOCK)){
            	if(isNotificationExpanded()){
            		mNotificationPanel.collapse();
            	}
            }
    	    // Aurora <tongyh> <2013-12-06> add voicewakeup unlock end

            // Aurora <Felix.Duan> <2015-2-6> <BEGIN> StatusBar color invert
            else if (action == "felix.duan.invertColor") {
                mColorInverter.invert(mStatusBarMode);
            }
            // Aurora <Felix.Duan> <2015-2-6> <END> StatusBar color invert
        }
    };
...
//                    mCurrentlyIntrudingNotification.getId());
          ...
          mCurrentlyIntrudingNotification.getTag(),
          mCurrentlyIntrudingNotification.getId(),
          mCurrentlyIntrudingNotification.getUserId());
		  //update to 5.0 end
...
// Aurora <zhanggp> <2013-10-17> modified for systemui end
    ...
    private void refreshExpandedView(Context context) {
    	...
    	ArrayList<Entry> activeNotifications = mNotificationData.getActiveNotifications();
//        for (int i = 0, n = this.mNotificationData.size(); i < n; i++) {
    	for (int i = 0, n = activeNotifications.size(); i < n; i++) {
//            Entry entry = this.mNotificationData.get(i);
...
//                    removeNotification(key);
	...
	public void refreshInCallState(StatusBarNotification notification){
		...
		if("com.android.phone".equals(notification.getPackageName()) && (2 == notification.getId() || 500 == notification.getId())){
			TelephonyManager telManager = (TelephonyManager)mContext.getSystemService(Context.TELEPHONY_SERVICE);	
			int state = telManager.getDefault().getCallState();
			final boolean hasActiveCall = (TelephonyManager.CALL_STATE_OFFHOOK == state && 2 == notification.getId()) 
				||(TelephonyManager.CALL_STATE_RINGING == state && 500 == notification.getId());
			if(hasActiveCall){
    ...
    public void refreshAlarmClockState(StatusBarNotification notification){
		if("com.android.deskclock".equals(notification.getPackageName())){
			final boolean hasActiveAlram = (16210 == notification.getId());
			if(hasActiveAlram){
					final View.OnClickListener l = getNotificationClickListener(notification);
					// Aurora <tongyh> <2014-09-16> alarm clock statusbar's view can drag statusbar begin
 ...
 // Aurora <tongyh> <2014-04-11> Add alarm retreated to the background of green tips end
	...
	public void setStatusbarBgFlag(int flag){
        Log.v("xiaoyong", "flag = " + flag + " BaseStatusBar.isCanSetStatusBarViewBg = " + BaseStatusBar.isCanSetStatusBarViewBg + " isExpanded = " + isExpanded());
	...
	public void showClearButton(){
    	//update to 5.0 begin
//        final boolean any = mNotificationData.size() > 0;
		final boolean any = mNotificationData.getActiveNotifications().size()> 0;
//        final boolean clearable = any && mNotificationData.hasClearableItems();
        final boolean clearable = any &&  mNotificationData.hasActiveClearableNotifications();
    	//update to 5.0 end
...
/*        if(num > 9){
        ...
        int drawableId = R.drawable.aurora_statusbar_merge_bg_one;
		if(num < 10){
			drawableId = R.drawable.aurora_statusbar_merge_bg_one;
		}else if(num >= 10 && num < 100){
			drawableId = R.drawable.aurora_statusbar_merge_bg_two;
		}
	...
	private boolean handleSpecialNotification(String key,StatusBarNotification notification){
		if(null == notification.getTag()){
			return false;
		}

        Log.v("0401", "handleSpecialNotification getTag() = " + notification.getTag());
		...
		if(AURORA_BLACK_IMMERSE.equals(notification.getTag())){
			Log.v("0401", "handleSpecialNotification AURORA_BLACK_IMMERSE");
			handled = true;
            invertColorStatusBar(true);
		}
		else if(AURORA_WHITE_IMMERSE.equals(notification.getTag())){
			Log.v("0401", "handleSpecialNotification AURORA_WHITE_IMMERSE");
			handled = true;
	...
	// Aurora <tongyh> <2014-09-01> But the notification icon does not show the number plus one begin
		...
		// Aurora <tongyh> <2014-09-19> the status bar icon code logic changes begin
//		if("com.tudou.android".equals(notification.getPackageName())){
	...
	private boolean canShowIcon(StatusBarNotification notification){
		...
		if((notification.getPackageName().equals("com.android.phone") && notification.getId() == 1)
				|| (notification.getPackageName().equals("com.android.phone") && notification.getId() == (AURORA_NOTIFICATION_ID + PHONE_NOTIFICATION_ID + CALL_FORWARDING_NOTIFICATION_ID))
		        || (notification.getPackageName().equals("com.android.phone") && notification.getId() == (AURORA_NOTIFICATION_ID + PHONE_NOTIFICATION_ID + CALL_FORWARDING_NOTIFICATION_SIM2_ID))){			    
    		return true;
    	}
		if(notification.getPackageName().equals("com.android.phone")
				|| notification.getPackageName().equals("com.tudou.android") 
				|| notification.getPackageName().equals("com.android.music")){
			return false;
		}
...
//		if(notification.isClearable()) return true;
		// Aurora <tongyh> <2014-09-19> the status bar icon code logic changes end
		if("android".equals(notification.getPackageName())
		||"com.android.systemui".equals(notification.getPackageName()) 
		|| "com.tencent.mobileqq".equals(notification.getPackageName())  
		|| "com.tencent.qqlite".equals(notification.getPackageName())  
		|| "com.tencent.mm".equals(notification.getPackageName()) 
		|| "com.android.mms".equals(notification.getPackageName())
		|| "com.android.providers.downloads".equals(notification.getPackageName())
		//Aurora <tongyh> <2014-11-05> add aurora privacymanage icon begin  
		|| ("com.aurora.privacymanage".equals(notification.getPackageName()) && notification.getId() == 10000)
		|| "com.android.email".equals(notification.getPackageName())
		//Aurora <tongyh> <2014-11-05> add aurora privacymanage icon end
		//Aurora <tongyh> <2015-03-10> add aurora account animation icon begin
		|| ("com.aurora.account".equals(notification.getPackageName()) && notification.getId() == 1000)) {
		//Aurora <tongyh> <2015-03-10> add aurora account animation icon end
			return true;
		}
		return false;
...
/*private View.OnTouchListener mQuickLockScreenTouchListenerSecond = new View.OnTouchListener() {
    ...
    // Aurora <Felix.Duan> <2014-7-20> <BEGIN> Feature: navigation key swap
    /**
     * Feature: swap MENU_KEY & BACK_KEY positions on navigation bar
     * Components:
     *      A new navigation layout for swap
     *      A new 0/1 flag: NAVI_KEY_SWAP in SettingsProvider
     *      A ContentObserver to update the changing
     * 
     * @author Felix.Duan
     * @date 2014-7-20
     */
    private static final String NAVI_KEY_SWAP = "navigation_key_swap";
    private ContentObserver mNaviKeySwapObserver = new ContentObserver(new Handler()) {
        boolean changing = false;
        @Override
        public void onChange(boolean selfChange) {
            if (mNavigationBarView == null || changing) return;
            changing = true;
            mWindowManager.removeView(mNavigationBarView);
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    boolean swap = naviKeySwapOn();
                    Log.d(TAG,"mNaviKeySwapObserver onChange() swap = " + swap);
                    if (swap)
                        mNavigationBarView = (NavigationBarView) View.inflate(mContext, R.layout.navigation_bar_swap, null);
                    else
    ...
    /**
     * Get NAVI_KEY_SWAP value in Settings.System
     * (content://settings/system/navigation_key_swap)
     *      0 & null    for swap off   default layout
     *      1 & others  for swap on    MENU_KEY & BACK_KEY swap positions
     * 
     * return true if swap on, else false
     *
     * @author Felix.Duan
     * @date 2014-7-20
     */
    private boolean naviKeySwapOn() {
        return 0 != Settings.System.getInt(
                mContext.getContentResolver(),
                NAVI_KEY_SWAP, 0 /*default */);
    }

    /**
     * Setup a ContentObserver to get notified when user changes setting.
     * Only when there is a navigation bar
     * 
     * @author Felix.Duan
     * @date 2014-7-20
     */
    private void setupNaviKeySwapObserver() {
        if(mNavigationBarView == null) return;
        Log.d(TAG,"setupNaviKeySwapObserver");
        mContext.getContentResolver().unregisterContentObserver(mNaviKeySwapObserver);
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(NAVI_KEY_SWAP),
                false, mNaviKeySwapObserver);
    }
    // Aurora <Felix.Duan> <2014-7-20> <END> Feature: navigation key swap
 ...
 // Aurora <tongyh> <2014-07-28> set navifation bar low profile end
    public boolean isConfigchang = false;
    public boolean isRefreshExpandedView = false;
    
    private SettingsObserver mSettingsObserver;
    
    private void initNavigationKeyObserver() {
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            mContext.getContentResolver().registerContentObserver(
            Settings.System.getUriFor("navigation_key_hide"),
            false, this);
        }
        ...
        void update() {
        	...
        	boolean canUpdate = (Settings.System.getInt(mContext.getContentResolver(),
        			"navigation_key_hide", 0) == 1) && (config.orientation == Configuration.ORIENTATION_LANDSCAPE);
        	if(canUpdate && isRefreshExpandedView && isConfigchang){
        		isRefreshExpandedView = false;
        		refreshExpandedView(mContext);
        	}
...
//Aurora <tongyh> <2015-01-30> statusbarIcon hook begin
    ...
    public void initSystemDrawableResourceId(){
    	//drawable id
    ...
    public void setAuroraStatusBarIconView(Entry ent, ArrayList<View> toShow, int drawableId){
    	final StatusBarIconView iconView = new StatusBarIconView(mContext,
				ent.notification.getPackageName() + "/0x" + Integer.toHexString(ent.notification.getId()),
				ent.notification.getNotification(), this);
        iconView.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        
        final StatusBarIcon ic = new StatusBarIcon(null,
        		ent.notification.getUser(),
        		drawableId,
                    ent.notification.getNotification().iconLevel,
                    ent.notification.getNotification().number,
                    ent.notification.getNotification().tickerText);
        if (iconView.set(ic)) {
        	toShow.add(iconView);
        }
    }
    
    public StatusBarIconView getAuroraStatusBarIconView(Entry ent, int drawableId){
    	StatusBarIconView iconView = ent.icon;
    	StatusBarIcon ic =  iconView.getStatusBarIcon().clone();
    	ic.iconId = drawableId;
    	iconView.set(ic);
        return iconView;
    }
    
    public int getFrameworkResourcesId(StatusBarNotification sbn){
    	int drawableId = 0;
    	if(sbn.getId() == notification_wifi_id || sbn.getId() == -2000010){
    		drawableId = com.aurora.R.drawable.stat_notify_wifi_in_range;
    	}else if(sbn.getNotification().icon == notification_wifi_id){
    		drawableId = com.aurora.R.drawable.stat_notify_wifi_in_range;
    	}else if(sbn.getId() == notification_adb_active_id){
    		drawableId = com.aurora.R.drawable.stat_sys_adb;
    	}else if(sbn.getId() == notification_stat_sys_data_usb_id || sbn.getNotification().icon == notification_stat_sys_data_usb_id){
    		drawableId = com.aurora.R.drawable.stat_sys_data_usb;
    	}else if(sbn.getId() == notification_stat_sys_warning_id){
    		drawableId = com.aurora.R.drawable.stat_notify_error;
    		...
    		drawableId = com.aurora.R.drawable.stat_sys_tether_general;
    	}else if(sbn.getId() == notification_stat_tether_usb_id){
    		drawableId = com.aurora.R.drawable.stat_sys_tether_usb;
    	}else if(sbn.getId() == notification_stat_tether_wifi_id){
    		drawableId = com.aurora.R.drawable.stat_sys_tether_wifi;
    	}else if(sbn.getNotification().icon == notification_stat_notify_disk_full_id){
    		drawableId = com.aurora.R.drawable.stat_notify_disk_full;
    	}else if(sbn.getNotification().icon == notification_stat_notify_sdcard_usb_id){
    		drawableId = com.aurora.R.drawable.stat_notify_sdcard_usb;
    	}else if(sbn.getNotification().icon == notification_stat_notify_sdcard_prepare_id){
    		drawableId = com.aurora.R.drawable.stat_notify_sdcard_prepare;
    	}else if(sbn.getNotification().icon == notification_stat_notify_rssi_in_range){
    		drawableId = com.aurora.R.drawable.stat_notify_rssi_in_range;
    	}
    		...
    		drawableId = com.aurora.R.drawable.stat_notify_sdcard_usb;
    	}else if(sbn.getNotification().icon == notification_stat_notify_stat_sys_download_done_id || sbn.getNotification().icon == 0x01080082){
    			drawableId = com.aurora.R.drawable.stat_sys_download_anim0;
    	}
    	else{
    		drawableId = 0;
    	}
