...
//        updateSearchPanel();
    ...
    public void addNotification(IBinder key, StatusBarNotification notification) {
    	Log.d("PhoneStatusBar", "addNotification--------notification.PackageName = " + notification.getPackageName());
...
/*        if(num > 9){
	...
	private boolean handleSpecialNotification(IBinder key,StatusBarNotification notification){
        ...
        Log.v("xiaoyong", "handleSpecialNotification getTag() = " + notification.getTag());
		...
		if(AURORA_BLACK_IMMERSE.equals(notification.getTag())){
			Log.v("xiaoyong", "handleSpecialNotification AURORA_BLACK_IMMERSE");
			handled = true;
            invertColorStatusBar(true);
		}
		else if(AURORA_WHITE_IMMERSE.equals(notification.getTag())){
			Log.v("xiaoyong", "handleSpecialNotification AURORA_WHITE_IMMERSE");
			handled = true;
...
/*private View.OnTouchListener mQuickLockScreenTouchListenerSecond = new View.OnTouchListener() {
    ...
    // Gionee <bug> <jiaoyuan> <20131130> add for CR00960202 begin
    private Runnable mInitRunnable = new Runnable() {
        @Override
        public void run() {
            try {
                AuroraDeviceSN.getDefault().AuroraCreatQcNvItems(mContext);
                Log.v("xiaoyong", "mInitRunnable.run()");
            } catch (Exception ex) {
            }
        }
    };
        
    private void AuroraCreatQcNvItems() {
        // Only run on U810 model family
        if (!Build.MODEL.contains("U810")/* && !Build.MODEL.contains("U3")*/) return;
    ...
    // Gionee <bug> <jiaoyuan> <20131130> add for CR00960202 end
