...
//M:xiejun bluetooth data_in & data_out begin
import android.content.ContentResolver;
import android.net.Uri;
//M:xiejun bluetooth data_in & data_out end
...
public class PhoneStatusBarPolicy {
    ...
    // Intent used to set NavigationBar color, must supply a int extra "mode" in range of -1 to 5
    ...
    */
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // Aurora <Felix.Duan> <2014-6-18> <BEGIN> Support NaviBar full mode change from intent
            } else if (action.equals(ACTION_SET_NAVIBAR_COLOR)) {
                /*
                 * Mode is defined in BarTransitions
                 * -1 stands for use android standard mode
                 */
                if (mCallback != null) {
                    int mode = intent.getIntExtra("mode", -1);
                    // Aurora <Felix.Duan> <2014-7-30> <BEGIN> Feature : custom navi bar color
                    // now we accept color value
                    String hexColor = intent.getStringExtra("color");
                    mCallback.updateNaviBarBgColor(mode, hexColor);
                    Log.d(TAG, "color PSBP ACTION_SET_NAVIBAR_COLOR  mode = " + mode + " hexColor = " + hexColor); 
	...
	public interface Callback {
        ...
        // Aurora <Felix.Duan> <2014-6-18> <BEGIN> Support NaviBar full mode change from intent
        // Aurora <Felix.Duan> <2014-7-30> <BEGIN> Feature : custom navi bar color
        // update interface to accept color value as android.graphics.Color@parceColor() style
    ...
    public PhoneStatusBarPolicy(Context context) {
        ...
        // Aurora <Felix.Duan> <2014-6-18> <BEGIN> Support NaviBar full mode change from intent
        filter.addAction(ACTION_SET_NAVIBAR_COLOR);
        // Aurora <Felix.Duan> <2014-6-18> <END> Support NaviBar full mode change from intent
        ...
        */
        ...
        /// @}
        //M:xiejun bluetooth data_in & data_out begin
        registBlueToothDataInOutObserver();
        //M:xiejun bluetooth data_in & data_out end
    }
    ...
    private final void updateBluetooth(Intent intent) {
        ...
        if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
            ...
            if (state == BluetoothAdapter.STATE_CONNECTED) {
				...
				//iconId =  R.drawable.stat_sys_ringer_silent;
				//contentDescription = mContext.getString(R.string.accessibility_ringer_silent);
				switch (mBluetoothDataState) {
				case DATA_NOT_IN_OUT:
					iconId = R.drawable.stat_sys_data_bluetooth_connected;
					contentDescription = mContext.getString(R.string.accessibility_bluetooth_connected);
					break;
				case DATA_IN:
					iconId = R.drawable.aurora_stat_sys_data_bluetooth_data_in;
					contentDescription = "";
					break;
				case DATA_OUT:
					iconId = R.drawable.aurora_stat_sys_data_bluetooth_data_out;
					contentDescription = "";
					break;
				case DATA_IN_OUT:
					iconId = R.drawable.aurora_stat_sys_data_bluetooth_data_in_out;
					contentDescription = "";
					break;
				default:
					iconId = R.drawable.stat_sys_data_bluetooth_connected;
    ...
    */
    ...
    //M:xiejun bluetooth data_in & data_out begin
    private static final int DATA_NOT_IN_OUT=0;
    private static final int DATA_IN=1;
    private static final int DATA_OUT=2;
    private static final int DATA_IN_OUT=3;
    private int mBluetoothDataState =DATA_NOT_IN_OUT;
    private boolean mBlueToothConnected = false;
    private ContentObserver mBlueToothDataInOut = new ContentObserver(mHandler) {
    	public void onChange(boolean selfChange) {
    		int in = Settings.System.getInt(mContext.getContentResolver(), "bluetooth.opp.in",0);
    		int out = Settings.System.getInt(mContext.getContentResolver(), "bluetooth.opp.out",0);
    		Log.i("BlueTooth","mBlueToothDataInOut in = "+in+"  ,  out = "+out);
    		if(in==1&&out==1){
    			mBluetoothDataState=DATA_IN_OUT;
    		}else if(in==1&&out==0){
    			mBluetoothDataState = DATA_IN;
    		}else if(in==0&&out==1){
    			mBluetoothDataState = DATA_OUT;
    		}else{
    			mBluetoothDataState =DATA_NOT_IN_OUT;
    		}
    		updateBluetooth();
    	};
	};
    ...
    private void updateBluetooth(){
        int iconId = R.drawable.stat_sys_data_bluetooth;
        String contentDescription = null;
        Log.i("BlueTooth","mBlueToothDataInOut mBlueToothConnected= "+mBlueToothConnected+"  mBluetoothDataState = "+mBluetoothDataState);
		switch (mBluetoothDataState) {
		case DATA_NOT_IN_OUT:
			iconId = R.drawable.stat_sys_data_bluetooth_connected;
			contentDescription = mContext.getString(R.string.accessibility_bluetooth_connected);
	...
	private void registBlueToothDataInOutObserver() {
		ContentResolver cr = mContext.getContentResolver();
		Uri uri1 = Settings.System.getUriFor("bluetooth.opp.in");
		if (uri1 != null) {
			cr.registerContentObserver(uri1, true, mBlueToothDataInOut);
		}
		Uri uri2 = Settings.System.getUriFor("bluetooth.opp.out");
		if (uri2 != null) {
			cr.registerContentObserver(uri2, true, mBlueToothDataInOut);
		}
	}
    //M:xiejun bluetooth data_in & data_out end
}
