...
public class NotificationPanelView extends PanelView {
    ...
    // Gionee <fengjianyi><2013-05-10> add for CR00800567 end
    private int initDisplayWidth = 0;
    private int initDisplayHeight = 0;
    int navHeightPx = 0;
    ...
    public NotificationPanelView(Context context, AttributeSet attrs) {
		...
		mDisplay = ((WindowManager)mContext.getSystemService(Context.WINDOW_SERVICE))
                .getDefaultDisplay();
		initDisplayWidth = mDisplay.getWidth();
		initDisplayHeight = mDisplay.getHeight();
		navHeightPx = mContext.getResources()
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed) {
            ...
            if (ToolbarIconUtils.GN_QUICK_SETTINGS_SUPPORT) {
				...
				// Aurora <zhanggp> <2013-10-08> modified for systemui begin
            	// Aurora <tongyh> <2014-08-11> problems caused by the navigation bar hidden begin
            	if(mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE){
            		if(Settings.System.getInt(mContext.getContentResolver(),
                			"navigation_key_hide", 0) == 1){
                		if((int) mHandleBarWidth == initDisplayHeight){
                			mHandleBarWidth = mHandleBarWidth + navHeightPx;
                		}
                	}else{
                		mHandleBarWidth = initDisplayHeight;
                	}
            	}else{
            		mHandleBarWidth = initDisplayWidth;
            	}
            	// Aurora <tongyh> <2014-08-11> problems caused by the navigation bar hidden end
...
//			mBackgroundImg.setImageBitmap(mPanelBg);
	...
	private Bitmap chopOffNaviBar(Bitmap srcBitmap) {
        ...
        //boolean hasNav = mContext.getResources()
        ...
        boolean hasNav = Utils.hasNavBar();
...
/*        int navHeightPx = mContext.getResources()
		...
		if(Settings.System.getInt(mContext.getContentResolver(),
			...
			if(ori == mConfiguration.ORIENTATION_LANDSCAPE){
				width = srcBitmap.getWidth() - (navHeightPx >> 2);
				height = srcBitmap.getHeight();
			}
		}
