...
public class AuroraSwipeHelper implements Gefingerpoken {
    static final String TAG = "com.android.systemui.AuroraSwipeHelper";
	...
	private View mShowingButtonView;
	private float mButtonWidth;
	
    public AuroraSwipeHelper(int swipeDirection, Callback callback, float densityScale,
            float pagingTouchSlop,float buttonWidth) {
        mCallback = callback;
    ...
    public static void invalidateGlobalRegion(View view, RectF childBounds) {
        ...
        while (view.getParent() != null && view.getParent() instanceof View) {
            ...
            if (DEBUG_INVALIDATE) {
                Log.d(TAG, "INVALIDATE(" + (int) Math.floor(childBounds.left)
                        + "," + (int) Math.floor(childBounds.top)
                        + "," + (int) Math.ceil(childBounds.right)
                        + "," + (int) Math.ceil(childBounds.bottom));
            }
	...
	public void snapShowButtonView(){
		if(null != mShowingButtonView){
			snapChild(mShowingButtonView, 200);
			mShowingButtonView = null;
		}
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mCurrView != null) {
					...
					if(mShowingButtonView != null && mCurrView != mShowingButtonView){
						snapChild(mShowingButtonView, 200);
						mShowingButtonView = null;
						return true;
					}
    ...
    public void snapChild(final View view, float velocity) {
        ...
        anim.setDuration(duration);
		float pos = getTranslation(view);
		final boolean canSetAlpha = (pos < 0 ? false : true);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mCurrView != null) {
                    ...
                    // maxScrollDistance
                    					//aurora
					if(delta < 0 && mCallback.canChildShowButton(mCurrView)){
						float size = mButtonWidth;
						if(-delta >= size){
							delta = -size;
							mShowingButtonView = mCurrView;
						}else{
							mShowingButtonView = null;
						}
					}else
    ...
    public interface Callback {
		...
		boolean canChildShowButton(View v);
