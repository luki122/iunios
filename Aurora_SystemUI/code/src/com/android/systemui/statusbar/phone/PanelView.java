...
//    public static final boolean DEBUG = PanelBar.DEBUG;
    ...
    private void runPeekAnimation() {
        ...
        Log.d(TAG, "---PanelView---peek to height= " + mPeekHeight);
    ...
    private void animationTick(long dtms) {
        if (!mTimeAnimator.isStarted()) {
            ...
            if (mRubberbanding) {
                ...
                Log.d(TAG, "---PanelView---animationTick---mFinalTouchY = " + mFinalTouchY);
                Log.d(TAG, "---PanelView---animationTick---(mFinalTouchY / getFullHeight()) = " + (mFinalTouchY / getFullHeight()));
            } else {
                mClosing = mExpandedHeight > 0 && mVel < 0;
                Log.d(TAG, "---PanelView---animationTick---mExpandedHeight = " + mExpandedHeight);
                Log.d(TAG, "---PanelView---animationTick---mVel = " + mVel);
            }
            Log.d(TAG, "---PanelView---animationTick---mClosing = " + mClosing);
        } else if (dtms > 0) {
            final float dt = dtms * 0.001f;                  // ms -> s
            Log.d(TAG, "---PanelView---tick: v,dt---"+mVel + "px/s,   " + dt + "s");
            if (DEBUG) LOG("tick: v=%.2fpx/s dt=%.4fs", mVel, dt);
            if (DEBUG) LOG("tick: before: h=%d", (int) mExpandedHeight);
            Log.d(TAG, "---PanelView---tick: before: h---"+(int) mExpandedHeight);
            ...
            Log.d(TAG, "---PanelView---tick: new h,closing---   " + ((int) h)+ "," + (mClosing?"true":"false"));
            if (DEBUG) LOG("tick: new h=%d closing=%s", (int) h, mClosing?"true":"false");

            setExpandedHeightInternal(h);

            mBar.panelExpansionChanged(PanelView.this, mExpandedFraction);
            Log.d(TAG, "---PanelView---tick: mVel,mClosing,mExpandedHeight,mRubberbanding,fh---   " + mVel + "," + (mClosing?"true":"false") + " , " + mExpandedHeight + " , " + mRubberbanding + " , " + fh);
            Log.d(TAG, "---PanelView---tick: mVel == 0,(mClosing && mExpandedHeight == 0),((mRubberbanding || !mClosing) && mExpandedHeight == fh)---" + (mVel == 0) + " , " + (mClosing && mExpandedHeight == 0) + " , " + (((mRubberbanding || !mClosing) && mExpandedHeight == fh)));
            if (mVel == 0
                    || (mClosing && mExpandedHeight == 0)
                    || ((mRubberbanding || !mClosing) && mExpandedHeight == fh)) {
            	Log.d(TAG, "---PanelView---tick:mStopAnimator-----------------------------------------");
                post(mStopAnimator);
            }
        }
    }
    ...
    protected void onFinishInflate() {
        ...
        loadDimens();
        Log.d(TAG, "---PanelView---handle view: " + mHandleView);
        ...
        if (mHandleView != null) {
            mHandleView.setOnTouchListener(new View.OnTouchListener() {
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    ...
                    Log.d(TAG, "---PanelView---event.getAction(),y,rawY,mTouchOffset---" + event.getAction() + " , " + y + " , " + rawY + " , " + mTouchOffset);
                    ...
                    switch (event.getAction()) {
                        case MotionEvent.ACTION_DOWN:
                        	Log.d(TAG, "---PanelView---ACTION_DOWN---mPeekHeight = " + mPeekHeight);
                        ...
                        case MotionEvent.ACTION_MOVE:
                            ...
                            Log.d(TAG, "---PanelView---ACTION_MOVE---h = " + h);
                            
                            if (h > mPeekHeight) {
                            	Log.d(TAG, "---PanelView---ACTION_MOVE------com in");
                            ...
                            if (!mJustPeeked) {
                            	Log.d(TAG, "---PanelView---ACTION_MOVE------setExpandedHeightInternal");
                        ...
                        case MotionEvent.ACTION_CANCEL:
                        	Log.d(TAG, "---PanelView---ACTION_UP---ACTION_CANCEL");
                            mFinalTouchY = y;
                            mTracking = false;
							// Aurora <zhanggp> <2013-10-17> added for systemui begin
							onTrackingEnd();
                            ...
                            if (DEBUG) LOG("gesture: dy=%f vel=(%f,%f) vlinear=%f",
                                    ...
                                    xVel, yVel,
                                    vel);
                            Log.d(TAG, "---PanelView---ACTION_UP---ACTION_CANCEL---deltaY,xVel,yVel,vel,negative > " + deltaY + " , " + xVel + " , " + yVel + " , " + vel+ " , " + negative);
                            fling(vel, true);
    ...
    public void fling(float vel, boolean always) {
    	Log.d(TAG, "---PanelView---fling: vel, this---" + vel+ " , " +this);
    ...
    protected void onViewAdded(View child) {
    	...
    	Log.d(TAG, "---PanelView---onViewAdded: " + child);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    	...
    	Log.d(TAG, "---PanelView---onMeasure---widthMeasureSpec, heightMeasureSpec, getMeasuredWidth(), getMeasuredHeight() -> " + widthMeasureSpec + " , " + heightMeasureSpec + " , " + getMeasuredWidth() + " , " + getMeasuredHeight());
    ...
    public void setExpandedHeight(float height) {
    	...
    	Log.d(TAG, "---PanelView---setExpandedHeight---height = " + height);
        mRubberbanding = false;
    ...
    protected void onLayout (boolean changed, int left, int top, int right, int bottom) {
    	...
    	Log.d(TAG, "---PanelView---onLayout: changed, left, top, right, bottom"+(changed?"T":"f") + " , " + left + " , " + top + " , " + right + " , " + bottom);
    ...
    public void setExpandedHeightInternal(float h) {
        ...
        Log.d(TAG, "---PanelView---setExpandedHeightInternal: height, fh, tracking, rubber, " + h + " , " + fh + " , " + (mTracking?"T":"f") + " , " + (mRubberbanding?"T":"f"));
...
//        setLayoutParams(lp);
    ...
    private float getFullHeight() {
        ...
        if (mFullHeight <= 0 || mIsFirstAnimTick) {
        	...
        	Log.d(TAG, "---PanelView---Forcing measure() since fullHeight=" + mFullHeight);
...
// Aurora <zhanggp> <2013-10-17> added for systemui end
    ...
    public void collapse() {
    	...
    	Log.d(TAG, "---PanelView---collapse: " + this);
        if (!isFullyCollapsed()) {
    ...
    public void expand() {
        ...
        Log.d(TAG, "---PanelView---expand: " + this);
        if (isFullyCollapsed()) {
            mBar.startOpeningPanel(this);
            fling(mSelfExpandVelocityPx, /*always=*/ true);
        } else if (DEBUG) {
            if (DEBUG) LOG("skipping expansion: is expanded");
        	Log.d(TAG, "---PanelView---skipping expansion: is expanded");
        }
    }
...
//        LOG("expandSlow: " + this);
    	Log.d(TAG, "---PanelView---expandSlow: " + this);
        if (isFullyCollapsed()) {
            mBar.startOpeningPanel(this);
