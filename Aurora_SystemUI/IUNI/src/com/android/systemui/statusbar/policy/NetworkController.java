...
//			if(mSimState.iccCardExist()){
    ...
    private final void updateDataNetType() {
        ...
        /// M: Support Roam Data Icon both show.
     // Aurora <tongyh> <2014-06-20> do not open Roam do not visiable R begin
        /*if (isCdma()) {
            if (isCdmaEri()) {
     ...
     // Aurora <tongyh> <2014-06-20> do not open Roam do not visiable R end
        if (mPhone.isNetworkRoaming()) {
            mRoamIconId = TelephonyIcons.ROAMING[simColor];
            mIsRoaming = true;
        } else {
        ...
        if (mIsWimaxEnabled && mWimaxConnected) {
            ...
            switch (mDataNetType) {
				...
				// Steve.Tang 2014-07-23 show 3g for China mobile's TD-SCDMA. start
				case TelephonyManager.NETWORK_TYPE_TD_SCDMA:
                // Steve.Tang 2014-07-23 show 3g for China mobile's TD-SCDMA. end
				    if (!mIsRoaming) {  /// M: Support Roam Data Icon both show.
...
//            str.append(spn);
    ...
    void refreshViews() {
        ...
        if (mAirplaneMode &&
         ...
         // Aurora <tongyh> <2014-06-20>  do not open Roam do not visiable R end
            	if (mPhone.isNetworkRoaming() && simColor >= 0 && simColor <= 3) {
                mRoamIconId = TelephonyIcons.ROAMING[simColor];
                mIsRoaming = true;
            /// M: [SystemUI] Support "SIM Color Change". }@
            } else {
