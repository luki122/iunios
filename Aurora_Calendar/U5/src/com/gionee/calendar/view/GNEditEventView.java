...
//Gionee <jiating> <2013-04-09> modify for CR00000000 use new edit event view begin
//Gionee <pengwei><20130809> modify for CR00853574 begin
public class GNEditEventView implements View.OnClickListener,
...
//	Button mEndDateButton;
	TextView mStartDataText;
	TextView mEndDataText;
	View mStartDataView;
	View mEndDataView;
	...
	RelativeLayout mVoiceAddEventButton;
	RelativeLayout  mVoiceAddEventLayout;
	int toolbarHeight;
	private   boolean isClickVoiceButton;
	private GNEditEventRelativelayout mEditEventLinearLayout;
	View mEditEvent;
	...
	private void populateWhen() {
		Log.i("jiating","populateWhen....mStartTime="+mStartTime.toString()+"mEndTime="+mEndTime.toString());
...
//		setDate(mEndDateButton, endMillis);
		setDate(mStartDataText, startMillis);
		setDate(mEndDataText, endMillis);
...
//				setEditTime(mActivity, startMillis,mStartDateButton);
//			}
//		});
...
//			public void onClick(View v) {
//				// TODO Auto-generated method stub
//				setEditTime(mActivity, endMillis,mEndDateButton);
//			}
//		});
...
//			TreeMap<String, String[]> titleToQueryResults = new TreeMap<String, String[]>(
...
//	}
	...
	public void onClick(View view) {
		...
		if(view.getId()==R.id.edit_event_more_button){
			mIsClickMoreButton=true;
			Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_CLICK_MORE);
			setMoreContentViewVisible(mIsClickMoreButton);
			
		}else if(view.getId()==R.id.gn_voice_add_activity_view){
			Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_CLICK_VOICE_BUTTON);
			if(isClickVoiceButton){
		        	...
		        	Log.i("jiating","isClickVoiceButton="+isClickVoiceButton);
		        	Intent intent = new Intent(EditEventFragment.GN_RECOGNIZE_ACTION); 
					intent.putExtra("scene", "schedule_create");
					intent.putExtra("appid", "com.android.calendar");
					mActivity.startActivityForResult(intent,EditEventFragment.GN_RECOGNIZE_ACTION_CODE);
...
//			setEditTime(mActivity, startMillis,0,mStartDateButton);
...
//			setEditTime(mActivity, endMillis,mStartTime.toMillis(false),mEndDateButton);
			setEditTime(mActivity, endMillis,mStartTime.toMillis(false),mEndDataView);
		}else{
			//Gionee <jiating><2013-07-03> modify for CR00830388 end
			...
			//Gionee jiating 2013-04-08 modify for new reminder view end
			updateRemindersVisibility(mReminderItems.size());
			EventViewUtils.updateAddReminderButton(mActivity,mView, mReminderItems,
	...
	public void requestFocus() {
		if (!(mTitleTextView.isFocused() || mDescriptionTextView.isFocused() || mAttendeesList
				.isFocused())) {
	...
	private boolean fillModelFromUI() {
    	...
    	for(int i=0;i<count;i++){
    		...
    		Log.i("jiating","fillModelFromUI="+mModel.mReminders.get(i).getMinutes()+"....mReminders.get(i).getMethod()"+mModel.mReminders.get(i).getMethod());
    		switch (mModel.mReminders.get(i).getMinutes()) {
			case 0:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_ZERO);
				break;
			case 1:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_ONE);
				break;
			case 5:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_FIVE);
				break;
			case 10:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_TEN);
				break;
			case 15:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_FIFTEEN);
				break;
				
			case 20:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_TWETY);
				break;
				
			case 25:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_TWETY_FIVE);
				break;
				
			case 30:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_THIRTY);
				break;
			case 45:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_FORTY_FIVE);
				break;
			case 60:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_ONE_HOUR);
				break;
				
			case 120:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_TWO_HOURS);
				break;
			case 180:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_THREE_HOURS);
				break;
			case 720:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_TWELVE_HOURS);
				break;
			case 1440:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_TWETY_FOUR);
				break;
			case 2880:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_TWO_DAYS);
				break;
				
			case 10080:
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_REMINDE_ONE_WEEK);
				break;
			default:
				break;
			}
	...
	private Handler handler = new Handler(Looper.getMainLooper()){
		...
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			if(mVoiceAddEventLayout.getVisibility()==0){
			LayoutParams toolbarLayoutParams = (LayoutParams) mVoiceAddEventLayout
            .getLayoutParams();
			switch (msg.what) {
			case SOFTWEARINPUTOUT:
				Log.i("jiating","inputout.....mVoiceAddEventButton="+mVoiceAddEventLayout.getHeight());
				toolbarHeight = mVoiceAddEventLayout.getHeight();
				toolbarLayoutParams.height =0;
				break;
			case SOFTWEARINPUTHIDE:
				Log.i("jiating","inpuIn.....mVoiceAddEventButton="+mVoiceAddEventLayout.getHeight());
				toolbarLayoutParams.height = toolbarHeight;
				break;
			...
			mVoiceAddEventLayout.setLayoutParams(toolbarLayoutParams);
	        Log.i("jiating","input.....mVoiceAddEventButton1111111111="+mVoiceAddEventLayout.getHeight());
//					mVoiceAddEventButton.setVisibility(View.GONE);
	        mVoiceAddEventLayout.invalidate();
			}
		}
	};
	...
	public GNEditEventView(Activity activity, View view, EditDoneRunnable done) {
	    ///M: #extension# @{
...
//		mEditEvent=view.findViewById(R.id.edit_event);
        mVoiceAddEventButton=(RelativeLayout)view.findViewById(R.id.gn_voice_add_activity_view);
        mVoiceAddEventLayout=(RelativeLayout)view.findViewById(R.id.gn_voice_add_activity_layout);
      
        if(EditEventFragment.checkApkExist(mActivity, EditEventFragment.VOICE_PACKAGE_NAME)){
        	mVoiceAddEventLayout.setVisibility(View.VISIBLE);
        	  mVoiceAddEventButton.setOnClickListener(this);
        }else{
        	mVoiceAddEventLayout.setVisibility(View.GONE);
        }
    
        mEditEventLinearLayout=(GNEditEventRelativelayout)view.findViewById(R.id.edit_event);
        mEditEventLinearLayout.setOnResizeListener(new OnResizeListener() {
			
			@Override
			public void OnResize(int w, int h, int oldw, int oldh) {
			    // Gionee <jiangxiao> <2013-06-22> add for CR00829027 begin
				if(mFirstTimeSizeChanged) {
					mFirstTimeSizeChanged = false;
					return;
				}
				// TODO Auto-generated method stub
				if(oldh > h /*&& oldh - h > 200*/) {
				    handler.sendEmptyMessage(SOFTWEARINPUTOUT);
				}else if(oldh < h /*&& h - oldh < 700*/){
					Log.i("jiating","inputhide.....toolbarHeight="+mVoiceAddEventButton.getHeight());
 
					handler.sendEmptyMessage(SOFTWEARINPUTHIDE);
				}
				// Gionee <jiangxiao> <2013-06-22> add for CR00829027 end
			}
		});
		// cache all the widgets
		mCalendarsSpinner = (AuroraSpinner) view.findViewById(R.id.calendars_spinner);
		mCalendarsSpinner.setPrompt(activity.getResources().getString(R.string.edit_event_calendar_label));
		mCalendarsSpinner.setPadding(0,0,0,0);
		mTitleTextView = (AuroraEditText) view.findViewById(R.id.title);
		descriptionEditText=(AuroraEditText) view.findViewById(R.id.description);
...
//		mEndDateButton.setPadding(0,0,0,0);
		mStartDataText=(TextView)view.findViewById(R.id.start_date);
		mStartDataView=view.findViewById(R.id.from_row);
		mEndDataText=(TextView)view.findViewById(R.id.end_date);
		mEndDataView=view.findViewById(R.id.to_row);
		//Gionee <jiating><2013-07-03> modify for CR00830388 end
		mRepeatsSpinner = (AuroraSpinner) view.findViewById(R.id.repeats);
		mRepeatsSpinner.setPrompt(activity.getResources().getString(R.string.accessibility_repeats));
        	mRepeatsSpinner.setOnTouchListener(new OnTouchListener() {
		...
		editEventView2=(LinearLayout)view.findViewById(R.id.gn_edit_event_view);
		//Gionee <jiating><2013-07-05> modify for CR00825949 begin
		mCalendarAccountAll=(LinearLayout)view.findViewById(R.id.calendar_select_account_all);
		mCalendarAccountName=(TextView)view.findViewById(R.id.calendar_select_account_name);
		//Gionee <jiating><2013-07-05> modify for CR00825949 end
		moreButton=view.findViewById(R.id.edit_event_more_button);
...
//		});
		...
		extension.setLengthInputFilter((AuroraEditText) mTitleTextView, mActivity,
				TITLE_MAX_LENGTH);
	...
	public void setModel(CalendarEventModel model) {
		...
		View.OnClickListener addReminderOnClickListener = new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_CLICK_ADD_REMINDER);
				addReminder();
			}
		};
 ...
 //Gionee <jiating><2013-06-04> modify for CR00000000 begin
		if(model.mTitle!=null || model.mDescription != null){
			 EditEventFragment.setSaveButtonVisible(true);
		}else{
			 EditEventFragment.setSaveButtonVisible(false);
		}
//Gionee <jiating><2013-06-04> modify for CR00000000 end		
...
//                mView.findViewById(R.id.calendar_group).setBackgroundColor(displayColor);
	...
	public void setCalendarsCursor(Cursor cursor, boolean userVisible) {
		...
		if (cursor == null || cursor.getCount() == 0) {
			...
			AuroraAlertDialog.Builder builder = new AuroraAlertDialog.Builder(mActivity);
...
//					mOriginalSpinnerPadding[3]);
	...
	private void updateAttendees(HashMap<String, Attendee> attendeesList) {
		Log.i("jiating","GNEditEventView...updateAttendees");
	...
	private void updateRemindersVisibility(int numReminders) {
		   Log.i("jiating","updateRemindersVisibility.....numReminders="+numReminders+"maxReminders...="+mModel.mCalendarMaxReminders);
...
//			Log.i("jiating","updateRemindersVisibility.......if22");
//			reminderAddButton.setVisibility(View.GONE);
//		} else if(numReminders==0){
//			Log.i("jiating","updateRemindersVisibility.......else if22");
//			
//			reminderAddButton.setVisibility(View.VISIBLE);
//			reminderAddButton.setBackgroundResource(R.drawable.gn_all_in_one_sliding_content_single_on_off);
//            if(mIsClickMoreButton){
//            	reminderAddButton.setAnimation(GNAnimationutils.editEventAddReminderClickMoreAnumation(mActivity));
//			 }
//		}else{
//			Log.i("jiating","updateRemindersVisibility.......else 22");
//			reminderAddButton.setVisibility(View.VISIBLE);
//			reminderAddButton.setBackgroundResource(R.drawable.gn_all_in_one_sliding_content_bottom_on_off);
//		    if(mIsClickMoreButton){
//	            	reminderAddButton.setAnimation(GNAnimationutils.editEventAddReminderClickMoreAnumation(mActivity));
//				 }
//		}
	...
	private void setDate(TextView view, long millis) {
		...
		Log.i("jiating","GNEditEventView......setDate...dateString="+dateString);
		if(dateString.contains(mActivity.getString(R.string.gn_agenda_time_noon))){
			
			dateString=dateString.replace(mActivity.getString(R.string.gn_agenda_time_noon), mActivity.getString(R.string.gn_agenda_time_noon_replace));
			Log.i("jiating","GNEditEventView..contains..dateString="+dateString);
		}
...
//			mCalendarSelectorGroup.setBackgroundColor(displayColor);
		...
		Log.i("GNEditEvent","GNEditEvent-maxRemindersColumn == " + maxRemindersColumn);
...
//		setDate(mEndDateButton, mEndTime.toMillis(false /* use isDst */));
		setDate(mStartDataText, mStartTime.toMillis(false /* use isDst */));
		setDate(mEndDataText, mEndTime.toMillis(false /* use isDst */));
		//Gionee <jiating><2013-07-03> modify for CR00830388 end
	}
...
//			updateHomeTime();
	...
	public  void setEditTime(final Context context, long timeMillis,long minTimeMillis,
			View view) {
		...
		GNDateTimeDialog alarmDialog = null;
		DateTimeListener  dateTimeListener=new DateTimeListener(view); 
		// white theme
		alarmDialog = new GNDateTimeDialog(context,
	...
	private void setMoreContentViewVisible(boolean mIsClickMoreButton){
		//Gionee <jiating><2013-07-05> modify for CR00825949 begin
		if(mIsClickMoreButton){
			editEventView2.setVisibility(View.VISIBLE);
			mCalendarAccountAll.setVisibility(View.VISIBLE);
			mCalendarAccountName.setVisibility(View.VISIBLE);
			editEventView2.setAnimation(GNAnimationutils.editEventClickMoreAnumation(mActivity));
			moreButton.setVisibility(View.GONE);
		}else{
			mCalendarAccountName.setVisibility(View.GONE);
			editEventView2.setVisibility(View.GONE);
			moreButton.setVisibility(View.VISIBLE);
			mCalendarAccountAll.setVisibility(View.INVISIBLE);
			//Gionee <jiating><2013-07-05> modify for CR00825949 end
		}
	}
...
//		setDate(mStartDateButton, startMillis);
	...
	public  boolean isClickVoiceButton() {
		return isClickVoiceButton;
	}

	public  void setClickVoiceButton(boolean isClickVoice) {
		isClickVoiceButton = isClickVoice;
	}
	 ...
	 private TextWatcher titleTextWatcher = new TextWatcher() {  
	        ...
	        public void onTextChanged(CharSequence s, int start, int before,     
	            ...
	            Log.d("jiating","onTextChanged--------------->"+s+"....s.size="+temp1.length()+">>>>count="+count+">>>>start="+start+">>>>>before="+before);    
	          if(temp1.length()>0){
	        	  EditEventFragment.setSaveButtonVisible(true);
	          }else if(temp1.length()<=0  && mDescriptionTextView.getText().toString().trim().replace("\n", "").replace("\\s","").length()<=0){
	        	  EditEventFragment.setSaveButtonVisible(false);             
	        }
	         }
	    ...
	    private TextWatcher descriptionTextWatcher = new TextWatcher() {  
	        ...
	        public void onTextChanged(CharSequence s, int start, int before,     
	          ...
	          if(temp1.length()>0){
	        	  EditEventFragment.setSaveButtonVisible(true);
	          }else if(temp1.length()<=0  && mTitleTextView.getText().toString().trim().replace("\n", "").replace("\\s","").length()<=0){
	        	  EditEventFragment.setSaveButtonVisible(false);             
	        }
	      }
...
//Gionee <jiating> <2013-04-09> modify for CR00000000 use new edit event view end
