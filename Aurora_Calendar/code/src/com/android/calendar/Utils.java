...
public class Utils {
    ...
    public static final String PC_SYNC_ACCOUNT_NAME = "PC Sync";
    ...
    public static final String PERIOD_SP = "period_sp";
    public static final String PERIOD_TIME = "period_time";
    public static final String PERIOD_CYCLE = "period_cycle";
    public static final String PERIOD_CLEAR_FLAG = "period_clear_flag";
    //0 first 1 no first
    public static final String IS_FIRST_KEY = "is_first";
    public static final String IS_SECOND_KEY = "is_second";

    // 0 not get 1 men 2 women
    private static int WOMEN_ENVIRONMENT_FLAG = 0;

    public static final String CALENDAR_DISPLAY_MODE = "calendar_display_mode";
    public static final int MODE_NORMAL = 0;
    public static final int MODE_PERIOD = 1;

    public static boolean displayPeriod(Context context) {
        SharedPreferences sp = context.getSharedPreferences(PERIOD_SP, Context.MODE_PRIVATE);
        return sp.getInt(CALENDAR_DISPLAY_MODE, MODE_NORMAL) == MODE_PERIOD;
    }

    public static int getPeriodSharePreference(Context context, String key, int defaultValue) {
        SharedPreferences sp = context.getSharedPreferences(PERIOD_SP, Context.MODE_PRIVATE);
        return sp.getInt(key, defaultValue);
    }

    public static void setPeriodSharePreference(Context context, String key, int value) {
        SharedPreferences sp = context.getSharedPreferences(PERIOD_SP, Context.MODE_PRIVATE);
...
//Gionee <jiating> <2013-04-11>  modify for CR00000000 other apckage use begin
    public static final String MACHINE_GENERATED_ADDRESS = "calendar.google.com";
//Gionee <jiating> <2013-04-11>  modify for CR00000000 other apckage use end
    ...
    public static boolean isIndiaVersion() {
        if (VERSION_FLAG == 0) {
            if (SystemProperties.get("ro.iuni.country.option", "unknown").equals("INDIA")) {
                VERSION_FLAG = 2;
            } else {
                VERSION_FLAG = 1;
            }
        ...
        if (VERSION_FLAG == 2 && !isWomenEnvironment()) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isWomenEnvironment() {
        if (WOMEN_ENVIRONMENT_FLAG == 0) {
            String womenProp = SystemProperties.get("persist.sys.aurora.overlay", "xxx");
            Log.e("JOY", womenProp);
            if (!womenProp.equals("xxx")) {
                File womenDir = new File(womenProp);
                if (womenDir.isDirectory()) {
                    WOMEN_ENVIRONMENT_FLAG = 2;
                } else {
                    WOMEN_ENVIRONMENT_FLAG = 1;
                }
            } else {
                WOMEN_ENVIRONMENT_FLAG = 1;
            }
        }
        if (WOMEN_ENVIRONMENT_FLAG == 1)
            return false;
        else
            return true;
    }
    ...
    public static int calStrLength(String strName) {
        int length = 0;
        char[] ch = strName.toCharArray();
        for (int i = 0; i < ch.length; i++) {
            char c = ch[i];
    ...
    public static int getViewTypeFromIntentAndSharedPref(AuroraActivity activity) {
...
//Gionee <jiating><2013-06-04>  modify for CR00822795 begin
        // Default to the last view
        return prefs.getInt(
                GeneralPreferences.KEY_START_VIEW, ViewType.MONTH);*/
        return ViewType.MONTH;
    }
  //Gionee <jiating><2013-06-04>  modify for CR00822795 end
    /**
    ...
    public static int getFirstDayOfWeek(Context context) {
        ...
        if (GeneralPreferences.WEEK_START_DEFAULT.equals(pref)) {
            ...
            String startDayStr = context.getResources().getString(R.string.preferences_week_start_day_default);
     ...
     */
    public static void setUpSearchView(SearchView view, AuroraActivity act) {
    ...
    public static int getDefaultReminderMinutes(Context context) {
        String minutes = getSharedPreference(context, "preferences_default_reminder", "10");
