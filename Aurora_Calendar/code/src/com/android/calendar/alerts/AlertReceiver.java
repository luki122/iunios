...
public class AlertReceiver extends BroadcastReceiver {
    ...
    public static NotificationWrapper makeBirthdayReminderNotification(Context context,
        ...
        // Create an intent triggered by clicking on the status icon, that dismisses the
        // notification and shows the birthday info.
        Intent clickIntent = new Intent();
        clickIntent.setClass(context, DismissAlarmsService.class);
        clickIntent.setAction("com.android.calendar.CLICK");
        ...
        clickIntent.putExtra(AlertUtils.SHOW_BIRTHDAY_INFO_KEY, true);
        PendingIntent pendingClickIntent = PendingIntent.getService(context, 0, clickIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);

        // Create an intent triggered by dismissing the digest notification that clears all
        // expired birthday reminders.
        Intent deleteIntent = new Intent();
        deleteIntent.setClass(context, DismissAlarmsService.class);
        deleteIntent.setAction(DELETE_ALL_ACTION);
        deleteIntent.putExtra(AlertUtils.EVENT_IDS_KEY, eventIds);
        ...
        notificationBuilder.setContentText(res.getString(R.string.aurora_birthday_wish));
        notificationBuilder.setSmallIcon(R.drawable.aurora_birthday_reminder_stat_notify);
        notificationBuilder.setContentIntent(pendingClickIntent);
        notificationBuilder.setDeleteIntent(pendingDeleteIntent);
        ...
        if (Utils.isJellybeanOrLater()) {
            ...
            RemoteViews contentView = new RemoteViews(context.getPackageName(),
                    R.layout.notification);
            contentView.setImageViewResource(R.id.image, R.drawable.aurora_birthday_reminder_stat_notify);
            contentView.setTextViewText(R.id.title, digestTitle);
            contentView.setTextViewText(R.id.text, res.getString(R.string.aurora_birthday_wish));
            contentView.setViewVisibility(R.id.time, View.VISIBLE);
            contentView.setViewVisibility(R.id.email_button, View.GONE);
            contentView.setViewVisibility(R.id.snooze_button, View.GONE);
            contentView.setViewVisibility(R.id.end_padding, View.VISIBLE);
