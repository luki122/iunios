...
public class AlertAdapter extends ResourceCursorAdapter {
    ...
    public static void updateView(Context context, View view, String eventName, String location,
        ...
        if (mFirstTime) {
            ...
            //Gionee <pengwei>  <2013-04-12> modify for alert begin
            mTitleColor = res.getColor(R.color.gn_text_alert);
            mOtherColor = res.getColor(R.color.gn_text_alert);
            //Gionee <pengwei>  <2013-04-12> modify for alert end
            mFirstTime = false;
        }
      ...
      //Gionee <jiating><2013-07-01> modify for CR00819533 begin
		Log.i("jiating","AlartAdapter....whenString="+when);
		if(when.contains(context.getString(R.string.gn_agenda_time_noon))){
			
			when=when.replace(context.getString(R.string.gn_agenda_time_noon), context.getString(R.string.gn_agenda_time_noon_replace));
		}
