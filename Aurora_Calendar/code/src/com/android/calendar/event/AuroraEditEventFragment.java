...
public class AuroraEditEventFragment extends Fragment implements EventHandler {
    ...
    public static final String REPEAT_CHOOSE_ACTION = "com.aurora.calendar.action.REPEAT_CHOOSE";
//	public static final int GN_RECOGNIZE_ACTION_CODE = 1;
	public static final int REPEAT_CHOOSE_ACTION_CODE = 120;
    ...
    private class QueryHandler extends AsyncQueryHandler {
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            final Activity activity = AuroraEditEventFragment.this.getActivity();
    ...
    public AuroraEditEventFragment() {
        this(null, false, null);
    }

    public AuroraEditEventFragment(EventInfo event, boolean readOnly, Intent intent) {
        mEvent = event;
...
//        setHasOptionsMenu(true);
    ...
    public void onAttach(Activity activity) {
        ...
        if (mIntent != null) {
            mCalendarId = mIntent.getLongExtra(Events.CALENDAR_ID, -1);
        }
    }
...
//        mActivity.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
        ...
        if (mIsReadOnly) {
            ...
            view = inflater.inflate(R.layout.aurora_edit_event_fragment, null);
        }
        mView = new AuroraEditEventView(mActivity, view, mOnDone);
        startQuery();
...
//        	if(AuroraEditEventView.mIsClickMoreButton){
//				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_SHAVE_MORE);
//        	}else{
				Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_SHAVE_NO_MORE);
//        	}
        	//Gionee <jiating> <2013-05-22> modify for CR00000000 statistical begin
        	...
        	//Gionee <jiating> <2013-05-22> modify for CR00000000 statistical begin
			Statistics.onEvent(mActivity, Statistics.EDIT_EVENT_CANCLE);
...
//			if (isEmptyNewEvent() && mView.isDescriptionEmpty()) {
//				doSaveAction();
//			} else {
				Toast.makeText(mActivity,
						R.string.aurora_event_edit_save_no_title,
						Toast.LENGTH_SHORT).show();
				
				if (mView.getTitleEditText() != null){
					mView.getTitleEditText().requestFocus();
					mInputMethodManager.showSoftInput(mView.getTitleEditText(), InputMethodManager.HIDE_NOT_ALWAYS);
				}
//			}
		} else {
			doSaveAction();
...
//    	}
	...
	public void doCancelAction() {
		AuroraAlertDialog dialog = new AuroraAlertDialog.Builder(getActivity())
				//.setTitle(R.string.aurora_event_edit_cancel)
				.setTitle(R.string.aurora_event_edit_cancel_info)
				.setPositiveButton(R.string.aurora_event_edit_cancel,
		...
		if (mOriginalModel == null) {
			if (mView.isTitleEmpty() && mView.isDescriptionEmpty()) {
				cancelSave();
			} else {
				dialog.show();
			}

		} else if (mView.prepareForSave() && EditEventHelper.hasChanged(mOriginalModel, mModel)) {
			dialog.show();
		} else {
			cancelSave();
		}
    ...
    protected void displayEditWhichDialog() {
        if (mModification == Utils.MODIFY_UNINITIALIZED) {
            ...
            mModifyDialog = new AuroraAlertDialog.Builder(mActivity).setTitle(R.string.edit_event_label)
            ...
            mModifyDialog.setOnCancelListener(new OnCancelListener() {
                ...
                public void onCancel(DialogInterface dialog) {
                    Activity a = AuroraEditEventFragment.this.getActivity();
                    if (a != null) {
                        a.finish();
                    }
                }
    ...
    class Done implements EditEventHelper.EditDoneRunnable {
        ...
        public void run() {
            ...
            if ((mCode & Utils.DONE_SAVE) != 0 && mModel != null
                    ...
                    && mView.prepareForSave() && setCalendarId()
                    && !mModel.isTitleEmpty()
//                    && !isEmptyNewEvent()
                ...
                Log.i("likai", "mModel.mCalendarId = " + mModel.mCalendarId);
                Activity editEventActivity = AuroraEditEventFragment.this.getActivity();
                if (editEventActivity != null) {
                    Intent intent = new Intent();
                    intent.putExtra(Events.TITLE, mModel.mTitle);
                    intent.putExtra(EXTRA_EVENT_BEGIN_TIME, mModel.mStart);

                    editEventActivity.setResult(Activity.RESULT_OK, intent);
                }
    ...
    private boolean setCalendarId() {
        if (mModel != null && mCalendarId != -1) {
            mModel.mCalendarId = mCalendarId;
        }
        Log.i("likai", "mCalendarId = " + mCalendarId);
        return true;
    }
...
///TODO: extract these code out
    ...
    public AuroraEditEventView getAuroraEditEventView(){
    	return mView;
    }
    ...
    AuroraEditEventView getEditEventView() {
        return mView;
    }
    ///@}
