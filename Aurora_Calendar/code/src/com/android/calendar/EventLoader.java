...
public class EventLoader {
    ...
    private static class LoadEventDaysRequest implements LoadRequest {
        ...
        @Override
        public synchronized void processRequest(EventLoader eventLoader)
        {
    ...
    private static class LoadEventsRequest implements LoadRequest {
        ...
        public void processRequest(EventLoader eventLoader) {
        	Log.d("GNMonthView", "to run processRequest() in LoadEventsRequest");
            ...
            while (true) {
                try {
                    ...
                    if (request instanceof ShutdownRequest) {
                    	Log.d("GNMonthView", "detect ShutdownRequest");
                        return;
                    }
                    Log.d("GNMonthView", "to process a LoadRequest");
    ...
    public void startBackgroundThread() {
    	Log.d("GNMonthView", "startBackgroundThread() has been invoked");
    ...
    public void stopBackgroundThread() {
    	Log.d("GNMonthView", "stopBackgroundThread() has been invoked");
     ...
     * via the uiCallback.
     */
    public void loadAllEventsInBackground(final ArrayList<Event> events,
            int startDay,int endDay,String query, final Runnable successCallback, final Runnable cancelCallback) {
        ...
        // Send the load request to the background thread
        LoadAllEventsRequest request = new LoadAllEventsRequest(id, startDay, endDay,
                events,query, successCallback, cancelCallback);
	...
	private static class LoadAllEventsRequest implements LoadRequest {
        ...
        public LoadAllEventsRequest(int id, int startDay, int endDay,ArrayList<Event> events,String query,
                final Runnable successCallback, final Runnable cancelCallback) {
            this.id = id;
            this.startDay = startDay;
            this.endDay=endDay;
        ...
        public void processRequest(EventLoader eventLoader) {
        	Log.d("GNMonthView", "to run processRequest() in LoadEventsRequest");
            Event.loadAllEvents(eventLoader.mContext, events, startDay,
                    endDay, query,id, eventLoader.mSequenceNumber);
            Log.i("jiating","LoadAllEventsRequest..id="+id+"eventLoader.mSequenceNumber.get()="+eventLoader.mSequenceNumber.get());
            // Check if we are still the most recent request.
            if (id == eventLoader.mSequenceNumber.get()) {
                eventLoader.mHandler.post(successCallback);
            } else {
            	 //Gionee <jiating><2013-06-26> modify for CR00829885 begin
            	Log.i("jiating","LoadAllEventsRequest..else");
            	return;
//                eventLoader.mHandler.post(cancelCallback);
        ...
        public void skipRequest(EventLoader eventLoader) {
        	Log.i("jiating","LoadAllEventsRequest..skipRequest");
            eventLoader.mHandler.post(cancelCallback);
        }
    }
