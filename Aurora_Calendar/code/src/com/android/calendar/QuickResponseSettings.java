 ...
 *
 */
public class QuickResponseSettings extends AuroraPreferenceFragment implements OnPreferenceChangeListener {
    private static final String TAG = "QuickResponseSettings";

    AuroraEditTextPreference[] mEditTextPrefs;
    String[] mResponses;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        AuroraPreferenceScreen ps = getPreferenceManager().createPreferenceScreen(getActivity());
        ps.setTitle(R.string.quick_response_settings_title);

        mResponses = Utils.getQuickResponses(getActivity());

        if (mResponses != null) {
            mEditTextPrefs = new AuroraEditTextPreference[mResponses.length];

            Arrays.sort(mResponses);
            int i = 0;
            for (String response : mResponses) {
                AuroraEditTextPreference et = new AuroraEditTextPreference(getActivity());
