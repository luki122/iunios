...
import com.gionee.widget.CalendarWidget.WeekNumberService;

import java.util.Calendar;

/**
 * This class is implements {@link RemoteViewsService.RemoteViewsFactory}, and it
 * provide the views for {@link CalendarWidget.WeekNumberService}. It will provide the
 * weeks' number of the year according to the position {@link #getViewAt(int)}.
 */
public class WeekNumberViews implements RemoteViewsService.RemoteViewsFactory {
    ...
    public static synchronized WeekNumberViews getOrCreateViews(Context context, int[] appWidgetIds) {
        if (sInstance != null) {
            return sInstance;
        } else {
    ...
    private void updateWeekNumber() {
    	if (Utility.DEBUG) {
			Log.i(TAG, "updateWeekNumber.");
    	}
        if (mViews == null) return;

        initWeekNumber();
        mAppWidgetManager.notifyAppWidgetViewDataChanged(mAppWidgetIds, R.id.week_number);
    ...
    public RemoteViews getViewAt(int position) {
        ...
        RemoteViews views = new RemoteViews(mContext.getPackageName(), R.layout.gn_widget_week_number);
        if (position < mWeekNumbers.length) {
            views.setTextViewText(R.id.week, String.valueOf(mWeekNumbers[position]));

            if (Utility.DEBUG) {
                Log.d(TAG, "get the view at: " + position + ", the week number is: " + mWeekNumbers[position]);
            }
        } else {
