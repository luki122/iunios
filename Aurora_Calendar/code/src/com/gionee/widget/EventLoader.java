...
public class EventLoader extends CursorLoader {
    ...
    private static final String[] COLS = new String[] {
        "monthDay", "eventCount", "firstEventTitle"
    };
    ...
    /**
     * We will use the {@link EventModel} to load the events info, and build the {@link EventCursor}
     * with the {@link #COLS}.
     */
    @Override
    ...
    /**
     * Cursor data specifically for used by the {@link DateViews}. Contains a cursor of
     * event in addition to the event count and first event title. For this, we could
     * easily get the values we want.
     */
    ...
    /**
     * We used this to load the events info.
     * We will get the info from the {@link CalendarContract.Instances}, and sort the result
     * by {@link CalendarContract.Instances#BEGIN} ascending.
     */
    static class EventModel {
        private Context mContext;
        private EventLoader mLoader;

        private static final String SORT_BY_BEGIN_ASC = Instances.BEGIN + " ASC";
        ...
        /**
         * Return the events map, the key is the date, and the value is the events' list.
         *
         * @return the events map
         */
        public synchronized HashMap<String, ArrayList<String>> getEventsMap() {
            ...
            Cursor cursor = mContext.getContentResolver().query(mUri, PROJECTION, null, null, SORT_BY_BEGIN_ASC);
            try {
                if (cursor == null || cursor.getCount() < 1) {
                    if (Utility.DEBUG) {
                        Log.w(TAG, "get the event cursor is null or it's count is 0");
                    }
                    return eventsMap;
                }
        ...
        private Uri buildUri() {
            ...
            // There is no need to reduce 1 on startDay.
            startTime.set(startDay, mLoader.mMonth, mLoader.mYear);
            mStartMillis = startTime.toMillis(true);

            Time endTime =  new Time();
            endTime.set(endDay + 1, mLoader.mMonth, mLoader.mYear);
            // Get the correct end time of the mouth.
            // For instance, current month is Oct 2012, then the endTime will
            // be 1351699200000, which is Nov 1 2012 00:00:00, we need to minus 1
            // millis so the mEndMillis will be 1351699199999, which is just the
            // last millis of the 31 Oct.
            mEndMillis = endTime.toMillis(true) - 1;

            Uri.Builder builder = Instances.CONTENT_URI.buildUpon();
            ContentUris.appendId(builder, mStartMillis);
