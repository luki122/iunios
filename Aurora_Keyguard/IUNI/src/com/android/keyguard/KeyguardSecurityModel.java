...
public class KeyguardSecurityModel {
    ...
    public enum SecurityMode {
        ...
        Direct // open screen directly, without any view shown
    }
     ...
     * this method check if SIM card needs our password.
     */
    public boolean needSimPassword() {
        ...
        if (numPhones > 0) {
			for (int i = 0; i < numPhones; i++) {
	            ...
	            // than these two.
	            Log.d("vulcan-direct",String.format("needSimPassword: simState(%d) = %s" ,i, simState));
    ...
    /**
     * vulcan created this method in 2014-9-12
     * if all the sim cards are ready, this method return true,
     * or it will return false
     * @return
     */
    public boolean SimCardsAreReady() {
        KeyguardUpdateMonitor updateMonitor = KeyguardUpdateMonitor.getInstance(mContext);
        IccCardConstants.State simState  = updateMonitor.getSimState();
        ...
        if (numPhones > 0) {
			for (int i = 0; i < numPhones; i++) {
	            ...
	            // than these two.
	            Log.d("vulcan-direct",String.format("needSimPassword: simState(%d) = %s" ,i, simState));
	            if (simState ==  IccCardConstants.State.NOT_READY
	               || simState ==  IccCardConstants.State.UNKNOWN) {
	                return false;
	            }
	        }
    ...
    public SecurityMode getSecurityMode() {
    	//Log.d("vulcan-direct",String.format("getSecurityMode: stack = %s", Trace.StackToString(new Throwable())));
    	boolean simNeedPassword = needSimPassword();
    	
    	Log.d("vulcan-direct","getSecurityMode: simNeedPassword = " + simNeedPassword);
        ...
        if (simState == IccCardConstants.State.PIN_REQUIRED) {
            ...
            Log.d("vulcan-direct","getSecurityMode: security(new) = " + security);
            switch (security) {
