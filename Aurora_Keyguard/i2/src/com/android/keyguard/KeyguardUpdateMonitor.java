...
public class KeyguardUpdateMonitor {
	...
	// Aurora liugj 2014-06-18 modified for MusicLockScreen start
    protected static final int MSG_MUSIC_PLAYSTATE_CHANGED = 321;
	// Aurora liugj 2014-06-18 modified for MusicLockScreen end
	 
	// Aurora liugj 2014-06-18 modified for MusicLockScreen start
    public static final String INTENT_MUSIC_PLAYSTATE_CHANGED = "com.android.auroramusic.playstatechanged"; // modify by tangjie 2015-3-26 for bug #12200
    // Aurora liugj 2014-06-18 modified for MusicLockScreen end
    ...
    public static final boolean
            sIsMultiSimEnabled = false/*GnTelephonyManager.isMultiSimEnabled()*/;
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_MUSIC_PLAYSTATE_CHANGED:
                	handleMusicPlayState((Boolean)msg.obj);
                	break;
    ...
    private DisplayClientState mDisplayClientState = new DisplayClientState();
	 ...
	 // Aurora liugj 2014-06-18 deleted for MusicLockScreen start
    /**
    ...
    /*private final IRemoteControlDisplay.Stub mRemoteControlDisplay =
	 ...
	 // Aurora liugj 2014-06-18 deleted for MusicLockScreen end
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
			...
			// Aurora liugj 2014-06-27 added for MusicLockScreen start
            }else if (action.equals(INTENT_MUSIC_PLAYSTATE_CHANGED)) {
            	boolean isPlaying = intent.getBooleanExtra("playing", false);
            	Log.d("liugj", "=====INTENT_MUSIC_PLAYSTATE_CHANGED====="+isPlaying);
            	Message msg = mHandler.obtainMessage(MSG_MUSIC_PLAYSTATE_CHANGED, isPlaying);
                mHandler.sendMessage(msg);
			 // Aurora liugj 2014-06-27 added for MusicLockScreen end
			}
        }
    };
    ...
    private static class SimArgs {
        ...
        static SimArgs fromIntent(Intent intent) {
		    ...
		    Log.v(TAG, "updateSimState for subscription :" + sub);
...
/*			if (simState != IccCardConstants.State.UNKNOWN && simState != mMSimState[sub]) {
            ...
            Log.v(TAG, "updateSimState simState = " + state);	
...
*/            
    ...
    /* package */ public static class BatteryStatus {
        ...
        public void setDone(boolean onDone) {
            this.isDone = onDone;
        }
	...
	// Aurora liugj 2014-06-18 added for MusicLockScreen start
    private void handleMusicPlayState(boolean isPlaying) {
		StatusView.isPlayingMusic = isPlaying;
		for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onMusicPlaybackStateChanged(isPlaying);
            }
        }
	}
	// Aurora liugj 2014-06-18 added for MusicLockScreen end
    ...
    private KeyguardUpdateMonitor(Context context) {
        ...
        // GnTelephonyManager.getPhoneCount() returns '1' for single SIM
		...
		// Aurora liugj 2014-06-27 added for MusicLockScreen start
        filter.addAction(INTENT_MUSIC_PLAYSTATE_CHANGED);
		// Aurora liugj 2014-06-27 added for MusicLockScreen end
    ...
    protected void handleBootCompleted() {
        ...
        //mAudioManager.registerRemoteControlDisplay(mRemoteControlDisplay);
		 // Aurora liugj 2014-06-27 modified for MusicLockScreen end
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
                cb.onBootCompleted();
    ...
    private void handleBatteryUpdate(BatteryStatus status) {
        ...
        if (status.level == 100 && status.level > mBatteryStatus.level) {
        	status.setDone(true);
       	 	PowerManager pm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE); 
       	 	if (!pm.isScreenOn()) {
       	 		WakeLock wl = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "show keyguard");  
       	 		wl.acquire();
       	 		wl.release();
       	 	}
		}
    ...
    private void sendUpdates(KeyguardUpdateMonitorCallback callback) {
        ...
        int subscription = 0/*GnTelephonyManager.getDefaultSubscription(mContext)*/;
        if (sIsMultiSimEnabled) {
    ...
    public IccCardConstants.State getSimState() {
        return getSimState(0/*GnTelephonyManager.getDefaultSubscription(mContext)*/);
    }

    public IccCardConstants.State getSimState(int subscription) {
		// Aurora liugj 2014-06-17 modified for bug-5217 start
    	if (0 <= subscription && subscription < mSimState.length) {
    		return mSimState[subscription];
		}else {
			return IccCardConstants.State.UNKNOWN;
		}
		// Aurora liugj 2014-06-17 modified for bug-5217 end
    }
    ...
    public void reportSimUnlocked() {
        reportSimUnlocked(0/*GnTelephonyManager.getDefaultSubscription(mContext)*/);
    }
    ...
    public CharSequence getTelephonyPlmn() {
        return getTelephonyPlmn(0/*GnTelephonyManager.getDefaultSubscription(mContext)*/);
    }
    ...
    public CharSequence getTelephonySpn() {
        return getTelephonySpn(0/*GnTelephonyManager.getDefaultSubscription(mContext)*/);
    }
