...
import com.android.keyguard.view.AuroraAnimationImageView;
 ...
 */
public class EmergencyButton extends AuroraAnimationImageView {
    ...
    private void updateEmergencyCallButton(State simState, int phoneState) {
        ...
        updateEmergencyCallButtonState(this, phoneState, enabled,
                LockScreenUtils.USE_UPPER_CASE, false);
    }

    
    public void updateEmergencyCallButtonState(ImageView button, int  phoneState, boolean shown,
            boolean upperCase, boolean showIcon) {
        if (mLockPatternUtils.isEmergencyCallCapable() && shown) {
        ...
        if ("CN".equals(mContext.getResources().getConfiguration().locale.getCountry())) {
            button.setImageResource(R.drawable.lockscreen_emergency_button);
        } else {
            button.setImageResource(R.drawable.lockscreen_emergency_eng_button);
        }
    }
}
