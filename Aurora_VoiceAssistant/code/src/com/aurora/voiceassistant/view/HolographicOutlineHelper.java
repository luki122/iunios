...
public class HolographicOutlineHelper {
    ...
    private final Paint mShowowPaint = new Paint();
...
//        final float scale = LauncherApplication.getScreenDensity();
        final float scale = MainActivity.getScreenDensity();

        MIN_OUTER_BLUR_RADIUS = (int) (scale * 1.0f);
        MAX_OUTER_BLUR_RADIUS = (int) (scale * 6.0f);
        ...
        sMediumOuterBlurMaskFilter = new BlurMaskFilter(scale * 2.0f, BlurMaskFilter.Blur.INNER);
    ...
    HolographicOutlineHelper() {
        ...
        mShowowPaint.setMaskFilter(sMediumOuterBlurMaskFilter);
        mShowowPaint.setFilterBitmap(true);
        mShowowPaint.setAntiAlias(true);
    }
    ...
    void applyExpensiveOutlineWithBlur(Bitmap srcDst, Canvas srcDstCanvas, int color,
            int outlineColor, int thickness,boolean b) {
        applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color, outlineColor, true,
                thickness,b);
    }
    
    void applyExpensiveOutlineWithBlur(Bitmap srcDst, Canvas srcDstCanvas, int color,
            int outlineColor, boolean clipAlpha, int thickness,boolean b) {
    	if(b){
    		 if (clipAlpha) {
    	            ...
    	            for (int i = 0; i < srcBuffer.length; i++) {
    	            	srcBuffer[i] = srcBuffer[i]&0x5FFFFFFF;
    	            }
    	            srcDst.setPixels(srcBuffer,
    	                    0, srcDst.getWidth(), 0, 0, srcDst.getWidth(), srcDst.getHeight());
    	        }
    	        int[] offsetXY = new int[2];
    		    Bitmap shadowImage = srcDst.extractAlpha(mShowowPaint, offsetXY);
    		    Bitmap shadow = shadowImage.copy(Bitmap.Config.ARGB_8888, true);
    		    srcDstCanvas.drawBitmap(srcDst, -offsetXY[0], -offsetXY[1], null);
    ...
    void applyExtraThickExpensiveOutlineWithBlur(Bitmap srcDst, Canvas srcDstCanvas, int color,
            int outlineColor) {
        applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color, outlineColor, EXTRA_THICK,false);
    }

    void applyThickExpensiveOutlineWithBlur(Bitmap srcDst, Canvas srcDstCanvas, int color,
            int outlineColor) {
        applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color, outlineColor, THICK,false);
    }
    ...
    void applyMediumExpensiveOutlineWithBlur(Bitmap srcDst, Canvas srcDstCanvas, int color,
            int outlineColor) {
        applyExpensiveOutlineWithBlur(srcDst, srcDstCanvas, color, outlineColor, MEDIUM,true);
    }
