...
public class Event implements Cloneable {
  ...
  //Gionee <jiating> <2013-04-22>  modify for CR00000000  getTimeZone begin
    public String timeZone;
  //Gionee <jiating> <2013-04-22>  modify for CR00000000  getTimeZone end
    ...
    //Gionee <pengwei><20130812> modify for CR00843322 begin
    /**
	...
	public static synchronized void loadEvents(Context context, ArrayList<Event> events, int startDay, int days) {
    	Log.d("GNMonthView", "Event.loadEvents() has been invoked");
...
//        	Log.d("GNMonthView", "to hide declined events? " + hideDeclined);
    ...
    //Gionee <pengwei><20130812> modify for CR00843322 end
    ...
    private static final Cursor instancesQuery(ContentResolver cr, String[] projection,
    	...
    	Log.d("GNMonthView", "Event.instancesQuery() invoked");
    ...
    public static void buildEventsFromCursor(
    	...
    	Log.d("GNMonthView", "Event.buildEventsFromCursor() invoked");
        ...
        if (count == 0) {
        	Log.d("GNMonthView", "no events in Cursor");
            return;
        }
        ...
        while (cEvents.moveToNext()) {
            ...
            events.add(e);
            Log.d("GNMonthView", "add a Event into list: " + e);
        }
    }
  ...
  //Gionee <jiating> <2013-04-22>  modify for CR00000000  loadAllEvents begin
     ...
     */
    private static final Cursor instancesQuery(ContentResolver cr, String[] projection,
            int startDay, int endDay, String query,String selection, String[] selectionArgs, String orderBy) {
        ...
        Uri queryUri= buildQueryUri(startDay,endDay,query);
      
        return cr.query(queryUri, projection, selection, selectionArgs,
                orderBy == null ? DEFAULT_SORT_ORDER : orderBy);
    }
    
    private static Uri buildQueryUri(int start, int end, String searchQuery) {
    	 Uri.Builder builder=null;
        if (searchQuery != null && searchQuery.length()>0) {
        	Log.i("jiating","buildQueryUri.....searchQuery");
        	Uri rootUri = Instances.CONTENT_SEARCH_BY_DAY_URI;
        	builder = rootUri.buildUpon();
            ContentUris.appendId(builder, start);
            ContentUris.appendId(builder, end);
            builder.appendPath(searchQuery);
        }else{
        	Log.i("jiating","buildQueryUri.....searchQuery else");
            builder = Instances.CONTENT_BY_DAY_URI.buildUpon();
            ContentUris.appendId(builder, start);
            ContentUris.appendId(builder, end);
        }
    ...
    private static String buildQuerySelection(Context context) {
        // Respect the preference to show/hide declined events
    	  /*SharedPreferences prefs = GeneralPreferences.getSharedPreferences(context);
          boolean hideDeclined = prefs.getBoolean(GeneralPreferences.KEY_HIDE_DECLINED, false);
        if (hideDeclined) {
     ...
     * Loads <i>days</i> days worth of instances starting at <i>startDay</i>.
     */
    public static void loadAllEvents(Context context, ArrayList<Event> events, int startDay, int endDay,String query,
            int requestId, AtomicInteger sequenceNumber) {
    	Log.d("GNMonthView", "Event.loadEvents() has been invoked");
        ...
        try {
        	...
        	String startdateViewText = DateUtils.formatDateRange(context, mFormatter, millis, millis, flags).toString();
        	long endmillis = date.setJulianDay(endDay);
        	String enddateViewText = DateUtils.formatDateRange(context, mFormatter, endmillis, endmillis, flags).toString();
        		Log.i("jiating","loadAllEvents...startDay="+startdateViewText+"endDay="+enddateViewText+"query="+query);
        		 String where=buildQuerySelection(context);
        		 cEvents = instancesQuery(context.getContentResolver(), EVENT_PROJECTION, startDay,
                         endDay, query, where, null, SORT_EVENTS_BY);
//        	}
...
//            buildEventsFromCursor(events, cAllday, context, startDay, endDay);
  ...
  //Gionee <jiating> <2013-04-22>  modify for CR00000000  loadAllEvents end
}
