...
public class NextDayTimeLayout extends LinearLayout {
    ...
    public static final String[] MONTH_TABLE = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
...
//    public static final int ALARM_ICON = R.drawable.ic_lock_idle_alarm;
	...
	private NextDayClockImageView mClockImageView1;
	private NextDayClockImageView mClockImageView2;
	private NextDayClockImageView mClockImageView3;
	private NextDayClockImageView mClockImageView4;
	private NextDayClockImageView mClockImageView5;
    ...
    public NextDayTimeLayout(Context context) {
        this(context, null);
    }

    public NextDayTimeLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NextDayTimeLayout(Context context, AttributeSet attrs, int defStyle) {
...
//        mDateView = ( TextView ) findViewById(R.id.date);
		...
		mClockImageView1 = (NextDayClockImageView)findViewById(R.id.clock_image_1);
		mClockImageView2 = (NextDayClockImageView)findViewById(R.id.clock_image_2);
		mClockImageView3 = (NextDayClockImageView)findViewById(R.id.clock_image_3);
		mClockImageView4 = (NextDayClockImageView)findViewById(R.id.clock_image_4);
		mClockImageView5 = (NextDayClockImageView)findViewById(R.id.clock_image_5);
...
//        mLunarDateView.setTypeface(Typeface.SANS_SERIF, Typeface.BOLD);
    ...
    public void refresh(int index, NextDayPictureInfo pictureInfo) {
        refreshDate(index, pictureInfo);
        refreshWeek(index, pictureInfo);
    }
...
//                DateFormat.format(DATE_DAY_FORMAT, new Date()));
    ...
    void refreshDate(int index, NextDayPictureInfo pictureInfo) {
    	String date = pictureInfo.getPictureTime();
    	int month = Integer.valueOf(date.substring(4, 6));
    	mDateMonthView.setText(MONTH_TABLE[month - 1]);
    	mDateDayView.setText(Integer.valueOf(date.substring(6, 8)) + "th");
...
//    	maybeSetUpperCaseText(mDateMonthView, mDateDayView, DateFormat.format(mDateFormatString, date));
    ...
    void refreshWeek(int index, NextDayPictureInfo pictureInfo) {
        setWeek(index, pictureInfo);
    }
    ...
    private void setWeek(int index, NextDayPictureInfo pictureInfo) {
        Calendar calendar = Calendar.getInstance();
        Date mDate = null;
        try {
        	mDate = new SimpleDateFormat("yyyyMMdd").parse(pictureInfo.getPictureTime());
		} catch (Exception e) {
			// TODO: handle exception
			Log.d("Wallpaper_DEBUG", "NextDayTimeLayout------setWeek-----Exception = "+e);
		}
        if (mDate != null) {
    ...
    public void onPageSelected(int position, NextDayPictureInfo pictureInfo) {
        onPageSelected(position, false, pictureInfo);
    }
    ...
    public void onPageSelected(int position, boolean isSingle, NextDayPictureInfo pictureInfo) {
        refresh(position, pictureInfo);
        
        mClockImageView1.onPageSelected(position, pictureInfo);
		mClockImageView2.onPageSelected(position, pictureInfo);
		mClockImageView3.onPageSelected(position, pictureInfo);
		mClockImageView4.onPageSelected(position, pictureInfo);
		mClockImageView5.onPageSelected(position, pictureInfo);
    }
    ...
    public void updateClock(NextDayPictureInfo pictureInfo) {
    	/*mClockImageView1.updateSingleTime();
		mClockImageView2.updateSingleTime();
		mClockImageView4.updateSingleTime();
		mClockImageView5.updateSingleTime();*/
		
		mClockImageView1.updateTime(pictureInfo);
		mClockImageView2.updateTime(pictureInfo);
		mClockImageView3.updateTime(pictureInfo);
		mClockImageView4.updateTime(pictureInfo);
		mClockImageView5.updateTime(pictureInfo);
	}
}
