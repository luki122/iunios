...
public class VpnSettings extends SettingsPreferenceFragment implements
        Handler.Callback, AuroraPreference.OnPreferenceClickListener,
    ...
    //Gionee <wangguojing> <2013-07-25> add for CR00837650 begin
    private static VpnSettings mVpnSettings ;
    public static VpnSettings getVpnSettingsInstance() {
		return mVpnSettings ;
    ...
    public void onCreate(Bundle savedState) {
        ...
        AuroraActionBar mActionBar = activity.getAuroraActionBar();
        mActionBar.addItem(AuroraActionBarItem.Type.Add, MENU_ID_VPN_CREATE);
        mActionBar.setOnAuroraActionBarListener(actionBarItemClickListener);

        if (!SystemProperties.getBoolean("persist.radio.imsregrequired", false)) {
        	activity.setAuroraMenuCallBack(auroraMenuCallBack);
            activity.setAuroraMenuItems(R.menu.aurora_vpn);
        }
        // Aurora <likai> <2013-10-28> add end
    }

    // Aurora <likai> <2013-10-28> delete begin
    /*
    @Override
    ...
    */
    // Aurora <likai> <2013-10-28> delete end
    ...
    public void onResume() {
        ...
        if (mPreferences.size() == 0) {
            AuroraPreferenceGroup group = getPreferenceScreen();
...
//            mKeyStore.put(Credentials.VPN + profile.key, profile.encode());
            ...
            if (Build.VERSION.SDK_INT >= 18) {
                try {
                    Class<?> sPolicy = null;
                    sPolicy = Class.forName("android.security.KeyStore");
                    Method method = sPolicy.getMethod("put", String.class, byte[].class, int.class, int.class);
                    method.invoke(mKeyStore, Credentials.VPN + profile.key, profile.encode(), KeyStore.UID_SELF, KeyStore.FLAG_ENCRYPTED);
                } catch (Exception e) {}  
            } else {
                try {
                    Class<?> sPolicy = null;
                    sPolicy = Class.forName("android.security.KeyStore");
                    Method method = sPolicy.getMethod("put", String.class, byte[].class);
                    method.invoke(mKeyStore, Credentials.VPN + profile.key, profile.encode());
                } catch (Exception e) {}
            }
    ...
    private static class VpnPreference extends AuroraPreference {
    ...
    public static class LockdownConfigFragment extends DialogFragment {
        ...
        private static class TitleAdapter extends ArrayAdapter<CharSequence> {
            public TitleAdapter(Context context, List<CharSequence> objects) {
            	super(context, R.layout.aurora_select_dialog_singlechoice_holo, android.R.id.text1, objects);
            }
        }
...
//                        keyStore.put(Credentials.LOCKDOWN_VPN, profile.key.getBytes());
                        if (Build.VERSION.SDK_INT >= 18) {
                            try {
                                ...
                                sPolicy = Class.forName("android.security.KeyStore");
                                Method method = sPolicy.getMethod("put", String.class, byte[].class, int.class, int.class);
                                method.invoke(keyStore, Credentials.VPN + profile.key, profile.encode(), KeyStore.UID_SELF, KeyStore.FLAG_ENCRYPTED);
                            } catch (Exception e) {}
                        } else {
                            try {
                                Class<?> sPolicy = null;
                                sPolicy = Class.forName("android.security.KeyStore");
                                Method method = sPolicy.getMethod("put", String.class, byte[].class);
                                method.invoke(keyStore, Credentials.VPN + profile.key, profile.encode());
                            } catch (Exception e) {}
                        }
                    }
