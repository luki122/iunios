...
public final class ChooseLockSettingsHelper {
    ...
    public boolean launchConfirmationActivity(int request, CharSequence message, CharSequence details) {
        ...
        switch (mLockPatternUtils.getKeyguardStoredPasswordQuality()) {
            case DevicePolicyManager.PASSWORD_QUALITY_SOMETHING:
                ...
                break;
            //Aurora <zhang_xin> <2013-12-16> modify for digit begin
            case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC:
                launched = confirmDigit(request);
                break;
            //Aurora <zhang_xin> <2013-12-16> modify for digit end
    ...
    //Aurora <zhang_xin> <2013-12-16> modify for digit password begin
    /**
     * Launch screen to confirm the existing lock digit.
     * @see #onActivityResult(int, int, android.content.Intent)
     * @return true if we launched an activity to confirm digit
     */
    private boolean confirmDigit(int request) {
        if (!mLockPatternUtils.isLockPasswordEnabled()) return false;
        final Intent intent = new Intent();
        intent.setClassName("com.android.settings", "com.android.settings.lockscreen.ConfirmLockDigit");
        if (mFragment != null) {
            mFragment.startActivityForResult(intent, request);
        } else {
            mActivity.startActivityForResult(intent, request);
    ...
    //Aurora <zhang_xin> <2013-12-16> modify for digit password end
}
