...
final class BluetoothDiscoverableEnabler implements AuroraPreference.OnPreferenceChangeListener {
// Aurora liugj 2013-10-22 modified for aurora's new feature end
    ...
    // Use the same preference key for discoverable timeout as the old AuroraListPreference.
	...
	// Aurora liugj 2013-10-22 modified for aurora's new feature start
    private final AuroraSwitchPreference mDiscoveryPreference;
	// Aurora liugj 2013-10-22 modified for aurora's new feature end
    ...
    BluetoothDiscoverableEnabler(Context context, LocalBluetoothAdapter adapter,
            AuroraSwitchPreference discoveryPreference) {
        mContext = context;
    ...
    public void resume() {
        ...
        if (mLocalAdapter.getScanMode() == BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE) {
        	...
        	if (mLocalAdapter.getScanMode() == BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE) { // qy end 
    ...
    public void pause() {
        ...
        mDiscoveryPreference.setOnPreferenceChangeListener(null);
    }
    ...
    public boolean onPreferenceChange(AuroraPreference preference, Object newValue) {
    	Log.e(TAG, newValue+"----onPreferenceChange----"+mDiscoverable);
    	if (mDiscoverable == (Boolean)newValue) {
			return true;
		}   	
...
//            mDiscoveryPreference.setChecked(true);
    ...
    private void setSummaryNotDiscoverable() {
        if (mNumberOfPairedDevices != 0) {
        	...
        	// qy R.string.bluetooth_not_visible_to_other_devices
            mDiscoveryPreference.setSummary(R.string.bluetooth_only_visible_to_paired_devices);
        }
    }
