...
//            com.android.internal.R.drawable.stat_notify_wifi_in_range;
    R.drawable.aurora_stat_notify_wifi_in_range;
    /**
    ...
    private static final String TAG="pgd";
    private WifiStateMachine mWifiStateMachine=null;
    private List<ScanResult> mscanResume=null;
        ...
        mContext.registerReceiver(
                new BroadcastReceiver() {
                    ...
                    public void onReceive(Context context, Intent intent) {
                    	Log.d(TAG, "pgd-- NOtification action:"+intent.getAction());
                        if (intent.getAction().equals(WifiManager.WIFI_STATE_CHANGED_ACTION)) {
                            ...
                            Log.i(TAG, " NetworkInfo   DetailedState : "+ mNetworkInfo.getDetailedState());
...
//        setNotificationVisible(true, 1, false, 0);
        
    }
    public WifiNotificationController(Context context, List<ScanResult> scanResume) {
        mContext = context;
        mWifiState = WifiManager.WIFI_STATE_UNKNOWN;
        mscanResume=scanResume;
        IntentFilter filter = new IntentFilter();
        filter.addAction(WifiManager.WIFI_STATE_CHANGED_ACTION);
        filter.addAction(WifiManager.NETWORK_STATE_CHANGED_ACTION);
        filter.addAction(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION);
   ...
   // 	Log.d(TAG, "checkAndSetNotification :: NotificationEnabled:"+mNotificationEnabled + " networkInfo: "+ (networkInfo == null) );
   //     if(networkInfo!=null)
   //     {
   //     	Log.d(TAG, "  NetworkInfo.State:"+networkInfo.getState());
   //     	Log.d(TAG, "  scanResults:"+(scanResults!=null) +  "    State(): "+networkInfo.getState()+ " size:"+ scanResults.size());
   //    }
 ...
 //                   {
  //                  	Log.d(TAG, "ssid:"+ scanResult.SSID  +"  scanResult.capabilities:" +scanResult.capabilities);
  //                      Log.d(TAG, " scanResult.capabilities: "+ (scanResult.capabilities!=null) + " scanResult.capabilities:"+ scanResult.capabilities.equals("[ESS]"));
  //                  }
                    if (scanResult.capabilities != null &&
...
//                numOpenNetworks++;
 //               Log.i(TAG, "  numOpenNetworks:"+numOpenNetworks);
                
                if (numOpenNetworks > 0) {
                	Log.i(TAG, "  mNumScansSinceNetworkStateChange:"+mNumScansSinceNetworkStateChange + "  NUM_SCANS_BEFORE_ACTUALLY_SCANNING:"+NUM_SCANS_BEFORE_ACTUALLY_SCANNING);
   ...
   // 	 Log.i(TAG, "  -- mNotificationShown:"+mNotificationShown+ "  visible:"+visible + " force:"+force);
        if (!visible && !mNotificationShown && !force) {   //PGD
            return;
        }
 ...
 //           Log.d(TAG, "title------------------------------");
 //           Log.d(TAG, "title:"+title.toString() + " detail:"+details.toString());
            mNotificationRepeatTime = System.currentTimeMillis() + NOTIFICATION_REPEAT_DELAY_MS;
           if(mNetworkInfo !=null && !mNetworkInfo.isConnected())
          {
            Log.d(TAG, "   display  notificationManager  ");
            notificationManager.notifyAsUser(null, -2000010, mNotification,
                        UserHandle.ALL);
          }else
          {
...
//        	Log.d(TAG, " clear notificationManager");
            notificationManager.cancelAsUser(null, -2000010, UserHandle.ALL);
        }
    ...
    private class NotificationEnabledSettingObserver extends ContentObserver {
        ...
        public void register() {
            ...
            synchronized (WifiNotificationController.this) {
                ...
                Log.d(TAG,  "     mNotificationEnabled:"+mNotificationEnabled);
            }
        }
...
//            Log.d(TAG, "onChange" );
            synchronized (WifiNotificationController.this) {
                mNotificationEnabled = getValue();
                Log.d(TAG, "onChange"+ "     mNotificationEnabled:"+mNotificationEnabled);
                resetNotification();
            }
        }
