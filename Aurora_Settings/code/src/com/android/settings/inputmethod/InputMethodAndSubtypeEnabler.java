...
public class InputMethodAndSubtypeEnabler extends SettingsPreferenceFragment {
    ...
    final private HashMap<String, List<AuroraPreference>> mInputMethodAndSubtypePrefsMap =
            new HashMap<String, List<AuroraPreference>>();
    final private HashMap<String, AuroraCheckBoxPreference> mSubtypeAutoSelectionCBMap =
            new HashMap<String, AuroraCheckBoxPreference>();
    ...
    public void onCreate(Bundle icicle) {
        ...
        // single AuroraActivity (see InputMethodAndSubtypeEnablerActivity). It should be available
        // from a preference argument when the preference is launched as a part of the other
        // AuroraActivity (like a right pane of 2-pane Settings app)
    ...
    public boolean onPreferenceTreeClick(
        ...
        if (preference instanceof AuroraCheckBoxPreference) {
            ...
            if (chkPref.isChecked()) {
                ...
                if (mDialog == null) {
                    mDialog = (new AuroraAlertDialog.Builder(getActivity()))
    ...
    private AuroraPreferenceScreen createPreferenceHierarchy() {
        // Root
        final AuroraPreferenceScreen root = getPreferenceManager().createPreferenceScreen(getActivity());
        ...
        for (int i = 0; i < N; ++i) {
            ...
            final AuroraPreferenceCategory keyboardSettingsCategory = new AuroraPreferenceCategory(context);
            ...
            // TODO: Use toggle AuroraPreference if images are ready.
            final AuroraCheckBoxPreference autoCB = new AuroraCheckBoxPreference(context);
            mSubtypeAutoSelectionCBMap.put(imiId, autoCB);
            keyboardSettingsCategory.addPreference(autoCB);

            final AuroraPreferenceCategory activeInputMethodsCategory = new AuroraPreferenceCategory(context);
            activeInputMethodsCategory.setTitle(R.string.active_input_method_subtypes);
            root.addPreference(activeInputMethodsCategory);

            boolean isAutoSubtype = false;
            CharSequence autoSubtypeLabel = null;
            final ArrayList<AuroraPreference> subtypePreferences = new ArrayList<AuroraPreference>();
            if (subtypeCount > 0) {
                for (int j = 0; j < subtypeCount; ++j) {
                    ...
                    if (subtype.overridesImplicitlyEnabledSubtype()) {
                        ...
                        final AuroraCheckBoxPreference chkbxPref = new SubtypeCheckBoxPreference(
    ...
    private boolean isNoSubtypesExplicitlySelected(String imiId) {
        ...
        final List<AuroraPreference> subtypePrefs = mInputMethodAndSubtypePrefsMap.get(imiId);
        for (AuroraPreference subtypePref: subtypePrefs) {
            if (subtypePref instanceof AuroraCheckBoxPreference
                    && ((AuroraCheckBoxPreference)subtypePref).isChecked()) {
    ...
    private void setSubtypeAutoSelectionEnabled(String imiId, boolean autoSelectionEnabled) {
        AuroraCheckBoxPreference autoSelectionCB = mSubtypeAutoSelectionCBMap.get(imiId);
        ...
        for (AuroraPreference subtypePref: subtypePrefs) {
            if (subtypePref instanceof AuroraCheckBoxPreference) {
                ...
                if (autoSelectionEnabled) {
                    ((AuroraCheckBoxPreference)subtypePref).setChecked(false);
                }
            }
        }
        if (autoSelectionEnabled) {
    ...
    private void updateImplicitlyEnabledSubtypes(String targetImiId, boolean check) {
        ...
        for (InputMethodInfo imi: mInputMethodProperties) {
            ...
            final AuroraCheckBoxPreference autoCB = mSubtypeAutoSelectionCBMap.get(imiId);
            ...
            for (AuroraPreference subtypePref: subtypePrefs) {
                if (subtypePref instanceof AuroraCheckBoxPreference) {
                    AuroraCheckBoxPreference cb = (AuroraCheckBoxPreference)subtypePref;
                    cb.setChecked(false);
                    if (check) {
    ...
    private static class SubtypeCheckBoxPreference extends AuroraCheckBoxPreference {
        ...
        @Override
        public int compareTo(AuroraPreference p) {
