...
public class InputMethodAndSubtypeUtil {
    ...
    public static void saveInputMethodSubtypeList(SettingsPreferenceFragment context,
        ...
        for (InputMethodInfo imi : inputMethodInfos) {
            ...
            AuroraPreference pref = context.findPreference(imiId);
            if (pref == null) continue;
            // In the Configure input method screen or in the subtype enabler screen.
            // pref is instance of AuroraCheckBoxPreference in the Configure input method screen.
            ...
            if ((!hasHardKeyboard && isAlwaysCheckedIme(imi, context.getActivity(), imiCount))
                ...
                for (int i = 0; i < subtypeCount; ++i) {
                    ...
                    AuroraCheckBoxPreference subtypePref = (AuroraCheckBoxPreference) context.findPreference(
    ...
    public static void loadInputMethodSubtypeList(
        ...
        for (InputMethodInfo imi : inputMethodInfos) {
            final String imiId = imi.getId();
            AuroraPreference pref = context.findPreference(imiId);
            if (pref != null && pref instanceof AuroraCheckBoxPreference) {
                AuroraCheckBoxPreference checkBoxPreference = (AuroraCheckBoxPreference) pref;
                boolean isEnabled = enabledSubtypes.containsKey(imiId);
                checkBoxPreference.setChecked(isEnabled);
                if (inputMethodPrefsMap != null) {
                    for (AuroraPreference childPref: inputMethodPrefsMap.get(imiId)) {
    ...
    public static void setSubtypesPreferenceEnabled(SettingsPreferenceFragment context,
        ...
        AuroraPreferenceScreen preferenceScreen = context.getPreferenceScreen();
        for (InputMethodInfo imi : inputMethodProperties) {
            if (id.equals(imi.getId())) {
                final int subtypeCount = imi.getSubtypeCount();
                for (int i = 0; i < subtypeCount; ++i) {
                    InputMethodSubtype subtype = imi.getSubtypeAt(i);
                    AuroraCheckBoxPreference pref = (AuroraCheckBoxPreference) preferenceScreen.findPreference(
                            id + subtype.hashCode());
                    if (pref != null) {
    ...
    public static boolean isAlwaysCheckedIme(InputMethodInfo imi, Context context, int imiCount) {
		...
		// gionee wangyaohui 20121217 modify for CR00740781 gegin
        // if (!isSystemIme(imi)) {
        //     return false;
        // }
		if (isSystemIme(imi)) {
		...
		// gionee wangyaohui 20121217 modify for CR00740781 end
