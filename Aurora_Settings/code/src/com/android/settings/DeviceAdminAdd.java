...
public class DeviceAdminAdd extends AuroraActivity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (GnSettingsUtils.getThemeType(/*getApplicationContext()*/DeviceAdminAdd.this).equals(GnSettingsUtils.TYPE_LIGHT_THEME)){ // qy modify 2014 05 04 for monkey
        ...
        if (DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN.equals(getIntent().getAction())) {
            ...
            if (mDPM.isAdminActive(cn)) {
                ...
                if (!mRefreshing) {
                    ...
                    setResult(AuroraActivity.RESULT_OK);
                    finish();
                    return;
                }
            }
...
//                                showDialog(DIALOG_WARNING, args); //  qy modify 2014 05 04 for monkey begin 
                                if (!isFinishing()) {  
                                	showDialog(DIALOG_WARNING, args); 
                                }
                                
                            //  qy modify 2014 05 04 for monkey end
                            }
                        }
                    });
                }
        ...
        if (!mRefreshing && mDPM.isAdminActive(mDeviceAdmin.getComponent())) {
            if (mActivePolicies.size() == 0) {
                ...
                for (int i=0; i<policies.size(); i++) {
                    ...
                    View view;
//                    if(mGnAppPermCtrl){
//                        view = GNAppSecurityPermissions.getPermissionItemView(
//                            this, getText(pi.label), "", true);
//                    }else{
                        view = AppSecurityPermissions.getPermissionItemView(
                                this, getText(pi.label), "", true);
//                    }
