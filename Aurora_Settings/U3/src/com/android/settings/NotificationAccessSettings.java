...
public class NotificationAccessSettings extends ListFragment {
    ...
    private static int getListeners(ArrayAdapter<ServiceInfo> adapter, PackageManager pm) {
        ...
        for (int i = 0, count = installedServices.size(); i < count; i++) {
            ...
            //add by penggangding 2015.01.26 begin for Hide Laser lock screen and Selected by default
            if(resolveInfo.serviceInfo.packageName.equals("com.android.keyguard"))
            {
            	continue;
            }
            //add by penggangding 2015.01.26 end for Hide Laser lock screen and Selected by default
            if (adapter != null) {
                adapter.add(info);
            }
            listeners++;
