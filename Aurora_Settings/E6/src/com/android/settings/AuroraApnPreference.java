...
//		mChecked = true;
	...
	// add by steve.tang 2014-06-11, for dual sim apn choose start.
		if(MULTI_SIM_SUPPORT) {
            if(sub_id==0)
            {
                resolver.update(Uri.parse(ApnSettings.MTK_PREFERRED_APN_URI_SIM1), values, null, null);
            }else if(sub_id==1)
            {
            	resolver.update(Uri.parse(ApnSettings.MTK_PREFERRED_APN_URI_SIM2), values, null, null);
            }
