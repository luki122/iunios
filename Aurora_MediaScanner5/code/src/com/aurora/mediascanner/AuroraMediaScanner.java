 ...
 * - scanDirectories() calls the native auroraProgressDirectory() for each of the specified directories.
 * - the auroraProgressDirectory() JNI method wraps the provided mediascanner client in a native
 *   'MyMediaScannerClient' class, then calls auroraProgressDirectory() on the native MediaScanner
 *   object (which got created when the Java MediaScanner was created).
 * - native MediaScanner.auroraProgressDirectory() calls
 ...
 *   MediaScanner.auroraProgressFile().
 * - MediaScanner.auroraProgressFile() calls one of several methods, depending on the type of the
 ...
 * - once auroraProgressFile returns and we're back in Java code in doScanFile(), it calls
 ...
 * Java MediaScanner auroraProgressDirectory (native method), which calls
 * native MediaScanner auroraProgressDirectory, which calls
 * native MyMediaScannerClient scanFile, which calls
 * Java MyMediaScannerClient scanFile, which calls
 * Java MediaScannerClient doScanFile, which calls
 * Java MediaScanner auroraProgressFile (native method), which calls
 * native MediaScanner auroraProgressFile, which calls
 * native parseMP3, parseMP4, parseMidi, parseOgg or parseWMA, which calls
 * native MyMediaScanner handleStringTag, which calls
 * Java MyMediaScanner handleStringTag.
 * Once MediaScanner auroraProgressFile returns, an entry is inserted in to the database.
 *
...
public class AuroraMediaScanner
{
    ...
    private class MyMediaScannerClient implements AuroraMediaScannerClient {
        ...
        public FileEntry beginFile(String path, String mimeType, long lastModified,
            ...
            if (!isDirectory) {
                ...
                // in these cases it will not call auroraProgressFile() but processImageFile() instead.
                // for these cases, we change the {mFileType} back to ZERO,
                // and let Media.getFileType(path) to determine the type,
                // so that it can call auroraProgressFile() as normal. @{
...
//           Log.v("JXH", "auroraHandleStringTag:"+toString());
        ...
        @Override
		public String toString() {
			return "MyMediaScannerClient [mArtist=" + mArtist + ", mAlbumArtist=" + mAlbumArtist + ", mAlbum=" + mAlbum + ", mTitle=" + mTitle + ", mComposer=" + mComposer + ", mGenre=" + mGenre
					+ ", mMimeType=" + mMimeType + ", mFileType=" + mFileType + ", mTrack=" + mTrack + ", mYear=" + mYear + ", mDuration=" + mDuration + ", mPath=" + mPath + ", mLastModified="
					+ mLastModified + ", mFileSize=" + mFileSize + ", mWriter=" + mWriter + ", mCompilation=" + mCompilation + ", mIsDrm=" + mIsDrm + ", mNoMedia=" + mNoMedia + ", mWidth=" + mWidth
					+ ", mHeight=" + mHeight + ", mDrmContentUr=" + mDrmContentUr + ", mDrmOffset=" + mDrmOffset + ", mDrmDataLen=" + mDrmDataLen + ", mDrmRightsIssuer=" + mDrmRightsIssuer
					+ ", mDrmContentName=" + mDrmContentName + ", mDrmContentDescriptioin=" + mDrmContentDescriptioin + ", mDrmContentVendor=" + mDrmContentVendor + ", mDrmIconUri=" + mDrmIconUri
					+ ", mDrmMethod=" + mDrmMethod + ", mIsLivePhoto=" + mIsLivePhoto + ", mSlowMotionSpeed=" + mSlowMotionSpeed + ", mOrientation=" + mOrientation + "]";
		}
        ...
        private Uri endFile(FileEntry entry, boolean ringtones, boolean notifications,
            ...
            //专辑变成歌曲文件夹名称，导致无法寻找歌曲歌词(xiami SDK)
...
//            c = mMediaProvider.query(mPackageName, mImagesUri, ID_PROJECTION, null, null, null, null);
            ...
            c=GnIContentProvider.query(mMediaProvider, mPackageName, mVideoUri,
    				ID_PROJECTION, null, null, null, null);
//            c = mMediaProvider.query(mPackageName, mVideoUri, ID_PROJECTION, null, null, null, null);
            ...
            c=GnIContentProvider.query(mMediaProvider, mPackageName, mAudioUri,
    				ID_PROJECTION, null, null, null, null);
...
//        LogUtil.d(TAG, "pruneDeadThumbnailFiles>>>");
...
//                    null, null);
        	c = GnIContentProvider.query(mMediaProvider, mPackageName,
        			mThumbsUri, new String [] { "_data" }, null,
        			null, null, null);
            ...
            c = GnIContentProvider.query(mMediaProvider, mPackageName,
            		mVideoThumbsUri, new String [] { "_data" }, null,
        			null, null, null);
//            c = mMediaProvider.query(
//                    mPackageName,
//                    mVideoThumbsUri,
...
//                mMediaInserter = new MediaInserter(mMediaProvider, mPackageName, 500);
            ...
            for (int i = 0; i < directories.length; i++) {
                auroraProgressDirectory(directories[i], mClient);
            }
...
//			LogUtil.d(TAG, " prescan time: " + (prescan - start) + "ms\n");
//			LogUtil.d(TAG, "    scan time: " + (scan - prescan) + "ms\n");
//			LogUtil.d(TAG, "postscan time: " + (end - scan) + "ms\n");
//			LogUtil.d(TAG, "   total time: " + (end - start) + "ms\n");
    ...
    public Uri auroraScanSingleFile(String path, String volumeName, String mimeType) {
        try {
    ...
    public static void clearMediaPathCache(boolean clearMediaPaths, boolean clearNoMediaPaths) {
        synchronized (AuroraMediaScanner.class) {
            if (clearMediaPaths) {
                mMediaPaths.clear();
            }
    ...
    public void scanMtpFile(String path, String volumeName, int objectHandle, int format) {
        ...
        if (!MediaFile.isAudioFileType(fileType) && !MediaFile.isVideoFileType(fileType) &&
            ...
            try {
                ...
                GnIContentProvider.update(mMediaProvider, mPackageName, Files.getMtpObjectsUri(volumeName),
						values, "_id=?", whereArgs);
            ...
            if (MediaFile.isPlayListFileType(fileType)) {
                ...
                if (entry != null) {
                	fileList = GnIContentProvider.query(mMediaProvider, mPackageName,
        					mFilesUri, FILES_PRESCAN_PROJECTION, null, null,
        					null, null);
...
//                    where, selectionArgs, null, null);
            c =  GnIContentProvider.query(mMediaProvider, mPackageName,
            		mFilesUriNoNotify, FILES_PRESCAN_PROJECTION, where, selectionArgs, null,
					null);
            if (c!=null&&c.moveToFirst()) {
                long rowId = c.getLong(FILES_PRESCAN_ID_COLUMN_INDEX);
                int format = c.getInt(FILES_PRESCAN_FORMAT_COLUMN_INDEX);
...
//                    mMediaProvider.insert(mPackageName, playlistUri, values);
                    GnIContentProvider.insert(mMediaProvider,
							mPackageName, playlistUri, values);
                    index++;
                } catch (RemoteException e) {
...
//            uri = mMediaProvider.insert(mPackageName, mPlaylistsUri, values);
            uri =  GnIContentProvider.insert(mMediaProvider,
					mPackageName, mPlaylistsUri, values);
...
//            mMediaProvider.delete(mPackageName, membersUri, null, null);
    ...
    private void processPlayLists() throws RemoteException {
        ...
        try {
        	...
        	fileList = GnIContentProvider.query(mMediaProvider, mPackageName,
        			mFilesUri, FILES_PRESCAN_PROJECTION,  "media_type=2", null, null,
					null);
...
//                    "media_type=2", null, null, null);
 ...
 // JNI方法
 	...
 	public native byte[] auroraExtractAlbumArt(FileDescriptor fd);
        ...
        try {
            ...
            for (String path : folders) {
                if (isSingelFile) {
                	...
                	auroraProgressDirectory(path, mClient);
                }
            }
            if (ENABLE_BULK_INSERTS) {
                /// flush remaining inserts
        ...
        try {
            ...
            for (String folder : folders) {
                  ...
                  if (!isSingelFileOrEmptyFolder) {
                    auroraProgressDirectory(folder, mClient);
                  }
            }
