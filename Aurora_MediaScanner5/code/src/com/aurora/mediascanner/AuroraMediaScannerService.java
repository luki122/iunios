...
public class AuroraMediaScannerService extends Service implements Runnable {
	...
	private final class AuroraServiceHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			try {
				if (sScanType != null) {
					// 扫描文件
					if (sScanType.equals(ScannerUtils.ACTION_FILE_SCAN)) {
					...
					} else if (sScanType.equals(ScannerUtils.ACTION_DIR_SCAN)) {// 扫描指定的目录
					...
					} else if (sScanType.equals(ScannerUtils.ACTION_EXT_SCAN)) {// 扫描根目录只扫描外部存储卷
						...
						if (directories != null && directories.length > 0) {
							...
							scanPaths.remove(ScannerUtils.ACTION_EXT_SCAN);
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (!TextUtils.isEmpty(scanPath)) {
			if (!scanPaths.contains(scanPath)) {
				...
				if (!sScanType.equals(ScannerUtils.ACTION_EXT_SCAN)) {
		...
		if (sScanType.equals(ScannerUtils.ACTION_EXT_SCAN)) {
			...
			if (!scanPaths.contains(ScannerUtils.ACTION_EXT_SCAN)) {
				scanPaths.add(ScannerUtils.ACTION_EXT_SCAN);
	...
	private void auroraScanDirectory(String[] directories) {
		...
		try {
			...
			e.printStackTrace();
			Log.e(TAG,
					"auroraScanDirectory  exception:" + e.getLocalizedMessage());
		} finally {
