...
public class LoginActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * TotalCount tl = new TotalCount(this, "1", "1", 1); tl.CountData();
		 */
		
		/* 权限对话框 */
		showSysAuthDialog(this);
	...
	public static void showSysAuthDialog(final Activity activity) {
		...
		final SharedPreferences sp = activity.getSharedPreferences(
				SYS_AUTO_PRE, Activity.MODE_PRIVATE);
		boolean showKey = sp.getBoolean(SYS_AUTO_SHOW_KEY, true); // 是否显示提示框
		if (showKey) {
			LayoutInflater inflater = activity.getLayoutInflater();
			View layout = inflater.inflate(
					R.layout.alert_dialog_authority_confirm, null);
			...
			builder.setPositiveButton(
					activity.getString(R.string.dialog_conntinus),
			...
			builder.setNegativeButton(
					activity.getString(R.string.dialog_quit),
					new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int id) {
							activity.finish();
	...
	private void initViews() {
		...
		mAccountEt.setPadding(mAccountEt.getPaddingLeft(),
				mAccountEt.getPaddingTop(), DensityUtil.dip2px(this, 16),
				mAccountEt.getPaddingBottom());
		// mAccountEt.setIsNeedDeleteAll(true);
		mPwdEt = (EditText) findViewById(R.id.password_cet);
		mPwdEt.setPadding(mPwdEt.getPaddingLeft(), mPwdEt.getPaddingTop(),
				DensityUtil.dip2px(this, 16), mPwdEt.getPaddingBottom());
		...
		mVCCodeEt.setPadding(mVCCodeEt.getPaddingLeft(),
				mVCCodeEt.getPaddingTop(), DensityUtil.dip2px(this, 16),
				mVCCodeEt.getPaddingBottom());
		initErrorViews();
			...
			public void onSizeChanged(int w, int h, int oldw, int oldh) {
				...
				if (h < oldh && h < oldh - VIRTUAL_BAR_HEIGHT) {
					if (mTopRl.getVisibility() == View.VISIBLE) {
						...
						CustomAnimation alphaAnimation = new CustomAnimation(
								new CustomAnimCallBack() {
									@Override
									public void callBack(
											float interpolatedTime,
		...
		if (apps != null && !apps.isEmpty()) {
			for (AppConfigInfo app : apps) {
				...
				if (!app.isApp_syncself()) {
					...
					result &= rc;
					Log.d(TAG,
							"Jim, clear syncId for " + app.getApp_packagename()
									+ " finished, result: " + rc);
				}
			}
		}
	...
	private void openMainActivity(int loginStatus) {
		...
		intent.putExtra(SyncAccountActivity.EXTRA_KEY_NEED_SET_EXIT_ANIMATION,
				mNeedExitAnim);
