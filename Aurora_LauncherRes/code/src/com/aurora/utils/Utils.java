package com.aurora.publicicon.res;

import java.util.HashMap;
import java.util.List;
import com.aurora.publicicon.res.R;
...
public class Utils {
	...
	private static final boolean IS_LAUNCHER = true;
	...
	public static Bitmap getRoundedBitmap(Bitmap bm ,Context context){
		...
		ICON_REAL_SIZE = res.getDimensionPixelSize(R.dimen.app_icon_real_size);
		APP_ICON_RADIUS = res.getDimensionPixelSize(R.dimen.app_icon_radius);
	...
	public  static  Drawable getSelfNotDrawDrawable(Context context,Drawable d){
		...
		if (USE_RIGHT_ANGLE && d != null) {
			...
			Bitmap bitmap2 = getRoundedBitmap(bitmap1,ICON_REAL_SIZE);
			if(USE_ICON_SHADOW){
				Bitmap bitmap3 = null;
				if (shadowId != 0) {
					Drawable shadow = resources.getDrawable(shadowId);
	...
	public synchronized static HashMap<String, String> configClassArray(Context context,
			String arrayName) {
		if (!isInitedConfig) {
			Context prePackageContext = null;
			try {
			...
			if (prePackageContext != null) {
				final int resId = prePackageContext.getResources()
						.getIdentifier(arrayName, "array", PUBLIC_ICON_RES);
				if (resId == 0) {
					return null;
				}
