...
public class ContactsProvider2 extends AbstractContactsProvider
    ...
    private static final int AURORA_DIALER_SEARCH = 90002;
    ...
    public static final Uri CONTENT_URI = Uri.parse("content://"
			+ ContactsContract.AUTHORITY + "/sync");
    //contacts sync end
    ...
    static {
        ...
        */
        ...
        //aurora add by liguangyu
        matcher.addURI(ContactsContract.AUTHORITY, "aurora_dialer_search/*", AURORA_DIALER_SEARCH);
        /// M: @}
        /// @}
        // aurora <wangth> <2013-12-9> add for aurora begin
        matcher.addURI(ContactsContract.AUTHORITY, "data/phone_emails_ime/filter/*", AURORA_SAMSUNG_MATCH);
        // aurora <wangth> <2013-12-9> add for aurora end
		...
		//contacts sync begin
		matcher.addURI(ContactsContract.AUTHORITY, "sync", SYNCS);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/#", SYNC);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/sync_up", SYNC_UP);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/sync_up/#/#", SYNC_UP_LIMIT);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/sync_up/#", SYNC_UP_LIMIT_COUNT);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/sync_up_result_multi", SYNC_UP_RESULT);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/sync_up_size", SYNC_UP_SIZE);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/sync_down_multi", SYNC_DOWN);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/accessory", SYNC_ACCESSORY);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/is_first_sync", URI_IS_FIRST_SYNC);
		
		
		
		matcher.addURI(ContactsContract.AUTHORITY, "sync/clean_account", URI_CLEAN_ACCOUNT);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/init_account", URI_INIT_ACCOUNT);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/init_account_multi", URI_INIT_ACCOUNT_MULTI);
		matcher.addURI(ContactsContract.AUTHORITY, "sync/clean_data", URI_CLEAN_DATA);
		//contacts sync end
		
		matcher.addURI(ContactsContract.AUTHORITY, "aurora_multi_search/*", AURORA_MULTI_HANZI);
    }
...
//        setAppOps(AppOpsManager.OP_READ_CONTACTS, AppOpsManager.OP_WRITE_CONTACTS);
    ...
    private void flushTransactionalChanges() {
        ...
        if (ContactsProvidersApplication.sIsGnSyncSupport) {
            ...
            if (!gnChangedRawContacts.isEmpty()) {
                ...
                mSb.append(")");
                mDbHelper.get().getWritableDatabase().execSQL(mSb.toString());
            }
        }
    ...
    protected Uri insertInTransaction(Uri uri, ContentValues values) {
        ...
        switch (match) {
    		...
    		case MARKS:
    			id = db.insert("mark", null, values); // 返回的是记录的行号，主键为int，实际上就是主键值
    			return ContentUris.withAppendedId(uri, id);
    		case MARK:
...
//    					SQLiteDatabase db=mActiveDb.get();
    					...
    					try {
    						for (int i = 0; i < jsonArray.length(); i++) {							
    							if(!TextUtils.isEmpty(ids.get(i)) && !TextUtils.isEmpty(flags.get(i))){
    							    if(!syncInitOne(db, ids.get(i), flags.get(i),dates.get(i))){
    ...
    private long insertRawContact(
        ...
        if(mDbHelper.get().isDefaultAccount(account)){
        	if (uri.getQueryParameter("sync")== null) {
        		values.put("sync3", System.currentTimeMillis());   
			}        
        }
    ...
    private long insertGroup(Uri uri, ContentValues inputValues, boolean callerIsSyncAdapter) {
        ...
        String accountType = values.getAsString(Groups.ACCOUNT_TYPE);
        String accountName = values.getAsString(Groups.ACCOUNT_NAME);
        Log.d(TAG, "values.containsKey(Groups.GROUP_VISIBLE) = " + values.containsKey(Groups.GROUP_VISIBLE));
        ...
        if (null != accountType) {
            ...
            try {
                c = mDbHelper.get().getWritableDatabase().query(Tables.SETTINGS,
        ...
        //Gionee:huangzy 20121128 add for CR00736966 start
        ContactsDatabaseHelper.writeGnVersion(getContext(), values);
        //Gionee:huangzy 20121128 add for CR00736966 end
        
       // aurora <wangth> <2014-03-07> add for aurora begin
        Cursor cur = null;
        try {
            cur = mDbHelper.get().getWritableDatabase().query(Tables.GROUPS,
            ...
            if (cur != null && cur.moveToFirst()) {
                do {
                    ...
                    Log.e(TAG, "dbTitle = " + dbTitle + "  mValues.getAsString(Groups.TITLE) = " + values.getAsString(Groups.TITLE));
                    if (dbTitle != null && dbTitle.equals(values.getAsString(Groups.TITLE))) {
                        exit = true;
                        break;
                    }
    ...
    protected int deleteInTransaction(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case RAW_CONTACTS:
                ...
                if (ContactsProvidersApplication.sIsAuroraPrivacySupport
                    ...
                    try {
                        ...
                        Cursor dataC = db.query(Tables.DATA,
    		...
    		case URI_CLEAN_ACCOUNT:
    			...
    			Long accountId = mDbHelper.get().getAccountIdOrNull(mDbHelper.get().getDefaultAccount());
    			Cursor needClean = db.query(Tables.RAW_CONTACTS, new String[]{"_id"}, "account_id="+accountId, null, null, null, null, null);
    			int count = 0;
    			if(needClean != null){
    				...
    				while(needClean.moveToNext()){
    					...
    					db.update(Tables.RAW_CONTACTS, values, "_id="+needCleanId, null);
//    					values.clear();
//    					values.putNull("data_sync4");
//    					db.update(Tables.DATA, values, "raw_contact_id="+needCleanId, null);
    				}
    				needClean.close();
    			}
    			int deleteCount = db.delete(Tables.RAW_CONTACTS, "sync4 is null and deleted=1 and account_id="+accountId, null);
    		...
    		case URI_CLEAN_DATA:
    			...
    			count=db.delete(Tables.RAW_CONTACTS, "is_privacy=0 and account_id="+accountId, null);
    			return count;
    		//contacts sync end
...
//    						updateCallsForBlack(number); // only update black_name
    					...
    					db.delete("black", "number='" + number + "' and isblack<1", null);
    				}
    			}
        ...
        try {
            while(c.moveToNext()) {
            	 ...
            	 // aurora <wangth> <2013-11-2> add for aurora begin
            	    String number = values.getAsString(Data.DATA1);
                    ...
                    if (Phone.CONTENT_ITEM_TYPE.equals(mime)) {                                              
                        if (ContactsProvidersApplication.sIsAuroraPrivacySupport && number == null) {
                            if (values.containsKey(AURORA_DEFAULT_PRIVACY_COLUMN)) {
                                number = c.getString(DataRowHandler.DataUpdateQuery.DATA1);
                                logs("updateDate number = " + number);
                                int is_privacy = values.getAsInteger(AURORA_DEFAULT_PRIVACY_COLUMN);
    ...
    private int updateContactOptions(
        ...
        if (hasStarredValue) {
            ...
            if (ContactsProvidersApplication.sIsGnSyncSupport) {
            	values.put(GnSyncColumns.GN_VERSION, 
        ...
        switch (match) {
            ...
            */
            ...
            case DIALER_SEARCH: {
                ...
                return mDialerSearchSupport.handleDialerSearchQueryEx(db, uri, selection, false);
            }
            /// M: @}
          //aurora add by liguangyu
            case AURORA_DIALER_SEARCH: {
                LogUtils.d(TAG, "AURORA-DialerSearch");
                return mDialerSearchSupport.handleDialerSearchQueryEx(db, uri, selection, true);
            }
            
    		case AURORA_MULTI_HANZI:
//			    return GnDialerSearchHelper.getInstance().queryMutil(db, uri, selection);
    			return null;
			...
			// reject begin
			case BLACKS:
				return db.query("black", projection, selection, selectionArgs,
						null, null, sortOrder);
			case BLACK:
				// 进行解析，返回值为10
				long blackid = ContentUris.parseId(uri);
...
//                    + sortOrderSuffix + ", " + sortOrder;
                		 + sortOrderSuffix + ", aurora_sort_key, " + sortOrder;
...
// ------------- MTK below ----------------------------
    ...
    private String queryNumberForId (String id) {
    	Log.e(TAG, "id = " + id);
        String result = null;
        Cursor cursor = mDbHelper.get().getWritableDatabase().query("black", new String[]{"number"}, "_id=" + id, null, null, null, null);
    ...
    private void updateCallsForBlack(String number, String name, boolean flag) {
    	...
    	try {
            ...
            mDbHelper.get().getWritableDatabase().update(Tables.CALLS, cv,
...
//							return false;
				...
				int delcount=getContext().getContentResolver().delete(Uri.parse("content://com.android.contacts/raw_contacts").buildUpon().appendQueryParameter("batch", "true")
						.appendQueryParameter(ContactsContract.CALLER_IS_SYNCADAPTER,String.valueOf(true)).build(), "_id=?",new String[]{contact_id+""});
						if(delcount>0){
							return true;
						}
			}
	...
	public boolean processAcc(JSONObject accessoryObjec,String updatePhoto){
		...
		long photoFileId = mDbHelper.get().getWritableDatabase().insert(Tables.PHOTO_FILES, null,
				cv);
		if (photoFileId < 1) {
			return false;
		}
