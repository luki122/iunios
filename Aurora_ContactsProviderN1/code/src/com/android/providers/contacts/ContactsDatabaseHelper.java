...
public class ContactsDatabaseHelper extends SQLiteOpenHelper {
     ...
     * @{
     */
    static final int DATABASE_VERSION = 4002;
    /** @} */
    ...
    // aurora reject begin
 	public static final class BlackTableData implements BaseColumns {
 		...
 		public static final Uri CONTENT_URI = Uri.parse("content://"
 				+ ContactsContract.AUTHORITY + "/black");
 	...
 	public static final class MarkTableData implements BaseColumns {
 		...
 		public static final Uri CONTENT_URI = Uri.parse("content://"
 				+ ContactsContract.AUTHORITY + "/mark");
...
//        ");");
        ...
        db.execSQL("CREATE TABLE " + Tables.CALLS + " (" +
                ...
                ",simid INTEGER DEFAULT -1" +             //add by liguangyu
                ",area" + " TEXT" +
    ...
    private void createContactsViews(SQLiteDatabase db) {
        ...
        String contactNameColumns =
		        ...
		        + ", name_raw_contact.aurora_sort_key AS aurora_sort_key"  // add by liguangyu
...
//            displayNameAlternative = bestDisplayName;
        ...
        if (sortKeyPrimary != null) {
        	//aurora liguangyu modify for 多音字
...
//            phonebookLabelPrimary = localeUtils.getBucketLabel(phonebookBucketPrimary);
            ...
            if (displayNameStyle == FullNameStyle.CHINESE
                ...
                ArrayList<Token> tokens = HanziToPinyin.getInstance().getTokens(target);
        ...
        auroraUpdateAuroraSortKey(db, displayNameStyle, String.valueOf(rawContactId));
  
        // aurora <wangth> <2013-12-13> add for aurora end
    }
    
    private static void auroraUpdateAuroraSortKey(SQLiteDatabase db, int displayNameStyle , String rawContactId) {
        Log.d(TAG, "updateRawContactDisplayName displayNameStyle = " + displayNameStyle);     
        ContentValues cv = new ContentValues();
        cv.put(AURORA_SORT_KEY, String.valueOf(displayNameStyle));
    ...
    private void upgradeToVersion4002(SQLiteDatabase db) {
         ...
         try {
            ...
            db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD " + Calls.IP_PREFIX + " TEXT; ");
            db.execSQL("ALTER TABLE " + Tables.CALLS + 
                    " ADD " + Calls.CONFERENCE_CALL_ID + " INTEGER; ");
        	 DialerSearchSupport.createDialerSearchTable(db);
             CallLogSearchSupport.createConferenceCallsTable(db);
             
             //dialersearch contact name
             Cursor c = db.query(Tables.DATA, new String[]{Data._ID,Data.RAW_CONTACT_ID}, 
            		 DataColumns.MIMETYPE_ID + "=7", null, null, null, null);
              
             if (null != c) {
                 if (c.moveToFirst()) {
                     do {
                    	 insertNameForDialerSearch(db, c.getInt(1), c.getInt(0));
                    	 updateRawContactDisplayName(db, c.getInt(1));
                     } while(c.moveToNext());
                 }
                 c.close();
             }
             
             //dialersearch contact number
             c = db.query(Tables.DATA, new String[]{Data._ID,Data.RAW_CONTACT_ID, Data.DATA1}, 
            		 DataColumns.MIMETYPE_ID + "=5", null, null, null, null);
             if (null != c) {
                 if (c.moveToFirst()) {
                     do {
                    	 insertDialerSearchNewRecord(db, c.getInt(1), c.getInt(0), c.getString(2));
                     } while(c.moveToNext());
                 }
                 c.close();
             }
                  
             //dialersearch calllog no contact
             c = db.query(Tables.CALLS, new String[]{Calls._ID, Calls.NUMBER}, 
                     Calls.CACHED_NAME + "=''", null, Calls.NUMBER, null, null);
             if (null != c) {
                 if (c.moveToFirst()) {
                     do {
                    	 insertDialerSearchCalllog(db,  c.getInt(0), c.getString(1));
                     } while(c.moveToNext());
                 }
                 c.close();
             }             
             ...
             SQLiteStatement update = db.compileStatement(
                     ...
                     " SET " + PhoneLookupColumns.MIN_MATCH + "=?" +
                     " WHERE " + PhoneLookupColumns.DATA_ID + "=?");
             c = db.query(Tables.PHONE_LOOKUP + " JOIN " + Tables.DATA +
                     " ON (" + PhoneLookupColumns.DATA_ID + "=" + DataColumns.CONCRETE_ID + ")",
                     new String[] {Data._ID, Phone.NUMBER}, null, null, null, null, null);
             if(c != null) {
            	  while (c.moveToNext()) {
    ...
    private void insertNameForDialerSearch(SQLiteDatabase db, long rawContactId, long dataId) {
    	SQLiteStatement  mDialerSearchNewRecordInsert;
    ...
    private void insertDialerSearchNewRecord(SQLiteDatabase db, long rawContactId, long dataId,
            String number) {
    	SQLiteStatement mDialerSearchNewRecordInsert;
        ...
        mDialerSearchNewRecordInsert.bindLong(5, 0);
        bindString(mDialerSearchNewRecordInsert, 6, number);
        mDialerSearchNewRecordInsert.executeInsert();
    }
    
    private void insertDialerSearchCalllog(SQLiteDatabase db, int calllogid, 
            String number) {
    	 ContentValues insertNameLookupValues = new ContentValues();
         insertNameLookupValues.put(DialerSearchLookupColumns.CALL_LOG_ID, calllogid);
         insertNameLookupValues.put(DialerSearchLookupColumns.NAME_TYPE,
                 DialerSearchLookupType.NO_NAME_CALL_LOG);
         insertNameLookupValues.put(DialerSearchLookupColumns.DATA_ID, -calllogid);
         insertNameLookupValues.put(DialerSearchLookupColumns.RAW_CONTACT_ID, -calllogid);
         insertNameLookupValues.put(DialerSearchLookupColumns.NORMALIZED_NAME, number);
         insertNameLookupValues.put(DialerSearchLookupColumns.NORMALIZED_NAME_ALTERNATIVE, number);
         db.insert(Tables.DIALER_SEARCH, null, insertNameLookupValues);
    }
}
