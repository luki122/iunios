...
public class Tab implements PictureListener {
    ...
    // Log Tag
    public static final String LOGTAG = "Tab";
    public static final boolean LOGD_ENABLED = com.android.browser.Browser.LOGD_ENABLED;
    ...
    // mMainView
    protected final BrowserDownloadListener mDownloadListener;
    // Listener used to know when we move forward or back in the history list.
    protected final WebBackForwardListClient mWebBackForwardListClient;
    ...
    protected BrowserSettings mSettings;
    ...
    private String mLastSucUrl;

    /**
     * See {@link #clearBackStackWhenItemAdded(String)}.
     */
    public Pattern mClearHistoryUrlPattern;

    private static synchronized Bitmap getDefaultFavicon(Context context) {
        if (sDefaultFavicon == null) {
            sDefaultFavicon = BitmapFactory.decodeResource(
                    context.getResources(), R.drawable.default_icon_title);
        }
    ...
    private void showError(ErrorDialog errDialog) {
        if (mInForeground) {
            AuroraAlertDialog d = new AuroraAlertDialog.Builder(mContext)
    ...
    public void setShowError(WebView view){
    	  ErrorView errorView = getErrorView(true);
          errorView.setErrorMessage(isNetCon,view);
          errorView.changeStatueOfErrorView(errorcode);
          mWebViewController.setShouldShowErrorView(isError());
      }
    ...
    private final WebViewClient mWebViewClient = new WebViewClient() {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            ...
            if ("file:///android_asset/index.html".equals(url) && mErrorView != null) {
                mWebViewController.setShouldShowErrorView(false);
            }
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            mWebViewController.changeStatueOfWebToolsButton();
        }
        ...
        public void onReceivedError(WebView view, int errorCode,
        	...
        	isError = true;
        	Log.i("browser", "onReceivedError--------isError:" + isError);
        	isNetCon=isNetwork(mContext);
        	errorcode = errorCode;
        	Log.i("xiexiujie11", "_______onReceivedError_________________"+errorCode);
        	Log.i("xiexiujie11", "_______onReceivedError_________________"+description);
        	Log.i("xiexiujie11", "_______onReceivedError_________________"+WebViewClient.ERROR_HOST_LOOKUP+WebViewClient.ERROR_CONNECT+WebViewClient.ERROR_BAD_URL+WebViewClient.ERROR_UNSUPPORTED_SCHEME+ WebViewClient.ERROR_FILE);
        	Log.i("xiexiujie11", "_______onReceivedError_2________________"+ErrorStrings.getString(errorCode, view.getContext())+isNetwork(mContext));
        	if (mInForeground) {
                ErrorView errorView = getErrorView(true);
                errorView.setErrorMessage(isNetwork(mContext),view);
                errorView.changeStatueOfErrorView(errorCode);
                mWebViewController.setShouldShowErrorView(true);
            }
            if (errorCode != WebViewClient.ERROR_HOST_LOOKUP &&
                ...
                String startstr = "net::ERR_";
            if(description.startsWith(startstr))
                description = ErrorStrings.getString(errorCode, view.getContext());
...
//            }
        ...
        public void onFormResubmission(WebView view, final Message dontResend,
            ...
            mResend = resend;
            new AuroraAlertDialog.Builder(mContext).setTitle(
        ...
        public void doUpdateVisitedHistory(WebView view, String url,
                boolean isReload) {
            mWebViewController.doUpdateVisitedHistory(Tab.this, isReload, url);
        }
    ...
    private final WebChromeClient mWebChromeClient = new WebChromeClient() {
        ...
        public boolean onCreateWindow(WebView view, final boolean dialog,
            ...
            final AuroraAlertDialog.OnClickListener allowListener =
                    new AuroraAlertDialog.OnClickListener() {
            ...
            final AuroraAlertDialog.OnClickListener blockListener =
                    new AuroraAlertDialog.OnClickListener() {
        ...
        public void onProgressChanged(WebView view, int newProgress) {
        	Log.i("browser", "progress:" + newProgress);
        	if(newProgress == 100){
        		if(!isError) {
        			isError = !isNetwork(mContext);
        		}
//        		Log.i("browser", "去掉错误页面?" + (!isError && !mLoadStopped));
        		Log.i("browser", "isError--" + isError);
        		Log.i("browser", "mLoadStopped--" + mLoadStopped);
        		if(!isError && !mLoadStopped){
        ...
        public void onReceivedTitle(WebView view, final String title) {
        	Log.i("xiexiujie11", "_____onReceivedTitle_________________"+isError);
...
//                getGeolocationPermissionsPrompt().show(origin, callback);
            	...
            	new AuroraAlertDialog.Builder(mContext).setTitle("获取地理位置信息")
            		.setMessage(String.format(
            				 mContext.getResources().getString(R.string.geolocation_permissions_prompt_message),
                            ("http".equals(uri.getScheme()) ?  origin.substring(7) : origin)))
            		.setNegativeButton(mContext.getResources().getString(R.string.geolocation_permissions_prompt_dont_share), new OnClickListener() {
            		...
            		.setPositiveButton(mContext.getResources().getString(R.string.allow), new OnClickListener() {
        ...
        public boolean onConsoleMessage(ConsoleMessage consoleMessage) {
        	Log.i("xiexiujie11", "_______onConsoleMessage____________");
            if (mInForeground) {
    ...
    // Construct a new tab
    protected Tab(WebViewController wvcontroller, WebView w) {
        this(wvcontroller, w, null);
    }

    protected Tab(WebViewController wvcontroller, Bundle state) {
        this(wvcontroller, null, state);
    }

    protected Tab(WebViewController wvcontroller, WebView w, Bundle state) {
        mWebViewController = wvcontroller;
        mContext = mWebViewController.getContext();
        mSettings = BrowserSettings.getInstance();
        mDataController = DataController.getInstance(mContext, AuroraDataController.class.getName());
        ...
        mCaptureWidth = getCaptureWidth();
        mCaptureHeight = getCaptureHeight();
    ...
    /**
     * 
     * Vulcan created this method in 2015年4月1日 下午2:03:50 .
     * @return
     */
    ...
    /**
     * 
     * Vulcan created this method in 2015年4月1日 下午2:03:47 .
     * @return
     */
    ...
    ///-------------add by xiexiujie
    protected ErrorView getErrorView(boolean createIfNecessary) {
        if (createIfNecessary && mErrorView == null) {
    ...
    protected void restoreState(Bundle b) {
        ...
        synchronized (Tab.this) {
            if (mCapture != null) {
                DataController.getInstance(mContext, AuroraDataController.class.getName()).loadThumbnail(this);
            }
        }
    }
    ...
    public void loadUrl(String url, Map<String, String> headers) {
        if (mMainView != null) {
            ...
            isError = false;
            mLoadStopped = false;
            Log.i("browser", "loadUrl-----------------------isError = false");
            mInPageLoad = true;
    ...
    protected void capture() {
        ...
        if(getUrl().equals("about:blank")) {
        	isError = false;
        }else if(!mWebViewController.getErrorViewState()) {
        	isError = false;
        }
    	if(!isError) {
			...
			//c.drawRect(0, 0, 1, mCapture.getHeight(), sAlphaPaint);/*left side*/
			//c.drawRect(mCapture.getWidth() - 1, 0, mCapture.getWidth(),mCapture.getHeight(), sAlphaPaint);/*right side*/
			//c.drawRect(0, 0, mCapture.getWidth(), 1, sAlphaPaint);/*top side*/
			//c.drawRect(0, mCapture.getHeight() - 1, mCapture.getWidth(),mCapture.getHeight(), sAlphaPaint);/*bottom side*/
			c.setBitmap(null);
    	}else {
    		mCapture = mWebViewController.getErrorBitmap(this, mCaptureWidth, mCaptureHeight, mMainView.getWidth());
    	}
    ...
    protected void persistThumbnail() {
        DataController.getInstance(mContext, AuroraDataController.class.getName()).saveThumbnail(this);
    }

    protected void deleteThumbnail() {
        DataController.getInstance(mContext, AuroraDataController.class.getName()).deleteThumbnail(this);
    }
    ...
    void updateCaptureFromBlob(byte[] blob) {
        synchronized (Tab.this) {
            ...
            try {
            	//prevent the exception of "Buffer not large enough for pixels"
            	//Vulcan Yang. 2015-3-16
            	buffer.rewind();
	...
	public String getmLastSucUrl() {
		return mLastSucUrl;
	}

	public void setmLastSucUrl(String mLastSucUrl) {
		this.mLastSucUrl = mLastSucUrl;
	}

	public void setmLoadStopped(boolean mLoadStopped) {
		this.mLoadStopped = mLoadStopped;
	}
