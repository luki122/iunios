...
public class BrowserProvider2 extends SQLiteContentProvider {
    ...
    static final String TABLE_IUNI_FLAG = "iuniflag";
    ...
    final class DatabaseHelper extends SQLiteOpenHelper {
        ...
        static final int DATABASE_VERSION = 35; //33 our own first version
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_IMAGES + " (" +
                    ...
                    ");");
            db.execSQL("CREATE INDEX IF NOT EXISTS imagesUrlIndex ON " + TABLE_IMAGES +
                    "(" + Images.URL + ")");

            db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_SEARCHES + " (" +
            ...
            db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_IUNI_FLAG + "(id integer)");
        ...
        public void onDowngrade(SQLiteDatabase db, int oldVersion,
        		int newVersion) {
        	Log.i("browser", "onDowngrade,oldVersion:" + oldVersion + "  newVersion:" + newVersion);
        	dropTables(db);
        	onCreate(db);
        }
        ...
        /**
         * 
         * Vulcan created this method in 2015年4月1日 下午6:18:51 .
         * @param db
         * @param oldVersion
         * @param newVersion
         */
        private void doChainUpgrade(SQLiteDatabase db,  int oldVersion, int newVersion) {
            //Vulcan Yang
            //2015-4-1
            //database history:
            //33 first version
            //34 second version,add some field to saving input record.
            //35 third version, add field to saving thumbnail file path.
            ...
            //first version: 33
            //second version: 34
            //in the second version, add some field to saving input record.
			if (currentVersion == 33) {
				// do something to upgrade database from 33 to 34
				currentVersion = 34;
			}
           
            
            //third version: 35
            //in the third version, add field to saving thumbnail file path.
            if(oldVersion == 34) {
            	currentVersion = 35;
            	// do something to upgrade database from 34 to 35
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        	Log.i("browser", "onUpgrade,oldVersion:" + oldVersion + "  newVersion:" + newVersion);
        	Cursor cursor = db.rawQuery("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='" + TABLE_IUNI_FLAG + "'", null);
        	...
        	//versions older than 34 are so stupid, we don't care!!!
        	if(oldVersion >= 34) {
        		doChainUpgrade(db, oldVersion, newVersion);
        		return;
        	}
        	
        	if(oldVersion < 34) {
        		db.execSQL("DROP VIEW IF EXISTS " + VIEW_OMNIBOX_SUGGESTIONS);
        		createOmniboxSuggestions(db);
        	}
        ...
        private void createDefaultBookmarks(SQLiteDatabase db) {
            ...
            addDefaultNetwork_address(db, FIXED_ID_ROOT);
        }
        ...
        private void addDefaultNetwork_address(SQLiteDatabase db, long parentId) {
            Resources res = getContext().getResources();
            final CharSequence[] network_address = res.getTextArray(
                    R.array.network_address);
            int size = network_address.length;
            TypedArray preloads = res.obtainTypedArray(R.array.network_address);
            try {
                String parent = Long.toString(parentId);
                String now = Long.toString(System.currentTimeMillis());
                for (int i = 0; i < size; i = i + 2) {
	...
	public void dropTables(SQLiteDatabase db) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_SEARCHES);
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_SETTINGS);
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_IUNI_FLAG);
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_THUMBNAILS);
        db.execSQL("DROP VIEW IF EXISTS " + VIEW_OMNIBOX_SUGGESTIONS);
	}
    ...
    private static final String SQL_CREATE_VIEW_OMNIBOX_SUGGESTIONS =
            ...
            + " AS "
            + "  SELECT _id, url, title, 1 AS bookmark, created AS visits, 0 AS date, folder"
            + "  FROM bookmarks "
            + " WHERE (deleted = 0 AND folder = 0)OR (folder =2 and deleted = 0) OR (folder = 3)  "

            + "  UNION ALL "
            + "  SELECT _id, url, title, 0 AS bookmark, visits, date , 'folder'"
            + "  FROM history "
            + "  WHERE url NOT IN (SELECT url FROM bookmarks"
            + "    WHERE deleted = 0 AND folder = 0) "
            + "  ORDER BY bookmark DESC, visits DESC, date DESC, folder DESC ";
