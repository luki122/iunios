...
public class BookmarkExpandableView extends ExpandableListView
    ...
    class BookmarkAccountAdapter extends BaseExpandableListAdapter {
        ...
        /**
         * 注意每次调用这个函数:1.如果是list模式,只能添加一行 2.如果是grid模式,只能添加grid的一行 
         */
        @Override
        public View getChildView(int groupPosition, int childPosition,
            ...
            for (int i = 0; i < rowCount; i++) { // 一个一个的添加view,注意:只添加可以看到的view
                View cv = null;
                if (row.getChildCount() > i) {
                    cv = row.getChildAt(i); //重用机制
                }
        ...
        public void measureChildren(int viewWidth) {
            ...
            int rowPadding = (viewWidth - (rowCount * mColumnWidth)) / 2; //设置gridview的padding，使其水平居中
            boolean notify = rowCount != mRowCount || rowPadding != mRowPadding; //是否发生了变化需要刷新, 会进行刷新view的操作。即从adapter getView
            mRowCount = rowCount;
