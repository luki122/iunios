...
public class StackViewLayoutAlgorithm {
    ...
    private static final float sDefaultStackWidthPaddingPct = 0.04444f;
    ...
    HashMap<View, Float> mTaskProgressMap = new HashMap<View, Float>();
    ...
    int mStackTopPaddingPx;
    int mViewTranslationZMinPx;
    int mViewTranslationZMaxPx;

    public StackViewLayoutAlgorithm(Context context) {
        mStackTopPaddingPx = StackViewUtil.dip2px(context, 16);
        mViewTranslationZMinPx = StackViewUtil.dip2px(context, 20);
        mViewTranslationZMaxPx = StackViewUtil.dip2px(context, 80);
    ...
    public void computeRects(int windowWidth, int windowHeight) {
        ...
        mStackRect.set(mViewRect);
        mStackVisibleRect.set(mViewRect);

        int widthPadding = (int) (sDefaultStackWidthPaddingPct * mStackRect.width());
        int heightPadding = mStackTopPaddingPx;
    ...
    void computeMinMaxScroll(List<View> viewList) {
...
//        float pAtSecondFrontMostCardTop = pAtBackMostCardTop;
        ...
        for (int i = 0; i < taskCount; i++) {
            ...
            mTaskProgressMap.put(view, pAtFrontMostCardTop);
...
//                pAtFrontMostCardTop += pPeek;
        ...
        mMinScrollP = viewList.size() == 1 ? Math.max(mMaxScrollP, 0f) : 0f;
        mInitialScrollP = Math.max(0, pAtFrontMostCardTop - 0.825f);
    }

    /** Update/get the transform */
    public StackViewTransform getStackTransform(View view, float stackScroll, StackViewTransform transformOut,
            StackViewTransform prevTransform) {
        // Return early if we have an invalid index
        if (view == null || !mTaskProgressMap.containsKey(view)) {
            transformOut.reset();
            return transformOut;
        }
        return getStackTransform(mTaskProgressMap.get(view), stackScroll, transformOut, prevTransform);
    }

    /** Update/get the transform */
    public StackViewTransform getStackTransform(float taskProgress, float stackScroll, StackViewTransform transformOut, StackViewTransform prevTransform) {
        ...
        int minZ = mViewTranslationZMinPx;
        int maxZ = mViewTranslationZMaxPx;
    ...
    private static void scaleRectAboutCenter(Rect r, float scale) {
        if (scale != 1.0f) {
            int cx = r.centerX();
            int cy = r.centerY();
            r.offset(-cx, -cy);
     ...
     */
    float getStackScrollForTask(View v) {
        return mTaskProgressMap.get(v);
    }
